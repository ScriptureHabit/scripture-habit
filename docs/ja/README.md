...
- **[チャット & ダッシュボード同期](feature-chat-dashboard.md)**
  ...
  - データと UI の分離。
  - 未読ステータスの同期方法。
- **[グループチャット（GroupChat）の設計と実装](groupchat-construction-guide.md)**
  - グループチャット機能の構成と概要。
  - 4系統のContext分離設計、カスタム状態管理エンジン、18種類の機能別フック、サブコンポーネント、および11種類のモーダル管理システムを解説。
- **[ノート作成・編集モーダル（NewNote）の設計と実装](newnote-construction-guide.md)**
  - ノート作成・編集モーダルの構成と概要。
  - フォームステート管理、URLメタデータ自動抽出、AI振り返り質問生成、ランダム聖句提案、共有範囲選択、投稿・ストリーク更新処理を解説。
- **[ダッシュボード ＆ マイノート（Dashboard / MyNotes）の設計と実装](dashboard-mynotes-construction-guide.md)**
  - 個人向けメイン画面ハブ（ダッシュボード、マイノート一覧、ノートカード）の構成と概要。
  - 通読ストリークカレンダー、習慣ペース判定アルゴリズム、ノート検索トークンエンジン、週次AI振り返り生成、およびGospel Libraryディープリンク構造を解説。
...
- **[Firestore トランザクション & カウンターサービス設計](firestore-transactions-counters.md)**
  - トランザクション時の動的な「書込前の読込（read-before-write）」順序制御、アトミックな複数ドキュメント更新、および読み取りコスト監査。
