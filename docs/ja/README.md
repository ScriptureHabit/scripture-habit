...
## User Experienceの心理学的考察 (UX Psychology & Retention Insights)
- **[マイルストーン達成 & リテンション心理学](logic-milestone-retention.md)**
  - 連続ストリークが途切れたときの挫折（損失回避や「どうにでもなれ効果」）を防ぐための合計日数モデルへの移行。
  - 10日および25日刻みのマイルストーン設定と、記念画像（html-to-image）による達成感の可視化。
- **[AI振り返りレターの心理学的効用とリテンション](ux-ai-reflection-letters.md)**
  - ユーザーヒアリングから判明した「AI振り返りレターを受け取ることが継続の大きな動機になっている」という発見。
  - 大人が日常で褒められる機会の少なさや人間関係でのフィードバックの難しさと、AIが提供する安心感・肯定的な振り返りの役割。
- **[少人数グループ（最大5人）とピア・アカウンタビリティの心理学](ux-small-groups-and-peer-accountability.md)**
  - グループ上限を5人に制限している理由（社会的手抜きや傍観者効果の防止、親密な関係を保てる規模）。
  - 親友や恋人など信頼関係のあるグループが続きやすい理由と、見知らぬ人同士のグループにおける課題・サポート機能。
