...
- **多層キャッシュによる読み取りコスト削減**:
  - バンドルAPIは**2段階のキャッシュ戦略**を採用しています。まず、サーバーの**インメモリキャッシュ（`bundleCache`）に120秒間**バンドルを保持します。同じグループへの後続リクエストはFirestoreへアクセスせずにキャッシュから即座に返されます。さらに、**Vercel Edge CDNに60秒間**（`s-maxage=60, stale-while-revalidate=120`）キャッシュされ、地理的に近いサーバーから超高速で配信されます。
  - バンドル対象の集計ドキュメント（`messages_latest/latest`）が存在しない新規グループの場合は、`/messages`サブコレクションから最新25件を直接クエリする**フォールバック処理**が実行されます。同時にバックグラウンドで自己修復（`reconcileLatestMessages`）が走り、次回からはバンドル経由で取得できるよう自動的に集計ドキュメントを生成します。
