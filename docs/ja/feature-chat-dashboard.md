...
### デルタ処理
同期エンジン（`src/components/groupchat/hooks/core/`内）は、変更または追加されたドキュメントのみを送信するFirestoreの機能を利用しています。
- **楽観的状態（Optimistic State）**: ユーザーがメッセージを読んだとき、UIはローカルの状態を即座に更新し、バックグラウンドでAPIの更新処理を行います。
- **スナップショットのマージ（Snapshot Merging）**: 新しいメッセージがローカルリストに追加されます。Firestoreの`doc.id`に基づいた安定したReactの`key`プロパティを使用することで、ディープツリーの再レンダリングを最小限に抑えます。
