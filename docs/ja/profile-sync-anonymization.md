...
### 3. メッセージ検索トークンの同期
グループチャット内で共有されたメッセージには、タイトル、本のアドバイス、コメント、話者（説教者名など）を含む `searchTokens` 配列が含まれています。ユーザーがプロフィール情報を更新した際、チャット内のメッセージの送信者表示名（`senderNickname`）や検索用メタデータが適切に伝播され、グループ検索における表示と機能の一貫性が維持されます。
