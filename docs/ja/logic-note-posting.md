...
## 🎉 累積学習日数マイルストーン ＆ ダッシュボード指標の移行

ユーザーの心理的負荷を軽減し、ストリーク（連続日数）の途切れによるモチベーション低下（1日休んだだけでリセットされる落胆）を防ぐため、アプリは**累積学習日数（daysStudiedCount）**をベースにした祝福モデルへと完全移行しました：

1. **ダッシュボードとプロフィールの指標表示**:
   ダッシュボードのメインカードやプロフィールの表示において、従来の「連続達成日数（`streakCount`）」に代わり、ユーザーがこれまでに学習した**「累積日数（`daysStudiedCount`）」**を最も目立つ指標として表示するように変更されました。
2. **マスコットのお祝いメッセージ**:
   ノート投稿時、マスコットキャラクターの吹き出しはユーザーの累積日数をもとに「すごい！累積{streak}日達成！」といった温かいお祝いメッセージを動的に表示します。
3. **グループチャットへのシステム告知の最適化**:
   すべての日常的な投稿をシステムメッセージでお知らせする（チャットの乱雑化や他者との比較ストレスの原因となる）のをやめ、ユーザーが以下の重要な**累積学習マイルストーン**を達成した記念日にのみ、グループチャット全体にお祝いメッセージを自動投稿するようになりました。
   - **固定マイルストーン**: **3日、7日、10日、21日、30日、50日、100日**
   - **定期的な継続マイルストーン**: 100日以降は **50日ごと**（150日、200日、250日...）
