...
*   **症状**: Android エミュレータまたは実機において、ライブリロードでの Vite サーバーの読み込み（`npx cap run android --livereload`）に失敗する、あるいはローカルの Express バックエンド（ポート 5000）へのアクセスに失敗する。
...
*   **解決方法**:
    ...
    4.  **エミュレータの代替手段**: Android エミュレータは、特別な IP `10.0.2.2` を使用してホストマシンにアクセスできます。たとえば、エミュレータ上で実行している場合は `http://10.0.2.2:5000/api` が機能します。
