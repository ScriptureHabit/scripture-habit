...
### ③ ユーザーの感情・トーンに応じた4つの動的レンズ適応
ノートの感情や悩みをAIが分析し、その時最も心に届くレンズを自律選択します：
- **レンズ1: 知的な気づき** *(知的好奇心・疑問)*: 古代の文化的背景や言葉の原義から目から鱗の知見を共有。
- **レンズ2: 詩的な逆説** *(試練・悲しみ・祈り)*: 闇と光の対比、弱さに宿る恵みを美しい詩的表現で包み込む。
- **レンズ3: 人間味ある自虐風刺** *(日常の失敗・空回り)*: 登場人物自身の失敗談（居眠り、弓折れ、逃亡等）でクスッと笑わせる。
- **レンズ4: 温かい解放** *(疲労・自己嫌悪・完璧主義)*: 完璧主義を解きほぐし、今日の一歩を全肯定する安心感を届ける。
- **多面的なエピソードの切り分け**: 人物をステレオタイプ化せず、レンズに合わせて生涯の特定の局面（強さ・弱さ・葛藤）にフォーカス（例: ニーファイの造船技術 vs 詩篇の涙 vs 弓折れパニック vs 暗闇の手探り）。

### ④ 手紙の構成と2段構成（ジョークと霊的感動のパート分け）
- **冒頭**: `「${userName}さんへ、わたし、AIは[人物名]になりきってあなたの最新の2つのノートを読ませていただきました。」`（AIの明示）
...
「わたしに従ってきなさい」の各日の聖句に対し、教会の学者・指導者のエッセンスを融合した**マルチレンズ・ブレンド（4つの視点）**により、**全11言語**でデイリーコメントを一括事前生成します：

### ① 4つの動的レンズによるマンネリ防止
単一のトーン（大喜利一辺倒）を避け、聖句の性格に合わせて以下の4視点を動的に切り替えます：
- **レンズ1: 知的な気づき・目から鱗** *(John W. Welch, Hugh Nibley)*: 古代の文化的背景、言葉の原義、構造の反転に着目。
- **レンズ2: 詩的な逆説・美しい格言** *(Neal A. Maxwell, Terryl Givens)*: 人間の限界と神のタイミングの対比、恵みと成長。
- **レンズ3: 人間味ある愛ある風刺・ツッコミ** *(Hugh Nibley, Dieter F. Uchtdorf)*: 登場人物のこじらせ具合や現代人の空回りへの温かい共感。
- **レンズ4: 温かい解放・完璧主義の手放し** *(Jeffrey R. Holland, Chieko Okazaki)*: 完璧主義を解きほぐし、今日の一歩を全肯定する安心感。

### ② プロンプト設計と制約ルール
- **英語指示 ＋ 日英Few-Shot**: モデルの推論精度を高めるため指示文は英語で統一し、Few-Shotに日英の模範例を併記することで日本語の翻訳調（「〜を思い出させます」等）を完全排除。
- **厳格な制約**: 改行なしの1文完結、絵文字は一切使用禁止、説教調（「〜しましょう」「〜の象徴です」）の禁止、知恵の言葉の遵守（コーヒー・お茶・お酒等の比喩禁止）。
- **11言語ネイティブローカライズ**: `ja`, `en`, `ko`, `zho`, `es`, `pt`, `vi`, `tl`, `th`, `sw`, `it` 各言語の文化とリズムに最適化。
