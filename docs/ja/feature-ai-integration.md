...
- **前半（アイスブレイク・人間味）**: 登場人物自身の失敗談や自虐（ペテロの居眠り・水没、ニーファイの弓折れ、ヨナの逃亡、アルマの気絶など）や現代あるあるを交え、親しみやすさを演出。
...
「わたしに従ってきなさい」の各日の聖句に対し、グループチャット等で表示されるデイリーコメントを10言語で一括事前生成します：
- **方針**: 教科書的な説教や定型句（「〜の象徴です」等）を排除し、聖典の人物や出来事のリアルな人間味に着目した**切れ味鋭い1行の共感・大喜利・現代あるある**を出力。
- **仕様**: 宿題的な問いかけ（「〜は何ですか？」）や絵文字は一切含めず、一言でクスッと笑えて心に刺さる1行完結型。
- **多言語ローカライズ**: 単純な直訳ではなく、各言語圏のネイティブが自然に共感できるニュアンスにローカライズ。
