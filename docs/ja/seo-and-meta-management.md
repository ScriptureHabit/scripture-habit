...
## 4. ビルド時の事前ローカライズとサーバー側のルーティング（クローラー最適化）

一般的な検索エンジンのクローラーやソーシャルメディアの共有スクレイパー（例: Slack、Twitter、LINEのボットなど）は、クライアント側の JavaScript を実行しないことが多いため、`SEOManager` React コンポーネントによって動的に更新されるメタタグを読み取れない場合があります。この課題を解決するため、**scripture-habit** はビルド時の事前ローカライズ戦略を採用しています。

### ビルド時の事前ローカライズ (`scripts/localize-meta.ts`)
本番ビルドのフロー（`package.json` で定義）において、Vite によるクライアントコンパイルが完了した直後に、[localize-meta.ts](../../scripture-habit/scripts/localize-meta.ts) スクリプトが実行されます：
1. コンパイル済みの生のテンプレート `dist/index.html` を読み込みます。
2. サポートされている各ロケール（例: `ja`、`es`、`pt`、`zho`）について、`src/locales/*` 下の言語ファイルからタイトルと説明の文字列を直接解析します。
3. 生の HTML の head タグ内にあるフォールバック用のタイトルとメタ説明プロパティを置き換え、言語ごとの静的 HTML テンプレート（例: `dist/index-ja.html`、`dist/index-es.html`）を生成します。

### Vercel サーバーのリライト (`vercel.json`)
クローラーやユーザーに対して、事前にローカライズされたテンプレートを動的に提供するため、[vercel.json](../../scripture-habit/vercel.json) の Vercel ルーティングルールによって、ローカライズされたパスプレフィックスを持つすべての受信リクエストを書き換えます（リライト）：
* `/ja/:path*` は `/index-ja.html` にリライトされます。
* `/es/:path*` は `/index-es.html` にリライトされます。
* プレフィックスのない英語パスや通常の静的ファイルは、`/index.html`（デフォルトの英語テンプレート）にフォールバックします。

このハイブリッドアプローチにより、ソーシャルスクレイパーボットは最初の生の HTML ペイロードでローカライズされたタイトルや OGP 情報を即座に受け取ることができ、その後はクライアント側の SPA 内で `SEOManager` がページ遷移時の動的なメタデータ同期をシームレスに行うことができます。
