...
*   **クライアント側でのエラー化**: グループの定員制限（500人制限）を検証して参加する処理や、`NoteService` による学習継続判定など、データベースの整合性（トランザクション）が必要な処理は**オフライン状態では動作しません**。
*   **安全なUIガード**: オフライン中にトランザクションが必要な操作を行うと、API通信レイヤーで即座にエラーとなり、画面上に *「ノートの保存やグループへの参加にはインターネット接続が必要です」* とトーストを表示して処理をブロックします。これにより、オフラインのモックデータによる分散カウンタシャードの破壊や集計の不整合を完全に防ぎます。
