...
- **AI ふり返りレター（手紙箱）**: ノート内容をAIが読み取り、聖典の登場人物や総大会の講話のエピソードを交えた心温まるフィードバックレターを届けてくれます。
- **開発者からのウェルカムレター**: 新規登録時に、開発者からの温かい歓迎メッセージが手紙箱に届きます（永久保存）。
- **リアルタイム作成可能バッジ**: 2件以上のノートが溜まり、手紙が作成可能になるとサイドバーのアイコンがピンク色に脈動し、通知音でお知らせします。
- **Firestore Native TTL（30日自動削除）**: 過去のレターは30日後にFirestoreのTTL機能で自動削除され、プライバシーに配慮した軽量な設計になっています。
