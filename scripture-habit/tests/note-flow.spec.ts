...
test.describe('Core Note Flow', () => {
  test('should allow a user to take and then edit a note', async ({ authenticatedPage }) => {
    ...
    const testId = `${Date.now().toString().slice(-6)}-${Math.random().toString(36).substring(2, 6)}`;
    ...
    // Fill the form using element-scoped actions instead of page.keyboard
    const categoryInput = page.getByTestId('new-note-category').locator('input').first();
    await categoryInput.fill('Book of Mormon');
    ...
    // Verify real-time database listener sync WITHOUT page reload workaround
    ...
    // Click 'Edit' inside the specific modal context to avoid selector conflicts
    const detailModal = page.locator('.ModalOverlay');
