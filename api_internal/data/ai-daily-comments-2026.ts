export interface AiDailyCommentData {
    date: string; // YYYY-MM-DD
    scripture: Record<string, string>;
    chapter: Record<string, string>;
    comment: Record<string, string>;
}

export const AI_DAILY_COMMENTS: Record<string, AiDailyCommentData> = {
    "2025-12-29": {
        date: "2025-12-29",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 6:63",
          "en": "Moses 6:63",
          "ko": "Moses 6:63",
          "zho": "モーセ書 6:63",
          "es": "Moses 6:63",
          "pt": "Moses 6:63",
          "vi": "Moses 6:63",
          "tl": "Moses 6:63",
          "th": "Moses 6:63",
          "sw": "Moses 6:63",
          "it": "Mosè 6:63"
        },
        comment: {
          "ja": "万物が神の証しだと言われても、日常の忙しさに追われていると目の前の真珠さえただの小石に見えてしまうのが人間というもの。",
          "en": "Being told everything is a witness of God is fine, but when life gets busy, we often mistake actual pearls for gravel.",
          "ko": "만물이 하나님의 증거라지만, 당장 눈앞의 일상에 치이다 보면 진주도 그저 흔한 돌멩이처럼 보이기 일쑤다.",
          "zho": "即便萬物都在訴說神的存在，但在忙碌的日常壓力下，我們往往還是會把珍珠錯看成路邊的碎石。",
          "es": "Nos dicen que todo es un testimonio de Dios, pero en el caos diario, a veces confundimos las perlas con simples piedras.",
          "pt": "Dizem que tudo é um testemunho de Deus, mas na correria do dia a dia, muitas vezes confundimos pérolas com pedregulhos.",
          "vi": "Dẫu biết vạn vật đều làm chứng về Chúa, nhưng khi bị cuộc sống cuốn đi, chúng ta lại thường nhầm lẫn những viên ngọc quý với sỏi đá.",
          "tl": "Sinasabing ang lahat ng bagay ay saksi sa Diyos, pero sa dami ng iniisip sa araw-araw, madalas nating mapagkamalang batong-bato ang mga perlas.",
          "th": "แม้จะบอกว่าทุกสิ่งคือพยานถึงพระเจ้า แต่ในวันที่ชีวิตวุ่นวาย เราก็มักมองข้ามไข่มุกแล้วเห็นเป็นแค่ก้อนกรวดอยู่ดี",
          "sw": "Tunaambiwa kila kitu ni ushahidi wa Mungu, lakini katika harakati za maisha, mara nyingi tunachanganya lulu na mawe ya kawaida.",
          "it": "Tra le mille cose da fare nella lista della spesa, finiamo sempre per calpestare le vere perle pensando che siano solo sassolini."
        }
    },
    "2025-12-30": {
        date: "2025-12-30",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 12:3-5",
          "en": "Exodus 12:3-5",
          "ko": "Exodus 12:3-5",
          "zho": "出エジプト記 12:3-5",
          "es": "Exodus 12:3-5",
          "pt": "Exodus 12:3-5",
          "vi": "Exodus 12:3-5",
          "tl": "Exodus 12:3-5",
          "th": "Exodus 12:3-5",
          "sw": "Exodus 12:3-5",
          "it": "Esodo 12:3-5"
        },
        comment: {
          "ja": "傷のない子羊を選別する厳格なルールを前に、自分の不完全さを棚に上げて完璧主義を演じる人間の緊張感が伝わってくる。",
          "en": "The strict requirement for a spotless lamb really highlights how we love to obsess over outer perfection while ignoring our own inner mess.",
          "ko": "흠 없는 어린양을 골라야 한다는 엄격한 규칙 앞에서, 정작 자신은 흠투성이면서 겉모습만 완벽해지려는 인간의 강박이 느껴진다.",
          "zho": "面對挑選無殘疾羔羊的嚴格規定，我們總是不自覺地忽略自己內心的混亂，卻對外在的完美斤斤計較。",
          "es": "Ante la exigencia del cordero sin mancha, uno no puede evitar notar cómo nos obsesionamos con la perfección externa mientras ignoramos nuestros propios defectos.",
          "pt": "Diante da regra rigorosa do cordeiro sem defeito, fica evidente como nos preocupamos com a perfeição externa enquanto ignoramos nossas próprias falhas.",
          "vi": "Trước quy định khắt khe về con chiên không tì vết, chúng ta thường mải mê theo đuổi sự hoàn hảo bên ngoài mà quên mất sự lộn xộn bên trong mình.",
          "tl": "Sa higpit ng panuntunan tungkol sa korderong walang kapintasan, ramdam ko ang pagiging mapagkunwari ng tao na gustong magmukhang perpekto kahit marami ring mali sa sarili.",
          "th": "กฎเรื่องลูกแกะที่ไร้ตำหนิทำให้เห็นชัดเลยว่า มนุษย์เราชอบโฟกัสกับความสมบูรณ์แบบภายนอกทั้งที่ข้างในตัวเองก็ไม่ได้ต่างกัน",
          "sw": "Sheria kali kuhusu mwana-kondoo asiye na dosari inatukumbusha jinsi tunavyopenda kuonekana wakamilifu kwa nje huku tukipuuza dosari zetu za ndani.",
          "it": "Ci scandalizziamo per i dettagli perfetti mentre in casa nostra regna il caos più totale, proprio un classico."
        }
    },
    "2025-12-31": {
        date: "2025-12-31",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "民数記 21:4-10",
          "en": "Numbers 21:4-10",
          "ko": "Numbers 21:4-10",
          "zho": "民数記 21:4-10",
          "es": "Numbers 21:4-10",
          "pt": "Numbers 21:4-10",
          "vi": "Numbers 21:4-10",
          "tl": "Numbers 21:4-10",
          "th": "Numbers 21:4-10",
          "sw": "Numbers 21:4-10",
          "it": "Numeri 21:4-10"
        },
        comment: {
          "ja": "文句を言いながら歩き続けたイスラエルの民、極限状態の空腹と疲労で何でもかんでも批判したくなる気持ちは正直よくわかる。",
          "en": "The Israelites complaining about everything while on the road is just a classic case of hangry people venting their frustrations on the wrong target.",
          "ko": "배고프고 힘들 때 만사를 불평하는 이스라엘 백성의 모습에서 퇴근길 지하철에 탄 내 모습을 본 것 같다.",
          "zho": "以色列人在曠野抱怨不斷，這種餓到極點就看什麼都不順眼的狀態，簡直就是現代人低血糖時的真實寫照。",
          "es": "Quejarse de todo mientras caminas bajo el sol es la reacción humana más predecible cuando el hambre y el cansancio ganan la partida.",
          "pt": "Reclamar de tudo no meio do deserto é a prova de que o ser humano, quando está com fome e cansado, perde a paciência até com o próprio milagre.",
          "vi": "Than vãn về mọi thứ khi đang mệt mỏi trên đường đi là phản ứng rất con người khi chúng ta để cái đói lấn át lý trí.",
          "tl": "Ang pagrereklamo ng mga Israelita habang naglalakad ay patunay na kapag pagod at gutom na ang tao, kahit anong milagro ay magiging dahilan pa rin ng reklamo.",
          "th": "การบ่นไปเดินไปของชาวอิสราเอลในทะเลทรายคือภาพสะท้อนความหงุดหงิดเวลาหิวและเหนื่อยที่ใครๆ ก็เคยเป็น",
          "sw": "Waisraeli kulalamika kila kitu wakiwa safarini ni dhihirisho la jinsi njaa na uchovu vinavyoweza kumfanya mtu akose shukrani hata kwa mambo makubwa.",
          "it": "Camminare sotto il sole con la pancia vuota e brontolare per ogni singola cosa è lo sport nazionale di ogni viaggiatore stanco."
        }
    },
    "2026-01-01": {
        date: "2026-01-01",
        scripture: {
          "ja": "新約聖書",
          "en": "New Testament",
          "ko": "New Testament",
          "zho": "新約聖書",
          "es": "New Testament",
          "pt": "New Testament",
          "vi": "New Testament",
          "tl": "New Testament",
          "th": "New Testament",
          "sw": "New Testament",
          "it": "Nuovo Testamento"
        },
        chapter: {
          "ja": "ヨハネによる福音書 1:4-17",
          "en": "John 1:4-17",
          "ko": "John 1:4-17",
          "zho": "ヨハネによる福音書 1:4-17",
          "es": "John 1:4-17",
          "pt": "John 1:4-17",
          "vi": "John 1:4-17",
          "tl": "John 1:4-17",
          "th": "John 1:4-17",
          "sw": "John 1:4-17",
          "it": "Giovanni 1:4-17"
        },
        comment: {
          "ja": "光が闇を照らしても闇が気づかないという皮肉、どれだけ近くに答えがあっても自分の頑固さでそれを見失うことってよくある。",
          "en": "The light shining in the darkness while the darkness remains oblivious is a sharp reminder of how often we miss the obvious because we are too focused on our own shadows.",
          "ko": "빛이 비치는데도 어둠이 깨닫지 못한다는 건, 사실 정답이 바로 옆에 있어도 자기 고집 때문에 못 보는 우리들의 일상과 다를 바 없다.",
          "zho": "光照進黑暗卻沒被察覺，這就像是答案明明就在眼前，我們卻因為固執而選擇性忽視一樣諷刺。",
          "es": "Que la luz brille y la oscuridad no la comprenda es la historia de nuestra vida: ignoramos la solución obvia solo por no querer cambiar el enfoque.",
          "pt": "A luz brilhar e a escuridão não compreender é o retrato de quando estamos tão focados nos nossos próprios problemas que perdemos de vista o que é óbvio.",
          "vi": "Ánh sáng chiếu vào bóng tối mà bóng tối không hiểu được, giống như việc chúng ta cứ loay hoay tìm kiếm câu trả lời dù nó đang ở ngay trước mắt.",
          "tl": "Ang liwanag na hindi maintindihan ng kadiliman ay parang tayo na nakatingin sa maling direksyon kahit na ang sagot ay nasa harap na natin.",
          "th": "แสงสว่างส่องท่ามกลางความมืดแต่ไม่มีใครเห็น ก็เหมือนกับคำตอบที่วางอยู่ตรงหน้าแต่เรากลับมองไม่เห็นเพราะมัวแต่ยึดติดกับความคิดเดิมๆ",
          "sw": "Nuru kuangaza gizani na giza kutoielewa ni ukumbusho kwamba mara nyingi tunashindwa kuona ukweli uliopo mbele yetu kwa sababu tu ya ukaidi wetu.",
          "it": "Ignorare la soluzione lampante che abbiamo proprio davanti al naso solo perché siamo troppo testardi per cambiare prospettiva."
        }
    },
    "2026-01-02": {
        date: "2026-01-02",
        scripture: {
          "ja": "特別宣言",
          "en": "Special Witness",
          "ko": "Special Witness",
          "zho": "特別宣言",
          "es": "Special Witness",
          "pt": "Special Witness",
          "vi": "Special Witness",
          "tl": "Special Witness",
          "th": "Special Witness",
          "sw": "Special Witness",
          "it": "Testimonianza speciale"
        },
        chapter: {
          "ja": "生けるキリスト",
          "en": "The Living Christ",
          "ko": "The Living Christ",
          "zho": "生けるキリスト",
          "es": "The Living Christ",
          "pt": "The Living Christ",
          "vi": "The Living Christ",
          "tl": "The Living Christ",
          "th": "The Living Christ",
          "sw": "The Living Christ",
          "it": "Il Cristo Vivente"
        },
        comment: {
          "ja": "歴史の教科書に載るような偉大な存在なのに、その正体が誰よりも身近な隣人として描かれているこのギャップがたまらない。",
          "en": "It is funny how the most important figure in history is also described as the most relatable neighbor you could possibly have.",
          "ko": "역사책에나 나올 법한 위대한 존재가 누구보다 친근한 이웃으로 묘사되는 이 간극이 참 매력적이다.",
          "zho": "明明是歷史上最偉大的存在，卻被描述成最親近的鄰居，這種反差感真的很耐人尋味。",
          "es": "Es curioso cómo la figura más importante de la historia se describe al mismo tiempo como el vecino más cercano que uno podría tener.",
          "pt": "É curioso como a figura mais importante da história é descrita ao mesmo tempo como o vizinho mais próximo que alguém poderia ter.",
          "vi": "Thật thú vị khi nhân vật vĩ đại nhất trong lịch sử lại được mô tả như một người hàng xóm gần gũi nhất mà bạn từng biết.",
          "tl": "Nakakatuwa na ang pinakamahalagang pigura sa kasaysayan ay inilalarawan din bilang ang pinakamalapit na kapitbahay na maaari mong magkaroon.",
          "th": "น่าทึ่งที่บุคคลที่สำคัญที่สุดในประวัติศาสตร์กลับถูกบรรยายให้ดูเหมือนเพื่อนบ้านที่ใกล้ชิดที่สุดเท่าที่คุณจะมีได้",
          "sw": "Inafurahisha jinsi mtu muhimu zaidi katika historia anavyoelezewa kama jirani wa karibu zaidi ambaye ungeweza kuwa naye.",
          "it": "Fa sorridere come la persona più importante della storia umana finisca per sembrare semplicemente il vicino di casa perfetto."
        }
    },
    "2026-01-03": {
        date: "2026-01-03",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 7:18-19",
          "en": "Moses 7:18-19",
          "ko": "Moses 7:18-19",
          "zho": "モーセ書 7:18-19",
          "es": "Moses 7:18-19",
          "pt": "Moses 7:18-19",
          "vi": "Moses 7:18-19",
          "tl": "Moses 7:18-19",
          "th": "Moses 7:18-19",
          "sw": "Moses 7:18-19",
          "it": "Mosè 7:18-19"
        },
        comment: {
          "ja": "心も一つ、思いも一つ、貧しい者もいないという理想郷、現代のSNSのギスギスした雰囲気からするとファンタジーすぎて逆に癒やされる。",
          "en": "A place with no poor and everyone in perfect unity sounds so much like a fantasy that it is actually refreshing compared to the chaos of modern social media.",
          "ko": "마음도 하나, 생각도 하나, 가난한 자도 없는 이상향이라니 요즘 SNS의 삭막한 분위기에 비하면 너무나 비현실적이라 오히려 위안이 된다.",
          "zho": "心意相通、沒有窮人的理想國度，對比現代社群媒體的紛擾，簡直像童話故事一樣治癒。",
          "es": "Un lugar sin pobres donde todos están en perfecta unidad suena tan fantástico que resulta refrescante comparado con el caos de las redes sociales actuales.",
          "pt": "Um lugar sem pobres onde todos estão em perfeita unidade soa tão fantástico que chega a ser refrescante comparado ao caos das redes sociais atuais.",
          "vi": "Một nơi không có người nghèo và mọi người đều hòa hợp tuyệt đối nghe thật như mơ, thật dễ chịu khi so sánh với sự hỗn loạn trên mạng xã hội ngày nay.",
          "tl": "Ang isang lugar na walang mahirap at ang lahat ay nagkakaisa ay parang isang panaginip, at nakaka-relax isipin kumpara sa gulo ng social media ngayon.",
          "th": "สถานที่ที่ไม่มีคนจนและทุกคนเป็นหนึ่งเดียวกันฟังดูเหมือนเทพนิยาย ซึ่งช่างน่าสบายใจเมื่อเทียบกับความวุ่นวายบนโซเชียลมีเดียในยุคนี้",
          "sw": "Mahali pasipo na maskini ambapo kila mtu ana umoja kamili inasikika kama hadithi ya kufikirika, jambo ambalo linaburudisha ikilinganishwa na machafuko ya mitandao ya kijamii ya sasa.",
          "it": "Un mondo senza disuguaglianze e in totale armonia sembra così irreale da far quasi sorridere rispetto alla confusione dei social media."
        }
    },
    "2026-01-04": {
        date: "2026-01-04",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "アブラハム書 3:22-28",
          "en": "Abraham 3:22-28",
          "ko": "Abraham 3:22-28",
          "zho": "アブラハム書 3:22-28",
          "es": "Abraham 3:22-28",
          "pt": "Abraham 3:22-28",
          "vi": "Abraham 3:22-28",
          "tl": "Abraham 3:22-28",
          "th": "Abraham 3:22-28",
          "sw": "Abraham 3:22-28",
          "it": "Abramo 3:22-28"
        },
        comment: {
          "ja": "宇宙規模の会議で自分が選ばれたと知った途端に、急に自分の重要性を過信し始めるのが人間の愛すべき性分というものだ。",
          "en": "Finding out you were chosen in a cosmic council meeting and immediately letting it go to your head is just peak human behavior.",
          "ko": "우주적인 회의에서 자신이 선택받았다는 사실을 알게 되자마자 바로 자의식 과잉에 빠지는 것이야말로 인간의 귀여운 본성이다.",
          "zho": "得知自己在宇宙會議中被選中後，立刻開始膨脹的自我感覺，這簡直就是人類最真實的本性。",
          "es": "Saber que fuiste elegido en un consejo cósmico y creerte inmediatamente el protagonista de la historia es la respuesta humana más típica.",
          "pt": "Saber que você foi escolhido em um conselho cósmico e imediatamente achar que é o centro do universo é a reação humana mais natural possível.",
          "vi": "Biết mình được chọn trong một hội nghị vũ trụ rồi ngay lập tức tự cao tự đại chính là bản tính đáng yêu của con người.",
          "tl": "Ang malaman na pinili ka sa isang cosmic council at agad na lumaki ang ulo ay ang pinaka-human na reaksyon na posible.",
          "th": "พอรู้ว่าตัวเองถูกเลือกในที่ประชุมระดับจักรวาลแล้วเกิดอาการหลงตัวเองขึ้นมาทันที นี่แหละนิสัยความเป็นมนุษย์ที่แก้ยาก",
          "sw": "Kujua ulichaguliwa katika baraza la mbinguni kisha kuanza kujiona kuwa wewe ni muhimu sana ni hulka ya kibinadamu inayotabirika.",
          "it": "Scoprire di avere un grande potenziale e montarsi la testa all'istante è proprio una tipica debolezza umana."
        }
    },
    "2026-01-05": {
        date: "2026-01-05",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 1:1-11",
          "en": "Moses 1:1-11",
          "ko": "Moses 1:1-11",
          "zho": "モーセ書 1:1-11",
          "es": "Moses 1:1-11",
          "pt": "Moses 1:1-11",
          "vi": "Moses 1:1-11",
          "tl": "Moses 1:1-11",
          "th": "Moses 1:1-11",
          "sw": "Moses 1:1-11",
          "it": "Mosè 1:1-11"
        },
        comment: {
          "ja": "神の威光を前にして腰を抜かしているモーセを見て、自分だけじゃないんだと妙に安心する。",
          "en": "Seeing Moses completely lose his cool in the presence of God is weirdly comforting because it reminds me I'm not the only one who panics.",
          "ko": "하나님의 위엄 앞에서 다리가 풀려버린 모세를 보니 나만 당황하는 게 아니라는 생각에 묘한 안도감이 든다.",
          "zho": "看到摩西在神面前嚇得雙腿發軟，心裡竟然莫名地感到安慰，原來不止我一個人會慌張。",
          "es": "Ver a Moisés perder los nervios ante la presencia de Dios es extrañamente reconfortante porque me recuerda que no soy el único que entra en pánico.",
          "pt": "Ver Moisés perder a compostura na presença de Deus é estranhamente reconfortante, pois me lembra que não sou o único que entra em pânico.",
          "vi": "Nhìn thấy Môi-se mất bình tĩnh trước sự hiện diện của Chúa khiến tôi thấy nhẹ nhõm vì biết mình không phải là người duy nhất hay hoảng loạn.",
          "tl": "Ang makitang mawala sa sarili si Moises sa harap ng Diyos ay nakakapanatag ng loob dahil hindi lang ako ang nagpa-panic.",
          "th": "เห็นโมเสสสติหลุดต่อหน้าพระพักตร์พระเจ้าแล้วรู้สึกสบายใจขึ้นมาแปลกๆ เพราะรู้ว่าเราไม่ได้เป็นคนเดียวที่ทำตัวไม่ถูก",
          "sw": "Kumwona Musa akipoteza utulivu wake mbele za Mungu kunanipa faraja ya ajabu kwa sababu inanikumbusha kuwa si mimi pekee ninayeingiwa na hofu.",
          "it": "Vedere un grande profeta perdere la pazienza e andare in confusione totale ci fa sentire decisamente meno soli nei nostri momenti no."
        }
    },
    "2026-01-06": {
        date: "2026-01-06",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 1:12-23",
          "en": "Moses 1:12-23",
          "ko": "Moses 1:12-23",
          "zho": "モーセ書 1:12-23",
          "es": "Moses 1:12-23",
          "pt": "Moses 1:12-23",
          "vi": "Moses 1:12-23",
          "tl": "Moses 1:12-23",
          "th": "Moses 1:12-23",
          "sw": "Moses 1:12-23",
          "it": "Mosè 1:12-23"
        },
        comment: {
          "ja": "神の栄光を見た直後に「お前はただの人間だ」とマウントをとってくるサタン、自信満々なときほど横から茶々を入れてくる嫌な上司そのもの。",
          "en": "Satan showing up to tell Moses he is just a man right after a divine encounter is classic workplace gaslighting.",
          "ko": "하나님의 영광을 체험하자마자 나타나서 딴지를 거는 사탄은 마치 잘나갈 때마다 굳이 찬물을 끼얹는 주변 사람 같아서 참 얄밉다.",
          "zho": "在剛體驗過神聖榮耀後就跑來潑冷水的撒但，簡直就像是看不得別人好、隨時準備來情緒勒索的討厭鬼。",
          "es": "Satanás apareciendo para recordarle a Moisés que es solo un hombre justo después de una experiencia divina es el equivalente celestial a un comentario envidioso en redes sociales.",
          "pt": "Satanás tentando diminuir Moisés logo após uma experiência gloriosa é o exemplo perfeito de alguém que não suporta ver o sucesso alheio.",
          "vi": "Satan đến đòi làm cha chú ngay sau khi Môi-se vừa trải nghiệm vinh quang của Chúa, đúng là kiểu người hay ghen ăn tức ở điển hình.",
          "tl": "Ang pagsulpot ni Satanas para sabihing tao lang si Moises pagkatapos ng isang banal na karanasan ay parang yung taong ayaw na ayaw makitang masaya ka.",
          "th": "ซาตานที่โผล่มาขัดจังหวะทันทีหลังจากโมเสสเห็นรัศมีของพระเจ้า ก็เหมือนคนที่คอยหาเรื่องมาบั่นทอนกำลังใจเราในวันที่เรากำลังไปได้สวย",
          "sw": "Shetani kujitokeza kumwambia Musa yeye ni mwanadamu tu baada tu ya tukio takatifu ni kama yule mtu anayependa kukukatisha tamaa wakati unapopiga hatua.",
          "it": "Arrivare carichi da un'esperienza speciale e trovare subito qualcuno pronto a sminuirci è la prova che la vita non cambia mai."
        }
    },
    "2026-01-07": {
        date: "2026-01-07",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 1:24-35",
          "en": "Moses 1:24-35",
          "ko": "Moses 1:24-35",
          "zho": "モーセ書 1:24-35",
          "es": "Moses 1:24-35",
          "pt": "Moses 1:24-35",
          "vi": "Moses 1:24-35",
          "tl": "Moses 1:24-35",
          "th": "Moses 1:24-35",
          "sw": "Moses 1:24-35",
          "it": "Mosè 1:24-35"
        },
        comment: {
          "ja": "宇宙の広大さを教えられても「で、結局人間はどうなるの？」と即座に自分中心の質問をしてしまうモーセ、なんだかんだ言って人間らしくて安心する。",
          "en": "Moses being shown the entire universe and immediately asking what it means for him is the most relatable human reaction ever.",
          "ko": "우주의 광활함을 보여주시는 하나님 앞에서 결국 내 존재의 의미부터 묻는 모세의 모습이 너무나 인간적이라 오히려 안심이 된다.",
          "zho": "上帝展示了浩瀚宇宙，摩西卻立刻問這對他個人有什麼意義，這種先顧好自己的反應實在是太真實了。",
          "es": "Que Moisés vea el universo entero y su primera pregunta sea sobre qué significa todo eso para él es la respuesta humana más honesta que existe.",
          "pt": "Moisés ver a vastidão do universo e imediatamente perguntar o que isso significa para ele é a reação mais humana e compreensível de todas.",
          "vi": "Môi-se được xem cả vũ trụ bao la mà câu đầu tiên lại hỏi 'thế còn tôi thì sao', đúng là cái tôi của con người luôn chiếm sóng.",
          "tl": "Ang makita ang buong sansinukob at ang unang itanong ni Moises ay kung ano ang kahulugan nito para sa kanya ay napaka-tao at nakaka-relate.",
          "th": "โมเสสเห็นความกว้างใหญ่ของจักรวาลแล้วถามกลับทันทีว่า 'แล้วมันเกี่ยวกับผมยังไง' เป็นปฏิกิริยาของมนุษย์ที่ซื่อตรงจนน่าเอ็นดู",
          "sw": "Musa kuonyeshwa ulimwengu mzima na kisha kuuliza haraka nini maana yake kwake ni mwitikio wa kibinadamu ambao ni rahisi sana kuuelewa.",
          "it": "Aver davanti l'infinito e chiedere subito cosa c'è in serbo per noi è la mossa più egoista e umana del mondo."
        }
    },
    "2026-01-08": {
        date: "2026-01-08",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 1:36-42",
          "en": "Moses 1:36-42",
          "ko": "Moses 1:36-42",
          "zho": "モーセ書 1:36-42",
          "es": "Moses 1:36-42",
          "pt": "Moses 1:36-42",
          "vi": "Moses 1:36-42",
          "tl": "Moses 1:36-42",
          "th": "Moses 1:36-42",
          "sw": "Moses 1:36-42",
          "it": "Mosè 1:36-42"
        },
        comment: {
          "ja": "宇宙の広さを語る神様を前に、自分の小ささに圧倒されるどころか、その計画の一部に自分を組み込んでくれたことにただただ驚くしかない。",
          "en": "Standing before the vastness of the universe, I am less overwhelmed by the scale and more stunned that I am even part of the plan at all.",
          "ko": "우주의 광대함을 말씀하시는 하나님 앞에서 내가 얼마나 작은 존재인지 깨닫기보다, 그 거대한 계획 속에 나를 포함해주셨다는 사실에 그저 놀라울 뿐이다.",
          "zho": "在上帝讲述宇宙广袤时，我不再纠结于自己的渺小，反而惊讶于自己竟然也是那宏大计划的一部分。",
          "es": "Ante la inmensidad del universo, me asombra menos mi propia pequeñez y más el hecho de que Dios me haya incluido en Su plan.",
          "pt": "Diante da vastidão do universo, fico menos impressionado com a minha pequenez e mais maravilhado por ter sido incluído nesse plano.",
          "vi": "Đứng trước sự bao la của vũ trụ, tôi không còn bận tâm về sự nhỏ bé của mình mà chỉ thấy kinh ngạc vì mình lại là một phần của kế hoạch đó.",
          "tl": "Sa harap ng lawak ng sansinukob, mas nakakamangha na kasama ako sa plano kaysa sa pagiging maliit ko.",
          "th": "เมื่ออยู่ต่อหน้าความกว้างใหญ่ของจักรวาล ผมไม่ได้กังวลเรื่องความเล็กจ้อยของตัวเอง แต่กลับทึ่งที่พระองค์ให้ผมเป็นส่วนหนึ่งของแผนการนี้",
          "sw": "Mbele ya ukubwa wa ulimwengu, nashangaa zaidi kuona kuwa nimejumuishwa katika mpango huo badala ya kuhisi udogo wangu.",
          "it": "Di fronte all'infinito dell'universo, la vera sorpresa non è sentirsi piccoli, ma scoprire di avere comunque un posto riservato nel piano."
        }
    },
    "2026-01-09": {
        date: "2026-01-09",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "アブラハム書 3:1-11",
          "en": "Abraham 3:1-11",
          "ko": "Abraham 3:1-11",
          "zho": "アブラハム書 3:1-11",
          "es": "Abraham 3:1-11",
          "pt": "Abraham 3:1-11",
          "vi": "Abraham 3:1-11",
          "tl": "Abraham 3:1-11",
          "th": "Abraham 3:1-11",
          "sw": "Abraham 3:1-11",
          "it": "Abramo 3:1-11"
        },
        comment: {
          "ja": "星の数や階層を計算して自分のランクを気にするアブラハムの姿は、現代のSNSでフォロワー数や評価を気にしてしまう私たちの痛いほどリアルな姿そのもの。",
          "en": "Abraham checking the ranks and numbers of stars is basically us obsessing over follower counts and social media metrics.",
          "ko": "별의 수와 서열을 계산하며 자신의 위치를 확인하려는 아브라함의 모습은, SNS에서 팔로워 수와 타인의 평가에 전전긍긍하는 우리의 모습과 소름 돋게 똑같다.",
          "zho": "亚伯拉罕计算星辰和阶层的样子，简直就是我们每天盯着社交媒体关注数和点赞量焦虑的真实写照。",
          "es": "Abraham calculando estrellas y jerarquías es básicamente nosotros revisando seguidores y métricas en redes sociales todo el día.",
          "pt": "Abraão calculando estrelas e hierarquias é basicamente a gente obcecado com número de seguidores e métricas nas redes sociais.",
          "vi": "Việc Abraham đếm các vì sao và tính toán thứ bậc giống hệt như cách chúng ta cứ loay hoay với số lượng người theo dõi trên mạng xã hội mỗi ngày.",
          "tl": "Ang pagbibilang ni Abraham sa mga bituin at pag-uuri sa mga ito ay parang tayo na laging nag-aalala sa dami ng followers at likes sa social media.",
          "th": "การที่อับราฮัมพยายามนับดวงดาวและจัดลำดับชั้นนั้น เหมือนกับพวกเราที่คอยกังวลเรื่องจำนวนผู้ติดตามและยอดไลก์บนโซเชียลมีเดียไม่มีผิด",
          "sw": "Ibrahim akihesabu nyota na vyeo vyake ni kama sisi tunavyohangaika na idadi ya wafuasi na maoni kwenye mitandao ya kijamii kila siku.",
          "it": "Abrahamo che controlla le gerarchie delle stelle dimostra che l'ossessione per i numeri e la visibilità ci accompagna da sempre."
        }
    },
    "2026-01-10": {
        date: "2026-01-10",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "アブラハム書 3:12-20",
          "en": "Abraham 3:12-20",
          "ko": "Abraham 3:12-20",
          "zho": "アブラハム書 3:12-20",
          "es": "Abraham 3:12-20",
          "pt": "Abraham 3:12-20",
          "vi": "Abraham 3:12-20",
          "tl": "Abraham 3:12-20",
          "th": "Abraham 3:12-20",
          "sw": "Abraham 3:12-20",
          "it": "Abramo 3:12-20"
        },
        comment: {
          "ja": "星の数ほどある宇宙の広大さと自分の小ささを突きつけられると、悩みなんてどうでもよくなる不思議な感覚。",
          "en": "Being reminded of the sheer scale of the cosmos is the ultimate reality check for how small my daily worries actually are.",
          "ko": "우주의 광활함 앞에서 내 고민의 크기를 다시 생각해보면 의외로 마음이 한결 가벼워진다.",
          "zho": "面对浩瀚星空时那种渺小感，往往能治愈现代人那些琐碎的焦虑。",
          "es": "Sentirse pequeño ante la inmensidad del universo es, irónicamente, lo que más calma mis preocupaciones diarias.",
          "pt": "Sentir-se pequeno diante da imensidão do universo é, ironicamente, o que mais acalma nossas preocupações diárias.",
          "vi": "Nhìn vào sự bao la của vũ trụ, bỗng thấy những rắc rối thường ngày chẳng đáng là bao.",
          "tl": "Ang pag-unawa sa lawak ng uniberso ay mabisang paalala na hindi naman dapat masyadong dibdibin ang mga problema sa buhay.",
          "th": "การได้เห็นความกว้างใหญ่ของจักรวาลช่วยเตือนสติได้ดีว่าปัญหาของเรามันเล็กน้อยแค่ไหน",
          "sw": "Kukumbushwa jinsi ulimwengu ulivyo mkubwa kunanifanya nitambue kuwa shida zangu za kila siku si kitu kikubwa sana.",
          "it": "Puntare lo sguardo sulla vastità del cosmo è il modo migliore per ridimensionare i piccoli grattacapi della giornata."
        }
    },
    "2026-01-11": {
        date: "2026-01-11",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "アブラハム書 3:21-28",
          "en": "Abraham 3:21-28",
          "ko": "Abraham 3:21-28",
          "zho": "アブラハム書 3:21-28",
          "es": "Abraham 3:21-28",
          "pt": "Abraham 3:21-28",
          "vi": "Abraham 3:21-28",
          "tl": "Abraham 3:21-28",
          "th": "Abraham 3:21-28",
          "sw": "Abraham 3:21-28",
          "it": "Abramo 3:21-28"
        },
        comment: {
          "ja": "ある者は神の計画を掲げ、ある者は自分の正しさを主張して揉める、この世の会議室の光景とあまりに重なる。",
          "en": "Watching the debate over whose plan to follow feels exactly like sitting through a corporate meeting where everyone thinks they have the best idea.",
          "ko": "누구의 계획이 옳은지를 두고 벌어지는 그 치열한 논쟁은 마치 오늘날의 회의실 풍경을 보는 것 같다.",
          "zho": "看着大家为了哪种方案更好而争论不休，这不就是现代职场办公室的日常写照吗。",
          "es": "Ver la discusión sobre qué plan seguir se siente como estar en una reunión de trabajo donde todos creen tener la mejor idea.",
          "pt": "Ver a discussão sobre qual plano seguir parece exatamente uma reunião de trabalho onde todos acham que têm a melhor ideia.",
          "vi": "Cảnh tranh luận xem nên theo kế hoạch nào chẳng khác gì những cuộc họp căng thẳng ở văn phòng ngày nay.",
          "tl": "Ang pagtatalo kung kaninong plano ang dapat sundin ay parang eksena lang sa isang meeting sa opisina na walang gustong magpatalo.",
          "th": "ฉากการถกเถียงเรื่องแผนการที่ควรทำ ดูแล้วเหมือนบรรยากาศในห้องประชุมงานที่ทุกคนต่างคิดว่าไอเดียตัวเองดีที่สุด",
          "sw": "Kuona watu wakibishana kuhusu mpango wa kufuata ni kama kuwa kwenye kikao cha kazi ambapo kila mtu anajiona ana wazo bora zaidi.",
          "it": "Assistere al dibattito su quale piano seguire è identico a partecipare a una riunione di lavoro dove tutti credono di avere l'idea geniale."
        }
    },
    "2026-01-12": {
        date: "2026-01-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 1:1-5",
          "en": "Genesis 1:1-5",
          "ko": "Genesis 1:1-5",
          "zho": "創世記 1:1-5",
          "es": "Genesis 1:1-5",
          "pt": "Genesis 1:1-5",
          "vi": "Genesis 1:1-5",
          "tl": "Genesis 1:1-5",
          "th": "Genesis 1:1-5",
          "sw": "Genesis 1:1-5",
          "it": "Genesi 1:1-5"
        },
        comment: {
          "ja": "無から世界を創り出す神の壮大なプロジェクトの幕開けだけど、最初の仕事が光と闇を分けることだなんて、なんだか大掃除の第一歩を見ているようで親近感がわく。",
          "en": "God kicking off the creation of the universe by just organizing light and dark feels like the ultimate relatable first step of a massive home renovation project.",
          "ko": "세상을 창조하는 거대한 프로젝트의 시작이 고작 빛과 어둠을 나누는 일이라니, 거창한 계획을 세우고 결국은 방 정리부터 시작하는 우리들의 모습과 다를 게 없다.",
          "zho": "神創造宇宙的宏大工程，起手式居然只是把光和暗分開，這簡直就像是我們面對大掃除時那種先從分類開始的既視感。",
          "es": "Dios comenzando la creación del universo simplemente poniendo orden entre la luz y la oscuridad es el recordatorio perfecto de que hasta los grandes proyectos empiezan con una pequeña organización.",
          "pt": "Deus começando a criação do universo apenas separando a luz das trevas é como aquele primeiro passo de organização que a gente dá antes de encarar um projeto gigante.",
          "vi": "Việc Chúa bắt đầu tạo dựng vũ trụ bằng cách phân chia ánh sáng và bóng tối giống như cách chúng ta bắt đầu một dự án lớn bằng việc dọn dẹp ngăn nắp vậy.",
          "tl": "Ang paglikha ng mundo na nagsimula sa paghihiwalay ng liwanag at dilim ay parang pag-aayos ng gamit bago magsimula ng isang malaking proyekto sa bahay.",
          "th": "การที่พระผู้เป็นเจ้าทรงเริ่มสร้างจักรวาลด้วยการแยกแสงสว่างออกจากความมืดนั้น ช่างดูเหมือนการเริ่มต้นจัดบ้านก่อนจะลงมือทำโปรเจกต์ใหญ่ที่เรารู้สึกคุ้นเคยดี",
          "sw": "Mungu kuanza uumbaji kwa kutenganisha nuru na giza ni kama vile hatua ya kwanza ya kupanga vitu kabla ya kuanza mradi mkubwa wa ukarabati nyumbani.",
          "it": "Mettere ordine tra luce e buio come primo passo della creazione assomiglia tanto a quelle pulizie di primavera che sembrano non finire mai."
        }
    },
    "2026-01-13": {
        date: "2026-01-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 1:6-13",
          "en": "Genesis 1:6-13",
          "ko": "Genesis 1:6-13",
          "zho": "創世記 1:6-13",
          "es": "Genesis 1:6-13",
          "pt": "Genesis 1:6-13",
          "vi": "Genesis 1:6-13",
          "tl": "Genesis 1:6-13",
          "th": "Genesis 1:6-13",
          "sw": "Genesis 1:6-13",
          "it": "Genesi 1:6-13"
        },
        comment: {
          "ja": "植物が芽を出した瞬間に神がそれを見てよしとされたけれど、自分の小さな成長に対してもこれくらい自分に甘く評価していいのかもしれない。",
          "en": "God took one look at the plants and immediately said they were good, which is exactly how we should treat our own tiny, awkward attempts at progress.",
          "ko": "식물이 돋아나자마자 좋았더라고 말씀하신 하나님을 보며, 나도 내 작은 성취에 대해 이만큼 관대하게 스스로를 칭찬해줘도 되지 않을까 싶다.",
          "zho": "看到植物剛冒芽就說這一切甚好，或許我們也該學著對自己那些微小的進步多給一點掌聲，而不是總想著還沒完成的部分。",
          "es": "Dios vio las plantas recién brotadas y dijo que estaban bien; quizás deberíamos ser así de amables con nosotros mismos cuando apenas empezamos a crecer en algo.",
          "pt": "Deus viu as plantas brotando e logo disse que estava tudo bem, o que é um ótimo lembrete de que devemos ser mais gentis com nossos próprios começos incertos.",
          "vi": "Chúa thấy cây cối đâm chồi và khen là tốt lành, có lẽ chúng ta cũng nên học cách tự khích lệ bản thân ngay từ những bước tiến nhỏ bé đầu đời.",
          "tl": "Nakita ng Diyos ang mga halaman na sumibol at agad itong tinawag na mabuti, isang paalala na dapat din nating pahalagahan ang ating maliliit na pagsisimula.",
          "th": "พระผู้เป็นเจ้าทรงเห็นพืชพรรณที่เพิ่งงอกเงยแล้วทรงชมว่าดีนักหนา เป็นบทเรียนที่บอกว่าเราควรใจดีกับก้าวแรกที่ยังไม่สมบูรณ์แบบของตัวเองบ้าง",
          "sw": "Mungu aliona mimea ikichipuka na kusema ni njema, labda tunapaswa kujifunza kuwa na huruma kama hiyo kwa hatua zetu ndogo za maendeleo.",
          "it": "Se le prime pianticelle sono state approvate subito, forse dovremmo essere un po' più indulgenti anche con i nostri timidi tentativi di crescita."
        }
    },
    "2026-01-14": {
        date: "2026-01-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 1:14-23",
          "en": "Genesis 1:14-23",
          "ko": "Genesis 1:14-23",
          "zho": "創世記 1:14-23",
          "es": "Genesis 1:14-23",
          "pt": "Genesis 1:14-23",
          "vi": "Genesis 1:14-23",
          "tl": "Genesis 1:14-23",
          "th": "Genesis 1:14-23",
          "sw": "Genesis 1:14-23",
          "it": "Genesi 1:14-23"
        },
        comment: {
          "ja": "何もない暗闇から光や星を次々とデザインしていく創造の過程、もはや壮大なDIYプロジェクトのようで見ているだけで圧倒される。",
          "en": "Watching the creation of light and stars from absolute darkness feels like the ultimate DIY project that makes my weekend to-do list look pathetic.",
          "ko": "아무것도 없는 어둠 속에서 별과 빛을 하나씩 설계해 나가는 창조의 과정은 마치 거대한 DIY 프로젝트를 보는 듯한 경외감을 준다.",
          "zho": "从虚无的黑暗中设计出光与星辰的创造过程，就像一场规模宏大的DIY工程，让人不得不感叹造物主的品味。",
          "es": "Ver cómo se diseña la luz y las estrellas desde la oscuridad total parece un proyecto de bricolaje a escala universal que nos deja a todos pequeños.",
          "pt": "Ver o design da luz e das estrelas surgindo do nada parece um projeto de DIY em escala universal que faz qualquer um se sentir um aprendiz.",
          "vi": "Chứng kiến quá trình kiến tạo ánh sáng và các vì sao từ hư không giống như một dự án DIY vĩ đại khiến mọi kế hoạch của chúng ta trở nên thật nhỏ bé.",
          "tl": "Ang pagdisenyo ng liwanag at mga bituin mula sa kawalan ay parang isang dambuhalang DIY project na nakakabilib sa sinumang makakakita.",
          "th": "การออกแบบแสงสว่างและดวงดาวจากความว่างเปล่าดูเหมือนโปรเจกต์ DIY ระดับจักรวาลที่ทำให้งานของมนุษย์ดูเล็กน้อยไปเลย",
          "sw": "Kuona jinsi mwanga na nyota vilivyobuniwa kutoka kwenye giza totoro ni kama mradi mkubwa wa ujenzi unaoacha kila mtu akishangaa.",
          "it": "Disegnare galassie dal nulla rende la nostra lista di cose da fare per il fine settimana decisamente ridicola."
        }
    },
    "2026-01-15": {
        date: "2026-01-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 1:24-31",
          "en": "Genesis 1:24-31",
          "ko": "Genesis 1:24-31",
          "zho": "創世記 1:24-31",
          "es": "Genesis 1:24-31",
          "pt": "Genesis 1:24-31",
          "vi": "Genesis 1:24-31",
          "tl": "Genesis 1:24-31",
          "th": "Genesis 1:24-31",
          "sw": "Genesis 1:24-31",
          "it": "Genesi 1:24-31"
        },
        comment: {
          "ja": "最後に人間を創造して「よし、これで完璧だ」と満足げな様子、その後の人類の散らかりっぷりを知ったら神様も苦笑いしそうだ。",
          "en": "God finishing creation with humans and calling it very good is a sweet moment, though he probably had no idea how messy we would actually get.",
          "ko": "인간을 창조하시고 보시기에 심히 좋았다고 하셨지만, 그 이후 우리가 얼마나 사고를 칠지 아셨다면 살짝 웃으셨을지도 모른다.",
          "zho": "造物主在创造人类后感叹一切甚好，若是预见到后来人类的各种搞怪和折腾，估计也会无奈地苦笑吧。",
          "es": "Dios terminó su obra y dijo que todo era muy bueno, pero probablemente no imaginó el desastre que íbamos a montar después.",
          "pt": "Deus finalizou a criação e disse que tudo era muito bom, mas provavelmente não imaginava a bagunça que a gente ia arrumar depois.",
          "vi": "Chúa tạo ra con người và thấy mọi thứ đều rất tốt đẹp, nhưng có lẽ Ngài đã không lường trước được sự rắc rối mà chúng ta sẽ gây ra sau này.",
          "tl": "Sinabi ng Diyos na napakabuti ng lahat matapos likhain ang tao, pero siguro ay mapapangiti na lang Siya kung nakita Niya ang gulo na gagawin natin.",
          "th": "พระเจ้าทรงสร้างมนุษย์และเห็นว่าดีมาก แต่ถ้าพระองค์ทรงเห็นความวุ่นวายที่เราก่อหลังจากนั้น คงได้แต่ยิ้มแห้งๆ ให้แน่",
          "sw": "Mungu alimaliza uumbaji na kusema kila kitu ni chema, ingawa huenda hakujua jinsi tutakavyokuwa na fujo huko mbele.",
          "it": "Dichiarare che tutto è molto buono dopo aver creato l'essere umano è un atto di grande fiducia, specie sapendo quanti guai combineremo."
        }
    },
    "2026-01-16": {
        date: "2026-01-16",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 2:1-7",
          "en": "Genesis 2:1-7",
          "ko": "Genesis 2:1-7",
          "zho": "創世記 2:1-7",
          "es": "Genesis 2:1-7",
          "pt": "Genesis 2:1-7",
          "vi": "Genesis 2:1-7",
          "tl": "Genesis 2:1-7",
          "th": "Genesis 2:1-7",
          "sw": "Genesis 2:1-7",
          "it": "Genesi 2:1-7"
        },
        comment: {
          "ja": "天地創造の最後にしっかり休息をとる神様を見て、現代の私たちは休み下手な自分を少し反省するべきかもしれない。",
          "en": "God taking a full day off after creating the universe is a gentle reminder that we are way too obsessed with our to-do lists.",
          "ko": "천지창조를 마치고 쉬시는 하나님을 보며, 쉼 없이 달리기만 하는 현대인인 나는 조금 뜨끔해진다.",
          "zho": "看著神在創造萬物後選擇休息，提醒了總是在瞎忙的我們，偶爾停下來也是一種神聖的責任。",
          "es": "Ver a Dios descansando tras la creación es la señal definitiva de que nuestro ritmo de vida moderno necesita bajar varias marchas.",
          "pt": "Ver Deus descansando após a criação é um lembrete gentil de que nosso vício moderno em produtividade está fora de controle.",
          "vi": "Chứng kiến cảnh Chúa nghỉ ngơi sau khi tạo dựng thế giới, chúng ta mới thấy mình cần học cách sống chậm lại thay vì cứ chạy theo guồng quay công việc.",
          "tl": "Ang pagpapahinga ng Diyos matapos ang paglikha ay isang paalala na hindi natin kailangang maging busy nang 24/7 para maging sapat.",
          "th": "การที่พระเจ้าทรงพักผ่อนหลังสร้างโลก เป็นการเตือนสติคนยุคใหม่ที่บ้างานว่าการหยุดพักบ้างไม่ใช่เรื่องผิด",
          "sw": "Mungu kupumzika baada ya kuumba ulimwengu ni ukumbusho tosha kwamba sisi sote tunahitaji kupunguza kasi ya maisha yetu yenye pilikapilika.",
          "it": "Se persino l'universo ha richiesto una pausa strategica, forse possiamo smetterla di sentirci in colpa quando ci riposiamo."
        }
    },
    "2026-01-17": {
        date: "2026-01-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 2:8-17",
          "en": "Genesis 2:8-17",
          "ko": "Genesis 2:8-17",
          "zho": "創世記 2:8-17",
          "es": "Genesis 2:8-17",
          "pt": "Genesis 2:8-17",
          "vi": "Genesis 2:8-17",
          "tl": "Genesis 2:8-17",
          "th": "Genesis 2:8-17",
          "sw": "Genesis 2:8-17",
          "it": "Genesi 2:8-17"
        },
        comment: {
          "ja": "楽園に住みながら禁断の木の実だけがやたら気になるアダムとエバ、人間が持っている「ダメと言われると余計にやりたくなる」根性は太古から健在だった。",
          "en": "Adam and Eve having everything in paradise but obsessing over the one forbidden tree is the ultimate proof that human curiosity is just plain stubborn.",
          "ko": "낙원의 모든 것을 누리면서 하필 금단의 열매에만 꽂히는 아담과 하와를 보면, 청개구리 같은 인간의 본성은 역사가 깊다는 걸 실감한다.",
          "zho": "在伊甸園擁有一切卻偏偏對禁果念念不忘，亞當與夏娃完美詮釋了人類那種「越不讓做越想做」的叛逆基因。",
          "es": "Tener el paraíso entero y obsesionarse con el único fruto prohibido demuestra que la curiosidad humana siempre ha sido nuestro punto más débil.",
          "pt": "Ter o paraíso inteiro à disposição e focar apenas no fruto proibido prova que a nossa teimosia humana é um problema antigo.",
          "vi": "Sở hữu cả thiên đường nhưng lại chỉ quan tâm đến trái cấm, Adam và Eva cho thấy bản tính tò mò thích làm ngược lại của con người vốn đã có từ thuở sơ khai.",
          "tl": "Ang pagkakaroon ng lahat sa paraiso ngunit pagtuon lang sa ipinagbabawal na bunga ay patunay na ang pagiging makulit ng tao ay hindi na bago.",
          "th": "การที่อาดัมกับเอวาสนใจแต่ผลไม้ต้องห้ามทั้งที่มีทุกอย่างในสวนอีเดน แสดงให้เห็นว่านิสัยดื้อรั้นของมนุษย์นั้นมีมานานแล้วจริงๆ",
          "sw": "Adam na Hawa kuwa na kila kitu katika bustani lakini wakajikita kwenye tunda moja lililokatazwa inathibitisha kuwa udadisi wa kibinadamu ni ukaidi wa asili.",
          "it": "Avere tutto il paradiso a disposizione ma fissarsi sull'unico frutto vietato conferma che la curiosità testarda è il nostro marchio di fabbrica."
        }
    },
    "2026-01-18": {
        date: "2026-01-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 2:18-25",
          "en": "Genesis 2:18-25",
          "ko": "Genesis 2:18-25",
          "zho": "創世記 2:18-25",
          "es": "Genesis 2:18-25",
          "pt": "Genesis 2:18-25",
          "vi": "Genesis 2:18-25",
          "tl": "Genesis 2:18-25",
          "th": "Genesis 2:18-25",
          "sw": "Genesis 2:18-25",
          "it": "Genesi 2:18-25"
        },
        comment: {
          "ja": "あらゆる動物に名前をつけた直後に理想のパートナーが現れる展開、もはや神様による完璧なマッチングアプリの演出にしか見えない。",
          "en": "Naming every animal on earth only to have your perfect match show up immediately is the ultimate divine matchmaking service.",
          "ko": "세상의 모든 동물에게 이름을 붙여준 직후 이상형이 나타나는 전개는 신께서 직접 주선해주신 완벽한 소개팅 그 자체다.",
          "zho": "给所有动物起完名字后就遇到了完美的伴侣，这简直就是上帝亲自操办的高级相亲。",
          "es": "Nombrar a todos los animales solo para que aparezca tu pareja ideal es el servicio de citas más eficiente de la historia.",
          "pt": "Dar nome a todos os animais e logo em seguida encontrar a sua cara-metade é o serviço de matchmaking mais eficiente que já existiu.",
          "vi": "Đặt tên cho muôn loài xong xuôi rồi tìm thấy người bạn đời lý tưởng, đây quả là dịch vụ mai mối thần thánh nhất lịch sử.",
          "tl": "Ang pangalanan ang lahat ng hayop at biglang lumitaw ang iyong kapareha ay ang pinaka-epektibong serbisyo ng matchmaking sa kasaysayan.",
          "th": "การตั้งชื่อให้สัตว์ทุกชนิดแล้วจู่ๆ ก็เจอคู่แท้โผล่มาตรงหน้า นี่มันบริการจัดหาคู่จากสวรรค์แบบครบวงจรชัดๆ",
          "sw": "Kuwapa wanyama wote majina kisha kukutana na mwenza wako mara moja ni huduma ya kutafuta wachumba bora kupita zote.",
          "it": "Dare un nome a tutti gli animali e poi trovarsi davanti l'anima gemella sembra proprio la versione divina di un'app di incontri che non sbaglia un colpo."
        }
    },
    "2026-01-19": {
        date: "2026-01-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 3:1-7",
          "en": "Genesis 3:1-7",
          "ko": "Genesis 3:1-7",
          "zho": "創世記 3:1-7",
          "es": "Genesis 3:1-7",
          "pt": "Genesis 3:1-7",
          "vi": "Genesis 3:1-7",
          "tl": "Genesis 3:1-7",
          "th": "Genesis 3:1-7",
          "sw": "Genesis 3:1-7",
          "it": "Genesi 3:1-7"
        },
        comment: {
          "ja": "蛇の甘い誘惑に負けてすぐに食べてしまう二人、禁断の果実という名の期間限定スイーツに抗える人間なんて最初からいなかったのだ。",
          "en": "Falling for the serpent's pitch immediately proves that humans never stood a chance against the original limited-time offer.",
          "ko": "뱀의 감언이설에 넘어가 금단의 열매를 바로 맛본 것을 보면, 인간은 애초에 한정판이라는 유혹을 이겨낼 재간이 없었던 것 같다.",
          "zho": "轻易被蛇的甜言蜜语说动并吃下禁果，这证明人类从一开始就没法抵御所谓的限时诱惑。",
          "es": "Caer ante la tentación de la serpiente demuestra que los humanos nunca tuvimos oportunidad contra la primera oferta de tiempo limitado.",
          "pt": "Cair na tentação da serpente prova que os humanos nunca tiveram a menor chance contra a primeira oferta de tempo limitado da história.",
          "vi": "Dễ dàng bị con rắn dụ dỗ và nếm thử trái cấm, cho thấy con người chưa bao giờ đủ bản lĩnh để cưỡng lại những lời chào mời ngọt ngào.",
          "tl": "Ang madaling mapaniwala ng ahas at pagkagat sa ipinagbabawal na bunga ay patunay na hindi talaga kaya ng tao ang mga tuksong mukhang napakaganda.",
          "th": "การหลงเชื่อคำหวานของงูและกินผลไม้ต้องห้ามทันที พิสูจน์ให้เห็นว่ามนุษย์เราไม่มีทางต้านทานข้อเสนอสุดพิเศษแบบจำกัดเวลาได้เลย",
          "sw": "Kuangukia mtego wa nyoka na kula tunda lililokatazwa mara moja ni uthibitisho kwamba wanadamu hawakuwahi kuwa na nafasi dhidi ya ofa hiyo ya kwanza ya kuvutia.",
          "it": "Cascare subito nella trappola del serpente dimostra che nessun essere umano ha mai avuto speranza contro il primo dolce fuori stagione."
        }
    },
    "2026-01-20": {
        date: "2026-01-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 3:8-19",
          "en": "Genesis 3:8-19",
          "ko": "Genesis 3:8-19",
          "zho": "創世記 3:8-19",
          "es": "Genesis 3:8-19",
          "pt": "Genesis 3:8-19",
          "vi": "Genesis 3:8-19",
          "tl": "Genesis 3:8-19",
          "th": "Genesis 3:8-19",
          "sw": "Genesis 3:8-19",
          "it": "Genesi 3:8-19"
        },
        comment: {
          "ja": "やらかした直後に木の陰に隠れてやり過ごそうとするアダムとエバの姿、気まずい時にスマホを見て現実逃避する現代人の姿と重なりすぎてつらい。",
          "en": "Adam and Eve hiding behind a tree to avoid the consequences of their mistake is basically every one of us dodging a difficult email.",
          "ko": "일 저지르고 나무 뒤에 숨어버린 아담과 하와를 보니, 곤란한 상황에서 못 본 척 회피하고 싶은 우리들의 모습이 떠올라 씁쓸하다.",
          "zho": "闖禍後躲在樹後想蒙混過關的亞當和夏娃，完全就是我們在尷尬時刻假裝沒看見訊息的現代寫照。",
          "es": "Ver a Adán y Eva escondiéndose tras los árboles para evadir su error es como vernos a nosotros mismos evitando una llamada incómoda.",
          "pt": "Adão e Eva se escondendo atrás das árvores para não encarar o erro é exatamente como a gente finge que não viu aquela mensagem difícil.",
          "vi": "Hình ảnh A-đam và Ê-va trốn sau bụi cây để tránh né hậu quả thật giống hệt cách chúng ta tìm cách lánh mặt khi vừa gây ra chuyện.",
          "tl": "Ang pagtatago nina Adan at Eva sa likod ng puno para iwasan ang pagkakamali nila ay parang tayo lang na umiiwas sa mahirap na usapan.",
          "th": "อดัมกับเอวาทําผิดแล้วไปแอบหลังต้นไม้ มันช่างเหมือนเวลาที่เราพยายามเนียนหายตัวไปตอนมีเรื่องยุ่งยากจริงๆ",
          "sw": "Adam na Hawa kujificha nyuma ya miti baada ya kufanya makosa ni kama sisi tunavyojaribu kukwepa wajibu wetu wakati mambo yameharibika.",
          "it": "Adamo ed Eva che si nascondono dietro un albero per evitare le conseguenze sono esattamente noi quando fingiamo di non aver visto quella mail di lavoro."
        }
    },
    "2026-01-21": {
        date: "2026-01-21",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 3:20-24",
          "en": "Genesis 3:20-24",
          "ko": "Genesis 3:20-24",
          "zho": "創世記 3:20-24",
          "es": "Genesis 3:20-24",
          "pt": "Genesis 3:20-24",
          "vi": "Genesis 3:20-24",
          "tl": "Genesis 3:20-24",
          "th": "Genesis 3:20-24",
          "sw": "Genesis 3:20-24",
          "it": "Genesi 3:20-24"
        },
        comment: {
          "ja": "エデンの園を追い出される直前に名前をつけ合う夫婦、どんなに状況が悪くてもとりあえず目の前の生活を回そうとする人間という生き物のたくましさよ。",
          "en": "Naming each other right before getting kicked out of Eden proves that humans will try to keep their daily routine going no matter how bad the disaster is.",
          "ko": "낙원에서 쫓겨나는 와중에도 이름을 지어주는 부부를 보니, 어떤 상황에서도 어떻게든 일상을 이어가려는 인간의 끈질긴 생명력이 느껴진다.",
          "zho": "即便被逐出伊甸園也要忙著取名，這對夫妻展現了人類無論處境多糟，都要努力維持日常生活的強大韌性。",
          "es": "Ponerse nombres justo antes de ser expulsados del Edén demuestra que, sin importar el caos, los humanos siempre intentamos normalizar nuestra rutina.",
          "pt": "Dar nomes um ao outro antes de serem expulsos do Éden mostra que, não importa o tamanho do desastre, o ser humano sempre tenta manter a rotina.",
          "vi": "Việc đặt tên cho nhau ngay trước khi bị trục xuất khỏi vườn Ê-đen cho thấy bản năng con người là luôn cố gắng duy trì nếp sống dù hoàn cảnh có tồi tệ đến đâu.",
          "tl": "Ang pagpapangalan sa isa't isa bago pa man palayasin sa Eden ay patunay na kahit gaano kalala ang sitwasyon, sinusubukan pa rin nating gawing normal ang lahat.",
          "th": "การตั้งชื่อให้กันก่อนจะถูกไล่ออกจากสวนเอเดน แสดงให้เห็นว่าไม่ว่าจะเจอวิกฤตแค่ไหน มนุษย์เราก็ยังพยายามจะใช้ชีวิตประจําวันกันต่อไป",
          "sw": "Kuitana majina kabla ya kufukuzwa kutoka Edeni inaonyesha jinsi wanadamu wanavyojitahidi kuendeleza maisha hata mambo yanapoharibika kabisa.",
          "it": "Mettersi d'accordo sui nomi un attimo prima di essere sfrattati dall'Eden dimostra che noi umani cerchiamo di mantenere la nostra routine anche durante l'apocalisse."
        }
    },
    "2026-01-22": {
        date: "2026-01-22",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 5:1-15",
          "en": "Moses 5:1-15",
          "ko": "Moses 5:1-15",
          "zho": "モーセ書 5:1-15",
          "es": "Moses 5:1-15",
          "pt": "Moses 5:1-15",
          "vi": "Moses 5:1-15",
          "tl": "Moses 5:1-15",
          "th": "Moses 5:1-15",
          "sw": "Moses 5:1-15",
          "it": "Mosè 5:1-15"
        },
        comment: {
          "ja": "エデンの園を追い出されてからが本番という現実は、新生活でいきなり壁にぶつかる社会人そのものだ。",
          "en": "Realizing that life actually begins after being kicked out of the garden is the ultimate adulting experience.",
          "ko": "에덴동산에서 쫓겨난 뒤부터가 진짜 시작이라는 현실은, 사회에 첫발을 내디딘 신입사원의 멘붕과 너무 닮았다.",
          "zho": "被逐出伊甸園才算真正生活的開始，這簡直就是剛踏入社會面對殘酷現實的職場新鮮人寫照。",
          "es": "Darse cuenta de que la vida empieza de verdad después de ser expulsado del jardín es la experiencia definitiva de crecer.",
          "pt": "Perceber que a vida realmente começa depois de ser expulso do jardim é a experiência definitiva de encarar a vida adulta.",
          "vi": "Nhận ra cuộc sống thực sự bắt đầu sau khi bị đuổi khỏi vườn địa đàng cũng giống như cảm giác vỡ mộng khi bắt đầu đi làm vậy.",
          "tl": "Ang mapagtantong nagsisimula lang ang tunay na buhay pagkatapos mong mapalayas sa hardin ay ang pinaka-relatable na karanasan sa pagtanda.",
          "th": "การที่ต้องเผชิญกับความจริงหลังจากถูกไล่ออกจากสวนเอเดน ก็ไม่ต่างอะไรกับเด็กจบใหม่ที่เพิ่งเริ่มเจอกับโลกของการทำงานจริงๆ",
          "sw": "Kutambua kuwa maisha huanza kweli baada ya kufukuzwa bustanini ni hali halisi inayowakabili watu wengi wanapoanza kujitegemea.",
          "it": "Realizzare che la vita vera comincia solo dopo lo sfratto dal giardino è la quintessenza del diventare grandi all'improvviso."
        }
    },
    "2026-01-23": {
        date: "2026-01-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 4:1-8",
          "en": "Genesis 4:1-8",
          "ko": "Genesis 4:1-8",
          "zho": "創世記 4:1-8",
          "es": "Genesis 4:1-8",
          "pt": "Genesis 4:1-8",
          "vi": "Genesis 4:1-8",
          "tl": "Genesis 4:1-8",
          "th": "Genesis 4:1-8",
          "sw": "Genesis 4:1-8",
          "it": "Genesi 4:1-8"
        },
        comment: {
          "ja": "神様からのお願いを無視してまで自分のこだわりを通そうとするカインの頑固さは、誰の心にも少しは住み着いている。",
          "en": "Cain ignoring heaven to double down on his own agenda is a level of stubbornness we all struggle with sometimes.",
          "ko": "하나님의 조언을 무시하고 끝까지 자기 고집을 부리는 가인의 모습은 우리 마음속 깊은 곳에 있는 옹고집과 너무 닮았다.",
          "zho": "該隱無視建議堅持己見的固執，簡直是我們每個人心中那個偶爾會跑出來搗亂的倔強小人。",
          "es": "La terquedad de Caín al ignorar el consejo divino para seguir su propio camino es algo con lo que todos luchamos a veces.",
          "pt": "A teimosia de Caim em ignorar o conselho divino para seguir seu próprio caminho é algo que todos nós enfrentamos às vezes.",
          "vi": "Sự bướng bỉnh của Cain khi phớt lờ lời khuyên để làm theo ý mình chính là hình ảnh phản chiếu sự cứng đầu mà ai cũng từng có.",
          "tl": "Ang katigasan ng ulo ni Cain na balewalain ang payo para lang masunod ang sariling gusto ay isang ugaling lahat tayo ay paminsan-minsang nakikipagbuno.",
          "th": "ความดื้อรั้นของคาอินที่เพิกเฉยต่อคำแนะนำเพื่อทำตามใจตัวเอง คือนิสัยที่เราทุกคนต่างก็เคยเผลอทำในบางครั้ง",
          "sw": "Ukaidi wa Kaini wa kupuuza ushauri ili tu afuate anayotaka ni tabia ambayo sote tunapambana nayo mara kwa mara.",
          "it": "La cocciutaggine di Caino che ignora il consiglio dall'alto per fare di testa sua è quel briciolo di testardaggine che ci portiamo dietro tutti."
        }
    },
    "2026-01-24": {
        date: "2026-01-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 4:9-16",
          "en": "Genesis 4:9-16",
          "ko": "Genesis 4:9-16",
          "zho": "創世記 4:9-16",
          "es": "Genesis 4:9-16",
          "pt": "Genesis 4:9-16",
          "vi": "Genesis 4:9-16",
          "tl": "Genesis 4:9-16",
          "th": "Genesis 4:9-16",
          "sw": "Genesis 4:9-16",
          "it": "Genesi 4:9-16"
        },
        comment: {
          "ja": "神様に聞かれて「知りません、私は弟の番人じゃないので」と逆ギレするカイン、都合が悪くなると急に開き直る自分の姿を見ているようで胃が痛い。",
          "en": "Cain snapping back at God that he is not his brother's keeper is the ultimate defensive move when you know you messed up.",
          "ko": "하나님 앞에서 뻔뻔하게 동생을 모른다고 잡아떼는 가인을 보면, 잘못했을 때 일단 발뺌부터 하고 보는 내 모습이 떠올라 찔린다.",
          "zho": "該隱反問神說我豈是看守我兄弟的嗎，那種理直氣壯的推卸責任，簡直是人類闖禍後的標準反應。",
          "es": "Caín respondiendo a Dios con esa defensiva de no ser el guardián de su hermano es la reacción humana clásica cuando sabemos que hemos metido la pata.",
          "pt": "Caim perguntando se é o guardião do irmão é a reação humana mais clássica de quem sabe que fez besteira e tenta sair pela tangente.",
          "vi": "Ca-in hỏi ngược lại Chúa xem mình có phải là người giữ em mình không, một kiểu chống chế đầy bản năng khi chúng ta lỡ tay gây chuyện.",
          "tl": "Ang pagsagot ni Cain kay Lord na hindi siya ang tagapangalaga ng kanyang kapatid ay ang pinaka-relatable na depensa kapag alam nating may nagawa tayong mali.",
          "th": "การที่คาอินย้อนถามพระเจ้าว่าเขาไม่ใช่ผู้ดูแลน้องชาย คือปฏิกิริยาป้องกันตัวแบบมนุษย์สุดๆ เวลาที่เรารู้ว่าตัวเองทำผิดพลาด",
          "sw": "Kaini kumjibu Mungu kuwa yeye si mlinzi wa ndugu yake ni ile hali ya kujitetea tunapojua kabisa tumekosea.",
          "it": "Caino che risponde male a Dio dicendo di non essere la bambinaia di suo fratello è la classica mossa da campioni quando veniamo beccati con le mani nel sacco."
        }
    },
    "2026-01-25": {
        date: "2026-01-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 4:17-26",
          "en": "Genesis 4:17-26",
          "ko": "Genesis 4:17-26",
          "zho": "創世記 4:17-26",
          "es": "Genesis 4:17-26",
          "pt": "Genesis 4:17-26",
          "vi": "Genesis 4:17-26",
          "tl": "Genesis 4:17-26",
          "th": "Genesis 4:17-26",
          "sw": "Genesis 4:17-26",
          "it": "Genesi 4:17-26"
        },
        comment: {
          "ja": "人類初の都市建設が殺人犯の名前から始まるという皮肉、どれだけ文明が発展しても人間の根っこにある矛盾はなかなか消えないものだ。",
          "en": "The irony of the first city being built by a murderer is a stark reminder that progress doesn't always wash away our messier human nature.",
          "ko": "살인자가 인류 최초의 도시를 세웠다는 아이러니는, 문명이 아무리 발전해도 그 안에 담긴 인간의 불완전함은 그대로라는 걸 보여준다.",
          "zho": "人類第一座城市竟然是由殺人犯建立的，這份諷刺感提醒我們，文明再進步也掩蓋不了人性深處的矛盾。",
          "es": "La ironía de que la primera ciudad fuera construida por un asesino nos recuerda que el progreso no siempre borra nuestra naturaleza más complicada.",
          "pt": "A ironia de a primeira cidade ter sido construída por um assassino é um lembrete de que o progresso nem sempre apaga nossas falhas humanas.",
          "vi": "Việc thành phố đầu tiên lại do một kẻ sát nhân xây dựng thật mỉa mai, nhắc nhở rằng văn minh không phải lúc nào cũng xóa sạch được sự bất toàn trong bản chất con người.",
          "tl": "Ang ironya na ang unang lungsod ay itinayo ng isang mamamatay-tao ay paalala na ang pag-unlad ay hindi palaging nakakapagbura sa ating mga pagkakamali.",
          "th": "ความย้อนแย้งที่ว่าเมืองแรกของมนุษย์ถูกสร้างโดยฆาตกร เป็นเครื่องเตือนใจว่าความเจริญไม่ได้ลบล้างความไม่สมบูรณ์แบบในใจคนได้เสมอไป",
          "sw": "Ukweli mchungu kuwa mji wa kwanza ulijengwa na muuaji unakumbusha kuwa maendeleo hayafuti kabisa udhaifu wa asili yetu ya kibinadamu.",
          "it": "Che la prima città della storia sia stata fondata da un assassino ci ricorda che il progresso non cancella mai del tutto i nostri pasticci."
        }
    },
    "2026-01-26": {
        date: "2026-01-26",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 6:1-9",
          "en": "Moses 6:1-9",
          "ko": "Moses 6:1-9",
          "zho": "モーセ書 6:1-9",
          "es": "Moses 6:1-9",
          "pt": "Moses 6:1-9",
          "vi": "Moses 6:1-9",
          "tl": "Moses 6:1-9",
          "th": "Moses 6:1-9",
          "sw": "Moses 6:1-9",
          "it": "Mosè 6:1-9"
        },
        comment: {
          "ja": "アダムが神の似姿として歩んだ記録を読んでから自分の日常を振り返ると、あまりの落差に思わず苦笑いしてしまう。",
          "en": "Reading about Adam walking with God and then looking at my own chaotic schedule is a humbling reality check.",
          "ko": "아담이 하나님의 형상대로 걸었다는 기록을 읽고 나서 나의 하루를 돌아보니 왠지 모를 씁쓸한 웃음이 나온다.",
          "zho": "讀完亞當按著神的樣式行走的記錄，再看看自己的日常，那種反差感真是讓人苦笑不得。",
          "es": "Leer sobre Adán caminando con Dios y luego ver mi propia rutina diaria es un contraste que me hace sonreír con algo de ironía.",
          "pt": "Ler sobre Adão andando com Deus e depois olhar para a minha própria rotina é um choque de realidade que me faz rir de nervoso.",
          "vi": "Đọc về cách A-đam bước đi cùng Chúa rồi nhìn lại cuộc sống thường nhật của chính mình, sự khác biệt thật khiến người ta phải bật cười.",
          "tl": "Ang pagbabasa tungkol sa paglakad ni Adan kasama ang Diyos at pagtingin sa sarili kong buhay ay isang realidad na nakakatuwang pagtawanan.",
          "th": "อ่านบันทึกเรื่องอาดัมดำเนินกับพระเจ้าแล้วหันมามองชีวิตประจำวันของตัวเอง ก็อดอมยิ้มให้กับความต่างไม่ได้เลย",
          "sw": "Kusoma kuhusu Adamu akitembea na Mungu na kisha kutazama maisha yangu ya kila siku ni tofauti inayokufanya utabasamu kwa aibu.",
          "it": "Leggere di Adamo che cammina a braccetto con il cielo e poi guardare alla nostra caotica lista della spesa fa quasi sorridere per il contrasto."
        }
    },
    "2026-01-27": {
        date: "2026-01-27",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 5:1-11",
          "en": "Genesis 5:1-11",
          "ko": "Genesis 5:1-11",
          "zho": "創世記 5:1-11",
          "es": "Genesis 5:1-11",
          "pt": "Genesis 5:1-11",
          "vi": "Genesis 5:1-11",
          "tl": "Genesis 5:1-11",
          "th": "Genesis 5:1-11",
          "sw": "Genesis 5:1-11",
          "it": "Genesi 5:1-11"
        },
        comment: {
          "ja": "何百歳も生きた長寿の記録が淡々と続くのを見ると、現代の忙しさに追われて焦っている自分が少し小さく見える。",
          "en": "Seeing these ancient lifespans listed so casually makes my current stress over a deadline feel almost comical.",
          "ko": "수백 년을 살았다는 기록을 담담하게 읽다 보면 오늘 하루 마감에 쫓기며 발을 동동 구르는 내 모습이 왠지 작아 보인다.",
          "zho": "看著這些長壽的記錄被如此平淡地寫下，突然覺得自己為現代生活的瑣事而焦慮顯得有點滑稽。",
          "es": "Ver estos registros de vidas tan largas contados con tanta sencillez hace que mi estrés por los plazos de entrega parezca casi cómico.",
          "pt": "Ver esses registros de longevidade contados com tanta naturalidade faz com que meu estresse com prazos pareça quase cômico.",
          "vi": "Nhìn những kỷ lục về tuổi thọ được ghi lại một cách bình thản, tôi thấy sự vội vã của mình trong cuộc sống hiện đại thật buồn cười.",
          "tl": "Ang makita ang mga tala ng mahabang buhay na isinulat nang simple ay nagpaparamdam sa akin na ang aking stress sa deadlines ay medyo nakakatawa.",
          "th": "การได้เห็นบันทึกเรื่องอายุขัยที่ยาวนานถูกเขียนไว้อย่างเรียบง่าย ทำให้ความเครียดเรื่องเดดไลน์ของฉันดูตลกไปเลย",
          "sw": "Kuona rekodi hizi za umri mrefu zikiorodheshwa kwa kawaida hufanya msongo wangu wa mawazo kuhusu makataa ya kazi uonekane kama mzaha.",
          "it": "Leggere di queste vite lunghe secoli e secoli fa sembrare la mia ansia per le scadenze di oggi una barzelletta."
        }
    },
    "2026-01-28": {
        date: "2026-01-28",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 5:12-20",
          "en": "Genesis 5:12-20",
          "ko": "Genesis 5:12-20",
          "zho": "創世記 5:12-20",
          "es": "Genesis 5:12-20",
          "pt": "Genesis 5:12-20",
          "vi": "Genesis 5:12-20",
          "tl": "Genesis 5:12-20",
          "th": "Genesis 5:12-20",
          "sw": "Genesis 5:12-20",
          "it": "Genesi 5:12-20"
        },
        comment: {
          "ja": "何百歳も生きる家系図の数字を見ていると、現代の私たちの人生はまるで早送りボタンを押されたような気分になる。",
          "en": "Looking at these lifespans makes our modern lives feel like someone hit the fast-forward button.",
          "ko": "수백 년씩 이어지는 족보의 숫자를 보고 있자니 현대인의 수명은 마치 빨리감기 버튼을 누른 것처럼 느껴진다.",
          "zho": "看著這些動輒幾百歲的家譜紀錄，現代人的壽命簡直像是被按下了快轉鍵。",
          "es": "Ver estas edades tan largas hace que nuestra vida moderna parezca que alguien presionó el botón de avance rápido.",
          "pt": "Olhar para essas idades faz com que a nossa vida moderna pareça que alguém apertou o botão de avanço rápido.",
          "vi": "Nhìn vào những con số tuổi thọ khổng lồ này, đời người hiện đại dường như chỉ như một cái chớp mắt.",
          "tl": "Sa haba ng mga taong nabuhay sa talaang ito, parang pinindot lang ang fast-forward button sa buhay natin ngayon.",
          "th": "การได้เห็นอายุขัยที่ยาวนานในลำดับพงศ์พันธุ์นี้ ทำให้ชีวิตสมัยใหม่ของเราดูเหมือนถูกกดปุ่มกรอไปข้างหน้าอย่างรวดเร็ว",
          "sw": "Kuona umri mrefu katika vizazi hivi kunafanya maisha yetu ya sasa yaonekane kama yamepigwa mbio kwa kasi.",
          "it": "A vedere queste età così lunghe, la nostra vita moderna sembra accelerata come un video riprodotto al doppio della velocità."
        }
    },
    "2026-01-29": {
        date: "2026-01-29",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 5:21-24",
          "en": "Genesis 5:21-24",
          "ko": "Genesis 5:21-24",
          "zho": "創世記 5:21-24",
          "es": "Genesis 5:21-24",
          "pt": "Genesis 5:21-24",
          "vi": "Genesis 5:21-24",
          "tl": "Genesis 5:21-24",
          "th": "Genesis 5:21-24",
          "sw": "Genesis 5:21-24",
          "it": "Genesi 5:21-24"
        },
        comment: {
          "ja": "神様と仲良すぎてそのまま連れ去られてしまったエノク、もはや究極の神隠しというよりは最高の早期リタイア生活。",
          "en": "Enoch being so close to God that he just vanished is less of a disappearance and more the ultimate early retirement.",
          "ko": "하나님과 너무 친해서 그냥 데려가심을 당한 에녹은 실종이라기보다 최고의 조기 은퇴 사례가 아닐까.",
          "zho": "以諾因為和神關係太好直接被接走，這與其說是失蹤，不如說是終極版的提早退休。",
          "es": "Enoc era tan cercano a Dios que simplemente se lo llevó; más que una desaparición, parece la jubilación anticipada perfecta.",
          "pt": "Enoque era tão próximo de Deus que foi levado; mais do que um desaparecimento, parece a aposentadoria antecipada perfeita.",
          "vi": "Hê-nóc quá thân thiết với Chúa đến mức Ngài đem ông đi, đó không phải là mất tích mà là sự nghỉ hưu sớm hoàn hảo nhất.",
          "tl": "Si Enoc na masyadong naging malapit sa Diyos kaya kinuha na lang siya, hindi ito pagkawala kundi ang pinaka-ultimate na early retirement.",
          "th": "เอโนคสนิทกับพระเจ้ามากจนพระองค์ทรงรับไป นี่ไม่ใช่อาการหายตัวไป แต่คือการเกษียณอายุก่อนกำหนดขั้นสุดยอด",
          "sw": "Enoki kuwa karibu sana na Mungu hadi akachukuliwa ni zaidi ya kutoweka, ni kama kustaafu mapema kwa njia ya kipekee.",
          "it": "Enoc era così in sintonia con il cielo che la sua scomparsa sembra più la pensione anticipata perfetta che un mistero."
        }
    },
    "2026-01-30": {
        date: "2026-01-30",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 6:35-46",
          "en": "Moses 6:35-46",
          "ko": "Moses 6:35-46",
          "zho": "モーセ書 6:35-46",
          "es": "Moses 6:35-46",
          "pt": "Moses 6:35-46",
          "vi": "Moses 6:35-46",
          "tl": "Moses 6:35-46",
          "th": "Moses 6:35-46",
          "sw": "Moses 6:35-46",
          "it": "Mosè 6:35-46"
        },
        comment: {
          "ja": "神様に「お前は雄弁じゃない」と先回りして言い訳を封じられるエノク、神様には隠し事も言い訳も通用しないという絶望的な事実がここにある。",
          "en": "Enoch trying to talk his way out of a mission only to have God shut down every excuse before he can finish is a humbling reminder that God sees right through our imposter syndrome.",
          "ko": "하나님 앞에서 자신의 부족함을 이유로 도망치려던 에녹이 결국 설득당하는 모습은, 우리가 늘어놓는 온갖 핑계들이 얼마나 무력한지 보여준다.",
          "zho": "以诺试图找借口推辞神圣使命，却被神当场识破每一个托词，这提醒我们，在神面前任何借口都是徒劳的。",
          "es": "Enoch tratando de poner excusas para no cumplir su misión mientras Dios desarma cada una de ellas es la prueba de que no podemos escondernos de nuestra propia vocación.",
          "pt": "Enoque tentando dar desculpas para não cumprir sua missão enquanto Deus desmonta cada uma delas é a prova de que não dá para fugir do nosso propósito.",
          "vi": "Việc Hê-nóc cố gắng đưa ra lý do để từ chối sứ mệnh nhưng bị Chúa chặn đứng mọi lời bào chữa cho thấy mọi sự biện hộ đều vô ích trước mặt Ngài.",
          "tl": "Ang pagtatangka ni Enoc na magdahilan para hindi tumanggap ng misyon, na agad namang sinagot ng Diyos, ay nagpapaalala sa atin na walang saysay ang mga palusot sa harap ng Maykapal.",
          "th": "เอโนคพยายามหาข้ออ้างเพื่อเลี่ยงภารกิจแต่ถูกพระเจ้าดักทางไว้ทุกประการ เป็นเครื่องเตือนใจว่าข้อแก้ตัวของเราไม่มีความหมายอะไรเลยในสายพระเนตรของพระองค์",
          "sw": "Enoki akijaribu kutoa visingizio ili kuepuka wito wake huku Mungu akivivunja vyote ni ukumbusho kwamba hatuwezi kujificha nyuma ya visingizio vyetu mbele ya Mungu.",
          "it": "Provare a inventare scuse con chi sa già tutto è la dimostrazione che non possiamo sfuggire alla nostra vocazione."
        }
    },
    "2026-01-31": {
        date: "2026-01-31",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 6:47-57",
          "en": "Moses 6:47-57",
          "ko": "Moses 6:47-57",
          "zho": "モーセ書 6:47-57",
          "es": "Moses 6:47-57",
          "pt": "Moses 6:47-57",
          "vi": "Moses 6:47-57",
          "tl": "Moses 6:47-57",
          "th": "Moses 6:47-57",
          "sw": "Moses 6:47-57",
          "it": "Mosè 6:47-57"
        },
        comment: {
          "ja": "生まれたばかりの赤ん坊が泣く理由を「罪を背負って生まれてきたから」と真剣に説くエノク、子育ての苦労を神学的に捉えすぎていて逆に面白い。",
          "en": "Enoch explaining that babies cry because they are born into a fallen world is a heavy theological take on the classic 'my baby won't stop screaming' dilemma.",
          "ko": "아기가 우는 이유를 죄와 타락의 관점에서 깊이 있게 설명하는 에녹을 보며, 육아의 고충을 너무 거창하게 해석하는 모습이 묘하게 인간적이다.",
          "zho": "以诺将婴儿啼哭解释为与生俱来的世俗重担，这种对育儿苦恼的超硬核神学解读，让人既佩服又觉得莫名可爱。",
          "es": "Enoch explicando que los bebés lloran porque nacen en un mundo caído es una forma muy profunda, y casi cómica, de ver el llanto de un recién nacido.",
          "pt": "Enoque explicando que os bebês choram porque nascem em um mundo caído é uma visão teológica pesada para lidar com um bebê que não para de gritar.",
          "vi": "Việc Hê-nóc giải thích rằng trẻ sơ sinh khóc vì chúng sinh ra trong một thế giới sa ngã là một cách nhìn nhận đầy tính thần học về tiếng khóc của trẻ nhỏ.",
          "tl": "Ang paliwanag ni Enoc na ang mga sanggol ay umiiyak dahil ipinanganak sila sa isang mundong puno ng kasalanan ay isang napakalalim na pagtingin sa simpleng pag-iyak ng bata.",
          "th": "เอโนคอธิบายว่าเด็กทารกร้องไห้เพราะเกิดมาในโลกที่ตกในบาป เป็นมุมมองทางเทววิทยาที่จริงจังเกินไปนิดสำหรับเรื่องที่เด็กแค่หิวนม",
          "sw": "Enoki kueleza kuwa watoto wachanga hulia kwa sababu wamezaliwa katika ulimwengu ulioanguka ni mtazamo mzito sana wa kiteolojia kuhusu kilio cha mtoto.",
          "it": "Spiegare il pianto di un neonato con la teologia è un modo decisamente originale per affrontare la stanchezza dei genitori."
        }
    },
    "2026-02-01": {
        date: "2026-02-01",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 6:58-68",
          "en": "Moses 6:58-68",
          "ko": "Moses 6:58-68",
          "zho": "モーセ書 6:58-68",
          "es": "Moses 6:58-68",
          "pt": "Moses 6:58-68",
          "vi": "Moses 6:58-68",
          "tl": "Moses 6:58-68",
          "th": "Moses 6:58-68",
          "sw": "Moses 6:58-68",
          "it": "Mosè 6:58-68"
        },
        comment: {
          "ja": "水と血と霊で清められるというのは、結局のところ自分という人間を一度リセットしてゼロからやり直すという究極の断捨離なのかもしれない。",
          "en": "Being cleansed by water, blood, and spirit is basically the ultimate spiritual decluttering where you reset your entire life from scratch.",
          "ko": "물과 피와 성령으로 거듭난다는 것은 결국 나라는 사람을 완전히 비우고 처음부터 다시 시작하는 궁극의 미니멀리즘 같은 것 아닐까.",
          "zho": "藉由水、血和靈來潔淨，說穿了就像是把那個滿身雜念的自我徹底歸零，來場靈魂層面的斷捨離。",
          "es": "Ser limpiado por el agua, la sangre y el espíritu es básicamente el minimalismo definitivo para resetear tu vida y empezar de cero.",
          "pt": "Ser purificado pela água, pelo sangue e pelo espírito é, no fundo, o minimalismo supremo para resetar quem somos e recomeçar do zero.",
          "vi": "Được thanh tẩy bởi nước, huyết và thần linh thực chất giống như một cuộc dọn dẹp tâm hồn triệt để để làm lại từ đầu.",
          "tl": "Ang malinis sa pamamagitan ng tubig, dugo, at espiritu ay parang ultimate na pag-declutter sa buhay para magsimulang muli nang tama.",
          "th": "การชำระล้างด้วยน้ำ เลือด และพระวิญญาณ ก็เหมือนการทำความสะอาดครั้งใหญ่เพื่อรีเซ็ตชีวิตตัวเองแล้วเริ่มต้นใหม่จากศูนย์",
          "sw": "Kutakaswa kwa maji, damu, na roho ni kama kufanya usafi wa kina wa nafsi ili uanze maisha upya kabisa.",
          "it": "Ricevere una pulizia profonda dello spirito è in fondo il modo migliore per fare il tagliando alla propria vita e ripartire."
        }
    },
    "2026-02-02": {
        date: "2026-02-02",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 7:1-11",
          "en": "Moses 7:1-11",
          "ko": "Moses 7:1-11",
          "zho": "モーセ書 7:1-11",
          "es": "Moses 7:1-11",
          "pt": "Moses 7:1-11",
          "vi": "Moses 7:1-11",
          "tl": "Moses 7:1-11",
          "th": "Moses 7:1-11",
          "sw": "Moses 7:1-11",
          "it": "Mosè 7:1-11"
        },
        comment: {
          "ja": "神様がニネベの民を心から愛していると聞いて、なんであんな奴らを許すんだと不貞腐れるエノクの姿に、正直すぎる人間味を感じてしまう。",
          "en": "Enoch being told God loves the people of Nineveh and then struggling to accept it is the most relatable reaction to someone we really do not like.",
          "ko": "하나님께서 니느웨 사람들을 사랑하신다는 말에 이해할 수 없다는 듯 뚱한 표정을 짓는 에녹의 모습에서 우리 모두의 솔직한 본심을 본다.",
          "zho": "聽到神愛著尼尼微人，エノク那種「憑什麼原諒他們」的彆扭反應，簡直就是我們面對討厭之人時最真實的內心戲。",
          "es": "Ver a Enoc luchar con la idea de que Dios ama a personas que él no soporta es el reflejo perfecto de nuestras propias reacciones humanas ante quienes nos caen mal.",
          "pt": "Ver Enoque tentando processar o amor de Deus por pessoas que ele não suporta é a descrição perfeita de como reagimos quando temos que perdoar quem não queremos.",
          "vi": "Phản ứng của Hê-nóc khi biết Chúa yêu thương dân Ni-ni-ve giống hệt cách chúng ta vẫn thường cằn nhằn khi thấy người mình ghét lại được đối xử tử tế.",
          "tl": "Ang makita si Enoc na nahihirapang tanggapin ang pagmamahal ng Diyos sa mga taong ayaw niya ay saktong-sakto sa nararamdaman natin kapag may taong hindi natin gusto.",
          "th": "การที่เอโนคพยายามทำความเข้าใจความรักของพระเจ้าที่มีต่อคนนีเนเวห์ คือภาพสะท้อนความรู้สึกจริงใจแบบมนุษย์เวลาเราต้องให้อภัยคนที่ไม่อยากให้อภัย",
          "sw": "Kuona jinsi Enoki anavyohangaika kukubali upendo wa Mungu kwa watu asiotaka kuwapenda ni kioo kamili cha jinsi tunavyohisi tunapokutana na watu tunaowachukia.",
          "it": "Fare i conti con l'idea che il cielo ami anche chi non sopportiamo è la prova più dura del nostro ego."
        }
    },
    "2026-02-03": {
        date: "2026-02-03",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 7:12-17",
          "en": "Moses 7:12-17",
          "ko": "Moses 7:12-17",
          "zho": "モーセ書 7:12-17",
          "es": "Moses 7:12-17",
          "pt": "Moses 7:12-17",
          "vi": "Moses 7:12-17",
          "tl": "Moses 7:12-17",
          "th": "Moses 7:12-17",
          "sw": "Moses 7:12-17",
          "it": "Mosè 7:12-17"
        },
        comment: {
          "ja": "敵陣のど真ん中で恐れずに言葉を放つエノク、メンタルが強すぎて現代のSNS炎上案件なら即座にミュートされそうなレベル。",
          "en": "Enoch walking into the middle of hostile territory to speak his mind is a level of guts that would get him instantly cancelled on modern social media.",
          "ko": "적진 한복판에서 두려움 없이 할 말을 다 하는 에녹을 보니 현대 사회의 키보드 워리어와는 차원이 다른 강철 멘탈이 느껴진다.",
          "zho": "以諾在敵營中毫不畏懼地直言不諱，這種強大的心理素質放在現代網路世界，絕對是引發熱議的狠角色。",
          "es": "Enoch entrando en territorio enemigo sin miedo a decir la verdad tiene una valentía que haría que lo bloquearan en cualquier red social actual.",
          "pt": "Enoch entrando em território inimigo sem medo de falar a verdade tem uma coragem que o faria ser bloqueado em qualquer rede social atual.",
          "vi": "Enoch đi thẳng vào hang cọp để nói lên sự thật, một tinh thần thép mà nếu ở thời đại mạng xã hội bây giờ thì đã bị chặn từ lâu rồi.",
          "tl": "Si Enoc na pumasok sa gitna ng kaaway para magsalita nang walang takot ay may tapang na siguradong maba-bash agad sa social media ngayon.",
          "th": "เอโนคเดินดุ่มๆ เข้าไปในดงศัตรูเพื่อพูดความจริงด้วยความกล้าหาญระดับที่ถ้าเป็นโซเชียลสมัยนี้คงโดนทัวร์ลงไปนานแล้ว",
          "sw": "Enoki kuingia katikati ya maadui zake bila hofu ili kusema kweli ni ujasiri ambao ungemfanya azuiwe kwenye mitandao ya kijamii ya sasa.",
          "it": "Entrare in territorio ostile per dire la verità richiede un coraggio tale che oggi verrebbe bloccato da qualsiasi algoritmo."
        }
    },
    "2026-02-04": {
        date: "2026-02-04",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 7:18-27",
          "en": "Moses 7:18-27",
          "ko": "Moses 7:18-27",
          "zho": "モーセ書 7:18-27",
          "es": "Moses 7:18-27",
          "pt": "Moses 7:18-27",
          "vi": "Moses 7:18-27",
          "tl": "Moses 7:18-27",
          "th": "Moses 7:18-27",
          "sw": "Moses 7:18-27",
          "it": "Mosè 7:18-27"
        },
        comment: {
          "ja": "みんなが争っている横でシオンという理想郷を作ってしまうエノクたち、周りに流されず自分のコミュニティを構築する強さを見習いたい。",
          "en": "Enoch and his people building a Zion while everyone else was busy fighting is the ultimate example of choosing your own environment over the toxic status quo.",
          "ko": "다들 싸우느라 바쁠 때 조용히 시온이라는 이상향을 건설해버린 에녹 일행, 주변 분위기에 휩쓸리지 않는 마이웨이의 끝판왕.",
          "zho": "當全世界都在爭鬥時，以諾和他的百姓卻默默建立起錫安，這根本是拒絕被負面環境同化、堅持走自己路的最高境界。",
          "es": "Enoch y su gente construyendo Sión mientras todos los demás estaban ocupados peleando es el ejemplo perfecto de elegir tu propio entorno sobre el caos general.",
          "pt": "Enoch e seu povo construindo Sião enquanto todos os outros estavam ocupados brigando é o exemplo perfeito de escolher seu próprio ambiente em vez do caos geral.",
          "vi": "Trong khi thiên hạ mải mê đánh đấm, nhóm của Enoch lại lặng lẽ xây dựng Sion, một ví dụ điển hình cho việc không để bản thân bị cuốn vào những tiêu cực xung quanh.",
          "tl": "Habang abala ang lahat sa pakikipag-away, si Enoc at ang kanyang mga tao ay tahimik na nagtayo ng Sion, isang magandang halimbawa ng pagpili sa sariling kapaligiran kaysa sa gulo.",
          "th": "ในขณะที่คนอื่นมัวแต่ทะเลาะกัน เอโนคและพวกพ้องกลับสร้างเมืองไซอันขึ้นมาได้ นี่คือตัวอย่างของการไม่ปล่อยให้สภาพแวดล้อมที่เป็นพิษมากำหนดชีวิตเรา",
          "sw": "Enoki na watu wake kujenga Sayuni wakati wengine wote walikuwa wakipigana ni mfano bora wa kuchagua mazingira yako mwenyewe badala ya kufuata mkumbo wa vurugu.",
          "it": "Costruire una comunità di pace mentre tutti litigano è la scelta migliore per isolarsi dal caos generale."
        }
    },
    "2026-02-05": {
        date: "2026-02-05",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 7:28-40",
          "en": "Moses 7:28-40",
          "ko": "Moses 7:28-40",
          "zho": "モーセ書 7:28-40",
          "es": "Moses 7:28-40",
          "pt": "Moses 7:28-40",
          "vi": "Moses 7:28-40",
          "tl": "Moses 7:28-40",
          "th": "Moses 7:28-40",
          "sw": "Moses 7:28-40",
          "it": "Mosè 7:28-40"
        },
        comment: {
          "ja": "人類がやらかし続けているのを見て神様が泣いているという事実は、現代のニュースを見ている時の我々の絶望感と完全に一致している。",
          "en": "The idea of God weeping over humanity's constant mess-ups is exactly how I feel watching the news every morning.",
          "ko": "인류가 끊임없이 사고를 치는 모습을 보며 신께서 우신다는 대목은, 매일 뉴스를 보며 느끼는 우리네 절망감과 너무나 닮아있다.",
          "zho": "看著人類不斷闖禍而神在哭泣，這場景簡直就是我們每天看新聞時感到絕望的真實寫照。",
          "es": "La imagen de Dios llorando por nuestras constantes meteduras de pata es exactamente cómo me siento al ver las noticias hoy en día.",
          "pt": "A ideia de Deus chorando pelos nossos erros constantes é exatamente como me sinto ao ver o noticiário hoje em dia.",
          "vi": "Việc Chúa rơi lệ trước những sai lầm liên tiếp của con người cũng giống hệt cảm giác tuyệt vọng của chúng ta khi xem tin tức mỗi ngày.",
          "tl": "Ang pag-iyak ng Diyos sa patuloy na pagkakamali ng sangkatauhan ay parang eksaktong nararamdaman natin tuwing nanonood ng balita.",
          "th": "การที่พระเจ้าทรงพระกันแสงเพราะความผิดพลาดของมนุษย์ช่างตรงกับความรู้สึกสิ้นหวังที่เรามีเวลาเห็นข่าวในทุกวันนี้เหลือเกิน",
          "sw": "Wazo la Mungu kulilia makosa yetu ya kila mara linafanana kabisa na jinsi ninavyohisi ninapotazama habari siku hizi.",
          "it": "Sapere che il cielo si rattrista per i nostri pasticci rispecchia esattamente lo stato d'animo di chi guarda i telegiornali."
        }
    },
    "2026-02-06": {
        date: "2026-02-06",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 7:41-47",
          "en": "Moses 7:41-47",
          "ko": "Moses 7:41-47",
          "zho": "モーセ書 7:41-47",
          "es": "Moses 7:41-47",
          "pt": "Moses 7:41-47",
          "vi": "Moses 7:41-47",
          "tl": "Moses 7:41-47",
          "th": "Moses 7:41-47",
          "sw": "Moses 7:41-47",
          "it": "Mosè 7:41-47"
        },
        comment: {
          "ja": "自分のことだけで手一杯な人間に対し、はるか未来の救済計画まで見通して淡々と語る神様のスケール感、もはやついていけないレベルで凄い。",
          "en": "While we are struggling to get through the day, God is casually mapping out the salvation of all generations, which is just a whole different level of perspective.",
          "ko": "오늘 하루 버티기도 벅찬 우리와 달리 인류 구원이라는 거대한 그림을 차분히 그려가시는 신의 스케일은 정말 따라갈 수가 없다.",
          "zho": "當我們還在為過好今天而掙扎時，神卻在從容地規劃全人類的救贖，這種宏大視角實在令人望塵莫及。",
          "es": "Mientras nosotros luchamos por superar el día a día, Dios está trazando tranquilamente la salvación de todas las generaciones, una perspectiva a otro nivel.",
          "pt": "Enquanto nós lutamos para passar o dia, Deus está calmamente mapeando a salvação de todas as gerações, uma perspectiva em outro nível.",
          "vi": "Trong khi chúng ta còn đang chật vật với cuộc sống thường nhật, Chúa đã thản nhiên vạch ra kế hoạch cứu rỗi cho mọi thế hệ, một tầm nhìn quá đỗi vĩ đại.",
          "tl": "Habang tayo ay hirap na hirap sa araw-araw, kalmado namang pinaplano ng Diyos ang kaligtasan ng lahat, isang perspektibong hindi natin kayang pantayan.",
          "th": "ในขณะที่เรากำลังดิ้นรนกับชีวิตวันต่อวัน พระเจ้ากลับกำลังวางแผนการช่วยให้รอดของทุกยุคสมัยอย่างใจเย็น ซึ่งเป็นมุมมองที่เหนือชั้นจริงๆ",
          "sw": "Wakati sisi tunahangaika na maisha ya kila siku, Mungu anapanga kwa utulivu wokovu wa vizazi vyote, mtazamo ambao hatuwezi kuufikia.",
          "it": "Mentre noi lottiamo per arrivare a sera, il piano universale va avanti con una prospettiva decisamente fuori dal comune."
        }
    },
    "2026-02-07": {
        date: "2026-02-07",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 7:48-59",
          "en": "Moses 7:48-59",
          "ko": "Moses 7:48-59",
          "zho": "モーセ書 7:48-59",
          "es": "Moses 7:48-59",
          "pt": "Moses 7:48-59",
          "vi": "Moses 7:48-59",
          "tl": "Moses 7:48-59",
          "th": "Moses 7:48-59",
          "sw": "Moses 7:48-59",
          "it": "Mosè 7:48-59"
        },
        comment: {
          "ja": "人類の失敗を全部見せられて泣き崩れるエノク、神様の苦労人っぷりを目の当たりにして言葉を失うのは当然の反応だ。",
          "en": "Enoch breaking down in tears after seeing the full scope of human failure is the only sane reaction to witnessing God's long-suffering patience.",
          "ko": "인류의 모든 실패를 보고 눈물을 쏟는 에녹의 마음은, 하나님의 고충을 실시간으로 목격한 사람만이 가질 수 있는 깊은 공감이다.",
          "zho": "以諾看見人類所有的失敗而痛哭流涕，這絕對是目睹了上帝為人類操碎了心之後的真實反應。",
          "es": "Ver a Enoc llorar desconsoladamente ante los errores de la humanidad es la reacción más lógica al darse cuenta de cuánto le duele a Dios vernos fallar.",
          "pt": "Ver Enoque chorar ao ver as falhas da humanidade é a reação mais humana de quem finalmente percebe o quanto Deus se importa conosco.",
          "vi": "Chứng kiến Hê-nóc bật khóc khi nhìn thấu những sai lầm của nhân loại, ta mới hiểu được gánh nặng trong lòng Đấng Tạo Hóa lớn đến nhường nào.",
          "tl": "Ang pag-iyak ni Enoc matapos makita ang lahat ng pagkakamali ng tao ay ang tanging normal na reaksyon kapag nakita mo kung gaano nasasaktan ang Diyos para sa atin.",
          "th": "การที่เอโนคต้องหลั่งน้ำตาเมื่อเห็นความล้มเหลวทั้งหมดของมนุษย์ คือปฏิกิริยาของคนที่เพิ่งเข้าใจว่าพระเจ้าทรงอดทนกับเรามากแค่ไหน",
          "sw": "Enoki kulia baada ya kuona makosa yote ya wanadamu ni jibu la kawaida la mtu anayegundua jinsi Mungu anavyotuvumilia kwa uchungu.",
          "it": "Vedere Enoc scoppiare in pianto davanti ai fallimenti dell'umanita e la reazione piu logica quando ci si rende conto di quanto dolore provi Dio nel vederci sbagliare."
        }
    },
    "2026-02-08": {
        date: "2026-02-08",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 7:60-69",
          "en": "Moses 7:60-69",
          "ko": "Moses 7:60-69",
          "zho": "モーセ書 7:60-69",
          "es": "Moses 7:60-69",
          "pt": "Moses 7:60-69",
          "vi": "Moses 7:60-69",
          "tl": "Moses 7:60-69",
          "th": "Moses 7:60-69",
          "sw": "Moses 7:60-69",
          "it": "Mosè 7:60-69"
        },
        comment: {
          "ja": "文明が滅びても神様は再会を待ち望んでいるという話、どれだけ裏切られても諦めない親の愛の究極系を見せられている気分だ。",
          "en": "The idea that God is waiting for a reunion even after civilization collapses is the ultimate picture of a parent who simply refuses to give up.",
          "ko": "문명이 멸망해도 다시 만날 날을 기다리시는 하나님의 모습은, 자식이 아무리 속을 썩여도 포기하지 않는 부모의 사랑 그 자체다.",
          "zho": "文明崩塌後上帝依然等待著重逢，這簡直就是將父母那種「無論你怎麼叛逆我都不會放棄」的愛展現到了極致。",
          "es": "Saber que Dios espera nuestro reencuentro incluso después de que todo se derrumbe es la prueba definitiva de un amor paternal que no sabe rendirse.",
          "pt": "Saber que Deus espera pelo nosso reencontro mesmo após o colapso da civilização é a prova máxima de um amor de pai que simplesmente não desiste.",
          "vi": "Việc Chúa vẫn chờ đợi ngày đoàn tụ ngay cả khi nền văn minh sụp đổ chính là minh chứng rõ nhất cho tình yêu của người cha không bao giờ từ bỏ con cái.",
          "tl": "Ang malaman na naghihintay ang Diyos sa ating muling pagkikita kahit matapos ang pagkawasak ng lahat ay ang pinakamalinaw na halimbawa ng magulang na hindi sumusuko.",
          "th": "การที่พระเจ้าทรงรอคอยการกลับมาพบกันแม้ในยามที่ทุกอย่างพังทลาย คือนิยามของความรักแบบพ่อแม่ที่ไม่มีวันตัดใจจากลูกได้เลย",
          "sw": "Kujua kuwa Mungu anasubiri tukutane tena hata baada ya kila kitu kuanguka ni uthibitisho wa upendo wa mzazi ambaye hawezi kukata tamaa.",
          "it": "Sapere che Dio aspetta il nostro ritorno anche quando tutto va in pezzi e la prova definitiva di un amore genitoriale che non sa proprio arrendersi."
        }
    },
    "2026-02-09": {
        date: "2026-02-09",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "モーセ書 8:1-30",
          "en": "Moses 8:1-30",
          "ko": "Moses 8:1-30",
          "zho": "モーセ書 8:1-30",
          "es": "Moses 8:1-30",
          "pt": "Moses 8:1-30",
          "vi": "Moses 8:1-30",
          "tl": "Moses 8:1-30",
          "th": "Moses 8:1-30",
          "sw": "Moses 8:1-30",
          "it": "Mosè 8:1-30"
        },
        comment: {
          "ja": "周囲がどれだけ腐敗していても一人黙々と箱舟を作り続けるノア、究極のマイペースというかメンタルの強さがもはや伝説の域。",
          "en": "Noah keeping his cool and building a boat while everyone else is losing their minds is the ultimate example of staying focused in a toxic environment.",
          "ko": "세상이 아무리 타락해도 묵묵히 방주를 짓는 노아를 보면, 멘탈 관리가 인생의 전부라는 생각이 든다.",
          "zho": "在眾人都沉浸在混亂中時，諾亞還能淡定地蓋他的方舟，這心態簡直是職場求生的終極典範。",
          "es": "Noé construyendo su arca mientras todo el mundo a su alrededor se vuelve loco es la lección definitiva sobre cómo mantener la calma en medio del caos.",
          "pt": "Noé construindo a arca enquanto todos ao redor perdiam o juízo é a definição perfeita de manter o foco em um ambiente caótico.",
          "vi": "Việc Nô-ê bình thản đóng tàu trong khi cả thế giới đang náo loạn chính là bài học đỉnh cao về việc giữ vững tinh thần giữa nghịch cảnh.",
          "tl": "Si Noe na matiyagang gumagawa ng arko habang ang lahat ay abala sa kaguluhan ay ang tunay na definition ng pagiging focused sa gitna ng pressure.",
          "th": "การที่โนอาห์ยังคงมุ่งมั่นสร้างเรือท่ามกลางผู้คนที่วุ่นวาย คือตัวอย่างของการมีสมาธิที่น่านับถือที่สุดในโลก",
          "sw": "Nuhu kujenga safina huku kila mtu akipoteza mwelekeo ni mfano bora wa jinsi ya kukaa makini katika mazingira magumu.",
          "it": "Noe che si costruisce la sua arca mentre tutti gli altri perdono la testa e l'esempio supremo di come mantenere la calma in mezzo a un delirio collettivo."
        }
    },
    "2026-02-10": {
        date: "2026-02-10",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 6:1-22",
          "en": "Genesis 6:1-22",
          "ko": "Genesis 6:1-22",
          "zho": "創世記 6:1-22",
          "es": "Genesis 6:1-22",
          "pt": "Genesis 6:1-22",
          "vi": "Genesis 6:1-22",
          "tl": "Genesis 6:1-22",
          "th": "Genesis 6:1-22",
          "sw": "Genesis 6:1-22",
          "it": "Genesi 6:1-22"
        },
        comment: {
          "ja": "何十年もかけて巨大な船を作っているノアを横目に、誰も気に留めなかった当時の人々のスルー力、ある意味で現代のSNSの反応と変わらない。",
          "en": "The fact that nobody cared about Noah building a massive boat for decades shows that human beings have always been experts at ignoring what they don't want to see.",
          "ko": "수십 년 동안 거대한 방주를 짓는 노아를 아무도 신경 쓰지 않았다는 건, 사람들이 보고 싶지 않은 건 끝까지 외면한다는 증거다.",
          "zho": "諾亞蓋了幾十年的方舟卻沒人理會，這種選擇性失明的能力，跟現代人滑過不想面對的新聞簡直如出一轍。",
          "es": "Que nadie prestara atención a Noé mientras construía un barco gigante durante décadas demuestra que somos expertos en ignorar lo que no nos conviene ver.",
          "pt": "O fato de ninguém ter dado bola para Noé construindo uma arca por décadas mostra que o ser humano sempre foi mestre em ignorar o que não quer ver.",
          "vi": "Việc không ai đoái hoài gì đến Nô-ê khi ông đóng con tàu khổng lồ suốt nhiều thập kỷ cho thấy con người vốn dĩ rất giỏi phớt lờ những gì mình không muốn thấy.",
          "tl": "Ang katotohanang walang pumansin kay Noe habang gumagawa siya ng arko sa loob ng ilang dekada ay nagpapakita na likas na sa tao ang magbulag-bulagan sa ayaw nilang harapin.",
          "th": "การที่ไม่มีใครสนใจโนอาห์ขณะสร้างเรือยักษ์ตลอดหลายสิบปี แสดงให้เห็นว่ามนุษย์เราเก่งเรื่องการมองข้ามสิ่งที่ไม่อยากเห็นมานานแล้ว",
          "sw": "Watu kutomjali Nuhu alipokuwa akijenga safina kwa miongo kadhaa inaonyesha jinsi wanadamu walivyo na kipaji cha kupuuza mambo wasiyotaka kukabiliana nayo.",
          "it": "Il fatto che nessuno se ne fregasse di Noe mentre costruiva una nave gigante per decenni dimostra che siamo sempre stati campioni nell'ignorare cio che non vogliamo vedere."
        }
    },
    "2026-02-11": {
        date: "2026-02-11",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 7:1-24",
          "en": "Genesis 7:1-24",
          "ko": "Genesis 7:1-24",
          "zho": "創世記 7:1-24",
          "es": "Genesis 7:1-24",
          "pt": "Genesis 7:1-24",
          "vi": "Genesis 7:1-24",
          "tl": "Genesis 7:1-24",
          "th": "Genesis 7:1-24",
          "sw": "Genesis 7:1-24",
          "it": "Genesi 7:1-24"
        },
        comment: {
          "ja": "動物たちを詰め込んで箱舟の扉を閉めるノア、現代の引っ越しで荷造りが終わらずにパニックになっている自分を重ねてしまう。",
          "en": "Noah loading up the ark and closing the door feels exactly like my own chaotic moving day where nothing fits.",
          "ko": "동물들을 태우고 방주의 문을 닫는 노아를 보니 이삿짐 싸느라 정신없는 제 모습이 떠올라 짠하네요.",
          "zho": "看著挪亞把動物趕進方舟並關上門，那種手忙腳亂的樣子簡直就是我搬家時的真實寫照。",
          "es": "Ver a Noé cerrando la puerta del arca con todos esos animales es como mirarme a mí mismo intentando organizar un caos total antes de un viaje.",
          "pt": "Ver Noé fechando a porta da arca com tantos animais me lembra o desespero de tentar organizar a bagunça antes de uma mudança.",
          "vi": "Nhìn Nô-ê đóng cửa tàu sau khi nhồi nhét bao nhiêu con vật, tôi lại thấy hình ảnh mình đang hoảng loạn vì đóng gói đồ đạc khi chuyển nhà.",
          "tl": "Ang pagpapasok ni Noe sa mga hayop at pagkulong sa ark ay parang ako lang tuwing nag-aayos ng gamit na hindi na kasya sa bag.",
          "th": "เห็นโนอาห์ต้อนสัตว์เข้าเรือและปิดประตูแล้วนึกถึงสภาพตัวเองตอนจัดกระเป๋าเดินทางที่ของล้นจนแทบปิดไม่ได้เลย",
          "sw": "Nuhu akipakia wanyama na kufunga mlango wa safina inanikumbusha jinsi ninavyohangaika kupakia mizigo yangu kabla ya safari.",
          "it": "Noe che chiude la porta dell'arca con tutti quegli animali dentro mi ricorda esattamente il caos dei miei giorni di trasloco quando niente sembra incastrarsi."
        }
    },
    "2026-02-12": {
        date: "2026-02-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 8:1-22",
          "en": "Genesis 8:1-22",
          "ko": "Genesis 8:1-22",
          "zho": "創世記 8:1-22",
          "es": "Genesis 8:1-22",
          "pt": "Genesis 8:1-22",
          "vi": "Genesis 8:1-22",
          "tl": "Genesis 8:1-22",
          "th": "Genesis 8:1-22",
          "sw": "Genesis 8:1-22",
          "it": "Genesi 8:1-22"
        },
        comment: {
          "ja": "カラスとハトを放して外の様子を確認するノア、遠隔操作でドローンを飛ばして外の空気を確かめる現代人の姿と何ら変わりない。",
          "en": "Noah sending out birds to check if it's safe outside is basically the ancient equivalent of checking the weather app before heading out.",
          "ko": "까마귀와 비둘기를 날려 밖을 확인하는 노아는 외출 전 날씨 앱을 켜서 나갈지 말지 고민하는 우리와 다를 게 없네요.",
          "zho": "挪亞放鴿子出去探路，這不就跟我們出門前先看天氣預報一樣，充滿了對不確定性的謹慎。",
          "es": "Noé enviando aves para ver si ya se puede salir es exactamente como revisar el pronóstico del tiempo en el celular antes de decidir si salgo de casa.",
          "pt": "Noé soltando aves para ver se a situação melhorou é o equivalente antigo de checar o aplicativo de previsão do tempo antes de sair de casa.",
          "vi": "Việc Nô-ê thả chim để thăm dò bên ngoài chẳng khác nào việc chúng ta mở app thời tiết kiểm tra trước khi quyết định ra khỏi nhà.",
          "tl": "Ang pagpapakawala ni Noe ng mga ibon para mag-check sa labas ay parang pagtingin lang natin sa weather app bago lumabas ng bahay.",
          "th": "โนอาห์ปล่อยนกออกไปสำรวจข้างนอกก็เหมือนเราที่เช็กแอปพยากรณ์อากาศก่อนจะก้าวเท้าออกจากบ้านนั่นแหละ",
          "sw": "Nuhu akituma ndege kuangalia hali ya nje ni kama sisi tunavyoangalia programu ya hali ya hewa kwenye simu kabla ya kutoka nje.",
          "it": "Noe che manda in giro gli uccelli per vedere se fuori fa bel tempo e praticamente l'antenato di chi controlla l'app meteo prima di uscire di casa."
        }
    },
    "2026-02-13": {
        date: "2026-02-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 9:1-29",
          "en": "Genesis 9:1-29",
          "ko": "Genesis 9:1-29",
          "zho": "創世記 9:1-29",
          "es": "Genesis 9:1-29",
          "pt": "Genesis 9:1-29",
          "vi": "Genesis 9:1-29",
          "tl": "Genesis 9:1-29",
          "th": "Genesis 9:1-29",
          "sw": "Genesis 9:1-29",
          "it": "Genesi 9:1-29"
        },
        comment: {
          "ja": "大洪水から生還した直後にぶどう酒でやらかして息子たちに醜態をさらすノア、どんな聖人でも油断すると人間味全開になるという教訓。",
          "en": "Noah surviving the flood only to get sloppy drunk and embarrass himself in front of his kids is the ultimate reminder that nobody is perfect.",
          "ko": "대홍수에서 살아남은 노아가 포도주에 취해 추태를 부리는 모습은 아무리 대단한 인물도 방심하면 한순간에 무너질 수 있다는 걸 보여준다.",
          "zho": "挪亞躲過了洪水，卻沒躲過酒精的誘惑，這告訴我們即使是聖人，一旦鬆懈也會暴露最真實的人性弱點。",
          "es": "Noé sobreviviendo al diluvio para luego terminar borracho y haciendo un papelón frente a sus hijos es la prueba de que nadie es perfecto.",
          "pt": "Noé sobreviver ao dilúvio para depois se embriagar e passar vergonha na frente dos filhos é a prova de que ninguém é perfeito o tempo todo.",
          "vi": "Noah sống sót qua trận đại hồng thủy chỉ để say xỉn và làm trò cười trước mặt các con, một lời nhắc nhở rằng ai cũng có lúc yếu lòng.",
          "tl": "Si Noe na nakaligtas sa baha pero nalasing at napahiya sa harap ng mga anak ay patunay na kahit ang pinakabanal ay may kahinaan din.",
          "th": "โนอาห์รอดพ้นจากน้ำท่วมโลกมาได้แต่กลับมาพลาดท่าเมามายจนเสียอาการต่อหน้าลูกๆ เป็นเครื่องเตือนใจว่าคนเราพลาดกันได้ทุกเมื่อ",
          "sw": "Nuhu kunusurika gharika kisha kulewa na kujiaibisha mbele ya wanawe ni ukumbusho kwamba hata mtu mkuu anaweza kuwa na udhaifu wa kibinadamu.",
          "it": "Noe che sopravvive al diluvio universale solo per fare una pessima figura davanti ai figli dimostra che nessuno e immune ai momenti di totale assurdita."
        }
    },
    "2026-02-14": {
        date: "2026-02-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 10:1-32",
          "en": "Genesis 10:1-32",
          "ko": "Genesis 10:1-32",
          "zho": "創世記 10:1-32",
          "es": "Genesis 10:1-32",
          "pt": "Genesis 10:1-32",
          "vi": "Genesis 10:1-32",
          "tl": "Genesis 10:1-32",
          "th": "Genesis 10:1-32",
          "sw": "Genesis 10:1-32",
          "it": "Genesi 10:1-32"
        },
        comment: {
          "ja": "延々と続く系図のリストは、当時の人々が自分たちのルーツを必死に記録しようとした、現代のSNSのフォロワーリスト以上に切実な生存証明書。",
          "en": "All those long lists of names are basically ancient attempts at tracking family roots, which is honestly more intense than any modern social media follow list.",
          "ko": "끝없이 이어지는 족보 명단은 당시 사람들이 자신의 뿌리를 증명하려 했던 필사적인 노력으로, 현대의 SNS 친구 목록보다 훨씬 더 무겁게 다가온다.",
          "zho": "那一長串沒完沒了的家譜名單，其實就是古代人為了證明自己存在而留下的備份，比現在的社群媒體追蹤名單還要嚴肅。",
          "es": "Esas listas interminables de nombres son básicamente el intento antiguo de rastrear sus raíces, algo mucho más serio que cualquier lista de seguidores actual.",
          "pt": "Essas listas intermináveis de nomes são apenas a tentativa antiga de registrar raízes, algo bem mais intenso do que qualquer lista de seguidores de rede social.",
          "vi": "Những danh sách gia phả dài dằng dặc ấy thực chất là cách người xưa lưu giữ nguồn cội, một bản ghi chép còn nghiêm túc hơn cả danh sách bạn bè trên mạng xã hội.",
          "tl": "Ang mahabang listahan ng mga pangalan ay pagtatangka ng mga sinaunang tao na itala ang kanilang pinagmulan, na mas seryoso pa sa kahit anong listahan ng followers ngayon.",
          "th": "รายชื่อบรรพบุรุษที่ยาวเหยียดคือความพยายามอย่างยิ่งยวดของคนสมัยก่อนในการบันทึกรากเหง้าของตน ซึ่งดูจริงจังกว่ารายชื่อผู้ติดตามในโซเชียลยุคนี้เสียอีก",
          "sw": "Orodha ndefu ya majina ni juhudi za watu wa kale kuhifadhi asili yao, jambo ambalo ni zito kuliko orodha yoyote ya wafuasi kwenye mitandao ya kijamii leo.",
          "it": "Quelle liste infinite di nomi sono fondamentalmente il tentativo dei nostri antenati di tracciare le proprie origini, molto piu serio di qualsiasi elenco di follower odierno."
        }
    },
    "2026-02-15": {
        date: "2026-02-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 11:1-32",
          "en": "Genesis 11:1-32",
          "ko": "Genesis 11:1-32",
          "zho": "創世記 11:1-32",
          "es": "Genesis 11:1-32",
          "pt": "Genesis 11:1-32",
          "vi": "Genesis 11:1-32",
          "tl": "Genesis 11:1-32",
          "th": "Genesis 11:1-32",
          "sw": "Genesis 11:1-32",
          "it": "Genesi 11:1-32"
        },
        comment: {
          "ja": "天まで届く塔を建てようとするバベルの人々の壮大な計画も、結局は言葉が通じないだけで瓦解するあたり、現代の会議の風景を見ているようで胃が痛くなる。",
          "en": "Watching the entire Tower of Babel project crumble just because of a communication breakdown is basically every corporate meeting I have ever attended.",
          "ko": "하늘까지 닿는 탑을 쌓으려던 바벨 사람들의 거창한 계획이 결국 소통의 부재로 무너지는 모습은 오늘날의 회의실 풍경을 보는 것 같아 마음이 아프다.",
          "zho": "看著巴別塔的人們試圖建造通天塔，最後卻因為語言不通而崩盤，這簡直就是現代職場溝通失敗的真實寫照。",
          "es": "Ver cómo todo el proyecto de la Torre de Babel se viene abajo solo por una falta de comunicación es básicamente cualquier reunión de trabajo que he tenido.",
          "pt": "Ver todo o projeto da Torre de Babel desmoronar apenas por uma falha de comunicação é basicamente qualquer reunião de trabalho que já tive.",
          "vi": "Chứng kiến cả dự án Tháp Babel sụp đổ chỉ vì sự cố giao tiếp cơ bản thực sự giống như bất kỳ cuộc họp công ty nào mà tôi từng tham gia.",
          "tl": "Ang makita ang pagbagsak ng buong proyekto ng Tore ni Babel dahil lang sa hindi pagkakaunawaan ay parang anumang meeting sa opisina na naranasan ko na.",
          "th": "เห็นโครงการหอคอยบาเบลพังทลายลงเพียงเพราะการสื่อสารล้มเหลว มันก็เหมือนกับการประชุมในออฟฟิศที่ฉันเคยเจอมาไม่มีผิด",
          "sw": "Kuona mradi mzima wa Mnara wa Babeli ukiporomoka kwa sababu tu ya kutoelewana ni kama mikutano yote ya kazini niliyowahi kuhudhuria.",
          "it": "Vedere il progetto della Torre di Babel fallire unicamente per un problema di comunicazione mi ricorda fin troppo certe riunioni di lavoro interminabili."
        }
    },
    "2026-02-16": {
        date: "2026-02-16",
        scripture: {
          "ja": "高価な真珠",
          "en": "Pearl of Great Price",
          "ko": "Pearl of Great Price",
          "zho": "高価な真珠",
          "es": "Pearl of Great Price",
          "pt": "Pearl of Great Price",
          "vi": "Pearl of Great Price",
          "tl": "Pearl of Great Price",
          "th": "Pearl of Great Price",
          "sw": "Pearl of Great Price",
          "it": "Perla di Gran Prezzo"
        },
        chapter: {
          "ja": "アブラハム書 1:1-31",
          "en": "Abraham 1:1-31",
          "ko": "Abraham 1:1-31",
          "zho": "アブラハム書 1:1-31",
          "es": "Abraham 1:1-31",
          "pt": "Abraham 1:1-31",
          "vi": "Abraham 1:1-31",
          "tl": "Abraham 1:1-31",
          "th": "Abraham 1:1-31",
          "sw": "Abraham 1:1-31",
          "it": "Abramo 1:1-31"
        },
        comment: {
          "ja": "神を信じようとしたアブラハムが、いきなり祭壇に縛り付けられるという理不尽な試練に直面する様子は、正しいことをしようとする時ほど逆風が吹く現実を突きつけてくる。",
          "en": "Abraham trying to do the right thing only to end up tied to an altar is a stark reminder that sometimes the universe tests you the hardest when you are trying your best.",
          "ko": "옳은 일을 하려던 아브라함이 제단에 묶이는 부당한 시련을 겪는 모습은, 진심으로 잘해보려 할 때 오히려 더 큰 역풍이 부는 현실을 보여주는 듯하다.",
          "zho": "亞伯拉罕試圖做正確的事，最後卻被綁在祭壇上，這提醒了我們，當你越想做好事時，往往會面臨最不可理喻的考驗。",
          "es": "Abraham intentando hacer lo correcto y terminando atado a un altar nos recuerda que, a veces, la vida te pone a prueba justo cuando más te esfuerzas por ser bueno.",
          "pt": "Abraão tentando fazer a coisa certa e acabando amarrado a um altar nos lembra que, às vezes, a vida nos testa justamente quando estamos tentando ser melhores.",
          "vi": "Abraham cố gắng làm điều đúng đắn nhưng lại bị trói vào bàn thờ là lời nhắc nhở rằng đôi khi cuộc đời thử thách ta khắc nghiệt nhất ngay lúc ta đang nỗ lực hết mình.",
          "tl": "Ang pagtatangka ni Abraham na gawin ang tama pero nauwi sa pagkatali sa altar ay isang paalala na kung minsan, sinusubok ka ng tadhana nang matindi kapag sinusubukan mong magpakabuti.",
          "th": "อับราฮัมพยายามทำสิ่งที่ถูกต้องแต่กลับต้องถูกมัดไว้ที่แท่นบูชา เป็นเครื่องเตือนใจว่าบางครั้งชีวิตก็ทดสอบเราหนักที่สุดในตอนที่เราพยายามทำดีที่สุดแล้ว",
          "sw": "Abraham kujaribu kufanya jambo sahihi na kuishia kufungwa kwenye madhabahu ni ukumbusho kwamba wakati mwingine maisha hukujaribu sana unapojitahidi kufanya mema.",
          "it": "Abrahamo che cerca di fare la cosa giusta e finisce legato su un altare ci ricorda che a volte la vita ti mette alla prova proprio quando ce la metti tutta."
        }
    },
    "2026-02-17": {
        date: "2026-02-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 12:1-9",
          "en": "Genesis 12:1-9",
          "ko": "Genesis 12:1-9",
          "zho": "創世記 12:1-9",
          "es": "Genesis 12:1-9",
          "pt": "Genesis 12:1-9",
          "vi": "Genesis 12:1-9",
          "tl": "Genesis 12:1-9",
          "th": "Genesis 12:1-9",
          "sw": "Genesis 12:1-9",
          "it": "Genesi 12:1-9"
        },
        comment: {
          "ja": "行き先も告げられずに引っ越しを命じられるアブラム、神様の無茶ぶりにもほどがあるけれど、それでも準備して出発するあたりが彼のすごいところだ。",
          "en": "Being told to pack your bags for an unknown destination is the ultimate leap of faith, but Abram just treating it like a normal Tuesday is honestly impressive.",
          "ko": "어디로 가는지도 모른 채 일단 떠나라는 하나님의 명령에 짐부터 싸고 보는 아브람의 순종은 가끔 이해를 뛰어넘는 대범함이 있다.",
          "zho": "連目的地都不知道就被要求搬家，亞伯蘭這種說走就走的執行力，簡直是現代人拖延症的終極反面教材。",
          "es": "Que te digan que te mudes sin decirte a dónde es el nivel máximo de confianza ciega, pero Abram simplemente empezó a empacar como si nada.",
          "pt": "Receber uma ordem para se mudar sem saber o destino é o teste supremo de confiança, mas Abrão simplesmente começou a arrumar as malas como se fosse um dia comum.",
          "vi": "Được lệnh dọn nhà mà không biết đi đâu, Abram cứ thế lên đường như thể đó là một việc hết sức bình thường, thật là một tinh thần thép.",
          "tl": "Ang utos na lumipat nang hindi alam ang destinasyon ay isang matinding pagsubok, pero si Abram ay naghanda na lang na parang normal na araw lang iyon.",
          "th": "การถูกสั่งให้ย้ายบ้านโดยไม่บอกจุดหมายคือบททดสอบความเชื่อที่หนักหนา แต่ดูเหมือนอับรามจะเตรียมตัวออกเดินทางเหมือนเป็นเรื่องปกติธรรมดามาก",
          "sw": "Kuambiwa uhame bila kuambiwa unaenda wapi ni jaribu kubwa la imani, lakini Abramu kuanza safari kama jambo la kawaida ni jambo la kushangaza.",
          "it": "Partire per una destinazione sconosciuta senza un navigatore è il massimo salto nel buio, ma Abramo ha fatto i bagagli come se fosse un martedì qualunque."
        }
    },
    "2026-02-18": {
        date: "2026-02-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 12:10-20",
          "en": "Genesis 12:10-20",
          "ko": "Genesis 12:10-20",
          "zho": "創世記 12:10-20",
          "es": "Genesis 12:10-20",
          "pt": "Genesis 12:10-20",
          "vi": "Genesis 12:10-20",
          "tl": "Genesis 12:10-20",
          "th": "Genesis 12:10-20",
          "sw": "Genesis 12:10-20",
          "it": "Genesi 12:10-20"
        },
        comment: {
          "ja": "異国の地で怖くなって妻を妹と偽るアブラム、保身に走るあまりの人間臭さに、完璧な聖人なんてどこにもいないんだと安心する。",
          "en": "Abram lying to save his own skin by calling his wife his sister is such a messy, human move that it makes him feel strangely relatable.",
          "ko": "낯선 땅에서 겁이 나 아내를 누이라고 속이는 아브람을 보니, 성경 속 인물들도 결국 우리처럼 고민하고 실수하는 평범한 사람이라는 게 느껴진다.",
          "zho": "亞伯蘭為了自保竟然謊稱妻子是妹妹，這種關鍵時刻掉鏈子的真實感，反而讓人覺得這故事沒什麼距離感。",
          "es": "Que Abram mienta diciendo que su esposa es su hermana por miedo es un gesto tan humano y desesperado que lo hace sentir increíblemente cercano.",
          "pt": "Abrão mentir dizendo que sua esposa era sua irmã por medo é uma atitude tão humana e falha que acaba tornando o personagem estranhamente próximo de nós.",
          "vi": "Việc Abram nói dối vợ mình là em gái vì sợ hãi là một hành động rất con người, cho thấy ngay cả những người vĩ đại cũng có lúc hoảng loạn.",
          "tl": "Ang pagsisinungaling ni Abram na kapatid niya ang kanyang asawa dahil sa takot ay isang makataong kahinaan na nagpapakita na hindi perpekto ang sinuman.",
          "th": "การที่อับรามโกหกว่าภรรยาเป็นน้องสาวเพราะความกลัว เป็นความผิดพลาดที่ดูเป็นมนุษย์สุดๆ จนทำให้เขารู้สึกเข้าถึงได้ง่ายขึ้นมาก",
          "sw": "Abramu kusema uongo kuwa mkewe ni dada yake kwa sababu ya hofu ni udhaifu wa kibinadamu unaotukumbusha kuwa hata watu wakuu wana nyakati za hofu.",
          "it": "Mentire sulla propria moglie per paura di rimetterci la pelle è una mossa così disperata e umana che rende Abramo stranamente vicino a noi."
        }
    },
    "2026-02-19": {
        date: "2026-02-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 13:1-18",
          "en": "Genesis 13:1-18",
          "ko": "Genesis 13:1-18",
          "zho": "創世記 13:1-18",
          "es": "Genesis 13:1-18",
          "pt": "Genesis 13:1-18",
          "vi": "Genesis 13:1-18",
          "tl": "Genesis 13:1-18",
          "th": "Genesis 13:1-18",
          "sw": "Genesis 13:1-18",
          "it": "Genesi 13:1-18"
        },
        comment: {
          "ja": "財産が増えた途端に部下同士が揉め始めるのは、いつの時代も変わらない人間関係の縮図ですね。",
          "en": "The moment the bank account grows, the staff starts squabbling; some things never change.",
          "ko": "재산이 늘어나자마자 부하들이 싸우기 시작하는 모습은 예나 지금이나 변함없는 인간관계의 현실입니다.",
          "zho": "財產一增加部下就開始吵架，這真是跨越時代的職場人際關係縮影。",
          "es": "Que los empleados empiecen a pelearse apenas aumenta la riqueza es la realidad de toda la vida en cualquier oficina.",
          "pt": "O pessoal começar a brigar assim que a conta bancária cresce é um clássico atemporal das relações humanas.",
          "vi": "Việc nhân viên cãi nhau ngay khi tài sản tăng lên chính là hình ảnh thu nhỏ của những rắc rối nhân sự muôn thuở.",
          "tl": "Ang mag-away ang mga tauhan kapag dumami ang pera ay isang klasikong problema sa opisina na hanggang ngayon ay totoo pa rin.",
          "th": "พอรวยขึ้นแล้วลูกน้องเริ่มทะเลาะกันเอง นี่มันปัญหาคลาสสิกของคนทำงานทุกยุคสมัยชัดๆ",
          "sw": "Wafanyakazi kuanza kugombana mara tu mali inapoongezeka ni hali ya kawaida ya mahusiano ya kibinadamu tangu zamani.",
          "it": "Appena il conto in banca cresce, i collaboratori iniziano a litigare, e certe dinamiche d'ufficio non cambiano proprio mai."
        }
    },
    "2026-02-20": {
        date: "2026-02-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 14:17-24",
          "en": "Genesis 14:17-24",
          "ko": "Genesis 14:17-24",
          "zho": "創世記 14:17-24",
          "es": "Genesis 14:17-24",
          "pt": "Genesis 14:17-24",
          "vi": "Genesis 14:17-24",
          "tl": "Genesis 14:17-24",
          "th": "Genesis 14:17-24",
          "sw": "Genesis 14:17-24",
          "it": "Genesi 14:17-24"
        },
        comment: {
          "ja": "王様からのお礼をきっぱり断るアブラム、損得勘定が先行する世の中で一番かっこいい生き方かもしれない。",
          "en": "Abram turning down the king's riches is the ultimate power move in a world obsessed with bottom lines.",
          "ko": "왕의 보상을 단칼에 거절하는 아브람의 모습은, 손익계산이 판치는 세상에서 가장 멋진 태도입니다.",
          "zho": "亞伯蘭斷然拒絕國王的賞賜，在凡事講求利益的世道裡，這簡直是最帥氣的生存哲學。",
          "es": "Rechazar las riquezas del rey con tanta elegancia es la mejor forma de decir que uno tiene prioridades más altas.",
          "pt": "Recusar as riquezas do rei de forma tão direta é uma lição de integridade em um mundo que só pensa em lucros.",
          "vi": "Việc Áp-ram từ chối của cải của vua một cách dứt khoát là hành động đỉnh cao trong một thế giới chỉ biết tính toán lợi ích.",
          "tl": "Ang pagtanggi ni Abram sa kayamanan ng hari ay isang napakagandang halimbawa ng pagpapahalaga sa prinsipyo higit sa pera.",
          "th": "การที่อับรามปฏิเสธของกำนัลจากกษัตริย์อย่างเด็ดขาด เป็นจุดยืนที่เท่ที่สุดในโลกที่เอาแต่คิดเรื่องผลประโยชน์แบบนี้",
          "sw": "Abram kukataa utajiri wa mfalme kwa ujasiri ni mfano bora wa kuwa na msimamo katika ulimwengu unaopenda faida tu.",
          "it": "Rifiutare le ricchezze del re con tanta naturalezza è la mossa di classe definitiva in un mondo ossessionato dal profitto."
        }
    },
    "2026-02-21": {
        date: "2026-02-21",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 16:1-16",
          "en": "Genesis 16:1-16",
          "ko": "Genesis 16:1-16",
          "zho": "創世記 16:1-16",
          "es": "Genesis 16:1-16",
          "pt": "Genesis 16:1-16",
          "vi": "Genesis 16:1-16",
          "tl": "Genesis 16:1-16",
          "th": "Genesis 16:1-16",
          "sw": "Genesis 16:1-16",
          "it": "Genesi 16:1-16"
        },
        comment: {
          "ja": "神の約束を待ちきれずに自分で解決策をひねり出したサライとアブラム、その焦りが後の壮大な家庭内トラブルの種になるあたりが人間臭くて切ない。",
          "en": "Sarai and Abram trying to help God out with a shortcut is a classic case of impatience creating a mess that lasts for generations.",
          "ko": "하나님의 약속을 기다리지 못하고 스스로 해결책을 찾으려던 사래와 아브람의 조급함은 우리 모두가 한 번쯤 겪는 실수와 너무 닮았다.",
          "zho": "撒拉和亚伯兰因为等不及神的应许而自作聪明，这种想要走捷径的焦急心态，果然是人类的老毛病。",
          "es": "Tratar de ayudar a Dios con atajos porque no tenemos paciencia es la forma más rápida de complicar nuestro futuro.",
          "pt": "Tentar dar um empurrãozinho nos planos de Deus por falta de paciência é a receita perfeita para um desastre familiar que dura gerações.",
          "vi": "Việc Sarai và Abram mất kiên nhẫn rồi tự tìm đường tắt cho lời hứa của Chúa chính là sai lầm kinh điển mà ai trong chúng ta cũng từng mắc phải.",
          "tl": "Ang paggawa ng sariling paraan dahil hindi na makapaghintay sa pangako ng Diyos ay isang klasikong pagkakamali na nagbubunga ng gulo.",
          "th": "การพยายามหาทางลัดเพราะรอคำสัญญาของพระเจ้าไม่ไหว คือความใจร้อนที่มักจะสร้างปัญหาใหญ่ในระยะยาวให้กับทุกคน",
          "sw": "Kujaribu kumsaidia Mungu kwa njia za mkato kwa sababu ya kukosa subira ni kosa la kibinadamu linaloleta matatizo yasiyoisha.",
          "it": "Cercare di dare una mano ai piani alti inventandosi scorciatoie è il modo perfetto per complicarsi la vita per generazioni."
        }
    },
    "2026-02-22": {
        date: "2026-02-22",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 17:1-27",
          "en": "Genesis 17:1-27",
          "ko": "Genesis 17:1-27",
          "zho": "創世記 17:1-27",
          "es": "Genesis 17:1-27",
          "pt": "Genesis 17:1-27",
          "vi": "Genesis 17:1-27",
          "tl": "Genesis 17:1-27",
          "th": "Genesis 17:1-27",
          "sw": "Genesis 17:1-27",
          "it": "Genesi 17:1-27"
        },
        comment: {
          "ja": "99歳になって神から名前を変えろと言われ、思わず吹き出して笑ってしまうアブラハム、聖人だって無理難題にはツッコミを入れたくなるものだ。",
          "en": "Abraham laughing at the idea of becoming a father at ninety-nine is the most honest reaction to a divine plot twist I have ever heard.",
          "ko": "아흔아홉 살에 아버지가 될 거라는 말을 듣고 헛웃음을 터뜨리는 아브라함이야말로 성경에서 가장 인간미 넘치는 장면 중 하나다.",
          "zho": "九十九岁时被告知要当爸爸而忍不住笑出声的亚伯拉罕，这种对神圣任务的真实反应实在太可爱了。",
          "es": "Que Abraham se riera ante la idea de ser padre a los noventa y nueve años demuestra que hasta los grandes profetas tienen sus momentos de incredulidad.",
          "pt": "Rir quando se ouve algo absurdo, mesmo vindo de Deus, é a reação mais humana e honesta que Abraão poderia ter tido aos noventa e nove anos.",
          "vi": "Phản ứng bật cười của Abraham khi nghe tin mình sẽ làm cha ở tuổi chín mươi chín chính là biểu cảm chân thật nhất trước một tình huống không tưởng.",
          "tl": "Ang pagtawa ni Abraham nang sabihan siyang magiging ama sa edad na siyamnapu't siyam ay ang pinaka-totoong reaksyon sa isang imposibleng sitwasyon.",
          "th": "การที่อับราฮัมหัวเราะออกมาเมื่อได้ยินว่าจะได้เป็นพ่อคนในวัยเก้าสิบเก้าปี เป็นปฏิกิริยาที่ดูเป็นมนุษย์ที่สุดเท่าที่จะจินตนาการได้",
          "sw": "Abraham kucheka baada ya kuambiwa atakuwa baba akiwa na umri wa miaka tisini na tisa ni mwitikio wa kweli kabisa wa kibinadamu mbele ya jambo lisilowezekana.",
          "it": "Farsi una risata alla sola idea di diventare genitori a novantanove anni è la reazione più onesta a un colpo di scena divino."
        }
    },
    "2026-02-23": {
        date: "2026-02-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 18:1-33",
          "en": "Genesis 18:1-33",
          "ko": "Genesis 18:1-33",
          "zho": "創世記 18:1-33",
          "es": "Genesis 18:1-33",
          "pt": "Genesis 18:1-33",
          "vi": "Genesis 18:1-33",
          "tl": "Genesis 18:1-33",
          "th": "Genesis 18:1-33",
          "sw": "Genesis 18:1-33",
          "it": "Genesi 18:1-33"
        },
        comment: {
          "ja": "神の使いが突然来訪したとき、とっさに最高級の料理を振る舞おうとするアブラハムのホスピタリティ精神、もはや過剰なまでの気遣いで親近感がわく。",
          "en": "Abraham immediately scrambling to serve his surprise guests the best meal possible is the ultimate peak of relatable over-accommodating energy.",
          "ko": "갑자기 찾아온 손님들에게 최고급 요리를 대접하려고 동분서주하는 아브라함의 모습은, 손님 접대에 진심인 한국인의 정서를 보는 것 같다.",
          "zho": "亞伯拉罕看到陌生人突然造訪，立刻忙進忙出準備豪華大餐，這種為了款待客人而過度緊張的模樣簡直是現代人的寫照。",
          "es": "Abraham apresurándose a cocinar un banquete para unos desconocidos a la carrera es el retrato perfecto de alguien que quiere quedar bien a toda costa.",
          "pt": "Abraão correndo para preparar um banquete para visitas inesperadas é a prova de que o desespero para ser um bom anfitrião é algo universal.",
          "vi": "Abraham vội vã chuẩn bị bữa ăn thịnh soạn cho những vị khách bất ngờ ghé thăm chính là hiện thân của tâm lý muốn chu toàn mọi thứ trong tích tắc.",
          "tl": "Ang pagmamadali ni Abraham na maghanda ng piging para sa mga hindi inaasahang bisita ay tunay na sumasalamin sa pagiging sobrang mapag-asikong tao.",
          "th": "อับราฮัมรีบเตรียมอาหารชุดใหญ่ต้อนรับแขกไม่ได้รับเชิญทันที เป็นภาพสะท้อนความกระตือรือร้นในการเป็นเจ้าบ้านที่ดีที่ดูแล้วอบอุ่นใจ",
          "sw": "Ibrahim kukimbia kuandaa karamu kwa wageni wasiotarajiwa ni kielelezo kamili cha mtu anayejaribu kufanya kila liwezekanalo ili kuwa mwenyeji bora.",
          "it": "Muoversi all'istante per cucinare un banchetto ai primi ospiti di passaggio è la quintessenza dell'ansia da padrone di casa perfetto."
        }
    },
    "2026-02-24": {
        date: "2026-02-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 19:1-38",
          "en": "Genesis 19:1-38",
          "ko": "Genesis 19:1-38",
          "zho": "創世記 19:1-38",
          "es": "Genesis 19:1-38",
          "pt": "Genesis 19:1-38",
          "vi": "Genesis 19:1-38",
          "tl": "Genesis 19:1-38",
          "th": "Genesis 19:1-38",
          "sw": "Genesis 19:1-38",
          "it": "Genesi 19:1-38"
        },
        comment: {
          "ja": "逃げる途中で未練たらしく後ろを振り返って塩の柱になってしまうロトの妻、過去を捨てきれない人間の弱さをこれ以上ないほど強烈に体現している。",
          "en": "Lot’s wife turning into a pillar of salt because she just couldn't resist one last look back is the ultimate warning against scrolling through your ex's social media.",
          "ko": "도망치는 와중에 미련을 버리지 못하고 뒤를 돌아보다가 소금 기둥이 된 롯의 아내, 과거에 집착하는 인간의 심리를 이보다 더 확실하게 보여줄 순 없다.",
          "zho": "羅得的妻子在逃難時忍不住回頭看，結果變成鹽柱，這簡直是告訴我們不要對過去念念不忘的最慘痛教訓。",
          "es": "La esposa de Lot convirtiéndose en estatua de sal por no poder evitar echar un último vistazo al pasado es la lección más drástica y memorable sobre soltar lo que ya fue.",
          "pt": "A esposa de Ló virando uma estátua de sal por não resistir a uma última olhadinha para trás é o lembrete definitivo de que olhar para o passado pode custar caro.",
          "vi": "Vợ của Lót biến thành cột muối chỉ vì không cưỡng lại được việc ngoái nhìn lại quá khứ là bài học đắt giá nhất về việc buông bỏ.",
          "tl": "Ang pagiging haligi ng asin ng asawa ni Lot dahil hindi mapigilang lumingon sa nakaraan ay ang pinakamabigat na paalala na huwag nang lingunin ang mga bagay na dapat nang iwanan.",
          "th": "ภรรยาของโลทกลายเป็นเสาเกลือเพียงเพราะอดใจไม่ไหวที่จะหันกลับไปมองอดีต เป็นบทเรียนที่เจ็บปวดที่สุดว่าบางครั้งการปล่อยวางก็จำเป็นจริงๆ",
          "sw": "Mke wa Lutu kugeuka kuwa nguzo ya chumvi kwa sababu hakuweza kujizuia kutazama nyuma ni onyo kali zaidi dhidi ya kung'ang'ania yaliyopita.",
          "it": "Voltarsi indietro per un'ultima occhiata e finire trasformati in sale è la lezione più drastica di sempre sull'importanza di lasciar andare il passato."
        }
    },
    "2026-02-25": {
        date: "2026-02-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 20:1-18",
          "en": "Genesis 20:1-18",
          "ko": "Genesis 20:1-18",
          "zho": "創世記 20:1-18",
          "es": "Genesis 20:1-18",
          "pt": "Genesis 20:1-18",
          "vi": "Genesis 20:1-18",
          "tl": "Genesis 20:1-18",
          "th": "Genesis 20:1-18",
          "sw": "Genesis 20:1-18",
          "it": "Genesi 20:1-18"
        },
        comment: {
          "ja": "妻を妹だと言い張るアブラハムの保身術、夫婦の危機管理としてはあまりに人間臭すぎて苦笑いしか出てこない。",
          "en": "Abraham trying to pass off his wife as his sister to save his own skin is the kind of messy human survival tactic that makes you wince.",
          "ko": "아내를 여동생이라고 우기는 아브라함의 생존 전략은 너무 인간적이라 오히려 짠하게 느껴진다.",
          "zho": "亞伯拉罕為了自保謊稱妻子是妹妹，這種極度人性化的求生欲讓人尷尬到想笑。",
          "es": "Abraham diciendo que su esposa era su hermana por puro miedo es esa clase de instinto de supervivencia que nos hace reírnos de nuestra propia torpeza.",
          "pt": "Abraão tentando passar a esposa por irmã para se salvar é aquele tipo de atitude humana atrapalhada que nos faz rir de nervoso.",
          "vi": "Abraham nói dối vợ mình là em gái để tự bảo vệ bản thân là một tình huống đầy tính người đến mức dở khóc dở cười.",
          "tl": "Ang pagpapanggap ni Abraham na kapatid niya ang kanyang asawa para lang makaligtas ay isang nakakatawang halimbawa ng pagiging tao.",
          "th": "การที่อับราฮัมบอกว่าภรรยาเป็นน้องสาวเพียงเพื่อเอาตัวรอดนั้น เป็นความย้อนแย้งของมนุษย์ที่ชวนให้ขำไม่ออกจริงๆ",
          "sw": "Abraham kusema mkewe ni dada yake ili ajilinde ni ule udhaifu wa kibinadamu unaotufanya tutabasamu kwa aibu.",
          "it": "Spacciare la propria moglie per la sorella per puro istinto di conservazione è quella tattica di sopravvivenza che strappa un amaro sorriso."
        }
    },
    "2026-02-26": {
        date: "2026-02-26",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 21:1-21",
          "en": "Genesis 21:1-21",
          "ko": "Genesis 21:1-21",
          "zho": "創世記 21:1-21",
          "es": "Genesis 21:1-21",
          "pt": "Genesis 21:1-21",
          "vi": "Genesis 21:1-21",
          "tl": "Genesis 21:1-21",
          "th": "Genesis 21:1-21",
          "sw": "Genesis 21:1-21",
          "it": "Genesi 21:1-21"
        },
        comment: {
          "ja": "長年待ち続けた子供がようやく生まれたのに、今度は家庭内のいざこざで追い出されるハガル、人生のハードモードぶりが切なすぎる。",
          "en": "Hagar getting evicted after finally reaching a milestone of happiness is a brutal reminder that life doesn't always go according to plan.",
          "ko": "오랜 기다림 끝에 아들을 얻었지만 곧바로 쫓겨나야 하는 하갈의 신세는 인생의 매운맛을 그대로 보여준다.",
          "zho": "夏甲在終於等到孩子出生後卻面臨被逐出的命運，這種突如其來的現實打擊簡直是人生的硬核模式。",
          "es": "Agar finalmente tiene a su hijo y termina expulsada; a veces la vida nos da una alegría y un problema al mismo tiempo.",
          "pt": "Hagar finalmente tem seu filho e acaba sendo expulsa; a vida tem dessas ironias cruéis que nos deixam sem chão.",
          "vi": "Hagar vừa mới có được niềm vui làm mẹ thì lại bị đuổi đi, cuộc đời quả thật luôn biết cách thử thách chúng ta vào lúc không ngờ nhất.",
          "tl": "Ang pagpapalayas kay Hagar matapos niyang magkaanak ay isang malupit na paalala na ang buhay ay hindi laging patas.",
          "th": "ฮาการ์เพิ่งได้ลูกมาแต่กลับต้องถูกไล่ออกไป ชีวิตคนเรามักจะมีบททดสอบโหดๆ มาแบบไม่ทันตั้งตัวเสมอ",
          "sw": "Hagar kupata mtoto baada ya kusubiri muda mrefu kisha kufukuzwa mara moja ni ukumbusho mchungu wa jinsi maisha yanavyoweza kubadilika ghafla.",
          "it": "Ritrovarsi a fare i conti con la dura realtà dopo un momento di pura gioia ci ricorda che la vita ama metterci alla prova proprio quando ci sentiamo sicuri."
        }
    },
    "2026-02-27": {
        date: "2026-02-27",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 21:22-34",
          "en": "Genesis 21:22-34",
          "ko": "Genesis 21:22-34",
          "zho": "創世記 21:22-34",
          "es": "Genesis 21:22-34",
          "pt": "Genesis 21:22-34",
          "vi": "Genesis 21:22-34",
          "tl": "Genesis 21:22-34",
          "th": "Genesis 21:22-34",
          "sw": "Genesis 21:22-34",
          "it": "Genesi 21:22-34"
        },
        comment: {
          "ja": "アブラハムが井戸を巡って近隣住民と契約を結ぶ様子は、現代の地主トラブルや近所付き合いの苦労と地続きすぎて笑ってしまう。",
          "en": "Abraham navigating border disputes and water rights with his neighbors feels like a suburban HOA nightmare brought to life.",
          "ko": "아브라함이 우물 문제로 이웃과 계약을 맺는 모습은 현대인의 층간소음이나 주차 갈등만큼이나 현실적인 고충으로 다가온다.",
          "zho": "亚伯拉罕为了水井问题与邻居立约，这种为了生活琐事与人周旋的场面，简直是现代邻里关系的真实写照。",
          "es": "Ver a Abraham negociando los derechos de un pozo con sus vecinos es un recordatorio de que los problemas de comunidad existen desde el principio de los tiempos.",
          "pt": "Abraham negociando o uso de um poço com os vizinhos mostra que brigas de condomínio e disputas de limite de terreno são problemas milenares.",
          "vi": "Việc Abraham phải đàm phán về giếng nước với hàng xóm cho thấy những rắc rối về quyền lợi đất đai đã tồn tại từ thời cổ đại.",
          "tl": "Ang pakikipagkasundo ni Abraham sa kanyang mga kapitbahay tungkol sa balon ay parang modernong awayan sa subdivision na hindi natatapos.",
          "th": "การที่อับราฮัมต้องเจรจาเรื่องบ่อน้ำกับเพื่อนบ้านทำให้เห็นว่าปัญหาเรื่องเขตแดนและพื้นที่ส่วนตัวเป็นเรื่องที่มนุษย์ปวดหัวกันมาทุกยุคสมัย",
          "sw": "Abraham kujadiliana na majirani zake kuhusu kisima inaonyesha kuwa migogoro ya mipaka ya ardhi na ujirani imekuwepo tangu zamani.",
          "it": "Negoziare i confini e i diritti dell'acqua con i vicini dimostra che le beghe di condominio esistono dall'alba dei tempi."
        }
    },
    "2026-02-28": {
        date: "2026-02-28",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 22:1-24",
          "en": "Genesis 22:1-24",
          "ko": "Genesis 22:1-24",
          "zho": "創世記 22:1-24",
          "es": "Genesis 22:1-24",
          "pt": "Genesis 22:1-24",
          "vi": "Genesis 22:1-24",
          "tl": "Genesis 22:1-24",
          "th": "Genesis 22:1-24",
          "sw": "Genesis 22:1-24",
          "it": "Genesi 22:1-24"
        },
        comment: {
          "ja": "息子を連れて山へ向かうアブラハムの心中を思うと、どんなに信仰があっても足がすくむのは人間として当然のことだ。",
          "en": "Abraham walking up that mountain with his son is the ultimate example of doing the hard thing while your heart is absolutely breaking.",
          "ko": "아들을 데리고 산으로 올라가는 아브라함의 뒷모습은 믿음이 무엇인지 고민하기 전에 인간으로서의 고뇌가 먼저 느껴져서 마음이 먹먹하다.",
          "zho": "带着儿子走向山顶的亚伯拉罕，在履行承诺时的那种挣扎与心碎，完美诠释了信仰与人性之间的极致拉扯。",
          "es": "Caminar hacia la montaña con su hijo es la prueba más extrema de que la fe a veces requiere hacer lo que más nos duele en el alma.",
          "pt": "Subir a montanha com o próprio filho é a definição máxima de ter que cumprir uma missão enquanto o coração está sendo despedaçado.",
          "vi": "Hình ảnh Abraham đưa con trai lên núi là minh chứng đau đớn nhất cho việc phải thực hiện điều khó khăn nhất khi lòng đang tan nát.",
          "tl": "Ang pag-akyat ni Abraham sa bundok kasama ang kanyang anak ay nagpapakita na ang pananampalataya ay minsan nangangailangan ng masakit na pagsasakripisyo.",
          "th": "การที่อับราฮัมต้องพาลูกชายขึ้นไปบนภูเขาเป็นบททดสอบที่หนักหนาที่สุด ซึ่งแสดงให้เห็นว่าความศรัทธามักมาพร้อมกับความเจ็บปวดที่คาดไม่ถึง",
          "sw": "Abraham kupanda mlima na mwanawe ni mfano mkuu wa jinsi imani inavyoweza kuhitaji ujasiri mkubwa hata wakati moyo wako unavunjika.",
          "it": "Camminare verso la montagna con proprio figlio è la prova estrema di dover fare la cosa giusta quando si ha il cuore a pezzi."
        }
    },
    "2026-03-01": {
        date: "2026-03-01",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 23:1-20",
          "en": "Genesis 23:1-20",
          "ko": "Genesis 23:1-20",
          "zho": "創世記 23:1-20",
          "es": "Genesis 23:1-20",
          "pt": "Genesis 23:1-20",
          "vi": "Genesis 23:1-20",
          "tl": "Genesis 23:1-20",
          "th": "Genesis 23:1-20",
          "sw": "Genesis 23:1-20",
          "it": "Genesi 23:1-20"
        },
        comment: {
          "ja": "最愛の人を失った悲しみの中で土地の交渉という現実的な事務作業をこなすアブラハム、人生の荒波でも日常を回さなきゃいけないという残酷なリアル。",
          "en": "Abraham having to negotiate real estate prices while grieving his wife is a brutally realistic reminder that life goes on even when your world stops.",
          "ko": "사랑하는 이를 떠나보낸 슬픔 속에서도 땅을 사기 위해 협상해야 하는 아브라함의 모습은, 슬픔 와중에도 일상을 이어가야 하는 삶의 잔인한 단면을 보여줍니다.",
          "zho": "亞伯拉罕在喪妻之痛中仍需處理繁瑣的土地買賣，這提醒了我們，生活總是在我們最想停下來時逼著我們繼續向前。",
          "es": "Tener que negociar el precio de un terreno mientras lloras a tu esposa es esa bofetada de realidad que te recuerda que la vida no se detiene ni por nuestro dolor más profundo.",
          "pt": "Ter de negociar o preço de um terreno enquanto chora a morte da esposa é um lembrete brutal de que a vida continua mesmo quando o nosso mundo para.",
          "vi": "Phải đi thương lượng giá đất trong lúc đau buồn vì mất vợ, Abraham cho thấy một sự thật phũ phàng rằng cuộc sống vẫn tiếp diễn ngay cả khi thế giới của ta sụp đổ.",
          "tl": "Ang kailangang makipag-negosasyon sa presyo ng lupa habang nagluluksa ay isang masakit na paalala na ang buhay ay hindi humihinto kahit sa gitna ng matinding pighati.",
          "th": "การต้องมาต่อรองราคาที่ดินในขณะที่กำลังโศกเศร้ากับการจากไปของภรรยา คือความจริงอันโหดร้ายของชีวิตที่บังคับให้เราต้องเดินหน้าต่อไปแม้ใจจะแหลกสลาย",
          "sw": "Kulazimika kujadiliana bei ya ardhi wakati unajawa na huzuni ya kumpoteza mkeo ni ukumbusho mchungu kwamba maisha hayasimami hata ulimwengu wako unapoanguka.",
          "it": "Dover sbrigare pratiche burocratiche e comprare terreni mentre si piange la propria sposa è la prova che la vita non si ferma nemmeno quando il tuo mondo crolla."
        }
    },
    "2026-03-02": {
        date: "2026-03-02",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 24:1-49",
          "en": "Genesis 24:1-49",
          "ko": "Genesis 24:1-49",
          "zho": "創世記 24:1-49",
          "es": "Genesis 24:1-49",
          "pt": "Genesis 24:1-49",
          "vi": "Genesis 24:1-49",
          "tl": "Genesis 24:1-49",
          "th": "Genesis 24:1-49",
          "sw": "Genesis 24:1-49",
          "it": "Genesi 24:1-49"
        },
        comment: {
          "ja": "息子の嫁探しを部下に丸投げするアブラハム、現代なら過保護な親と自立できない息子の究極の形かもしれない。",
          "en": "Abraham outsourcing his son's marriage search to a servant is the ultimate example of a helicopter parent who just cannot let go of the reins.",
          "ko": "아들의 배우자를 찾는 일을 종에게 전적으로 맡기는 아브라함의 모습은, 자식의 인생에 한 발짝도 떼지 못하는 극성 부모의 원조를 보는 듯합니다.",
          "zho": "亞伯拉罕把幫兒子找對象的任務全權交給僕人，這簡直是現代直升機家長插手子女生活的極致範本。",
          "es": "Que Abraham le encargue a su criado buscarle esposa a su hijo es el ejemplo definitivo de un padre helicóptero que no puede soltar las riendas ni un momento.",
          "pt": "Abraham delegar a busca por uma esposa para o filho ao seu servo é o exemplo máximo de um pai superprotetor que não consegue largar o controle.",
          "vi": "Việc Abraham giao phó hoàn toàn chuyện tìm vợ cho con trai vào tay người đầy tớ là ví dụ điển hình của những bậc phụ huynh can thiệp quá sâu vào đời tư của con cái.",
          "tl": "Ang pagpapasa ni Abraham sa kanyang alipin ng paghahanap ng mapapangasawa ng kanyang anak ay ang pinaka-halimbawa ng isang magulang na hindi kayang bitawan ang pagkontrol sa buhay ng anak.",
          "th": "การที่อับราฮัมมอบหมายให้คนใช้ไปหาคู่ครองให้ลูกชายนั้น ช่างเป็นตัวอย่างที่ชัดเจนของพ่อแม่ที่ก้าวก่ายชีวิตลูกจนไม่ยอมปล่อยให้เขาได้ตัดสินใจเอง",
          "sw": "Abraham kumkabidhi mtumishi wake jukumu la kumtafutia mwanawe mke ni mfano mkuu wa mzazi anayeshindwa kuacha kudhibiti maisha ya mtoto wake.",
          "it": "Delegare la ricerca della moglie di tuo figlio a un sottoposto è l'esempio definitivo di un genitore iperprotettivo che proprio non sa mollare la presa."
        }
    },
    "2026-03-03": {
        date: "2026-03-03",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 24:50-66",
          "en": "Genesis 24:50-66",
          "ko": "Genesis 24:50-66",
          "zho": "創世記 24:50-66",
          "es": "Genesis 24:50-66",
          "pt": "Genesis 24:50-66",
          "vi": "Genesis 24:50-66",
          "tl": "Genesis 24:50-66",
          "th": "Genesis 24:50-66",
          "sw": "Genesis 24:50-66",
          "it": "Genesi 24:50-66"
        },
        comment: {
          "ja": "見知らぬ土地へ行く決断を即座に下すリベカ、今の時代ならSNSで大炎上しそうだけど、その潔さは一周回って憧れる。",
          "en": "Rebekah agreeing to marry a stranger and move to a new land in five seconds flat is the ultimate leap of faith that would trigger a million concerned comments today.",
          "ko": "낯선 땅으로 가겠다는 리브가의 즉흥적인 결단력은 요즘 시대라면 SNS에서 엄청난 화제가 되었겠지만, 그 과감함만큼은 확실히 배울 점이 있다.",
          "zho": "利百加毫不猶豫地答應嫁給陌生人並前往異地，這份果斷放在現代簡直是連網紅都做不到的極限挑戰。",
          "es": "Rebeca aceptando irse con un extraño para casarse al instante es el tipo de decisión radical que en la actualidad nos haría cuestionar su salud mental, pero qué valentía.",
          "pt": "Rebeca aceitando casar com um estranho e partir para terras distantes instantaneamente é aquele tipo de decisão impulsiva que hoje em dia nos faria surtar.",
          "vi": "Việc Rê-be-ca đồng ý kết hôn với người lạ và rời bỏ quê hương ngay lập tức là một quyết định táo bạo mà thời nay chắc chắn sẽ khiến cả mạng xã hội bùng nổ.",
          "tl": "Ang pagpayag ni Rebeca na magpakasal sa estranghero at lumipat agad ng lugar ay ang uri ng desisyon na kung gagawin ngayon ay pagpipiyestahan ng buong internet.",
          "th": "เรเบคาห์ตัดสินใจแต่งงานกับคนแปลกหน้าและย้ายถิ่นฐานในทันทีเป็นความกล้าที่ถ้าเกิดขึ้นในยุคนี้คงเป็นประเด็นร้อนแรงในโซเชียลไปแล้ว",
          "sw": "Rebeka kukubali kuolewa na mgeni na kuhama nchi papo hapo ni uamuzi wa kijasiri ambao leo hii ungewafanya watu wengi kwenye mitandao kupagawa kwa mshangao.",
          "it": "Accettare di sposare uno sconosciuto e partire all'istante è un atto di fede radicale che oggi farebbe gridare al miracolo su qualsiasi social network."
        }
    },
    "2026-03-04": {
        date: "2026-03-04",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 26:1-35",
          "en": "Genesis 26:1-35",
          "ko": "Genesis 26:1-35",
          "zho": "創世記 26:1-35",
          "es": "Genesis 26:1-35",
          "pt": "Genesis 26:1-35",
          "vi": "Genesis 26:1-35",
          "tl": "Genesis 26:1-35",
          "th": "Genesis 26:1-35",
          "sw": "Genesis 26:1-35",
          "it": "Genesi 26:1-35"
        },
        comment: {
          "ja": "隣人の井戸を埋めたり掘り返したりを繰り返すイサク、もはや平和的解決というより根気比べの持久戦でしかない。",
          "en": "Isaac dealing with his neighbors by constantly digging and re-digging wells is less about conflict resolution and more about who has the most stubborn patience.",
          "ko": "이웃들이 우물을 메우면 다시 파고 또 파는 이삭의 모습은 인내의 아이콘이라기보다 거의 고집 대결의 끝판왕을 보는 것 같다.",
          "zho": "以撒面對鄰居不斷填井又重挖的過程，與其說是修行，不如說是場比誰更固執的耐力賽。",
          "es": "Isaac lidiando con vecinos que le tapan los pozos una y otra vez es la definición bíblica de tener una paciencia nivel experto en medio de gente insoportable.",
          "pt": "Isaque lidando com vizinhos que vivem tapando seus poços é a prova de que a paciência tem limites, mas ele decidiu testar o limite de todos.",
          "vi": "Việc Y-sác kiên trì đào lại những cái giếng bị lấp đầy bởi hàng xóm cho thấy rằng đôi khi sự nhẫn nhịn cũng cần một sức chịu đựng bền bỉ đến kinh ngạc.",
          "tl": "Ang paulit-ulit na paghukay ni Isaac ng mga balon na tinatambakan ng kanyang mga kapitbahay ay patunay na minsan ang pasensya ay isa ring uri ng digmaan.",
          "th": "การที่อิสอัคขุดบ่อน้ำใหม่ซ้ำแล้วซ้ำเล่าหลังจากถูกเพื่อนบ้านกลบฝัง เป็นบทเรียนที่บอกว่าบางครั้งความอดทนก็คือการรบทางจิตวิทยาดีๆ นี่เอง",
          "sw": "Isaka kuchimba visima mara kwa mara baada ya majirani kuvifukia ni ushahidi kwamba uvumilivu wakati mwingine ni mchezo wa nani atachoka kwanza.",
          "it": "Affrontare i vicini che continuano a tappare i pozzi è la classica lezione su chi possiede la pazienza più testarda al mondo."
        }
    },
    "2026-03-05": {
        date: "2026-03-05",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 27:18-46",
          "en": "Genesis 27:18-46",
          "ko": "Genesis 27:18-46",
          "zho": "創世記 27:18-46",
          "es": "Genesis 27:18-46",
          "pt": "Genesis 27:18-46",
          "vi": "Genesis 27:18-46",
          "tl": "Genesis 27:18-46",
          "th": "Genesis 27:18-46",
          "sw": "Genesis 27:18-46",
          "it": "Genesi 27:18-46"
        },
        comment: {
          "ja": "毛皮をまとって父親を騙すヤコブの必死さと、それにまんまと騙されるイサクのポンコツ具合、家族の修羅場はいつの時代もコントのようですね。",
          "en": "Jacob wearing goat skin to trick his dad and Isaac falling for it hook, line, and sinker is the ultimate reminder that family drama is basically a sitcom.",
          "ko": "아버지의 눈을 속이려고 염소 가죽을 뒤집어쓴 야곱의 필사적인 모습은 가족 사이의 갈등이 얼마나 황당하게 흘러갈 수 있는지 보여줍니다.",
          "zho": "雅各為了騙過父親而披上羊皮，而以撒竟然就這樣上當了，這種家庭鬧劇簡直像極了現代的黑色喜劇。",
          "es": "Jacob usando pieles de cabra para engañar a su padre y a Isaac creyéndoselo todo es la prueba de que el drama familiar siempre ha sido un poco caótico.",
          "pt": "Jacó se fantasiando de pele de cabrito para enganar o pai e Isaque caindo na armadilha é a prova de que o drama familiar sempre foi um show de horrores.",
          "vi": "Gia-cốp khoác da dê để lừa cha và Y-sác lại tin sái cổ, đúng là những vở kịch gia đình luôn có những tình tiết khó tin nhất.",
          "tl": "Si Jacob na nagbalat ng kambing para lokohin ang ama at si Isaac na naniwala naman ay patunay na ang drama sa pamilya ay parang isang sitcom noon pa man.",
          "th": "ยาโคบที่เอาหนังแพะมาคลุมตัวเพื่อหลอกพ่อและอิสอัคที่ดันเชื่อสนิทใจ เป็นเครื่องเตือนใจว่าดราม่าในครอบครัวมักจะตลกขบขันเสมอ",
          "sw": "Yakobo kuvaa ngozi ya mbuzi ili kumdanganya baba yake na Isaka kuamini hivyo ni ushahidi kwamba migogoro ya kifamilia imekuwa ni kama mchezo wa kuigiza tangu zamani.",
          "it": "Mettersi pelli di capra per ingannare il padre e cascarci in pieno dimostra che i drammi familiari sono sempre stati un po' caotici."
        }
    },
    "2026-03-06": {
        date: "2026-03-06",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 29:1-35",
          "en": "Genesis 29:1-35",
          "ko": "Genesis 29:1-35",
          "zho": "創世記 29:1-35",
          "es": "Genesis 29:1-35",
          "pt": "Genesis 29:1-35",
          "vi": "Genesis 29:1-35",
          "tl": "Genesis 29:1-35",
          "th": "Genesis 29:1-35",
          "sw": "Genesis 29:1-35",
          "it": "Genesi 29:1-35"
        },
        comment: {
          "ja": "7年も働いていざ結婚と思ったら妹を押し付けられるヤコブ、世の中そう甘くないという現実をこれでもかと突きつけられている。",
          "en": "Working seven years for the woman you love only to end up with her sister instead is the ultimate lesson that reality never goes according to plan.",
          "ko": "7년을 뼈 빠지게 일하고 드디어 결혼하나 싶었는데 동생을 떠맡게 된 야곱의 상황은 인생이 계획대로 되지 않는다는 뼈아픈 교훈을 줍니다.",
          "zho": "苦幹七年以為終於能抱得美人歸，結果卻被塞個妹妹，雅各的遭遇完美詮釋了什麼叫人生總有意外。",
          "es": "Trabajar siete años por amor solo para que te den a la hermana equivocada es la lección definitiva de que la vida nunca sale como planeaste.",
          "pt": "Trabalhar sete anos por amor e acabar casado com a irmã errada é a prova de que a vida adora pregar peças na gente.",
          "vi": "Làm lụng vất vả bảy năm trời rồi cuối cùng bị lừa lấy nhầm người, Gia-cốp đã nếm trải sự thật cay đắng rằng đời không như mơ.",
          "tl": "Ang magtrabaho ng pitong taon para sa taong mahal mo tapos ay mapunta sa kapatid niya ay patunay na ang buhay ay sadyang hindi sumusunod sa plano.",
          "th": "ทำงานเจ็ดปีเพื่อจะได้แต่งงานกับคนที่รักแต่กลับได้น้องสาวมาแทน เป็นบทเรียนราคาแพงว่าชีวิตมักจะหักมุมได้เสมอ",
          "sw": "Kufanya kazi miaka saba kwa ajili ya mwanamke unayempenda kisha kupewa dada yake ni funzo tosha kwamba maisha hayafuati mipango yako.",
          "it": "Lavorare sette anni per amore e ritrovarsi con la sorella sbagliata è la prova che la vita non segue mai i piani stabiliti."
        }
    },
    "2026-03-07": {
        date: "2026-03-07",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 30:25-43",
          "en": "Genesis 30:25-43",
          "ko": "Genesis 30:25-43",
          "zho": "創世記 30:25-43",
          "es": "Genesis 30:25-43",
          "pt": "Genesis 30:25-43",
          "vi": "Genesis 30:25-43",
          "tl": "Genesis 30:25-43",
          "th": "Genesis 30:25-43",
          "sw": "Genesis 30:25-43",
          "it": "Genesi 30:25-43"
        },
        comment: {
          "ja": "ヤコブが羊の毛色を操作する必死の努力とラバンのあくどい駆け引き、もはや古代のスタートアップ企業における泥沼の契約交渉を見ている気分になる。",
          "en": "Watching Jacob manipulate the sheep breeding and Laban’s shady contract changes feels exactly like a messy startup negotiation gone wrong.",
          "ko": "야곱이 양의 색깔을 조종하려고 애쓰는 모습과 라반의 끈질긴 계약 변경은 마치 현대 스타트업의 지저분한 협상 과정을 보는 것 같다.",
          "zho": "雅各為了羊群毛色費盡心機，拉班則是不斷修改合約，這簡直就是現代創業公司裡那場搞砸了的談判現場。",
          "es": "Ver a Jacob manipulando la cría de ovejas y a Labán cambiando los términos del contrato parece una negociación de negocios moderna que terminó en un desastre.",
          "pt": "Ver Jacó manipulando a criação das ovelhas e Labão mudando os termos do contrato parece uma negociação de negócios moderna que deu totalmente errado.",
          "vi": "Việc Gia-cốp cố gắng điều khiển màu lông cừu và sự lươn lẹo của La-ban chẳng khác nào một cuộc đàm phán hợp đồng đầy toan tính trong các công ty khởi nghiệp hiện nay.",
          "tl": "Ang pagmamanipula ni Jacob sa mga tupa at ang maduming taktikang negosasyon ni Laban ay parang eksena sa isang kumpanya na puro siraan ang kontrata.",
          "th": "ความพยายามของยาโคบในการคัดเลือกสายพันธุ์สัตว์กับเล่ห์เหลี่ยมของลาบันช่างเหมือนการต่อรองธุรกิจในบริษัทสตาร์ทอัพที่เต็มไปด้วยความวุ่นวาย",
          "sw": "Jitihada za Yakobo za kubadili rangi za kondoo na hila za Labani katika mikataba ni kama mazungumzo ya biashara yaliyoharibika katika kampuni za kisasa.",
          "it": "Assistere alle strane strategie di allevamento e ai contratti truccati sembra proprio una trattativa d'affari moderna finita male."
        }
    },
    "2026-03-08": {
        date: "2026-03-08",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 32:1-32",
          "en": "Genesis 32:1-32",
          "ko": "Genesis 32:1-32",
          "zho": "創世記 32:1-32",
          "es": "Genesis 32:1-32",
          "pt": "Genesis 32:1-32",
          "vi": "Genesis 32:1-32",
          "tl": "Genesis 32:1-32",
          "th": "Genesis 32:1-32",
          "sw": "Genesis 32:1-32",
          "it": "Genesi 32:1-32"
        },
        comment: {
          "ja": "夜通し天使と格闘してまで祝福を勝ち取ろうとするヤコブ、ここまで必死になれるものがあるのは羨ましいけれど、腰を痛めるのは避けたいところ。",
          "en": "Wrestling an angel all night just to get a blessing is peak Jacob energy, though I personally would prefer to avoid the permanent hip injury.",
          "ko": "축복을 받겠다고 밤새 천사와 씨름하는 야곱의 집념은 대단하지만, 개인적으로는 영구적인 고관절 부상은 피하고 싶다.",
          "zho": "為了得到祝福竟然跟天使摔角摔了一整晚，雅各的執著確實驚人，但這種會讓腰部受傷的拼命方式還是免了吧。",
          "es": "Luchar toda la noche contra un ángel solo para obtener una bendición es pura energía de Jacob, aunque personalmente preferiría evitar la lesión de cadera.",
          "pt": "Lutar a noite toda contra um anjo só para conseguir uma bênção é a cara do Jacó, embora eu pessoalmente preferisse evitar a lesão no quadril.",
          "vi": "Gia-cốp vật lộn với thiên sứ suốt đêm để giành lấy phước lành là một sự kiên trì đáng nể, dù tôi chắc chắn không muốn kết thúc bằng một chấn thương hông như thế.",
          "tl": "Ang pakikipagbuno ni Jacob sa anghel buong gabi para lang sa basbas ay nagpapakita ng kanyang determinasyon, kahit na ayaw ko sigurong magkaproblema sa balakang.",
          "th": "การปล้ำสู้กับทูตสวรรค์ทั้งคืนเพื่อให้ได้พรแสดงถึงความมุ่งมั่นของยาโคบอย่างที่สุด แม้ว่าส่วนตัวแล้วผมอยากจะเลี่ยงอาการบาดเจ็บที่สะโพกก็ตาม",
          "sw": "Kupambana na malaika usiku kucha ili kupata baraka ni ujasiri wa kipekee wa Yakobo, ingawa binafsi ningependa kuepuka jeraha la kudumu la nyonga.",
          "it": "Lottare tutta la notte contro un angelo per ottenere una benedizione è tipico di Jacob, anche se un mal di schiena simile è decisamente da evitare."
        }
    },
    "2026-03-09": {
        date: "2026-03-09",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 37:1-22",
          "en": "Genesis 37:1-22",
          "ko": "Genesis 37:1-22",
          "zho": "創世記 37:1-22",
          "es": "Genesis 37:1-22",
          "pt": "Genesis 37:1-22",
          "vi": "Genesis 37:1-22",
          "tl": "Genesis 37:1-22",
          "th": "Genesis 37:1-22",
          "sw": "Genesis 37:1-22",
          "it": "Genesi 37:1-22"
        },
        comment: {
          "ja": "派手なコートを着て夢の話を兄たちに語るヨセフの空気の読めなさは、もはや才能としか言いようがない。",
          "en": "Joseph wearing his fancy coat and bragging about his dreams to his brothers is honestly a masterclass in zero social awareness.",
          "ko": "형들에게 꿈 이야기를 신나게 떠드는 요셉의 눈치 제로 능력은 거의 재능 수준이다.",
          "zho": "约瑟穿着那件花哨的长袍，还跑去跟哥哥们炫耀自己的梦，这毫无眼力见的程度简直是一种天赋。",
          "es": "Joseph luciendo su abrigo llamativo y contando sus sueños a sus hermanos es el ejemplo perfecto de alguien que no tiene ni idea de cómo leer el ambiente.",
          "pt": "José desfilando com sua túnica colorida e contando seus sonhos para os irmãos é a definição de falta de noção social.",
          "vi": "Joseph mặc áo choàng sặc sỡ rồi đi khoe giấc mơ với các anh mình, đúng là bậc thầy trong việc không biết đọc tình huống.",
          "tl": "Si Jose na suot ang kanyang makulay na damit at nagyayabang ng panaginip sa mga kapatid ay tunay na walang pakiramdam sa paligid.",
          "th": "โยเซฟที่ใส่เสื้อคลุมสีฉูดฉาดแล้วไปเล่าความฝันให้พี่ๆ ฟัง คือตัวอย่างของคนที่อ่านบรรยากาศไม่เป็นอย่างแท้จริง",
          "sw": "Yusufu kuvaa kanzu yake ya rangi na kujigamba kuhusu ndoto zake mbele ya kaka zake ni mfano bora wa mtu asiyejua kusoma mazingira.",
          "it": "Giuseppe che va in giro con la sua tunica sgargiante a raccontare sogni ai fratelli è l'incarnazione perfetta della totale mancanza di tatto."
        }
    },
    "2026-03-10": {
        date: "2026-03-10",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 37:23-36",
          "en": "Genesis 37:23-36",
          "ko": "Genesis 37:23-36",
          "zho": "創世記 37:23-36",
          "es": "Genesis 37:23-36",
          "pt": "Genesis 37:23-36",
          "vi": "Genesis 37:23-36",
          "tl": "Genesis 37:23-36",
          "th": "Genesis 37:23-36",
          "sw": "Genesis 37:23-36",
          "it": "Genesi 37:23-36"
        },
        comment: {
          "ja": "血まみれの服を見せて父親を騙す兄弟たちの演技力、その必死な隠蔽工作ぶりに闇の深さを感じる。",
          "en": "The brothers faking a crime scene with blood-stained clothes is such a desperate and dark level of family drama.",
          "ko": "피 묻은 옷으로 아버지를 속이는 형들의 연기력, 그 필사적인 은폐 공작에서 인간의 끝없는 밑바닥을 본다.",
          "zho": "哥哥们用染血的衣服欺骗父亲，这种拙劣又黑暗的掩盖真相手段，简直把家庭矛盾演到了极致。",
          "es": "Los hermanos fingiendo una tragedia con ropa manchada de sangre es un nivel de drama familiar tan desesperado como oscuro.",
          "pt": "Os irmãos forjando uma cena de crime com roupas manchadas de sangue mostram o quão longe o drama familiar pode chegar.",
          "vi": "Các anh của Joseph diễn kịch với chiếc áo đẫm máu để lừa cha, đúng là một màn kịch che đậy sự thật đen tối đến đáng sợ.",
          "tl": "Ang mga kapatid na nanlinlang sa ama gamit ang duguang damit ay nagpapakita kung gaano kadilim ang kayang gawin ng tao dahil sa inggit.",
          "th": "เหล่าพี่ชายที่สร้างฉากโกหกพ่อด้วยเสื้อผ้าเปื้อนเลือด เป็นการปกปิดความผิดที่ทั้งมืดมนและสิ้นหวังสุดๆ",
          "sw": "Kaka zake kumdanganya baba yao kwa nguo zilizolowa damu ni kiwango cha uongo na drama ya kifamilia inayotisha.",
          "it": "I fratelli che mettono in scena una finta tragedia con la veste sporca di sangue ci ricordano quanto sappia essere contorto il dramma familiare."
        }
    },
    "2026-03-11": {
        date: "2026-03-11",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 38:12-30",
          "en": "Genesis 38:12-30",
          "ko": "Genesis 38:12-30",
          "zho": "創世記 38:12-30",
          "es": "Genesis 38:12-30",
          "pt": "Genesis 38:12-30",
          "vi": "Genesis 38:12-30",
          "tl": "Genesis 38:12-30",
          "th": "Genesis 38:12-30",
          "sw": "Genesis 38:12-30",
          "it": "Genesi 38:12-30"
        },
        comment: {
          "ja": "義父の家系図を複雑にするタマルの執念、もはやドラマの脚本家も真っ青な展開で目が離せない。",
          "en": "Tamar rewriting her own family tree through sheer audacity is the kind of plot twist that would make any soap opera writer quit their job.",
          "ko": "시아버지의 족보를 뒤흔들어 놓은 다말의 집념은 어지간한 막장 드라마 작가도 울고 갈 수준이다.",
          "zho": "他瑪為了延續家族血脈所展現的執著，其劇情轉折之大，連編劇都要甘拜下風。",
          "es": "La audacia de Tamar para cambiar su destino familiar es el tipo de giro argumental que dejaría sin trabajo a cualquier guionista de telenovelas.",
          "pt": "A audácia de Tamar em reescrever sua própria história familiar é o tipo de reviravolta que faria qualquer roteirista de novela pedir demissão.",
          "vi": "Sự quyết liệt của Ta-ma trong việc thay đổi vận mệnh gia đình là một cú bẻ lái khiến các biên kịch phim truyền hình cũng phải chào thua.",
          "tl": "Ang tapang ni Tamar na baguhin ang kanyang kapalaran ay isang plot twist na hindi kayang isipin kahit ng mga manunulat ng teleserye.",
          "th": "ความกล้าของทามาร์ในการพลิกชะตาชีวิตตัวเองเป็นพล็อตเรื่องที่หักมุมจนนักเขียนบทละครยังต้องยอมแพ้",
          "sw": "Ujasiri wa Tamari katika kubadili hatima ya familia yake ni aina ya mabadiliko ya hadithi ambayo ingemfanya mwandishi yeyote wa tamthilia aache kazi.",
          "it": "La mossa di Tamar per riprendersi il proprio futuro è un colpo di scena che farebbe invidia a qualsiasi sceneggiatore di soap opera."
        }
    },
    "2026-03-12": {
        date: "2026-03-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 39:1-23",
          "en": "Genesis 39:1-23",
          "ko": "Genesis 39:1-23",
          "zho": "創世記 39:1-23",
          "es": "Genesis 39:1-23",
          "pt": "Genesis 39:1-23",
          "vi": "Genesis 39:1-23",
          "tl": "Genesis 39:1-23",
          "th": "Genesis 39:1-23",
          "sw": "Genesis 39:1-23",
          "it": "Genesi 39:1-23"
        },
        comment: {
          "ja": "無実の罪で投獄されても腐らずに結果を出すヨセフ、もはや鋼のメンタルという言葉では片付けられないレベル。",
          "en": "Joseph getting thrown into prison for a crime he didn't commit and still ending up running the place is the ultimate definition of turning lemons into lemonade.",
          "ko": "억울한 누명을 쓰고 감옥에 갇혔는데도 그곳마저 접수해버리는 요셉의 멘탈은 강철을 넘어선 무언가다.",
          "zho": "約瑟在考驗中依然保持正直，因為他確信主與他同在；主的同在是我們最大的力量。\n即使在面對困難時，你如何感受到主與你同在？ 📖✨",
          "es": "Que José terminara administrando la cárcel después de ser encarcelado injustamente es la definición definitiva de convertir los limones en limonada.",
          "pt": "José ser preso injustamente e acabar gerenciando a prisão é a definição máxima de transformar limões em uma limonada perfeita.",
          "vi": "Việc Giô-sép bị bỏ tù oan nhưng vẫn xoay sở để quản lý cả nhà ngục chính là minh chứng đỉnh cao cho khả năng biến nghịch cảnh thành cơ hội.",
          "tl": "Ang pagkakulong nang walang kasalanan ni Jose pero sa huli ay siya pa ang nagpatakbo ng kulungan ay ang tunay na kahulugan ng paggawa ng lemonade mula sa mga dayap.",
          "th": "การที่โยเซฟถูกขังคุกทั้งที่ไม่ได้ทำผิดแต่กลับกลายเป็นผู้คุมคุกเสียเอง คือนิยามของการเปลี่ยนวิกฤตให้เป็นโอกาสที่เหนือชั้นที่สุด",
          "sw": "Yusufu kufungwa jela bila kosa na bado akaishia kusimamia gereza hilo zima ni mfano mkuu wa kugeuza changamoto kuwa fursa.",
          "zh": "約瑟即使身陷冤獄卻依然能把監獄經營得有聲有色，這份抗壓性簡直是職場生存的天花板。",
          "it": "Finire in prigione da innocente e finire comunque a gestire tutto dimostra che Giuseppe sapeva davvero fare di necessità virtù."
        }
    },
    "2026-03-13": {
        date: "2026-03-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 40:1-23",
          "en": "Genesis 40:1-23",
          "ko": "Genesis 40:1-23",
          "zho": "創世記 40:1-23",
          "es": "Genesis 40:1-23",
          "pt": "Genesis 40:1-23",
          "vi": "Genesis 40:1-23",
          "tl": "Genesis 40:1-23",
          "th": "Genesis 40:1-23",
          "sw": "Genesis 40:1-23",
          "it": "Genesi 40:1-23"
        },
        comment: {
          "ja": "助けてやった相手に速攻で忘れ去られるヨセフの悲哀、現代の職場で義理を通したのに恩を仇で返されるあの虚しさそのものだ。",
          "en": "Joseph getting ghosted by the cupbearer he just helped is the ancient version of being left on read after doing someone a huge favor.",
          "ko": "자기가 도와준 사람에게 바로 잊혀진 요셉의 상황은 도움 주고도 뒤통수 맞는 현대 직장인의 비애와 다를 게 없다.",
          "zho": "约瑟帮了酒政却被转头就忘，这种被过河拆桥的无力感，简直是职场社交的残酷真相。",
          "es": "Que el copero se olvidara de José justo después de que lo ayudó es el ejemplo bíblico de un favor que nunca es correspondido.",
          "pt": "José ser esquecido pelo copeiro depois de ajudá-lo é a prova de que a falta de gratidão é um clássico atemporal.",
          "vi": "Việc Giô-sép bị người hầu rượu quên lãng ngay sau khi được giúp đỡ chính là minh chứng cho sự bạc bẽo thường thấy trong các mối quan hệ.",
          "tl": "Ang makalimutan ni Jose matapos niyang tulungan ang cupbearer ay ang sinaunang bersyon ng pagiging seenzoned matapos kang gumawa ng pabor.",
          "th": "โยเซฟที่ถูกลืมหลังจากช่วยคนอื่นไว้ได้สำเร็จ เป็นภาพสะท้อนความใจดำของมนุษย์ที่พบได้ทั่วไปในชีวิตจริง",
          "sw": "Yusufu kusahauliwa na mnyweshaji baada ya kumsaidia ni mfano wa jinsi watu wanavyosahau fadhila haraka sana.",
          "it": "Essere scaricato dal coppiere dopo averlo aiutato è l'antenato biblico del visualizzato e non risposto dopo un grosso favore."
        }
    },
    "2026-03-14": {
        date: "2026-03-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 41:1-36",
          "en": "Genesis 41:1-36",
          "ko": "Genesis 41:1-36",
          "zho": "創世記 41:1-36",
          "es": "Genesis 41:1-36",
          "pt": "Genesis 41:1-36",
          "vi": "Genesis 41:1-36",
          "tl": "Genesis 41:1-36",
          "th": "Genesis 41:1-36",
          "sw": "Genesis 41:1-36",
          "it": "Genesi 41:1-36"
        },
        comment: {
          "ja": "ファラオの夢を解き明かして突然のCEO抜擢、準備不足のままいきなり大役を押し付けられるヨセフの戸惑いは現代の転職と重なる。",
          "en": "Going from prison to CEO overnight because you interpreted a dream is the most stressful promotion in history.",
          "ko": "감옥에서 바로 총리로 신분 상승한 요셉의 상황은 준비도 안 된 상태에서 갑자기 중책을 맡게 된 현대인의 당혹감과 똑같다.",
          "zho": "从囚犯直接变成法老的左右手，约瑟这种突如其来的职场大跳跃，简直是压力山大的终极职场挑战。",
          "es": "Pasar de la cárcel a ser el segundo al mando de Egipto porque supiste explicar un sueño es el ascenso más estresante de la historia.",
          "pt": "Ir da prisão direto para o cargo de CEO só por interpretar um sonho é a promoção mais estressante de todos os tempos.",
          "vi": "Từ tù nhân trở thành người quyền lực thứ hai chỉ sau một đêm vì giải mã giấc mơ, đây đúng là kiểu thăng chức gây áp lực nhất mọi thời đại.",
          "tl": "Ang biglaang pag-angat ni Jose mula bilanggo patungong CEO dahil lang sa interpretasyon ng panaginip ay ang pinaka-nakaka-stress na promotion sa kasaysayan.",
          "th": "จากนักโทษกลายเป็นผู้บริหารสูงสุดเพียงข้ามคืนเพราะทำนายฝันได้ เป็นการเลื่อนตำแหน่งที่กดดันที่สุดในประวัติศาสตร์",
          "sw": "Kutoka gerezani hadi kuwa kiongozi mkuu mara moja kwa sababu tu ya kutafsiri ndoto ni kupandishwa cheo chenye shinikizo kubwa zaidi kuwahi kutokea.",
          "it": "Passare dalla cella al vertice solo perché hai interpretato un sogno è la promozione più stressante della storia."
        }
    },
    "2026-03-15": {
        date: "2026-03-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 41:37-57",
          "en": "Genesis 41:37-57",
          "ko": "Genesis 41:37-57",
          "zho": "創世記 41:37-57",
          "es": "Genesis 41:37-57",
          "pt": "Genesis 41:37-57",
          "vi": "Genesis 41:37-57",
          "tl": "Genesis 41:37-57",
          "th": "Genesis 41:37-57",
          "sw": "Genesis 41:37-57",
          "it": "Genesi 41:37-57"
        },
        comment: {
          "ja": "奴隷からいきなり国家のトップに大抜擢されるヨセフを見ていると、人生のフラグ回収が早すぎてついていけない。",
          "en": "Watching Joseph go from slave to second-in-command overnight is the ultimate reminder that life can change faster than you can update your resume.",
          "ko": "노예에서 하루아침에 국무총리가 된 요셉을 보며 인생의 반전은 예고 없이 찾아온다는 걸 다시금 실감한다.",
          "zho": "看著約瑟從奴隸一躍成為國家二把手，簡直是人生履歷更新速度最快的一次驚人反轉。",
          "es": "Ver a José pasar de esclavo a ser la mano derecha del Faraón de la noche a la mañana nos recuerda que la vida da giros que ni el mejor guionista podría imaginar.",
          "pt": "Ver José subir de escravo a braço direito do Faraó da noite para o dia nos lembra que a vida tem reviravoltas que nem o melhor roteirista escreveria.",
          "vi": "Nhìn Giô-sép từ thân phận nô lệ bỗng chốc trở thành tể tướng, ta thấy cuộc đời đôi khi còn khó đoán hơn cả phim truyền hình.",
          "tl": "Ang pag-angat ni Jose mula sa pagiging alipin patungo sa pagiging kanang-kamay ng Faraon ay patunay na ang buhay ay may mga plot twist na hindi natin inaasahan.",
          "th": "การที่โยเซฟพลิกชีวิตจากทาสกลายเป็นรองผู้ปกครองประเทศในชั่วข้ามคืน ทำให้รู้ว่าชีวิตจริงยิ่งกว่าละครที่เดาทางไม่ได้เลย",
          "sw": "Kumwona Yusufu akitoka utumwani na kuwa kiongozi mkuu wa nchi kwa usiku mmoja ni ukumbusho kwamba maisha hubadilika kwa njia tusizozitarajia.",
          "it": "Vedere Giuseppe passare da schiavo a braccio destro del faraone ci ricorda che la vita cambia più in fretta di un curriculum aggiornato."
        }
    },
    "2026-03-16": {
        date: "2026-03-16",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 42:1-38",
          "en": "Genesis 42:1-38",
          "ko": "Genesis 42:1-38",
          "zho": "創世記 42:1-38",
          "es": "Genesis 42:1-38",
          "pt": "Genesis 42:1-38",
          "vi": "Genesis 42:1-38",
          "tl": "Genesis 42:1-38",
          "th": "Genesis 42:1-38",
          "sw": "Genesis 42:1-38",
          "it": "Genesi 42:1-38"
        },
        comment: {
          "ja": "かつて自分を売った兄弟たちと再会して、わざと厳しい態度で揺さぶりをかけるヨセフの心中を察すると、胃が痛くなるような気まずさがある。",
          "en": "Joseph putting his brothers through the wringer when they finally reunite is the ultimate high-stakes awkward family dinner.",
          "ko": "자신을 팔아넘겼던 형들을 다시 만난 요셉이 일부러 냉정하게 대하는 모습에서 묘하게 꼬인 가족 간의 긴장감이 느껴져 등골이 서늘하다.",
          "zho": "約瑟與當年賣掉他的兄弟重逢，還要故意折磨他們一番，這種夾雜著報復與複雜情感的家庭重聚，簡直尷尬到極點。",
          "es": "La tensión de José al reencontrarse con los hermanos que lo vendieron es el epítome de una reunión familiar donde nadie sabe qué decir.",
          "pt": "A tensão de José ao reencontrar os irmãos que o venderam é o epítome daquele jantar de família onde o clima está insuportável.",
          "vi": "Chứng kiến Giô-sép thử lòng những người anh từng bán mình, ta thấy rõ sự đấu tranh nội tâm của một người đang cố tỏ ra cứng rắn trước quá khứ đau thương.",
          "tl": "Ang pagsubok ni Jose sa kanyang mga kapatid na nagbenta sa kanya ay patunay na kahit ang pagpapatawad ay may kasamang matinding emosyonal na bagahe.",
          "th": "ฉากที่โยเซฟแกล้งทำเป็นดุใส่พี่ชายที่เคยขายเขาไป สะท้อนถึงความอึดอัดใจเวลาต้องเผชิญหน้ากับอดีตที่ยังลืมไม่ลง",
          "sw": "Yusufu kuwajaribu ndugu zake waliomuuza zamani ni kielelezo cha jinsi ilivyo vigumu kusahau yaliyopita wakati wa kukutana tena na familia.",
          "it": "La tensione al primo incontro con i fratelli dopo anni è il prototipo della cena di famiglia imbarazzante."
        }
    },
    "2026-03-17": {
        date: "2026-03-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 43:1-34",
          "en": "Genesis 43:1-34",
          "ko": "Genesis 43:1-34",
          "zho": "創世記 43:1-34",
          "es": "Genesis 43:1-34",
          "pt": "Genesis 43:1-34",
          "vi": "Genesis 43:1-34",
          "tl": "Genesis 43:1-34",
          "th": "Genesis 43:1-34",
          "sw": "Genesis 43:1-34",
          "it": "Genesi 43:1-34"
        },
        comment: {
          "ja": "散々自分たちを苦しめた兄弟を前にして、涙をこらえきれず奥の部屋で号泣するヨセフ、憎しみよりも懐かしさが勝ってしまうのが人間というものだ。",
          "en": "Joseph running into another room to cry because he couldn't handle seeing his brothers is a reminder that some wounds are just too deep to hide.",
          "ko": "형들을 보고 감정을 주체하지 못해 방으로 숨어들어 눈물을 쏟는 요셉을 보니, 용서란 머리가 아니라 가슴으로 하는 고통스러운 일임을 알 것 같다.",
          "zho": "約瑟面對曾經傷害過自己的兄弟，躲進內室痛哭，原來放下仇恨比想像中更需要勇氣與柔軟。",
          "es": "Ver a José esconderse para llorar al ver a sus hermanos nos recuerda que, a veces, el perdón es un proceso mucho más desordenado y humano de lo que admitimos.",
          "pt": "José chorando escondido ao ver seus irmãos mostra que, por trás de toda postura firme, ainda existe um coração que sente saudades do que foi quebrado.",
          "vi": "Joseph trốn vào phòng để khóc khi thấy anh em mình cho thấy rằng, đôi khi tha thứ là một cuộc chiến thầm lặng bên trong chính chúng ta.",
          "tl": "Ang pagtatago ni Jose para umiyak sa harap ng kanyang mga kapatid ay nagpapakita na ang pagpapatawad ay hindi laging madali, ito ay isang emosyonal na laban.",
          "th": "โยเซฟแอบไปร้องไห้เพราะทนเห็นพี่น้องไม่ได้ เตือนใจเราว่าบางครั้งการให้อภัยก็เป็นเรื่องของหัวใจที่บอบช้ำมากกว่าแค่การใช้เหตุผล",
          "sw": "Yosefu kujificha ili kulia anapowaona ndugu zake inatukumbusha kuwa kusamehe mara nyingi ni mchakato wa ndani wenye maumivu makali.",
          "it": "Giuseppe che corre a piangere di nascosto ci mostra che il perdono è un percorso molto più disordinato e umano di quanto pensiamo."
        }
    },
    "2026-03-18": {
        date: "2026-03-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 44:6-34",
          "en": "Genesis 44:6-34",
          "ko": "Genesis 44:6-34",
          "zho": "創世記 44:6-34",
          "es": "Genesis 44:6-34",
          "pt": "Genesis 44:6-34",
          "vi": "Genesis 44:6-34",
          "tl": "Genesis 44:6-34",
          "th": "Genesis 44:6-34",
          "sw": "Genesis 44:6-34",
          "it": "Genesi 44:6-34"
        },
        comment: {
          "ja": "弟を人質に取られ、身代わりを申し出るユダの姿に、かつてヨセフを売った男の面影はなく、ただ家族を守ろうと必死な一人の父親の姿がある。",
          "en": "Watching Judah offer himself as a slave to save his brother shows that people really can change, even when they have a messy past.",
          "ko": "예전에는 동생을 팔아넘겼던 유다가 이제는 동생을 위해 스스로 노예가 되겠다고 나서는 모습에서, 사람은 누구나 변할 수 있다는 희망을 본다.",
          "zho": "曾經賣掉弟弟的猶大，如今為了保護兄弟甘願犧牲自己，這證明了即使過去再糟糕，人依然有重寫結局的機會。",
          "es": "Ver a Judá ofrecerse como esclavo para salvar a su hermano demuestra que, por muy oscuro que haya sido nuestro pasado, siempre podemos elegir un camino diferente.",
          "pt": "Judá, que antes vendeu o irmão, agora se oferece como escravo para protegê-lo, provando que a redenção é uma mudança real de atitude, não apenas de palavras.",
          "vi": "Judah từ kẻ bán anh em mình trở thành người sẵn sàng hy sinh để bảo vệ em trai là minh chứng rõ nhất cho thấy con người có thể thay đổi hoàn toàn.",
          "tl": "Ang pag-aalok ni Juda sa kanyang sarili bilang alipin para iligtas ang kapatid ay patunay na kahit ang taong may madilim na nakaraan ay kayang magbago para sa tama.",
          "th": "ยูดาห์ที่เคยขายน้องชายตัวเองกลับมายอมเป็นทาสเพื่อช่วยน้อง พิสูจน์ให้เห็นว่าคนเราสามารถเปลี่ยนไปในทางที่ดีขึ้นได้จริงๆ ไม่ว่าอดีตจะเคยพลาดแค่ไหน",
          "sw": "Yuda ambaye zamani alimuuza ndugu yake, sasa anajitolea kuwa mtumwa ili kumlinda, ni ushahidi kuwa watu wanaweza kubadilika na kufanya yaliyo sawa.",
          "it": "Vedere Giuda offrirsi come schiavo per salvare il fratello dimostra che anche le persone con un passato pesante possono cambiare davvero."
        }
    },
    "2026-03-19": {
        date: "2026-03-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 45:16-28",
          "en": "Genesis 45:16-28",
          "ko": "Genesis 45:16-28",
          "zho": "創世記 45:16-28",
          "es": "Genesis 45:16-28",
          "pt": "Genesis 45:16-28",
          "vi": "Genesis 45:16-28",
          "tl": "Genesis 45:16-28",
          "th": "Genesis 45:16-28",
          "sw": "Genesis 45:16-28",
          "it": "Genesi 45:16-28"
        },
        comment: {
          "ja": "散々苦労をかけた息子が実は大出世していたと知ったときの父の驚きは、現代の親がSNSで子供の成功を初めて知る瞬間に通じるものがある。",
          "en": "The shock of a father realizing his long-lost son is actually a high-ranking official is basically the ancient version of finding out your kid is a CEO on LinkedIn.",
          "ko": "고생만 시킨 줄 알았던 아들이 사실은 엄청난 성공을 거두었다는 사실을 알게 된 아버지의 심정은, 자식의 성공을 SNS로 뒤늦게 확인한 부모의 마음과 다를 게 없다.",
          "zho": "得知失蹤多年的兒子竟然位高權重，那種驚嚇程度簡直就像在社交媒體上意外發現自家小孩成了名人一樣。",
          "es": "Enterarse de que el hijo que creías perdido es ahora un alto funcionario es el equivalente bíblico a descubrir por LinkedIn que tu hijo es el jefe de la empresa.",
          "pt": "Descobrir que o filho que você achava que tinha desaparecido é, na verdade, uma autoridade poderosa é como ver o sucesso do seu filho no LinkedIn e não acreditar.",
          "vi": "Việc người cha bàng hoàng khi biết đứa con tưởng đã mất lại là một quan chức quyền lực cũng giống như cảm giác khi cha mẹ bất ngờ thấy con mình thành đạt trên mạng xã hội.",
          "tl": "Ang gulat ng ama nang malaman na ang anak na akala niya ay nawala na ay isa palang mataas na opisyal ay parang pagtuklas sa tagumpay ng anak mo sa social media.",
          "th": "การที่พ่อรู้ว่าลูกชายที่หายไปนานกลายเป็นผู้มีอำนาจยิ่งใหญ่ ก็เหมือนความรู้สึกพ่อแม่สมัยนี้ที่เพิ่งรู้ว่าลูกตัวเองประสบความสำเร็จจากโซเชียลมีเดีย",
          "sw": "Baba kugundua kuwa mwanawe aliyedhaniwa kuwa amepotea ni kiongozi mkuu ni kama mzazi anayegundua mafanikio ya mtoto wake kupitia mitandao ya kijamii.",
          "it": "Scoprire che il figlio che credevi disperso è diventato un pezzo grosso è la versione antica di vederlo comparire su LinkedIn come amministratore delegato."
        }
    },
    "2026-03-20": {
        date: "2026-03-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 46:28-34",
          "en": "Genesis 46:28-34",
          "ko": "Genesis 46:28-34",
          "zho": "創世記 46:28-34",
          "es": "Genesis 46:28-34",
          "pt": "Genesis 46:28-34",
          "vi": "Genesis 46:28-34",
          "tl": "Genesis 46:28-34",
          "th": "Genesis 46:28-34",
          "sw": "Genesis 46:28-34",
          "it": "Genesi 46:28-34"
        },
        comment: {
          "ja": "数十年ぶりに再会した父と子の抱擁シーン、言葉はいらないはずなのに結局あれこれ話が止まらないのは人間味の極み。",
          "en": "The reunion hug between father and son is so emotional that even when they decide to stop talking, the relief of just being together says everything.",
          "ko": "수십 년 만에 재회한 부자의 포옹 장면, 말보다는 눈물이 앞서야 할 순간인데 결국 서로 할 말이 산더미처럼 쌓여있는 모습이 참 인간적이다.",
          "zho": "父子重逢的擁抱感人至深，明明應該無語凝噎，最後卻還是說個不停，這才是最真實的人性寫照。",
          "es": "El abrazo entre padre e hijo tras décadas de separación es tan emotivo que, aunque sobran las palabras, no pueden evitar hablar sin parar.",
          "pt": "O abraço entre pai e filho após décadas de separação é tão emocionante que, mesmo que as palavras sejam desnecessárias, eles não conseguem parar de conversar.",
          "vi": "Cảnh cha con ôm nhau sau hàng chục năm xa cách, dù biết rằng không cần lời nói nhưng họ vẫn cứ nói không ngừng, thật là một nét tính cách con người rất đỗi bình thường.",
          "tl": "Ang yakapan ng mag-ama pagkalipas ng ilang dekada ay napaka-emosyonal, at kahit na hindi na kailangan ng salita, hindi nila mapigilang magkwentuhan nang walang katapusan.",
          "th": "ฉากกอดกันของพ่อลูกที่ไม่ได้เจอกันหลายสิบปี มันเป็นช่วงเวลาที่คำพูดอาจไม่จำเป็น แต่การที่พวกเขาคุยกันไม่หยุดนี่แหละคือความจริงใจแบบมนุษย์สุดๆ",
          "sw": "Kukumbatiana kwa baba na mwana baada ya miongo kadhaa ni tukio la kugusa moyo, ambapo hata kama maneno hayahitajiki, hawawezi kujizuia kuzungumza.",
          "it": "L abbraccio tra padre e figlio dopo decenni è così travolgente che, pur sapendo che le parole sono superflue, finiscono inevitabilmente per raccontarsi di tutto."
        }
    },
    "2026-03-21": {
        date: "2026-03-21",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 48:1-22",
          "en": "Genesis 48:1-22",
          "ko": "Genesis 48:1-22",
          "zho": "創世記 48:1-22",
          "es": "Genesis 48:1-22",
          "pt": "Genesis 48:1-22",
          "vi": "Genesis 48:1-22",
          "tl": "Genesis 48:1-22",
          "th": "Genesis 48:1-22",
          "sw": "Genesis 48:1-22",
          "it": "Genesi 48:1-22"
        },
        comment: {
          "ja": "孫の頭に手を置くときにわざわざ手を交差させて長男を焦らせるヤコブ、死の間際まで親のえこひいき全開で笑うしかない。",
          "en": "Jacob crossing his hands to bless the younger grandson just to stir the pot shows that even on his deathbed, he was still playing favorites.",
          "ko": "손자들에게 축복할 때 굳이 손을 엇갈려 놓으며 장남을 당황시키는 야곱을 보니, 죽는 순간까지 편애를 멈추지 않는 인간미가 느껴진다.",
          "zho": "雅各在祝福孙子时故意交叉双手，让长子在一旁干着急，这临终前的偏心程度真是让人哭笑不得。",
          "es": "Jacob cruzando las manos para bendecir al nieto menor solo para ver la cara de su primogénito demuestra que el favoritismo no tiene edad.",
          "pt": "Jacó cruzando as mãos para abençoar o neto mais novo só para ver o filho mais velho entrar em pânico mostra que o favoritismo de pai não tem fim.",
          "vi": "Gia-cốp cố tình bắt chéo tay khi chúc phước cho cháu nội chỉ để làm khó con trưởng, đúng là kiểu ưu ái gia đình không bao giờ cũ.",
          "tl": "Ang pag-cross ng kamay ni Jacob sa pagbasbas sa mga apo para lang guluhin ang panganay ay patunay na kahit sa huling sandali ay hilig niya ang paboritismo.",
          "th": "ยาโคบไขว้มือตอนอวยพรหลานเพื่อให้ลูกชายคนโตตกใจ เป็นการแสดงความลำเอียงที่ยังคงเส้นคงวาจนวินาทีสุดท้ายจริงๆ",
          "sw": "Yakobo kuvuka mikono yake ili kubariki mjukuu mdogo ili tu kumchanganya mzaliwa wa kwanza inaonyesha kuwa upendeleo hauna ukomo hata ukubwani.",
          "it": "Giacobbe che incrocia le mani per benedire il nipote minore e far innervosire il primogenito dimostra che il favoritismo di famiglia non va mai in pensione."
        }
    },
    "2026-03-22": {
        date: "2026-03-22",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "創世記 49:29-33",
          "en": "Genesis 49:29-33",
          "ko": "Genesis 49:29-33",
          "zho": "創世記 49:29-33",
          "es": "Genesis 49:29-33",
          "pt": "Genesis 49:29-33",
          "vi": "Genesis 49:29-33",
          "tl": "Genesis 49:29-33",
          "th": "Genesis 49:29-33",
          "sw": "Genesis 49:29-33",
          "it": "Genesi 49:29-33"
        },
        comment: {
          "ja": "死に際に「絶対にこの場所に埋めてくれ」と念押しするヤコブ、死後の住所までこだわり抜くその執着心がもはや清々しい。",
          "en": "Jacob insisting on exactly where he should be buried proves that his need to micromanage everything didn't end with his final breath.",
          "ko": "죽는 순간까지 묻힐 장소를 꼼꼼히 지정하는 야곱을 보며, 죽어서도 모든 것을 통제하고 싶어 하는 인간의 본능을 엿본다.",
          "zho": "临终前还要反复强调安葬地点的雅各，那种死后也要掌控全局的执着真是让人佩服。",
          "es": "Jacob dando instrucciones precisas sobre dónde ser enterrado demuestra que ni siquiera la muerte detiene su necesidad de controlarlo todo.",
          "pt": "Jacó dando instruções detalhadas sobre onde ser enterrado mostra que sua necessidade de controlar tudo não parou nem na hora da morte.",
          "vi": "Gia-cốp dặn dò kỹ lưỡng về nơi an nghỉ cuối cùng, cho thấy ngay cả khi cận kề cái chết, ông vẫn muốn kiểm soát mọi thứ đến cùng.",
          "tl": "Ang pag-utos ni Jacob kung saan eksaktong ililibing ay patunay na hanggang sa huling hininga ay gusto niya pa ring kontrolado ang lahat.",
          "th": "ยาโคบสั่งเสียเรื่องที่ฝังศพแบบเป๊ะๆ แสดงให้เห็นว่าความต้องการควบคุมทุกอย่างของเขาไม่ได้จบลงแม้ในวาระสุดท้ายของชีวิต",
          "sw": "Yakobo kusisitiza mahali hasa anapopaswa kuzikwa inaonyesha kwamba hitaji lake la kudhibiti kila kitu halikuishia hata kwenye pumzi yake ya mwisho.",
          "it": "Giacobbe che detta disposizioni precise sul proprio luogo di sepoltura conferma che il suo bisogno di pianificare tutto non si ferma nemmeno davanti all aldilà."
        }
    },
    "2026-03-23": {
        date: "2026-03-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 1:1-22",
          "en": "Exodus 1:1-22",
          "ko": "Exodus 1:1-22",
          "zho": "出エジプト記 1:1-22",
          "es": "Exodus 1:1-22",
          "pt": "Exodus 1:1-22",
          "vi": "Exodus 1:1-22",
          "tl": "Exodus 1:1-22",
          "th": "Exodus 1:1-22",
          "sw": "Exodus 1:1-22",
          "it": "Esodo 1:1-22"
        },
        comment: {
          "ja": "新しい王が過去の功績を忘れて手のひらを返す展開、歴史の教科書というより現代の組織の縮図を見ているようで胃が痛い。",
          "en": "A new king forgetting history and turning on his people overnight feels less like ancient history and more like a typical Monday at the office.",
          "ko": "새로운 왕이 과거의 공로를 잊고 태도를 바꾸는 모습은 역사책이라기보다 오늘날 조직의 흔한 풍경 같아서 씁쓸하다.",
          "zho": "新王上位就忘記前朝功績並翻臉，這場景與其說是歷史，不如說是現代職場中常見的政治現實。",
          "es": "Un nuevo rey que olvida el pasado y cambia de actitud de la noche a la mañana es un recordatorio de que la política de oficina no ha cambiado nada.",
          "pt": "Um novo rei que esquece o passado e vira as costas ao povo é um lembrete de que a política organizacional é antiga e implacável.",
          "vi": "Một vị vua mới lên ngôi quên sạch công lao quá khứ và trở mặt ngay lập tức, nghe chẳng khác nào tình huống oái oăm chốn công sở thời nay.",
          "tl": "Ang paglimot ng bagong hari sa nakaraang kabutihan at biglaang pagbabago ng trato ay parang eksena lang sa opisina na nakaka-stress.",
          "th": "กษัตริย์องค์ใหม่ที่ลืมความดีความชอบเก่าๆ แล้วเปลี่ยนท่าทีทันที เป็นสถานการณ์ที่ดูเหมือนการเมืองในที่ทำงานยุคปัจจุบันมากกว่าประวัติศาสตร์",
          "sw": "Mfalme mpya kusahau yaliyopita na kubadili msimamo ghafla ni ukumbusho kwamba siasa za ofisini hazijawahi kubadilika tangu zamani.",
          "it": "Un nuovo sovrano che dimentica il passato e cambia rotta da un giorno all altro è il classico promemoria di come certe dinamiche di potere non cambino mai."
        }
    },
    "2026-03-24": {
        date: "2026-03-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 2:1-25",
          "en": "Exodus 2:1-25",
          "ko": "Exodus 2:1-25",
          "zho": "出エジプト記 2:1-25",
          "es": "Exodus 2:1-25",
          "pt": "Exodus 2:1-25",
          "vi": "Exodus 2:1-25",
          "tl": "Exodus 2:1-25",
          "th": "Exodus 2:1-25",
          "sw": "Exodus 2:1-25",
          "it": "Esodo 2:1-25"
        },
        comment: {
          "ja": "川に流された赤ん坊を王女が拾うという展開、あまりにドラマチックすぎて現実味がないようでいて実は一番人間らしいドラマが詰まっている。",
          "en": "A princess finding a baby in a basket is the kind of plot twist that sounds like fiction until you realize life is often just a series of bizarre coincidences.",
          "ko": "강물에 떠내려가는 아기를 공주가 줍는 전개는 너무 드라마틱해서 비현실적이지만 사실 인생이란 원래 이런 우연의 연속이다.",
          "zho": "公主在河邊撿到嬰兒的情節簡直比電視劇還誇張，但現實生活往往就是由這種不可思議的巧合堆疊而成。",
          "es": "Que una princesa encuentre a un bebé en una cesta parece un guion de película, pero la vida real suele ser así de extraña e inesperada.",
          "pt": "Uma princesa encontrar um bebê no rio parece roteiro de filme, mas a vida real é cheia dessas coincidências que a gente não consegue explicar.",
          "vi": "Công chúa nhặt được em bé trôi sông nghe như phim truyền hình, nhưng cuộc đời thực vốn dĩ là một chuỗi những sự tình cờ không tưởng.",
          "tl": "Ang pagpulot ng prinsesa sa sanggol na nasa basket ay parang eksena sa pelikula, pero ang totoong buhay ay puno talaga ng mga ganyang hindi inaasahang pagkakataon.",
          "th": "ฉากเจ้าหญิงเก็บทารกได้จากแม่น้ำดูเหมือนพล็อตละครจนไม่น่าเชื่อ แต่ชีวิตจริงมักเต็มไปด้วยเหตุบังเอิญที่เหนือความคาดหมายแบบนี้เสมอ",
          "sw": "Binti wa mfalme kuokota mtoto mtoni inaonekana kama hadithi ya filamu, lakini maisha halisi yamejaa matukio ya ajabu tusiyoweza kuyatabiri.",
          "it": "Trovare un neonato in una cesta lungo il fiume sembra una scena da film, ma la vita reale sa essere sempre più imprevedibile di qualsiasi sceneggiatura."
        }
    },
    "2026-03-25": {
        date: "2026-03-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 3:1-22",
          "en": "Exodus 3:1-22",
          "ko": "Exodus 3:1-22",
          "zho": "出エジプト記 3:1-22",
          "es": "Exodus 3:1-22",
          "pt": "Exodus 3:1-22",
          "vi": "Exodus 3:1-22",
          "tl": "Exodus 3:1-22",
          "th": "Exodus 3:1-22",
          "sw": "Exodus 3:1-22",
          "it": "Esodo 3:1-22"
        },
        comment: {
          "ja": "燃えているのに燃え尽きない茂みを見て足を止めるモーセ、現代ならとりあえずスマホで動画を撮ってSNSに投稿しそうな距離感。",
          "en": "Moses pausing to watch a bush that burns but never turns to ash is basically the ancient version of stopping to film something viral for your feed.",
          "ko": "타오르지만 타지 않는 떨기나무를 보고 발걸음을 멈춘 모세는 현대라면 일단 스마트폰을 꺼내 촬영부터 했을 것 같은 느낌이다.",
          "zho": "看見燃燒卻不燒毀的荊棘而停下腳步的摩西，若在現代大概會先拿出手機錄影上傳社群媒體。",
          "es": "Moisés deteniéndose ante una zarza que arde pero no se consume es básicamente el equivalente antiguo de pararse a grabar un video viral para las redes.",
          "pt": "Moisés parando para ver uma sarça que arde mas não se consome é basicamente o equivalente antigo de parar tudo para gravar um vídeo viral.",
          "vi": "Môi-se dừng lại trước bụi gai cháy mà không rụi giống như cách chúng ta dừng lại để quay phim một hiện tượng lạ cho mạng xã hội vậy.",
          "tl": "Ang paghinto ni Moises sa harap ng naglalablab na palumpong ay parang paghinto natin ngayon para i-video ang isang viral na pangyayari.",
          "th": "การที่โมเสสหยุดดูพุ่มไม้ที่ไฟไหม้แต่ไม่มอดนั้น เหมือนกับคนยุคนี้ที่หยุดถ่ายคลิปวิดีโอเพื่อลงโซเชียลมีเดียเมื่อเจออะไรแปลกๆ",
          "sw": "Musa kusimama kutazama kichaka kinachowaka moto bila kuungua ni sawa na mtu wa leo kusimama kurekodi tukio la ajabu kwa ajili ya mitandao ya kijamii.",
          "it": "Mosè che si ferma a guardare un cespuglio che brucia senza consumarsi è l equivalente antico di fermarsi a filmare qualcosa di assurdo per caricarlo sui social."
        }
    },
    "2026-03-26": {
        date: "2026-03-26",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 4:1-17",
          "en": "Exodus 4:1-17",
          "ko": "Exodus 4:1-17",
          "zho": "出エジプト記 4:1-17",
          "es": "Exodus 4:1-17",
          "pt": "Exodus 4:1-17",
          "vi": "Exodus 4:1-17",
          "tl": "Exodus 4:1-17",
          "th": "Exodus 4:1-17",
          "sw": "Exodus 4:1-17",
          "it": "Esodo 4:1-17"
        },
        comment: {
          "ja": "神様に大役を任された途端に自分の口下手さを言い訳にするモーセ、大事なプレゼンの直前に腹痛を訴える学生そのもの。",
          "en": "Moses immediately citing his poor speaking skills the moment he gets a divine mission is exactly like a student suddenly feeling sick right before a big presentation.",
          "ko": "큰 사명을 받자마자 말주변이 없다고 핑계 대는 모세는 중요한 발표 직전에 배가 아프다고 하는 학생의 마음과 똑같다.",
          "zho": "摩西一接到重大任務就開始以口才不好為藉口，簡直就像是學生在重要報告前夕突然喊肚子痛一樣。",
          "es": "Moisés poniendo excusas sobre su falta de elocuencia justo cuando recibe una misión importante es como ese estudiante que de repente le duele la panza antes de una exposición.",
          "pt": "Moisés dando desculpas sobre sua oratória assim que recebe uma missão importante é como aquele estudante que de repente passa mal antes de uma apresentação.",
          "vi": "Môi-se viện cớ mình ăn nói kém ngay khi nhận trọng trách giống hệt như học sinh bỗng dưng thấy đau bụng trước giờ thuyết trình quan trọng.",
          "tl": "Ang pagdadahilan ni Moises na hindi siya mahusay magsalita matapos bigyan ng misyon ay parang estudyante na biglang sumasakit ang tiyan bago ang report.",
          "th": "โมเสสพยายามอ้างว่าพูดไม่เก่งทันทีที่ได้รับมอบหมายงานใหญ่ เหมือนนักเรียนที่จู่ๆ ก็ปวดท้องก่อนขึ้นไปนำเสนองานหน้าชั้นเรียนเป๊ะ",
          "sw": "Musa kutoa visingizio kuhusu kutoweza kwake kuongea vizuri baada ya kupewa wito ni sawa na mwanafunzi anayesema anaumwa tumbo kabla ya kutoa mada darasani.",
          "it": "Mosè che si inventa scuse sulla propria parlantina appena riceve una grande missione è identico a chi cerca di svignarsela prima di una riunione importante."
        }
    },
    "2026-03-27": {
        date: "2026-03-27",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 4:18-31",
          "en": "Exodus 4:18-31",
          "ko": "Exodus 4:18-31",
          "zho": "出エジプト記 4:18-31",
          "es": "Exodus 4:18-31",
          "pt": "Exodus 4:18-31",
          "vi": "Exodus 4:18-31",
          "tl": "Exodus 4:18-31",
          "th": "Exodus 4:18-31",
          "sw": "Exodus 4:18-31",
          "it": "Esodo 4:18-31"
        },
        comment: {
          "ja": "仕事の準備が万端とは言えない状態で、とりあえず家族を連れて現地に向かうモーセの背中には、新生活に不安な現代人の哀愁が漂っている。",
          "en": "Moses heading off to his big assignment with his whole family in tow feels exactly like that one coworker who is clearly winging it but somehow getting it done.",
          "ko": "준비도 제대로 안 된 상태에서 일단 가족들 챙겨서 떠나는 모세의 모습이, 인생의 큰 결정을 내리고 불안해하는 우리들의 모습과 너무 닮았다.",
          "zho": "摩西在準備不足的情況下帶著全家出發，那種硬著頭皮上陣的背影，簡直就是現代人面對新挑戰時的真實寫照。",
          "es": "Moisés yendo a su gran misión con toda la familia a cuestas es el reflejo perfecto de cualquiera de nosotros cuando empezamos algo nuevo sin tener ni idea de qué pasará.",
          "pt": "Moisés partindo para sua grande missão com a família a tiracolo parece muito com qualquer um de nós tentando improvisar enquanto lida com as incertezas da vida.",
          "vi": "Môi-se dẫn cả gia đình đi làm nhiệm vụ trong khi bản thân vẫn còn đầy lo âu giống hệt như cách chúng ta bước vào những chương mới của cuộc đời một cách đầy bỡ ngỡ.",
          "tl": "Ang pag-alis ni Moises kasama ang buong pamilya nang hindi pa handa ay parang tayo lang na sumusubok mag-adjust sa mga biglaang pagbabago sa buhay.",
          "th": "การที่โมเสสพาครอบครัวออกเดินทางทั้งที่ยังไม่พร้อมดูเหมือนชีวิตจริงของพวกเราทุกคนที่ต้องก้าวไปข้างหน้าทั้งที่ใจยังหวั่นๆ",
          "sw": "Musa kuondoka na familia yake kuelekea kusiko julikana ni kama sisi tunavyoanza mambo mapya huku tukijaribu kuonekana kama tuko sawa.",
          "it": "Partire per una grande avventura con tutta la famiglia al seguito senza avere la minima idea di cosa succederà è il ritratto perfetto dell incertezza quotidiana."
        }
    },
    "2026-03-28": {
        date: "2026-03-28",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 5:15-23",
          "en": "Exodus 5:15-23",
          "ko": "Exodus 5:15-23",
          "zho": "出エジプト記 5:15-23",
          "es": "Exodus 5:15-23",
          "pt": "Exodus 5:15-23",
          "vi": "Exodus 5:15-23",
          "tl": "Exodus 5:15-23",
          "th": "Exodus 5:15-23",
          "sw": "Exodus 5:15-23",
          "it": "Esodo 5:15-23"
        },
        comment: {
          "ja": "神様に言われた通りに動いたはずなのに事態がむしろ悪化して、思わず「なぜこんなことに？」と天を仰ぐモーセの気持ち、痛いほどよくわかる。",
          "en": "Moses doing exactly what he was told only for things to get worse is the ultimate reminder that following instructions doesn't always guarantee a smooth commute.",
          "ko": "하나님 말씀대로 했는데 상황이 더 꼬여버려서 당황하는 모세의 심정은, 열심히 했는데 결과가 안 좋을 때의 우리 마음과 똑같다.",
          "zho": "明明按照指示去做了，結果情況反而變得更糟，摩西那種「怎麼會這樣」的無奈，完全是我們在職場受挫時的真實心聲。",
          "es": "Hacer exactamente lo que te piden y que aun así todo salga mal es la clásica frustración de Moisés con la que cualquiera que haya tenido un mal día se identifica.",
          "pt": "Fazer exatamente o que foi pedido e ver as coisas piorarem é a frustração clássica de Moisés, algo que qualquer um de nós que já teve um dia ruim conhece bem.",
          "vi": "Làm đúng như lời dặn mà mọi việc vẫn tệ đi, sự bất lực của Môi-se khi hỏi tại sao chính là cảm giác của chúng ta mỗi khi nỗ lực nhưng không được đền đáp.",
          "tl": "Ang gawin ang lahat ng tama pero lalong gumulo ang sitwasyon ay ang klasikong pagkadismaya ni Moises na ramdam na ramdam nating lahat.",
          "th": "การทำตามคำสั่งแล้วผลลัพธ์กลับแย่ลงเป็นความรู้สึกของโมเสสที่พวกเราทุกคนคงเคยเจอเวลาทุ่มเทเต็มที่แต่กลับไม่ได้ดั่งใจ",
          "sw": "Kufanya yale uliyoambiwa lakini mambo yakaharibika zaidi ni ile hali ya kuchanganyikiwa ya Musa ambayo sote tumeipitia tunapohisi juhudi zetu hazina matokeo.",
          "it": "Seguire le istruzioni alla lettera e ritrovarsi la situazione peggiorata è quella frustrazione universale che chiunque abbia avuto una giornata storta conosce bene."
        }
    },
    "2026-03-29": {
        date: "2026-03-29",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 6:14-30",
          "en": "Exodus 6:14-30",
          "ko": "Exodus 6:14-30",
          "zho": "出エジプト記 6:14-30",
          "es": "Exodus 6:14-30",
          "pt": "Exodus 6:14-30",
          "vi": "Exodus 6:14-30",
          "tl": "Exodus 6:14-30",
          "th": "Exodus 6:14-30",
          "sw": "Exodus 6:14-30",
          "it": "Esodo 6:14-30"
        },
        comment: {
          "ja": "口下手で自信がないモーセに、神様が「じゃあ兄貴を代弁者にすればいい」と即座に妥協案を出すあたり、神様との交渉も案外現実的な落とし所があるんだなと思わされる。",
          "en": "Moses trying to get out of the job by claiming he is not a good speaker, only for God to immediately assign his brother as his PR manager, feels like a very relatable office compromise.",
          "ko": "말주변이 없다고 버티는 모세에게 형을 대변인으로 붙여주시는 장면을 보면 신과 인간의 대화도 결국 현실적인 타협점을 찾아가는 과정 같아서 흥미롭다.",
          "zho": "摩西以口才不好為由推託重任，神卻直接安排他哥哥當代言人，這種職場般的折衷方案讓人感到意外的親切。",
          "es": "Que Moisés intente librarse de su misión por no ser elocuente y que Dios le asigne a su hermano como portavoz se siente como el arreglo más humano y realista de la historia.",
          "pt": "Moisés tentando se esquivar da missão por não ser um bom orador e Deus simplesmente contratar o irmão dele como porta-voz parece o tipo de acordo prático que fazemos no dia a dia.",
          "vi": "Việc Môi-se cố thoái thác vì không giỏi ăn nói và được Chúa sắp xếp cho anh trai làm người phát ngôn nghe chẳng khác nào một sự thỏa hiệp đầy thực tế nơi công sở.",
          "tl": "Ang pagdadahilan ni Moises na hindi siya mahusay magsalita at ang agad na pagtalaga ng Diyos sa kapatid niya bilang tagapagsalita ay parang isang napaka-relatable na kompromiso sa trabaho.",
          "th": "การที่โมเสสพยายามเลี่ยงงานเพราะพูดไม่เก่ง แต่พระเจ้ากลับแก้ปัญหาด้วยการหาพี่ชายมาเป็นโฆษกให้ทันทีนั้น ดูเป็นทางออกที่สมเหตุสมผลเหมือนการเจรจางานในชีวิตจริงเลย",
          "sw": "Musa akijaribu kukwepa kazi kwa kisingizio cha kutokuwa msemaji mzuri na Mungu kumpa kaka yake kama msemaji ni kama maelewano ya kirafiki tunayopata kazini kila siku.",
          "it": "Che Mosè cerchi di defilarsi dicendo di non saper parlare e Dio gli affianchi subito il fratello come portavoce sembra il classico compromesso d'ufficio."
        }
    },
    "2026-03-30": {
        date: "2026-03-30",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 61:1-3",
          "en": "Isaiah 61:1-3",
          "ko": "Isaiah 61:1-3",
          "zho": "イザヤ書 61:1-3",
          "es": "Isaiah 61:1-3",
          "pt": "Isaiah 61:1-3",
          "vi": "Isaiah 61:1-3",
          "tl": "Isaiah 61:1-3",
          "th": "Isaiah 61:1-3",
          "sw": "Isaiah 61:1-3",
          "it": "Isaia 61:1-3"
        },
        comment: {
          "ja": "灰の代わりに冠を、悲しみの代わりに喜びの油を与えようという言葉は、どん底の時にこそ一番欲しい救済の形そのものだ。",
          "en": "Trading ashes for a crown and mourning for the oil of joy is the exact kind of emotional upgrade we all crave when we hit rock bottom.",
          "ko": "재 대신 관을, 슬픔 대신 기쁨의 기름을 주겠다는 약속은 인생이 바닥을 칠 때 가장 간절히 바라는 위로의 정석이다.",
          "zho": "用華冠代替灰燼，用喜樂油代替悲哀，這正是我們在人生低谷時最渴望得到的情感救贖。",
          "es": "Cambiar las cenizas por una corona y la tristeza por aceite de alegría es exactamente el tipo de rescate emocional que todos buscamos cuando tocamos fondo.",
          "pt": "Trocar cinzas por uma coroa e tristeza por óleo de alegria é o tipo de redenção emocional que todos desejamos quando estamos no fundo do poço.",
          "vi": "Đổi tro bụi lấy mão triều và sự than vãn lấy dầu vui mừng chính là hình thức cứu rỗi mà ai cũng khao khát khi rơi vào hoàn cảnh tồi tệ nhất.",
          "tl": "Ang pagpapalit ng abo ng korona at ng pighati ng langis ng kagalakan ay ang eksaktong uri ng paghilom na hinahanap nating lahat kapag nasa pinakamababa tayong estado.",
          "th": "การเปลี่ยนเถ้าถ่านเป็นมงกุฎและเปลี่ยนความโศกเศร้าเป็นน้ำมันแห่งความชื่นชมยินดี คือการเยียวยาจิตใจที่ทุกคนปรารถนาเมื่อถึงจุดต่ำสุดของชีวิต",
          "sw": "Kubadilisha majivu na taji na huzuni na mafuta ya furaha ndiyo aina ya wokovu wa kihisia tunaotamani zaidi tunapokuwa katika hali ngumu kabisa.",
          "it": "Scambiare la cenere con una corona e il lutto con l'olio della gioia è esattamente il tipo di riscatto emotivo che cerchiamo quando tocchiamo il fondo."
        }
    },
    "2026-03-31": {
        date: "2026-03-31",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 53:4",
          "en": "Isaiah 53:4",
          "ko": "Isaiah 53:4",
          "zho": "イザヤ書 53:4",
          "es": "Isaiah 53:4",
          "pt": "Isaiah 53:4",
          "vi": "Isaiah 53:4",
          "tl": "Isaiah 53:4",
          "th": "Isaiah 53:4",
          "sw": "Isaiah 53:4",
          "it": "Isaia 53:4"
        },
        comment: {
          "ja": "誰にも言えない痛みを抱えたまま平然と振る舞う私たちにとって、先に傷ついてくれた存在がいるという事実は救い以外の何物でもない。",
          "en": "For those of us faking a smile while hiding deep pain, knowing someone already endured it for us is the ultimate relief.",
          "ko": "남모를 아픔을 감추고 겉으로는 괜찮은 척하는 우리에게, 이미 우리보다 먼저 아파해준 존재가 있다는 건 정말 큰 위로다.",
          "zho": "對於那些習慣隱藏痛苦卻裝作若無其事的我們來說，有人曾先為我們承受了一切，這簡直是最大的救贖。",
          "es": "Para quienes fingimos estar bien mientras cargamos penas invisibles, saber que alguien ya cargó con ellas por nosotros es un alivio inmenso.",
          "pt": "Para nós que fingimos estar bem enquanto escondemos dores profundas, saber que alguém já sofreu por nós é o maior dos confortos.",
          "vi": "Với những người luôn tỏ ra ổn trong khi giấu kín nỗi đau, việc biết rằng có người đã chịu đựng thay mình thực sự là một sự cứu rỗi.",
          "tl": "Para sa ating nagkukunwaring maayos habang nagtatago ng sakit, ang malaman na may nagtiis na para sa atin ay isang malaking kaginhawaan.",
          "th": "สำหรับพวกเราที่แกล้งทำเป็นโอเคขณะซ่อนความเจ็บปวดไว้ การรู้ว่ามีคนยอมเจ็บแทนเรามาก่อนแล้วนั้นถือเป็นความสบายใจที่สุด",
          "sw": "Kwa sisi tunaojifanya wazima huku tukiwa na maumivu ya ndani, kujua kuwa kuna mtu alishateseka kwa ajili yetu ni faraja kubwa.",
          "it": "Per chi finge un sorriso mentre nasconde un dolore profondo, sapere che qualcuno lo ha già provato prima di noi è un sollievo immenso."
        }
    },
    "2026-04-01": {
        date: "2026-04-01",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 12:30",
          "en": "Exodus 12:30",
          "ko": "Exodus 12:30",
          "zho": "出エジプト記 12:30",
          "es": "Exodus 12:30",
          "pt": "Exodus 12:30",
          "vi": "Exodus 12:30",
          "tl": "Exodus 12:30",
          "th": "Exodus 12:30",
          "sw": "Exodus 12:30",
          "it": "Esodo 12:30"
        },
        comment: {
          "ja": "昨夜までは何事もなく日常を過ごしていたのに、一晩で家中の空気が凍りつくような喪失感を味わうエジプトの光景は、人生の脆さを突きつけてくる。",
          "en": "The sudden shift from a normal night to a house full of mourning in Egypt is a brutal reminder of how fragile our sense of normalcy really is.",
          "ko": "평온했던 일상이 한순간에 무너져 내리는 이집트의 밤은, 우리가 당연하게 여기는 평범함이 얼마나 연약한지 뼈저리게 보여준다.",
          "zho": "原本平靜的夜晚轉眼間變得哀鴻遍野，埃及的那一幕殘酷地提醒著我們，所謂的日常究竟有多麼脆弱。",
          "es": "El cambio repentino de una noche normal a una casa llena de luto en Egipto es un recordatorio brutal de lo frágil que es nuestra normalidad.",
          "pt": "A mudança repentina de uma noite comum para uma casa cheia de luto no Egito é um lembrete brutal de quão frágil é a nossa normalidade.",
          "vi": "Sự thay đổi đột ngột từ một đêm bình thường sang khung cảnh tang thương khắp Ai Cập là lời nhắc nhở tàn nhẫn về sự mong manh của cuộc sống thường nhật.",
          "tl": "Ang biglaang pagbabago mula sa isang normal na gabi patungo sa bahay na puno ng pagdadalamhati sa Ehipto ay isang malupit na paalala kung gaano kahina ang ating normal na buhay.",
          "th": "การเปลี่ยนจากคืนที่ปกติสุขไปสู่ความโศกเศร้าทั่วทั้งอียิปต์ในชั่วข้ามคืน เป็นเครื่องเตือนใจที่โหดร้ายว่าความปกติของเรานั้นเปราะบางแค่ไหน",
          "sw": "Mabadiliko ya ghafla kutoka usiku wa kawaida hadi nyumba iliyojaa maombolezo huko Misri ni ukumbusho mkali wa jinsi maisha yetu yalivyo dhaifu.",
          "it": "Il passaggio improvviso da una notte tranquilla a una casa piena di lutto in Egitto è un promemoria brutale di quanto sia fragile la nostra normalità."
        }
    },
    "2026-04-02": {
        date: "2026-04-02",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 53:1-12",
          "en": "Isaiah 53:1-12",
          "ko": "Isaiah 53:1-12",
          "zho": "イザヤ書 53:1-12",
          "es": "Isaiah 53:1-12",
          "pt": "Isaiah 53:1-12",
          "vi": "Isaiah 53:1-12",
          "tl": "Isaiah 53:1-12",
          "th": "Isaiah 53:1-12",
          "sw": "Isaiah 53:1-12",
          "it": "Isaia 53:1-12"
        },
        comment: {
          "ja": "何一つ見栄えのしない姿で現れた救い主を誰も気に留めなかったという事実は、現代の私たちが外見や肩書きで人を判断しがちなことへの強烈な皮肉だ。",
          "en": "The fact that the Savior appeared in a way that didn't impress anyone is a brutal critique of our modern obsession with appearances and status.",
          "ko": "화려한 겉모습이 없다는 이유로 구세주를 아무도 거들떠보지 않았다는 것은, 외면과 스펙으로 사람을 판단하는 현대인들의 자화상을 그대로 보여준다.",
          "zho": "救主以平凡無奇的形象降臨卻被眾人忽視，這簡直是對現代人以貌取人和崇尚名利的絕佳諷刺。",
          "es": "Que el Salvador pasara desapercibido por no lucir impresionante es el reflejo perfecto de cómo solemos juzgar a los demás solo por su apariencia o estatus.",
          "pt": "O fato de o Salvador ter aparecido sem qualquer aparência chamativa e sido ignorado é uma crítica afiada à nossa mania moderna de julgar tudo pela capa.",
          "vi": "Việc Đấng Cứu Rỗi xuất hiện mà chẳng có vẻ ngoài hào nhoáng để rồi bị người đời ngó lơ là lời nhắc nhở cay đắng về thói đời trọng hình thức của chúng ta.",
          "tl": "Ang pagdating ng Tagapagligtas na walang anumang karangyaan at hindi napansin ng nakararami ay isang malinaw na salamin sa hilig nating manghusga base lang sa panlabas na anyo.",
          "th": "การที่พระผู้ช่วยให้รอดเสด็จมาโดยไม่มีรูปลักษณ์ที่น่าดึงดูดจนถูกมองข้าม เป็นการตบหน้าคนยุคนี้ที่มักตัดสินคนอื่นแค่จากเปลือกนอกและตำแหน่งหน้าที่",
          "sw": "Ukweli kwamba Mwokozi alikuja bila muonekano wa kuvutia na kupuuzwa na wengi ni onyo kali kuhusu tabia yetu ya kisasa ya kuhukumu watu kwa sura zao pekee.",
          "it": "Che il Salvatore sia apparso senza alcun tratto appariscente è una critica feroce alla nostra moderna ossessione per l'apparenza e lo status."
        }
    },
    "2026-04-03": {
        date: "2026-04-03",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 53:6-7",
          "en": "Isaiah 53:6-7",
          "ko": "Isaiah 53:6-7",
          "zho": "イザヤ書 53:6-7",
          "es": "Isaiah 53:6-7",
          "pt": "Isaiah 53:6-7",
          "vi": "Isaiah 53:6-7",
          "tl": "Isaiah 53:6-7",
          "th": "Isaiah 53:6-7",
          "sw": "Isaiah 53:6-7",
          "it": "Isaia 53:6-7"
        },
        comment: {
          "ja": "自分の好きな道へ勝手に迷い込んだ挙句に助けを求める人間と、黙々と屠殺場へ引かれる羊のように耐える姿の対比が、あまりに一方的で胸が痛い。",
          "en": "The contrast between us wandering off into our own messes and Him remaining silent like a sheep being led to slaughter is a painful reminder of how much He carries for us.",
          "ko": "제멋대로 길을 잃고 헤매는 우리와 도살장으로 끌려가는 양처럼 묵묵히 견디시는 그분의 모습은, 우리가 얼마나 많은 빚을 지고 사는지 새삼 깨닫게 한다.",
          "zho": "我們總是任性地偏行己路，而祂卻像被牽去宰殺的羊一樣沉默承受，這種對比讓人感到心酸又慚愧。",
          "es": "El contraste entre nosotros desviándonos por nuestra propia cuenta y Él permaneciendo en silencio como un cordero ante el matadero es un recordatorio doloroso de lo que realmente significa el sacrificio.",
          "pt": "A distância entre nós, que vivemos nos perdendo por vontade própria, e Ele, que se cala como um cordeiro a caminho do matadouro, é um contraste que dói na alma.",
          "vi": "Sự đối lập giữa việc chúng ta cứ tự ý đi chệch đường và Ngài thì lặng lẽ chịu đựng như con chiên bị dẫn đến nơi giết mổ thật sự là một lời nhắc nhở đau lòng.",
          "tl": "Ang pagkakaiba ng paglayas natin sa sarili nating mga maling desisyon at ang pananahimik Niya na parang tupang dinadala sa katayan ay isang masakit na paalala ng ating mga pagkukulang.",
          "th": "ความต่างระหว่างเราที่ชอบเตลิดเปิดเปิงไปตามทางของตัวเอง กับพระองค์ที่ยอมเงียบเหมือนลูกแกะที่ถูกนำไปเชือด เป็นความจริงที่น่าจุกอกและชวนให้คิดทบทวน",
          "sw": "Tofauti kati ya sisi kupotea katika njia zetu wenyewe na Yeye kunyamaza kama kondoo anayepelekwa machinjoni ni ukumbusho wa kusikitisha wa uzito wa kile alichokibeba kwa ajili yetu.",
          "it": "Il contrasto tra noi che ci perdiamo nei nostri guai e Lui che resta in silenzio come un agnello al mattatoio fa capire quanto peso porti per noi."
        }
    },
    "2026-04-04": {
        date: "2026-04-04",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 16:8-11",
          "en": "Psalms 16:8-11",
          "ko": "Psalms 16:8-11",
          "zho": "詩篇 16:8-11",
          "es": "Psalms 16:8-11",
          "pt": "Psalms 16:8-11",
          "vi": "Psalms 16:8-11",
          "tl": "Psalms 16:8-11",
          "th": "Psalms 16:8-11",
          "sw": "Psalms 16:8-11",
          "it": "Salmi 16:8-11"
        },
        comment: {
          "ja": "右を見ても左を見ても不安だらけの人生で、神様だけを指標にするという究極のブレない生き方はもはや職人芸。",
          "en": "Living a life of total uncertainty while keeping your eyes fixed on God is honestly a masterclass in staying centered.",
          "ko": "좌우 어디를 봐도 불안한 인생에서 오직 하나님만 바라보는 삶의 태도는 가히 달인의 경지다.",
          "zho": "在充滿變數的人生中，能將神視為唯一的指標，這種極致的專注簡直是生活的藝術。",
          "es": "Mantener la mirada puesta solo en Dios en medio de un mundo lleno de incertidumbre es, sin duda, un arte de maestros.",
          "pt": "Manter o foco apenas em Deus em meio a um mundo cheio de incertezas é, sem dúvida, uma arte de mestres.",
          "vi": "Giữa cuộc đời đầy bất an, việc chỉ giữ mắt hướng về Chúa đúng là một nghệ thuật sống đỉnh cao.",
          "tl": "Ang pananatiling nakatuon sa Diyos sa gitna ng magulong buhay ay isang antas ng disiplina na mahirap pantayan.",
          "th": "ท่ามกลางความไม่แน่นอนของชีวิต การยึดพระเจ้าเป็นศูนย์กลางเพียงหนึ่งเดียวถือเป็นทักษะขั้นสูงของการใช้ชีวิต",
          "sw": "Kuishi maisha yaliyojaa utata huku ukiweka macho yako kwa Mungu pekee ni sanaa ya hali ya juu ya utulivu.",
          "it": "Mantenere lo sguardo fisso su Dio in un mondo pieno di incertezze è onestamente un capolavoro di equilibrio interiore."
        }
    },
    "2026-04-05": {
        date: "2026-04-05",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 25:8-9",
          "en": "Isaiah 25:8-9",
          "ko": "Isaiah 25:8-9",
          "zho": "イザヤ書 25:8-9",
          "es": "Isaiah 25:8-9",
          "pt": "Isaiah 25:8-9",
          "vi": "Isaiah 25:8-9",
          "tl": "Isaiah 25:8-9",
          "th": "Isaiah 25:8-9",
          "sw": "Isaiah 25:8-9",
          "it": "Isaia 25:8-9"
        },
        comment: {
          "ja": "泣きっ面に蜂のような絶望の淵で、最後には必ず宴会が待っていると信じ抜くのは、もはや最強のポジティブシンキング。",
          "en": "Believing there is a feast waiting for you at the end of your worst day is the ultimate form of optimism.",
          "ko": "최악의 상황 속에서도 결국엔 잔치가 벌어질 것이라 믿는 그 마음가짐이야말로 최고의 긍정 에너지다.",
          "zho": "在絕望的深淵裡，依然深信最後會有盛宴等待著，這簡直是最高級的樂觀主義。",
          "es": "Creer que te espera un banquete al final de tu peor día es la forma más pura de optimismo.",
          "pt": "Acreditar que um banquete o aguarda no final do seu pior dia é a forma mais pura de otimismo.",
          "vi": "Tin rằng một bữa tiệc đang chờ đón ở cuối chuỗi ngày tồi tệ nhất chính là đỉnh cao của sự lạc quan.",
          "tl": "Ang paniniwalang may handaan na naghihintay sa dulo ng pinakamahirap mong araw ay ang pinakamataas na antas ng pag-asa.",
          "th": "การเชื่อว่าจะมีงานเลี้ยงรอคุณอยู่ในวันที่แย่ที่สุด คือรูปแบบของความคิดบวกที่ทรงพลังที่สุดแล้ว",
          "sw": "Kuamini kuwa kuna karamu inakusubiri mwishoni mwa siku yako ngumu zaidi ndiyo namna bora zaidi ya kuwa na matumaini.",
          "it": "Credere che ci sia un banchetto ad attenderci alla fine del giorno peggiore è la forma più pura di ottimismo."
        }
    },
    "2026-04-06": {
        date: "2026-04-06",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 7:1-25",
          "en": "Exodus 7:1-25",
          "ko": "Exodus 7:1-25",
          "zho": "出エジプト記 7:1-25",
          "es": "Exodus 7:1-25",
          "pt": "Exodus 7:1-25",
          "vi": "Exodus 7:1-25",
          "tl": "Exodus 7:1-25",
          "th": "Exodus 7:1-25",
          "sw": "Exodus 7:1-25",
          "it": "Esodo 7:1-25"
        },
        comment: {
          "ja": "杖を投げたら蛇になるという超常現象を目の当たりにしても、まずは手品師を呼んで対抗しようとするエジプト側の必死すぎる現実逃避が人間臭い。",
          "en": "Watching a staff turn into a serpent and immediately trying to find a magician to match the trick is the most stubborn display of denial I have ever seen.",
          "ko": "지팡이가 뱀으로 변하는 기적을 보고도 마술사를 불러 대응하려 하는 이집트인들의 모습은 현실을 부정하고 싶은 인간의 본능을 그대로 보여준다.",
          "zho": "亲眼目睹了权杖变成蛇的神迹，第一反应竟然是叫魔术师来比拼，这种为了面子而强撑的固执简直太真实了。",
          "es": "Ver un milagro y tratar de igualarlo con trucos de magia en lugar de reconocer la realidad es el ejemplo perfecto de nuestra terquedad humana.",
          "pt": "Ver um milagre acontecer e tentar superá-lo com truques de mágica em vez de encarar a realidade é o auge da teimosia humana.",
          "vi": "Chứng kiến phép màu mà vẫn cố gọi phù thủy đến để so tài, sự phủ nhận thực tế này đúng là bản chất khó bỏ của con người.",
          "tl": "Ang makakita ng himala at subukan pa ring tapatan ito ng mahika sa halip na maniwala ay patunay ng katigasan ng ulo ng tao.",
          "th": "การเห็นปาฏิหาริย์แล้วพยายามหาทางแก้ด้วยกลมายาแทนที่จะยอมรับความจริงนั้นช่างเป็นความดื้อรั้นที่น่าเห็นใจจริงๆ",
          "sw": "Kuona miujiza na kujaribu kuishinda kwa uchawi badala ya kukubali ukweli ni kilele cha ukaidi wa kibinadamu.",
          "it": "Vedere un bastone trasformarsi in serpente e cercare subito un mago per pareggiare il conto è la dimostrazione più ostinata di negazione mai vista."
        }
    },
    "2026-04-07": {
        date: "2026-04-07",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 8:1-32",
          "en": "Exodus 8:1-32",
          "ko": "Exodus 8:1-32",
          "zho": "出エジプト記 8:1-32",
          "es": "Exodus 8:1-32",
          "pt": "Exodus 8:1-32",
          "vi": "Exodus 8:1-32",
          "tl": "Exodus 8:1-32",
          "th": "Exodus 8:1-32",
          "sw": "Exodus 8:1-32",
          "it": "Esodo 8:1-32"
        },
        comment: {
          "ja": "カエルが大量発生して家の中まで占拠されているのに、少し落ち着くとまた元の態度に戻るパロのメンタルの切り替えの早さはある意味すごい。",
          "en": "Pharaoh promising to let the people go the moment the frogs show up and then changing his mind the second they are gone is the ultimate example of a fair-weather repentance.",
          "ko": "개구리 떼가 온 집안을 점령할 땐 당장이라도 보내줄 것처럼 굴다가, 상황이 나아지자마자 태도를 바꾸는 파라오의 모습에서 익숙한 인간의 변덕을 본다.",
          "zho": "青蛙占领了卧室时答应放人，青蛙一消失就立刻反悔，法老这种见风使舵的变脸速度简直是人类劣根性的教科书。",
          "es": "Prometer que dejará ir al pueblo cuando las ranas invaden su casa y retractarse apenas se van es el ejemplo perfecto de arrepentimiento por conveniencia.",
          "pt": "Prometer libertar o povo quando os sapos invadem a casa e mudar de ideia assim que eles somem é o exemplo máximo de arrependimento por conveniência.",
          "vi": "Hứa thả dân khi ếch nhái đầy nhà rồi đổi ý ngay khi chúng biến mất, cách lật mặt của Pha-ra-ôn đúng là kiểu hối cải chỉ khi gặp nạn.",
          "tl": "Ang mangako na palalayain ang bayan kapag puno ng palaka ang bahay at bawiin ito agad pagkaalis ng mga ito ay ang pinakamalinaw na halimbawa ng panandaliang pagsisisi.",
          "th": "การสัญญว่าจะปล่อยคนไปตอนที่กบเต็มบ้านแล้วกลับคำทันทีที่พวกมันหายไปนั้น เป็นตัวอย่างของการสำนึกผิดแบบฉาบฉวยที่เห็นได้บ่อยในชีวิตจริง",
          "sw": "Kuahidi kuwapa watu uhuru wakati vyura wamejaa nyumbani na kubadili nia mara tu wanapotoweka ni mfano bora wa toba ya msimu.",
          "it": "Promettere di liberare il popolo finché le rane invadono la casa e rimangiarsi tutto appena spariscono è il manuale del pentimento di convenienza."
        }
    },
    "2026-04-08": {
        date: "2026-04-08",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 9:1-35",
          "en": "Exodus 9:1-35",
          "ko": "Exodus 9:1-35",
          "zho": "出エジプト記 9:1-35",
          "es": "Exodus 9:1-35",
          "pt": "Exodus 9:1-35",
          "vi": "Exodus 9:1-35",
          "tl": "Exodus 9:1-35",
          "th": "Exodus 9:1-35",
          "sw": "Exodus 9:1-35",
          "it": "Esodo 9:1-35"
        },
        comment: {
          "ja": "パロが災いに襲われるたびに降伏のフリをしては、嵐が過ぎ去るとすぐ態度を翻す姿、都合が悪くなると急に謙虚になる自分を見ているようで苦しい。",
          "en": "Pharaoh promising to change only when things get tough and immediately reverting once the pressure is off is uncomfortably similar to my own New Year's resolutions.",
          "ko": "재앙이 닥칠 때마다 굴복하는 척하다가 상황이 좀 나아지면 다시 마음을 바꾸는 바로의 모습에서 인간의 간사한 본성을 보게 된다.",
          "zho": "法老每次在災難面前假裝妥協，一旦危機解除又立刻變臉，這不就是我們在壓力下才懂得謙卑的寫照嗎。",
          "es": "Faraón prometiendo cambiar cuando las cosas se ponen feas y volviendo a las andadas apenas pasa el peligro es básicamente la historia de mis promesas incumplidas.",
          "pt": "O Faraó prometendo mudar só quando a situação aperta e voltando atrás assim que a crise passa é o retrato fiel da nossa inconstância humana.",
          "vi": "Pharaoh hứa hẹn thay đổi mỗi khi gặp tai ương rồi lại lật lọng ngay khi mọi chuyện êm xuôi, nhìn mà thấy bóng dáng chính mình trong đó.",
          "tl": "Ang pagpapanggap ni Faraon na magbago kapag may sakuna at pagbabalik sa dati kapag ligtas na ay parang repleksyon ng ating sariling kawalan ng paninindigan.",
          "th": "ฟาโรห์ที่สัญญาว่าจะกลับตัวตอนเจอภัยพิบัติแต่พอสถานการณ์ดีขึ้นก็กลับไปเป็นเหมือนเดิม คือภาพสะท้อนความดื้อรั้นของมนุษย์ที่เห็นได้ชัดจริงๆ",
          "sw": "Farao kuahidi kubadilika wakati wa shida na kurejea kwenye tabia zake mara tu mambo yanapotulia ni mfano kamili wa jinsi tunavyokuwa na msimamo duni.",
          "it": "Faraone che promette di cambiare solo quando le cose si mettono male e torna subito alle vecchie abitudini mi ricorda fin troppo i miei buoni propositi di inizio anno."
        }
    },
    "2026-04-09": {
        date: "2026-04-09",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 10:1-29",
          "en": "Exodus 10:1-29",
          "ko": "Exodus 10:1-29",
          "zho": "出エジプト記 10:1-29",
          "es": "Exodus 10:1-29",
          "pt": "Exodus 10:1-29",
          "vi": "Exodus 10:1-29",
          "tl": "Exodus 10:1-29",
          "th": "Exodus 10:1-29",
          "sw": "Exodus 10:1-29",
          "it": "Esodo 10:1-29"
        },
        comment: {
          "ja": "目の前で国が壊滅に向かっているのに、まだ自分のプライドを守ろうと交渉の余地を探るパロの頑固さは、もはや一周回って逆にすごい。",
          "en": "Pharaoh trying to negotiate terms while his entire kingdom is literally crumbling around him is the ultimate masterclass in denial.",
          "ko": "나라가 망해가고 있는데도 끝까지 협상 조건을 따지며 자존심을 챙기는 바로의 고집은 정말 대단할 지경이다.",
          "zho": "國家都要毀滅了還在討價還價想維持面子，法老的這份固執簡直是人類拒絕面對現實的巔峰之作。",
          "es": "Que Faraón intente negociar los términos mientras su reino se desmorona a su alrededor es el ejemplo definitivo de negación ante la realidad.",
          "pt": "O Faraó tentando negociar termos enquanto seu reino desmorona ao seu redor é o exemplo máximo de alguém que se recusa a encarar a realidade.",
          "vi": "Vương quốc đang tan tành mà vẫn cố mặc cả để giữ thể diện, sự cứng đầu của Pharaoh thực sự là đỉnh cao của việc trốn tránh thực tại.",
          "tl": "Ang pagtatangkang makipag-negosasyon ni Faraon habang guguho na ang kanyang kaharian ay ang pinakamataas na antas ng pagtanggi sa katotohanan.",
          "th": "ฟาโรห์ที่ยังพยายามต่อรองเงื่อนไขทั้งที่อาณาจักรกำลังล่มสลาย คือตัวอย่างของคนที่ปฏิเสธความจริงได้อย่างน่าเหลือเชื่อ",
          "sw": "Farao kujaribu kufanya mazungumzo wakati ufalme wake wote unaporomoka ni mfano mkuu wa mtu anayekataa kukubali uhalisia wa mambo.",
          "it": "Vedere il Faraone che cerca di negoziare i termini mentre il suo regno cade a pezzi è il capolavoro assoluto della negazione della realtà."
        }
    },
    "2026-04-10": {
        date: "2026-04-10",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 11:1-10",
          "en": "Exodus 11:1-10",
          "ko": "Exodus 11:1-10",
          "zho": "出エジプト記 11:1-10",
          "es": "Exodus 11:1-10",
          "pt": "Exodus 11:1-10",
          "vi": "Exodus 11:1-10",
          "tl": "Exodus 11:1-10",
          "th": "Exodus 11:1-10",
          "sw": "Exodus 11:1-10",
          "it": "Esodo 11:1-10"
        },
        comment: {
          "ja": "散々強情を張った挙句に結局最後は「とにかく出て行ってくれ」と懇願するファラオ、追い詰められてから慌てるのはいつの時代も変わらない。",
          "en": "Pharaoh being stubborn for ages only to finally beg them to leave the second things get real is the ultimate lesson in waiting too long.",
          "ko": "끝까지 고집을 부리다가 막판에 가서야 제발 좀 나가달라고 애원하는 파라오를 보면, 뒤늦은 후회는 예나 지금이나 인간의 고질병인가 봅니다.",
          "zho": "法老在百般固執後終於崩潰求人快走，這種不到黃河心不死的性格，簡直是人類拖延症的極致展現。",
          "es": "El Faraón siendo terco hasta el último momento y luego rogando que todos se vayan es la prueba de que el ser humano siempre espera a estar contra la pared para reaccionar.",
          "pt": "O Faraó sendo teimoso até o último segundo e depois implorando para que todos saiam mostra que o ser humano só muda quando não tem mais saída.",
          "vi": "Pharaoh cố chấp đến phút cuối rồi mới vội vã cầu xin mọi người đi cho nhanh, đúng là kiểu người chỉ chịu thay đổi khi không còn đường lui.",
          "tl": "Ang pagmamatigas ni Faraon hanggang sa huli at ang biglang pagmamakaawa na umalis na sila ay patunay na tao nga talaga ang madalas maghintay sa huling sandali.",
          "th": "ฟาโรห์ที่ดื้อรั้นมาตลอดจนถึงนาทีสุดท้ายแล้วค่อยมาไล่คนอื่นให้ออกไปเร็วๆ นี่มันนิสัยคนเราชัดๆ ที่มักจะรอจนถึงวินาทีสุดท้ายค่อยยอมแพ้",
          "sw": "Farao kuwa mkaidi hadi dakika ya mwisho kisha kuwasihi watu waondoke haraka ni mfano kamili wa jinsi binadamu tunavyosubiri hadi hali iwe mbaya ndipo tutende.",
          "it": "Il Faraone che fa il testardo fino all'ultimo secondo per poi supplicare tutti di andarsene dimostra che aspettiamo sempre di essere con le spalle al muro prima di muoverci."
        }
    },
    "2026-04-11": {
        date: "2026-04-11",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 12:31-51",
          "en": "Exodus 12:31-51",
          "ko": "Exodus 12:31-51",
          "zho": "出エジプト記 12:31-51",
          "es": "Exodus 12:31-51",
          "pt": "Exodus 12:31-51",
          "vi": "Exodus 12:31-51",
          "tl": "Exodus 12:31-51",
          "th": "Exodus 12:31-51",
          "sw": "Exodus 12:31-51",
          "it": "Esodo 12:31-51"
        },
        comment: {
          "ja": "パン種を入れる暇もないほど急いで脱出したイスラエルの人々、準備万端なんて幻想で、人生は結局いつもバタバタの連続なのかもしれない。",
          "en": "The Israelites rushing out so fast they could not even let their bread rise is a reminder that life is rarely perfectly planned and usually just a frantic scramble.",
          "ko": "빵을 부풀릴 시간도 없이 급하게 떠난 이스라엘 백성들을 보니, 완벽한 준비란 환상이고 인생은 원래 정신없이 흘러가는 것 같네요.",
          "zho": "以色列人連麵糰發酵的時間都沒有就倉促逃離，這告訴我們人生很少有萬事俱備的時候，大多數時候都是在混亂中前行。",
          "es": "Los israelitas saliendo tan rápido que ni siquiera dejaron leudar el pan nos recuerda que la vida nunca es perfecta y casi siempre estamos improvisando sobre la marcha.",
          "pt": "Os israelitas saindo tão apressados que nem deram tempo ao pão de crescer mostra que a vida raramente é planejada e quase sempre é um corre-corre.",
          "vi": "Người Israel vội vã ra đi đến nỗi không kịp để bột nở, nhắc nhở chúng ta rằng cuộc đời hiếm khi nào hoàn hảo và thường là những chuỗi ngày chạy đua với thời gian.",
          "tl": "Ang pagmamadali ng mga Israelita na wala man lang panahon para pampaalsahin ang tinapay ay paalala na ang buhay ay madalas na hindi planado kundi isang malaking pagmamadali.",
          "th": "ชาวอิสราเอลที่รีบออกเดินทางจนไม่มีเวลาแม้แต่จะรอให้แป้งขนมปังขึ้นฟู เป็นเครื่องเตือนใจว่าชีวิตจริงไม่ค่อยมีอะไรที่เตรียมพร้อมสมบูรณ์แบบหรอก ส่วนใหญ่ก็ลนลานกันไปแบบนี้แหละ",
          "sw": "Waisraeli walivyotoka haraka kiasi cha kutopata muda wa kuchachusha mikate inatukumbusha kuwa maisha ni nadra kupangwa kikamilifu na mara nyingi huwa ni kukimbizana na wakati.",
          "it": "Gli israeliani che scappano così di fretta da non avere il tempo di far lievitare il pane ci ricordano che la vita non è mai perfettamente pianificata ma quasi sempre un'improvvisazione continua."
        }
    },
    "2026-04-12": {
        date: "2026-04-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 13:1-22",
          "en": "Exodus 13:1-22",
          "ko": "Exodus 13:1-22",
          "zho": "出エジプト記 13:1-22",
          "es": "Exodus 13:1-22",
          "pt": "Exodus 13:1-22",
          "vi": "Exodus 13:1-22",
          "tl": "Exodus 13:1-22",
          "th": "Exodus 13:1-22",
          "sw": "Exodus 13:1-22",
          "it": "Esodo 13:1-22"
        },
        comment: {
          "ja": "出発の準備でバタバタしている時にまで細かく指示が飛んでくるのは、神様もかなり心配性な保護者という証拠。",
          "en": "God giving detailed instructions while everyone is already frantic to leave proves that even the Almighty has a bit of an anxious parent energy.",
          "ko": "출발 준비로 정신없는 와중에도 꼼꼼한 지침이 내려오는 걸 보니 하나님도 꽤나 걱정 많은 부모님 스타일이신가 봅니다.",
          "zho": "在大家忙著打包行李準備出發時，上帝還在給予鉅細靡遺的指示，看來上帝也是個操碎了心的家長。",
          "es": "Que Dios diera instrucciones tan detalladas justo cuando todos estaban frenéticos por salir demuestra que hasta el Todopoderoso tiene un poco de ansiedad paternal.",
          "pt": "Deus dando instruções detalhadas bem quando todos estavam em pânico para sair prova que até o Todo-Poderoso tem um pouco de energia de pai preocupado.",
          "vi": "Việc Chúa đưa ra những chỉ dẫn chi tiết ngay lúc mọi người đang hối hả chuẩn bị lên đường cho thấy ngay cả Đấng Tối Cao cũng có chút lo lắng của một phụ huynh.",
          "tl": "Ang pagbibigay ng detalyadong utos ni God habang nagmamadali ang lahat ay patunay na kahit ang Maykapal ay may pagka-anxious na magulang din.",
          "th": "การที่พระเจ้าคอยกำชับรายละเอียดต่างๆ ในตอนที่ทุกคนกำลังรีบเก็บของหนีตาย แสดงว่าพระองค์ก็มีความเป็นผู้ปกครองขี้กังวลอยู่ไม่น้อย",
          "sw": "Mungu kutoa maagizo ya kina wakati kila mtu anahangaika kuondoka inadhihirisha kwamba hata Mwenyezi Mungu ana chembechembe za mzazi mwenye wasiwasi.",
          "it": "Che Dio dia istruzioni così dettagliate proprio mentre tutti sono nel panico più totale per partire dimostra che perfino l'Altissimo ha un po' di ansia da genitore apprensivo."
        }
    },
    "2026-04-13": {
        date: "2026-04-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 14:1-14",
          "en": "Exodus 14:1-14",
          "ko": "Exodus 14:1-14",
          "zho": "出エジプト記 14:1-14",
          "es": "Exodus 14:1-14",
          "pt": "Exodus 14:1-14",
          "vi": "Exodus 14:1-14",
          "tl": "Exodus 14:1-14",
          "th": "Exodus 14:1-14",
          "sw": "Exodus 14:1-14",
          "it": "Esodo 14:1-14"
        },
        comment: {
          "ja": "目の前に海、後ろから敵が迫る絶体絶命の状況で文句を言うイスラエルの民、追い詰められた時の人間は結局いつも同じ反応をする。",
          "en": "The Israelites complaining with the sea in front and the army behind is the ultimate example of how humans always revert to panic when cornered.",
          "ko": "앞에는 바다, 뒤에는 적군이 닥친 절체절명의 순간에 불평부터 쏟아내는 이스라엘 백성들을 보니, 궁지에 몰린 인간의 반응은 예나 지금이나 똑같네요.",
          "zho": "前有大海後有追兵，以色列百姓在絕境中仍不忘抱怨，看來人類在走投無路時的反應永遠都是那一套。",
          "es": "Que los israelitas se quejaran con el mar enfrente y el ejército detrás es el ejemplo perfecto de cómo los humanos siempre entramos en pánico cuando estamos acorralados.",
          "pt": "Os israelitas reclamando com o mar à frente e o exército atrás é o exemplo perfeito de como os humanos sempre entram em pânico quando estão encurralados.",
          "vi": "Việc người dân Y-sơ-ra-ên than phiền khi trước mặt là biển cả còn sau lưng là quân địch chính là minh chứng rõ nhất cho việc con người luôn hoảng loạn khi bị dồn vào đường cùng.",
          "tl": "Ang pagrereklamo ng mga Israelita habang nasa harap ang dagat at nasa likod ang kalaban ay patunay na lagi tayong nagpa-panic kapag naiipit sa sulok.",
          "th": "การที่ชาวอิสราเอลเอาแต่บ่นทั้งที่ข้างหน้าคือทะเลข้างหลังคือศัตรู เป็นเครื่องพิสูจน์ได้ดีว่ามนุษย์เรามักจะมีปฏิกิริยาแบบเดียวกันเสมอเมื่อจนตรอก",
          "sw": "Waisraeli kulalamika huku wakiwa wamebanwa na bahari mbele na adui nyuma ni mfano kamili wa jinsi binadamu wanavyopagawa kila wanapofikishwa pabaya.",
          "it": "Gli israeliani che si lamentano con il mare davanti e l'esercito alle spalle sono la prova provata che noi umani andiamo sempre nel panico quando siamo accerchiati."
        }
    },
    "2026-04-14": {
        date: "2026-04-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 14:15-31",
          "en": "Exodus 14:15-31",
          "ko": "Exodus 14:15-31",
          "zho": "出エジプト記 14:15-31",
          "es": "Exodus 14:15-31",
          "pt": "Exodus 14:15-31",
          "vi": "Exodus 14:15-31",
          "tl": "Exodus 14:15-31",
          "th": "Exodus 14:15-31",
          "sw": "Exodus 14:15-31",
          "it": "Esodo 14:15-31"
        },
        comment: {
          "ja": "絶体絶命の崖っぷちで神様に文句を言いたくなるのは、いつの時代も変わらない人間のデフォルト設定ですね。",
          "en": "Complaining to God when your back is against the wall is clearly the default human setting throughout history.",
          "ko": "막다른 골목에서 신을 원망하고 싶어지는 건 시대를 막론하고 인간의 기본 설정값인 것 같네요.",
          "zho": "在绝境中忍不住向神抱怨，这看来是古今中外人类的共同出厂设置。",
          "es": "Quejarse con Dios cuando estamos contra la pared es, claramente, la configuración predeterminada de los humanos a través de la historia.",
          "pt": "Reclamar com Deus quando estamos contra a parede é, claramente, a configuração padrão dos humanos através da história.",
          "vi": "Than vãn với Chúa khi lâm vào đường cùng dường như là phản ứng mặc định của con người qua mọi thời đại.",
          "tl": "Ang pagrereklamo sa Diyos kapag tayo ay gipit na gipit ay tila default na setting ng tao sa buong kasaysayan.",
          "th": "การบ่นกับพระเจ้าเมื่อเราจนมุมดูจะเป็นการตั้งค่าเริ่มต้นของมนุษย์ทุกยุคทุกสมัยจริงๆ",
          "sw": "Kulalamika kwa Mungu unapokuwa umekwama ni kama mpangilio wa kawaida wa binadamu katika historia yote.",
          "it": "Lamentarsi con il cielo quando ci si ritrova con le spalle al muro è chiaramente l'impostazione di fabbrica di noi umani da sempre."
        }
    },
    "2026-04-15": {
        date: "2026-04-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 15:1-22",
          "en": "Exodus 15:1-22",
          "ko": "Exodus 15:1-22",
          "zho": "出エジプト記 15:1-22",
          "es": "Exodus 15:1-22",
          "pt": "Exodus 15:1-22",
          "vi": "Exodus 15:1-22",
          "tl": "Exodus 15:1-22",
          "th": "Exodus 15:1-22",
          "sw": "Exodus 15:1-22",
          "it": "Esodo 15:1-22"
        },
        comment: {
          "ja": "奇跡を目の当たりにした直後にまた不平を言い始めるイスラエルの民、まるで三歩歩くと忘れるあの方のようで他人事とは思えません。",
          "en": "The Israelites complaining again right after witnessing a miracle is so relatable it hurts.",
          "ko": "기적을 목격하자마자 다시 불평을 시작하는 이스라엘 백성을 보니, 금방 잊어버리는 우리들의 모습이 떠올라 뜨끔하네요.",
          "zho": "亲眼见证神迹后转头就开始抱怨，以色列人这翻脸比翻书还快的操作，简直就是我们自己的缩影。",
          "es": "Que los israelitas se pusieran a quejarse justo después de ver un milagro es tan humano que duele.",
          "pt": "Os israelitas voltarem a reclamar logo após testemunharem um milagre é tão humano que chega a doer.",
          "vi": "Việc người Israel tiếp tục than vãn ngay sau khi chứng kiến phép lạ thật sự quá gần gũi với tính cách con người chúng ta.",
          "tl": "Ang pagrereklamo ulit ng mga Israelita pagkatapos makakita ng milagro ay sobrang relatable kaya nakakasakit ng damdamin.",
          "th": "การที่ชาวอิสราเอลกลับมาบ่นทันทีหลังจากเห็นปาฏิหาริย์เป็นอะไรที่เข้าใจได้จนน่าเจ็บใจ",
          "sw": "Waisraeli kulalamika tena mara baada ya kushuhudia muujiza ni hali ya kibinadamu inayoumiza kwa jinsi inavyotufanana.",
          "it": "Gli israeliani che ricominciano a lamentarsi subito dopo aver visto un miracolo sono di una coerenza disarmante che fa quasi male."
        }
    },
    "2026-04-16": {
        date: "2026-04-16",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 15:23-27",
          "en": "Exodus 15:23-27",
          "ko": "Exodus 15:23-27",
          "zho": "出エジプト記 15:23-27",
          "es": "Exodus 15:23-27",
          "pt": "Exodus 15:23-27",
          "vi": "Exodus 15:23-27",
          "tl": "Exodus 15:23-27",
          "th": "Exodus 15:23-27",
          "sw": "Exodus 15:23-27",
          "it": "Esodo 15:23-27"
        },
        comment: {
          "ja": "奇跡を体験した直後に水が苦いと文句を言う民の切り替えの早さは、現代のSNSの炎上の速さといい勝負をしている。",
          "en": "The people complaining about bitter water right after a miracle is the ancient equivalent of leaving a one-star review immediately after a great meal.",
          "ko": "기적을 경험한 직후 물이 쓰다고 불평하는 이스라엘 백성의 태도는 맛집에 가서 바로 악플을 다는 현대인의 성격 급함과 다를 게 없다.",
          "zho": "剛經歷過神蹟卻因為水苦就開始抱怨，這種翻臉比翻書還快的節奏，完全就是現代人面對不如意時的真實寫照。",
          "es": "Quejarse del agua amarga justo después de un milagro es el equivalente bíblico a dejar una reseña de una estrella inmediatamente después de un gran servicio.",
          "pt": "Reclamar da água amarga logo após um milagre é o equivalente bíblico de deixar uma avaliação negativa logo após um ótimo atendimento.",
          "vi": "Vừa được chứng kiến phép màu đã quay sang cằn nhằn vì nước đắng, thái độ này chẳng khác nào kiểu người vừa được giúp đỡ đã vội đòi hỏi thêm.",
          "tl": "Ang pagrereklamo sa mapait na tubig pagkatapos ng himala ay parang pag-iwan ng isang star na review pagkatapos ng masarap na pagkain.",
          "th": "เพิ่งเจอเรื่องมหัศจรรย์มาหมาดๆ แต่พอเจอน้ำขมก็บ่นทันที นี่มันวิถีมนุษย์ที่ลืมบุญคุณไวจนน่าตกใจจริงๆ",
          "sw": "Kulalamika kuhusu maji machungu mara tu baada ya muujiza ni sawa na mtu kutoa maoni mabaya mara baada ya kupata huduma bora.",
          "it": "Lamentarsi dell'acqua amara subito dopo un miracolo è l'equivalente biblico di lasciare una recensione a una stella un attimo dopo aver mangiato benissimo."
        }
    },
    "2026-04-17": {
        date: "2026-04-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 17:8-16",
          "en": "Exodus 17:8-16",
          "ko": "Exodus 17:8-16",
          "zho": "出エジプト記 17:8-16",
          "es": "Exodus 17:8-16",
          "pt": "Exodus 17:8-16",
          "vi": "Exodus 17:8-16",
          "tl": "Exodus 17:8-16",
          "th": "Exodus 17:8-16",
          "sw": "Exodus 17:8-16",
          "it": "Esodo 17:8-16"
        },
        comment: {
          "ja": "腕を上げ続けるのが限界でモーセが支えてもらう様子は、一人で全部抱え込まずにさっさと助けを求めることの大切さを教えてくれる。",
          "en": "Moses needing someone to hold up his tired arms is the ultimate reminder that even the strongest leaders cannot do everything alone.",
          "ko": "팔이 아파서 손을 들고 있기 힘든 모세의 모습은 아무리 대단한 리더라도 혼자 모든 것을 해결할 수는 없다는 뼈아픈 현실을 보여준다.",
          "zho": "摩西累到需要旁人幫忙撐著手，這簡直是告訴我們，再強的領導者如果硬撐著不求援，最後也會崩潰。",
          "es": "Que Moisés necesitara ayuda para mantener sus brazos en alto es el recordatorio definitivo de que incluso los líderes más fuertes no pueden hacerlo todo solos.",
          "pt": "Moisés precisar de ajuda para manter os braços erguidos é o lembrete definitivo de que até os líderes mais fortes não podem fazer tudo sozinhos.",
          "vi": "Việc Môi-se cần người đỡ lấy đôi tay mỏi mệt cho thấy ngay cả những người mạnh mẽ nhất cũng không thể làm mọi thứ một mình.",
          "tl": "Ang pagkapagod ni Moises na kailangan pang suportahan ang kanyang mga kamay ay paalala na kahit ang pinakamalakas na lider ay hindi kayang gawin ang lahat nang mag-isa.",
          "th": "ภาพโมเสสที่แขนล้าจนต้องมีคนมาช่วยพยุง เป็นเครื่องเตือนใจชั้นดีว่าต่อให้เก่งแค่ไหน ก็ไม่ควรแบกทุกอย่างไว้คนเดียว",
          "sw": "Musa kuhitaji watu wa kumsaidia kuinua mikono yake iliyochoka ni ukumbusho kwamba hata viongozi hodari hawawezi kufanya kila kitu peke yao.",
          "it": "Mosè che ha bisogno di qualcuno che gli tenga su le braccia stanche è il promemoria definitivo che nessuno, nemmeno il più forte, può fare tutto da solo."
        }
    },
    "2026-04-18": {
        date: "2026-04-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 18:1-12",
          "en": "Exodus 18:1-12",
          "ko": "Exodus 18:1-12",
          "zho": "出エジプト記 18:1-12",
          "es": "Exodus 18:1-12",
          "pt": "Exodus 18:1-12",
          "vi": "Exodus 18:1-12",
          "tl": "Exodus 18:1-12",
          "th": "Exodus 18:1-12",
          "sw": "Exodus 18:1-12",
          "it": "Esodo 18:1-12"
        },
        comment: {
          "ja": "モーセが義父イテロに武勇伝を語る姿は、久しぶりに実家に帰って親に仕事の苦労を聞いてもらうアラフォーの姿そのもの。",
          "en": "Moses catching up with his father-in-law feels exactly like a guy visiting his parents and finally venting about his stressful job.",
          "ko": "모세가 장인 이드로에게 무용담을 늘어놓는 모습은 오랜만에 고향에 내려가 부모님께 직장 고민을 털어놓는 우리네 모습과 다를 게 없다.",
          "zho": "摩西向岳父叶特罗分享奇迹，就像是久违回家向父母倾诉工作苦水的职场人，那种既委屈又自豪的心情太真实了。",
          "es": "Moisés contándole sus hazañas a su suegro se parece sospechosamente a cualquiera de nosotros visitando a la familia y desahogándose sobre el trabajo.",
          "pt": "Moisés contando suas aventuras ao sogro parece exatamente alguém visitando os pais e finalmente desabafando sobre o estresse do trabalho.",
          "vi": "Môi-se kể cho cha vợ nghe về những kỳ tích giống hệt như người làm con lâu ngày về nhà lại được dịp than thở về công việc với cha mẹ.",
          "tl": "Ang pagkuwento ni Moises ng kanyang mga tagumpay sa kanyang biyenan ay parang isang anak na umuwi sa probinsya para magsumbong tungkol sa hirap ng trabaho.",
          "th": "โมเสสเล่าเรื่องราวความสำเร็จให้พ่อตาฟังดูเหมือนลูกที่กลับบ้านไปบ่นเรื่องงานให้พ่อแม่ฟังด้วยความอัดอั้นใจจริงๆ",
          "sw": "Musa kumweleza baba mkwe wake mambo aliyopitia ni kama mtu anayetembelea wazazi wake na hatimaye kuelezea shinikizo zote za kazi yake.",
          "it": "Mosè che racconta le sue imprese al suocero sembra esattamente uno di noi che torna a casa dai genitori per sfogarsi sul lavoro."
        }
    },
    "2026-04-19": {
        date: "2026-04-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 18:13-27",
          "en": "Exodus 18:13-27",
          "ko": "Exodus 18:13-27",
          "zho": "出エジプト記 18:13-27",
          "es": "Exodus 18:13-27",
          "pt": "Exodus 18:13-27",
          "vi": "Exodus 18:13-27",
          "tl": "Exodus 18:13-27",
          "th": "Exodus 18:13-27",
          "sw": "Exodus 18:13-27",
          "it": "Esodo 18:13-27"
        },
        comment: {
          "ja": "一人で全部抱え込んでパンク寸前のモーセに組織論を説くイテロ、現場の空気を読まない上司というよりは、限界を迎えた部下を救う有能なコンサルタント。",
          "en": "Jethro telling Moses to delegate before he burns out is the ultimate reality check for anyone who thinks they have to do everything themselves.",
          "ko": "혼자 다 짊어지고 과부하가 걸린 모세에게 조직 관리법을 제안하는 이드로는, 꼰대 상사가 아니라 번아웃 직전의 부하를 살리는 유능한 컨설턴트다.",
          "zho": "叶特罗劝摩西学会放权，这简直是给所有试图一人扛下所有工作的完美主义者上了一堂职场生存课。",
          "es": "Jetro aconsejando a Moisés que delegue es el mejor recordatorio para todos los que creen que el mundo se acaba si no lo hacen todo ellos mismos.",
          "pt": "Jetro aconselhando Moisés a delegar é o choque de realidade perfeito para quem acha que precisa carregar o mundo nas costas.",
          "vi": "Việc Giê-trô khuyên Môi-se chia sẻ bớt công việc là lời nhắc nhở thực tế cho những ai cứ khăng khăng tự mình làm hết mọi thứ.",
          "tl": "Ang payo ni Jetro kay Moises na mag-delegate ay ang pinakamagandang sampal ng realidad para sa mga taong akala ay sila lang ang makakagawa ng lahat.",
          "th": "เยโธรแนะนำให้โมเสสรู้จักแบ่งงานเป็นบทเรียนที่เตือนใจคนชอบแบกโลกไว้คนเดียวได้ดีที่สุด",
          "sw": "Yethro kumshauri Musa kugawa majukumu ni ukumbusho mzuri kwa mtu yeyote anayefikiri kwamba lazima afanye kila kitu peke yake ili mambo yaende.",
          "it": "Jetro che consiglia a Mosè di delegare è il promemoria perfetto per chi pensa di dover fare sempre tutto da solo."
        }
    },
    "2026-04-20": {
        date: "2026-04-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 19:1-25",
          "en": "Exodus 19:1-25",
          "ko": "Exodus 19:1-25",
          "zho": "出エジプト記 19:1-25",
          "es": "Exodus 19:1-25",
          "pt": "Exodus 19:1-25",
          "vi": "Exodus 19:1-25",
          "tl": "Exodus 19:1-25",
          "th": "Exodus 19:1-25",
          "sw": "Exodus 19:1-25",
          "it": "Esodo 19:1-25"
        },
        comment: {
          "ja": "シナイ山に登るモーセを麓で待つイスラエルの民、神聖な場所へのアクセス権を制限されると途端に不安になるあたり、現代の行列待ちと変わらない。",
          "en": "The Israelites waiting at the foot of the mountain and getting anxious about being kept at a distance is just like waiting in line for a concert that might sell out.",
          "ko": "시내산 아래에서 모세가 내려오기만을 기다리며 발을 동동 구르는 이스라엘 백성들을 보니, 연락 없는 상대방을 기다리는 현대인의 조급함이 느껴진다.",
          "zho": "看著以色列人在山腳下焦急地等候摩西，那種被排除在核心圈之外的焦慮，簡直就像是沒收到派對邀請的現代人寫照。",
          "es": "Ver a los israelitas esperando al pie del monte, ansiosos por estar cerca de la acción, es como esperar en una fila interminable sin saber si llegarás al frente.",
          "pt": "Os israelitas esperando no sopé do monte, ansiosos por estarem fora do círculo principal, parecem qualquer um de nós esperando por uma resposta importante no celular.",
          "vi": "Dân Y-sơ-ra-ên đứng chờ dưới chân núi và lo lắng vì bị giữ khoảng cách, giống hệt cảm giác khi chúng ta phải đứng ngoài danh sách khách mời của một sự kiện quan trọng.",
          "tl": "Ang paghihintay ng mga Israelita sa paanan ng bundok na parang naiinip sa pila ay patunay na kahit noon pa man, mahirap talaga ang maghintay nang hindi nakikita ang nasa harapan.",
          "th": "ชาวอิสราเอลที่ยืนรออยู่ที่ตีนเขาด้วยความกระวนกระวายใจเพราะถูกกันให้อยู่ห่างๆ ช่างเหมือนกับความรู้สึกเวลาเราต้องรอคิวเข้างานสำคัญโดยไม่รู้ว่าเมื่อไหร่จะได้เข้า",
          "sw": "Waisraeli walipokuwa wakimsubiri Musa chini ya mlima kwa wasiwasi, walionyesha jinsi wanadamu tunavyochoka haraka tunapokosa mwongozo wa moja kwa moja.",
          "it": "Vedere gli israeliti in ansia ai piedi della montagna è come fare una fila infinita senza sapere se arriverai mai in cima."
        }
    },
    "2026-04-21": {
        date: "2026-04-21",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 20:1-26",
          "en": "Exodus 20:1-26",
          "ko": "Exodus 20:1-26",
          "zho": "出エジプト記 20:1-26",
          "es": "Exodus 20:1-26",
          "pt": "Exodus 20:1-26",
          "vi": "Exodus 20:1-26",
          "tl": "Exodus 20:1-26",
          "th": "Exodus 20:1-26",
          "sw": "Exodus 20:1-26",
          "it": "Esodo 20:1-26"
        },
        comment: {
          "ja": "十戒という究極のガイドラインを授かった直後なのに、もう次の失敗の準備をしている人間たち、学習能力の低さが逆に愛おしい。",
          "en": "Getting the Ten Commandments and immediately being ready to mess up is a level of human consistency that is almost impressive.",
          "ko": "십계명이라는 인생의 정답지를 바로 손에 쥐고도 곧바로 딴길로 샐 준비를 하는 인간의 모습은, 어쩌면 우리 모두의 평소 모습과 너무 똑같다.",
          "zho": "剛收到十誡這份人生終極指南，轉身就準備好要犯錯，這種人類特有的不穩定性真是讓人又氣又好笑。",
          "es": "Recibir los Diez Mandamientos y estar a punto de romperlos inmediatamente es la muestra más clara de que la coherencia humana es bastante cuestionable.",
          "pt": "Receber os Dez Mandamentos e já começar a planejar o próximo erro é um nível de teimosia humana que chega a ser surpreendente.",
          "vi": "Vừa nhận được mười điều răn như một bản hướng dẫn sống, con người đã sẵn sàng để phạm lỗi ngay sau đó, một sự nhất quán đáng kinh ngạc trong bản tính loài người.",
          "tl": "Ang matanggap ang Sampung Utos at agad na maghandang sumuway ay ang pinaka-relatable na bahagi ng pagiging tao na kahit kailan ay hindi nagbabago.",
          "th": "เพิ่งได้รับบัญญัติสิบประการเป็นแนวทางชีวิต แต่ก็พร้อมจะทำผิดกฎในทันที ความดื้อรั้นระดับนี้คือความเป็นมนุษย์ที่แท้จริงเลย",
          "sw": "Kupokea Amri Kumi na kuwa tayari kuzivunja mara moja ni ushahidi kwamba wanadamu tuna kipaji cha kipekee cha kujikwaa hata baada ya kuonyeshwa njia sahihi.",
          "it": "Ricevere i Dieci Comandamenti ed essere già pronti a infrangerli dimostra quanto la coerenza umana sia discutibile."
        }
    },
    "2026-04-22": {
        date: "2026-04-22",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 24:1-18",
          "en": "Exodus 24:1-18",
          "ko": "Exodus 24:1-18",
          "zho": "出エジプト記 24:1-18",
          "es": "Exodus 24:1-18",
          "pt": "Exodus 24:1-18",
          "vi": "Exodus 24:1-18",
          "tl": "Exodus 24:1-18",
          "th": "Exodus 24:1-18",
          "sw": "Exodus 24:1-18",
          "it": "Esodo 24:1-18"
        },
        comment: {
          "ja": "モーセが山で神と対話している間に下界で金の子牛を作ってしまうイスラエルの民、待つことの苦手さは今も昔も変わらない。",
          "en": "The Israelites building a golden calf while Moses was on the mountain proves that waiting patiently has never been humanity's strong suit.",
          "ko": "모세가 산에서 하나님과 대화하는 동안 금송아지를 만드는 이스라엘 백성들을 보면 기다림의 미학은 예나 지금이나 참 어렵다는 생각이 든다.",
          "zho": "當摩西在山上與神對話時，以色列人卻在山下造了一隻金牛犢，看來耐心等待這件事，古今中外都是人類的軟肋。",
          "es": "Los israelitas creando un becerro de oro mientras Moisés estaba en la montaña demuestra que la paciencia nunca ha sido nuestra mayor virtud.",
          "pt": "Os israelitas construindo um bezerro de ouro enquanto Moisés estava na montanha prova que a paciência nunca foi o forte da humanidade.",
          "vi": "Dân Israel đúc bò vàng trong lúc Môi Se đang ở trên núi cho thấy sự thiếu kiên nhẫn là bản tính khó bỏ của con người.",
          "tl": "Ang paggawa ng mga Israelita ng gintong baka habang nasa bundok si Moises ay patunay na ang paghihintay ay hindi kailanman naging madali para sa tao.",
          "th": "ชาวอิสราเอลสร้างรูปเคารพวัวทองคำในขณะที่โมเสสกำลังเข้าเฝ้าพระเจ้า พิสูจน์ให้เห็นว่าความอดทนรอคอยเป็นเรื่องยากสำหรับมนุษย์เสมอมา",
          "sw": "Waisraeli kutengeneza ndama wa dhahabu wakati Musa alipokuwa mlimani inaonyesha kuwa uvumilivu umekuwa changamoto kwa wanadamu tangu zamani.",
          "it": "Gli israeliti che creano un vitello d'oro mentre Mosè è sul monte dimostrano che la pazienza non è mai stata il nostro forte."
        }
    },
    "2026-04-23": {
        date: "2026-04-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 31:1-18",
          "en": "Exodus 31:1-18",
          "ko": "Exodus 31:1-18",
          "zho": "出エジプト記 31:1-18",
          "es": "Exodus 31:1-18",
          "pt": "Exodus 31:1-18",
          "vi": "Exodus 31:1-18",
          "tl": "Exodus 31:1-18",
          "th": "Exodus 31:1-18",
          "sw": "Exodus 31:1-18",
          "it": "Esodo 31:1-18"
        },
        comment: {
          "ja": "神殿の設計図を細部まで任されるベツァルエル、専門技術がいざという時に神の業を支えるという事実は、現代のエンジニアやクリエイターにも勇気をくれる。",
          "en": "God choosing a craftsman to build the tabernacle reminds us that our professional skills are just as much a part of His work as our prayers.",
          "ko": "하나님께서 성막 제작을 위해 기술자를 선택하신 것을 보면, 우리의 일상적인 재능과 기술 또한 하나님의 일을 완성하는 중요한 퍼즐 조각임을 깨닫게 된다.",
          "zho": "神挑選工匠來建造會幕，這提醒我們，我們在職場上的專業技能其實也是神聖事工的一部分。",
          "es": "Que Dios eligiera a un artesano para construir el tabernáculo nos recuerda que nuestras habilidades profesionales son tan valiosas para Su obra como nuestras oraciones.",
          "pt": "Deus escolher um artesão para construir o tabernáculo nos lembra de que nossas habilidades profissionais são tão valiosas para a obra d'Ele quanto nossas orações.",
          "vi": "Việc Chúa chọn một người thợ thủ công để xây dựng đền tạm nhắc nhở rằng kỹ năng nghề nghiệp của chúng ta cũng là một phần trong công việc của Ngài.",
          "tl": "Ang pagpili ng Diyos sa isang bihasang manggagawa para sa tabernakulo ay paalala na ang ating mga propesyonal na kasanayan ay bahagi rin ng Kanyang gawain.",
          "th": "การที่พระเจ้าทรงเลือกช่างฝีมือมาสร้างพลับพลาเตือนใจเราว่าทักษะวิชาชีพของเราก็เป็นส่วนหนึ่งของงานของพระองค์เช่นกัน",
          "sw": "Mungu kumchagua fundi stadi kujenga hema la kukutania inatukumbusha kuwa ujuzi wetu wa kitaaluma ni sehemu muhimu ya kazi Yake kama vile sala zetu.",
          "it": "Che Dio scelga un artigiano per costruire il tabernacolo ci ricorda che le nostre capacità professionali valgono quanto le nostre preghiere."
        }
    },
    "2026-04-24": {
        date: "2026-04-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 32:1-35",
          "en": "Exodus 32:1-35",
          "ko": "Exodus 32:1-35",
          "zho": "出エジプト記 32:1-35",
          "es": "Exodus 32:1-35",
          "pt": "Exodus 32:1-35",
          "vi": "Exodus 32:1-35",
          "tl": "Exodus 32:1-35",
          "th": "Exodus 32:1-35",
          "sw": "Exodus 32:1-35",
          "it": "Esodo 32:1-35"
        },
        comment: {
          "ja": "リーダーが少し席を外した隙に金の子牛を拝み出すイスラエルの民、監視の目がないと途端に自分たちのやりたい放題を正当化する人間の脆さそのもの。",
          "en": "The Israelites turning to a golden calf the moment Moses steps away is the ultimate testament to how quickly we abandon our values when we think no one is watching.",
          "ko": "지도자가 자리를 비운 짧은 순간에 금송아지를 만들어 숭배하는 이스라엘 백성들을 보면, 눈앞의 유혹에 얼마나 쉽게 흔들리는지 인간의 본성을 그대로 보여준다.",
          "zho": "摩西才离开一会儿，以色列人就急着铸造金牛犊，这完美演绎了当没有人监督时，人性是多么容易放飞自我。",
          "es": "Que los israelitas se pusieran a adorar un becerro de oro en cuanto Moisés se dio la vuelta es la prueba definitiva de nuestra capacidad para inventar excusas cuando no nos vigilan.",
          "pt": "Os israelitas criarem um bezerro de ouro assim que Moisés se virou é o exemplo perfeito de como facilmente justificamos nossos desvios quando achamos que ninguém está olhando.",
          "vi": "Việc dân Y-sơ-ra-ên đúc bò vàng ngay khi Môi-se vừa khuất bóng cho thấy bản chất con người dễ dàng sa ngã vào những giải pháp nhanh chóng thế nào khi không có ai giám sát.",
          "tl": "Ang paggawa ng gintong baka ng mga Israelita sa sandaling tumalikod si Moises ay patunay kung gaano kabilis tayong gumawa ng maling desisyon kapag akala natin ay walang nakakakita.",
          "th": "การที่ชาวอิสราเอลหันไปกราบไหว้รูปเคารพทันทีที่โมเสสไม่อยู่ เป็นเครื่องพิสูจน์ชัดเจนว่ามนุษย์เราพร้อมจะหาทางลัดให้ตัวเองแค่ไหนเมื่อไม่มีใครคอยเตือน",
          "sw": "Waisraeli kutengeneza ndama wa dhahabu mara tu Musa alipoondoka ni uthibitisho kamili wa jinsi tunavyojihalalishia mambo tunapofikiri hakuna anayetutazama.",
          "it": "Adorare un vitello d'oro appena Mosè si volta è la prova definitiva di come dimentichiamo i nostri valori quando nessuno ci guarda."
        }
    },
    "2026-04-25": {
        date: "2026-04-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 33:1-23",
          "en": "Exodus 33:1-23",
          "ko": "Exodus 33:1-23",
          "zho": "出エジプト記 33:1-23",
          "es": "Exodus 33:1-23",
          "pt": "Exodus 33:1-23",
          "vi": "Exodus 33:1-23",
          "tl": "Exodus 33:1-23",
          "th": "Exodus 33:1-23",
          "sw": "Exodus 33:1-23",
          "it": "Esodo 33:1-23"
        },
        comment: {
          "ja": "神様がもう一緒に行かないと言い出した途端、慌てて装飾品を外して反省モードに入る民、結局のところ損得勘定で動いているのが人間味あふれる。",
          "en": "The Israelites frantically stripping off their jewelry the moment God threatens to stop walking with them is the most relatable display of conditional repentance ever.",
          "ko": "하나님께서 동행하지 않겠다고 하시자마자 다급하게 장신구를 벗어던지며 회개하는 척하는 모습이 참으로 인간적인 손익 계산의 끝판왕이다.",
          "zho": "上帝一说不跟他们同行，以色列人就忙不迭地摘掉首饰装可怜，这种典型的利益导向悔改方式简直太真实了。",
          "es": "Que los israelitas se quitaran las joyas en cuanto Dios amenazó con dejarlos solos es el ejemplo más humano y realista de arrepentimiento condicionado que existe.",
          "pt": "Os israelitas tirarem as joias às pressas assim que Deus ameaçou não caminhar mais com eles é o exemplo mais honesto de arrependimento por interesse que já vi.",
          "vi": "Việc dân Y-sơ-ra-ên vội vàng tháo bỏ trang sức ngay khi Chúa dọa sẽ không đồng hành cùng họ là minh chứng rõ nhất cho kiểu ăn năn có điều kiện rất đỗi con người.",
          "tl": "Ang pagmamadaling hubarin ng mga Israelita ang kanilang mga alahas nang sabihin ng Diyos na hindi na Siya sasama sa kanila ay isang napakatotoong halimbawa ng pagsisising nakadepende sa sitwasyon.",
          "th": "การที่ชาวอิสราเอลรีบถอดเครื่องประดับออกทันทีที่พระเจ้าขู่ว่าจะไม่ไปต่อด้วย เป็นภาพสะท้อนความเห็นแก่ตัวแบบมนุษย์ๆ ที่ยอมสำนึกผิดเฉพาะตอนที่กลัวจะเสียผลประโยชน์",
          "sw": "Waisraeli kuvua vito vyao haraka mara tu Mungu alipotishia kutotembea nao ni mfano wa kweli wa jinsi tunavyotubu pale tu tunapohisi tunapoteza faida.",
          "it": "Gli israeliti che si tolgono i gioielli non appena Dio minaccia di lasciarli soli sono il ritratto perfetto del pentimento condizionato."
        }
    },
    "2026-04-26": {
        date: "2026-04-26",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 34:1-35",
          "en": "Exodus 34:1-35",
          "ko": "Exodus 34:1-35",
          "zho": "出エジプト記 34:1-35",
          "es": "Exodus 34:1-35",
          "pt": "Exodus 34:1-35",
          "vi": "Exodus 34:1-35",
          "tl": "Exodus 34:1-35",
          "th": "Exodus 34:1-35",
          "sw": "Exodus 34:1-35",
          "it": "Esodo 34:1-35"
        },
        comment: {
          "ja": "モーセが山から降りてきたら顔が光りすぎていて周囲がドン引きしている図、仕事で成果を出しすぎて逆に浮いてしまう悲しきエースそのもの。",
          "en": "Moses coming down from the mountain glowing so brightly that everyone is terrified is the ultimate example of being too successful for the room.",
          "ko": "산에서 내려온 모세의 얼굴이 너무 빛나서 다들 뒷걸음질 치는 상황, 회사에서 너무 열정적으로 일해서 동료들 눈치 보는 내 모습 같다.",
          "zho": "摩西下山時臉上發光到讓眾人退避三舍，這簡直就是因為表現太突出而顯得格格不入的職場精英寫照。",
          "es": "Moisés bajando de la montaña brillando tanto que todos se asustan es la representación perfecta de cuando destacas tanto que terminas incomodando a los demás.",
          "pt": "Moisés descendo da montanha brilhando tanto que o pessoal se assusta é o retrato fiel de quando você se esforça demais e acaba virando o estranho do grupo.",
          "vi": "Moses xuống núi với khuôn mặt sáng rực khiến mọi người sợ hãi, đúng là cảm giác khi mình quá nhiệt tình trong công việc mà lại thành ra lạc quẻ.",
          "tl": "Si Moises na bumaba mula sa bundok na nakakasilaw ang mukha kaya natakot ang lahat ay parang yung time na masyado kang magaling sa trabaho kaya naiilang na sayo ang mga kasama mo.",
          "th": "โมเสสลงจากภูเขาพร้อมใบหน้าที่ส่องสว่างจนคนอื่นตกใจกลัว เป็นภาพสะท้อนของคนที่ทำงานทุ่มเทเกินจนเพื่อนร่วมงานทำตัวไม่ถูก",
          "sw": "Musa kushuka kutoka mlimani akiwa anang'aa sana kiasi kwamba watu wanamwogopa ni kama vile unavyojituma sana kazini hadi wenzako wanakuona kama mgeni.",
          "it": "Mosè che scende dalla montagna splendendo così tanto da spaventare tutti è la rappresentazione classica di chi eccede troppo."
        }
    },
    "2026-04-27": {
        date: "2026-04-27",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 35:1-35",
          "en": "Exodus 35:1-35",
          "ko": "Exodus 35:1-35",
          "zho": "出エジプト記 35:1-35",
          "es": "Exodus 35:1-35",
          "pt": "Exodus 35:1-35",
          "vi": "Exodus 35:1-35",
          "tl": "Exodus 35:1-35",
          "th": "Exodus 35:1-35",
          "sw": "Exodus 35:1-35",
          "it": "Esodo 35:1-35"
        },
        comment: {
          "ja": "神殿の設えのために皆が材料を寄付しすぎて現場からストップがかかる事態、やる気が空回りするどころか溢れ出している人間臭さが最高。",
          "en": "People donating so many materials for the tabernacle that they had to be told to stop is the kind of over-enthusiasm we all wish we had for our chores.",
          "ko": "성막을 짓겠다고 다들 너무 열심히 기부해서 그만 가져오라고 말려야 했던 상황, 우리도 숙제나 집안일 앞에선 이 정도 열정이 있었으면 좋겠다.",
          "zho": "大家為了建造會幕捐獻太多物資以至於需要被喊停，這種熱情過剩到需要踩剎車的狀態，真是讓人哭笑不得的可愛。",
          "es": "Que la gente donara tantos materiales para el tabernáculo que tuvieran que pedirles que pararan es el tipo de entusiasmo que ojalá tuviéramos para las tareas del hogar.",
          "pt": "O pessoal doou tanto material para o tabernáculo que tiveram que pedir para pararem, um nível de empolgação que eu queria ter para lavar a louça.",
          "vi": "Mọi người quyên góp vật liệu xây đền tạm nhiều đến mức phải ra lệnh dừng lại, cái sự nhiệt tình quá mức này đúng là thứ chúng ta cần cho những công việc nhà chán ngắt.",
          "tl": "Yung sobra-sobra ang donasyon ng mga tao para sa tabernakulo kaya pinatigil na sila, sana ganyan din tayo ka-excited sa paggawa ng gawaing bahay.",
          "th": "ผู้คนบริจาคของสร้างพลับพลามากจนต้องประกาศให้หยุด เป็นความกระตือรือร้นที่ล้นทะลักจนน่าอิจฉา ผิดกับเวลาที่เราต้องทำงานบ้านจริงๆ",
          "sw": "Watu kutoa michango mingi sana kwa ajili ya hema ya kukutania kiasi kwamba walilazimika kuambiwa wasimamishe ni kiwango cha shauku tunachotamani kuwa nacho kwenye kazi za kawaida.",
          "it": "Donare così tanti materiali da dover fermare la gente è il tipo di entusiasmo che vorremmo avere per le faccende domestiche."
        }
    },
    "2026-04-28": {
        date: "2026-04-28",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 36:8-38",
          "en": "Exodus 36:8-38",
          "ko": "Exodus 36:8-38",
          "zho": "出エジプト記 36:8-38",
          "es": "Exodus 36:8-38",
          "pt": "Exodus 36:8-38",
          "vi": "Exodus 36:8-38",
          "tl": "Exodus 36:8-38",
          "th": "Exodus 36:8-38",
          "sw": "Exodus 36:8-38",
          "it": "Esodo 36:8-38"
        },
        comment: {
          "ja": "幕屋の細部までこだわりすぎて過集中気味の職人たち、こだわりが強すぎて納期に追われる現代のクリエイターそのもの。",
          "en": "The artisans getting lost in the minor details of the tabernacle design is exactly what happens when you spend three hours on a slide deck that nobody will read.",
          "ko": "성막의 세부 사항까지 집착하며 몰입하는 장인들의 모습, 마감 직전까지 디테일에 목숨 거는 현대의 직장인과 다를 게 없네요.",
          "zho": "工匠們對帳幕細節的過度執著，簡直就像現代人為了簡報的一個字體而熬夜加班的樣子。",
          "es": "Los artesanos perdiéndose en los detalles minuciosos del tabernáculo es la prueba de que el perfeccionismo nos hace perder de vista el panorama general desde el principio de los tiempos.",
          "pt": "Os artesãos obcecados por cada detalhe do tabernáculo são a prova de que o perfeccionismo e a procrastinação criativa sempre andaram de mãos dadas.",
          "vi": "Các nghệ nhân quá chú tâm vào từng chi tiết nhỏ của đền tạm, y hệt như chúng ta khi dành cả buổi chỉ để sửa font chữ cho một bài thuyết trình.",
          "tl": "Ang mga artisan na masyadong nakatutok sa maliliit na detalye ng tabernakulo ay parang tayo na naglalaan ng tatlong oras para sa isang slide na hindi naman babasahin ng iba.",
          "th": "ช่างฝีมือที่หมกมุ่นอยู่กับรายละเอียดเล็กๆ น้อยๆ ของพลับพลา ก็เหมือนกับเราที่ใช้เวลาทั้งวันกับการปรับฟอนต์ในสไลด์ที่ไม่มีใครสนใจจะอ่าน",
          "sw": "Mafundi waliozama kwenye maelezo madogo ya hema ya kukutania ni sawa na sisi tunavyopoteza muda mwingi kwenye vitu vidogo huku kazi kubwa ikisubiri.",
          "it": "Quegli artigiani persi nei dettagli del tabernacolo mi ricordano esattamente quando passo tre ore su una presentazione che nessuno leggerà mai."
        }
    },
    "2026-04-29": {
        date: "2026-04-29",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 38:1-31",
          "en": "Exodus 38:1-31",
          "ko": "Exodus 38:1-31",
          "zho": "出エジプト記 38:1-31",
          "es": "Exodus 38:1-31",
          "pt": "Exodus 38:1-31",
          "vi": "Exodus 38:1-31",
          "tl": "Exodus 38:1-31",
          "th": "Exodus 38:1-31",
          "sw": "Exodus 38:1-31",
          "it": "Esodo 38:1-31"
        },
        comment: {
          "ja": "数え切れないほどの金属を計量して記録する事務作業、どんなに聖なるプロジェクトでも最後は結局膨大な書類仕事に追われるという現実。",
          "en": "No matter how spiritual the mission, it always ends with someone having to sit down and do the tedious inventory spreadsheet.",
          "ko": "아무리 거룩한 사명이라도 결국 마지막엔 지루한 재고 조사와 서류 작업이 기다리고 있다는 현실이 묘하게 인간적이네요.",
          "zho": "無論使命多麼神聖，最後總歸是要面對繁瑣的庫存清點和文件處理，這就是現實。",
          "es": "Sin importar cuán sagrada sea la misión, al final siempre alguien tiene que sentarse a hacer el tedioso inventario de todo lo que se gastó.",
          "pt": "Não importa o quão sagrada seja a missão, no final do dia, alguém sempre tem que lidar com a planilha de inventário e a papelada chata.",
          "vi": "Dù sứ mệnh có thiêng liêng đến đâu, cuối cùng vẫn phải có người ngồi lại để kiểm kê sổ sách và làm những công việc giấy tờ nhàm chán.",
          "tl": "Kahit gaano kasagrado ang misyon, sa huli ay kailangan pa ring maglista ng imbentaryo at mag-ayos ng mga dokumento na nakakaantok.",
          "th": "ไม่ว่าภารกิจจะศักดิ์สิทธิ์แค่ไหน สุดท้ายก็หนีไม่พ้นการมานั่งทำบัญชีคลังสินค้าและจัดการงานเอกสารที่น่าเบื่อหน่ายอยู่ดี",
          "sw": "Haijalishi kazi ni takatifu kiasi gani, mwishowe lazima mtu akae chini na kufanya hesabu za kuchosha za vitu vilivyotumika.",
          "it": "Non importa quanto sia spirituale la missione, alla fine c'è sempre qualcuno costretto a fare il noioso inventario di fine mese."
        }
    },
    "2026-04-30": {
        date: "2026-04-30",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 39:1-43",
          "en": "Exodus 39:1-43",
          "ko": "Exodus 39:1-43",
          "zho": "出エジプト記 39:1-43",
          "es": "Exodus 39:1-43",
          "pt": "Exodus 39:1-43",
          "vi": "Exodus 39:1-43",
          "tl": "Exodus 39:1-43",
          "th": "Exodus 39:1-43",
          "sw": "Exodus 39:1-43",
          "it": "Esodo 39:1-43"
        },
        comment: {
          "ja": "神殿の細部まで厳格に指示通り作り上げた職人たち、こだわりが強すぎて締め切り前夜のクリエイターの顔になっている。",
          "en": "The craftsmen following every single detail of the tabernacle specs is the ultimate vibe of a perfectionist pulling an all-nighter.",
          "ko": "성막의 세세한 부분까지 완벽하게 만들어낸 장인들을 보니 마감 직전의 예민한 창작자를 보는 것 같아 동질감이 느껴진다.",
          "zho": "看著工匠們將會幕的細節做到精確無誤，簡直就是強迫症晚期患者在趕最後期限的真實寫照。",
          "es": "Ver a los artesanos seguir cada detalle técnico al pie de la letra me recuerda a cuando uno intenta que todo salga perfecto bajo presión.",
          "pt": "Ver os artesãos seguindo cada detalhe técnico à risca me lembra a sensação de tentar a perfeição sob pressão total.",
          "vi": "Những người thợ làm việc tỉ mỉ đến từng chi tiết cho đền tạm trông giống hệt như những người cầu toàn đang cố gắng hoàn thành deadline vậy.",
          "tl": "Ang mga artisan na sumusunod sa bawat detalye ng tabernakulo ay parang mga taong perfectionist na nagmamadali bago ang deadline.",
          "th": "เหล่าช่างฝีมือที่ตั้งใจทำรายละเอียดทุกอย่างให้เป๊ะตามสั่งดูเหมือนคนที่กำลังปั่นงานให้เสร็จก่อนเดดไลน์สุดๆ",
          "sw": "Wafundi wanaofuata kila maelekezo ya hema takatifu kwa usahihi wanaonekana kama watu wanaojitahidi sana kumaliza kazi kabla ya muda kuisha.",
          "it": "Vedere gli artigiani seguire ogni singolo dettaglio tecnico è la quintessenza del perfezionista che fa le ore piccole prima della consegna."
        }
    },
    "2026-05-01": {
        date: "2026-05-01",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 40:1-38",
          "en": "Exodus 40:1-38",
          "ko": "Exodus 40:1-38",
          "zho": "出エジプト記 40:1-38",
          "es": "Exodus 40:1-38",
          "pt": "Exodus 40:1-38",
          "vi": "Exodus 40:1-38",
          "tl": "Exodus 40:1-38",
          "th": "Exodus 40:1-38",
          "sw": "Exodus 40:1-38",
          "it": "Esodo 40:1-38"
        },
        comment: {
          "ja": "ついに完成した幕屋に雲が降りてきて神様が住まうという展開、もはや壮大な引越し完了報告を見ている気分になる。",
          "en": "The cloud finally descending on the finished tabernacle is basically the biblical version of a really intense housewarming party.",
          "ko": "성막이 완성되자마자 구름이 내려와 하나님이 입주하시는 장면을 보니 마치 거대한 이사가 성공적으로 끝난 것 같은 기분이다.",
          "zho": "帳幕終於大功告成，雲彩降臨象徵上帝入住，這簡直就是一場史詩級的暖房派對。",
          "es": "Que la nube descienda sobre el tabernáculo recién terminado se siente como el momento en que finalmente te mudas y todo empieza a encajar.",
          "pt": "A nuvem descendo sobre o tabernáculo recém-terminado parece aquele momento em que você finalmente se muda e tudo começa a se encaixar.",
          "vi": "Đám mây hạ xuống đền tạm sau khi hoàn thành giống như khoảnh khắc bạn dọn vào nhà mới và mọi thứ cuối cùng cũng ổn định.",
          "tl": "Ang ulap na bumaba sa natapos na tabernakulo ay parang ang eksena kung saan sa wakas ay nakalipat na ang may-ari ng bahay.",
          "th": "ฉากที่เมฆลงมาปกคลุมพลับพลาหลังสร้างเสร็จดูเหมือนโมเมนต์ที่ย้ายเข้าบ้านใหม่แล้วทุกอย่างลงตัวพอดี",
          "sw": "Wingu kushuka juu ya hema takatifu baada ya kukamilika ni kama wakati ambapo mtu anahamia nyumba mpya na kila kitu kinaanza kuwa sawa.",
          "it": "La nube che finalmente scende sul tabernacolo completato è la versione biblica di un trasloco andato incredibilmente bene."
        }
    },
    "2026-05-02": {
        date: "2026-05-02",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "レビ記 1:1-17",
          "en": "Leviticus 1:1-17",
          "ko": "Leviticus 1:1-17",
          "zho": "レビ記 1:1-17",
          "es": "Leviticus 1:1-17",
          "pt": "Leviticus 1:1-17",
          "vi": "Leviticus 1:1-17",
          "tl": "Leviticus 1:1-17",
          "th": "Leviticus 1:1-17",
          "sw": "Leviticus 1:1-17",
          "it": "Levitico 1:1-17"
        },
        comment: {
          "ja": "儀式の細かすぎる手順を読みながら、マニュアルを完璧にこなすこと自体が目的になっていないかと少しだけ心配になる。",
          "en": "Reading through all these hyper-specific sacrificial instructions, I have to wonder if we sometimes get so caught up in the manual that we miss the point.",
          "ko": "제사의 복잡한 절차를 읽다 보면 형식에 너무 치중해서 본질을 놓치기 쉬운 우리들의 모습이 떠오른다.",
          "zho": "看著這些繁瑣的獻祭儀式，不禁感嘆人類有時真的會為了遵守規矩而忘了初衷。",
          "es": "Al leer todas estas instrucciones tan detalladas sobre los sacrificios, me pregunto si a veces nos enfocamos tanto en el manual que perdemos de vista el propósito.",
          "pt": "Ao ler todas essas instruções detalhadas sobre os sacrifícios, percebo como é fácil nos perdermos nas regras e esquecermos o que realmente importa.",
          "vi": "Đọc những quy định chi tiết về tế lễ, tôi tự hỏi liệu có khi nào chúng ta quá chú trọng vào hình thức mà quên đi mục đích cốt lõi.",
          "tl": "Sa dami ng mga detalyadong tagubilin sa paghahandog, napapaisip ako kung minsan ay nagiging mas importante na lang ang pagsunod sa manual kaysa sa nilalaman ng puso.",
          "th": "เมื่ออ่านขั้นตอนการถวายเครื่องบูชาที่ละเอียดหยุมหยิม ก็อดคิดไม่ได้ว่าบางทีเราก็มักจะยึดติดกับพิธีรีตองจนลืมความหมายที่แท้จริงไป",
          "sw": "Kusoma maelekezo yote haya ya kina kuhusu dhabihu kunanifanya nijiulize kama wakati mwingine tunajikita sana kwenye taratibu kiasi cha kusahau kusudi la kweli.",
          "it": "Leggendo tutte queste istruzioni iper-specifiche sui sacrifici, mi chiedo se a volte ci perdiamo nel manuale dimenticandoci del punto centrale."
        }
    },
    "2026-05-03": {
        date: "2026-05-03",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "レビ記 16:1-34",
          "en": "Leviticus 16:1-34",
          "ko": "Leviticus 16:1-34",
          "zho": "レビ記 16:1-34",
          "es": "Leviticus 16:1-34",
          "pt": "Leviticus 16:1-34",
          "vi": "Leviticus 16:1-34",
          "tl": "Leviticus 16:1-34",
          "th": "Leviticus 16:1-34",
          "sw": "Leviticus 16:1-34",
          "it": "Levitico 16:1-34"
        },
        comment: {
          "ja": "罪を負わせたヤギを荒野に追い出すという大胆な解決策、自分の悩みもこうやって誰かがどこかへ連れ去ってくれたらどんなに楽かと思う。",
          "en": "The idea of loading all your problems onto a goat and sending it into the wilderness is a level of stress management I think we could all use.",
          "ko": "죄를 염소에게 씌워 광야로 보내버리는 이 파격적인 방식, 내 고민도 이렇게 깔끔하게 떠넘길 수 있다면 얼마나 좋을까.",
          "zho": "把罪孽都推給一隻羊並趕到曠野，這種處理壓力的方式如果能用在現代生活該有多好。",
          "es": "La idea de cargar todos tus problemas en un chivo y enviarlo al desierto es un nivel de gestión del estrés que todos envidiamos.",
          "pt": "A ideia de colocar todos os problemas em um bode e mandá-lo para o deserto é um nível de gestão de estresse que todos nós gostaríamos de experimentar.",
          "vi": "Ý tưởng trút hết gánh nặng tội lỗi lên một con dê rồi thả nó vào hoang mạc quả là cách giải tỏa căng thẳng mà ai cũng mong muốn.",
          "tl": "Ang ideya na ipasa ang lahat ng problema sa isang kambing at palayasin ito sa ilang ay isang paraan ng pag-manage ng stress na sana ay magagawa rin natin.",
          "th": "การโยนความผิดทั้งหมดให้แพะแล้วไล่มันเข้าป่าไป เป็นวิธีจัดการความเครียดที่ดูเรียบง่ายแต่ได้ผลอย่างน่าทึ่ง",
          "sw": "Wazo la kubebesha mbuzi dhambi zako zote na kumfukuza jangwani ni mbinu ya kushughulikia msongo wa mawazo ambayo tungependa sote tuiweze.",
          "it": "L'idea di caricare tutti i problemi su una capra e mandarla nel deserto è un livello di gestione dello stress che farebbe comodo a tutti."
        }
    },
    "2026-05-04": {
        date: "2026-05-04",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "民数記 11:1-35",
          "en": "Numbers 11:1-35",
          "ko": "Numbers 11:1-35",
          "zho": "民数記 11:1-35",
          "es": "Numbers 11:1-35",
          "pt": "Numbers 11:1-35",
          "vi": "Numbers 11:1-35",
          "tl": "Numbers 11:1-35",
          "th": "Numbers 11:1-35",
          "sw": "Numbers 11:1-35",
          "it": "Numeri 11:1-35"
        },
        comment: {
          "ja": "贅沢な食生活を夢見て文句を言い続ける民を前に、ついに神様に愚痴り始めたモーセの心中、完全に中間管理職のそれ。",
          "en": "Moses finally venting his frustrations to God because of the people's endless complaints about food is the ultimate middle-management mood.",
          "ko": "질 좋은 식단을 요구하며 불평을 쏟아내는 백성들 사이에서 하나님께 하소연하는 모세의 마음은 영락없는 중간 관리자의 비애다.",
          "zho": "看著百姓對食物永無止境的抱怨，摩西終於忍不住向神訴苦，這簡直是現代職場中層主管的真實寫照。",
          "es": "Moisés desahogándose con Dios ante las quejas constantes del pueblo por la comida es el perfecto reflejo de un gerente agotado por su equipo.",
          "pt": "Moisés desabafando com Deus por causa das reclamações incessantes do povo sobre a comida é a pura definição de um gerente sobrecarregado.",
          "vi": "Chứng kiến dân chúng không ngừng cằn nhằn về thực đơn, việc Môi-se phải than thở với Chúa chính là nỗi lòng của những người làm quản lý.",
          "tl": "Ang pagdaing ni Moises sa Diyos dahil sa walang katapusang reklamo ng mga tao tungkol sa pagkain ay tunay na damdamin ng isang pagod na lider.",
          "th": "การที่โมเสสต้องบ่นกับพระเจ้าเพราะคนในชาติเอาแต่โวยวายเรื่องอาหารช่างเป็นอารมณ์ของหัวหน้างานที่แบกรับความกดดันไว้เต็มที่",
          "sw": "Musa kulalamika kwa Mungu kwa sababu ya malalamiko ya watu kuhusu chakula ni hali halisi ya kiongozi aliyelemewa na majukumu.",
          "it": "Mosè che finalmente sbotta con Dio a causa delle lamentele sul cibo è lo stereotipo perfetto del manager allo stremo con la sua squadra."
        }
    },
    "2026-05-05": {
        date: "2026-05-05",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "民数記 13:1-38",
          "en": "Numbers 13:1-38",
          "ko": "Numbers 13:1-38",
          "zho": "民数記 13:1-38",
          "es": "Numbers 13:1-38",
          "pt": "Numbers 13:1-38",
          "vi": "Numbers 13:1-38",
          "tl": "Numbers 13:1-38",
          "th": "Numbers 13:1-38",
          "sw": "Numbers 13:1-38",
          "it": "Numeri 13:1-38"
        },
        comment: {
          "ja": "目の前の現実に圧倒されて自分たちを勝手に草むらの虫扱いする偵察隊、ポジティブ思考の難しさを痛感する。",
          "en": "The scouts looking at the giants and immediately deciding they are just grasshoppers is a masterclass in self-sabotaging insecurity.",
          "ko": "거인들을 보고 겁먹어 스스로를 메뚜기라 칭하는 정탐꾼들을 보며, 부정적인 생각이 얼마나 쉽게 자신감을 갉아먹는지 깨닫는다.",
          "zho": "偵察隊看到巨人就自降身分說自己像蚱蜢一樣，這種還沒開打就先自我否定的心態真是太寫實了。",
          "es": "Los espías viendo gigantes y sintiéndose como saltamontes es el ejemplo perfecto de cómo nuestras inseguridades nos hacen sentir pequeños.",
          "pt": "Os espiões vendo gigantes e se sentindo como gafanhotos é o exemplo perfeito de como a insegurança pode nos diminuir antes mesmo da luta começar.",
          "vi": "Việc các thám tử nhìn thấy những người khổng lồ rồi tự coi mình là châu chấu chính là ví dụ điển hình cho việc tự ti làm lu mờ năng lực bản thân.",
          "tl": "Ang mga espiya na nakakita ng mga higante at nag-isip agad na sila ay parang mga tipaklong ay tunay na halimbawa ng pagliit ng tingin sa sarili dahil sa takot.",
          "th": "การที่เหล่าคนสอดแนมเห็นยักษ์แล้วรีบตัดสินใจว่าตัวเองเป็นแค่ตั๊กแตนช่างเป็นตัวอย่างของความไม่มั่นใจที่ทำลายตัวเองได้ดีจริงๆ",
          "sw": "Wapelelezi kuona majitu na kujiona kama panzi ni mfano bora wa jinsi ukosefu wa kujiamini unavyotufanya tujidharau kabla ya pambano.",
          "it": "Le spie che guardano i giganti e si sentono subito come cavallette sono la prova lampante di come l'insicurezza ci faccia rimpicciolire."
        }
    },
    "2026-05-06": {
        date: "2026-05-06",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "民数記 14:6-45",
          "en": "Numbers 14:6-45",
          "ko": "Numbers 14:6-45",
          "zho": "民数記 14:6-45",
          "es": "Numbers 14:6-45",
          "pt": "Numbers 14:6-45",
          "vi": "Numbers 14:6-45",
          "tl": "Numbers 14:6-45",
          "th": "Numbers 14:6-45",
          "sw": "Numbers 14:6-45",
          "it": "Numeri 14:6-45"
        },
        comment: {
          "ja": "散々文句を言った挙句に神様に怒られてから急にやる気を出して突撃し、返り討ちにあうイスラエルの人々、計画性のなさが一周回って他人事とは思えない。",
          "en": "The Israelites complaining, getting scolded, and then blindly charging into battle only to get crushed is the ultimate case of bad timing and poor planning.",
          "ko": "불평만 늘어놓다가 혼나고 나서야 갑자기 의욕적으로 돌격했다가 참패하는 이스라엘 백성을 보니, 계획 없는 행동의 최후가 남 일 같지 않네요.",
          "zho": "以色列人抱怨完被责备后，突然热血冲锋却惨遭反杀，这种毫无规划的鲁莽行为简直是现实生活的写照。",
          "es": "Los israelitas quejándose, siendo regañados y luego lanzándose a la batalla sin pensar para terminar derrotados es el ejemplo perfecto de actuar sin medir las consecuencias.",
          "pt": "Os israelitas reclamando, levando um puxão de orelha e depois correndo para a batalha sem preparo algum só para serem derrotados é a definição de agir por impulso.",
          "vi": "Người Israel phàn nàn rồi bị quở trách, sau đó lại vội vàng xông vào trận chiến để rồi nhận lấy thất bại, đúng là bài học về việc hành động thiếu suy nghĩ.",
          "tl": "Ang mga Israelita na nagreklamo, napagalitan, at biglang sumugod sa labanan para lang matalo ay patunay na ang padalos-dalos na desisyon ay laging nauuwi sa gulo.",
          "th": "ชาวอิสราเอลที่บ่นกระปอดกระแปดพอโดนดุเข้าหน่อยก็ฮึดสู้จนบุกไปแพ้กลับมา เป็นตัวอย่างชัดเจนของการทำอะไรโดยไม่คิดหน้าคิดหลัง",
          "sw": "Waisraeli kulalamika, kukemewa, halafu kukimbilia vitani bila mpango na kushindwa vibaya ni mfano kamili wa jinsi tunavyofanya maamuzi ya haraka bila kufikiri.",
          "it": "Israele che si lamenta, viene sgridato e poi si lancia alla cieca in battaglia rimediando una sconfitta è l'apice della pessima pianificazione."
        }
    },
    "2026-05-07": {
        date: "2026-05-07",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "民数記 20:14-29",
          "en": "Numbers 20:14-29",
          "ko": "Numbers 20:14-29",
          "zho": "民数記 20:14-29",
          "es": "Numbers 20:14-29",
          "pt": "Numbers 20:14-29",
          "vi": "Numbers 20:14-29",
          "tl": "Numbers 20:14-29",
          "th": "Numbers 20:14-29",
          "sw": "Numbers 20:14-29",
          "it": "Numeri 20:14-29"
        },
        comment: {
          "ja": "モーセが一生懸命お願いしたのにエドムの王に「絶対に通さない」と門前払いされる様子は、どれほどリーダーでも交渉がうまくいかない時の気まずさを物語っている。",
          "en": "Moses getting a hard no from the King of Edom after asking nicely is a brutal reminder that even the greatest leaders get ghosted.",
          "ko": "모세가 정중하게 부탁했는데도 에돔 왕에게 단칼에 거절당하는 장면을 보니, 아무리 위대한 리더라도 거절의 쓴맛은 피할 수 없나 봅니다.",
          "zho": "摩西客客气气地请求却被以东王冷酷拒绝，这说明无论你是多么伟大的领袖，都难免会遇到这种尴尬的碰壁时刻。",
          "es": "Moisés pidiendo permiso amablemente y siendo rechazado rotundamente por el rey de Edom nos recuerda que incluso a los grandes líderes les cierran la puerta en la cara.",
          "pt": "Moisés pedindo gentilmente e sendo rejeitado pelo rei de Edom é a prova de que até os maiores líderes passam por aquele momento constrangedor de ter a porta fechada na cara.",
          "vi": "Môi-se lịch sự xin phép nhưng bị vua Ê-đôm từ chối thẳng thừng, đúng là minh chứng cho thấy ngay cả lãnh đạo vĩ đại cũng có lúc gặp cảnh bẽ bàng khi đàm phán.",
          "tl": "Ang magalang na pakiusap ni Moises na tinanggihan lang ng hari ng Edom ay patunay na kahit ang pinakamahusay na lider ay nakakaranas ng pagkabigo sa negosasyon.",
          "th": "โมเสสขอร้องอย่างสุภาพแต่กลับถูกกษัตริย์แห่งเอโดมปฏิเสธหน้าตาเฉย เป็นเครื่องเตือนใจว่าต่อให้เป็นผู้นำที่ยิ่งใหญ่แค่ไหนก็ต้องมีวันที่เจรจาไม่สำเร็จ",
          "sw": "Musa kuomba kwa heshima na kukataliwa na mfalme wa Edomu ni ukumbusho kwamba hata viongozi wakuu hupata wakati mgumu wakati mambo hayaendi kama walivyopanga.",
          "it": "Mosè che riceve un secco rifiuto dal re di Edom ci ricorda che a volte ricevono un bel due di picche anche i leader più grandi."
        }
    },
    "2026-05-08": {
        date: "2026-05-08",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "民数記 22:1-41",
          "en": "Numbers 22:1-41",
          "ko": "Numbers 22:1-41",
          "zho": "民数記 22:1-41",
          "es": "Numbers 22:1-41",
          "pt": "Numbers 22:1-41",
          "vi": "Numbers 22:1-41",
          "tl": "Numbers 22:1-41",
          "th": "Numbers 22:1-41",
          "sw": "Numbers 22:1-41",
          "it": "Numeri 22:1-41"
        },
        comment: {
          "ja": "自分のロバに論破されるまで神の意志に気づかないバラム、空気を読めない頑固者の末路としてこれ以上の教訓はない。",
          "en": "Balaam not realizing he is disobeying God until his own donkey starts talking back is the ultimate reality check for the stubborn.",
          "ko": "나귀에게까지 논리적으로 반박당하고 나서야 정신을 차리는 발람을 보면, 고집불통이 얼마나 무서운 것인지 실감하게 된다.",
          "zho": "直到被自己的驴子反驳才意识到违背了神意，巴兰这种固执到极致的表现真是让人哭笑不得。",
          "es": "Que un burro tenga que abrirle los ojos a Balaam sobre su terquedad es la lección de humildad más humillante de toda la Biblia.",
          "pt": "Balaam não perceber que estava indo contra Deus até ser confrontado pelo próprio jumento é o auge da teimosia humana.",
          "vi": "Việc Balaam phải để một con lừa chỉ ra sai lầm của mình mới chịu tỉnh ngộ chính là bài học cay đắng cho những kẻ cứng đầu.",
          "tl": "Ang mapahiya ng sarili mong asno dahil sa katigasan ng ulo mo ay isang antas ng pagkakamali na hindi mo na gugustuhing maranasan.",
          "th": "การที่ต้องให้ลามาพูดเตือนสติถึงจะยอมหยุดดื้อรั้น แสดงให้เห็นว่าบางครั้งมนุษย์เราก็ต้องการการเตือนที่แรงกว่าคำพูดคนทั่วไป",
          "sw": "Balaamu kushindwa kuelewa mapenzi ya Mungu hadi punda wake alipoanza kumkosoa ni kielelezo kamili cha jinsi ukaidi unavyotufanya kuwa vipofu.",
          "it": "Che un asino debba rimettere in riga Balaam è la prova definitiva che la testardaggine non ha limiti."
        }
    },
    "2026-05-09": {
        date: "2026-05-09",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "民数記 23:1-30",
          "en": "Numbers 23:1-30",
          "ko": "Numbers 23:1-30",
          "zho": "民数記 23:1-30",
          "es": "Numbers 23:1-30",
          "pt": "Numbers 23:1-30",
          "vi": "Numbers 23:1-30",
          "tl": "Numbers 23:1-30",
          "th": "Numbers 23:1-30",
          "sw": "Numbers 23:1-30",
          "it": "Numeri 23:1-30"
        },
        comment: {
          "ja": "呪ってくれと頼まれているのに祝福しか出てこないバラム、もはや自分の口が制御不能になっているのが滑稽で人間味がある。",
          "en": "Balaam desperately trying to curse but only spitting out blessings instead is a hilarious reminder that sometimes your mouth just won't cooperate with your bad attitude.",
          "ko": "저주를 퍼부어달라는 의뢰를 받았는데 자꾸 축복만 튀어나오는 발람을 보면, 결국 신의 뜻은 인간의 의도보다 훨씬 강하다는 게 느껴진다.",
          "zho": "明明想受雇去咒诅，结果出口全是祝福，巴兰这种想坏却坏不彻底的窘境其实挺真实的。",
          "es": "Balaam intentando maldecir y terminando por bendecir a todos es la prueba de que, a veces, tus propias palabras te traicionan para hacer lo correcto.",
          "pt": "Balaam tentando amaldiçoar e só conseguindo abençoar é a prova de que, quando o plano é de Deus, nem a sua má vontade consegue atrapalhar.",
          "vi": "Cố gắng buông lời nguyền rủa nhưng kết quả lại toàn là ban phước, Balaam cho thấy rằng đôi khi chính miệng chúng ta cũng phải phục tùng ý muốn cao hơn.",
          "tl": "Ang pilit mong manumpa pero puro pagpapala lang ang lumalabas sa bibig mo ay patunay na minsan, hindi mo kayang labanan ang plano ng nasa itaas.",
          "th": "การที่พยายามจะแช่งตามคำสั่งแต่กลับหลุดออกมาเป็นคำอวยพร แสดงให้เห็นว่าบางครั้งความตั้งใจของมนุษย์ก็แพ้ทางให้กับแผนการที่ใหญ่กว่าเสมอ",
          "sw": "Balaamu kujaribu kulaani lakini maneno ya baraka yakamtoka badala yake ni ukumbusho kwamba mipango ya Mungu huwa inashinda nia zetu mbaya.",
          "it": "Balaam che prova a insultare tutti ma riesce solo a fare complimenti dimostra che a volte la lingua va per conto suo."
        }
    },
    "2026-05-10": {
        date: "2026-05-10",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "民数記 27:1-23",
          "en": "Numbers 27:1-23",
          "ko": "Numbers 27:1-23",
          "zho": "民数記 27:1-23",
          "es": "Numbers 27:1-23",
          "pt": "Numbers 27:1-23",
          "vi": "Numbers 27:1-23",
          "tl": "Numbers 27:1-23",
          "th": "Numbers 27:1-23",
          "sw": "Numbers 27:1-23",
          "it": "Numeri 27:1-23"
        },
        comment: {
          "ja": "相続権がなかった娘たちが前例を覆して権利を勝ち取った様子は、現代の会議室で声を上げる若手社員のたくましさと重なる。",
          "en": "The daughters of Zelophehad challenging the status quo to claim their inheritance is the ultimate 'speak up for yourself' power move.",
          "ko": "상속권이 없던 딸들이 전례를 깨고 당당히 권리를 요구하는 모습은 현대 사회의 당찬 직장인들을 보는 것 같다.",
          "zho": "西罗非哈的女儿们打破传统争取继承权，这种勇于挑战现状的精神，简直就是现代职场敢于发声的年轻人缩影。",
          "es": "Las hijas de Zelofehad rompiendo las reglas para reclamar su herencia es la prueba de que a veces hay que alzar la voz para que las cosas cambien.",
          "pt": "As filhas de Zelofeade quebrando protocolos para reivindicar sua herança é a prova de que, às vezes, é preciso desafiar o sistema para ser ouvida.",
          "vi": "Những người con gái của Xê-lô-phát dám phá vỡ quy tắc để đòi quyền lợi cho mình thực sự là hình mẫu của sự bản lĩnh trong mọi thời đại.",
          "tl": "Ang mga anak na babae ni Zelofehad na lumaban para sa kanilang karapatan ay patunay na minsan kailangan mong magsalita para mabago ang sistema.",
          "th": "เหล่าลูกสาวของเศโลเฟหัดที่กล้าท้าทายจารีตเพื่อทวงสิทธิ์ของตน คือตัวอย่างของความกล้าหาญที่ทันสมัยสุดๆ",
          "sw": "Binti za Selofehadi kupinga utaratibu ili kudai haki yao ni mfano bora wa ujasiri wa kusimamia kile unachostahili.",
          "it": "Le figlie di Zelofhead che ribaltano le regole per avere ciò che spetta loro sono il classico esempio di chi non si arrende mai."
        }
    },
    "2026-05-11": {
        date: "2026-05-11",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "申命記 6:1-25",
          "en": "Deuteronomy 6:1-25",
          "ko": "Deuteronomy 6:1-25",
          "zho": "申命記 6:1-25",
          "es": "Deuteronomy 6:1-25",
          "pt": "Deuteronomy 6:1-25",
          "vi": "Deuteronomy 6:1-25",
          "tl": "Deuteronomy 6:1-25",
          "th": "Deuteronomy 6:1-25",
          "sw": "Deuteronomy 6:1-25",
          "it": "Deuteronomio 6:1-25"
        },
        comment: {
          "ja": "家中に教えを書き記せという教訓、現代ならスマホの待ち受け画面を全部聖句にするくらい必死な親心を感じる。",
          "en": "Telling parents to keep commandments on their doorposts and foreheads is basically the ancient version of setting a daily notification reminder on your phone.",
          "ko": "집안 곳곳에 말씀을 붙여두라는 가르침은 오늘날로 치면 휴대폰 배경화면을 성구로 도배해두는 것과 다를 바 없다.",
          "zho": "把诫命刻在门框上，这简直就是古人版的手机壁纸提醒，生怕自己一转头就忘得一干二净。",
          "es": "Tener los mandamientos siempre presentes en la puerta y en la frente es básicamente el equivalente bíblico de poner un recordatorio constante en el celular.",
          "pt": "Ter os mandamentos sempre à vista é basicamente a versão antiga de colocar um lembrete no celular para não esquecer o que realmente importa.",
          "vi": "Dặn dò khắc ghi lời dạy khắp nhà chẳng khác nào việc cài thông báo nhắc nhở trên điện thoại để không quên những điều quan trọng mỗi ngày.",
          "tl": "Ang pagpapaskil ng mga utos sa pinto ay parang paglalagay ng wallpaper sa cellphone para hindi makalimutan ang mga dapat gawin sa araw-araw.",
          "th": "การให้เขียนคำสอนไว้ที่ประตูบ้านก็เหมือนกับการตั้งเตือนในมือถือเพื่อให้เราไม่ลืมสิ่งที่สำคัญที่สุดในชีวิตทุกวัน",
          "sw": "Kuandika mafundisho kila mahali ni kama kuweka ukumbusho kwenye simu ili usisahau mambo muhimu unayopaswa kufanya kila siku.",
          "it": "Mettere i comandamenti ovunque in casa è l'antenato spirituale del mettere le notifiche importanti sullo schermo del telefono."
        }
    },
    "2026-05-12": {
        date: "2026-05-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "申命記 7:1-26",
          "en": "Deuteronomy 7:1-26",
          "ko": "Deuteronomy 7:1-26",
          "zho": "申命記 7:1-26",
          "es": "Deuteronomy 7:1-26",
          "pt": "Deuteronomy 7:1-26",
          "vi": "Deuteronomy 7:1-26",
          "tl": "Deuteronomy 7:1-26",
          "th": "Deuteronomy 7:1-26",
          "sw": "Deuteronomy 7:1-26",
          "it": "Deuteronomio 7:1-26"
        },
        comment: {
          "ja": "異文化と関わるとすぐ流されてしまう自分たちを自覚しているからこそ、極端なまでの距離感を強いるモーセの必死さが逆に人間臭い。",
          "en": "Moses demanding total isolation because he knows his people have zero impulse control when it comes to bad influences is honestly a mood.",
          "ko": "나쁜 유혹에 너무 쉽게 흔들리는 우리 자신의 연약함을 알기에, 아예 거리를 두라고 강조하는 모세의 마음이 왠지 짠하게 다가온다.",
          "zho": "因为深知自己抵挡不住诱惑，所以摩西干脆要求彻底隔离，这种对人性弱点的精准拿捏反倒显得格外真实。",
          "es": "Moisés exigiendo distancia total porque sabe que su pueblo se distrae con cualquier cosa es básicamente un padre tratando de evitar que sus hijos se metan en problemas.",
          "pt": "Moisés exigindo distância total porque sabe que o seu povo se deixa levar por qualquer influência é basicamente um pai tentando evitar que os filhos se metam em encrenca.",
          "vi": "Việc Môi-se yêu cầu cách biệt hoàn toàn vì biết dân mình dễ bị lung lay cho thấy ông hiểu rõ bản chất con người dễ bị ảnh hưởng bởi môi trường xung quanh đến mức nào.",
          "tl": "Ang paghingi ni Moises ng total na distansya dahil alam niyang madaling maimpluwensyahan ang mga tao ay parang isang magulang na takot na mapahamak ang mga anak.",
          "th": "การที่โมเสสสั่งให้ตัดขาดจากอิทธิพลภายนอกเพราะรู้ว่าคนของเขาใจอ่อนไหวง่ายนั้น ช่างเป็นความกังวลที่เข้าถึงอารมณ์มนุษย์สุดๆ",
          "sw": "Musa kudai watu wajitenge kabisa kwa sababu anajua jinsi watu wake wanavyoshawishika kirahisi ni kweli kabisa kulingana na udhaifu wa kibinadamu.",
          "it": "Mosè che pretende una distanza di sicurezza perché sa benissimo quanto siano distraibili i suoi è di una sincerità disarmante."
        }
    },
    "2026-05-13": {
        date: "2026-05-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "申命記 8:1-20",
          "en": "Deuteronomy 8:1-20",
          "ko": "Deuteronomy 8:1-20",
          "zho": "申命記 8:1-20",
          "es": "Deuteronomy 8:1-20",
          "pt": "Deuteronomy 8:1-20",
          "vi": "Deuteronomy 8:1-20",
          "tl": "Deuteronomy 8:1-20",
          "th": "Deuteronomy 8:1-20",
          "sw": "Deuteronomy 8:1-20",
          "it": "Deuteronomio 8:1-20"
        },
        comment: {
          "ja": "成功して調子に乗るたびに「お前がやったんじゃないぞ」と釘を刺される申命記8章、現代の自分にも毎日突きつけたいリマインダーだ。",
          "en": "Deuteronomy 8 is basically the universe reminding us not to get an ego boost just because things are finally going well for once.",
          "ko": "모든 게 잘 풀릴 때마다 '네가 잘나서 된 게 아니다'라고 팩트 폭격을 날리는 신명기 8장은 우리 모두에게 필요한 정신 차리기용 메시지다.",
          "zho": "每当生活顺遂就开始飘飘然时，申命记第8章就像是一记响亮的耳光，提醒我们别把运气当成自己的本事。",
          "es": "Deuteronomio 8 es básicamente el recordatorio constante de que no debemos creernos los mejores solo porque las cosas nos salieron bien por una vez.",
          "pt": "Deuteronômio 8 é basicamente o lembrete constante de que não devemos nos achar os melhores só porque as coisas deram certo por uma vez.",
          "vi": "Phục-truyền 8 giống như một lời nhắc nhở thực tế rằng đừng quá tự mãn khi mọi việc đang suôn sẻ, vì thành công không chỉ đến từ nỗ lực của riêng ta.",
          "tl": "Ang Deuteronomio 8 ay parang paalala na huwag lumaki ang ulo tuwing nagtatagumpay tayo, dahil hindi lang tayo ang gumawa ng paraan para mangyari iyon.",
          "th": "เฉลยธรรมบัญญัติบทที่ 8 เหมือนเป็นเสียงเตือนสติไม่ให้เราหลงระเริงกับความสำเร็จจนลืมไปว่ามีอะไรช่วยเราอยู่เบื้องหลังบ้าง",
          "sw": "Kumbukumbu la Torati 8 ni kama ukumbusho wa kila siku kwetu tusijivune tunapofanikiwa, tukasahau kuwa mafanikio yetu yanategemea msaada wa Mungu.",
          "it": "Deuteronomio 8 è quel promemoria quotidiano perfetto per ricordarci di non montarci la testa quando le cose vanno troppo bene."
        }
    },
    "2026-05-14": {
        date: "2026-05-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "申命記 15:1-23",
          "en": "Deuteronomy 15:1-23",
          "ko": "Deuteronomy 15:1-23",
          "zho": "申命記 15:1-23",
          "es": "Deuteronomy 15:1-23",
          "pt": "Deuteronomy 15:1-23",
          "vi": "Deuteronomy 15:1-23",
          "tl": "Deuteronomy 15:1-23",
          "th": "Deuteronomy 15:1-23",
          "sw": "Deuteronomy 15:1-23",
          "it": "Deuteronomio 15:1-23"
        },
        comment: {
          "ja": "7年ごとに借金を帳消しにするという制度、現代の住宅ローン返済中の身としては神の慈悲そのものにしか見えない。",
          "en": "The idea of a debt jubilee every seven years is the kind of divine mercy my bank account is currently praying for.",
          "ko": "7년마다 빚을 탕감해 주는 제도는 현재 대출 상환에 시달리는 현대인들에게는 그야말로 신의 자비 그 자체다.",
          "zho": "每七年就免除債務的規定，對於正在背房貸的現代人來說，簡直是神給的最溫柔的救贖。",
          "es": "La idea de perdonar las deudas cada siete años es la clase de misericordia divina que mi cuenta bancaria necesita desesperadamente.",
          "pt": "A ideia de perdoar dívidas a cada sete anos é o tipo de misericórdia divina que minha conta bancária precisa desesperadamente.",
          "vi": "Việc xóa nợ mỗi bảy năm là kiểu lòng thương xót mà bất cứ ai đang gánh nợ cũng phải ước ao.",
          "tl": "Ang ideya ng pagpapatawad ng utang tuwing pitong taon ay ang uri ng awa na talagang kailangan ng pitaka ko ngayon.",
          "th": "กฎการล้างหนี้ทุกเจ็ดปีนี่คือความเมตตาที่มนุษย์เงินผ่อนทุกคนใฝ่ฝันอยากให้มีจริงในยุคนี้",
          "sw": "Wazo la kusamehe madeni kila baada ya miaka saba ni aina ya huruma ya kimungu ambayo akaunti yangu ya benki inaihitaji sana.",
          "it": "Cancellare tutti i debiti ogni sette anni è esattamente quel tipo di clemenza divina che farebbe comodo a chiunque oggi."
        }
    },
    "2026-05-15": {
        date: "2026-05-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "申命記 18:1-22",
          "en": "Deuteronomy 18:1-22",
          "ko": "Deuteronomy 18:1-22",
          "zho": "申命記 18:1-22",
          "es": "Deuteronomy 18:1-22",
          "pt": "Deuteronomy 18:1-22",
          "vi": "Deuteronomy 18:1-22",
          "tl": "Deuteronomy 18:1-22",
          "th": "Deuteronomy 18:1-22",
          "sw": "Deuteronomy 18:1-22",
          "it": "Deuteronomio 18:1-22"
        },
        comment: {
          "ja": "神様が直々に語りかける預言者がいる一方で、結局は聞き手の好みのメッセージしか信じない人間の性もまた変わらない。",
          "en": "Even with prophets speaking for God, humanity still has a knack for only listening to the advice that confirms what they already wanted to do.",
          "ko": "하나님이 직접 예언자를 통해 말씀하셔도 결국 자기 입맛에 맞는 말만 골라 듣는 인간의 고질병은 예나 지금이나 똑같다.",
          "zho": "即使有先知親自傳達神的話語，人類依然習慣只聽自己想聽的，這種人性弱點真是古今皆同。",
          "es": "Aun con profetas hablando de parte de Dios, los humanos seguimos teniendo el talento de escuchar solo el consejo que confirma lo que ya queríamos hacer.",
          "pt": "Mesmo com profetas falando da parte de Deus, os humanos continuam tendo o talento de ouvir apenas o conselho que confirma o que já queriam fazer.",
          "vi": "Dù có tiên tri nói lời của Chúa, con người vẫn giữ thói quen chỉ nghe những gì mình muốn nghe mà thôi.",
          "tl": "Kahit may mga propeta na nagsasalita para sa Diyos, likas pa rin sa tao na piliin lang ang payong sang-ayon sa gusto na nilang gawin.",
          "th": "ถึงจะมีผู้เผยพระวจนะมาบอกกล่าว แต่คนเราก็ยังไม่วายเลือกฟังเฉพาะสิ่งที่ตัวเองอยากได้ยินอยู่ดี",
          "sw": "Hata manabii wanapozungumza kwa niaba ya Mungu, wanadamu bado wana tabia ya kusikiliza tu ushauri unaothibitisha kile walichotaka kufanya.",
          "it": "Anche con i profeti a disposizione, gli umani riescono sempre ad ascoltare solo quello che fa comodo alle proprie convinzioni."
        }
    },
    "2026-05-16": {
        date: "2026-05-16",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "申命記 29:1-29",
          "en": "Deuteronomy 29:1-29",
          "ko": "Deuteronomy 29:1-29",
          "zho": "申命記 29:1-29",
          "es": "Deuteronomy 29:1-29",
          "pt": "Deuteronomy 29:1-29",
          "vi": "Deuteronomy 29:1-29",
          "tl": "Deuteronomy 29:1-29",
          "th": "Deuteronomy 29:1-29",
          "sw": "Deuteronomy 29:1-29",
          "it": "Deuteronomio 29:1-29"
        },
        comment: {
          "ja": "散々奇跡を見せられたのにすぐ忘れて偶像に走るイスラエルの民、記憶力のなさと気の多さが現代人のスマホ依存と重なって笑えない。",
          "en": "The Israelites witnessing miracles and immediately chasing idols is basically the ancient version of doomscrolling through distractions.",
          "ko": "기적을 보고도 금방 잊고 우상을 찾는 이스라엘 백성의 모습은 마치 스마트폰만 보느라 눈앞의 현실을 놓치는 우리와 너무 닮았다.",
          "zho": "以色列人剛看完神蹟就轉身去拜偶像，這種健忘程度簡直跟現代人放下手機就忘記初衷一模一樣。",
          "es": "Los israelitas olvidando los milagros apenas ocurre algo nuevo es el reflejo perfecto de nuestra falta de atención moderna.",
          "pt": "Os israelitas esquecendo os milagres logo após vê-los é o retrato fiel da nossa distração constante nos dias de hoje.",
          "vi": "Người Israel quên sạch các phép lạ ngay khi gặp khó khăn, thật giống với cái cách chúng ta dễ dàng mất tập trung trong cuộc sống hiện đại.",
          "tl": "Ang mga Israelita na nakakita na ng himala pero naghahanap pa rin ng ibang sasambahin ay eksaktong repleksyon ng pagiging makalimutin nating mga tao.",
          "th": "ชาวอิสราเอลที่เห็นปาฏิหาริย์แล้วยังหันไปหาเทวรูปเหมือนคนสมัยนี้ที่สมาธิสั้นจนลืมสิ่งที่สำคัญที่สุดในชีวิตไปเลย",
          "sw": "Waisraeli kusahau miujiza mara tu wanapoona changamoto ni kama sisi tunavyosahau mambo ya msingi kwa sababu ya usumbufu wa kila siku.",
          "it": "Dimenticarsi dei miracoli visti il giorno prima per inseguire distrazioni è un vizio vecchio quanto il mondo."
        }
    },
    "2026-05-17": {
        date: "2026-05-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "申命記 30:1-20",
          "en": "Deuteronomy 30:1-20",
          "ko": "Deuteronomy 30:1-20",
          "zho": "申命記 30:1-20",
          "es": "Deuteronomy 30:1-20",
          "pt": "Deuteronomy 30:1-20",
          "vi": "Deuteronomy 30:1-20",
          "tl": "Deuteronomy 30:1-20",
          "th": "Deuteronomy 30:1-20",
          "sw": "Deuteronomy 30:1-20",
          "it": "Deuteronomio 30:1-20"
        },
        comment: {
          "ja": "命か死か、祝福か呪いかを選べと言われて結局いつも迷走する人間、選択肢がシンプルすぎて逆に選びにくいという矛盾。",
          "en": "Being told to choose between life and death sounds simple, yet we still manage to overcomplicate the decision every single time.",
          "ko": "삶과 죽음, 축복과 저주 중에서 고르라는 간단한 선택지 앞에서도 굳이 복잡하게 꼬아 생각하는 게 딱 인간의 본성이다.",
          "zho": "神說要在生命與死亡之間做選擇，結果我們人類最擅長的就是把簡單的事複雜化，搞得自己無所適從。",
          "es": "Elegir entre la vida y la muerte suena sencillo, pero los humanos siempre encontramos la forma de complicar las decisiones más claras.",
          "pt": "Escolher entre a vida e a morte parece simples, mas nós humanos sempre damos um jeito de complicar o que deveria ser óbvio.",
          "vi": "Được bảo hãy chọn giữa sự sống và cái chết, vậy mà con người lúc nào cũng giỏi biến những lựa chọn đơn giản thành rắc rối cho chính mình.",
          "tl": "Pinapipili tayo sa pagitan ng buhay at kamatayan, pero dahil tayo ay tao, lagi nating ginagawang mas komplikado ang dapat sana ay simpleng desisyon.",
          "th": "พระเจ้าบอกให้เลือกชีวิตหรือความตาย แต่คนเรากลับเก่งเหลือเกินในการทำให้เรื่องง่ายๆ กลายเป็นเรื่องซับซ้อนจนตัดสินใจไม่ได้",
          "sw": "Kuambiwa uchague kati ya uzima na mauti inaonekana rahisi, lakini sisi wanadamu tuna kipaji cha kufanya maamuzi rahisi kuwa magumu.",
          "it": "Tra vita e morte la scelta dovrebbe essere ovvia, eppure riusciamo sempre a complicarci la vita da soli."
        }
    },
    "2026-05-18": {
        date: "2026-05-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨシュア記 1:1-18",
          "en": "Joshua 1:1-18",
          "ko": "Joshua 1:1-18",
          "zho": "ヨシュア記 1:1-18",
          "es": "Joshua 1:1-18",
          "pt": "Joshua 1:1-18",
          "vi": "Joshua 1:1-18",
          "tl": "Joshua 1:1-18",
          "th": "Joshua 1:1-18",
          "sw": "Joshua 1:1-18",
          "it": "Giosuè 1:1-18"
        },
        comment: {
          "ja": "偉大なリーダーの跡を継ぐプレッシャーで押しつぶされそうなヨシュアに、神様が最初にしたアドバイスがとにかく強くあれという精神論なのは、現代の管理職の悩みと変わらない。",
          "en": "God telling Joshua to just be strong and courageous right after he takes over a legendary role is the kind of high-stakes pep talk that would make any manager sweat.",
          "ko": "전설적인 지도자의 뒤를 이어 엄청난 부담감을 느꼈을 여호수아에게 하나님이 가장 먼저 건네신 조언이 담대하라는 격려인 것을 보면, 예나 지금이나 리더의 고충은 비슷한 것 같다.",
          "zho": "約書亞剛接下重擔就接到要剛強壯膽的指令，這種臨危受命的壓力，簡直就是職場新鮮人被塞進主管位置的真實寫照。",
          "es": "Que lo primero que Dios le diga a Josué tras heredar un puesto tan difícil sea que tenga valor, es el tipo de empujón que todos necesitamos cuando el síndrome del impostor ataca.",
          "pt": "Deus dizer a Josué para ser forte e corajoso logo após ele assumir um cargo tão difícil é o tipo de incentivo que qualquer um precisaria ao encarar um novo desafio.",
          "vi": "Lời dặn dò phải can đảm ngay khi vừa kế nhiệm vị trí lãnh đạo vĩ đại của Joshua cho thấy ngay cả những người vĩ đại nhất cũng cần một cú hích tinh thần trước áp lực.",
          "tl": "Ang pagpapakatatag ni Josue sa kabila ng bigat ng responsibilidad na iniwan sa kanya ay isang paalala na ang pagiging lider ay laging may kasamang kaba at kailangang lakas ng loob.",
          "th": "การที่พระเจ้าบอกให้โยชูวากล้าหาญทันทีที่เขารับตำแหน่งต่อจากผู้นำผู้ยิ่งใหญ่ เป็นบททดสอบความกดดันที่คนรับงานต่อจากมือโปรทุกคนต้องเข้าใจดี",
          "sw": "Mungu kumwambia Yoshua awe jasiri mara tu baada ya kuchukua nafasi ya kiongozi mkuu ni aina ya motisha ambayo kila mtu anahitaji anapokabili changamoto mpya kazini.",
          "it": "Che Dio dica a Giosuè di farsi coraggio subito dopo aver ereditato un ruolo impossibile è quel genere di spinta che farebbe sudare freddo qualsiasi manager moderno."
        }
    },
    "2026-05-19": {
        date: "2026-05-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨシュア記 3:1-17",
          "en": "Joshua 3:1-17",
          "ko": "Joshua 3:1-17",
          "zho": "ヨシュア記 3:1-17",
          "es": "Joshua 3:1-17",
          "pt": "Joshua 3:1-17",
          "vi": "Joshua 3:1-17",
          "tl": "Joshua 3:1-17",
          "th": "Joshua 3:1-17",
          "sw": "Joshua 3:1-17",
          "it": "Giosuè 3:1-17"
        },
        comment: {
          "ja": "川の真ん中で立ち止まって水が引くのを待つなんて、現実的に考えれば正気の沙汰ではないけれど、神様のタイミングを信じるしかなかった彼らの緊張感は想像を絶する。",
          "en": "Standing in the middle of a river waiting for the water to stop is the ultimate test of faith that definitely makes waiting for a slow internet connection feel like nothing.",
          "ko": "강 한복판에 서서 물이 갈라지기를 기다리는 그 긴장감은, 현대인이 겪는 그 어떤 마감 기한의 압박보다 훨씬 더 아찔했을 것이다.",
          "zho": "站在河中央等水退去，這種心跳加速的等待，絕對比現代人等系統加載還要考驗耐心與信心。",
          "es": "Quedarse plantado en medio del río esperando a que el agua se detenga es el nivel máximo de paciencia y fe, mucho más estresante que cualquier espera en una sala de urgencias.",
          "pt": "Ficar parado no meio do rio esperando a água parar é o teste supremo de paciência, algo que faz qualquer espera moderna parecer um passeio no parque.",
          "vi": "Đứng giữa dòng sông chờ nước rút là một thử thách lòng kiên nhẫn tột độ, khiến cho việc chờ đợi bất cứ điều gì trong cuộc sống hiện đại cũng trở nên nhẹ nhàng hơn hẳn.",
          "tl": "Ang pagtayo sa gitna ng ilog habang hinihintay na huminto ang tubig ay isang antas ng pagtitiwala na hindi kayang tapatan ng kahit anong modernong paghihintay.",
          "th": "การยืนรออยู่กลางแม่น้ำจนกว่าน้ำจะหยุดไหลคือบททดสอบความอดทนขั้นสุด ที่ทำให้การรอคิวในชีวิตประจำวันดูเป็นเรื่องเล็กไปเลย",
          "sw": "Kusimama katikati ya mto ukisubiri maji yaache kutiririka ni jaribu kuu la imani ambalo hufanya kusubiri chochote katika maisha ya sasa kuonekana kuwa rahisi.",
          "it": "Rimanere piantati in mezzo a un fiume ad aspettare che l'acqua si fermi è il test di pazienza supremo, molto più stressante di qualsiasi attesa con la connessione lenta."
        }
    },
    "2026-05-20": {
        date: "2026-05-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨシュア記 4:15-24",
          "en": "Joshua 4:15-24",
          "ko": "Joshua 4:15-24",
          "zho": "ヨシュア記 4:15-24",
          "es": "Joshua 4:15-24",
          "pt": "Joshua 4:15-24",
          "vi": "Joshua 4:15-24",
          "tl": "Joshua 4:15-24",
          "th": "Joshua 4:15-24",
          "sw": "Joshua 4:15-24",
          "it": "Giosuè 4:15-24"
        },
        comment: {
          "ja": "川の真ん中に石を積んでおかないとすぐ忘れる自分の記憶力、ヨシュアの時代から人類共通の悩みだったようだ。",
          "en": "If we don't pile up stones in the middle of the river, we'll forget everything by next week, just like the Israelites.",
          "ko": "강 한가운데 돌을 쌓아두지 않으면 금방 까먹는 건 예나 지금이나 인간의 공통적인 건망증인가 보다.",
          "zho": "如果不在河中間堆石頭就會馬上忘記，這種健忘症看來是從約書亞時代就傳下來的通病。",
          "es": "Si no dejamos piedras marcadas en el camino, olvidamos todo a la semana siguiente; al parecer, la mala memoria es un problema universal.",
          "pt": "Se não deixarmos pedras marcadas no caminho, esquecemos tudo na semana seguinte; parece que a má memória é um problema universal.",
          "vi": "Nếu không chất đá giữa sông để ghi nhớ thì chúng ta sẽ quên sạch mọi thứ, có vẻ như bệnh đãng trí là đặc điểm chung của nhân loại từ thời xưa.",
          "tl": "Kung hindi tayo magtatambak ng bato sa gitna ng ilog, siguradong makakalimutan natin ang lahat, dahil ganyan talaga tayong mga tao.",
          "th": "ถ้าไม่วางหินกองไว้กลางแม่น้ำเราก็คงลืมกันหมด ดูเหมือนว่าความขี้ลืมจะเป็นปัญหาของมนุษย์มาตั้งแต่ยุคโยชูวาแล้ว",
          "sw": "Kama hatutajenga jiwe la ukumbusho katikati ya mto, tutasahau kila kitu, inaonekana kusahau ni tatizo la kibinadamu tangu enzi za Yoshua.",
          "it": "Se non lasciamo pietre segnate lungo la strada dimentichiamo tutto alla settimana prossima, a quanto pare la pessima memoria è un problema universale."
        }
    },
    "2026-05-21": {
        date: "2026-05-21",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨシュア記 6:1-27",
          "en": "Joshua 6:1-27",
          "ko": "Joshua 6:1-27",
          "zho": "ヨシュア記 6:1-27",
          "es": "Joshua 6:1-27",
          "pt": "Joshua 6:1-27",
          "vi": "Joshua 6:1-27",
          "tl": "Joshua 6:1-27",
          "th": "Joshua 6:1-27",
          "sw": "Joshua 6:1-27",
          "it": "Giosuè 6:1-27"
        },
        comment: {
          "ja": "ただ静かに歩いて最後に叫ぶだけで壁が崩れるなんて、世の中そんなに都合よくいけば苦労しないよと当時の住民は思ったはず。",
          "en": "The people of Jericho must have thought it was a prank until the walls actually collapsed, proving that sometimes the most absurd plans are the ones that work.",
          "ko": "그냥 조용히 걷다가 소리 한 번 질렀다고 성벽이 무너지다니, 당시 주민들은 무슨 엉뚱한 장난인가 싶었을 것이다.",
          "zho": "只是繞著城走再大喊一聲城牆就倒了，當時的居民一定覺得這根本是在開什麼國際玩笑。",
          "es": "Caminar en silencio y gritar para que se caigan los muros suena a broma pesada; los habitantes de Jericó debieron quedar en shock al ver que funcionó.",
          "pt": "Caminhar em silêncio e gritar para que as muralhas caiam parece uma piada de mau gosto; os habitantes de Jericó devem ter ficado chocados ao ver que funcionou.",
          "vi": "Chỉ đi bộ rồi hét lên mà tường thành đổ sập, cư dân Jericho chắc hẳn đã nghĩ đây là một trò đùa quái đản trước khi mọi thứ trở nên nghiêm trọng.",
          "tl": "Ang paglalakad lang nang tahimik at pagsigaw para gumuho ang pader ay parang nakakatawang joke, pero gulat ang mga taga-Jericho nang gumana ito.",
          "th": "แค่เดินวนแล้วตะโกนจนกำแพงพัง ฟังดูเหมือนเรื่องตลก แต่ชาวเมืองเยริโคคงขำไม่ออกเมื่อมันเกิดขึ้นจริง",
          "sw": "Kutembea kimya kimya kisha kupiga kelele ili kuta zianguke inaonekana kama utani, lakini wakazi wa Yeriko walishtuka kuona mpango huo wa ajabu ukifanya kazi.",
          "it": "Camminare in silenzio e urlare per far crollare le mura sembra uno scherzo di cattivo gusto, i residenti di Gerico saranno rimasti scioccati nel vedere che ha funzionato."
        }
    },
    "2026-05-22": {
        date: "2026-05-22",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨシュア記 7:1-26",
          "en": "Joshua 7:1-26",
          "ko": "Joshua 7:1-26",
          "zho": "ヨシュア記 7:1-26",
          "es": "Joshua 7:1-26",
          "pt": "Joshua 7:1-26",
          "vi": "Joshua 7:1-26",
          "tl": "Joshua 7:1-26",
          "th": "Joshua 7:1-26",
          "sw": "Joshua 7:1-26",
          "it": "Giosuè 7:1-26"
        },
        comment: {
          "ja": "たった一人の隠し事のせいでチーム全体が敗北するヨシュア記の展開、組織の連帯責任の重さを突きつけられて背筋が凍る。",
          "en": "The whole team losing a battle because of one person's secret stash is a brutal reminder of how one person's choices ripple through the whole group.",
          "ko": "한 사람의 비밀 때문에 팀 전체가 패배하는 상황을 보니 조직 안에서 개인의 선택이 얼마나 큰 파장을 일으키는지 실감하게 된다.",
          "zho": "因为一个人的私藏导致全军覆没，这简直是团队协作中最让人心惊胆战的真实写照。",
          "es": "Que todo un equipo pierda una batalla por el secreto de una sola persona es un recordatorio brutal de cómo nuestras acciones afectan a los que nos rodean.",
          "pt": "Todo um exército ser derrotado por causa de um único segredo é um lembrete brutal de como nossas escolhas individuais impactam o grupo todo.",
          "vi": "Cả một đội quân thất bại chỉ vì một người giấu diếm đồ vật là lời nhắc nhở đau đớn về việc một cá nhân có thể ảnh hưởng đến tập thể như thế nào.",
          "tl": "Ang pagkatalo ng buong grupo dahil sa isang lihim na pagtatago ng gamit ay isang matinding paalala na ang maling desisyon ng isa ay damay ang lahat.",
          "th": "การที่คนทั้งกองทัพต้องพ่ายแพ้เพราะความลับของคนคนเดียว เป็นเครื่องเตือนใจที่น่ากลัวว่าการตัดสินใจของคนหนึ่งคนส่งผลกระทบต่อส่วนรวมได้มากแค่ไหน",
          "sw": "Kikosi kizima kushindwa vita kwa sababu ya siri ya mtu mmoja ni ukumbusho mkali wa jinsi maamuzi ya mtu mmoja yanavyoathiri wote.",
          "it": "Che un'intera squadra perda una battaglia per il segreto di una singola persona è un promemoria brutale di come le nostre azioni si ripercuotano su chi ci circonda."
        }
    },
    "2026-05-23": {
        date: "2026-05-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨシュア記 8:1-35",
          "en": "Joshua 8:1-35",
          "ko": "Joshua 8:1-35",
          "zho": "ヨシュア記 8:1-35",
          "es": "Joshua 8:1-35",
          "pt": "Joshua 8:1-35",
          "vi": "Joshua 8:1-35",
          "tl": "Joshua 8:1-35",
          "th": "Joshua 8:1-35",
          "sw": "Joshua 8:1-35",
          "it": "Giosuè 8:1-35"
        },
        comment: {
          "ja": "一度失敗した後のリベンジ戦で、前回の反省を活かして着実に作戦を実行するヨシュアたちの姿に、泥臭い努力の大切さを感じる。",
          "en": "Watching Joshua pivot and execute a better plan after a failed attempt is a masterclass in learning from your mistakes.",
          "ko": "실패를 딛고 다시 전략을 짜서 성공해내는 여호수아의 모습은 시행착오 끝에 결국 답을 찾아가는 우리네 인생과 똑같다.",
          "zho": "在失败后调整心态并制定更好的策略，约书亚的这次反击简直是教科书级别的从错误中学习。",
          "es": "Ver a Josué ajustando su estrategia después de un fracaso es la prueba viviente de que aprender de los errores es la mejor manera de avanzar.",
          "pt": "Ver Josué ajustar sua estratégia após um fracasso é uma verdadeira aula de como aprender com os próprios erros para seguir em frente.",
          "vi": "Nhìn cách Giô-suê điều chỉnh chiến thuật sau thất bại, ta thấy được bài học quý giá về việc rút kinh nghiệm để tiến xa hơn.",
          "tl": "Ang makita si Josue na nagbago ng istratehiya matapos ang pagkakamali ay isang magandang halimbawa ng pagkatuto mula sa mga pagkukulang.",
          "th": "การเห็นโยชูวาปรับเปลี่ยนกลยุทธ์หลังจากความล้มเหลว เป็นบทเรียนชั้นดีของการเรียนรู้จากความผิดพลาดเพื่อก้าวไปข้างหน้า",
          "sw": "Kuona Yoshua akibadilisha mkakati baada ya kushindwa ni somo bora la kujifunza kutokana na makosa ili kusonga mbele.",
          "it": "Vedere Giosuè che raddrizza il tiro e mette in campo una strategia migliore dopo un fallimento è la prova lampante che imparare dai propri errori è l'unico modo per avanzare."
        }
    },
    "2026-05-24": {
        date: "2026-05-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨシュア記 23:1-16",
          "en": "Joshua 23:1-16",
          "ko": "Joshua 23:1-16",
          "zho": "ヨシュア記 23:1-16",
          "es": "Joshua 23:1-16",
          "pt": "Joshua 23:1-16",
          "vi": "Joshua 23:1-16",
          "tl": "Joshua 23:1-16",
          "th": "Joshua 23:1-16",
          "sw": "Joshua 23:1-16",
          "it": "Giosuè 23:1-16"
        },
        comment: {
          "ja": "散々助けてもらったのにすぐ忘れて別の神様に浮気するイスラエル、人間という生き物の記憶力の短さと移ろいやすさを体現しすぎている。",
          "en": "Israel forgetting all the help they received and drifting to other gods so quickly is a masterclass in human short-term memory.",
          "ko": "도움을 받고도 금세 잊어버리고 다른 신에게 한눈파는 이스라엘 백성을 보니 인간의 변덕스러운 기억력이란 정말 끝이 없구나 싶다.",
          "zho": "以色列人明明受過恩惠卻轉頭就忘，轉而去追隨其他神明，這簡直是人類健忘與善變的極致寫照。",
          "es": "Ver a Israel olvidarse tan rápido de toda la ayuda recibida y distraerse con otros dioses es el reflejo perfecto de nuestra memoria selectiva.",
          "pt": "Ver Israel esquecer tão rápido toda a ajuda recebida e se distrair com outros deuses é o reflexo perfeito da nossa memória seletiva.",
          "vi": "Việc dân Y-sơ-ra-ên nhanh chóng quên đi mọi sự giúp đỡ và chạy theo thần khác chính là minh chứng rõ nhất cho sự mau quên và dễ thay lòng của con người.",
          "tl": "Ang mabilis na paglimot ng Israel sa lahat ng tulong na natanggap nila at pagbaling sa ibang diyos ay patunay lamang kung gaano kabilis makalimot ang tao.",
          "th": "การที่อิสราเอลลืมความช่วยเหลือทั้งหมดที่ได้รับและหันไปหาพระอื่นอย่างรวดเร็ว สะท้อนให้เห็นความขี้ลืมและความไม่มั่นคงของมนุษย์ได้อย่างชัดเจน",
          "sw": "Waisraeli kusahau haraka sana msaada wote waliopokea na kugeukia miungu mingine ni uthibitisho kamili wa jinsi kumbukumbu za binadamu zilivyo fupi na tete.",
          "it": "Vedere Israele dimenticarsi così in fretta di tutto l'aiuto ricevuto per distrurarsi altrove è il riflesso perfetto della nostra memoria a breve termine."
        }
    },
    "2026-05-25": {
        date: "2026-05-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "士師記 2:1-23",
          "en": "Judges 2:1-23",
          "ko": "Judges 2:1-23",
          "zho": "士師記 2:1-23",
          "es": "Judges 2:1-23",
          "pt": "Judges 2:1-23",
          "vi": "Judges 2:1-23",
          "tl": "Judges 2:1-23",
          "th": "Judges 2:1-23",
          "sw": "Judges 2:1-23",
          "it": "Giudici 2:1-23"
        },
        comment: {
          "ja": "世代交代した途端に先人の苦労を忘れて好き勝手やり始める若者たちの姿、いつの時代も変わらない組織のリアルなあるあるすぎる。",
          "en": "The younger generation completely ignoring the struggles of their predecessors as soon as they take over is the most timeless office-politics trope in history.",
          "ko": "세대교체가 되자마자 선조들의 고생은 까맣게 잊고 자기 마음대로 행동하는 후대를 보니 어느 시대나 조직의 흐름은 참 비슷하다.",
          "zho": "下一代一接手就忘記前人的辛勞，開始隨心所欲地胡搞，這種組織更迭的戲碼在任何時代都顯得無比真實。",
          "es": "Que la nueva generación se olvide de los sacrificios de sus antecesores en cuanto toma el mando es el tropo más antiguo y real de cualquier organización.",
          "pt": "A nova geração esquecer os sacrifícios dos antecessores assim que assume o controle é o clichê mais antigo e real de qualquer organização.",
          "vi": "Thế hệ sau vừa tiếp quản đã quên sạch những hy sinh của người đi trước để làm theo ý mình, quả là thực tế phũ phàng của mọi tổ chức qua mọi thời đại.",
          "tl": "Ang bagong henerasyon na nakakalimot sa sakripisyo ng mga nauna sa kanila sa sandaling humawak sila ng kapangyarihan ay isang klasikong kwento sa kahit anong organisasyon.",
          "th": "การที่คนรุ่นหลังลืมความเหนื่อยยากของคนรุ่นก่อนทันทีที่ได้รับอำนาจ เป็นเรื่องจริงที่พบเห็นได้ทั่วไปในทุกองค์กรไม่ว่ายุคสมัยไหน",
          "sw": "Kizazi kipya kusahau mara moja taabu za wazee wao punde tu wanaposhika madaraka ni jambo la kawaida sana katika kila shirika tangu zamani.",
          "it": "Che la nuova generazione ignori i sacrifici di chi l'ha preceduta non appena prende il comando è il cliché più antico e reale della storia di qualsiasi ufficio."
        }
    },
    "2026-05-26": {
        date: "2026-05-26",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "士師記 4:1-24",
          "en": "Judges 4:1-24",
          "ko": "Judges 4:1-24",
          "zho": "士師記 4:1-24",
          "es": "Judges 4:1-24",
          "pt": "Judges 4:1-24",
          "vi": "Judges 4:1-24",
          "tl": "Judges 4:1-24",
          "th": "Judges 4:1-24",
          "sw": "Judges 4:1-24",
          "it": "Giudici 4:1-24"
        },
        comment: {
          "ja": "戦いの最前線に行きたがらない将軍に代わって、ヤエルがテントの釘一本で伝説を作ってしまうあたり、結局現場で一番強いのは覚悟を決めた人だという現実。",
          "en": "When a general refuses to lead the charge, Jael ends up making history with a single tent peg, proving that the real power lies with whoever actually shows up to do the job.",
          "ko": "전장에 나가기 싫어하는 장군 대신 텐트 말뚝 하나로 전설을 쓴 야엘을 보면, 결국 현장에서 끝장을 보는 사람이 진짜 강자라는 생각이 든다.",
          "zho": "當將軍不願親自上陣時，雅億竟用一根帳棚橛子寫下傳奇，這說明了真正強大的永遠是那些敢於付諸行動的人。",
          "es": "Cuando un general se niega a entrar en batalla, Jael hace historia con una sola estaca, demostrando que la verdadera fuerza está en quien se atreve a actuar cuando hace falta.",
          "pt": "Quando um general se recusa a ir para o campo de batalha, Jael faz história com uma simples estaca, provando que a verdadeira força está em quem realmente põe a mão na massa.",
          "vi": "Khi vị tướng không muốn ra trận, Jael đã tạo nên lịch sử chỉ với một chiếc cọc lều, minh chứng rằng người làm được việc cuối cùng lại chính là người dám hành động.",
          "tl": "Noong ayaw lumaban ng heneral, isang tulos lang ng tolda ang ginamit ni Jael para gumawa ng kasaysayan, na nagpapatunay na ang tunay na lakas ay nasa taong hindi natatakot kumilos.",
          "th": "ในขณะที่แม่ทัพไม่ยอมออกรบ ยาเอลกลับสร้างตำนานด้วยหมุดเต็นท์เพียงตัวเดียว ซึ่งพิสูจน์ให้เห็นว่าคนที่ลงมือทำจริงนั่นแหละคือผู้ที่แข็งแกร่งที่สุด",
          "sw": "Wakati jemadari anapoogopa kwenda vitani, Jael anatumia kigingi kimoja tu cha hema kuweka historia, ikithibitisha kuwa nguvu halisi iko kwa yule anayethubutu kuchukua hatua.",
          "it": "Quando un generale rifiuta di andare al fronte, Giaele fa la storia con un solo picchetto da tenda, dimostrando che chi ha davvero polso risolve la situazione."
        }
    },
    "2026-05-27": {
        date: "2026-05-27",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "士師記 6:1-24",
          "en": "Judges 6:1-24",
          "ko": "Judges 6:1-24",
          "zho": "士師記 6:1-24",
          "es": "Judges 6:1-24",
          "pt": "Judges 6:1-24",
          "vi": "Judges 6:1-24",
          "tl": "Judges 6:1-24",
          "th": "Judges 6:1-24",
          "sw": "Judges 6:1-24",
          "it": "Giudici 6:1-24"
        },
        comment: {
          "ja": "隠れて麦を打っているときに神様から「勇士よ」と声をかけられたギデオン、今の自分の姿と神様からの期待値のギャップに一番戸惑っているのは本人だろう。",
          "en": "Gideon hiding while threshing wheat only to be called a mighty warrior by God is the ultimate example of feeling like an imposter when you are just trying to survive the day.",
          "ko": "숨어서 밀을 타작하던 기드온에게 대뜸 용사라고 부르시는 하나님을 보며, 자신의 초라한 현실과 하나님의 기대치 사이에서 가장 당황했을 기드온의 마음이 느껴진다.",
          "zho": "基甸躲起來打麥子時被神稱為大能的勇士，這種現實感與神對他期望之間的巨大落差，大概連他自己都覺得荒謬。",
          "es": "Que Dios llame guerrero valiente a Gedeón mientras este se esconde para trillar trigo es el ejemplo perfecto de sentirse un impostor cuando solo intentas sobrevivir al día.",
          "pt": "Gideão escondido debulhando trigo sendo chamado de valente guerreiro por Deus é a prova definitiva de que nos sentimos impostores quando Ele vê um potencial que nem imaginamos.",
          "vi": "Khi Gideôn đang trốn để đập lúa mà được Chúa gọi là dũng sĩ, sự tương phản giữa thực tại nhỏ bé của ông và kỳ vọng của Chúa hẳn là điều khiến ông hoang mang nhất.",
          "tl": "Nang tawaging matapang na mandirigma si Gideon habang nagtatago sa paggiik ng trigo, ramdam ko ang pagkalito niya sa pagitan ng kanyang abang kalagayan at ng tiwala ng Diyos.",
          "th": "การที่พระเจ้าเรียกกิเดโอนว่านักรบผู้กล้าหาญในขณะที่เขากำลังแอบนวดข้าว แสดงให้เห็นถึงความแตกต่างระหว่างความเป็นจริงที่เขารู้สึกกับสิ่งที่พระเจ้ามองเห็นในตัวเขา",
          "sw": "Gideoni akiwa anajificha akipura ngano na kuitwa shujaa na Mungu ni kielelezo tosha cha jinsi tunavyohisi kama watu wa kawaida sana wakati Mungu anapoona uwezo mkubwa ndani yetu.",
          "it": "Che Dio chiami forte guerriero Gedeone mentre si nasconde per battere il grano è il ritratto perfetto di chi si sente un impostor pur cercando solo di sopravvivere alla giornata."
        }
    },
    "2026-05-28": {
        date: "2026-05-28",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "士師記 7:1-25",
          "en": "Judges 7:1-25",
          "ko": "Judges 7:1-25",
          "zho": "士師記 7:1-25",
          "es": "Judges 7:1-25",
          "pt": "Judges 7:1-25",
          "vi": "Judges 7:1-25",
          "tl": "Judges 7:1-25",
          "th": "Judges 7:1-25",
          "sw": "Judges 7:1-25",
          "it": "Giudici 7:1-25"
        },
        comment: {
          "ja": "3万2千人から300人にまで減らされるギデオンの軍隊、神様の戦力調整は時として経営コンサルタントより容赦がない。",
          "en": "God trimming Gideon's army from 32,000 to 300 is a reminder that divine math has zero interest in corporate efficiency standards.",
          "ko": "기드온의 군사를 3만 2천 명에서 300명으로 줄이시는 하나님의 전략은 효율성 따위는 안중에도 없는 파격 그 자체다.",
          "zho": "神將基甸的三萬二千大軍縮減至三百人，這波操作簡直比任何管理顧問都要狠心。",
          "es": "Que Dios reduzca un ejército de 32,000 a solo 300 hombres nos recuerda que los planes divinos no siguen ninguna lógica de recursos humanos.",
          "pt": "Deus reduzir o exército de Gideão de 32 mil para 300 homens mostra que a estratégia divina não se importa nem um pouco com as métricas de sucesso deste mundo.",
          "vi": "Việc Chúa cắt giảm đội quân của Ghê-đê-ôn từ 32.000 xuống còn 300 người cho thấy chiến lược của Ngài chẳng bao giờ quan tâm đến các con số thống kê.",
          "tl": "Ang pagbabawas ng Diyos sa hukbo ni Gideon mula 32,000 tungong 300 ay patunay na hindi kailanman sumusunod sa logic ng tao ang plano ng langit.",
          "th": "การที่พระเจ้าลดกองทัพของกิเดโอนจาก 32,000 เหลือ 300 คน บอกเราว่ากลยุทธ์ของพระองค์ไม่ได้สนตรรกะเรื่องความคุ้มค่าของมนุษย์เลยสักนิด",
          "sw": "Mungu kupunguza jeshi la Gideoni kutoka 32,000 hadi 300 inatukumbusha kuwa hesabu za Mungu hazijali kanuni zozote za kijeshi za binadamu.",
          "it": "Ridurre l'esercito da trentaduemila a trecento uomini dimostra che la logica divina non segue proprio gli standard di efficienza delle risorse umane."
        }
    },
    "2026-05-29": {
        date: "2026-05-29",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "士師記 8:18-35",
          "en": "Judges 8:18-35",
          "ko": "Judges 8:18-35",
          "zho": "士師記 8:18-35",
          "es": "Judges 8:18-35",
          "pt": "Judges 8:18-35",
          "vi": "Judges 8:18-35",
          "tl": "Judges 8:18-35",
          "th": "Judges 8:18-35",
          "sw": "Judges 8:18-35",
          "it": "Giudici 8:18-35"
        },
        comment: {
          "ja": "王になりたいと言いつつ、最後は金でエフォドを作って民を迷わせるギデオン、成功した後に一番やらかしやすいタイプだ。",
          "en": "Gideon rejecting the crown only to build a golden idol that traps everyone is the classic case of someone losing the plot after hitting the jackpot.",
          "ko": "왕이 되기를 거절해놓고 결국 금으로 에봇을 만들어 백성을 미혹한 기드온을 보면, 성공한 뒤에 초심을 잃는 게 얼마나 쉬운지 알겠다.",
          "zho": "基甸拒絕了王位，最後卻鑄造了一件讓人陷入網羅的以弗得，這說明了人在成功後最容易讓自我膨脹。",
          "es": "Gedeón rechazando el trono para luego crear un ídolo que desvió a todo el pueblo es el ejemplo perfecto de cómo el éxito puede nublar el juicio de cualquiera.",
          "pt": "Gideão recusar ser rei e, logo depois, criar um ídolo que desviou o povo, é o lembrete de que o maior perigo de um herói é o seu próprio legado.",
          "vi": "Ghê-đê-ôn từ chối làm vua nhưng lại đúc một cái ê-phốt khiến cả dân sự sa ngã, một minh chứng cho thấy người ta dễ đánh mất phương hướng sau khi đạt được thành công.",
          "tl": "Ang pagtanggi ni Gideon sa trono kapalit ng paggawa ng isang idolo na nagligaw sa bayan ay isang babala na ang tagumpay ay madaling makasilaw sa kahit sino.",
          "th": "กิเดโอนปฏิเสธตำแหน่งกษัตริย์แต่กลับสร้างเอโฟดทองคำที่ทำให้คนทั้งชาติหลงทาง นี่คือบทเรียนราคาแพงของคนที่เริ่มเหลิงหลังจากประสบความสำเร็จ",
          "sw": "Gideoni kukataa kuwa mfalme kisha kutengeneza sanamu iliyopoteza watu ni ukumbusho kuwa mafanikio yanaweza kumfanya mtu ajikwae kwa urahisi sana.",
          "it": "Gedeone che rifiuta la corona ma poi costruisce un idolo d'oro per tutti è l'esempio perfetto di chi perde completamente la bussola dopo una grande vittoria."
        }
    },
    "2026-05-30": {
        date: "2026-05-30",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "士師記 14:1-20",
          "en": "Judges 14:1-20",
          "ko": "Judges 14:1-20",
          "zho": "士師記 14:1-20",
          "es": "Judges 14:1-20",
          "pt": "Judges 14:1-20",
          "vi": "Judges 14:1-20",
          "tl": "Judges 14:1-20",
          "th": "Judges 14:1-20",
          "sw": "Judges 14:1-20",
          "it": "Giudici 14:1-20"
        },
        comment: {
          "ja": "親の反対を押し切ってまで異邦人の女性に一目惚れするサムソン、恋は盲目という言葉がここまで似合う男も珍しい。",
          "en": "Samson ignoring his parents' advice just to chase a girl he saw once is the ultimate example of love being blind.",
          "ko": "부모님의 반대에도 불구하고 이방 여인에게 첫눈에 반해버린 삼손을 보면 사랑은 정말 눈을 멀게 한다는 말이 딱 맞다.",
          "zho": "參孫不顧父母反對執意要娶外邦女子，這完美詮釋了什麼叫愛情讓人盲目。",
          "es": "Sansón ignorando los consejos de sus padres solo por seguir a alguien que le gustó es la prueba viviente de que el amor es ciego.",
          "pt": "Sansão ignorando os conselhos dos pais só para seguir alguém que ele viu uma vez é a prova viva de que o amor é cego.",
          "vi": "Samson phớt lờ lời khuyên của cha mẹ chỉ để theo đuổi một cô gái mình vừa gặp là minh chứng rõ nhất cho việc tình yêu làm con người mù quáng.",
          "tl": "Ang pagbalewala ni Samson sa payo ng kanyang mga magulang para lang sa isang babaeng nagustuhan niya ay patunay na bulag talaga ang pag-ibig.",
          "th": "แซมซั่นที่ไม่ฟังคำเตือนของพ่อแม่เพราะหลงรักหญิงต่างชาติเพียงแรกเห็น คือนิยามของคำว่าความรักทำให้คนตาบอดได้ชัดเจนที่สุด",
          "sw": "Samson kupuuza ushauri wa wazazi wake kwa sababu tu ya kumpenda mwanamke aliyemwona mara moja ni uthibitisho tosha kuwa mapenzi ni upofu.",
          "it": "Sansone che ignora i consigli dei genitori solo per correre dietro a una ragazza vista una volta è la prova vivente che l'amore sa essere proprio cieco."
        }
    },
    "2026-05-31": {
        date: "2026-05-31",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "士師記 16:1-31",
          "en": "Judges 16:1-31",
          "ko": "Judges 16:1-31",
          "zho": "士師記 16:1-31",
          "es": "Judges 16:1-31",
          "pt": "Judges 16:1-31",
          "vi": "Judges 16:1-31",
          "tl": "Judges 16:1-31",
          "th": "Judges 16:1-31",
          "sw": "Judges 16:1-31",
          "it": "Giudici 16:1-31"
        },
        comment: {
          "ja": "怪力無双の英雄なのに女性の口説き文句にだけは弱すぎるサムソン、強さの定義を根本から考えさせられる。",
          "en": "Samson is the strongest man alive but folds instantly at the first sign of emotional manipulation, which really makes you question his priorities.",
          "ko": "천하장사 삼손이 여성의 말 한마디에 무너지는 걸 보면 진정한 강함이란 무엇인가 다시 생각하게 된다.",
          "zho": "身為力大無窮的英雄卻敗在女人的枕邊細語下，參孫的故事讓人不得不重新思考強大的定義。",
          "es": "Sansón es el hombre más fuerte del mundo pero cae rendido ante la manipulación emocional, lo que te hace cuestionar sus prioridades.",
          "pt": "Sansão é o homem mais forte do mundo, mas desmorona diante da manipulação emocional, o que faz a gente questionar as prioridades dele.",
          "vi": "Samson là người đàn ông mạnh nhất thế gian nhưng lại gục ngã trước những lời dụ dỗ, thật khiến người ta phải suy ngẫm về định nghĩa của sức mạnh.",
          "tl": "Si Samson ang pinakamalakas na lalaki pero natalo lang sa lambing, kaya mapapaisip ka talaga kung ano ba ang tunay na kahulugan ng lakas.",
          "th": "แซมซั่นคือชายที่แข็งแกร่งที่สุดแต่กลับพ่ายแพ้ให้กับคำหวานเพียงไม่กี่คำ ทำให้ต้องกลับมาทบทวนใหม่ว่าความแข็งแกร่งที่แท้จริงคืออะไร",
          "sw": "Samson ni mwanamume mwenye nguvu kuliko wote lakini anashindwa na ushawishi wa kihisia, jambo linalofanya mtu ajiulize kuhusu vipaumbele vyake.",
          "it": "Sansone è l'uomo più forte del mondo ma crolla subito davanti a una manipolazione emotiva, il che fa seriamente dubitare delle sue priorità."
        }
    },
    "2026-06-01": {
        date: "2026-06-01",
        scripture: {
          "ja": "聖典",
          "en": "Scriptures",
          "ko": "Scriptures",
          "zho": "聖典",
          "es": "Scriptures",
          "pt": "Scriptures",
          "vi": "Scriptures",
          "tl": "Scriptures",
          "th": "Scriptures",
          "sw": "Scriptures",
          "it": "Scritture"
        },
        chapter: {
          "ja": "聖典  Ruth 1:1-22",
          "en": " Ruth 1:1-22",
          "ko": " Ruth 1:1-22",
          "zho": "聖典  Ruth 1:1-22",
          "es": " Ruth 1:1-22",
          "pt": " Ruth 1:1-22",
          "vi": " Ruth 1:1-22",
          "tl": " Ruth 1:1-22",
          "th": " Ruth 1:1-22",
          "sw": " Ruth 1:1-22",
          "it": "Rut 1:1-22"
        },
        comment: {
          "ja": "義理の母についていくと決めたルツの決断、実家に戻れば楽だったはずなのにあえて茨の道を選ぶその強さはもはやかっこよすぎて心配になるレベル。",
          "en": "Ruth choosing to stick with her mother-in-law instead of taking the easy way back home is the kind of loyalty that makes you want to both cheer and ask if she is okay.",
          "ko": "시어머니를 따라가기로 한 룻의 결단은 편한 길을 두고 굳이 가시밭길을 택하는 것이라 멋지면서도 한편으론 걱정이 앞선다.",
          "zho": "路得决定跟随婆婆离开家乡，明明回娘家会轻松许多，却偏要选择那条荆棘之路，这份忠诚让人敬佩到想替她捏把汗。",
          "es": "La decisión de Rut de seguir a su suegra en lugar de volver a la comodidad de su hogar es una lealtad tan intensa que resulta admirable y preocupante a la vez.",
          "pt": "A decisão de Rute de seguir a sogra em vez de voltar para o conforto de casa é uma lealdade tão intensa que chega a ser admirável e preocupante ao mesmo tempo.",
          "vi": "Quyết định đi theo mẹ chồng của Ru-tơ thay vì trở về nhà đầy đủ tiện nghi là một sự trung thành khiến người ta vừa ngưỡng mộ vừa thấy lo thay cho cô ấy.",
          "tl": "Ang pagpili ni Ruth na sumama sa kanyang biyenan sa halip na bumalik sa ginhawa ng tahanan ay isang katapatan na nakakamangha pero nakakabahala rin.",
          "th": "การที่นางรูธเลือกติดตามแม่สามีไปแทนที่จะกลับไปใช้ชีวิตสบายๆ ที่บ้านเดิม เป็นความภักดีที่น่าชื่นชมจนอดห่วงไม่ได้จริงๆ",
          "sw": "Uamuzi wa Ruth kumfuata mama mkwe wake badala ya kurudi nyumbani kwa raha ni uaminifu wa ajabu ambao unafurahisha lakini pia unatia wasiwasi.",
          "it": "La decisione di Rut di seguire la suocera invece di tornarsene comoda a casa è una lealtà così estrema che fa quasi preoccupare per la sua sanità mentale."
        }
    },
    "2026-06-02": {
        date: "2026-06-02",
        scripture: {
          "ja": "聖典",
          "en": "Scriptures",
          "ko": "Scriptures",
          "zho": "聖典",
          "es": "Scriptures",
          "pt": "Scriptures",
          "vi": "Scriptures",
          "tl": "Scriptures",
          "th": "Scriptures",
          "sw": "Scriptures",
          "it": "Scritture"
        },
        chapter: {
          "ja": "聖典  Ruth 3:1-22",
          "en": " Ruth 3:1-22",
          "ko": " Ruth 3:1-22",
          "zho": "聖典  Ruth 3:1-22",
          "es": " Ruth 3:1-22",
          "pt": " Ruth 3:1-22",
          "vi": " Ruth 3:1-22",
          "tl": " Ruth 3:1-22",
          "th": " Ruth 3:1-22",
          "sw": " Ruth 3:1-22",
          "it": "Rut 3:1-22"
        },
        comment: {
          "ja": "真夜中に寝ているボアズの足元に潜り込んで結婚を迫るルツの作戦、現代なら完全にアウトだが当時の必死さが痛いほど伝わってくる奇策。",
          "en": "Ruth sneaking into the threshing floor to propose to Boaz in the middle of the night is a move that would get you arrested today, but it shows how desperate and bold she was.",
          "ko": "한밤중에 보아스의 발치에 몰래 들어가 프러포즈하는 룻의 작전은 현대라면 당장 신고감이지만, 당시 그녀가 얼마나 절박했는지 보여주는 극단적인 용기다.",
          "zho": "路得半夜潜入打谷场向波阿斯求婚，这操作放到现代绝对会被当成私闯民宅，但也足见她为了未来有多豁得出去。",
          "es": "La estrategia de Rut de colarse en la era a medianoche para proponerle matrimonio a Booz sería un escándalo hoy en día, pero demuestra el nivel de audacia desesperada de la época.",
          "pt": "A estratégia de Rute de entrar na eira à meia-noite para propor casamento a Boaz seria um escândalo hoje em dia, mas mostra o nível de audácia desesperada da época.",
          "vi": "Việc Ru-tơ lẻn vào sân đập lúa lúc nửa đêm để cầu hôn Bô-ô là một nước đi mà thời nay chắc chắn sẽ bị bắt, nhưng nó cho thấy cô ấy đã táo bạo và tuyệt vọng đến mức nào.",
          "tl": "Ang pagpasok ni Ruth sa giikan sa hatinggabi para alukin ng kasal si Boaz ay isang hakbang na ikukulong ka na sa panahon ngayon, pero nagpapakita lang ito kung gaano siya katapang at desperada.",
          "th": "แผนการของนางรูธที่แอบเข้าไปหาโบอาซกลางดึกเพื่อขอแต่งงาน ถ้าทำสมัยนี้คงโดนแจ้งความแน่ๆ แต่ก็แสดงให้เห็นว่าเธอทุ่มเทและกล้าหาญแค่ไหนในตอนนั้น",
          "sw": "Mpango wa Ruth wa kujipenyeza kwenye uwanja wa kupuria nafaka usiku wa manane ili kumchumbia Boazi ni jambo ambalo linaweza kusababisha matatizo leo, lakini linaonyesha ujasiri wake mkubwa.",
          "it": "Rut che si intrufola nell'aia a mezzanotte per fare la proposta a Booz sarebbe uno scandalo imperdibile oggi, ma dimostra una determinazione fuori dal comune."
        }
    },
    "2026-06-03": {
        date: "2026-06-03",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 1:1-28",
          "en": "1 Samuel 1:1-28",
          "ko": "1 Samuel 1:1-28",
          "zho": "サムエル記上 1:1-28",
          "es": "1 Samuel 1:1-28",
          "pt": "1 Samuel 1:1-28",
          "vi": "1 Samuel 1:1-28",
          "tl": "1 Samuel 1:1-28",
          "th": "1 Samuel 1:1-28",
          "sw": "1 Samuel 1:1-28",
          "it": "1 Samuele 1:1-28"
        },
        comment: {
          "ja": "ハンナの切実な祈りを酔っ払いと勘違いするエリ、人の心に寄り添うことの難しさを痛感させられる。",
          "en": "Eli mistaking Hannah's soul-deep prayer for drunkenness is a sobering reminder of how easily we misjudge people's hidden struggles.",
          "ko": "한나의 간절한 기도를 술주정으로 오해한 엘리를 보며, 타인의 고통을 읽어내는 일이 얼마나 어려운지 새삼 느낀다.",
          "zho": "以利把哈拿發自肺腑的祈禱當成醉話，這提醒了我們，誤解別人的隱痛往往就在一念之間。",
          "es": "Que Elí confundiera la oración sincera de Ana con una borrachera nos recuerda lo fácil que es juzgar mal el dolor ajeno.",
          "pt": "Elí confundir a oração profunda de Ana com embriaguez é um lembrete de como muitas vezes julgamos mal as dores ocultas dos outros.",
          "vi": "Việc Ê-li hiểu lầm lời cầu nguyện chân thành của An-ne thành sự say xỉn nhắc nhở ta rằng việc thấu hiểu nỗi đau của người khác khó khăn đến nhường nào.",
          "tl": "Ang pag-aakala ni Eli na lasing si Ana sa kabila ng taimtim nitong panalangin ay isang paalala kung gaano kadaling maliin ang pinagdadaanan ng iba.",
          "th": "การที่เอลีเข้าใจผิดว่าคำอธิษฐานอันลึกซึ้งของฮันนาห์คืออาการเมา เป็นเครื่องเตือนใจว่าเรามักมองข้ามความเจ็บปวดที่ซ่อนอยู่ของผู้อื่นได้ง่ายเพียงใด",
          "sw": "Eli kudhani kuwa sala ya dhati ya Hana ilikuwa ulevi inatukumbusha jinsi tunavyoweza kuhukumu vibaya maumivu ya watu wengine kwa urahisi.",
          "it": "Eli che scambia la preghiera profonda di Anna per una sbronza ci ricorda quanto sia facile giudicare male le battaglie silenziose degli altri."
        }
    },
    "2026-06-04": {
        date: "2026-06-04",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 2:12-36",
          "en": "1 Samuel 2:12-36",
          "ko": "1 Samuel 2:12-36",
          "zho": "サムエル記上 2:12-36",
          "es": "1 Samuel 2:12-36",
          "pt": "1 Samuel 2:12-36",
          "vi": "1 Samuel 2:12-36",
          "tl": "1 Samuel 2:12-36",
          "th": "1 Samuel 2:12-36",
          "sw": "1 Samuel 2:12-36",
          "it": "1 Samuele 2:12-36"
        },
        comment: {
          "ja": "神殿でやりたい放題の息子たちを注意しきれないエリを見ていると、親の甘さと責任の重さという永遠の難問にぶち当たる。",
          "en": "Watching Eli struggle to rein in his sons' antics at the temple is a timeless lesson on the painful balance between parental love and accountability.",
          "ko": "성전에서 제멋대로인 아들들을 제대로 꾸짖지 못하는 엘리를 보면, 부모의 사랑과 훈육 사이의 아슬아슬한 줄타기는 예나 지금이나 참 어렵다.",
          "zho": "看著以利無法管教在聖殿胡作非為的兒子，深刻體會到為人父母在溺愛與責任之間掙扎的艱難。",
          "es": "Ver a Elí incapaz de corregir a sus hijos en el templo nos pone frente al eterno dilema de los padres entre el cariño y la responsabilidad.",
          "pt": "Ver Elí lutando para controlar as travessuras dos filhos no templo é uma lição atemporal sobre o equilíbrio difícil entre o amor dos pais e a responsabilidade.",
          "vi": "Chứng kiến Ê-li không thể kiểm soát hành vi của các con trai tại đền thờ gợi lên bài học muôn thuở về sự cân bằng đầy đau đớn giữa tình yêu thương và sự nghiêm khắc của cha mẹ.",
          "tl": "Ang makita si Eli na hindi mapagsabihan ang kanyang mga anak sa templo ay isang paalala sa walang katapusang hamon ng pagiging magulang sa pagitan ng pagmamahal at disiplina.",
          "th": "การได้เห็นเอลีไม่สามารถจัดการพฤติกรรมแย่ๆ ของลูกชายในพระวิหารได้ ทำให้เห็นภาพความลำบากใจของพ่อแม่ในการรักษาสมดุลระหว่างความรักกับการตักเตือน",
          "sw": "Kumtazama Eli akishindwa kuwazuia wanawe wasio na heshima hekaluni ni funzo la milele kuhusu ugumu wa uwiano kati ya upendo wa wazazi na uwajibikaji.",
          "it": "Vedere Eli incapace di rimettere in riga i figli nel tempio ci sbatte in faccia il difficile equilibrio tra affetto genitoriale e responsabilità."
        }
    },
    "2026-06-05": {
        date: "2026-06-05",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 3:1-21",
          "en": "1 Samuel 3:1-21",
          "ko": "1 Samuel 3:1-21",
          "zho": "サムエル記上 3:1-21",
          "es": "1 Samuel 3:1-21",
          "pt": "1 Samuel 3:1-21",
          "vi": "1 Samuel 3:1-21",
          "tl": "1 Samuel 3:1-21",
          "th": "1 Samuel 3:1-21",
          "sw": "1 Samuel 3:1-21",
          "it": "1 Samuele 3:1-21"
        },
        comment: {
          "ja": "夜中に何度も名前を呼ばれてエリのところへ走り回るサムエル、寝起きが悪いどころの騒ぎではない理不尽な深夜の呼び出しに同情しかない。",
          "en": "Samuel getting woken up repeatedly to run errands for Eli is the ultimate lesson in why getting a good night's sleep is a privilege.",
          "ko": "밤중에 자꾸 이름을 불러서 엘리에게 달려가는 사무엘을 보니 잠결에 불려 나가는 직장인의 비애가 느껴진다.",
          "zho": "半夜一直被叫醒还要跑腿的撒母耳，简直就是现代熬夜加班族的真实写照。",
          "es": "Samuel siendo despertado en mitad de la noche para hacer recados es el retrato perfecto de una mala noche de sueño.",
          "pt": "Samuel sendo acordado no meio da noite repetidamente para atender Eli é o resumo da vida de quem não consegue ter uma noite de sono decente.",
          "vi": "Samuel bị gọi dậy giữa đêm nhiều lần để chạy việc cho Eli giống hệt cảm giác của những người bị làm phiền khi đang ngủ ngon.",
          "tl": "Si Samuel na pabalik-balik kay Eli sa gitna ng gabi ay parang sinumang ginising sa mahimbing na tulog para sa isang hindi importanteng utos.",
          "th": "ซามูเอลที่ถูกปลุกกลางดึกให้วิ่งไปหาเอลีหลายรอบช่างน่าเห็นใจเหมือนคนที่กำลังหลับสบายแล้วถูกเรียกตัวแบบไม่เกรงใจ.",
          "sw": "Samweli kuamshwa mara kwa mara katikati ya usiku ili kumtumikia Eli ni mfano tosha wa jinsi usingizi wa mtu unavyoweza kuvurugwa bila huruma.",
          "it": "Samuele svegliato nel cuore della notte per correre avanti e indietro da Eli è la rappresentazione grafica di quanto il sonno sia un lusso da custodire."
        }
    },
    "2026-06-06": {
        date: "2026-06-06",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 4:1-22",
          "en": "1 Samuel 4:1-22",
          "ko": "1 Samuel 4:1-22",
          "zho": "サムエル記上 4:1-22",
          "es": "1 Samuel 4:1-22",
          "pt": "1 Samuel 4:1-22",
          "vi": "1 Samuel 4:1-22",
          "tl": "1 Samuel 4:1-22",
          "th": "1 Samuel 4:1-22",
          "sw": "1 Samuel 4:1-22",
          "it": "1 Samuele 4:1-22"
        },
        comment: {
          "ja": "ピンチになるととりあえず神殿から契約の箱を持ち出せばなんとかなると思っているイスラエルの人々、お守り代わりに神様を振り回すその短絡的な思考はまさに人間そのもの。",
          "en": "Israelites thinking they could just drag the Ark into battle like a magical lucky charm is the ultimate example of using faith as a quick fix.",
          "ko": "위기 때마다 언약궤만 가져오면 다 해결될 거라 믿는 이스라엘 백성의 모습은 마치 부적을 맹신하는 인간의 본성을 보는 것 같다.",
          "zho": "以色列人以为把约柜搬到战场就能解决一切危机，这种把信仰当成护身符的短视行为真是人类的通病。",
          "es": "Los israelitas pensando que llevar el arca a la batalla era suficiente para ganar es el ejemplo perfecto de tratar la fe como un amuleto de la suerte.",
          "pt": "Os israelitas achando que levar a arca para a batalha resolveria tudo é o exemplo clássico de quem trata a fé como um amuleto de sorte.",
          "vi": "Người Israel nghĩ rằng chỉ cần mang hòm bia ra trận là mọi chuyện sẽ ổn, đúng là tư duy lấy đức tin làm bùa hộ mệnh điển hình.",
          "tl": "Ang pagdadala ng mga Israelita sa kaban ng tipan sa labanan na parang isang lucky charm ay patunay na minsan ginagawa nating shortcut ang pananampalataya.",
          "th": "ชาวอิสราเอลที่คิดว่าแค่ยกหีบพันธสัญญาไปสนามรบแล้วทุกอย่างจะจบ เป็นภาพสะท้อนของมนุษย์ที่ชอบมองความเชื่อเป็นเหมือนเครื่องรางของขลัง.",
          "sw": "Waisraeli kufikiri kuwa kubeba sanduku la agano vitani kutatatua matatizo yao ni kielelezo cha jinsi watu wanavyotumia imani kama hirizi ya bahati.",
          "it": "Pensare di trascinare l'arca in battaglia come un talismano portafortuna dimostra che la tendenza a usare la fede come soluzione rapida è sempre esistita."
        }
    },
    "2026-06-07": {
        date: "2026-06-07",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 5:1-12",
          "en": "1 Samuel 5:1-12",
          "ko": "1 Samuel 5:1-12",
          "zho": "サムエル記上 5:1-12",
          "es": "1 Samuel 5:1-12",
          "pt": "1 Samuel 5:1-12",
          "vi": "1 Samuel 5:1-12",
          "tl": "1 Samuel 5:1-12",
          "th": "1 Samuel 5:1-12",
          "sw": "1 Samuel 5:1-12",
          "it": "1 Samuele 5:1-12"
        },
        comment: {
          "ja": "ダゴンの神像がアークの前に倒れて首が折れた話は、神の存在を無視して自分の都合の良い場所に置こうとすると痛い目を見るという最高の反面教師。",
          "en": "The statue of Dagon ending up face-down before the ark is a classic reminder that trying to force God to fit into your own space only leads to a messy collapse.",
          "ko": "다곤 신상이 궤 앞에 엎드러진 사건은 하나님을 자기 영역에 가두려다 오히려 망신을 당하는 인간의 어리석음을 제대로 보여준다.",
          "zho": "大袞神像在約櫃前倒地折斷，這告訴我們試圖把上帝放在自己空間裡當裝飾品，最後尷尬的通常是我們自己。",
          "es": "La estatua de Dagón cayendo ante el arca es el recordatorio perfecto de que intentar acomodar a Dios a tu conveniencia solo termina en un desastre.",
          "pt": "A estátua de Dagom caindo diante da arca mostra que tentar encaixar Deus na nossa própria agenda geralmente acaba em humilhação própria.",
          "vi": "Việc bức tượng Dagon đổ sấp mặt trước hòm bia là bài học nhớ đời cho thấy việc cố gắng sắp xếp Chúa theo ý mình chỉ mang lại kết cục ê chề.",
          "tl": "Ang pagkadapa ng rebulto ni Dagon sa harap ng kaban ay patunay na kapag pinilit mong ilagay ang Diyos sa iyong sariling termino, ikaw lang din ang mapapahiya.",
          "th": "รูปเคารพดากันที่ล้มคว่ำต่อหน้าหีบพันธสัญญาเป็นบทเรียนเตือนใจว่าการพยายามจัดระเบียบพระเจ้าให้เข้ากับความต้องการส่วนตัวมักจบลงที่ความพินาศของตัวเอง",
          "sw": "Sanamu ya Dagon kuanguka mbele ya sanduku ni ukumbusho kwamba kujaribu kumlazimisha Mungu afuate mipango yako mwenyewe ni njia ya haraka ya kujiaibisha.",
          "it": "La statua di Dagone che finisce faccia a terra davanti all'arca dimostra che cercare di incastrare Dio nei propri schemi finisce sempre male."
        }
    },
    "2026-06-08": {
        date: "2026-06-08",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 8:1-22",
          "en": "1 Samuel 8:1-22",
          "ko": "1 Samuel 8:1-22",
          "zho": "サムエル記上 8:1-22",
          "es": "1 Samuel 8:1-22",
          "pt": "1 Samuel 8:1-22",
          "vi": "1 Samuel 8:1-22",
          "tl": "1 Samuel 8:1-22",
          "th": "1 Samuel 8:1-22",
          "sw": "1 Samuel 8:1-22",
          "it": "1 Samuele 8:1-22"
        },
        comment: {
          "ja": "隣の芝生が青く見えて王様を欲しがるイスラエルの民、結局は自分たちの選択の重さに後で苦しむことになるという典型的な隣人コンプレックス。",
          "en": "The people demanding a king just to be like everyone else is the ancient equivalent of buying something you don't need just to keep up with the neighbors.",
          "ko": "남들 다 왕이 있으니 우리도 달라고 떼쓰는 이스라엘 백성은, 겉모습만 따라 하다가 정작 중요한 것을 놓치는 현대인의 모습을 그대로 닮았다.",
          "zho": "以色列人吵著要立王只為了跟鄰國一樣，這簡直就像是為了跟風而買了根本不需要的奢侈品，最後才後悔當初。",
          "es": "El pueblo exigiendo un rey solo por encajar es el equivalente bíblico de comprar algo que no necesitas para impresionar a gente que no te importa.",
          "pt": "O povo exigindo um rei só para ser igual aos vizinhos é o equivalente antigo de comprar o que não precisa só para manter as aparências.",
          "vi": "Dân Israel đòi có vua chỉ để giống các nước lân cận giống hệt như việc cố chạy theo xu hướng mà không biết mình đang tự rước họa vào thân.",
          "tl": "Ang pagpilit ng mga Israelita na magkaroon ng hari para lang maging katulad ng ibang bansa ay parang pagbili ng gamit na hindi mo kailangan para lang makipagsabayan sa kapitbahay.",
          "th": "การที่คนอิสราเอลเรียกร้องขอมีกษัตริย์เพียงเพราะอยากเหมือนคนอื่น ก็เหมือนการซื้อของที่ไม่จำเป็นเพียงเพื่อรักษาหน้าตาให้เท่าเทียมกับเพื่อนบ้าน",
          "sw": "Waisraeli kudai mfalme ili wafanane na mataifa mengine ni kama kununua kitu usichokihitaji ili tu uonekane kama jirani zako.",
          "it": "Il popolo che vuole un re a tutti i costi è l'equivalente antico di comprare cose inutili solo per fare come i vicini."
        }
    },
    "2026-06-09": {
        date: "2026-06-09",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 9:1-24",
          "en": "1 Samuel 9:1-24",
          "ko": "1 Samuel 9:1-24",
          "zho": "サムエル記上 9:1-24",
          "es": "1 Samuel 9:1-24",
          "pt": "1 Samuel 9:1-24",
          "vi": "1 Samuel 9:1-24",
          "tl": "1 Samuel 9:1-24",
          "th": "1 Samuel 9:1-24",
          "sw": "1 Samuel 9:1-24",
          "it": "1 Samuele 9:1-24"
        },
        comment: {
          "ja": "迷子のロバを探しに出たはずが王様になるという展開、人生の迷走が実は正解への伏線だったという現実味のないラッキーすぎて羨ましい。",
          "en": "Going out to look for lost donkeys and ending up as king is the kind of accidental career pivot we all wish for.",
          "ko": "잃어버린 나귀 찾으러 나갔다가 왕이 되어 돌아오는 전개라니, 인생의 꼬인 실타래가 의외의 대박으로 이어지는 마법 같다.",
          "zho": "出门找驴却意外当上国王，这种人生弯路走得太精彩，让人不得不感叹命运的剧本总是这么出人意料。",
          "es": "Salir a buscar burros perdidos y terminar siendo rey es el giro de guion más envidiable que he visto en toda la historia.",
          "pt": "Sair para procurar jumentos e acabar virando rei é o tipo de reviravolta na carreira que a gente só vê uma vez na vida.",
          "vi": "Đi tìm lừa lạc mà lại được chọn làm vua, cái kết quá bất ngờ cho một chuyến đi tìm kiếm tưởng chừng như vô vọng.",
          "tl": "Ang maghanap ng nawawalang asno at mauwi sa pagiging hari ay ang uri ng plot twist na sana ay nangyayari rin sa totoong buhay.",
          "th": "ออกไปตามหาลาที่หายไปแต่กลับได้เป็นกษัตริย์ เป็นการเปลี่ยนเส้นทางชีวิตที่เหนือความคาดหมายและน่าอิจฉาที่สุด",
          "sw": "Kutoka kutafuta punda waliopotea na kuishia kuwa mfalme ni aina ya mabadiliko ya maisha ambayo kila mtu anatamani yatokee kwake.",
          "it": "Uscire a cercare asini smarriti e ritrovarsi re è la svolta di carriera più invidiabile della storia."
        }
    },
    "2026-06-10": {
        date: "2026-06-10",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 9:25-27",
          "en": "1 Samuel 9:25-27",
          "ko": "1 Samuel 9:25-27",
          "zho": "サムエル記上 9:25-27",
          "es": "1 Samuel 9:25-27",
          "pt": "1 Samuel 9:25-27",
          "vi": "1 Samuel 9:25-27",
          "tl": "1 Samuel 9:25-27",
          "th": "1 Samuel 9:25-27",
          "sw": "1 Samuel 9:25-27",
          "it": "1 Samuele 9:25-27"
        },
        comment: {
          "ja": "屋上で二人きりの密談とか、まるでドラマのワンシーンだけど、サウルにとっては緊張で朝食どころじゃなかったはず。",
          "en": "Having a private deep talk on the rooftop sounds like a romantic movie scene, but for Saul, it was probably just pure anxiety.",
          "ko": "지붕 위에서 나누는 둘만의 비밀 대화라니 영화 같은 분위기지만, 사울에게는 밥이 코로 들어가는지 입으로 들어가는지 모를 긴장의 순간이었을 듯.",
          "zho": "在房顶上进行秘密谈话听起来很浪漫，但对于当时的扫罗来说，那种压力估计连早餐都吃不下。",
          "es": "Tener una charla profunda en la azotea suena a escena de película, pero para Saúl probablemente fue un momento de puro nerviosismo.",
          "pt": "Uma conversa profunda no telhado parece cena de filme, mas para o Saúl deve ter sido puro nervosismo antes de começar o dia.",
          "vi": "Một cuộc trò chuyện riêng tư trên mái nhà nghe thật lãng mạn, nhưng với Saul, đó chắc hẳn là khoảnh khắc hồi hộp đến nghẹt thở.",
          "tl": "Ang pribadong usapan sa bubong ay parang eksena sa pelikula, pero para kay Saul, malamang na sobrang kaba ang naramdaman niya.",
          "th": "การคุยลับๆ บนดาดฟ้าฟังดูเหมือนฉากในหนัง แต่สำหรับซาอูลแล้ว มันคงเป็นช่วงเวลาที่ตึงเครียดจนกินอะไรไม่ลง",
          "sw": "Mazungumzo ya siri juu ya paa yanaonekana kama eneo la filamu, lakini kwa Sauli, labda ilikuwa wakati wa wasiwasi mwingi.",
          "it": "Fare due chiacchiere serie sul tetto sembra una scena da film romantico, ma per Saùl era solo pura ansia."
        }
    },
    "2026-06-11": {
        date: "2026-06-11",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 10:17-27",
          "en": "1 Samuel 10:17-27",
          "ko": "1 Samuel 10:17-27",
          "zho": "サムエル記上 10:17-27",
          "es": "1 Samuel 10:17-27",
          "pt": "1 Samuel 10:17-27",
          "vi": "1 Samuel 10:17-27",
          "tl": "1 Samuel 10:17-27",
          "th": "1 Samuel 10:17-27",
          "sw": "1 Samuel 10:17-27",
          "it": "1 Samuele 10:17-27"
        },
        comment: {
          "ja": "王様になりたくなくて荷物の後ろに隠れるサウル、大役を任されそうになった時のあの逃げ出したい気持ち、全人類が共感する。",
          "en": "Saul hiding behind the baggage to avoid being king is the ultimate mood for anyone dodging a major responsibility.",
          "ko": "왕이 되기 싫어서 짐 뒤에 숨어버린 사울을 보니, 큰 책임을 맡기 직전의 그 도망치고 싶은 심정이 너무나 이해된다.",
          "zho": "掃羅為了逃避當國王而躲在行李後面，這種面對重大責任時想臨陣脫逃的心情，簡直是全人類的共鳴。",
          "es": "Saúl escondiéndose detrás del equipaje para no ser rey es exactamente la reacción de cualquiera ante una responsabilidad que le queda grande.",
          "pt": "Saul se escondendo atrás da bagagem para não ser rei é a reação mais humana de quem quer fugir de uma grande responsabilidade.",
          "vi": "Việc Saul trốn sau đống hành lý để tránh làm vua giống hệt cảm giác muốn bỏ chạy khi phải nhận một trọng trách quá lớn.",
          "tl": "Si Saul na nagtago sa likod ng mga gamit para hindi maging hari ay ang pinaka-relatable na reaksyon kapag ayaw nating tanggapin ang isang malaking responsibilidad.",
          "th": "การที่ซาอูลแอบหลังสัมภาระเพื่อหลีกเลี่ยงการเป็นกษัตริย์ คือความรู้สึกที่ทุกคนน่าจะเข้าใจดีเวลาต้องรับผิดชอบอะไรที่ใหญ่เกินตัว",
          "sw": "Sauli kujificha nyuma ya mizigo ili asifanywe mfalme ni hisia ile ile tunayokuwa nayo tunapotaka kukwepa jukumu zito.",
          "it": "Saùl che si nasconde tra i bagagli per non fare il re è lo stato d'animo universale quando si evita una grande responsabilità."
        }
    },
    "2026-06-12": {
        date: "2026-06-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 13:1-23",
          "en": "1 Samuel 13:1-23",
          "ko": "1 Samuel 13:1-23",
          "zho": "サムエル記上 13:1-23",
          "es": "1 Samuel 13:1-23",
          "pt": "1 Samuel 13:1-23",
          "vi": "1 Samuel 13:1-23",
          "tl": "1 Samuel 13:1-23",
          "th": "1 Samuel 13:1-23",
          "sw": "1 Samuel 13:1-23",
          "it": "1 Samuele 13:1-23"
        },
        comment: {
          "ja": "サムエルを待てずに自分で祭壇を準備して大失敗するサウル、焦って自爆する姿は現代の空回りするリーダーそのもの。",
          "en": "Saul losing patience and doing things his own way only to ruin everything is the classic case of a leader self-sabotaging out of anxiety.",
          "ko": "사무엘을 기다리지 못하고 성급하게 제사를 드려 일을 망친 사울을 보면, 조급함 때문에 스스로 무덤을 파는 리더의 전형을 보는 것 같다.",
          "zho": "掃羅等不及撒母耳就擅自獻祭而釀成大禍，這種因為焦慮而自亂陣腳的行為，簡直是職場崩潰領導者的縮影。",
          "es": "Saúl perdiendo la paciencia y arruinándolo todo por querer tomar el control es el retrato perfecto de un líder autosaboteándose por ansiedad.",
          "pt": "Saul perdendo a paciência e estragando tudo ao tentar fazer as coisas do seu jeito é o clássico exemplo de um líder que se sabota pela ansiedade.",
          "vi": "Việc Saul mất kiên nhẫn và tự ý làm mọi việc để rồi hỏng bét là hình ảnh điển hình của một nhà lãnh đạo tự hủy hoại mình vì lo âu.",
          "tl": "Si Saul na nawalan ng pasensya at gumawa ng sariling paraan hanggang sa masira ang lahat ay tipikal na halimbawa ng lider na nag-a-autosabotage dahil sa kaba.",
          "th": "การที่ซาอูลหมดความอดทนและทำอะไรตามใจตัวเองจนพังไม่เป็นท่า คือตัวอย่างคลาสสิกของผู้นำที่ทำตัวเองพังเพราะความวิตกกังวล",
          "sw": "Sauli kukosa subira na kujaribu kufanya mambo kwa njia yake hadi kuharibu kila kitu ni mfano kamili wa kiongozi anayejiharibia mambo kwa sababu ya wasiwasi.",
          "it": "Saùl che perde la pazienza e rovina tutto da solo è il classico ritratto del leader che si autosabota per l'ansia."
        }
    },
    "2026-06-13": {
        date: "2026-06-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 15:1-35",
          "en": "1 Samuel 15:1-35",
          "ko": "1 Samuel 15:1-35",
          "zho": "サムエル記上 15:1-35",
          "es": "1 Samuel 15:1-35",
          "pt": "1 Samuel 15:1-35",
          "vi": "1 Samuel 15:1-35",
          "tl": "1 Samuel 15:1-35",
          "th": "1 Samuel 15:1-35",
          "sw": "1 Samuel 15:1-35",
          "it": "1 Samuele 15:1-35"
        },
        comment: {
          "ja": "言い訳を並べて自分の都合を押し通そうとするサウル、怒られると分かっているのに保身に走るあの時の自分を見ているようで胃が痛い。",
          "en": "Saul scrambling to make excuses the second he gets caught is just a mirror of every time I have tried to talk my way out of my own mistakes.",
          "ko": "변명을 늘어놓으며 자기 합리화를 하는 사울의 모습에서 들켰을 때 어떻게든 빠져나가려 애쓰던 내 과거를 보는 것 같아 마음이 아프다.",
          "zho": "掃羅被抓包後急著找藉口推卸責任的樣子，簡直就是每個人在犯錯時試圖掩飾尷尬的真實寫照。",
          "es": "Saúl buscando excusas en cuanto lo atrapan es el reflejo perfecto de cuando intentamos justificar nuestros propios errores ante lo inevitable.",
          "pt": "Saul tentando se justificar assim que é pego é o retrato perfeito de quando tentamos dar desculpas para nossos próprios erros.",
          "vi": "Saul cố gắng biện minh ngay khi bị bắt quả tang chính là hình ảnh phản chiếu của bất kỳ ai trong chúng ta khi cố bao biện cho sai lầm của mình.",
          "tl": "Ang pagdadahilan ni Saul nang mahuli siya ay parang tayo lang na pilit nagpapaliwanag kahit alam nating mali ang ginawa natin.",
          "th": "การที่ซาอูลพยายามหาข้ออ้างทันทีที่ถูกจับได้นั้น เป็นภาพสะท้อนของตัวเราเวลาที่พยายามแก้ต่างให้ความผิดของตัวเองอย่างชัดเจนที่สุด",
          "sw": "Sauli akitafuta visingizio punde tu anapokamatwa ni kioo cha kila wakati tunapojaribu kujitetea kwa makosa yetu wenyewe.",
          "it": "Saùl che cerca scuse appena viene beccato è lo specchio di tutte le volte in cui ho cercato di giustificare i miei errori."
        }
    },
    "2026-06-14": {
        date: "2026-06-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 16:1-23",
          "en": "1 Samuel 16:1-23",
          "ko": "1 Samuel 16:1-23",
          "zho": "サムエル記上 16:1-23",
          "es": "1 Samuel 16:1-23",
          "pt": "1 Samuel 16:1-23",
          "vi": "1 Samuel 16:1-23",
          "tl": "1 Samuel 16:1-23",
          "th": "1 Samuel 16:1-23",
          "sw": "1 Samuel 16:1-23",
          "it": "1 Samuele 16:1-23"
        },
        comment: {
          "ja": "外見だけでリーダーを決めようとする人間をよそに、神様は全く別のところを見ているという現実は、評価に一喜一憂する私たちへの強烈な皮肉だ。",
          "en": "People judging leadership by appearance while God looks at the heart is a brutal reminder that we are usually looking at the wrong things.",
          "ko": "사람들은 외모로 리더를 뽑으려 하지만 하나님은 전혀 다른 곳을 보고 계신다는 사실은, 남의 평가에 일희일비하는 우리에게 신선한 충격이다.",
          "zho": "人們總愛以貌取人，而上帝卻看重內心，這狠狠打了那些只會看外表的人一巴掌。",
          "es": "Que la gente juzgue el liderazgo por la apariencia mientras Dios mira el corazón es un recordatorio de que casi siempre nos fijamos en lo superficial.",
          "pt": "As pessoas julgarem a liderança pela aparência enquanto Deus olha para o coração é o lembrete de que quase sempre focamos no que não importa.",
          "vi": "Việc con người đánh giá năng lực qua vẻ bề ngoài trong khi Chúa nhìn thấu tâm can là lời nhắc nhở rằng chúng ta thường quá chú trọng vào những thứ nông cạn.",
          "tl": "Ang paghusga ng mga tao sa liderato base sa hitsura habang ang Diyos naman ay sa puso tumitingin ay isang paalala na madalas ay mali ang ating prayoridad.",
          "th": "การที่มนุษย์มักตัดสินผู้นำจากรูปลักษณ์ภายนอก ในขณะที่พระเจ้าทรงมองที่หัวใจ เป็นเครื่องเตือนใจว่าเรามักจะให้ความสำคัญกับสิ่งที่ผิดพลาดเสมอ",
          "sw": "Watu kuhukumu uongozi kwa sura wakati Mungu anaangalia moyo ni ukumbusho kwamba mara nyingi sisi huangalia vitu visivyofaa.",
          "it": "Che gli uomini giudichino l'apparenza mentre Dio guarda il cuore è un promemoria sul fatto che guardiamo sempre le cose sbagliate."
        }
    },
    "2026-06-15": {
        date: "2026-06-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 17:1-37",
          "en": "1 Samuel 17:1-37",
          "ko": "1 Samuel 17:1-37",
          "zho": "サムエル記上 17:1-37",
          "es": "1 Samuel 17:1-37",
          "pt": "1 Samuel 17:1-37",
          "vi": "1 Samuel 17:1-37",
          "tl": "1 Samuel 17:1-37",
          "th": "1 Samuel 17:1-37",
          "sw": "1 Samuel 17:1-37",
          "it": "1 Samuele 17:1-37"
        },
        comment: {
          "ja": "兄貴たちに「お前は何しに来たんだ」とマウントを取られても怯まないダビデのメンタル、現代の職場で一番見習いたい。",
          "en": "David keeping his cool while his older brothers try to put him in his place is the kind of professional poise we all need.",
          "ko": "형들에게 한소리 들으면서도 기죽지 않는 다윗의 멘탈은 현대 직장인들에게 꼭 필요한 덕목이다.",
          "zho": "大衛被哥哥們冷嘲熱諷卻毫不退縮的心理素質，簡直是現代職場生存指南。",
          "es": "La entereza de David ante los ataques de sus hermanos es el tipo de confianza que todos necesitamos en un mal día.",
          "pt": "A calma de Davi enquanto os irmãos tentam diminuí-lo é a autoconfiança que todo mundo queria ter em uma reunião de trabalho.",
          "vi": "Cách Đa-vít giữ vững tinh thần khi bị các anh coi thường là bài học lớn về bản lĩnh trong môi trường đầy áp lực.",
          "tl": "Ang tibay ng loob ni David kahit minamaliit siya ng kanyang mga kuya ay ang uri ng confidence na kailangan nating lahat.",
          "th": "การที่ดาวิดยังนิ่งได้แม้จะโดนพี่ชายกดดันใส่ คือความสตรองที่คนทำงานยุคนี้ต้องเรียนรู้",
          "sw": "Ujasiri wa Daudi mbele ya ndugu zake wanaomdharau ni mfano mzuri wa jinsi ya kubaki imara katikati ya shinikizo.",
          "it": "La calma di Davide mentre i fratelli maggiori cercano di metterlo in un angolo è la sicurezza che tutti vorremmo avere al lavoro."
        }
    },
    "2026-06-16": {
        date: "2026-06-16",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 17:38-58",
          "en": "1 Samuel 17:38-58",
          "ko": "1 Samuel 17:38-58",
          "zho": "サムエル記上 17:38-58",
          "es": "1 Samuel 17:38-58",
          "pt": "1 Samuel 17:38-58",
          "vi": "1 Samuel 17:38-58",
          "tl": "1 Samuel 17:38-58",
          "th": "1 Samuel 17:38-58",
          "sw": "1 Samuel 17:38-58",
          "it": "1 Samuele 17:38-58"
        },
        comment: {
          "ja": "最新鋭の重装備を断っていつもの石ころで戦うダビデ、結局は自分らしいやり方が最強という身も蓋もない真実。",
          "en": "David ditching the heavy armor for his usual stones is a great reminder that trying to be someone else rarely works.",
          "ko": "최신식 갑옷을 벗어 던지고 평소 쓰던 돌멩이를 집어 든 다윗은 남의 방식보다 내 방식이 최고라는 진리를 몸소 보여준다.",
          "zho": "大衛拒絕重裝備而選擇自己習慣的石頭，這說明了與其模仿別人，不如發揮自己的長處。",
          "es": "David descartando la armadura pesada para usar sus piedras es la prueba de que intentar ser alguien que no eres nunca termina bien.",
          "pt": "Davi rejeitando a armadura pesada para usar suas pedras habituais é um lembrete clássico de que tentar ser outra pessoa raramente funciona.",
          "vi": "Đa-vít từ chối bộ giáp cồng kềnh để chọn những viên đá quen thuộc, một bài học đắt giá về việc sống thật với chính mình.",
          "tl": "Ang pagtanggi ni David sa mabigat na armor para gamitin ang kanyang mga bato ay patunay na mas epektibo ang maging sarili mo kaysa gayahin ang iba.",
          "th": "การที่ดาวิดทิ้งชุดเกราะหนักๆ แล้วเลือกหยิบหินที่ถนัดมือ คือหลักฐานว่าการเป็นตัวเองนั้นทรงพลังที่สุด",
          "sw": "Daudi kukataa silaha nzito ili atumie mawe anayoyajua ni ukumbusho kuwa kujaribu kuwa mtu mwingine hakukupi ushindi.",
          "it": "Davide che rifiuta l'armatura pesante per usare le sue pietre ci ricorda che cercare di essere qualcun altro non funziona mai."
        }
    },
    "2026-06-17": {
        date: "2026-06-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 24:1-22",
          "en": "1 Samuel 24:1-22",
          "ko": "1 Samuel 24:1-22",
          "zho": "サムエル記上 24:1-22",
          "es": "1 Samuel 24:1-22",
          "pt": "1 Samuel 24:1-22",
          "vi": "1 Samuel 24:1-22",
          "tl": "1 Samuel 24:1-22",
          "th": "1 Samuel 24:1-22",
          "sw": "1 Samuel 24:1-22",
          "it": "1 Samuele 24:1-22"
        },
        comment: {
          "ja": "洞窟でサウルを暗殺する絶好のチャンスが訪れたのに、結局服の端を切るだけで済ませるダビデの詰めが甘いのか徳が高いのか、判断に迷うところだ。",
          "en": "David having the perfect chance to take out Saul in the cave and settling for just a piece of his robe is either a testament to his high character or his biggest missed opportunity.",
          "ko": "동굴에서 사울을 암살할 절호의 기회를 얻고도 옷자락만 자르고 마는 다윗을 보면, 이게 성인군자의 면모인지 아니면 결정적인 순간의 망설임인지 알다가도 모를 일이다.",
          "zho": "大衛在洞穴中面對殺掉掃羅的絕佳機會，卻只剪下衣襟，這種舉動究竟是高尚的情操，還是關鍵時刻的猶豫，實在讓人難以定義。",
          "es": "Que David tuviera a Saúl a su merced en la cueva y solo le cortara un trozo de tela es la prueba definitiva de que a veces la bondad es más desconcertante que la propia venganza.",
          "pt": "Davi ter a chance de ouro de acabar com Saul na caverna e optar apenas por cortar um pedaço da capa é a prova de que, às vezes, a bondade chega a ser frustrante.",
          "vi": "Việc Đa-vít có cơ hội ngàn năm có một để hạ gục Sau-lơ trong hang nhưng chỉ cắt một vạt áo cho thấy lòng nhân từ đôi khi còn khó hiểu hơn cả sự trả thù.",
          "tl": "Ang pagkakaroon ni David ng perpektong pagkakataon na patayin si Saul sa loob ng kuweba pero ang pinili lang ay gupitan ang laylayan ng damit ay patunay na minsan, ang kabutihan ay mas nakakalito kaysa sa paghihiganti.",
          "th": "การที่ดาวิดมีโอกาสทองที่จะจัดการกับซาอูลในถ้ำแต่กลับเลือกแค่ตัดชายเสื้อทิ้งไปนั้น เป็นบทพิสูจน์ว่าบางครั้งความใจดีก็ดูน่าหงุดหงิดยิ่งกว่าการแก้แค้นเสียอีก",
          "sw": "Daudi kupata nafasi nzuri ya kumuua Sauli pangoni lakini akaishia kukata tu kipande cha vazi lake ni uthibitisho kwamba wakati mwingine wema unaweza kuwa wa ajabu kuliko kisasi chenyewe.",
          "it": "Avere la vittoria in mano nella grotta e cavarsela tagliando solo un pezzo di stoffa dimostra una pazienza che rasenta l'incredibile."
        }
    },
    "2026-06-18": {
        date: "2026-06-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 25:1-44",
          "en": "1 Samuel 25:1-44",
          "ko": "1 Samuel 25:1-44",
          "zho": "サムエル記上 25:1-44",
          "es": "1 Samuel 25:1-44",
          "pt": "1 Samuel 25:1-44",
          "vi": "1 Samuel 25:1-44",
          "tl": "1 Samuel 25:1-44",
          "th": "1 Samuel 25:1-44",
          "sw": "1 Samuel 25:1-44",
          "it": "1 Samuele 25:1-44"
        },
        comment: {
          "ja": "無礼な夫ナバルに代わって、アビガイルが先回りして謝罪と根回しを完璧にこなす姿は、現代の優秀な秘書や調整役の苦労そのものだ。",
          "en": "Abigail scrambling to apologize and clean up her husband Nabal’s mess is the ultimate relatable moment for anyone who has ever had to act as a professional damage-control expert in their own home.",
          "ko": "남편 나발이 저지른 무례한 짓을 수습하느라 아비가일이 동분서주하는 모습은, 현대 사회에서 타인의 실수를 대신 챙기느라 고생하는 사람들의 애환 그 자체다.",
          "zho": "亞比該忙著為丈夫拿八的無理行徑道歉並善後，這簡直就是現代社會中那些每天都在幫人擦屁股的苦命人的寫照。",
          "es": "Abigail haciendo control de daños y disculpándose por su marido maleducado es la viva imagen de cualquier persona que vive arreglando los errores de otros en el trabajo.",
          "pt": "Abigail correndo para pedir desculpas e limpar a bagunça do marido Nabal é o retrato fiel de quem precisa ser o gestor de crises da própria família.",
          "vi": "Việc A-bi-ga-in phải vội vàng đi xin lỗi và dọn dẹp đống hỗn độn do chồng mình gây ra chính là nỗi lòng của bất kỳ ai đang phải đóng vai người hòa giải trong gia đình.",
          "tl": "Ang pagmamadali ni Abigail na mag-sorry at ayusin ang gulo na ginawa ng asawa niyang si Nabal ay tunay na makaka-relate ang sinumang naging taga-linis ng kalat ng iba sa kanilang sariling tahanan.",
          "th": "การที่อาบีกายิลต้องรีบไปขอโทษและตามแก้ปัญหาที่สามีจอมหยาบคายก่อไว้ คือภาพสะท้อนชีวิตของคนที่ต้องคอยตามเช็ดตามล้างความผิดพลาดของคนอื่นในครอบครัวได้ดีที่สุด",
          "sw": "Abigail kukimbia na kuomba radhi kwa ajili ya ujinga wa mume wake Nabali ni kielelezo kamili cha mtu yeyote ambaye amewahi kulazimika kurekebisha makosa ya wengine nyumbani kwake.",
          "it": "Abigail che rincorre tutti per rimediare ai guai del marito è l'incarnazione di chiunque debba fare da mediatore in famiglia."
        }
    },
    "2026-06-19": {
        date: "2026-06-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記上 26:1-25",
          "en": "1 Samuel 26:1-25",
          "ko": "1 Samuel 26:1-25",
          "zho": "サムエル記上 26:1-25",
          "es": "1 Samuel 26:1-25",
          "pt": "1 Samuel 26:1-25",
          "vi": "1 Samuel 26:1-25",
          "tl": "1 Samuel 26:1-25",
          "th": "1 Samuel 26:1-25",
          "sw": "1 Samuel 26:1-25",
          "it": "1 Samuele 26:1-25"
        },
        comment: {
          "ja": "寝ているサウルを殺すチャンスを前にして、結局手を出さないダビデの慎重さは、もはや恩を仇で返さないというレベルを超えた執念を感じる。",
          "en": "David not taking the shot at a sleeping Saul is less about mercy and more about having a level of self-control that most of us can only dream of.",
          "ko": "잠든 사울을 죽일 기회를 앞에 두고도 끝내 참아내는 다윗의 인내심은, 거의 부처님 수준의 자기 통제력을 보여준다.",
          "zho": "大衛面對熟睡的掃羅卻選擇不動手，這種極致的自制力簡直是把「不報復」修煉成了藝術。",
          "es": "David teniendo a Saúl dormido frente a él y decidiendo no hacer nada es el nivel definitivo de control emocional que todos quisiéramos tener en una discusión.",
          "pt": "David diante de Saul dormindo e optando por não fazer nada é o nível máximo de autocontrole que todos nós gostaríamos de ter em uma discussão.",
          "vi": "Đa-vít đứng trước một Sau-lơ đang ngủ say mà vẫn không ra tay, đó là đỉnh cao của sự kiềm chế mà người thường khó lòng làm được.",
          "tl": "Ang hindi pagpatay ni David kay Saul habang natutulog ito ay nagpapakita ng antas ng pagtitimpi na halos imposible para sa ating mga ordinaryong tao.",
          "th": "ดาวิดเห็นซาอูลนอนหลับอยู่ตรงหน้าแต่กลับไม่ทำอะไรเลย นี่คือระดับของการยับยั้งชั่งใจที่มนุษย์ทั่วไปทำได้ยากมาก",
          "sw": "Daudi kumuona Sauli akiwa amelala na kuamua kutomdhuru ni kielelezo cha uvumilivu ambacho wengi wetu tungependa kuwa nacho kwenye migogoro.",
          "it": "Lasciare dormire Saul senza fargli nulla è quel livello di autocontrollo che vorremmo tutti avere durante una discussione accesa."
        }
    },
    "2026-06-20": {
        date: "2026-06-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記下 5:1-25",
          "en": "2 Samuel 5:1-25",
          "ko": "2 Samuel 5:1-25",
          "zho": "サムエル記下 5:1-25",
          "es": "2 Samuel 5:1-25",
          "pt": "2 Samuel 5:1-25",
          "vi": "2 Samuel 5:1-25",
          "tl": "2 Samuel 5:1-25",
          "th": "2 Samuel 5:1-25",
          "sw": "2 Samuel 5:1-25",
          "it": "2 Samuele 5:1-25"
        },
        comment: {
          "ja": "ようやく王になったと思ったら、今度はペリシテ人が攻めてくるという、ダビデの人生が休む暇なさすぎて逆に笑えてくる。",
          "en": "The moment David finally becomes king, the Philistines immediately show up to cause trouble, proving that being the boss never actually means less stress.",
          "ko": "마침내 왕이 되었는데 곧바로 블레셋이 쳐들어오는 상황을 보니, 리더의 자리는 책임감의 무게만큼이나 끊임없는 위기의 연속인 것 같다.",
          "zho": "大衛終於當上國王，結果非利士人馬上就來找麻煩，這證明了當上老大從來就不代表可以輕鬆過日子。",
          "es": "David finalmente se convierte en rey y los filisteos aparecen de inmediato, demostrando que ser el jefe nunca significa menos estrés.",
          "pt": "David finalmente se torna rei e os filisteus aparecem imediatamente, provando que ser o chefe nunca significa menos estresse.",
          "vi": "Đa-vít vừa lên ngôi thì dân Phi-li-tin đã kéo đến gây hấn, chứng tỏ rằng lên làm sếp không có nghĩa là cuộc đời sẽ bớt sóng gió.",
          "tl": "Nang maging hari na si David, biglang sumugod ang mga Filisteo; patunay na ang pagiging boss ay hindi kailanman nangangahulugan ng mas kaunting problema.",
          "th": "ดาวิดเพิ่งจะได้เป็นกษัตริย์ ชาวฟีลิสเตียก็บุกมาหาเรื่องทันที พิสูจน์ให้เห็นว่าการเป็นหัวหน้าไม่เคยหมายถึงชีวิตที่ไร้ความเครียด",
          "sw": "Daudi anapokuwa mfalme tu, Wafilisti wanavamia; hii inathibitisha kuwa kuwa kiongozi hakuwahi kumaanisha maisha ya raha na amani.",
          "it": "Diventare finalmente re e vedersi subito attaccare dai filistei dimostra che salire di grado non riduce mai lo stress."
        }
    },
    "2026-06-21": {
        date: "2026-06-21",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記下 7:1-29",
          "en": "2 Samuel 7:1-29",
          "ko": "2 Samuel 7:1-29",
          "zho": "サムエル記下 7:1-29",
          "es": "2 Samuel 7:1-29",
          "pt": "2 Samuel 7:1-29",
          "vi": "2 Samuel 7:1-29",
          "tl": "2 Samuel 7:1-29",
          "th": "2 Samuel 7:1-29",
          "sw": "2 Samuel 7:1-29",
          "it": "2 Samuele 7:1-29"
        },
        comment: {
          "ja": "神様のために立派な神殿を建てようと張り切るダビデに、いい意味で肩の力を抜かせる神様の対応は、空回りしがちな真面目な人への最高の処方箋だ。",
          "en": "God gently telling David to take a seat instead of building a temple is the ultimate reality check for those of us who constantly overcommit to be helpful.",
          "ko": "하나님을 위해 거창한 계획을 세우는 다윗에게 잠시 멈추라고 하시는 하나님의 응답은, 의욕만 앞서서 무리하는 우리 모두에게 필요한 브레이크다.",
          "zho": "大衛想為神大興土木卻被勸阻，這種被神溫柔喊停的經驗，簡直是所有熱心過頭的人最需要的降溫劑。",
          "es": "Que Dios le pida a David que se tome un respiro en lugar de construir el templo es el mejor consejo para quienes siempre estamos tratando de hacer de más para impresionar.",
          "pt": "Deus pedir a Davi que espere em vez de construir o templo é o conselho perfeito para quem vive tentando fazer tudo ao mesmo tempo para ser útil.",
          "vi": "Việc Chúa bảo Đa-vít cứ bình tĩnh thay vì vội vàng xây đền thờ là lời nhắc nhở nhẹ nhàng cho những ai luôn cố gắng ôm đồm mọi việc.",
          "tl": "Ang pagpapahinto ng Diyos kay David sa pagtatayo ng templo ay isang paalala na hindi laging kailangan ng Diyos ang ating pagmamadali.",
          "th": "การที่พระเจ้าบอกให้ดาวิดใจเย็นๆ แทนที่จะรีบสร้างพระวิหาร คือคำแนะนำที่ดีที่สุดสำหรับคนที่ชอบแบกโลกไว้คนเดียวจนเกินตัว",
          "sw": "Mungu kumwambia Daudi atulie badala ya kujenga hekalu ni ukumbusho mzuri kwa wale wanaohisi lazima wafanye kila kitu ili kumpendeza Mungu.",
          "it": "Che Dio dica a Davide di fermarsi invece di costruire il tempio è il miglior consiglio per chi tende sempre a strafare."
        }
    },
    "2026-06-22": {
        date: "2026-06-22",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "サムエル記下 11:1-27",
          "en": "2 Samuel 11:1-27",
          "ko": "2 Samuel 11:1-27",
          "zho": "サムエル記下 11:1-27",
          "es": "2 Samuel 11:1-27",
          "pt": "2 Samuel 11:1-27",
          "vi": "2 Samuel 11:1-27",
          "tl": "2 Samuel 11:1-27",
          "th": "2 Samuel 11:1-27",
          "sw": "2 Samuel 11:1-27",
          "it": "2 Samuele 11:1-27"
        },
        comment: {
          "ja": "王としての責任を放り出して昼寝から目覚めたダビデが深みにはまっていく様子は、一瞬の気の緩みが人生をどう変えるかという教訓を冷や汗が出るほど見せつけてくる。",
          "en": "Watching David go from a lazy afternoon nap to a total moral collapse is a terrifying reminder of how quickly one bad choice spirals out of control.",
          "ko": "낮잠 자고 일어나서 별생각 없이 시작한 다윗의 실수가 걷잡을 수 없이 커지는 걸 보면, 잠깐의 방심이 얼마나 무서운지 소름이 돋는다.",
          "zho": "看著大衛從一場午睡開始，一步步滑向道德深淵，這簡直是關於一時鬆懈如何毀掉人生的最驚悚教材。",
          "es": "Ver a David pasar de una siesta tranquila a un desastre moral absoluto es un recordatorio escalofriante de lo rápido que una mala decisión puede arruinarlo todo.",
          "pt": "Ver Davi ir de uma soneca tranquila a um colapso moral total é um lembrete assustador de como uma pequena má escolha pode virar uma bola de neve.",
          "vi": "Chứng kiến Đa-vít từ một giấc ngủ trưa dẫn đến những quyết định sai lầm nối tiếp nhau là lời cảnh tỉnh về việc một phút lơ là có thể phá hỏng cả cuộc đời.",
          "tl": "Ang pagkakita kay David na nagmula sa isang idlip hanggang sa pagbagsak ng kanyang moralidad ay isang nakakatakot na paalala kung gaano kabilis magulo ang lahat dahil sa isang maling desisyon.",
          "th": "การเห็นดาวิดตื่นจากงีบหลับแล้วค่อยๆ ถลำลึกสู่ความผิดพลาด เป็นบทเรียนที่ทำให้เหงื่อตกได้เลยว่าแค่เผลอใจไปนิดเดียวชีวิตก็เปลี่ยนไปได้ขนาดไหน",
          "sw": "Kuona Daudi akitoka kwenye usingizi wa mchana na kuangukia dhambini ni ukumbusho wa kutisha jinsi uamuzi mmoja mbaya unavyoweza kuharibu kila kitu.",
          "it": "Vedere Davide passare da un sonnellino pomeridiano a un disastro morale totale è il promemoria di quanto velocemente una pessima scelta possa rovinare tutto."
        }
    },
    "2026-06-23": {
        date: "2026-06-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 3:1-28",
          "en": "1 Kings 3:1-28",
          "ko": "1 Kings 3:1-28",
          "zho": "列王紀上 3:1-28",
          "es": "1 Kings 3:1-28",
          "pt": "1 Kings 3:1-28",
          "vi": "1 Kings 3:1-28",
          "tl": "1 Kings 3:1-28",
          "th": "1 Kings 3:1-28",
          "sw": "1 Kings 3:1-28",
          "it": "1 Re 3:1-28"
        },
        comment: {
          "ja": "赤ちゃんを真っ二つにするという過激な判決を下すふりをして真実を暴くソロモン、現代のバラエティ番組のドッキリ並みに心臓に悪い。",
          "en": "Solomon threatening to slice a baby in half to uncover the truth is basically the most stressful reality TV plot twist ever.",
          "ko": "아기를 반으로 가르겠다는 파격적인 판결로 진실을 밝혀낸 솔로몬, 현대 예능 프로그램의 몰래카메라보다 심장이 쫄깃해지는 순간이다.",
          "zho": "所羅門假裝要將嬰兒劈成兩半來查明真相，這心理博弈的程度簡直比現代綜藝節目還要驚心動魄。",
          "es": "Salomón amenazando con partir un bebé por la mitad para descubrir la verdad es el giro de guion más estresante de la historia.",
          "pt": "Salomão ameaçando dividir o bebê ao meio para revelar a verdade é o plot twist mais estressante da história.",
          "vi": "Việc Solomon dọa xẻ đôi đứa bé để tìm ra sự thật chẳng khác nào một cú lừa ngoạn mục trong các chương trình truyền hình thực tế.",
          "tl": "Ang pagbabanta ni Solomon na hahatiin ang sanggol para malaman ang katotohanan ay ang pinaka-nakaka-stress na plot twist sa kasaysayan.",
          "th": "โซโลมอนที่ขู่ว่าจะผ่าเด็กทารกครึ่งหนึ่งเพื่อหาความจริงนั้น ตื่นเต้นยิ่งกว่าดูรายการเรียลลิตี้โชว์สมัยนี้เสียอีก",
          "sw": "Sulemani kutishia kumkata mtoto katikati ili kupata ukweli ni mbinu ya kisaikolojia inayoshinda hata vipindi vya televisheni vya leo.",
          "it": "Minacciare di tagliare un neonato a metà per scoprire la verità è il colpo di scena più stressante della storia."
        }
    },
    "2026-06-24": {
        date: "2026-06-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 6:1-38",
          "en": "1 Kings 6:1-38",
          "ko": "1 Kings 6:1-38",
          "zho": "列王紀上 6:1-38",
          "es": "1 Kings 6:1-38",
          "pt": "1 Kings 6:1-38",
          "vi": "1 Kings 6:1-38",
          "tl": "1 Kings 6:1-38",
          "th": "1 Kings 6:1-38",
          "sw": "1 Kings 6:1-38",
          "it": "1 Re 6:1-38"
        },
        comment: {
          "ja": "神殿建設に7年も費やしたソロモン、こだわりの強いこだわり派のDIY愛好家が理想の家を建てるとこうなるという良い例。",
          "en": "Solomon spending seven years building the temple is the ultimate example of what happens when a perfectionist DIY enthusiast gets a budget.",
          "ko": "성전 건축에만 7년을 쏟아부은 솔로몬, 완벽주의 DIY 애호가가 예산을 무제한으로 지원받으면 어떤 일이 벌어지는지 보여주는 완벽한 사례다.",
          "zho": "所羅門花了七年蓋聖殿，這完全是完美主義者在預算充足時折騰出來的終極DIY工程。",
          "es": "Salomón tardando siete años en construir el templo es el ejemplo perfecto de lo que pasa cuando un perfeccionista del bricolaje tiene presupuesto ilimitado.",
          "pt": "Salomão levando sete anos para construir o templo é o exemplo máximo do que acontece quando um perfeccionista do faça-você-mesmo tem orçamento ilimitado.",
          "vi": "Việc Solomon mất bảy năm để xây đền thờ là minh chứng rõ nhất cho thấy chuyện gì xảy ra khi một người đam mê DIY cầu toàn có trong tay ngân sách không giới hạn.",
          "tl": "Ang paggugol ni Solomon ng pitong taon sa pagtatayo ng templo ay ang pinakamagandang halimbawa ng mangyayari kapag ang isang perfectionist na DIY enthusiast ay may unlimited budget.",
          "th": "โซโลมอนใช้เวลาสร้างพระวิหารถึง 7 ปี เป็นตัวอย่างชั้นดีของความเป๊ะระดับมือโปรเมื่อคนรักงานฝีมือได้งบประมาณมาทำตามใจฝัน",
          "sw": "Sulemani kutumia miaka saba kujenga hekalu ni mfano kamili wa kile kinachotokea wakati mtu anayependa ubora anapokuwa na bajeti isiyo na kikomo ya ujenzi.",
          "it": "Solomone che ci mette sette anni a costruire il tempio è l'esempio perfetto di cosa succede quando un perfezionista ha budget illimitato."
        }
    },
    "2026-06-25": {
        date: "2026-06-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 7:1-51",
          "en": "1 Kings 7:1-51",
          "ko": "1 Kings 7:1-51",
          "zho": "列王紀上 7:1-51",
          "es": "1 Kings 7:1-51",
          "pt": "1 Kings 7:1-51",
          "vi": "1 Kings 7:1-51",
          "tl": "1 Kings 7:1-51",
          "th": "1 Kings 7:1-51",
          "sw": "1 Kings 7:1-51",
          "it": "1 Re 7:1-51"
        },
        comment: {
          "ja": "ソロモンが神殿より先に自分の宮殿に時間をかけているあたり、どんなに偉くなっても自分の部屋のインテリアにはこだわりたい人間の性が見える。",
          "en": "Solomon taking longer to build his own palace than the temple is a classic case of human priorities getting a little distracted by home renovation.",
          "ko": "솔로몬이 성전보다 자신의 궁전을 짓는 데 더 공을 들인 걸 보면, 아무리 대단한 사람도 결국 인테리어 욕심은 못 버리는 것 같다.",
          "zho": "所羅門花在蓋自己宮殿的時間比蓋聖殿還久，看來就算再偉大的人，也難免會被自己的裝潢品味給耽誤。",
          "es": "Que Salomón tardara más en construir su propio palacio que el templo demuestra que, por muy sabio que seas, remodelar la casa siempre es la prioridad.",
          "pt": "Salomão gastar mais tempo na própria casa do que no templo mostra que, mesmo sendo sábio, a gente sempre prioriza o nosso próprio conforto.",
          "vi": "Việc Solomon dành nhiều thời gian xây cung điện cho mình hơn là đền thờ cho thấy dù có khôn ngoan đến đâu, con người vẫn không thể cưỡng lại việc ưu tiên cho không gian sống của bản thân.",
          "tl": "Ang paglaan ni Solomon ng mas mahabang panahon sa sarili niyang palasyo kaysa sa templo ay patunay na kahit gaano tayo katalino, prayoridad pa rin natin ang sarili nating bahay.",
          "th": "การที่ซาโลมอนใช้เวลาสร้างวังของตัวเองนานกว่าพระวิหาร สะท้อนให้เห็นว่าต่อให้ฉลาดแค่ไหน มนุษย์ก็ยังอดไม่ได้ที่จะให้ความสำคัญกับการแต่งบ้านของตัวเองก่อน",
          "sw": "Sulemani kutumia muda mwingi kujenga jumba lake kuliko hekalu inaonyesha kuwa hata ukiwa na hekima kiasi gani, bado tunapenda kipaumbele chetu cha kwanza kiwe nyumba zetu wenyewe.",
          "it": "Metterci più tempo per il proprio palazzo che per il tempio dimostra che, per quanto saggi si sia, la ristrutturazione di casa resta la priorità."
        }
    },
    "2026-06-26": {
        date: "2026-06-26",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 8:1-53",
          "en": "1 Kings 8:1-53",
          "ko": "1 Kings 8:1-53",
          "zho": "列王紀上 8:1-53",
          "es": "1 Kings 8:1-53",
          "pt": "1 Kings 8:1-53",
          "vi": "1 Kings 8:1-53",
          "tl": "1 Kings 8:1-53",
          "th": "1 Kings 8:1-53",
          "sw": "1 Kings 8:1-53",
          "it": "1 Re 8:1-53"
        },
        comment: {
          "ja": "神殿が完成して祝杯をあげた直後に、神様から「もし背いたら全部ぶち壊すからな」と釘を刺されるソロモン、絶頂期の緊張感たるや。",
          "en": "Solomon finishing the temple and immediately getting a warning that it could all be torn down if he messes up is the ultimate reality check after a big win.",
          "ko": "성전 완공의 기쁨도 잠시, 바로 다음 순간에 '말 안 들으면 다 부숴버리겠다'는 경고를 듣는 솔로몬을 보니 인생의 쓴맛이 느껴진다.",
          "zho": "所羅門剛慶祝完聖殿落成，馬上就被提醒如果走偏了就會被毀掉，這大概就是人生巔峰期的冷水澡。",
          "es": "Que Salomón recibiera una advertencia sobre la posible destrucción del templo justo después de terminarlo es el recordatorio más intenso de que el éxito viene con condiciones.",
          "pt": "Salomão terminar o templo e logo receber um aviso de que tudo poderia ser destruído é o tipo de balde de água fria que só a vida real oferece.",
          "vi": "Việc Solomon vừa khánh thành đền thờ xong đã bị cảnh báo rằng mọi thứ có thể tan thành mây khói nếu đi chệch đường là một gáo nước lạnh thực tế ngay trên đỉnh vinh quang.",
          "tl": "Ang pagbabala kay Solomon na mawawasak ang templo kung hindi siya susunod, agad pagkatapos itong matapos, ay isang matinding paalala na ang tagumpay ay may kaakibat na pananagutan.",
          "th": "ซาโลมอนเพิ่งฉลองสร้างพระวิหารเสร็จก็โดนเตือนทันทีว่าถ้าพลาดไปอาจจะพังไม่เหลือชิ้นดี เป็นการเตือนสติที่มาได้ถูกจังหวะจริงๆ",
          "sw": "Sulemani kumaliza hekalu na kuonywa mara moja kuwa linaweza kuharibiwa kama atapotoka ni ukumbusho mkali kwamba mafanikio yanakuja na wajibu mkubwa.",
          "it": "Ricevere un avvertimento sui rischi del futuro proprio mentre si festeggia il grande traguardo è la doccia fredda definitiva dopo il successo."
        }
    },
    "2026-06-27": {
        date: "2026-06-27",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 8:54-66",
          "en": "1 Kings 8:54-66",
          "ko": "1 Kings 8:54-66",
          "zho": "列王紀上 8:54-66",
          "es": "1 Kings 8:54-66",
          "pt": "1 Kings 8:54-66",
          "vi": "1 Kings 8:54-66",
          "tl": "1 Kings 8:54-66",
          "th": "1 Kings 8:54-66",
          "sw": "1 Kings 8:54-66",
          "it": "1 Re 8:54-66"
        },
        comment: {
          "ja": "盛大な奉献式のあとに七日間も祝宴を開くソロモン、どれだけパーティー好きなのかと思いつつそのエネルギーに圧倒される。",
          "en": "Solomon throwing a seven-day party after the temple dedication is the ultimate flex of spiritual and physical stamina.",
          "ko": "성전 봉헌식 후에 7일 동안이나 잔치를 벌이는 솔로몬을 보니 그의 엄청난 에너지와 파티에 대한 진심이 느껴진다.",
          "zho": "所羅門在聖殿落成後連開七天派對，這份體力與對慶祝的執著簡直讓人歎為觀止。",
          "es": "Salomón organizando una fiesta de siete días tras la dedicación del templo es el nivel definitivo de entusiasmo y resistencia.",
          "pt": "Salomão dando uma festa de sete dias após a dedicação do templo é o nível máximo de entusiasmo e energia.",
          "vi": "Solomon tổ chức tiệc suốt bảy ngày sau khi dâng hiến đền thờ đúng là đỉnh cao của sự nhiệt huyết và sức bền.",
          "tl": "Ang pagdaraos ni Solomon ng pitong araw na piging pagkatapos ng pagtatalaga ng templo ay nagpapakita ng kanyang walang kapantay na sigla.",
          "th": "โซโลมอนจัดงานเลี้ยงฉลองเจ็ดวันหลังสร้างพระวิหารเสร็จ เป็นตัวอย่างของความทุ่มเทและการเฉลิมฉลองที่ยิ่งใหญ่จริงๆ",
          "sw": "Sulemani kuandaa sherehe ya siku saba baada ya kuweka wakfu hekalu inaonyesha kiwango cha juu cha shauku na nishati.",
          "it": "Organizzare una festa di sette giorni dopo la dedicazione del tempio è il livello definitivo di resistenza spirituale e fisica."
        }
    },
    "2026-06-28": {
        date: "2026-06-28",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 11:1-43",
          "en": "1 Kings 11:1-43",
          "ko": "1 Kings 11:1-43",
          "zho": "列王紀上 11:1-43",
          "es": "1 Kings 11:1-43",
          "pt": "1 Kings 11:1-43",
          "vi": "1 Kings 11:1-43",
          "tl": "1 Kings 11:1-43",
          "th": "1 Kings 11:1-43",
          "sw": "1 Kings 11:1-43",
          "it": "1 Re 11:1-43"
        },
        comment: {
          "ja": "知恵の代名詞だったはずのソロモンが、結局は周りの意見に流されて迷走する姿、人間ってどこまでいっても環境に弱い生き物だと痛感する。",
          "en": "Watching the wisest man in history get swayed by everyone else's opinions is a humbling reminder that no one is immune to social pressure.",
          "ko": "지혜의 대명사였던 솔로몬조차 주변 분위기에 휩쓸려 길을 잃는 것을 보면 인간이 환경에 얼마나 취약한지 다시금 깨닫게 된다.",
          "zho": "看到曾是智慧代名詞的所羅門最終因隨波逐流而迷失，才發現即使再聰明的人也難逃環境的影響。",
          "es": "Ver al hombre más sabio de la historia dejarse llevar por la presión social es un recordatorio de que nadie es inmune a las malas influencias.",
          "pt": "Ver o homem mais sábio da história se deixar levar pela pressão social é um lembrete de que ninguém está imune a más influências.",
          "vi": "Chứng kiến người khôn ngoan nhất lịch sử bị lung lay bởi áp lực xung quanh là lời nhắc nhở rằng không ai miễn nhiễm với những ảnh hưởng xấu.",
          "tl": "Ang makitang ang pinakamatalinong tao sa kasaysayan ay nagpadala sa impluwensya ng iba ay isang paalala na walang sinuman ang ligtas sa panggigipit ng lipunan.",
          "th": "การได้เห็นคนที่ฉลาดที่สุดในประวัติศาสตร์ยังหลงทางเพราะคล้อยตามคนรอบข้าง เป็นเครื่องเตือนใจว่าไม่มีใครต้านทานแรงกดดันทางสังคมได้จริงๆ",
          "sw": "Kumwona mtu mwenye hekima zaidi katika historia akishawishiwa na maoni ya wengine ni ukumbusho kwamba hakuna aliye salama dhidi ya shinikizo la kijamii.",
          "it": "Vedere l'uomo più saggio della storia cedere alla pressione sociale è la prova che nessuno è immune alle cattive influenze."
        }
    },
    "2026-06-29": {
        date: "2026-06-29",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 12:1-33",
          "en": "1 Kings 12:1-33",
          "ko": "1 Kings 12:1-33",
          "zho": "列王紀上 12:1-33",
          "es": "1 Kings 12:1-33",
          "pt": "1 Kings 12:1-33",
          "vi": "1 Kings 12:1-33",
          "tl": "1 Kings 12:1-33",
          "th": "1 Kings 12:1-33",
          "sw": "1 Kings 12:1-33",
          "it": "1 Re 12:1-33"
        },
        comment: {
          "ja": "若手の意見を完全無視して老臣の強気な助言を採用し、結果として国を真っ二つにしたレハベアム、上司選びの重要性をこれ以上ないほど教えてくれる。",
          "en": "Rehoboam ignoring his advisors and listening to his buddies instead is a masterclass in how to ruin a legacy in one day.",
          "ko": "원로들의 조언을 무시하고 자기 친구들의 말만 듣다가 나라를 반으로 쪼갠 르호보암을 보니, 리더의 귀가 얼마나 중요한지 뼈저리게 느껴진다.",
          "zho": "羅波安無視長者建議、只聽損友的話導致國家分裂，這簡直是選錯團隊就毀掉前程的教科書式範例。",
          "es": "Roboam ignorando a los sabios para escuchar solo a sus amigos y terminar dividiendo su reino es el recordatorio definitivo de por qué elegir bien a tus asesores es vital.",
          "pt": "Roboão ignorar os conselheiros experientes para ouvir os amigos e acabar dividindo o reino é a lição definitiva sobre como não gerir uma liderança.",
          "vi": "Rehoboam bỏ ngoài tai lời khuyên của các bậc tiền bối để nghe theo đám bạn trẻ và làm tan nát cả vương quốc, một bài học đắt giá về việc chọn bạn mà chơi.",
          "tl": "Ang pagbalewala ni Rehoboam sa payo ng matatanda para lang makinig sa mga tropa niya na nagresulta sa pagkakawatak-watak ng kaharian ay isang malaking babala sa maling desisyon.",
          "th": "เรโหโบอัมเมินคำแนะนำของผู้อาวุโสแล้วไปฟังพวกพ้องจนอาณาจักรแตกแยก คือบทเรียนราคาแพงของการเลือกที่ปรึกษาผิดชีวิตเปลี่ยน",
          "sw": "Rehoboamu kupuuza ushauri wa wazee na kusikiliza marafiki zake hadi ufalme ukagawanyika ni funzo tosha kuhusu umuhimu wa kuchagua washauri sahihi.",
          "it": "Roboam che ignora i saggi per ascoltare gli amici e finisce per dividere il regno è il manuale perfetto su come rovinare tutto in un giorno."
        }
    },
    "2026-06-30": {
        date: "2026-06-30",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 13:1-34",
          "en": "1 Kings 13:1-34",
          "ko": "1 Kings 13:1-34",
          "zho": "列王紀上 13:1-34",
          "es": "1 Kings 13:1-34",
          "pt": "1 Kings 13:1-34",
          "vi": "1 Kings 13:1-34",
          "tl": "1 Kings 13:1-34",
          "th": "1 Kings 13:1-34",
          "sw": "1 Kings 13:1-34",
          "it": "1 Re 13:1-34"
        },
        comment: {
          "ja": "神様との約束を破った預言者がライオンに襲われるという結末、誘惑に負けて自分を正当化した瞬間の脆さを突きつけられているようで背筋が凍る。",
          "en": "The prophet getting taken out by a lion for one simple mistake is a brutal reminder that knowing the rules and actually following them are two different things.",
          "ko": "하나님과의 약속을 어긴 예언자가 사자에게 당하는 결말을 보면, 유혹 앞에서 적당히 타협하려는 마음이 얼마나 위험한지 다시금 깨닫게 된다.",
          "zho": "先知因為一個小失誤就被獅子咬死，這殘酷的結局提醒我們，知道規矩和嚴格執行規矩完全是兩回事。",
          "es": "Que el profeta termine mal por un solo descuido después de recibir instrucciones claras es un recordatorio frío de que la obediencia parcial es una trampa peligrosa.",
          "pt": "O profeta ser punido por um único deslize após instruções claras é um lembrete gelado de que saber o que é certo e fazer o que é certo são coisas bem diferentes.",
          "vi": "Việc vị tiên tri bị sư tử vồ chỉ vì một sai lầm nhỏ sau khi đã nhận chỉ dẫn rõ ràng là lời nhắc nhở đau đớn rằng sự vâng lời nửa vời là cái bẫy nguy hiểm.",
          "tl": "Ang pagkamatay ng propeta dahil sa isang pagkakamali matapos bigyan ng malinaw na tagubilin ay isang matinding paalala na ang pagsunod ay hindi pwedeng pili-pili lang.",
          "th": "การที่ผู้เผยพระวจนะต้องจบชีวิตเพราะพลาดไปเพียงครั้งเดียวหลังจากได้รับคำสั่งชัดเจน เป็นเครื่องเตือนใจว่าการเชื่อฟังแบบครึ่งๆ กลางๆ นั้นอันตรายแค่ไหน",
          "sw": "Nabii kuangamia kwa kosa moja baada ya kupewa maagizo wazi ni ukumbusho mkali kwamba kutii kwa nusu ni hatari kuliko kutotii kabisa.",
          "it": "Il profeta punito da un leone per una sola distrazione ci ricorda quanto sia pericolosa l'obbedienza a metà."
        }
    },
    "2026-07-01": {
        date: "2026-07-01",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 17:1-24",
          "en": "1 Kings 17:1-24",
          "ko": "1 Kings 17:1-24",
          "zho": "列王紀上 17:1-24",
          "es": "1 Kings 17:1-24",
          "pt": "1 Kings 17:1-24",
          "vi": "1 Kings 17:1-24",
          "tl": "1 Kings 17:1-24",
          "th": "1 Kings 17:1-24",
          "sw": "1 Kings 17:1-24",
          "it": "1 Re 17:1-24"
        },
        comment: {
          "ja": "預言者として大役を任された直後に隠れ家でカラスから食料をもらう生活を始めるエリヤ、神の采配の斜め上を行く展開に振り回される苦労が滲み出ている。",
          "en": "Elijah being tasked with a massive prophetic mission only to end up hiding in a cave waiting for takeout delivered by crows is the ultimate lesson in trusting the weirdest divine plans.",
          "ko": "예언자로 부름받자마자 까마귀가 배달해주는 음식으로 연명하게 된 엘리야, 하나님의 계획은 가끔 이해하기 힘들 정도로 독특해서 당황스럽다.",
          "zho": "以利亞才剛領受先知重任，下一秒就被安排躲在溪邊吃烏鴉送來的外送，這種神操作讓人深刻體會到人生計畫永遠趕不上變化。",
          "es": "Que Elías pase de ser un gran profeta a vivir escondido esperando que los cuervos le traigan el almuerzo demuestra que los planes de Dios son tan impredecibles como efectivos.",
          "pt": "Elias ser chamado para uma missão épica e acabar escondido num riacho esperando corvos trazerem seu lanche é a prova de que Deus tem um senso de humor bem peculiar.",
          "vi": "Elijah vừa được giao trọng trách đã phải đi trốn và chờ quạ mang thức ăn tới, cách sắp xếp của Chúa đôi khi kỳ lạ đến mức khiến ta chỉ biết cạn lời.",
          "tl": "Si Elias na inutusang maging propeta tapos ay kailangan pang magtago at maghintay ng pagkain mula sa mga uwak ay patunay na minsan, ang plano ng Diyos ay sadyang nakakaloka.",
          "th": "เอลียาห์ได้รับภารกิจยิ่งใหญ่แต่กลับต้องไปหลบซ่อนรออาหารจากอีกา การทรงนำของพระเจ้านี่มันเหนือความคาดหมายจนทำตัวไม่ถูกจริงๆ",
          "sw": "Eliya kuitwa kuwa nabii na kisha kuishia kujificha akisubiri kunguru wamletee chakula ni funzo tosha kwamba mipango ya Mungu huwa haifuati taratibu zetu za kawaida.",
          "it": "Elia che si nasconde in una grotta aspettando il cibo portato dai corvi dimostra che i piani divini sanno essere davvero imprevedibili."
        }
    },
    "2026-07-02": {
        date: "2026-07-02",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 18:1-46",
          "en": "1 Kings 18:1-46",
          "ko": "1 Kings 18:1-46",
          "zho": "列王紀上 18:1-46",
          "es": "1 Kings 18:1-46",
          "pt": "1 Kings 18:1-46",
          "vi": "1 Kings 18:1-46",
          "tl": "1 Kings 18:1-46",
          "th": "1 Kings 18:1-46",
          "sw": "1 Kings 18:1-46",
          "it": "1 Re 18:1-46"
        },
        comment: {
          "ja": "バアルの預言者たちと対決する直前にわざわざ祭壇に水をぶっかけるエリヤ、ここぞという時の煽りスキルの高さに人間味を感じる。",
          "en": "Elijah drenching the altar with water right before a showdown just to show off is the ultimate mic drop moment in prophetic history.",
          "ko": "바알 선지자들과의 대결 직전에 제단에 물을 들이붓는 엘리야의 여유, 이 정도면 도발의 고수라고 인정할 수밖에 없다.",
          "zho": "以利亞在決鬥前還特地把祭壇淋濕，這種在重要關頭還要加碼增加難度的心理戰，簡直是把挑釁技能點滿了。",
          "es": "Que Elías empapara el altar con agua justo antes del gran duelo es el nivel máximo de confianza y, honestamente, una jugada maestra de provocación.",
          "pt": "Elias encharcar o altar com água antes do grande confronto é o auge da autoconfiança e uma provocação épica que deixa qualquer um sem palavras.",
          "vi": "Việc Elijah dội nước lên bàn thờ trước khi đối đầu cho thấy ông không chỉ có đức tin mà còn có kỹ năng khiêu khích đối thủ ở đẳng cấp chuyên nghiệp.",
          "tl": "Ang pagbuhos ni Elias ng tubig sa altar bago ang malaking tapatan ay patunay na ang tunay na tiwala sa Diyos ay may kasama ring konting tapang na mang-asar.",
          "th": "เอลียาห์ราดน้ำลงบนแท่นบูชาก่อนการประลองครั้งสำคัญ นี่มันไม่ใช่แค่ความเชื่อ แต่มันคือการปั่นประสาทคู่ต่อสู้แบบเหนือชั้นจริงๆ",
          "sw": "Eliya kumwagia maji madhabahu kabla ya mashindano makuu ni uthibitisho kwamba wakati mwingine imani inahitaji ujasiri wa hali ya juu na kidogo ya kuwashangaza maadui.",
          "it": "Elia che bagna l'altare con l'acqua prima della grande sfida è la quintessenza della sicurezza e della provocazione in grande stile."
        }
    },
    "2026-07-03": {
        date: "2026-07-03",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 19:1-21",
          "en": "1 Kings 19:1-21",
          "ko": "1 Kings 19:1-21",
          "zho": "列王紀上 19:1-21",
          "es": "1 Kings 19:1-21",
          "pt": "1 Kings 19:1-21",
          "vi": "1 Kings 19:1-21",
          "tl": "1 Kings 19:1-21",
          "th": "1 Kings 19:1-21",
          "sw": "1 Kings 19:1-21",
          "it": "1 Re 19:1-21"
        },
        comment: {
          "ja": "大勝利の直後に一人の女性から脅されただけで死にたがるエリヤ、偉大な預言者でもメンタルが折れる時は一瞬という親近感。",
          "en": "Elijah winning a massive spiritual battle only to spiral into a meltdown because of one angry note is the most relatable burnout ever.",
          "ko": "거대한 승리를 거둔 직후 단 한 사람의 협박에 무너져 죽고 싶다고 외치는 엘리야를 보니 위대한 예언자도 결국 사람이구나 싶다.",
          "zho": "以利亞才剛大獲全勝，卻因一個人的威脅就想放棄人生，這種偉大預言家也會瞬間崩潰的反差感實在太真實了。",
          "es": "Ver a Elías desmoronarse por una sola amenaza después de una gran victoria nos recuerda que incluso los más grandes necesitan un descanso y un poco de perspectiva.",
          "pt": "Elias ganhando uma batalha épica e logo depois querendo desistir por causa de uma única ameaça é a definição perfeita de esgotamento emocional.",
          "vi": "Elijah vừa giành chiến thắng vang dội đã muốn bỏ cuộc chỉ vì một lời đe dọa, cho thấy ngay cả những vị tiên tri vĩ đại cũng có lúc kiệt sức.",
          "tl": "Si Elias na nagwagi sa isang malaking laban pero biglang nag-collapse dahil sa isang banta ay patunay na kahit ang pinakamalakas ay napapagod din.",
          "th": "เอลียาห์เพิ่งชนะศึกใหญ่มาหมาดๆ แต่กลับอยากยอมแพ้เพียงเพราะคำขู่ของคนคนเดียว เป็นภาพสะท้อนของอาการหมดไฟที่มนุษย์ทุกคนเข้าใจได้ดี",
          "sw": "Elijah kushinda vita kuu na kisha kutaka kukata tamaa kwa sababu ya tishio moja tu ni kielelezo kamili cha jinsi uchovu wa kiroho unavyoweza kutupata sote.",
          "it": "Vedere Elia crollare per una singola minaccia dopo una grande vittoria ci ricorda che tutti abbiamo bisogno di una pausa."
        }
    },
    "2026-07-04": {
        date: "2026-07-04",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 20:1-43",
          "en": "1 Kings 20:1-43",
          "ko": "1 Kings 20:1-43",
          "zho": "列王紀上 20:1-43",
          "es": "1 Kings 20:1-43",
          "pt": "1 Kings 20:1-43",
          "vi": "1 Kings 20:1-43",
          "tl": "1 Kings 20:1-43",
          "th": "1 Kings 20:1-43",
          "sw": "1 Kings 20:1-43",
          "it": "1 Re 20:1-43"
        },
        comment: {
          "ja": "神様がせっかく勝利をくれたのに、敵の王を勝手に逃がして怒られるアハブ、空気を読まないにもほどがある。",
          "en": "Ahab being handed a victory by God and then blowing it by letting the enemy king go is the ultimate example of missing the point.",
          "ko": "하나님께서 승리를 주셨는데도 자기 마음대로 적의 왕을 살려주어 화를 자초하는 아합을 보면 정말 답답해서 말이 안 나온다.",
          "zho": "上帝明明都賜下勝利了，亞哈卻自作主張放走敵王，這種完全抓不到重點的行為真的讓人無言。",
          "es": "Ahab recibiendo una victoria de parte de Dios y luego arruinándolo todo por dejar ir al rey enemigo es el ejemplo perfecto de no entender la misión.",
          "pt": "Acabe recebendo a vitória das mãos de Deus e estragando tudo ao deixar o rei inimigo escapar mostra que, às vezes, somos nosso pior inimigo.",
          "vi": "Được Chúa ban cho chiến thắng nhưng Ahab lại tự ý thả vua kẻ thù, đây đúng là đỉnh cao của việc không hiểu chuyện.",
          "tl": "Binigyan na ng tagumpay ni God si Ahab, pinalampas pa niya ang hari ng mga kaaway; minsan talaga, tayo ang humaharang sa sarili nating pagpapala.",
          "th": "พระเจ้าประทานชัยชนะให้แท้ๆ แต่อาหับกลับปล่อยราชาศัตรูไปเองจนเรื่องบานปลาย นี่คือตัวอย่างของการทำลายโอกาสตัวเองชัดๆ",
          "sw": "Ahab kupewa ushindi na Mungu kisha kuuharibu kwa kumwachilia mfalme wa adui ni mfano bora wa jinsi tunavyoweza kujikwaza wenyewe.",
          "it": "Acab che riceve la vittoria da Dio e poi la rovina lasciando andare il re nemico è l'esempio perfetto di chi non capisce la missione."
        }
    },
    "2026-07-05": {
        date: "2026-07-05",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀上 22:1-53",
          "en": "1 Kings 22:1-53",
          "ko": "1 Kings 22:1-53",
          "zho": "列王紀上 22:1-53",
          "es": "1 Kings 22:1-53",
          "pt": "1 Kings 22:1-53",
          "vi": "1 Kings 22:1-53",
          "tl": "1 Kings 22:1-53",
          "th": "1 Kings 22:1-53",
          "sw": "1 Kings 22:1-53",
          "it": "1 Re 22:1-53"
        },
        comment: {
          "ja": "都合の悪い予言は聞かなかったことにして突き進むアハブ王、自分に甘い鏡を見ているようでいたたまれない。",
          "en": "King Ahab ignoring the uncomfortable truth just to hear what he wants is like watching ourselves at our most stubborn.",
          "ko": "불편한 예언은 무시하고 듣고 싶은 말만 골라 듣는 아합 왕의 모습에서 나의 고집스러운 일면을 본다.",
          "zho": "亞哈王選擇性地忽略不中聽的預言，這種自欺欺人的樣子簡直就是我們內心深處那份固執的縮影。",
          "es": "El rey Acab ignorando la verdad solo para escuchar lo que quería oír es el reflejo perfecto de nuestra propia terquedad.",
          "pt": "O rei Acabe ignorando a verdade só para ouvir o que queria é o reflexo perfeito da nossa própria teimosia.",
          "vi": "Vua A-háp lờ đi sự thật chỉ để nghe những điều mình muốn là hình ảnh phản chiếu sự bướng bỉnh của chính chúng ta.",
          "tl": "Ang pagbalewala ni Haring Ahab sa katotohanan para lang marinig ang gusto niyang pakinggan ay salamin ng ating sariling katigasan ng ulo.",
          "th": "กษัตริย์อาหับที่เลือกฟังแต่สิ่งที่อยากได้ยินและเพิกเฉยต่อความจริง คือกระจกสะท้อนความดื้อรั้นในใจเราทุกคน",
          "sw": "Mfalme Ahab kupuuza ukweli ili asikie tu kile anachotaka kusikia ni kioo kamili cha ukaidi wetu wenyewe.",
          "it": "Il re Acab che ignora la verità per sentirsi dire solo ciò che vuole è lo specchio perfetto della nostra stessa testardaggine."
        }
    },
    "2026-07-06": {
        date: "2026-07-06",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 2:1-25",
          "en": "2 Kings 2:1-25",
          "ko": "2 Kings 2:1-25",
          "zho": "列王紀下 2:1-25",
          "es": "2 Kings 2:1-25",
          "pt": "2 Kings 2:1-25",
          "vi": "2 Kings 2:1-25",
          "tl": "2 Kings 2:1-25",
          "th": "2 Kings 2:1-25",
          "sw": "2 Kings 2:1-25",
          "it": "2 Re 2:1-25"
        },
        comment: {
          "ja": "エリヤの昇天を目の当たりにして服を破るエリシャ、師匠との別れへの動揺が隠しきれず人間味に溢れている。",
          "en": "Elisha tearing his clothes in grief after watching Elijah go is a raw, messy reminder that even prophets have feelings.",
          "ko": "엘리야를 떠나보내며 옷을 찢는 엘리사의 모습은 위대한 선지자도 결국 사람임을 실감하게 한다.",
          "zho": "以利沙目睹以利亞升天後撕裂衣服，那份藏不住的哀傷讓人感嘆，即使是先知也有脆弱的一面。",
          "es": "Eliseo rasgando sus vestidos al ver partir a Elías nos recuerda que, incluso para un profeta, decir adiós es desgarrador.",
          "pt": "Eliseu rasgando suas vestes ao ver Elias partir nos lembra que, mesmo para um profeta, dizer adeus é devastador.",
          "vi": "Ê-li-sê xé áo khi thấy Ê-li được đưa lên trời nhắc nhở chúng ta rằng ngay cả một nhà tiên tri cũng có những cảm xúc con người rất thực.",
          "tl": "Ang pagpunit ni Eliseo sa kanyang damit habang nakikita ang pag-akyat ni Elias ay paalala na kahit ang mga propeta ay may pusong marunong masaktan.",
          "th": "เอลีชาฉีกเสื้อตัวเองเมื่อเห็นเอลียาห์จากไป เป็นภาพที่เตือนใจว่าแม้แต่ผู้เผยพระวจนะก็มีความรู้สึกที่เปราะบางเช่นกัน",
          "sw": "Elisha kurarua nguo zake baada ya kumwona Eliya akipaa ni ukumbusho mbichi kwamba hata manabii wana hisia za kibinadamu.",
          "it": "Eliseo che si strappa le vesti per il dolore ci ricorda che dire addio a un maestro è devastante anche per un profeta."
        }
    },
    "2026-07-07": {
        date: "2026-07-07",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 3:1-27",
          "en": "2 Kings 3:1-27",
          "ko": "2 Kings 3:1-27",
          "zho": "列王紀下 3:1-27",
          "es": "2 Kings 3:1-27",
          "pt": "2 Kings 3:1-27",
          "vi": "2 Kings 3:1-27",
          "tl": "2 Kings 3:1-27",
          "th": "2 Kings 3:1-27",
          "sw": "2 Kings 3:1-27",
          "it": "2 Re 3:1-27"
        },
        comment: {
          "ja": "ピンチになった途端に神様を頼り出す王たちの姿、結局最後は神頼みという人間の身勝手さは古今東西変わりませんね。",
          "en": "Kings only calling on God once they are in deep trouble is a timeless reminder that we are all fair-weather believers at heart.",
          "ko": "궁지에 몰려서야 하나님을 찾는 왕들의 모습에서 평소엔 잊고 살다가 급할 때만 찾는 우리의 이기적인 모습을 봅니다.",
          "zho": "國王們到了走投無路才想起求神，這種臨渴掘井的人性弱點真是古今皆然。",
          "es": "Los reyes que solo buscan a Dios cuando están en apuros son el reflejo perfecto de nuestra propia tendencia a acordarnos del cielo solo en las malas.",
          "pt": "Reis que só recorrem a Deus quando estão em apuros são o espelho perfeito da nossa tendência de só pedir ajuda quando a situação aperta.",
          "vi": "Các vị vua chỉ tìm đến Chúa khi gặp khó khăn là minh chứng rõ nhất cho thói quen chỉ cầu nguyện khi nước đến chân mới nhảy của con người.",
          "tl": "Ang mga hari na tumatawag lang sa Diyos kapag gipit na gipit na ay tunay na repleksyon ng ating pagiging makasarili sa pananampalataya.",
          "th": "เหล่ากษัตริย์ที่หันมาพึ่งพระเจ้าก็ต่อเมื่อจนตรอก สะท้อนให้เห็นถึงนิสัยมนุษย์ที่มักจะนึกถึงฟ้าดินก็ต่อเมื่อเกิดเรื่องเท่านั้น",
          "sw": "Wafalme kumtafuta Mungu wakiwa wameishiwa na njia ni kielelezo kamili cha jinsi binadamu tunavyokumbuka kumuomba Mungu wakati wa shida pekee.",
          "it": "Cercare Dio solo quando si e nei guai dimostra che siamo tutti credenti part-time."
        }
    },
    "2026-07-08": {
        date: "2026-07-08",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 4:1-17",
          "en": "2 Kings 4:1-17",
          "ko": "2 Kings 4:1-17",
          "zho": "列王紀下 4:1-17",
          "es": "2 Kings 4:1-17",
          "pt": "2 Kings 4:1-17",
          "vi": "2 Kings 4:1-17",
          "tl": "2 Kings 4:1-17",
          "th": "2 Kings 4:1-17",
          "sw": "2 Kings 4:1-17",
          "it": "2 Re 4:1-17"
        },
        comment: {
          "ja": "空の器が油で満たされるまで借りてこいと言われて、近所中から器を集めて回る母子の必死さは、もはや現代のフリマアプリの出品作業より大変そう。",
          "en": "The mother and son scrambling to borrow every empty jar in the neighborhood makes modern-day frantic online shopping look like a walk in the park.",
          "ko": "기름을 채우기 위해 동네방네 빈 그릇을 빌리러 다니는 모자의 모습은 현대의 바쁜 일상보다 더 숨 가쁜 긴박함이 느껴집니다.",
          "zho": "為了裝油而向鄰居借遍所有瓶瓶罐罐，這種為了奇蹟而忙得團團轉的母子，真實得讓人想幫他們一把。",
          "es": "La desesperación de la madre y el hijo buscando jarras vacías por todo el vecindario es tan humana que casi se puede sentir el estrés del momento.",
          "pt": "A correria da mãe e do filho pedindo jarras emprestadas por toda a vizinhança é tão real que quase dá para sentir o suor do desespero deles.",
          "vi": "Cảnh hai mẹ con chạy đôn chạy đáo mượn từng chiếc bình khắp xóm thật sự là một ví dụ điển hình về sự nỗ lực hết mình khi cơ hội đến.",
          "tl": "Ang pagmamadali ng mag-ina na humiram ng lahat ng bote sa kapitbahayan ay isang nakaka-relate na eksena ng pagsisikap para sa isang himala.",
          "th": "ภาพแม่ลูกที่วิ่งวุ่นยืมภาชนะไปทั่วละแวกบ้านเพื่อรองรับน้ำมัน เป็นความพยายามที่ดูจริงจังและน่าเอาใจช่วยอย่างบอกไม่ถูก",
          "sw": "Mama na mwanae kukimbia kote wakiazima vyombo kwa ajili ya mafuta ni kielelezo cha jinsi tunavyopaswa kujiandaa kupokea baraka kwa bidii.",
          "it": "La disperazione di questa madre che cerca barattoli vuoti in tutto il vicinato fa sembrare lo shopping online moderno una passeggiata."
        }
    },
    "2026-07-09": {
        date: "2026-07-09",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 4:18-44",
          "en": "2 Kings 4:18-44",
          "ko": "2 Kings 4:18-44",
          "zho": "列王紀下 4:18-44",
          "es": "2 Kings 4:18-44",
          "pt": "2 Kings 4:18-44",
          "vi": "2 Kings 4:18-44",
          "tl": "2 Kings 4:18-44",
          "th": "2 Kings 4:18-44",
          "sw": "2 Kings 4:18-44",
          "it": "2 Re 4:18-44"
        },
        comment: {
          "ja": "毒入りの鍋に粉を投げ込んで中和させるエリシャ、現代の料理で失敗したときのリカバリー術として参考にしたいレベル。",
          "en": "Elisha tossing flour into a poisonous pot to fix dinner is the ultimate masterclass in kitchen damage control.",
          "ko": "독이 든 솥에 가루를 뿌려 해결하는 엘리사를 보면 요리하다 실수했을 때의 대처 능력이 새삼 존경스러워진다.",
          "zho": "以利沙往毒鍋裡撒把麵粉就化險為夷，這簡直是廚房翻車時最完美的補救教學。",
          "es": "Elisha salvando una cena envenenada con un poco de harina es el nivel de improvisación culinaria que todos necesitamos en un mal día.",
          "pt": "Eliseu jogando farinha na panela envenenada para salvar o jantar é o tipo de improviso culinário que todos nós precisamos em um dia ruim.",
          "vi": "Elisha cứu cả nồi súp bị nhiễm độc chỉ bằng một ít bột mì đúng là bậc thầy xử lý khủng hoảng trong nhà bếp.",
          "tl": "Ang paglagay ni Eliseo ng harina sa nilalasong nilaga ay ang ultimate life hack para sa mga palpak na lutuin.",
          "th": "เอลีชาแก้ปัญหาหม้อซุปที่มีพิษด้วยการโรยแป้งลงไป เป็นเทคนิคกู้สถานการณ์ในครัวที่น่าเอาไปใช้ตามจริงๆ",
          "sw": "Elisha kutupa unga kwenye sufuria yenye sumu ili kuokoa chakula ni ustadi wa hali ya juu wa kutatua matatizo jikoni.",
          "it": "Eliseo che salva una cena avvelenata con un po' di farina e il livello di improvvisazione culinaria che tutti vorremmo avere in un giorno storto."
        }
    },
    "2026-07-10": {
        date: "2026-07-10",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 5:1-27",
          "en": "2 Kings 5:1-27",
          "ko": "2 Kings 5:1-27",
          "zho": "列王紀下 5:1-27",
          "es": "2 Kings 5:1-27",
          "pt": "2 Kings 5:1-27",
          "vi": "2 Kings 5:1-27",
          "tl": "2 Kings 5:1-27",
          "th": "2 Kings 5:1-27",
          "sw": "2 Kings 5:1-27",
          "it": "2 Re 5:1-27"
        },
        comment: {
          "ja": "高級将軍なのにヨルダン川で洗えと言われてプライドが邪魔をするナアマン、簡単な解決策ほど受け入れられないのは今も昔も変わらない。",
          "en": "Naaman nearly blowing his chance at healing just because the instructions felt too beneath his status is peak human ego.",
          "ko": "요단강에 몸을 씻으라는 말에 자존심부터 내세우는 나아만을 보니 쉬운 해결책일수록 받아들이기 힘든 인간의 본성이 보인다.",
          "zho": "乃縵將軍因為嫌約旦河水太普通而差點拒絕醫治，這種因為自尊心作祟而錯過簡單解法的戲碼，現代職場也常上演。",
          "es": "Naaman casi arruina su sanación solo porque el remedio le pareció demasiado sencillo para su estatus, un recordatorio de que el ego es nuestro peor enemigo.",
          "pt": "Naaman quase perdeu a chance de ser curado só porque o remédio era simples demais para o seu ego, provando que a simplicidade é o maior desafio do orgulho.",
          "vi": "Naaman suýt chút nữa đã bỏ lỡ cơ hội được chữa lành chỉ vì ông thấy phương pháp đó quá tầm thường, đúng là cái tôi luôn cản trở những giải pháp đơn giản nhất.",
          "tl": "Halos palampasin ni Naaman ang himala dahil lang sa pride niya sa simpleng utos, patunay na minsan ang ego natin ang hadlang sa solusyon.",
          "th": "นาอามานเกือบพลาดโอกาสรักษาโรคเพียงเพราะวิธีแก้ดูเรียบง่ายเกินไปสำหรับตำแหน่งของเขา เป็นบทเรียนว่าอีโก้มักจะขวางทางความสำเร็จเสมอ",
          "sw": "Naaman karibu akose uponyaji kwa sababu tu maagizo yalionekana kuwa ya kawaida sana, ikithibitisha kuwa kiburi ndicho kikwazo kikubwa cha suluhisho rahisi.",
          "it": "Naaman che quasi rovina la sua guarigione solo perche la cura sembrava troppo semplice e la prova che il nostro ego e il peggior nemico."
        }
    },
    "2026-07-11": {
        date: "2026-07-11",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 6:1-33",
          "en": "2 Kings 6:1-33",
          "ko": "2 Kings 6:1-33",
          "zho": "列王紀下 6:1-33",
          "es": "2 Kings 6:1-33",
          "pt": "2 Kings 6:1-33",
          "vi": "2 Kings 6:1-33",
          "tl": "2 Kings 6:1-33",
          "th": "2 Kings 6:1-33",
          "sw": "2 Kings 6:1-33",
          "it": "2 Re 6:1-33"
        },
        comment: {
          "ja": "借りた斧を池に落として絶望する弟子と、それをあっさり回収してあげるエリシャの温度差が、まさに厳しくも優しい上司と部下の関係そのもの。",
          "en": "The panic of a disciple losing a borrowed axe versus Elisha just casually retrieving it is the ultimate mood of a high-stress boss-employee dynamic.",
          "ko": "빌린 도끼를 물에 빠뜨려 절망하는 제자와 아무렇지 않게 건져주는 엘리사의 온도 차이는 마치 현실 직장 상사와 부하 직원의 모습을 보는 듯하다.",
          "zho": "借來的斧頭掉進水裡就陷入絕望的門徒，與輕而易舉就幫忙撈起的以利沙，這溫差簡直就是現實中嚴厲主管與下屬的寫照。",
          "es": "El pánico del discípulo al perder el hacha prestada frente a la calma de Eliseo recuperándola es la viva imagen de la diferencia entre un empleado estresado y un jefe experimentado.",
          "pt": "O desespero do discípulo por perder o machado emprestado versus a calma de Eliseu em recuperá-lo é o retrato fiel da dinâmica entre um funcionário aflito e um chefe experiente.",
          "vi": "Sự hoảng loạn của người môn đệ khi làm mất chiếc rìu đi mượn so với cách Ê-li-sê thản nhiên lấy lại nó chính là bài học về sự khác biệt giữa người lo lắng và người làm chủ tình hình.",
          "tl": "Ang pagpapanic ng alagad dahil sa nawalang palakol kumpara sa kalmadong pagkuha ni Eliseo rito ay eksaktong sumasalamin sa ugnayan ng isang nag-aalalang empleyado at isang bihasang boss.",
          "th": "ความตื่นตระหนกของศิษย์ที่ทำขวานยืมเขามาตกน้ำ กับการที่เอลีชาช่วยกู้คืนมาให้ได้อย่างง่ายดายนั้น เป็นภาพสะท้อนความสัมพันธ์ระหว่างลูกน้องที่ขี้กังวลกับเจ้านายที่เก๋าเกมได้ดีจริงๆ",
          "sw": "Woga wa mwanafunzi aliyepoteza shoka alilokopa ikilinganishwa na jinsi Elisha alivyolichota kwa urahisi ni mfano kamili wa tofauti kati ya mfanyakazi mwenye hofu na bosi mwenye uzoefu.",
          "it": "Il panico del discepolo per un'ascia prestata rispetto alla calma di Eliseo e la perfetta dinamica tra un dipendente in ansia e un capo esperto."
        }
    },
    "2026-07-12": {
        date: "2026-07-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 7:1-20",
          "en": "2 Kings 7:1-20",
          "ko": "2 Kings 7:1-20",
          "zho": "列王紀下 7:1-20",
          "es": "2 Kings 7:1-20",
          "pt": "2 Kings 7:1-20",
          "vi": "2 Kings 7:1-20",
          "tl": "2 Kings 7:1-20",
          "th": "2 Kings 7:1-20",
          "sw": "2 Kings 7:1-20",
          "it": "2 Re 7:1-20"
        },
        comment: {
          "ja": "目の前の奇跡を信じきれず、結局恩恵に預かれないまま踏みつけられて終わる高官、理屈で考えすぎてチャンスを逃す現代人の末路を見ているようで苦しい。",
          "en": "The royal officer who couldn't believe the miracle and ended up getting trampled is a harsh reminder of how overthinking can make us miss out on everything.",
          "ko": "눈앞의 기적을 끝까지 의심하다가 결국 혜택은 누리지도 못하고 짓밟히고 만 장관, 논리만 따지다 기회를 놓치는 현대인의 말로 같아서 씁쓸하다.",
          "zho": "對眼前的神蹟心存懷疑，結果連好處都沒撈到就被踐踏而死的官員，簡直是那些因為想太多而錯失良機的現代人的寫照。",
          "es": "El oficial que no pudo creer en el milagro y terminó siendo pisoteado es un recordatorio cruel de cómo el exceso de análisis nos hace perder las mejores oportunidades.",
          "pt": "O oficial que não conseguiu acreditar no milagre e acabou sendo pisoteado é um lembrete duro de como o excesso de racionalidade nos faz perder as melhores chances.",
          "vi": "Vị quan không tin vào phép màu rồi cuối cùng bị giẫm đạp đến chết là một lời nhắc nhở cay đắng về việc suy nghĩ quá nhiều có thể khiến ta bỏ lỡ mọi cơ hội tốt đẹp.",
          "tl": "Ang opisyal na hindi naniwala sa himala at sa huli ay nayurakan lang ay isang malupit na paalala kung paano tayo nawawalan ng pagkakataon dahil sa sobrang pag-iisip.",
          "th": "ขุนนางที่ไม่ยอมเชื่อในปาฏิหาริย์ตรงหน้าจนสุดท้ายต้องจบชีวิตด้วยการถูกเหยียบตาย เป็นเครื่องเตือนใจที่เจ็บปวดว่าการคิดเยอะเกินไปอาจทำให้เราพลาดทุกโอกาสดีๆ ในชีวิต",
          "sw": "Ofisa aliyeshindwa kuamini muujiza na mwishowe kukanyagwa hadi kufa ni ukumbusho mchungu wa jinsi kufikiri kupita kiasi kunavyoweza kutufanya tukose fursa zote muhimu.",
          "it": "L'ufficiale che non ha creduto al miracolo e ci e rimasto sotto e il promemoria crudele di come pensare troppo ci faccia perdere le occasioni migliori."
        }
    },
    "2026-07-13": {
        date: "2026-07-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 16:1-20",
          "en": "2 Kings 16:1-20",
          "ko": "2 Kings 16:1-20",
          "zho": "列王紀下 16:1-20",
          "es": "2 Kings 16:1-20",
          "pt": "2 Kings 16:1-20",
          "vi": "2 Kings 16:1-20",
          "tl": "2 Kings 16:1-20",
          "th": "2 Kings 16:1-20",
          "sw": "2 Kings 16:1-20",
          "it": "2 Re 16:1-20"
        },
        comment: {
          "ja": "親が敬虔だったからといって子供が自動的に立派になるわけではないという、アハズ王という残酷な現実に直面する朝。",
          "en": "King Ahaz proves that having a godly parent is no guarantee you will not turn out to be a disaster yourself.",
          "ko": "부모가 경건하다고 해서 자녀가 자동으로 훌륭해지는 것은 아니라는 아하스 왕의 사례가 참 뼈아프게 다가옵니다.",
          "zho": "亚哈斯王提醒我们，生在敬虔的家庭并不代表自己就能自动成为圣人，遗传基因里可没写着信仰。",
          "es": "El rey Acaz nos recuerda que tener padres piadosos no garantiza que uno no termine tomando las peores decisiones de su vida.",
          "pt": "O rei Acaz nos lembra que ter pais piedosos não é garantia de que os filhos não escolham o caminho mais desastroso possível.",
          "vi": "Vua A-cha là minh chứng cho thấy việc sinh ra trong một gia đình đạo đức không tự động biến bạn thành một người tốt.",
          "tl": "Ang kwento ni Haring Ahaz ay patunay na hindi awtomatikong nagiging mabuti ang isang tao kahit pa matuwid ang kanyang mga magulang.",
          "th": "กษัตริย์อาหัสเป็นเครื่องเตือนใจว่าการมีพ่อแม่ที่ยำเกรงพระเจ้าไม่ได้การันตีว่าลูกจะเดินตามรอยนั้นเสมอไป",
          "sw": "Mfalme Ahazi anatukumbusha kuwa kuwa na wazazi wema hakuhakikishi kwamba mtoto hatachagua njia mbaya zaidi maishani.",
          "it": "Il re Acaz ci ricorda che avere genitori esemplari non ti impedisce di combinare guai colossali."
        }
    },
    "2026-07-14": {
        date: "2026-07-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 18:1-37",
          "en": "2 Kings 18:1-37",
          "ko": "2 Kings 18:1-37",
          "zho": "列王紀下 18:1-37",
          "es": "2 Kings 18:1-37",
          "pt": "2 Kings 18:1-37",
          "vi": "2 Kings 18:1-37",
          "tl": "2 Kings 18:1-37",
          "th": "2 Kings 18:1-37",
          "sw": "2 Kings 18:1-37",
          "it": "2 Re 18:1-37"
        },
        comment: {
          "ja": "神殿の宝物を削ってまで敵に貢ぎ物を送るヒゼキヤの姿、追い詰められた時の人間は本当に冷静な判断ができなくなるものだ。",
          "en": "Hezekiah cutting up temple treasures to pay off the enemy shows just how much we lose our cool when we are truly backed into a corner.",
          "ko": "성전 기물까지 뜯어내며 적에게 바치는 히스기야를 보니, 벼랑 끝에 몰린 인간이 얼마나 판단력을 잃기 쉬운지 알 것 같습니다.",
          "zho": "希西家为了买平安连圣殿的财宝都拆了，人在走投无路时做出的决定往往比想象中还要荒唐。",
          "es": "Ezequías desmantelando el templo para pagarle al enemigo es el ejemplo perfecto de cómo el pánico nos hace tomar decisiones desesperadas.",
          "pt": "Ezequias desmontando o templo para subornar o inimigo é a prova de que, quando estamos acuados, o desespero dita as regras.",
          "vi": "Việc Ê-xê-chi-a tháo dỡ đền thờ để cống nạp cho kẻ thù cho thấy con người dễ đánh mất sự sáng suốt thế nào khi bị dồn vào chân tường.",
          "tl": "Ang pagbaklas ni Hezekias sa mga kayamanan ng templo para pambayad sa kaaway ay nagpapakita kung gaano tayo kawalang-kibo kapag desperado na.",
          "th": "การที่เฮเซคียาห์ยอมถอดสมบัติในพระวิหารเพื่อไปจ่ายค่าไถ่ศัตรู ทำให้เห็นว่าเมื่อคนเราจนตรอกเราก็มักจะตัดสินใจอะไรที่พลาดไปได้ง่ายๆ",
          "sw": "Hezekia kung'oa dhahabu kwenye hekalu ili kumlipa adui inaonyesha jinsi tunavyoweza kupoteza busara tunapokuwa kwenye shinikizo kubwa.",
          "it": "Ezechia che smantella il tempio per pagare i nemici dimostra quanto il panico ci faccia perdere la bussola."
        }
    },
    "2026-07-15": {
        date: "2026-07-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 19:1-37",
          "en": "2 Kings 19:1-37",
          "ko": "2 Kings 19:1-37",
          "zho": "列王紀下 19:1-37",
          "es": "2 Kings 19:1-37",
          "pt": "2 Kings 19:1-37",
          "vi": "2 Kings 19:1-37",
          "tl": "2 Kings 19:1-37",
          "th": "2 Kings 19:1-37",
          "sw": "2 Kings 19:1-37",
          "it": "2 Re 19:1-37"
        },
        comment: {
          "ja": "絶体絶命の危機に手紙を広げて神様に助けを求めるヒゼキヤ、結局最後は神頼みという人間の究極の正直さが出ている。",
          "en": "Hezekiah taking a threatening letter straight to the temple shows that his ultimate strategy was just hitting the panic button and letting God handle the rest.",
          "ko": "절체절명의 위기 상황에서 편지를 펴놓고 하나님께 매달리는 히스기야의 모습은 결국 마지막엔 하나님뿐이라는 우리네 솔직한 본심을 보여준다.",
          "zho": "希西家在生死存亡之際把恐嚇信攤在神面前，這種最後關頭全靠神處理的反應，簡直是人類最真實的寫照。",
          "es": "Ezequías llevando la carta amenazante ante el altar demuestra que, cuando todo se complica, nuestra estrategia más honesta es simplemente dejar que Dios se encargue.",
          "pt": "Ezequias levando a carta de ameaça ao templo mostra que, no limite, nossa estratégia mais honesta é apenas entregar o problema para Deus.",
          "vi": "Việc Ê-xê-chia mang bức thư đe dọa đến trước mặt Chúa cho thấy chiến lược cuối cùng của con người khi bế tắc chính là phó thác hoàn toàn cho Ngài.",
          "tl": "Ang pagdadala ni Hezekias ng sulat ng pananakot sa templo ay nagpapakita na sa harap ng matinding krisis, ang pinaka-tapat nating magagawa ay ipaubaya na lang sa Diyos ang lahat.",
          "th": "การที่เฮเซคียาห์นำจดหมายข่มขู่ไปวางต่อหน้าพระเจ้าแสดงให้เห็นว่า ในนาทีวิกฤตที่สุด กลยุทธ์ที่จริงใจที่สุดของเราก็คือการปล่อยให้พระองค์จัดการทุกอย่าง",
          "sw": "Hezekia kupeleka barua ya vitisho hekaluni inaonyesha kuwa katika wakati wa dhiki kuu, mbinu yetu ya kweli ni kumwachia Mungu ashughulikie kila kitu.",
          "it": "Ezechia che porta una lettera minacciosa direttamente in preghiera ci ricorda che l'onesta estrema e la prima cosa che facciamo quando siamo con le spalle al muro."
        }
    },
    "2026-07-16": {
        date: "2026-07-16",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 20:1-21",
          "en": "2 Kings 20:1-21",
          "ko": "2 Kings 20:1-21",
          "zho": "列王紀下 20:1-21",
          "es": "2 Kings 20:1-21",
          "pt": "2 Kings 20:1-21",
          "vi": "2 Kings 20:1-21",
          "tl": "2 Kings 20:1-21",
          "th": "2 Kings 20:1-21",
          "sw": "2 Kings 20:1-21",
          "it": "2 Re 20:1-21"
        },
        comment: {
          "ja": "死の宣告を受けて命乞いをして15年寿命を延ばしたヒゼキヤ、その後の傲慢な態度を見ると、人間は時間ができても結局同じ間違いを繰り返す生き物だと痛感する。",
          "en": "Hezekiah begged for fifteen more years of life, only to spend them showing off his treasure, proving that extra time doesn't always lead to extra wisdom.",
          "ko": "죽음 앞에서 15년 생명을 연장받은 히스기야가 나중에 보물을 자랑하는 모습을 보면, 인간은 시간이 더 주어진다고 해서 꼭 지혜로워지는 건 아니라는 게 뼈저리게 느껴진다.",
          "zho": "希西家求來了十五年的壽命，結果卻用來炫耀財富，這證明了多給人類一點時間，並不一定會讓我們變得更聰明。",
          "es": "Ezequías rogó por quince años más de vida y terminó usándolos para presumir sus tesoros, lo que demuestra que tener más tiempo no siempre nos hace más sabios.",
          "pt": "Ezequias pediu mais quinze anos de vida e acabou usando-os para exibir seus tesouros, provando que tempo extra nem sempre resulta em mais sabedoria.",
          "vi": "Ê-xê-chia cầu xin thêm mười lăm năm cuộc đời rồi lại dùng chính thời gian đó để khoe khoang của cải, chứng tỏ rằng thêm thời gian không phải lúc nào cũng mang lại thêm sự khôn ngoan.",
          "tl": "Nagmakaawa si Hezekias para sa karagdagang labinlimang taon, pero ginamit lang niya ito para ipagmalaki ang kanyang mga kayamanan, na nagpapatunay na ang dagdag na oras ay hindi laging nangangahulugan ng dagdag na karunungan.",
          "th": "เฮเซคียาห์ขอชีวิตเพิ่มอีก 15 ปี แต่กลับใช้เวลาที่ได้มานั้นไปกับการอวดสมบัติ ซึ่งพิสูจน์ให้เห็นว่าเวลาที่เพิ่มขึ้นไม่ได้ทำให้เราฉลาดขึ้นเสมอไป",
          "sw": "Hezekia aliomba kuongezewa miaka kumi na mitano ya kuishi, lakini akaitumia kujivunia mali zake, ikithibitisha kuwa muda wa ziada haufanyi mtu kuwa na hekima zaidi.",
          "it": "Ezechia che implora altri quindici anni di vita per poi sprecarli a fare lo sborone dimostra che il tempo extra non ci rende automaticamente piu saggi."
        }
    },
    "2026-07-17": {
        date: "2026-07-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 22:1-20",
          "en": "2 Kings 22:1-20",
          "ko": "2 Kings 22:1-20",
          "zho": "列王紀下 22:1-20",
          "es": "2 Kings 22:1-20",
          "pt": "2 Kings 22:1-20",
          "vi": "2 Kings 22:1-20",
          "tl": "2 Kings 22:1-20",
          "th": "2 Kings 22:1-20",
          "sw": "2 Kings 22:1-20",
          "it": "2 Re 22:1-20"
        },
        comment: {
          "ja": "何十年も放置されていた古い律法の書を偶然見つけて大騒ぎになる様子は、年末の大掃除で失くしたはずの通帳が出てきた時のあの気まずさに似ている。",
          "en": "Finding a long-lost scroll of the law during a renovation is basically the biblical equivalent of finding your forgotten bank statement during spring cleaning.",
          "ko": "몇십 년간 방치된 율법책을 우연히 발견하고 당황하는 모습은 대청소하다가 잃어버린 줄 알았던 통장을 찾아냈을 때의 그 묘한 기분과 똑같다.",
          "zho": "在修缮圣殿时意外翻出被遗忘已久的律法书，这感觉就像大扫除时突然从旧衣柜深处翻出了一张还有余额的旧存折。",
          "es": "Encontrar el libro de la ley olvidado durante una reforma es como encontrar un billete olvidado en el bolsillo de un abrigo viejo justo cuando más lo necesitabas.",
          "pt": "Encontrar o livro da lei esquecido durante uma reforma é basicamente o equivalente bíblico de achar dinheiro esquecido no bolso de um casaco antigo.",
          "vi": "Việc tình cờ tìm thấy cuộn sách luật pháp bị lãng quên trong lúc sửa đền thờ cũng giống như cảm giác tìm thấy cuốn sổ tiết kiệm cũ trong lúc dọn nhà vậy.",
          "tl": "Ang aksidenteng pagkakadiskubre sa nakalimutang aklat ng kautusan habang naglilinis ay parang paghahanap ng nawawalang gamit sa ilalim ng kama na matagal mo nang hinahanap.",
          "th": "การบังเอิญไปเจอหนังสือธรรมบัญญัติที่ถูกลืมไว้ตอนซ่อมพระวิหาร ก็เหมือนกับการจัดบ้านแล้วไปเจอสมุดบัญชีเงินฝากเก่าที่ลืมไปแล้วว่ามีอยู่",
          "sw": "Kupata kitabu cha sheria kilichosahaulika wakati wa ukarabati ni kama kugundua pesa ulizozisahau kwenye koti la zamani wakati wa kufanya usafi.",
          "it": "Ritrovare un vecchio rotolo della legge dimenticato durante i lavori di ristrutturazione è un po' come trovare l'estratto conto della banca dimenticato tra le vecchie ricevute durante le pulizie di primavera."
        }
    },
    "2026-07-18": {
        date: "2026-07-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 23:31-37",
          "en": "2 Kings 23:31-37",
          "ko": "2 Kings 23:31-37",
          "zho": "列王紀下 23:31-37",
          "es": "2 Kings 23:31-37",
          "pt": "2 Kings 23:31-37",
          "vi": "2 Kings 23:31-37",
          "tl": "2 Kings 23:31-37",
          "th": "2 Kings 23:31-37",
          "sw": "2 Kings 23:31-37",
          "it": "2 Re 23:31-37"
        },
        comment: {
          "ja": "たった数ヶ月で王位を追われるエホアハズを見ていると、現代の激務な職場での異動や短期プロジェクトの儚さを突きつけられているようで胃が痛くなる。",
          "en": "Watching Jehoahaz get ousted as king after only three months is a brutal reminder of how quickly some professional careers can crash and burn.",
          "ko": "겨우 석 달 만에 왕위에서 쫓겨난 여호아하스를 보면 요즘 시대의 짧은 계약직이나 불안정한 사회생활의 비애가 떠올라 왠지 짠하다.",
          "zho": "看到约哈斯当了三个月国王就被废黜，这简直是职场上那些还没适应岗位就被调走或裁员的悲剧缩影。",
          "es": "Ver a Joacaz perder su trono tras solo tres meses es un recordatorio bastante cruel de lo rápido que puede cambiar el panorama en cualquier trabajo.",
          "pt": "Ver Jeoacaz perder o trono após apenas três meses é um lembrete brutal de como as coisas podem dar errado rapidamente em qualquer cargo de responsabilidade.",
          "vi": "Nhìn cảnh vua Giô-a-cha bị phế truất chỉ sau ba tháng cầm quyền, thật khó mà không liên tưởng đến những công việc ngắn hạn đầy áp lực trong thời đại ngày nay.",
          "tl": "Ang makitang mapatalsik si Jehoahaz matapos ang tatlong buwan ay parang isang brutal na paalala kung gaano kabilis magbago ang takbo ng buhay sa trabaho.",
          "th": "การเห็นเยโฮอาหาสถูกถอดจากตำแหน่งกษัตริย์หลังครองราชย์ได้แค่สามเดือน ช่างเป็นภาพสะท้อนความไม่แน่นอนของตำแหน่งงานยุคปัจจุบันที่น่าใจหาย",
          "sw": "Kuona Jehoahazi akiondolewa madarakani baada ya miezi mitatu tu ni ukumbusho mchungu wa jinsi mambo yanavyoweza kubadilika haraka kazini.",
          "it": "Vedere Ioacaz spodestato dopo appena tre mesi è un promemoria brutale di quanto velocemente possano cambiare le cose in certi ambienti lavorativi."
        }
    },
    "2026-07-19": {
        date: "2026-07-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "列王紀下 25:1-30",
          "en": "2 Kings 25:1-30",
          "ko": "2 Kings 25:1-30",
          "zho": "列王紀下 25:1-30",
          "es": "2 Kings 25:1-30",
          "pt": "2 Kings 25:1-30",
          "vi": "2 Kings 25:1-30",
          "tl": "2 Kings 25:1-30",
          "th": "2 Kings 25:1-30",
          "sw": "2 Kings 25:1-30",
          "it": "2 Re 25:1-30"
        },
        comment: {
          "ja": "王様が捕まって目をくり抜かれるという結末はあまりに無慈悲だけれど、国が滅びるまで現実逃避し続けた末の光景だと思うと背筋が凍る。",
          "en": "The king being captured and blinded is brutal, but it is a chilling reminder of what happens when you spend your whole reign ignoring the writing on the wall.",
          "ko": "왕이 눈이 뽑히는 비극적인 결말을 맞이하기 전까지 끝없이 현실을 부정했던 모습은 우리네 고집스러운 일상을 떠올리게 한다.",
          "zho": "國王最後被俘且雙眼被剜的慘狀固然殘酷，但這也是長久以來選擇性無視現實所必須付出的代價。",
          "es": "El trágico final del rey es brutal, pero es el recordatorio perfecto de lo que pasa cuando ignoramos las advertencias hasta que ya no hay vuelta atrás.",
          "pt": "O fim trágico do rei é brutal, mas serve como um lembrete do que acontece quando passamos tempo demais ignorando os sinais de alerta.",
          "vi": "Cái kết bị bắt và chọc mù mắt của vị vua thật tàn khốc, nhưng đó là kết quả tất yếu của việc cố chấp trốn tránh thực tại quá lâu.",
          "tl": "Ang malagim na wakas ng hari ay brutal, pero paalala ito na ang pagbalewala sa mga babala ay humahantong sa mga desisyong hindi na mababawi.",
          "th": "จุดจบที่แสนโหดร้ายของกษัตริย์เป็นเครื่องเตือนใจว่าการเพิกเฉยต่อความจริงมาตลอดชีวิตนั้นต้องแลกด้วยราคาที่แพงมหาศาล",
          "sw": "Mwisho wa mfalme ni wa kutisha, lakini ni ukumbusho kwamba kupuuza ukweli kwa muda mrefu kuna gharama zake kubwa.",
          "it": "La fine tragica del re è brutale, ma fa gelare il sangue pensare a cosa succede quando si ignorano tutti i segnali di allarme fino a non avere più via d'uscita."
        }
    },
    "2026-07-20": {
        date: "2026-07-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "歴代志下 14:1-15",
          "en": "2 Chronicles 14:1-15",
          "ko": "2 Chronicles 14:1-15",
          "zho": "歴代志下 14:1-15",
          "es": "2 Chronicles 14:1-15",
          "pt": "2 Chronicles 14:1-15",
          "vi": "2 Chronicles 14:1-15",
          "tl": "2 Chronicles 14:1-15",
          "th": "2 Chronicles 14:1-15",
          "sw": "2 Chronicles 14:1-15",
          "it": "2 Cronache 14:1-15"
        },
        comment: {
          "ja": "圧倒的な敵を前にして神様を頼るアサ王の姿はかっこいいけれど、その後の慢心で足が病むまで神に頼らなくなった展開が人間臭すぎて切ない。",
          "en": "King Asa starts off strong by leaning on God against impossible odds, only to stop doing so when things get comfortable, which is a classic human trajectory.",
          "ko": "거대한 적 앞에서 하나님을 의지하던 아사 왕이 상황이 좀 나아지자 마음이 변해버리는 모습은 너무나도 인간적이라 씁쓸하다.",
          "zho": "亞撒王起初面對強敵時展現的信心很帥氣，但後來因驕傲而不再尋求神，這種心態變化實在太像普通人了。",
          "es": "El rey Asa empieza confiando en Dios ante lo imposible, pero en cuanto las cosas van bien, se olvida de Él; un error humano tan común como doloroso.",
          "pt": "O rei Asa começa confiando em Deus diante do impossível, mas assim que as coisas melhoram, ele se esquece de quem o ajudou, um erro humano muito comum.",
          "vi": "Vua A-sa khởi đầu đầy đức tin khi đối mặt với kẻ thù, nhưng rồi lại quên mất Chúa khi mọi thứ trở nên suôn sẻ, một sự thay đổi tâm lý rất đỗi con người.",
          "tl": "Si Haring Asa ay nagsimula nang may pananalig sa harap ng panganib, ngunit nakakalungkot na kinalimutan niya ang Diyos nang guminhawa na ang buhay.",
          "th": "กษัตริย์อาสาเริ่มต้นด้วยการพึ่งพาพระเจ้าในยามวิกฤต แต่พอชีวิตเริ่มสบายก็กลับลืมพระองค์ไปเสียสนิท ซึ่งช่างเป็นนิสัยมนุษย์ที่คุ้นเคยเหลือเกิน",
          "sw": "Mfalme Asa alianza kwa kumtegemea Mungu dhidi ya maadui, lakini alipoanza kufanikiwa akamsahau Mungu, jambo ambalo ni kosa la kawaida la kibinadamu.",
          "it": "Il re Asa che si affida a Dio davanti all'impossibile è ammirevole, ma smettere di farlo appena le cose vanno bene è un errore umano tanto comune quanto doloroso."
        }
    },
    "2026-07-21": {
        date: "2026-07-21",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "歴代志下 16:1-14",
          "en": "2 Chronicles 16:1-14",
          "ko": "2 Chronicles 16:1-14",
          "zho": "歴代志下 16:1-14",
          "es": "2 Chronicles 16:1-14",
          "pt": "2 Chronicles 16:1-14",
          "vi": "2 Chronicles 16:1-14",
          "tl": "2 Chronicles 16:1-14",
          "th": "2 Chronicles 16:1-14",
          "sw": "2 Chronicles 16:1-14",
          "it": "2 Cronache 16:1-14"
        },
        comment: {
          "ja": "神様を頼りにしていたアサ王が晩年になって急に外交と賄賂に頼りだす姿、成功体験に甘えて初心を忘れる人間の脆さを突きつけられている気分だ。",
          "en": "King Asa going from relying on God to relying on bribes in his later years is a brutal reminder of how easily we trade our principles for quick fixes.",
          "ko": "하나님을 의지하던 아사 왕이 말년에 뇌물과 외교에 의존하기 시작하는 모습은, 성공한 뒤 초심을 잃어버리는 인간의 나약함을 그대로 보여준다.",
          "zho": "亞撒王晚年不再依靠神，轉而依賴賄賂和外交手段，這簡直是成功後容易忘記初衷的人性寫照。",
          "es": "Ver al rey Asa pasar de confiar en Dios a depender de sobornos al final de su vida es un recordatorio incómodo de lo fácil que es perder la perspectiva con los años.",
          "pt": "O rei Asa trocando sua confiança em Deus por subornos e diplomacia na velhice é um lembrete doloroso de como é fácil esquecer nossas raízes quando as coisas ficam difíceis.",
          "vi": "Vua A-sa từ bỏ niềm tin vào Chúa để cậy nhờ vào hối lộ và ngoại giao lúc về già là minh chứng rõ nhất cho việc con người dễ đánh mất bản chất khi áp lực ập đến.",
          "tl": "Ang paglipat ni Haring Asa mula sa pagtitiwala sa Diyos patungo sa paggamit ng suhol at diplomasya ay isang masakit na paalala kung gaano kadaling makalimot sa prinsipyo kapag tayo ay tumatanda.",
          "th": "กษัตริย์อาสาที่เคยพึ่งพาพระเจ้ากลับหันไปพึ่งพาการติดสินบนในบั้นปลายชีวิต เป็นภาพสะท้อนที่ชัดเจนว่าเมื่อเราประสบความสำเร็จเรามักจะลืมว่าอะไรคือสิ่งสำคัญที่สุด",
          "sw": "Mfalme Asa kuacha kumtegemea Mungu na kuanza kutegemea rushwa na diplomasia katika uzee wake ni ukumbusho mchungu jinsi ilivyo rahisi kupoteza mwelekeo tunapozeeka.",
          "it": "Vedere il re Asa passare dal confidare in Dio a ricorrere ai favori negli ultimi anni di vita è un promemoria scomodo di quanto sia facile perdere i propri principi con il tempo."
        }
    },
    "2026-07-22": {
        date: "2026-07-22",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "歴代志下 17:1-19",
          "en": "2 Chronicles 17:1-19",
          "ko": "2 Chronicles 17:1-19",
          "zho": "歴代志下 17:1-19",
          "es": "2 Chronicles 17:1-19",
          "pt": "2 Chronicles 17:1-19",
          "vi": "2 Chronicles 17:1-19",
          "tl": "2 Chronicles 17:1-19",
          "th": "2 Chronicles 17:1-19",
          "sw": "2 Chronicles 17:1-19",
          "it": "2 Cronache 17:1-19"
        },
        comment: {
          "ja": "ヨシャパテが律法の本を持って国中を回らせたのは、現代で言えばインフルエンサーが正しい情報を拡散してバズらせるようなものだが、その影響力は桁違いだ。",
          "en": "Jehoshaphat sending teachers across the land with the law is basically the ancient version of a viral campaign for truth, and honestly, we could use more of that energy.",
          "ko": "여호사밧이 율법책을 들고 전국을 순회하게 한 것은 현대의 인플루언서가 올바른 정보를 확산시키는 것과 다름없는데, 그 파급력은 차원이 다르다.",
          "zho": "約沙法派人帶著律法書巡迴全國教導百姓，這簡直是古代版的真相推廣活動，其影響力遠勝於現代任何網紅。",
          "es": "Que Josafat enviara a gente a enseñar la ley por todo el país es básicamente la versión antigua de una campaña viral de valores, y vaya que sí funcionó.",
          "pt": "Josafá enviando mestres com o livro da lei por toda a terra é basicamente a versão antiga de uma campanha viral de impacto real, e precisamos muito disso hoje.",
          "vi": "Việc Giô-sa-phát cử người mang sách luật đi khắp nơi dạy dỗ dân chúng chẳng khác nào một chiến dịch lan tỏa giá trị cốt lõi, thứ mà thời nay hiếm thấy.",
          "tl": "Ang pagpapadala ni Josaphat ng mga guro sa buong lupain para ituro ang batas ay parang sinaunang bersyon ng isang viral campaign para sa katotohanan, at kailangan natin ang ganyang klaseng impluwensya ngayon.",
          "th": "การที่เยโฮชาฟัทส่งคนไปสอนธรรมบัญญัติทั่วแผ่นดินก็ไม่ต่างจากการทำแคมเปญสร้างสรรค์สังคมให้เป็นกระแส และผลลัพธ์ที่ได้นั้นทรงพลังยิ่งกว่าการตลาดสมัยไหนๆ",
          "sw": "Yehoshafati kutuma walimu na kitabu cha sheria nchi nzima ni kama kampeni ya kisasa ya kusambaza ukweli kwa ufanisi, na kwa kweli tungependa kuona mambo kama hayo yakitokea leo.",
          "it": "Giosafat che manda maestri a insegnare la legge in tutto il paese è fondamentalmente la versione antica di una campagna virale di valori, e onore al merito per aver funzionato."
        }
    },
    "2026-07-23": {
        date: "2026-07-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "歴代志下 18:1-34",
          "en": "2 Chronicles 18:1-34",
          "ko": "2 Chronicles 18:1-34",
          "zho": "歴代志下 18:1-34",
          "es": "2 Chronicles 18:1-34",
          "pt": "2 Chronicles 18:1-34",
          "vi": "2 Chronicles 18:1-34",
          "tl": "2 Chronicles 18:1-34",
          "th": "2 Chronicles 18:1-34",
          "sw": "2 Chronicles 18:1-34",
          "it": "2 Cronache 18:1-34"
        },
        comment: {
          "ja": "変装して戦場に紛れ込めばバレないと信じたアハブの甘い期待と、結局流れ矢に当たるという人生の皮肉、もはやコントを見ている気分になる。",
          "en": "Ahab thinking a simple disguise would keep him safe from destiny only to be taken out by a random stray arrow is the ultimate reminder that you cannot outrun the inevitable.",
          "ko": "변장을 하면 전장에서 무사할 거라 믿었던 아합의 순진한 착각과 결국 화살 한 방에 무너지는 결말은 인생이 얼마나 마음대로 안 되는지 보여준다.",
          "zho": "亞哈王以為換身裝扮就能躲過災禍，結果卻被流箭射中，這完美的詮釋了什麼叫躲得過初一躲不過十五。",
          "es": "Ahab creyendo que con un simple disfraz pasaría desapercibido en plena batalla, solo para ser alcanzado por una flecha perdida, es la prueba de que no puedes esconderte de tu propio destino.",
          "pt": "Acabe achando que um simples disfarce o protegeria no meio da batalha, apenas para ser atingido por uma flecha perdida, é a prova final de que não dá para fugir do inevitável.",
          "vi": "Vua A-háp nghĩ rằng cải trang là có thể an toàn trong trận chiến, để rồi bị một mũi tên lạc kết liễu, một bài học đắt giá về việc đừng bao giờ cố đánh lừa định mệnh.",
          "tl": "Ang akala ni Ahab na ligtas siya sa labanan dahil lang sa pagpapalit ng damit, hanggang sa tamaan siya ng isang ligaw na palaso, ay patunay na hindi mo matatakasan ang tadhana.",
          "th": "อาหับคิดว่าแค่ปลอมตัวไปรบก็คงรอด แต่สุดท้ายก็มาตายเพราะธนูหลงทาง เป็นบทเรียนที่บอกว่าเราหนีความจริงไม่พ้นจริงๆ",
          "sw": "Ahab kudhani kuwa kujificha kutamfanya awe salama vitani, kisha kupigwa na mshale wa bahati mbaya, ni ukumbusho kuwa huwezi kukimbia hatima yako.",
          "it": "Acab che pensa di passare inosservato in battaglia con un travestimento da quattro soldi, solo per beccarsi una freccia persa, è la prova che non puoi sfuggire al tuo destino."
        }
    },
    "2026-07-24": {
        date: "2026-07-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "歴代志下 20:1-37",
          "en": "2 Chronicles 20:1-37",
          "ko": "2 Chronicles 20:1-37",
          "zho": "歴代志下 20:1-37",
          "es": "2 Chronicles 20:1-37",
          "pt": "2 Chronicles 20:1-37",
          "vi": "2 Chronicles 20:1-37",
          "tl": "2 Chronicles 20:1-37",
          "th": "2 Chronicles 20:1-37",
          "sw": "2 Chronicles 20:1-37",
          "it": "2 Cronache 20:1-37"
        },
        comment: {
          "ja": "敵の大軍を前にして武器ではなく聖歌隊を先頭に立たせるヨシャパトの作戦、常識外れすぎて逆に最強のメンタルケアに見えてくる。",
          "en": "Sending a choir to the front lines instead of an army is the kind of bold, irrational faith that makes total sense only when everything is already falling apart.",
          "ko": "군대 대신 찬양대를 앞세워 적진으로 나아가는 여호사밧의 전략은 너무 비상식적이라 오히려 그 대담함에 할 말을 잃게 만든다.",
          "zho": "面對大軍壓境，約沙法不派軍隊卻派詩班打頭陣，這種不按牌理出牌的信仰，簡直是把心理戰術玩到了極致。",
          "es": "Josafat enviando al coro al frente de batalla en lugar del ejército es el tipo de estrategia arriesgada y absurda que solo tiene sentido cuando ya no tienes nada más que perder.",
          "pt": "Josafá enviando o coral para a linha de frente em vez do exército é o tipo de estratégia audaz e absurda que só faz sentido quando você percebe que a fé é a única arma que resta.",
          "vi": "Việc Giô-sa-phát cử đội hát thánh ca lên hàng đầu thay vì quân đội là một chiến thuật kỳ lạ đến mức khiến người ta phải nể phục sự liều lĩnh đầy đức tin của ông.",
          "tl": "Ang pagpapadala ni Jehosapat sa koro sa harapan ng labanan imbes na sa mga sundalo ay isang kakaibang diskarte na nagpapakita na minsan, pananampalataya lang ang tanging armas na kailangan.",
          "th": "เยโฮชาฟัทส่งนักร้องประสานเสียงไปหน้ากองทัพแทนที่จะเป็นทหาร เป็นกลยุทธ์ที่แปลกประหลาดจนน่าทึ่งว่าความเชื่อมั่นมันทรงพลังได้ขนาดนี้เลยหรือ",
          "sw": "Yehoshafati kutuma kwaya mstari wa mbele badala ya jeshi ni mkakati wa ajabu ambao unaonyesha kuwa wakati mwingine imani ndiyo silaha pekee inayohitajika.",
          "it": "Mandare un coro in prima linea invece dell'esercito è quel tipo di mossa folle e audace che ha senso solo quando ormai non hai più nulla da perdere."
        }
    },
    "2026-07-25": {
        date: "2026-07-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "歴代志下 26:1-23",
          "en": "2 Chronicles 26:1-23",
          "ko": "2 Chronicles 26:1-23",
          "zho": "歴代志下 26:1-23",
          "es": "2 Chronicles 26:1-23",
          "pt": "2 Chronicles 26:1-23",
          "vi": "2 Chronicles 26:1-23",
          "tl": "2 Chronicles 26:1-23",
          "th": "2 Chronicles 26:1-23",
          "sw": "2 Chronicles 26:1-23",
          "it": "2 Cronache 26:1-23"
        },
        comment: {
          "ja": "成功して調子に乗った途端に分をわきまえられなくなるウジヤ王、どんなに有能でも最後は謙虚さという装備を外してはいけないという教訓。",
          "en": "Uzziah acing everything and then letting the success go to his head is a classic reminder that the hardest part of winning is staying humble.",
          "ko": "성공 가도를 달리다 결국 자제력을 잃고 만 웃시야 왕을 보면, 잘나갈 때일수록 겸손이라는 안전벨트가 얼마나 중요한지 실감하게 됩니다.",
          "zho": "烏西雅王在成功後便開始得意忘形，這提醒我們無論多有才華，保持謙卑才是最難的修煉。",
          "es": "El rey Uzías logrando todo y luego arruinándolo por orgullo es el recordatorio perfecto de que el éxito suele ser más peligroso que el fracaso.",
          "pt": "O rei Uzias conquistando tudo e depois perdendo a noção por causa do orgulho é o lembrete perfeito de que o sucesso pode ser mais perigoso que o fracasso.",
          "vi": "Vua U-si-gia thành công mọi việc rồi lại tự mãn đến mức đánh mất mình, một bài học đắt giá rằng giữ được sự khiêm nhường còn khó hơn nhiều so với việc giành chiến thắng.",
          "tl": "Si Haring Uzias na nagtagumpay sa lahat pero kinalaunan ay naging mayabang ay isang paalala na mas mahirap manatiling mapagkumbaba kaysa sa magtagumpay.",
          "th": "กษัตริย์อุสซียาห์ที่ทำทุกอย่างสำเร็จจนเหลิงไปกับอำนาจ เตือนให้เรารู้ว่าความสำเร็จที่ปราศจากความถ่อมตนนั้นน่ากลัวแค่ไหน",
          "sw": "Mfalme Uzia kufanikiwa katika kila kitu kisha kulewa sifa ni ukumbusho kwamba sehemu ngumu zaidi ya kushinda ni kubaki mnyenyekevu.",
          "it": "Il re Ozzia che si fa dare alla testa dal successo fino a perdere la testa è la dimostrazione che la parte più difficile di una vittoria è rimanere con i piedi per terra."
        }
    },
    "2026-07-26": {
        date: "2026-07-26",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "歴代志下 30:1-27",
          "en": "2 Chronicles 30:1-27",
          "ko": "2 Chronicles 30:1-27",
          "zho": "歴代志下 30:1-27",
          "es": "2 Chronicles 30:1-27",
          "pt": "2 Chronicles 30:1-27",
          "vi": "2 Chronicles 30:1-27",
          "tl": "2 Chronicles 30:1-27",
          "th": "2 Chronicles 30:1-27",
          "sw": "2 Chronicles 30:1-27",
          "it": "2 Cronache 30:1-27"
        },
        comment: {
          "ja": "予定より遅れても、形式にこだわらずにみんなで集まることを優先したヒゼキヤの柔軟さ、完璧主義より大切なことがあると教えてくれる。",
          "en": "Hezekiah prioritizing togetherness over strict scheduling shows that sometimes doing it late is still better than not doing it at all.",
          "ko": "일정이 늦어졌음에도 형식보다 모임을 강행한 히스기야의 유연함은 완벽주의보다 관계가 훨씬 중요하다는 것을 보여줍니다.",
          "zho": "希西家為了讓大家團聚而打破僵化的時間表，這證明了有時候心意比準時更加重要。",
          "es": "Hizequías priorizando la unidad sobre el calendario perfecto nos enseña que a veces es mejor avanzar juntos, aunque sea un poco tarde.",
          "pt": "Ezequias priorizando a união em vez de um calendário perfeito nos ensina que, às vezes, fazer algo um pouco atrasado é muito melhor do que não fazer.",
          "vi": "Ê-xê-chia ưu tiên sự gắn kết thay vì cứng nhắc theo lịch trình cho thấy đôi khi việc làm muộn vẫn tốt hơn là không làm gì cả.",
          "tl": "Ang pag-una ni Hezekias sa pagsasama kaysa sa mahigpit na iskedyul ay nagtuturo sa atin na minsan, mas mahalaga ang intensyon kaysa sa pagiging perpekto.",
          "th": "กษัตริย์เฮเซคียาห์ที่ยอมยืดหยุ่นเรื่องเวลาเพื่อให้ทุกคนได้มารวมตัวกัน สอนให้รู้ว่าความรักและความสามัคคีสำคัญกว่าการทำตามกฎเป๊ะๆ",
          "sw": "Hezekia kuweka umoja mbele badala ya ratiba ngumu inatufundisha kuwa wakati mwingine kufanya mambo kwa kuchelewa ni bora kuliko kutoyafanya kabisa.",
          "it": "Ezechia che mette la comunità davanti alla rigida tabella di marcia ci ricorda che a volte fare le cose con calma è comunque meglio che non farle affatto."
        }
    },
    "2026-07-27": {
        date: "2026-07-27",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エズラ記 1:1-11",
          "en": "Ezra 1:1-11",
          "ko": "Ezra 1:1-11",
          "zho": "エズラ記 1:1-11",
          "es": "Ezra 1:1-11",
          "pt": "Ezra 1:1-11",
          "vi": "Ezra 1:1-11",
          "tl": "Ezra 1:1-11",
          "th": "Ezra 1:1-11",
          "sw": "Ezra 1:1-11",
          "it": "Esdra 1:1-11"
        },
        comment: {
          "ja": "異国の王様が突然神様の感動で帰還を命じるなんて、人生の急展開すぎてついていくのが大変そう。",
          "en": "A foreign king suddenly getting a divine nudge to send everyone home is the kind of plot twist that makes you question your life plan.",
          "ko": "이방 나라 왕이 갑자기 하나님의 감동으로 귀환을 명령하다니 인생의 급전개란 이런 건가 싶네요.",
          "zho": "外邦君王突然受神感動下令讓百姓回歸，這種人生神展開真的讓人措手不及。",
          "es": "Que un rey extranjero reciba una revelación divina para enviarte de vuelta a casa es el giro de guion más inesperado de la historia.",
          "pt": "Um rei estrangeiro ser subitamente movido por Deus para te mandar de volta para casa é o tipo de reviravolta que ninguém planeja.",
          "vi": "Được một vị vua ngoại bang đột ngột ra lệnh cho phép trở về nhờ sự cảm động từ Chúa là một cú bẻ lái cuộc đời không ai ngờ tới.",
          "tl": "Ang biglaang pag-utos ng isang dayuhang hari na pauwiin kayo dahil sa utos ng Diyos ay isang plot twist na hindi mo mahuhulaan.",
          "th": "จู่ๆ กษัตริย์ต่างแดนก็ได้รับดลใจให้ปล่อยตัวกลับบ้าน นี่มันคือจุดเปลี่ยนชีวิตแบบที่ไม่มีใครคาดคิดมาก่อนเลยจริงๆ",
          "sw": "Mfalme wa kigeni kuamua ghafla kukuacha urudi nyumbani kwa sababu ya msukumo wa Mungu ni jambo la kushangaza sana maishani.",
          "it": "Un re straniero che riceve una spinta divina per rispedire tutti a casa è il colpo di scena che ribalta completamente ogni piano."
        }
    },
    "2026-07-28": {
        date: "2026-07-28",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エズラ記 4:1-24",
          "en": "Ezra 4:1-24",
          "ko": "Ezra 4:1-24",
          "zho": "エズラ記 4:1-24",
          "es": "Ezra 4:1-24",
          "pt": "Ezra 4:1-24",
          "vi": "Ezra 4:1-24",
          "tl": "Ezra 4:1-24",
          "th": "Ezra 4:1-24",
          "sw": "Ezra 4:1-24",
          "it": "Esdra 4:1-24"
        },
        comment: {
          "ja": "手伝うふりをして実は邪魔をする、という現代の職場で一番遭遇したくないタイプの人間関係がエズラ記にもあったとは。",
          "en": "Finding out that the ancient Israelites had to deal with fake allies who pretend to help while actually sabotaging them is just too relatable.",
          "ko": "도와주는 척하면서 은근슬쩍 방해하는 사람들을 성경 시대에도 겪어야 했다니 인간 관계는 예나 지금이나 똑같네요.",
          "zho": "原來以斯拉時代就已經有那種假裝幫忙、背地裡卻搞破壞的偽君子，這種職場生態真是跨越千年都不變。",
          "es": "Descubrir que en tiempos de Esdras ya lidiaban con falsos aliados que fingen ayudar mientras sabotean el trabajo es demasiado real.",
          "pt": "Saber que o povo de Esdras já lidava com gente que finge ajudar para sabotar o projeto é um lembrete de que certas coisas nunca mudam.",
          "vi": "Hóa ra thời của Ê-xơ-ra đã có những người giả vờ giúp đỡ nhưng thực chất là phá hoại, đúng là kiểu người mà ai cũng từng gặp trong đời.",
          "tl": "Ang makita na noon pa man ay may mga taong nagkukunwaring tumutulong pero nanggugulo lang ay patunay na hindi nagbabago ang ugali ng tao.",
          "th": "การที่ต้องรับมือกับพวกแสร้งทำเป็นหวังดีแต่จริงๆ แล้วจ้องจะขัดขวางตั้งแต่ยุคเอสรานี่มันเป็นปัญหาคลาสสิกจริงๆ",
          "sw": "Kugundua kuwa hata wakati wa Ezra watu walilazimika kukabiliana na watu wanaojifanya kusaidia huku wakihujumu kazi ni jambo linaloeleweka sana.",
          "it": "Scoprire che già gli antichi israeliti dovevano vedersela con falsi alleati pronti a sabotare tutto è fin troppo familiare."
        }
    },
    "2026-07-29": {
        date: "2026-07-29",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エズラ記 6:1-22",
          "en": "Ezra 6:1-22",
          "ko": "Ezra 6:1-22",
          "zho": "エズラ記 6:1-22",
          "es": "Ezra 6:1-22",
          "pt": "Ezra 6:1-22",
          "vi": "Ezra 6:1-22",
          "tl": "Ezra 6:1-22",
          "th": "Ezra 6:1-22",
          "sw": "Ezra 6:1-22",
          "it": "Esdra 6:1-22"
        },
        comment: {
          "ja": "何年も放置していた神殿建設が、王様の一言で急に予算が降りて再開する様子は、まさに締め切り直前の現場のドタバタそのもの。",
          "en": "The temple construction sitting idle for years only to suddenly get funding once the king says so is the ultimate example of a project waiting for the deadline to hit.",
          "ko": "몇 년 동안 방치되던 성전 건축이 왕의 말 한마디에 갑자기 예산이 내려와 재개되는 모습은 마감 직전의 현장을 보는 듯하다.",
          "zho": "荒废多年的圣殿工程因为国王的一句话突然拨下预算并重启，简直就是典型的拖延症遇上最后通牒的真实写照。",
          "es": "Ver cómo el proyecto del templo, estancado por años, avanza de repente solo porque el rey dio la orden, es la prueba de que a veces solo necesitamos un empujón desde arriba.",
          "pt": "Ver o projeto do templo, parado por anos, ser subitamente retomado só porque o rei deu uma ordem, é a prova de que às vezes só precisamos de um empurrão vindo de cima.",
          "vi": "Việc xây dựng đền thờ bị trì hoãn nhiều năm rồi đột ngột được tái khởi động ngay khi nhà vua ra lệnh giống hệt như cảnh chạy deadline vào phút cuối.",
          "tl": "Ang pagpapatuloy ng konstruksyon ng templo matapos ang ilang taong pagkakaantala dahil lang sa utos ng hari ay parang pagmamadali sa deadline ng isang proyekto.",
          "th": "การที่งานสร้างพระวิหารที่ถูกทิ้งร้างมานานกลับมาคึกคักทันทีเมื่อกษัตริย์สั่งให้ทำต่อนั้น ช่างเหมือนกับการเร่งงานให้เสร็จก่อนเดดไลน์ไม่มีผิด",
          "sw": "Ujenzi wa hekalu uliokuwa umesimama kwa miaka mingi kuanza tena kwa amri ya mfalme ni mfano kamili wa jinsi tunavyosubiri mpaka dakika ya mwisho ili kumaliza mambo.",
          "it": "Il tempio lasciato in sospeso per anni che riceve fondi all'improvviso solo perché il capo lo ordina è il classico progetto salvato dalla scadenza imminente."
        }
    },
    "2026-07-30": {
        date: "2026-07-30",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ネヘミヤ記 2:1-20",
          "en": "Nehemiah 2:1-20",
          "ko": "Nehemiah 2:1-20",
          "zho": "ネヘミヤ記 2:1-20",
          "es": "Nehemiah 2:1-20",
          "pt": "Nehemiah 2:1-20",
          "vi": "Nehemiah 2:1-20",
          "tl": "Nehemiah 2:1-20",
          "th": "Nehemiah 2:1-20",
          "sw": "Nehemiah 2:1-20",
          "it": "Neemia 2:1-20"
        },
        comment: {
          "ja": "王の前で緊張して顔色が悪いことを隠しきれないネヘミヤ、どんなに準備しても結局最後は素の自分が漏れ出てしまう人間味が愛おしい。",
          "en": "Nehemiah failing to hide his anxiety before the king is a reminder that no matter how much you prepare, your face will eventually give away what is really on your mind.",
          "ko": "왕 앞에서 긴장한 기색을 숨기지 못하는 느헤미야를 보니, 아무리 철저히 준비해도 결국 얼굴에 다 드러나는 인간적인 모습이 참 안쓰럽고도 친근하다.",
          "zho": "尼希米在国王面前藏不住紧张的神情，这提醒我们无论做了多少心理建设，脸上的表情总会诚实地出卖内心的真实想法。",
          "es": "Nehemías tratando de ocultar su ansiedad ante el rey nos recuerda que, por mucho que nos preparemos, nuestras expresiones siempre terminan delatando lo que sentimos.",
          "pt": "Neemias tentando esconder sua ansiedade diante do rei nos lembra que, por mais que nos preparemos, nossas expressões sempre acabam entregando o que sentimos.",
          "vi": "Việc Nê-hê-mi không thể giấu nổi vẻ lo lắng trước mặt nhà vua nhắc nhở chúng ta rằng dù chuẩn bị kỹ đến đâu, gương mặt vẫn luôn phản bội lại suy nghĩ thật của mình.",
          "tl": "Ang hindi pagtatago ni Nehemias sa kanyang kaba sa harap ng hari ay nagpapaalala na kahit gaano tayo maghanda, palaging mababasa sa ating mukha ang tunay nating nararamdaman.",
          "th": "การที่เนหะมีย์ปิดบังความกังวลต่อหน้ากษัตริย์ไม่ได้เลย ทำให้รู้ว่าไม่ว่าเราจะเตรียมตัวมาดีแค่ไหน สีหน้าก็มักจะฟ้องความรู้สึกข้างในอยู่ดี",
          "sw": "Nehemia kushindwa kuficha wasiwasi wake mbele ya mfalme ni ukumbusho kwamba hata uwe umejiandaa vipi, uso wako utaonyesha kile kilichomo moyoni mwako.",
          "it": "Neemia che non riesce a nascondere l'ansia davanti al re dimostra che, per quanto ti impegni, la faccia ti tradisce sempre."
        }
    },
    "2026-07-31": {
        date: "2026-07-31",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ネヘミヤ記 4:7-23",
          "en": "Nehemiah 4:7-23",
          "ko": "Nehemiah 4:7-23",
          "zho": "ネヘミヤ記 4:7-23",
          "es": "Nehemiah 4:7-23",
          "pt": "Nehemiah 4:7-23",
          "vi": "Nehemiah 4:7-23",
          "tl": "Nehemiah 4:7-23",
          "th": "Nehemiah 4:7-23",
          "sw": "Nehemiah 4:7-23",
          "it": "Neemia 4:7-23"
        },
        comment: {
          "ja": "片手に武器を持ちもう片手でレンガを積むネヘミヤの姿は、締め切りに追われながら来るメールを捌く現代のマルチタスクの極み。",
          "en": "Nehemiah building a wall with one hand and holding a weapon with the other is the ultimate biblical version of multitasking through a deadline.",
          "ko": "한 손에는 무기를 들고 다른 한 손으로는 성벽을 쌓는 느헤미야의 모습은 마감에 쫓기며 메일을 처리하는 현대인의 멀티태스킹 그 자체입니다.",
          "zho": "尼希米一手拿兵器一手砌牆，這簡直就是現代人在截稿壓力下還要同時回覆訊息的極限操作。",
          "es": "Nehemías construyendo un muro con una mano y sosteniendo un arma con la otra es la definición bíblica de trabajar bajo presión extrema.",
          "pt": "Neemias construindo um muro com uma mão e segurando uma arma com a outra é a definição bíblica de lidar com prazos apertados.",
          "vi": "Việc Nê-hê-mi vừa xây tường vừa cầm vũ khí chính là hình ảnh đỉnh cao của việc đa nhiệm khi bị áp lực công việc bủa vây.",
          "tl": "Ang pagtatayo ni Nehemias ng pader habang may hawak na armas ay ang biblikal na bersyon ng pagtatrabaho sa gitna ng matinding pressure.",
          "th": "เนฮามีย์ที่สร้างกำแพงไปถืออาวุธไป คือภาพสะท้อนของการทำงานแบบมัลติทาสก์ในยุคปัจจุบันที่ต้องรับมือกับเดดไลน์ไปพร้อมกับป้องกันตัว",
          "sw": "Nehemia kujenga ukuta kwa mkono mmoja na kushika silaha kwa mwingine ni mfano halisi wa kufanya kazi nyingi kwa wakati mmoja chini ya shinikizo.",
          "it": "Neemia che costruisce un muro con una mano e tiene un'arma con l'altra è la versione biblica del multitasking sotto scadenza."
        }
    },
    "2026-08-01": {
        date: "2026-08-01",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ネヘミヤ記 6:1-19",
          "en": "Nehemiah 6:1-19",
          "ko": "Nehemiah 6:1-19",
          "zho": "ネヘミヤ記 6:1-19",
          "es": "Nehemiah 6:1-19",
          "pt": "Nehemiah 6:1-19",
          "vi": "Nehemiah 6:1-19",
          "tl": "Nehemiah 6:1-19",
          "th": "Nehemiah 6:1-19",
          "sw": "Nehemiah 6:1-19",
          "it": "Neemia 6:1-19"
        },
        comment: {
          "ja": "忙しいからと面会を断り続けるネヘミヤは、面倒な会議の誘いを丁重に断り続ける現代の社畜の鏡である。",
          "en": "Nehemiah repeatedly declining meetings because he is busy is the ultimate role model for anyone trying to dodge unnecessary office calls.",
          "ko": "바쁘다며 끝까지 면회를 거절하는 느헤미야는 불필요한 회의 요청을 정중하게 거절하는 현대 직장인의 표본입니다.",
          "zho": "尼希米以工作繁忙為由不斷拒絕面談，簡直是現代職場中拒絕無效會議的最佳教科書。",
          "es": "Nehemías rechazando reuniones una y otra vez porque está ocupado es el modelo a seguir para cualquiera que intente evitar llamadas innecesarias.",
          "pt": "Neemias recusando reuniões repetidamente por estar ocupado é o maior exemplo para quem tenta se livrar de chamadas desnecessárias no escritório.",
          "vi": "Việc Nê-hê-mi liên tục từ chối gặp mặt vì bận rộn là tấm gương sáng cho bất kỳ ai muốn né tránh những cuộc họp vô bổ.",
          "tl": "Ang paulit-ulit na pagtanggi ni Nehemias sa mga meeting dahil siya ay abala ay huwaran para sa sinumang umiiwas sa mga hindi kailangang tawag sa opisina.",
          "th": "เนฮามีย์ที่ปฏิเสธการนัดพบซ้ำแล้วซ้ำเล่าเพราะยุ่งอยู่ เป็นต้นแบบที่ดีที่สุดสำหรับคนที่พยายามหลีกเลี่ยงการประชุมที่ไม่จำเป็นในที่ทำงาน",
          "sw": "Nehemia kukataa mikutano mara kwa mara kwa sababu ya shughuli nyingi ni mfano bora kwa yeyote anayejaribu kukwepa vikao visivyo vya lazima.",
          "it": "Neemia che rifiuta continuamente le riunioni perché troppo occupato è il modello ideale per chiunque voglia evitare incontri inutili."
        }
    },
    "2026-08-02": {
        date: "2026-08-02",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ネヘミヤ記 8:1-18",
          "en": "Nehemiah 8:1-18",
          "ko": "Nehemiah 8:1-18",
          "zho": "ネヘミヤ記 8:1-18",
          "es": "Nehemiah 8:1-18",
          "pt": "Nehemiah 8:1-18",
          "vi": "Nehemiah 8:1-18",
          "tl": "Nehemiah 8:1-18",
          "th": "Nehemiah 8:1-18",
          "sw": "Nehemiah 8:1-18",
          "it": "Neemia 8:1-18"
        },
        comment: {
          "ja": "朝から晩まで立ちっぱなしで律法を読み上げるエズラに、お腹が空いて集中力が切れてくる民の姿は、長すぎる会議に耐える現代の私たちと何ら変わりない。",
          "en": "The people struggling to focus as Ezra reads the law from dawn to dusk is basically every modern professional stuck in an endless all-day meeting.",
          "ko": "아침부터 저녁까지 서서 율법을 듣느라 지쳐가는 사람들의 모습은 긴 회의에 지친 현대인들의 모습과 다를 게 없다.",
          "zho": "人们从早到晚站着听律法书，那种逐渐失去耐心和体力的感觉，简直就是现代人面对超长会议的真实写照。",
          "es": "La gente tratando de prestar atención a la lectura de la ley de sol a sol es la prueba de que las reuniones interminables siempre han sido un desafío.",
          "pt": "O povo tentando se concentrar na leitura da lei do amanhecer ao anoitecer é a prova de que reuniões longas sempre foram um desafio universal.",
          "vi": "Đám đông cố gắng tập trung nghe đọc luật pháp từ sáng đến tối cho thấy sự kiên nhẫn có giới hạn, y hệt chúng ta trong những cuộc họp kéo dài vô tận.",
          "tl": "Ang mga taong hirap nang makinig sa pagbasa ng batas mula umaga hanggang gabi ay parang tayo lang din sa loob ng isang napakahabang meeting.",
          "th": "ผู้คนที่พยายามตั้งใจฟังการอ่านธรรมบัญญัติจากเช้าจรดเย็นช่างเหมือนกับเราในที่ประชุมที่ยาวนานจนแทบจะหลับคาเก้าอี้",
          "sw": "Watu waliojitahidi kusikiliza kusomwa kwa sheria kuanzia asubuhi hadi jioni ni kama sisi tunavyohisi katika mikutano mirefu isiyoisha.",
          "it": "La folla che perde la concentrazione mentre Esdra legge la legge dall'alba al tramonto è chiunque sia mai rimasto bloccato in una riunione infinita."
        }
    },
    "2026-08-03": {
        date: "2026-08-03",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エステル記 1:1-22",
          "en": "Esther 1:1-22",
          "ko": "Esther 1:1-22",
          "zho": "エステル記 1:1-22",
          "es": "Esther 1:1-22",
          "pt": "Esther 1:1-22",
          "vi": "Esther 1:1-22",
          "tl": "Esther 1:1-22",
          "th": "Esther 1:1-22",
          "sw": "Esther 1:1-22",
          "it": "Ester 1:1-22"
        },
        comment: {
          "ja": "王様が酔った勢いで妻に無茶ぶりをして、断られたらプライドを傷つけられたと大騒ぎする姿、現代のSNSで見る炎上騒動と本質が変わらなくて笑ってしまう。",
          "en": "The King drunkenly demanding his wife perform for his friends and then throwing a tantrum when she refuses is basically a classic case of fragile male ego.",
          "ko": "술김에 아내에게 무리한 부탁을 했다가 거절당하자 자존심 상해하며 난리를 피우는 왕의 모습은 예나 지금이나 참 찌질하다.",
          "zho": "国王酒后强求王后献丑，被拒绝后竟恼羞成怒，这种因玻璃心而引发的大动干戈，放在今天也就是个典型的职场闹剧。",
          "es": "Un rey borracho haciendo exigencias absurdas y luego armando un drama porque su esposa no lo obedeció es el ejemplo perfecto de un ego frágil.",
          "pt": "Um rei bêbado fazendo exigências absurdas e depois fazendo um drama só porque a esposa não o obedeceu é o exemplo perfeito de ego frágil.",
          "vi": "Ông vua say xỉn đưa ra yêu cầu vô lý rồi làm ầm ĩ khi bị từ chối chính là hình mẫu kinh điển của sự sĩ diện hão huyền.",
          "tl": "Ang haring lasing na nag-utos ng kung ano-ano at nag-wala nang tumanggi ang asawa ay isang klasikong halimbawa ng marupok na ego.",
          "th": "ราชาที่เมามายแล้วสั่งให้มเหสีทำเรื่องไร้สาระ พอถูกปฏิเสธก็โกรธจนเป็นเรื่องใหญ่ ช่างเป็นตัวอย่างของอีโก้ที่เปราะบางเกินจะรับไหว",
          "sw": "Mfalme mlevi anayetoa amri za kipumbavu na kuanzisha fujo baada ya kukataliwa na mkewe ni mfano halisi wa mtu mwenye kiburi na ego dhaifu.",
          "it": "Un re ubriaco che fa richieste assurde e fa scenate quando viene rifiutato è la prova che certi ego fragili non cambiano mai."
        }
    },
    "2026-08-04": {
        date: "2026-08-04",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エステル記 2:1-23",
          "en": "Esther 2:1-23",
          "ko": "Esther 2:1-23",
          "zho": "エステル記 2:1-23",
          "es": "Esther 2:1-23",
          "pt": "Esther 2:1-23",
          "vi": "Esther 2:1-23",
          "tl": "Esther 2:1-23",
          "th": "Esther 2:1-23",
          "sw": "Esther 2:1-23",
          "it": "Ester 2:1-23"
        },
        comment: {
          "ja": "王妃選びのオーディションに強制参加させられるエステル、いざという時の度胸がすさまじすぎて推せる。",
          "en": "Esther being forced into a beauty pageant and somehow coming out on top is the ultimate underdog story.",
          "ko": "왕비 선발 대회에 강제로 끌려갔음에도 당당하게 자기 자리를 찾아가는 에스더를 보면 인생은 정말 알 수 없다는 생각이 든다.",
          "zho": "被迫參加王妃選拔大賽的以斯帖，在關鍵時刻展現出的膽識與智慧，簡直是職場求生的高手。",
          "es": "Ester siendo arrastrada a un concurso de belleza y terminando como reina es la prueba de que a veces la vida te lleva donde menos esperas.",
          "pt": "Ester sendo levada para um concurso de beleza contra a vontade e ainda assim mantendo a compostura é o auge da resiliência.",
          "vi": "Việc Ê-xơ-tê bị ép tham gia cuộc thi tuyển hoàng hậu mà vẫn giữ được bản lĩnh cho thấy cô ấy thực sự là một người phụ nữ phi thường.",
          "tl": "Si Ester na sapilitang isinama sa patimpalak ng kagandahan pero nagwagi pa rin ay patunay na kahit sa gulo, may plano ang tadhana.",
          "th": "เอสเธอร์ถูกบังคับให้เข้าประกวดนางงามแต่ดันชนะจนได้ตำแหน่งราชินี เป็นบทพิสูจน์ว่าชีวิตมักพาเราไปในที่ที่ไม่คาดคิดเสมอ",
          "sw": "Esta kulazimishwa kushiriki shindano la urembo na hatimaye kuwa malkia ni ushahidi kwamba wakati mwingine maisha hutupeleka pasipokutarajiwa.",
          "it": "Ester trascinata in un concorso reale e capace di cavarsela con un coraggio pazzesco è un vero mito."
        }
    },
    "2026-08-05": {
        date: "2026-08-05",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エステル記 3:1-15",
          "en": "Esther 3:1-15",
          "ko": "Esther 3:1-15",
          "zho": "エステル記 3:1-15",
          "es": "Esther 3:1-15",
          "pt": "Esther 3:1-15",
          "vi": "Esther 3:1-15",
          "tl": "Esther 3:1-15",
          "th": "Esther 3:1-15",
          "sw": "Esther 3:1-15",
          "it": "Ester 3:1-15"
        },
        comment: {
          "ja": "モルデカイに挨拶されないだけで国中の民族を滅ぼそうとするハマン、プライドの高さがもはや一周回って滑稽だ。",
          "en": "Haman wanting to commit genocide just because one guy wouldn't bow to him is the most pathetic display of ego in the Bible.",
          "ko": "모르드개에게 인사 한번 못 받았다고 민족 전체를 몰살하려 드는 하만의 옹졸함은 찌질함의 끝판왕이다.",
          "zho": "僅僅因為末底改不向他下跪，就要毀滅整個民族，哈曼的報復心與自尊心簡直是職場情緒管理失敗的教科書。",
          "es": "Que Amán quiera destruir a todo un pueblo solo porque alguien no lo saludó es el ejemplo perfecto de un ego herido fuera de control.",
          "pt": "Hamã querer destruir um povo inteiro só porque um homem não se curvou diante dele é o auge da fragilidade masculina levada ao extremo.",
          "vi": "Việc Ha-man đòi tiêu diệt cả một dân tộc chỉ vì một người không cúi chào cho thấy cái tôi quá lớn có thể khiến con người trở nên mù quáng đến mức nào.",
          "tl": "Ang magalit nang sobra dahil lang hindi pinagbigyan sa pagyukod at gustong ubusin ang isang lahi ay patunay na ang ego ni Haman ay napakababaw.",
          "th": "ฮามานโกรธถึงขั้นจะฆ่าล้างเผ่าพันธุ์คนทั้งชาติเพียงเพราะคนคนเดียวไม่ก้มหัวให้ เป็นตัวอย่างของคนที่เอาอีโก้เป็นใหญ่จนน่าสมเพช",
          "sw": "Hamani kutaka kuangamiza watu wote kwa sababu tu mtu mmoja hakumwinamia ni kielelezo kamili cha mtu mwenye kiburi na ego iliyopitiliza.",
          "it": "Aman che vuole sterminare un popolo intero solo perché uno non si è inchinato è il livello massimo di permalosità."
        }
    },
    "2026-08-06": {
        date: "2026-08-06",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エステル記 4:9-17",
          "en": "Esther 4:9-17",
          "ko": "Esther 4:9-17",
          "zho": "エステル記 4:9-17",
          "es": "Esther 4:9-17",
          "pt": "Esther 4:9-17",
          "vi": "Esther 4:9-17",
          "tl": "Esther 4:9-17",
          "th": "Esther 4:9-17",
          "sw": "Esther 4:9-17",
          "it": "Ester 4:9-17"
        },
        comment: {
          "ja": "死を覚悟して王の前に立つエステルの勇気は、ここぞという時に言い訳を探す自分にはあまりに眩しすぎる。",
          "en": "Esther stepping up to face the king when her life is on the line makes my own habit of making excuses look pretty pathetic.",
          "ko": "죽음을 각오하고 왕 앞에 선 에스더의 용기는 평소 핑계만 찾는 내 모습과 너무 비교되어 뼈를 때린다.",
          "zho": "以斯帖冒死进谏的勇气，对比起那个总在关键时刻找借口的自己，实在让人羞愧。",
          "es": "La valentía de Ester al arriesgarlo todo me hace ver lo mucho que me cuesta dar un paso al frente cuando realmente importa.",
          "pt": "A coragem de Ester de arriscar tudo me faz ver o quanto eu costumo inventar desculpas quando a situação aperta.",
          "vi": "Sự dũng cảm của Ê-xơ-tê khi đối mặt với cái chết khiến tôi tự nhìn lại bản thân, người luôn tìm lý do thoái thác khi cần thiết.",
          "tl": "Ang tapang ni Ester na harapin ang kamatayan ay nakakahiyang ikumpara sa sarili kong ugali na laging naghahanap ng palusot.",
          "th": "ความกล้าของเอสเธอร์ที่ยอมเสี่ยงชีวิตทำให้เห็นชัดเลยว่าตัวเรามักหาข้ออ้างหลีกเลี่ยงเสมอในเวลาที่สำคัญจริงๆ",
          "sw": "Ujasiri wa Esta wa kuhatarisha maisha yake unanikumbusha jinsi ninavyotafuta visingizio kila wakati ninapopaswa kuchukua hatua.",
          "it": "Il coraggio di Ester di rischiare tutto rende la mia solita abitudine di cercare scuse a tutti i costi davvero patetica."
        }
    },
    "2026-08-07": {
        date: "2026-08-07",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エステル記 6:1-14",
          "en": "Esther 6:1-14",
          "ko": "Esther 6:1-14",
          "zho": "エステル記 6:1-14",
          "es": "Esther 6:1-14",
          "pt": "Esther 6:1-14",
          "vi": "Esther 6:1-14",
          "tl": "Esther 6:1-14",
          "th": "Esther 6:1-14",
          "sw": "Esther 6:1-14",
          "it": "Ester 6:1-14"
        },
        comment: {
          "ja": "自分を陥れたはずの敵を盛大に称える羽目になったハマン、自尊心が完全に崩壊する様は皮肉すぎて笑えない。",
          "en": "Haman being forced to parade his enemy through town is the kind of karmic irony that is painful to even watch.",
          "ko": "자신이 파멸시키려던 적을 직접 왕의 옷을 입혀 행진시켜야 했던 하만의 굴욕은 인생 최고의 블랙코미디다.",
          "zho": "哈曼费尽心机想除掉仇人，最后却被迫亲自为对方牵马游行，这种报应简直是职场版的顶级黑色幽默。",
          "es": "Ver a Amán obligado a honrar al hombre que odiaba es la lección más amarga y sarcástica sobre el orgullo que he visto.",
          "pt": "Ver Haman sendo forçado a honrar publicamente o seu maior inimigo é um lembrete irônico de que o orgulho acaba sempre na lama.",
          "vi": "Việc Haman bị buộc phải tôn vinh kẻ thù của mình là một cú tát thực tế đau đớn về cái giá của sự kiêu ngạo.",
          "tl": "Ang mapilitang parangalan ang taong gusto mong ipapatay ay isang uri ng karma na talagang nakakahiya.",
          "th": "การที่ฮามานต้องออกมาป่าวประกาศเกียรติให้ศัตรูที่ตัวเองเกลียดชัง เป็นบทเรียนเรื่องความหยิ่งยโสที่เจ็บแสบที่สุด",
          "sw": "Haman kulazimishwa kumheshimisha adui yake ni funzo kubwa kuhusu jinsi kiburi kinavyoweza kugeuka kuwa fedheha kuu.",
          "it": "Costringere Aman a sfilare per la città per onorare il suo peggior nemico è quel tipo di ironia della sorte che fa male anche solo da guardare."
        }
    },
    "2026-08-08": {
        date: "2026-08-08",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エステル記 8:1-17",
          "en": "Esther 8:1-17",
          "ko": "Esther 8:1-17",
          "zho": "エステル記 8:1-17",
          "es": "Esther 8:1-17",
          "pt": "Esther 8:1-17",
          "vi": "Esther 8:1-17",
          "tl": "Esther 8:1-17",
          "th": "Esther 8:1-17",
          "sw": "Esther 8:1-17",
          "it": "Ester 8:1-17"
        },
        comment: {
          "ja": "王の印章リングをもらって即座に権力を行使するモルデカイ、出世した途端に服が変わる同僚を見ているような複雑な気持ちになる。",
          "en": "Watching Mordecai get the king's ring and immediately start throwing his weight around is like watching a coworker suddenly act like a CEO after a promotion.",
          "ko": "왕의 인장 반지를 받자마자 권력을 휘두르는 모르드개는 승진하자마자 태도가 돌변하는 직장 동료를 보는 듯한 묘한 기분을 느끼게 한다.",
          "zho": "看著末底改拿到國王的印戒後立刻大顯身手，這種感覺就像看到同事升職後態度大轉變一樣真實。",
          "es": "Ver a Mardoqueo recibir el anillo real y empezar a ejercer poder de inmediato es como ver a ese colega que cambia de actitud apenas le dan un ascenso.",
          "pt": "Ver Mardoqueu receber o anel do rei e logo começar a exercer poder é como ver aquele colega que muda de postura assim que recebe uma promoção.",
          "vi": "Nhìn Mạc-đô-chê nhận chiếc nhẫn của vua và lập tức phô trương quyền lực, cảm giác thật giống như khi thấy đồng nghiệp đổi thái độ ngay sau khi được thăng chức.",
          "tl": "Ang makitang hawak na ni Mardokeo ang singsing ng hari at agad na nagpapalakad ng kapangyarihan ay parang pagtingin sa katrabahong nagbago ang ugali nang ma-promote.",
          "th": "การเห็นมอร์เดคัยได้รับแหวนของกษัตริย์แล้วใช้อำนาจทันที ทำให้รู้สึกเหมือนเห็นเพื่อนร่วมงานเปลี่ยนท่าทีไปทันทีที่ได้เลื่อนตำแหน่ง",
          "sw": "Kuona Mordekai akipokea pete ya mfalme na kuanza kuitumia mamlaka yake mara moja ni kama kumtazama mfanyakazi mwenzako anayebadilika ghafla baada ya kupandishwa cheo.",
          "it": "Vedere Mardocheo ricevere l'anello e iniziare subito a comandare tutti mi ricorda quel collega che cambia atteggiamento appena riceve una promozione."
        }
    },
    "2026-08-09": {
        date: "2026-08-09",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エステル記 9:20-32",
          "en": "Esther 9:20-32",
          "ko": "Esther 9:20-32",
          "zho": "エステル記 9:20-32",
          "es": "Esther 9:20-32",
          "pt": "Esther 9:20-32",
          "vi": "Esther 9:20-32",
          "tl": "Esther 9:20-32",
          "th": "Esther 9:20-32",
          "sw": "Esther 9:20-32",
          "it": "Ester 9:20-32"
        },
        comment: {
          "ja": "自分たちの勝利を祝うために記念日を作って毎年盛り上がるエステル記の結末、人間、結局イベントと美味しいものが大好きなのは昔も今も変わらない。",
          "en": "Creating a national holiday just to celebrate a personal win is the ultimate human move; we have never really changed when it comes to wanting a party and good food.",
          "ko": "승리를 기념하려고 매년 축제를 만들기로 한 에스더 시대의 사람들, 파티와 맛있는 음식을 좋아하는 인간의 본능은 예나 지금이나 똑같다.",
          "zho": "為了慶祝勝利而設立節日並年年狂歡，人類對派對和美食的熱愛果然是幾千年來都沒變過的本能。",
          "es": "Crear una festividad nacional solo para celebrar una victoria personal es una movida muy humana; al final, lo que más nos gusta es una fiesta y una buena comida.",
          "pt": "Criar um feriado nacional só para celebrar uma vitória pessoal é uma atitude muito humana; no fim, o que a gente gosta mesmo é de uma festa e comida boa.",
          "vi": "Việc lập ra một ngày lễ quốc gia chỉ để ăn mừng chiến thắng cá nhân là một hành động rất con người; xét cho cùng, chúng ta chưa bao giờ thay đổi trong việc thích tiệc tùng và đồ ăn ngon.",
          "tl": "Ang paggawa ng holiday para lang ipagdiwang ang isang tagumpay ay tunay na makatao; hindi talaga nagbabago ang hilig natin sa party at masarap na pagkain.",
          "th": "การตั้งวันหยุดเพื่อฉลองชัยชนะของตัวเองเป็นเรื่องที่เข้าถึงอารมณ์มนุษย์สุดๆ เพราะไม่ว่าจะยุคไหนเราก็ยังชอบงานปาร์ตี้กับของอร่อยเหมือนเดิม",
          "sw": "Kuanzisha sikukuu ya kitaifa ili kusherehekea ushindi ni tabia ya kibinadamu sana; hatujabadilika kamwe linapokuja suala la kupenda sherehe na chakula kizuri.",
          "it": "Inventarsi una festa nazionale solo per celebrare una vittoria personale è una mossa geniale; alla fine, un buon banchetto mette sempre tutti d'accordo."
        }
    },
    "2026-08-10": {
        date: "2026-08-10",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨブ記 1:1-22",
          "en": "Job 1:1-22",
          "ko": "Job 1:1-22",
          "zho": "ヨブ記 1:1-22",
          "es": "Job 1:1-22",
          "pt": "Job 1:1-22",
          "vi": "Job 1:1-22",
          "tl": "Job 1:1-22",
          "th": "Job 1:1-22",
          "sw": "Job 1:1-22",
          "it": "Giobbe 1:1-22"
        },
        comment: {
          "ja": "完璧な人生を送っていたはずのヨブが、ある日突然どん底に突き落とされる展開、人生の理不尽さを凝縮しすぎていて震える。",
          "en": "Job going from having it all to losing everything in one afternoon is a brutal reminder of how quickly life can flip the script.",
          "ko": "완벽한 삶을 살던 욥이 하루아침에 나락으로 떨어지는 모습은 인생의 허무함을 너무나도 적나라하게 보여준다.",
          "zho": "約伯原本擁有完美人生，卻在一夕之間跌入谷底，這劇情簡直是人生無常的極致寫照。",
          "es": "Ver a Job perderlo todo en un abrir y cerrar de ojos es un recordatorio brutal de lo rápido que puede cambiar la vida.",
          "pt": "Ver Jó perder tudo do dia para a noite é um lembrete brutal de quão rápido a vida pode mudar.",
          "vi": "Việc Gióp mất tất cả chỉ trong một ngày là lời nhắc nhở phũ phàng về sự bấp bênh của cuộc đời.",
          "tl": "Ang makitang mawala ang lahat kay Job sa isang iglap ay isang matinding paalala kung gaano kabilis magbago ang buhay.",
          "th": "การที่โยบสูญเสียทุกอย่างไปในพริบตาเป็นเครื่องเตือนใจที่เจ็บปวดว่าชีวิตนั้นไม่แน่นอนเพียงใด",
          "sw": "Kuona Ayubu akipoteza kila kitu kwa muda mfupi ni ukumbusho mkali jinsi maisha yanavyoweza kubadilika ghafla.",
          "it": "Vedere Giobbe perdere tutto nel giro di poche ore è un promemoria brutale di quanto velocemente la vita possa ribaltare la situazione."
        }
    },
    "2026-08-11": {
        date: "2026-08-11",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨブ記 12:1-25",
          "en": "Job 12:1-25",
          "ko": "Job 12:1-25",
          "zho": "ヨブ記 12:1-25",
          "es": "Job 12:1-25",
          "pt": "Job 12:1-25",
          "vi": "Job 12:1-25",
          "tl": "Job 12:1-25",
          "th": "Job 12:1-25",
          "sw": "Job 12:1-25",
          "it": "Giobbe 12:1-25"
        },
        comment: {
          "ja": "散々な目に遭って皮肉が止まらないヨブの言い分、限界まで追い詰められた人間がいかに毒を吐くかを教えてくれる。",
          "en": "Job’s sarcastic clapbacks toward his friends show exactly how much sass a person can muster when they have absolutely nothing left to lose.",
          "ko": "친구들을 향해 쏟아내는 욥의 날카로운 독설은 사람이 한계에 다다랐을 때 얼마나 솔직해질 수 있는지를 보여준다.",
          "zho": "約伯對朋友們展開的嘲諷攻勢，完美示範了當一個人失去所有時，防禦機制會如何轉化為尖銳的毒舌。",
          "es": "El sarcasmo de Job hacia sus amigos demuestra hasta dónde puede llegar la paciencia de alguien cuando ya no tiene nada que perder.",
          "pt": "O sarcasmo de Jó com os amigos mostra o quanto alguém pode ser afiado quando não tem mais nada a perder.",
          "vi": "Sự mỉa mai của Gióp đối với bạn bè cho thấy mức độ sắc sảo của con người khi chẳng còn gì để mất.",
          "tl": "Ang pagiging sarkastiko ni Job sa kanyang mga kaibigan ay nagpapakita kung gaano katapang ang taong wala nang mawawala.",
          "th": "ความประชดประชันของโยบต่อเพื่อนๆ แสดงให้เห็นว่าคนเราจะปากร้ายได้แค่ไหนเมื่อถึงจุดที่ไม่มีอะไรจะเสียแล้ว",
          "sw": "Ucheshi mchungu wa Ayubu kwa marafiki zake unaonyesha jinsi mtu anavyoweza kuwa mkali anapokuwa hana cha kupoteza tena.",
          "it": "Il sarcasmo di Giobbe verso i suoi amici dimostra quanta pazienza ti rimanga quando non hai più assolutamente nulla da perdere."
        }
    },
    "2026-08-12": {
        date: "2026-08-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨブ記 14:1-22",
          "en": "Job 14:1-22",
          "ko": "Job 14:1-22",
          "zho": "ヨブ記 14:1-22",
          "es": "Job 14:1-22",
          "pt": "Job 14:1-22",
          "vi": "Job 14:1-22",
          "tl": "Job 14:1-22",
          "th": "Job 14:1-22",
          "sw": "Job 14:1-22",
          "it": "Giobbe 14:1-22"
        },
        comment: {
          "ja": "人生を花に例えてはかなく散ると嘆くヨブ、どん底の時に限って人生の哲学を語りたくなるのは人間の性かもしれない。",
          "en": "Comparing life to a fading flower while you are suffering is peak dramatic existentialism, yet we have all been there.",
          "ko": "인생을 덧없는 꽃에 비유하며 한탄하는 욥을 보니, 가장 힘들 때 철학자가 되고 싶은 건 사람의 본능인가 봅니다.",
          "zho": "把人生比作转瞬即逝的花朵，约伯在谷底时这种想搞哲学思考的心情，简直太真实了。",
          "es": "Comparar la vida con una flor que se marchita mientras lo pierdes todo es el drama existencial con el que todos nos hemos identificado alguna vez.",
          "pt": "Comparar a vida a uma flor que murcha no momento em que tudo dá errado é o tipo de drama existencial que todos nós já sentimos.",
          "vi": "Ví von cuộc đời như đóa hoa chóng tàn ngay lúc khốn cùng nhất, tâm trạng muốn triết lý hóa nỗi đau của Gióp thật quá đỗi con người.",
          "tl": "Ang paghambing sa buhay sa isang bulaklak na nalalanta habang nasa gitna ng pagdurusa ay isang napaka-makataong paraan ng pagharap sa kawalan.",
          "th": "การเปรียบชีวิตเป็นดอกไม้ที่ร่วงโรยในยามที่ทุกข์ที่สุด เป็นความรู้สึกดราม่าที่มนุษย์ทุกคนเข้าใจได้ดี",
          "sw": "Kulinganisha maisha na ua linalonyauka wakati unapopitia magumu ni hali ya kibinadamu ambayo sote tunaweza kuhusiana nayo.",
          "it": "Mettersi a filosofeggiare sulla brevità della vita mentre tutto va a rotoli è il massimo del dramma esistenziale, eppure ci siamo passati tutti."
        }
    },
    "2026-08-13": {
        date: "2026-08-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨブ記 21:17-34",
          "en": "Job 21:17-34",
          "ko": "Job 21:17-34",
          "zho": "ヨブ記 21:17-34",
          "es": "Job 21:17-34",
          "pt": "Job 21:17-34",
          "vi": "Job 21:17-34",
          "tl": "Job 21:17-34",
          "th": "Job 21:17-34",
          "sw": "Job 21:17-34",
          "it": "Giobbe 21:17-34"
        },
        comment: {
          "ja": "悪人がなぜか成功して平穏に暮らしているという理不尽を突きつけるヨブ、現代のSNSを見ていて感じるモヤモヤと全く同じだ。",
          "en": "Job pointing out that bad people often seem to live their best lives is a frustration that feels like scrolling through social media today.",
          "ko": "악인이 왜 저렇게 잘 먹고 잘 사냐며 따지는 욥을 보니, 현대판 SNS를 보며 느끼는 상대적 박탈감과 다를 게 없네요.",
          "zho": "约伯抱怨坏人为何总是活得风生水起，这种对世道不公的吐槽跟我们刷社交媒体时的心情如出一辙。",
          "es": "Quejarse de por qué a la gente mala le va tan bien es una frustración universal que hoy en día sentimos cada vez que abrimos cualquier red social.",
          "pt": "Reclamar que pessoas más parecem viver vidas perfeitas é uma frustração tão antiga quanto o mundo, e tão atual quanto o nosso feed de notícias.",
          "vi": "Việc Gióp thắc mắc tại sao kẻ ác lại sống sung túc cũng giống hệt cảm giác bất công khi chúng ta lướt mạng xã hội ngày nay.",
          "tl": "Ang pagtataka ni Job kung bakit tila mas masaya ang mga taong masama ay isang frustrasyon na ramdam natin tuwing nag-i-scroll tayo sa social media.",
          "th": "การที่โยบตั้งคำถามว่าทำไมคนไม่ดีถึงได้ดีเสมอ เป็นความรู้สึกขัดใจที่คนยุคนี้เข้าใจได้ดีเวลาไถฟีดโซเชียลมีเดีย",
          "sw": "Kulalamika kuwa watu wabaya wanaonekana kufanikiwa zaidi ni hisia ya kukata tamaa ambayo sote tunayo tunapovinjari mitandao ya kijamii leo.",
          "it": "Infastidirsi perché ai malvagi sembra andare sempre tutto bene è la stessa frustrazione che provi oggi guardando i social network."
        }
    },
    "2026-08-14": {
        date: "2026-08-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨブ記 24:1-25",
          "en": "Job 24:1-25",
          "ko": "Job 24:1-25",
          "zho": "ヨブ記 24:1-25",
          "es": "Job 24:1-25",
          "pt": "Job 24:1-25",
          "vi": "Job 24:1-25",
          "tl": "Job 24:1-25",
          "th": "Job 24:1-25",
          "sw": "Job 24:1-25",
          "it": "Giobbe 24:1-25"
        },
        comment: {
          "ja": "理不尽な世の中に憤るヨブの叫びは、まるで深夜のSNSで社会の不公平を呟く我々の姿そのものだ。",
          "en": "Job venting about how unfair the world is feels exactly like scrolling through social media at 2 AM and feeling the same frustration.",
          "ko": "세상의 불공평함에 소리치는 욥의 모습은 마치 새벽에 SNS로 세상의 부조리를 한탄하는 우리와 다를 게 없다.",
          "zho": "约伯对世道不公的愤怒呐喊，简直就像是在深夜社群媒体上抱怨社会现实的我们。",
          "es": "El grito de Job ante la injusticia del mundo es básicamente el equivalente antiguo de descargar nuestra frustración en redes sociales a medianoche.",
          "pt": "O desabafo de Jó sobre a injustiça do mundo é basicamente o equivalente antigo de postar nossas frustrações nas redes sociais de madrugada.",
          "vi": "Lời than trách của Gióp về sự bất công của thế giới chẳng khác nào cách chúng ta trút giận lên mạng xã hội lúc nửa đêm.",
          "tl": "Ang pagdaing ni Job tungkol sa kawalang-katarungan sa mundo ay parang tayo lang na naglalabas ng sama ng loob sa social media tuwing madaling araw.",
          "th": "เสียงคร่ำครวญของโยบต่อความไม่ยุติธรรมในโลกนี้ เหมือนกับเราที่กำลังระบายความคับข้องใจลงบนโซเชียลมีเดียตอนดึกไม่มีผิด",
          "sw": "Kilio cha Ayubu kuhusu ukosefu wa haki duniani ni kama sisi tunavyotoa malalamiko yetu kwenye mitandao ya kijamii usiku wa manane.",
          "it": "Lo sfogo di Giobbe contro le ingiustizie del mondo è praticamente l'equivalente antico di lamentarsi di tutto online a notte fonda."
        }
    },
    "2026-08-15": {
        date: "2026-08-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨブ記 39:1-30",
          "en": "Job 39:1-30",
          "ko": "Job 39:1-30",
          "zho": "ヨブ記 39:1-30",
          "es": "Job 39:1-30",
          "pt": "Job 39:1-30",
          "vi": "Job 39:1-30",
          "tl": "Job 39:1-30",
          "th": "Job 39:1-30",
          "sw": "Job 39:1-30",
          "it": "Giobbe 39:1-30"
        },
        comment: {
          "ja": "「大自然の驚異を理解できるか」と神に問われ、言葉を失うヨブを見ていると、自分の悩みがいかにちっぽけかと思い知らされてぐうの音も出ない。",
          "en": "God asking Job if he understands the mysteries of nature is the ultimate reality check that makes all our personal dramas seem tiny.",
          "ko": "대자연의 신비를 이해하느냐는 하나님의 질문 앞에 할 말을 잃은 욥을 보면, 내 고민이 얼마나 사소한 것인지 깨닫고 조용해질 뿐이다.",
          "zho": "当上帝问约伯是否了解大自然的奥秘时，约伯的哑口无言提醒了我们：跟宇宙的宏大相比，自己的纠结真的不值一提。",
          "es": "Cuando Dios le pregunta a Job si entiende los misterios de la naturaleza, es el golpe de realidad definitivo que nos hace ver lo pequeños que son nuestros dramas personales.",
          "pt": "Quando Deus pergunta a Jó se ele entende os mistérios da natureza, é o choque de realidade definitivo que nos faz ver o quão pequenos são nossos dramas pessoais.",
          "vi": "Khi Chúa hỏi Gióp liệu ông có hiểu được sự huyền bí của thiên nhiên không, đó chính là cú tát thực tế khiến ta nhận ra những lo âu cá nhân của mình thật nhỏ bé.",
          "tl": "Nang tanungin ng Diyos si Job kung nauunawaan niya ang mga hiwaga ng kalikasan, ito ang sampal ng realidad na nagpapaalala sa atin kung gaano kaliit ang ating mga personal na problema.",
          "th": "เมื่อพระเจ้าถามโยบว่าเข้าใจความลึกลับของธรรมชาติหรือไม่ นั่นคือการเตือนสติชั้นดีที่ทำให้รู้ว่าปัญหาชีวิตของเรานั้นเล็กจ้อยแค่ไหน",
          "sw": "Mungu anapomuuliza Ayubu ikiwa anaelewa siri za asili, ni ukumbusho mkubwa unaotufanya tutambue jinsi matatizo yetu ya kibinafsi yalivyo madogo.",
          "it": "Quando Dio ricorda a Giobbe la grandezza della natura, è il definitivo bagno di umiltà che ridimensiona immediatamente tutti i nostri piccoli drammi quotidiani."
        }
    },
    "2026-08-16": {
        date: "2026-08-16",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨブ記 40:1-24",
          "en": "Job 40:1-24",
          "ko": "Job 40:1-24",
          "zho": "ヨブ記 40:1-24",
          "es": "Job 40:1-24",
          "pt": "Job 40:1-24",
          "vi": "Job 40:1-24",
          "tl": "Job 40:1-24",
          "th": "Job 40:1-24",
          "sw": "Job 40:1-24",
          "it": "Giobbe 40:1-24"
        },
        comment: {
          "ja": "人生どん底のヨブに神様がベヒモスの強さを語り出す展開、もはや壮大なスケールすぎて逆に励まされるのか困惑するのか紙一重。",
          "en": "God responding to Job’s suffering by bragging about a giant sea monster is such a flex that it somehow makes me feel both comforted and extremely confused.",
          "ko": "인생 최악의 시기를 겪는 욥에게 하나님이 베헤못의 강함을 설파하시는 장면, 그 압도적인 스케일에 위로를 받아야 할지 당황해야 할지 모르겠다.",
          "zho": "在約伯人生低谷時，上帝卻開始講述巨獸的強大，這種宏大的敘事方式讓人既感到震撼又覺得有些哭笑不得。",
          "es": "Que Dios le responda a Job sobre sus sufrimientos hablándole de la fuerza de un monstruo gigante es tan abrumador que no sé si sentirme consolado o totalmente desconcertado.",
          "pt": "Deus responder ao sofrimento de Jó falando sobre a força de um monstro gigante é uma abordagem tão grandiosa que deixa a gente sem saber se ri ou se chora.",
          "vi": "Việc Chúa đáp lại nỗi đau của Gióp bằng cách kể về sức mạnh của quái vật khổng lồ thực sự là một màn đối thoại đầy khó hiểu nhưng cũng rất đáng suy ngẫm.",
          "tl": "Ang pagsagot ng Diyos sa pagdurusa ni Job sa pamamagitan ng pagyayabang tungkol sa isang dambuhalang hayop ay nakakalito pero nakakamangha sa sobrang lalim.",
          "th": "การที่พระเจ้าตอบความทุกข์ของโยบด้วยการเล่าเรื่องความยิ่งใหญ่ของสัตว์ยักษ์เป็นอะไรที่เหนือชั้นจนไม่รู้จะขำหรือจะซึ้งดี",
          "sw": "Mungu kumjibu Ayubu kuhusu mateso yake kwa kuelezea nguvu za mnyama mkubwa ni jambo la kushangaza ambalo linakuacha ukiwa na mchanganyiko wa mshangao na faraja.",
          "it": "Quando sei nel bel mezzo di una crisi esistenziale e Dio decide di consolarti parlandoti di un mostro marino gigante, non sai se ridere o piangere."
        }
    },
    "2026-08-17": {
        date: "2026-08-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 1:1-6",
          "en": "Psalms 1:1-6",
          "ko": "Psalms 1:1-6",
          "zho": "詩篇 1:1-6",
          "es": "Psalms 1:1-6",
          "pt": "Psalms 1:1-6",
          "vi": "Psalms 1:1-6",
          "tl": "Psalms 1:1-6",
          "th": "Psalms 1:1-6",
          "sw": "Psalms 1:1-6",
          "it": "Salmi 1:1-6"
        },
        comment: {
          "ja": "悪人の集まりには近づくなと釘を刺す詩篇1篇、結局のところ環境が人間を形成するという、現代のSNS断捨離と全く同じことを言っている。",
          "en": "Psalm 1 telling us to avoid the company of scoffers is basically just an ancient, divine version of curating your social media feed for better mental health.",
          "ko": "악인의 꾀를 좇지 말라는 시편 1편의 조언은 결국 주변 환경이 내 멘탈을 만든다는 현대의 SNS 다이어트와 일맥상통한다.",
          "zho": "诗篇第1篇告诫不要与恶人同坐，这简直就是古老的断舍离指南，告诉我们环境对心态的影响有多大。",
          "es": "El Salmo 1 nos advierte sobre no juntarnos con gente tóxica, lo cual es básicamente un consejo milenario para cuidar nuestra salud mental en la era digital.",
          "pt": "O Salmo 1 avisando para não andar com escarnecedores é basicamente um conselho milenar sobre como filtrar o seu feed de redes sociais para manter a paz mental.",
          "vi": "Thi thiên 1 khuyên chúng ta nên tránh xa những kẻ nhạo báng, nghe chẳng khác nào lời khuyên hiện đại về việc thanh lọc mạng xã hội để giữ tinh thần lạc quan.",
          "tl": "Ang payo ng Awit 1 na huwag sumama sa mga mapanlait ay parang sinaunang bersyon ng pag-unfollow sa mga toxic na tao sa social media para sa iyong peace of mind.",
          "th": "สดุดีบทที่ 1 ที่สอนไม่ให้คบคนพาล ก็คือคำแนะนำเรื่องการคัดกรองเพื่อนในโซเชียลมีเดียเพื่อสุขภาพจิตที่ดีในยุคปัจจุบันนั่นเอง",
          "sw": "Zaburi 1 inayotushauri kuepuka watu wabaya ni kama ushauri wa kisasa wa kuchuja watu unaowafuatilia kwenye mitandao ya kijamii ili kulinda amani yako ya akili.",
          "it": "Evitare la gente tossica secondo il Salmo 19 è fondamentalmente la versione antica del ripulire il feed dei social per salvare la propria sanità mentale."
        }
    },
    "2026-08-18": {
        date: "2026-08-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 19:1-14",
          "en": "Psalms 19:1-14",
          "ko": "Psalms 19:1-14",
          "zho": "詩篇 19:1-14",
          "es": "Psalms 19:1-14",
          "pt": "Psalms 19:1-14",
          "vi": "Psalms 19:1-14",
          "tl": "Psalms 19:1-14",
          "th": "Psalms 19:1-14",
          "sw": "Psalms 19:1-14",
          "it": "Salmi 19:1-14"
        },
        comment: {
          "ja": "スマホの通知ばかり気にして空を見上げる余裕を失っている現代人に、詩篇19篇の壮大な宇宙の語りかけは耳が痛いほど突き刺さる。",
          "en": "In a world where we spend more time staring at notifications than the sky, Psalms 19 is a humbling reminder of what we are missing.",
          "ko": "스마트폰 알림만 보느라 하늘을 올려다볼 여유를 잃어버린 우리에게 시편 19편은 뼈 아픈 한마디를 건넨다.",
          "zho": "在我們整天盯著螢幕、忽略窗外星空的時代，詩篇19篇這番對宇宙的讚嘆簡直是給現代人的當頭棒喝。",
          "es": "En un mundo donde miramos más las notificaciones que el cielo, el Salmo 19 es un recordatorio humilde de lo que nos estamos perdiendo.",
          "pt": "Num mundo onde passamos mais tempo olhando para notificações do que para o céu, o Salmo 19 é um lembrete humilhante do que estamos perdendo.",
          "vi": "Trong thế giới mà chúng ta dành nhiều thời gian nhìn vào thông báo hơn là ngước nhìn bầu trời, Thi Thiên 19 là một lời nhắc nhở khiêm nhường về những gì chúng ta đang bỏ lỡ.",
          "tl": "Sa mundong mas nakatingin tayo sa mga notification kaysa sa langit, ang Awit 19 ay isang mapagpakumbabang paalala sa mga bagay na nakakaligtaan natin.",
          "th": "ในโลกที่เราก้มหน้าดูการแจ้งเตือนมากกว่าแหงนมองท้องฟ้า สดุดีบทที่ 19 คือการเตือนใจให้เราเห็นสิ่งที่พลาดไปอย่างเจ็บแสบ",
          "sw": "Katika ulimwengu ambapo tunatumia muda mwingi kutazama arifa kuliko anga, Zaburi 19 ni ukumbusho wa unyenyekevu wa mambo tunayopoteza.",
          "it": "Passiamo così tanto tempo a fissare le notifiche del telefono che un richiamo a guardare il cielo come fa il Salmo 19 ci spiazza non poco."
        }
    },
    "2026-08-19": {
        date: "2026-08-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 22:1-31",
          "en": "Psalms 22:1-31",
          "ko": "Psalms 22:1-31",
          "zho": "詩篇 22:1-31",
          "es": "Psalms 22:1-31",
          "pt": "Psalms 22:1-31",
          "vi": "Psalms 22:1-31",
          "tl": "Psalms 22:1-31",
          "th": "Psalms 22:1-31",
          "sw": "Psalms 22:1-31",
          "it": "Salmi 22:1-31"
        },
        comment: {
          "ja": "神に見捨てられたと叫びながらも結局神に祈り続けるダビデの姿は、どん底のときほど本音が出る人間の矛盾そのものだ。",
          "en": "David screaming that God has abandoned him while simultaneously praying to Him is the most relatable display of human spiritual whiplash.",
          "ko": "버림받았다고 울부짖으면서도 끝까지 하나님을 찾는 다윗의 모습은 절망 속에서 갈팡질팡하는 우리네 인생과 너무나 닮았다.",
          "zho": "大衛一邊哭喊著被神遺棄，一邊卻又不停地向神祈禱，這種糾結簡直就是我們在人生低谷時的真實寫照。",
          "es": "David gritando que Dios lo ha abandonado mientras le sigue rezando es la representación más honesta de nuestras propias crisis de fe.",
          "pt": "Davi gritando que Deus o abandonou, enquanto continua a orar a Ele, é o retrato mais honesto de nossas próprias crises de fé.",
          "vi": "Việc Đa-vít vừa kêu gào rằng Chúa đã bỏ rơi mình vừa tiếp tục cầu nguyện với Ngài chính là biểu hiện chân thực nhất về sự giằng xé tâm linh của con người.",
          "tl": "Ang paghiyaw ni David na iniwan siya ng Diyos habang patuloy pa rin siyang nagdarasal ay ang pinakatotoong pagpapakita ng ating sariling krisis sa pananampalataya.",
          "th": "การที่ดาวิดตะโกนว่าพระเจ้าทอดทิ้งเขาในขณะที่ยังคงอธิษฐานต่อพระองค์ คือภาพสะท้อนที่ตรงไปตรงมาที่สุดของความสับสนทางจิตวิญญาณของมนุษย์",
          "sw": "Daudi kulia kwamba Mungu amemwacha huku akiendelea kumwomba ni kielelezo cha kweli cha jinsi tunavyohisi wakati wa majaribu makali.",
          "it": "Urlare che Dio ti ha abbandonato mentre continui a pregarlo è la rappresentazione più onesta e caotica delle nostre crisi personali."
        }
    },
    "2026-08-20": {
        date: "2026-08-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 25:1-22",
          "en": "Psalms 25:1-22",
          "ko": "Psalms 25:1-22",
          "zho": "詩篇 25:1-22",
          "es": "Psalms 25:1-22",
          "pt": "Psalms 25:1-22",
          "vi": "Psalms 25:1-22",
          "tl": "Psalms 25:1-22",
          "th": "Psalms 25:1-22",
          "sw": "Psalms 25:1-22",
          "it": "Salmi 25:1-22"
        },
        comment: {
          "ja": "失敗だらけの過去を棚に上げて神に許しを請うダビデの厚かましさ、結局のところ人間が救われる唯一の道かもしれない。",
          "en": "David asking for forgiveness while sweeping his past mistakes under the rug is perhaps the most honest way to approach God.",
          "ko": "수많은 실수를 저질러놓고 신에게 용서를 구하는 다윗의 뻔뻔함이 어쩌면 인간이 구원받는 유일한 방법일지도 모르겠다.",
          "zho": "大衛在犯了一堆錯後還厚著臉皮求神原諒，這或許就是人類能獲得救贖的唯一途徑。",
          "es": "Pedir perdón a Dios después de haber metido la pata hasta el fondo es, irónicamente, el acto de mayor honestidad humana.",
          "pt": "Pedir perdão a Deus depois de errar tanto é, ironicamente, o ato de maior honestidade humana.",
          "vi": "Việc Đa-vít cầu xin sự tha thứ sau khi phạm bao nhiêu sai lầm có lẽ là cách chân thật nhất để đến với Chúa.",
          "tl": "Ang paghingi ni David ng tawad sa kabila ng dami ng kanyang pagkakamali ay marahil ang pinaka-tapat na paraan para lumapit sa Diyos.",
          "th": "การที่ดาวิดกล้าขอให้พระเจ้าอภัยทั้งที่ตัวเองทำพลาดมาเยอะ อาจจะเป็นวิธีที่จริงใจที่สุดในการเข้าหาพระเจ้าแล้ว",
          "sw": "Daudi kuomba msamaha baada ya kufanya makosa mengi pengine ndiyo njia ya kweli zaidi ya kumwendea Mungu.",
          "it": "Chiedere scusa dopo aver combinato un pasticcio colossale è il modo più ironico e umano per ripartire da zero."
        }
    },
    "2026-08-21": {
        date: "2026-08-21",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 28:1-9",
          "en": "Psalms 28:1-9",
          "ko": "Psalms 28:1-9",
          "zho": "詩篇 28:1-9",
          "es": "Psalms 28:1-9",
          "pt": "Psalms 28:1-9",
          "vi": "Psalms 28:1-9",
          "tl": "Psalms 28:1-9",
          "th": "Psalms 28:1-9",
          "sw": "Psalms 28:1-9",
          "it": "Salmi 28:1-9"
        },
        comment: {
          "ja": "静かに祈っていたはずが気づけば敵への不満をぶちまけているダビデ、結局神様は愚痴の聞き役としても最強ということか。",
          "en": "David trying to pray but ending up venting his frustrations about his enemies is a reminder that God is the ultimate listener for our rants.",
          "ko": "조용히 기도하려다가 결국 적들에 대한 불평만 늘어놓는 다윗을 보니 신은 참으로 인간의 푸념을 들어주는 최고의 청취자 같다.",
          "zho": "大衛本想禱告，最後卻變成對敵人的抱怨大會，看來神確實是我們發牢騷時最好的傾聽對象。",
          "es": "David intentando orar pero terminando por desahogarse contra sus enemigos nos recuerda que Dios es el mejor oyente para nuestras quejas.",
          "pt": "Davi tentando orar mas acabando por desabafar contra seus inimigos nos lembra que Deus é o melhor ouvinte para nossas lamúrias.",
          "vi": "Việc Đa-vít cố gắng cầu nguyện nhưng cuối cùng lại trút giận lên kẻ thù nhắc nhở rằng Chúa là người lắng nghe tuyệt vời nhất cho những lời than vãn của chúng ta.",
          "tl": "Ang pagtatangka ni David na manalangin na nauuwi sa paglalabas ng sama ng loob sa mga kaaway ay paalala na ang Diyos ang pinakamahusay na tagapakinig sa ating mga hinaing.",
          "th": "ดาวิดตั้งใจจะอธิษฐานแต่สุดท้ายกลายเป็นการระบายความแค้นใส่ศัตรู ทำให้รู้เลยว่าพระเจ้าคือที่รองรับอารมณ์ที่ดีที่สุดจริงๆ",
          "sw": "Daudi kujaribu kuomba lakini anamalizia kwa kulalamika kuhusu maadui zake inatukumbusha kuwa Mungu ndiye msikilizaji bora wa malalamiko yetu.",
          "it": "Iniziare la preghiera con i migliori propositi e finire per sfogarsi sui propri nemici dimostra che lassù ci ascoltano davvero anche quando brontoliamo."
        }
    },
    "2026-08-22": {
        date: "2026-08-22",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 31:1-24",
          "en": "Psalms 31:1-24",
          "ko": "Psalms 31:1-24",
          "zho": "詩篇 31:1-24",
          "es": "Psalms 31:1-24",
          "pt": "Psalms 31:1-24",
          "vi": "Psalms 31:1-24",
          "tl": "Psalms 31:1-24",
          "th": "Psalms 31:1-24",
          "sw": "Psalms 31:1-24",
          "it": "Salmi 31:1-24"
        },
        comment: {
          "ja": "散々な目に遭って人間不信になりそうなダビデが、それでも最後に神にすべてを委ねる姿は、一周回って最強の開き直り術だ。",
          "en": "David having every reason to lose faith in humanity yet choosing to dump his problems on God is the ultimate masterclass in letting go.",
          "ko": "온갖 고난 속에서 사람들에게 배신당하고도 결국 신에게 모든 것을 맡기는 다윗의 모습은 일종의 완벽한 마음 비우기 기술이다.",
          "zho": "大衛在經歷了人生的背叛後，依然選擇將一切交託給神，這簡直是最高境界的看開與放下。",
          "es": "David, después de que todo le sale mal, decide dejar sus problemas en manos de Dios; es el arte de soltar todo cuando ya no puedes más.",
          "pt": "Davi, após passar por poucas e boas, decide simplesmente entregar tudo nas mãos de Deus, provando que às vezes a melhor estratégia é desistir de controlar.",
          "vi": "David sau khi chịu đủ mọi tổn thương vẫn chọn cách phó thác cho Chúa, đó chính là nghệ thuật buông bỏ đỉnh cao khi đời quá mệt mỏi.",
          "tl": "Si David na halos mawalan na ng tiwala sa tao dahil sa dami ng pinagdaanan, pero pinili pa ring ipaubaya ang lahat sa Diyos, ay ang tunay na definition ng pag-let go.",
          "th": "ดาวิดผู้ผ่านเรื่องราวมาจนแทบจะหมดศรัทธาในมนุษย์ แต่สุดท้ายเลือกที่จะฝากทุกอย่างไว้กับพระเจ้า คือวิชาการปล่อยวางที่เหนือชั้นที่สุด",
          "sw": "Daudi kupitia yote na bado kuchagua kumkabidhi Mungu mzigo wake ni somo kuu la jinsi ya kuacha yasiyowezekana na kusonga mbele.",
          "it": "Quando tutto va storto e decidi di mollare il colpo affidandoti a Qualcuno più in alto, scopri la vera arte del lasciar andare."
        }
    },
    "2026-08-23": {
        date: "2026-08-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 33:1-22",
          "en": "Psalms 33:1-22",
          "ko": "Psalms 33:1-22",
          "zho": "詩篇 33:1-22",
          "es": "Psalms 33:1-22",
          "pt": "Psalms 33:1-22",
          "vi": "Psalms 33:1-22",
          "tl": "Psalms 33:1-22",
          "th": "Psalms 33:1-22",
          "sw": "Psalms 33:1-22",
          "it": "Salmi 33:1-22"
        },
        comment: {
          "ja": "王様がどれだけ軍勢を揃えても結局は神頼みという事実は、現代の私たちがどれだけスマホで検索しても結局は運に任せるしかない心理と似ている。",
          "en": "The fact that kings realized their armies couldn't save them without God is just like us realizing that endless Googling won't actually solve our deepest problems.",
          "ko": "아무리 강한 군대를 가진 왕도 결국 신의 도움이 필요하다는 사실은, 아무리 스마트폰으로 검색해도 결국은 운에 맡겨야 하는 우리의 모습과 다를 게 없다.",
          "zho": "國王擁有再強大的軍隊最終還是得靠神，這跟我們現代人就算查遍網路搜尋引擎，最後還是得看命運安排沒什麼兩樣。",
          "es": "Que los reyes se dieran cuenta de que sus ejércitos no servían de nada sin Dios es igual a cuando buscamos mil soluciones en Google y al final entendemos que todo es cuestión de suerte.",
          "pt": "Reis que tinham exércitos poderosos mas ainda assim precisavam de Deus são o reflexo de nós que, mesmo com todo o acesso à informação, no fim das contas só podemos confiar no que não controlamos.",
          "vi": "Các vị vua có trong tay đội quân hùng mạnh vẫn cần đến Chúa, giống như việc chúng ta dù có tra Google cả ngày thì cuối cùng vẫn phải chấp nhận rằng có những thứ nằm ngoài tầm kiểm soát.",
          "tl": "Ang katotohanang kahit ang mga hari ay kailangan pa rin ng tulong ng Diyos sa kabila ng kanilang hukbo ay parang tayo lang na kahit i-Google mo na ang lahat, kailangan mo pa ring magtiwala sa tadhana.",
          "th": "กษัตริย์ที่มีกองทัพยิ่งใหญ่ยังต้องพึ่งพาพระเจ้า ไม่ต่างอะไรกับเราที่แม้จะเสิร์ชหาคำตอบจากกูเกิลจนล้นจอ สุดท้ายก็ต้องยอมรับว่ามีหลายอย่างที่ควบคุมไม่ได้",
          "sw": "Wafalme kutambua kuwa majeshi yao hayatoshi bila Mungu ni sawa na sisi tunavyotafuta majibu yote mtandaoni lakini mwisho wa siku tunatambua hatuna udhibiti wa kila kitu.",
          "it": "Capire che i re non potevano salvarsi da soli è un po' come quando cerchiamo mille risposte online prima di accorgerci che ci vuole ben altro."
        }
    },
    "2026-08-24": {
        date: "2026-08-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 49:1-20",
          "en": "Psalms 49:1-20",
          "ko": "Psalms 49:1-20",
          "zho": "詩篇 49:1-20",
          "es": "Psalms 49:1-20",
          "pt": "Psalms 49:1-20",
          "vi": "Psalms 49:1-20",
          "tl": "Psalms 49:1-20",
          "th": "Psalms 49:1-20",
          "sw": "Psalms 49:1-20",
          "it": "Salmi 49:1-20"
        },
        comment: {
          "ja": "どれだけ高級車やブランド品を買い込んでも死ぬときは全部置いていくという事実に、改めて財布の紐を締め直したくなる瞬間です。",
          "en": "The reminder that you cannot take your luxury cars or bank accounts to the grave is the ultimate reality check for our modern obsession with status.",
          "ko": "아무리 명품을 휘감고 다녀도 결국 빈손으로 떠난다는 사실은 우리가 집착하는 성공의 덧없음을 제대로 찌르네요.",
          "zho": "無論擁有多少財富，死後都帶不走分毫，這番話對於現代人過度追求物質的執著真是一記當頭棒喝。",
          "es": "Darse cuenta de que no puedes llevarte tus posesiones a la tumba es el golpe de realidad que todos necesitamos cuando nos obsesionamos demasiado con el estatus.",
          "pt": "Lembrar que não levamos nada material para o túmulo é aquele choque de realidade que coloca nossas prioridades de consumo de volta no lugar.",
          "vi": "Việc nhận ra rằng bạn không thể mang theo tài sản xuống mồ chính là cú tát tỉnh táo cho những ai đang quá mải mê chạy theo danh vọng.",
          "tl": "Ang paalala na hindi natin madadala ang ating mga ari-arian sa kabilang buhay ay isang matinding sampal ng katotohanan para sa ating pagkahumaling sa status.",
          "th": "การเตือนสติว่าไม่มีใครเอาทรัพย์สินติดตัวไปได้เมื่อตายคือความจริงที่เจ็บปวดที่สุดสำหรับคนที่มัวแต่ยึดติดกับชื่อเสียงและเงินทอง",
          "sw": "Kukumbushwa kuwa huwezi kuondoka na utajiri wako kwenda kaburini ni ukweli mchungu unaotufanya tutafakari upya vipaumbele vyetu vya maisha.",
          "it": "Ricordarsi che non possiamo portarci dietro l'estratto conto nella tomba è quella doccia di umiltà che rimette a posto le priorità della spesa."
        }
    },
    "2026-08-25": {
        date: "2026-08-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 51:1-19",
          "en": "Psalms 51:1-19",
          "ko": "Psalms 51:1-19",
          "zho": "詩篇 51:1-19",
          "es": "Psalms 51:1-19",
          "pt": "Psalms 51:1-19",
          "vi": "Psalms 51:1-19",
          "tl": "Psalms 51:1-19",
          "th": "Psalms 51:1-19",
          "sw": "Psalms 51:1-19",
          "it": "Salmi 51:1-19"
        },
        comment: {
          "ja": "盛大にやらかした後に言い訳せず潔く謝るダビデの姿は、プライドが邪魔をして謝罪をこじらせがちな現代人の鏡のようです。",
          "en": "David owning his massive mistakes without making excuses is a masterclass in humility that most of us, with our fragile egos, really need to study.",
          "ko": "큰 잘못을 저지른 뒤 변명 없이 깔끔하게 사죄하는 다윗의 모습은 자존심 때문에 사과를 미루는 우리에게 뼈아픈 교훈을 주네요.",
          "zho": "大衛在犯錯後不找藉口、坦誠認錯的態度，簡直是給那些因為自尊心作祟而難以開口道歉的現代人上了一課。",
          "es": "La capacidad de David para admitir sus errores sin poner excusas es una lección de humildad que a muchos nos hace falta cuando el orgullo nos impide pedir perdón.",
          "pt": "A forma como David assume seus erros sem dar desculpas é uma aula de humildade para quem, como nós, costuma complicar um simples pedido de desculpas por puro orgulho.",
          "vi": "Cách David thành thật nhận lỗi mà không cần bào chữa là bài học quý giá cho những ai thường vì cái tôi quá lớn mà khó lòng mở lời xin lỗi.",
          "tl": "Ang pag-amin ni David sa kanyang pagkakamali nang walang palusot ay isang aral sa pagpapakumbaba na kailangan nating lahat kapag pinipigilan tayo ng ating pride.",
          "th": "การที่ดาวิดยอมรับผิดโดยไม่หาข้ออ้างเป็นบทเรียนเรื่องความถ่อมใจที่ยอดเยี่ยมสำหรับคนที่มักจะปล่อยให้ทิฐิขวางทางคำขอโทษ",
          "sw": "Jinsi Daudi alivyokiri makosa yake bila visingizio ni funzo kuu la unyenyekevu kwa sisi ambao mara nyingi tunashindwa kuomba msamaha kwa sababu ya kiburi.",
          "it": "Ammettere di aver fatto un errore senza inventare scuse è una lezione di stile che faremmo tutti bene a ripassare ogni tanto."
        }
    },
    "2026-08-26": {
        date: "2026-08-26",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 64:1-10",
          "en": "Psalms 64:1-10",
          "ko": "Psalms 64:1-10",
          "zho": "詩篇 64:1-10",
          "es": "Psalms 64:1-10",
          "pt": "Psalms 64:1-10",
          "vi": "Psalms 64:1-10",
          "tl": "Psalms 64:1-10",
          "th": "Psalms 64:1-10",
          "sw": "Psalms 64:1-10",
          "it": "Salmi 64:1-10"
        },
        comment: {
          "ja": "陰口を叩く人たちの矢が自分に刺さる前に神様に助けを求めるダビデ、人間関係の悩みはいつの時代も変わらない。",
          "en": "David praying for protection from backstabbing gossipers reminds us that toxic office politics are as ancient as time.",
          "ko": "뒷담화하는 사람들의 화살이 나를 향하기 전에 하나님께 숨는 다윗을 보니 인간관계의 고단함은 예나 지금이나 똑같네요.",
          "zho": "大衛在流言蜚語中向神尋求庇護，看來職場和社交圈的勾心鬥角早在幾千年前就存在了。",
          "es": "David pidiendo protección ante las lenguas afiladas nos recuerda que los chismes y las traiciones son un problema tan antiguo como la humanidad misma.",
          "pt": "Davi buscando refúgio contra as fofocas alheias nos mostra que o drama das relações humanas é um clássico que nunca sai de moda.",
          "vi": "David tìm kiếm sự bảo vệ trước những lời đàm tiếu cho thấy rằng vấn đề thị phi trong các mối quan hệ đã tồn tại từ ngàn xưa.",
          "tl": "Ang paghingi ni David ng proteksyon laban sa mga tsismoso ay patunay na ang toxic na ugali ng tao ay hindi nagbabago sa paglipas ng panahon.",
          "th": "ดาวิดขอให้พระเจ้าช่วยให้พ้นจากคำนินทาที่เหมือนลูกธนู ทำให้รู้ว่าปัญหาเรื่องคนช่างเม้าท์มอยเป็นเรื่องคลาสสิกที่อยู่มาทุกยุคสมัย",
          "sw": "Daudi kutafuta hifadhi dhidi ya maneno ya watu inatukumbusha kuwa tabia ya kusengenya ni tatizo la kale sana lisiloisha.",
          "it": "David che chiede protezione dai pettegolezzi ci ricorda che le dinamiche tossiche tra colleghi sono antiche quanto il mondo."
        }
    },
    "2026-08-27": {
        date: "2026-08-27",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 69:19-36",
          "en": "Psalms 69:19-36",
          "ko": "Psalms 69:19-36",
          "zho": "詩篇 69:19-36",
          "es": "Psalms 69:19-36",
          "pt": "Psalms 69:19-36",
          "vi": "Psalms 69:19-36",
          "tl": "Psalms 69:19-36",
          "th": "Psalms 69:19-36",
          "sw": "Psalms 69:19-36",
          "it": "Salmi 69:19-36"
        },
        comment: {
          "ja": "誰にもわかってもらえない孤独の中で神様だけが頼みの綱という状況、強がっている人ほど実は一番寂しいのかもしれない。",
          "en": "Feeling like nobody gets you and realizing God is your only real listener is the ultimate reality check for the lonely at heart.",
          "ko": "아무도 내 마음을 몰라줄 때 하나님만이 유일한 대화 상대가 되는 경험, 강한 척하는 사람일수록 사실은 가장 외로운 법이죠.",
          "zho": "當全世界都沒人理解你，而神是你唯一的依靠時，那種孤獨感其實是每個人內心深處最真實的寫照。",
          "es": "Sentir que nadie te entiende y que solo Dios te escucha es el momento de mayor vulnerabilidad y, a la vez, de mayor conexión.",
          "pt": "Sentir que ninguém te compreende e ter apenas a Deus como refúgio é o ponto em que até os mais durões precisam admitir sua solidão.",
          "vi": "Cảm giác không ai hiểu mình và chỉ còn Chúa là chỗ dựa duy nhất chính là khoảnh khắc cô đơn nhất nhưng cũng chân thực nhất của con người.",
          "tl": "Ang pakiramdam na walang nakakaunawa sa iyo at tanging Diyos lang ang nakikinig ay isang mapait na katotohanan na madalas nating nararanasan.",
          "th": "การรู้สึกว่าไม่มีใครเข้าใจเราเลยนอกจากพระเจ้า เป็นช่วงเวลาที่เปราะบางที่สุดแต่ก็จริงใจกับตัวเองที่สุดเช่นกัน",
          "sw": "Kuhisi kuwa hakuna anayekuelewa na Mungu ndiye msikilizaji wako pekee ni hali inayomfanya hata mtu mwenye nguvu ajihisi mpweke.",
          "it": "Sentire che nessuno ti capisce e realizzare che lassù c'è qualcuno in ascolto è la vera svolta quando ci si sente soli."
        }
    },
    "2026-08-28": {
        date: "2026-08-28",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 72:1-20",
          "en": "Psalms 72:1-20",
          "ko": "Psalms 72:1-20",
          "zho": "詩篇 72:1-20",
          "es": "Psalms 72:1-20",
          "pt": "Psalms 72:1-20",
          "vi": "Psalms 72:1-20",
          "tl": "Psalms 72:1-20",
          "th": "Psalms 72:1-20",
          "sw": "Psalms 72:1-20",
          "it": "Salmi 72:1-20"
        },
        comment: {
          "ja": "理想のリーダー像をこれでもかと詰め込まれたソロモンの祈り、現代の選挙公約なら満場一致で当選確実だけど現実はそう甘くない。",
          "en": "Solomon praying for the perfect leadership qualities is the ultimate campaign promise that sounds great until you actually have to govern people.",
          "ko": "솔로몬이 구한 이상적인 지도자의 모습은 선거 공약으로는 만점이지만 현실의 인간관계에 적용하기엔 너무 높은 장벽이다.",
          "zho": "所羅門祈求成為理想領袖的禱告，如果放在現代選舉公報上絕對高票當選，但現實中的領導力遠比口號困難得多。",
          "es": "La oración de Salomón describiendo al líder perfecto suena como la promesa de campaña ideal, pero todos sabemos que gobernar personas es mucho más complicado.",
          "pt": "A oração de Salomão por uma liderança perfeita soa como a promessa de campanha ideal, mas governar pessoas na vida real é bem mais complexo.",
          "vi": "Lời cầu nguyện của Sa-lô-môn về một nhà lãnh đạo hoàn hảo nghe như một lời hứa tranh cử tuyệt vời, nhưng thực tế việc quản lý con người lại khó hơn nhiều.",
          "tl": "Ang panalangin ni Solomon para sa perpektong pamumuno ay parang perpektong pangako sa kampanya, pero alam nating lahat na mas mahirap mamuno sa totoong buhay.",
          "th": "คำอธิษฐานของซาโลมอนที่ขอให้เป็นผู้นำที่สมบูรณ์แบบฟังดูเหมือนคำสัญญาหาเสียงที่ยอดเยี่ยม แต่ในความเป็นจริงการปกครองคนนั้นยากกว่าที่คิดเยอะ",
          "sw": "Sala ya Sulemani ya kuomba uongozi bora inasikika kama ahadi za kampeni za kuvutia, lakini tunajua sote kuwa kuongoza watu ni changamoto kubwa zaidi.",
          "it": "La preghiera di Salomone per un governo perfetto suona benissimo sulla carta, peccato che gestire le persone sia tutta un'altra storia."
        }
    },
    "2026-08-29": {
        date: "2026-08-29",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 78:1-55",
          "en": "Psalms 78:1-55",
          "ko": "Psalms 78:1-55",
          "zho": "詩篇 78:1-55",
          "es": "Psalms 78:1-55",
          "pt": "Psalms 78:1-55",
          "vi": "Psalms 78:1-55",
          "tl": "Psalms 78:1-55",
          "th": "Psalms 78:1-55",
          "sw": "Psalms 78:1-55",
          "it": "Salmi 78:1-55"
        },
        comment: {
          "ja": "神の奇跡を散々見ておきながら、喉が渇いただけで文句を言って不平を垂れるイスラエルの民、もはや人間あるあるの極み。",
          "en": "Israelites witnessing massive miracles only to complain the moment they get thirsty is the ultimate display of human short-term memory.",
          "ko": "기적을 수도 없이 체험하고도 목이 마르다고 바로 불평하는 이스라엘 백성을 보면 인간의 망각 능력은 정말 탁월하다는 생각밖에 안 든다.",
          "zho": "以色列人見證了無數神蹟，卻因為口渴就立刻抱怨，這完全展現了人類過目即忘的頂級技能。",
          "es": "Los israelitas presenciando milagros increíbles y quejándose apenas les da un poco de sed es la prueba definitiva de que nuestra memoria a corto plazo es terrible.",
          "pt": "Os israelitas testemunhando milagres incríveis e reclamando assim que sentem sede é a prova definitiva de que nossa memória de curto prazo é péssima.",
          "vi": "Dân Y-sơ-ra-ên chứng kiến những phép lạ vĩ đại rồi lại càm ràm ngay khi khát nước là minh chứng rõ nhất cho trí nhớ ngắn hạn của con người.",
          "tl": "Ang mga Israelita na nakakita ng napakaraming himala pero nagrereklamo agad pagkauhaw ay patunay na napakabilis talaga nating makalimot ng tao.",
          "th": "ชาวอิสราเอลที่เห็นปาฏิหาริย์มามากมายแต่กลับบ่นทันทีที่หิวน้ำ เป็นเครื่องพิสูจน์ชั้นดีว่าความจำระยะสั้นของมนุษย์นั้นสั้นยิ่งกว่าอะไร",
          "sw": "Waisraeli kushuhudia miujiza mikuu na kisha kulalamika mara tu wanapopata kiu ni uthibitisho tosha kuwa kumbukumbu zetu za kibinadamu ni fupi ajabu.",
          "it": "Vedere gli israeliti assistere a miracoli incredibili e lamentarsi per un po' di sete dimostra quanto la nostra memoria a breve termine faccia acqua."
        }
    },
    "2026-08-30": {
        date: "2026-08-30",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 78:56-72",
          "en": "Psalms 78:56-72",
          "ko": "Psalms 78:56-72",
          "zho": "詩篇 78:56-72",
          "es": "Psalms 78:56-72",
          "pt": "Psalms 78:56-72",
          "vi": "Psalms 78:56-72",
          "tl": "Psalms 78:56-72",
          "th": "Psalms 78:56-72",
          "sw": "Psalms 78:56-72",
          "it": "Salmi 78:56-72"
        },
        comment: {
          "ja": "散々助けてもらった恩を忘れてはまた神様に背くイスラエルの民、まるで学習能力ゼロの自分を見ているようで苦しい。",
          "en": "Watching Israel forget every miracle the moment things get slightly uncomfortable is a painful reminder of my own selective memory.",
          "ko": "도움을 받고도 금세 잊어버리고 딴길로 새는 이스라엘 백성은 마치 저의 평소 모습을 거울로 보는 것 같습니다.",
          "zho": "以色列人一面享受恩典一面又重蹈覆轍，這種健忘的程度簡直跟我自己一模一樣。",
          "es": "Ver a Israel olvidar sus bendiciones apenas surgen problemas es un recordatorio doloroso de mi propia memoria selectiva.",
          "pt": "Ver Israel esquecer as bênçãos assim que surge um problema é um lembrete doloroso da minha própria memória seletiva.",
          "vi": "Thấy dân Y-sơ-ra-ên quên ơn ngay khi gặp khó khăn, tôi lại thấy bóng dáng mình trong đó.",
          "tl": "Ang makitang nakalimutan ng Israel ang lahat ng tulong sa oras na mahirapan sila ay parang nakikita ko na ang sarili ko.",
          "th": "การที่อิสราเอลลืมทุกอย่างทันทีที่เจอปัญหาช่างเป็นภาพสะท้อนที่เจ็บปวดของความขี้ลืมในตัวเราเอง",
          "sw": "Kuona Israeli wakisahau baraka zote pindi tu wanapopata matatizo ni ukumbusho mchungu wa jinsi tunavyosahau haraka.",
          "it": "Guardare Israele dimenticarsi di ogni benedizione appena sorge un problema è un doloroso promemoria della mia stessa memoria selettiva."
        }
    },
    "2026-08-31": {
        date: "2026-08-31",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 102:1-28",
          "en": "Psalms 102:1-28",
          "ko": "Psalms 102:1-28",
          "zho": "詩篇 102:1-28",
          "es": "Psalms 102:1-28",
          "pt": "Psalms 102:1-28",
          "vi": "Psalms 102:1-28",
          "tl": "Psalms 102:1-28",
          "th": "Psalms 102:1-28",
          "sw": "Psalms 102:1-28",
          "it": "Salmi 102:1-28"
        },
        comment: {
          "ja": "孤独で消え入りそうな夜に神様へ愚痴をこぼす詩篇の作者、誰にも言えない弱音を吐ける相手がいることの救いよ。",
          "en": "The author dumping all their lonely, midnight anxieties on God is the ultimate proof that prayer is basically a divine therapy session.",
          "ko": "밤새 외로움에 사무쳐 하나님께 넋두리를 늘어놓는 시편 기자를 보니 기도는 역시 최고의 상담 시간인 것 같습니다.",
          "zho": "詩人深夜向神傾訴滿腹委屈，這證明了禱告其實就是最好的心理諮商。",
          "es": "Ver al salmista desahogando sus ansiedades nocturnas con Dios confirma que la oración es, básicamente, una sesión de terapia divina.",
          "pt": "Ver o salmista desabafando suas ansiedades noturnas com Deus confirma que a oração é, basicamente, uma sessão de terapia divina.",
          "vi": "Việc tác giả thi thiên trút hết nỗi lòng với Chúa trong đêm khuya chứng tỏ cầu nguyện chính là liệu pháp tâm lý tốt nhất.",
          "tl": "Ang pagbubuhos ng salmista ng lahat ng kanyang takot sa Diyos sa kalagitnaan ng gabi ay patunay na ang panalangin ay parang isang divine therapy session.",
          "th": "การที่ผู้เขียนสดุดีระบายความกังวลใจทั้งหมดกับพระเจ้าในยามค่ำคืนเป็นหลักฐานว่าการอธิษฐานคือการบำบัดจิตใจที่ดีที่สุด",
          "sw": "Mtunga-zaburi kumwaga wasiwasi wake wote kwa Mungu usiku wa manane ni uthibitisho kuwa sala ni kama tiba ya kiroho.",
          "it": "Buttare tutte le proprie ansie notturne addosso al cielo conferma che la preghiera è fondamentalmente una seduta di terapia gratuita."
        }
    },
    "2026-09-01": {
        date: "2026-09-01",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 116:1-19",
          "en": "Psalms 116:1-19",
          "ko": "Psalms 116:1-19",
          "zho": "詩篇 116:1-19",
          "es": "Psalms 116:1-19",
          "pt": "Psalms 116:1-19",
          "vi": "Psalms 116:1-19",
          "tl": "Psalms 116:1-19",
          "th": "Psalms 116:1-19",
          "sw": "Psalms 116:1-19",
          "it": "Salmi 116:1-19"
        },
        comment: {
          "ja": "死の淵から生還した途端に神への恩義を語りまくる詩人のテンション、助かった時の人間ってこんなにも現金で正直なものか。",
          "en": "The psalmist talking about his devotion the moment he survives death is a hilarious reminder of how quickly we become religious when we are actually out of trouble.",
          "ko": "죽음의 문턱에서 살아나자마자 신에게 충성을 맹세하는 시인의 모습은 위기 탈출 직후 인간이 얼마나 솔직하고 현실적인지를 보여준다.",
          "zho": "剛從鬼門關走一遭就急著對神表忠心，詩篇作者這份死裡逃生後的激動，完全體現了人類在驚嚇過後的真實反應。",
          "es": "El salmista prometiendo el cielo y la tierra justo después de salvarse es el ejemplo perfecto de cómo nos volvemos súper devotos cuando el susto ya pasó.",
          "pt": "O salmista prometendo tudo a Deus logo após escapar da morte é a prova viva de que o ser humano fica muito religioso quando o perigo vai embora.",
          "vi": "Tác giả thi thiên thề thốt lòng thành ngay sau khi thoát chết là minh chứng rõ nhất cho việc con người dễ dàng trở nên ngoan đạo thế nào khi cơn nguy khốn qua đi.",
          "tl": "Ang pagpapasalamat ng salmista pagkatapos maligtas ay paalala na tayong mga tao ay sadyang nagiging mapagkumbaba lang kapag tapos na ang krisis.",
          "th": "การที่ผู้เขียนสดุดีรีบประกาศความภักดีทันทีที่รอดพ้นจากความตาย ช่างสะท้อนให้เห็นว่ามนุษย์เรามีความจริงใจและตื่นตัวแค่ไหนหลังจากผ่านวิกฤตมาได้",
          "sw": "Mtunga-zaburi anayeapa utii mara tu baada ya kunusurika kifo ni ukumbusho kamili wa jinsi tunavyokuwa waaminifu tunapopona kutoka kwenye matatizo.",
          "it": "Il salmista che promette fedeltà eterna appena scampa a un pericolo è l'esempio perfetto di quanto diventiamo devoti quando lo spavento è passato."
        }
    },
    "2026-09-02": {
        date: "2026-09-02",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 119:9-72",
          "en": "Psalms 119:9-72",
          "ko": "Psalms 119:9-72",
          "zho": "詩篇 119:9-72",
          "es": "Psalms 119:9-72",
          "pt": "Psalms 119:9-72",
          "vi": "Psalms 119:9-72",
          "tl": "Psalms 119:9-72",
          "th": "Psalms 119:9-72",
          "sw": "Psalms 119:9-72",
          "it": "Salmi 119:9-72"
        },
        comment: {
          "ja": "若いうちから清く正しく生きるなんて無理ゲーだと嘆きつつ、結局そこにすがりたくなるのは人生の難易度を痛感している証拠だ。",
          "en": "Complaining that living a pure life is impossible while secretly wishing you could do it anyway is just the classic human struggle with our own limitations.",
          "ko": "젊은 나이에 흠 없이 사는 것이 불가능하다고 투덜대면서도 결국 그 길을 동경하는 것은 우리 모두가 인생의 높은 난이도를 실감하고 있다는 증거다.",
          "zho": "一邊抱怨年輕時要過得清白純潔簡直是地獄難度，一邊又忍不住想嘗試，這份糾結完全是每個人在成長路上的必經過程。",
          "es": "Quejarse de que vivir una vida impecable es imposible mientras uno intenta ser mejor es la lucha interna más clásica de nuestra existencia.",
          "pt": "Reclamar que viver uma vida pura é impossível enquanto tenta fazer o seu melhor é a luta interna mais clássica que todo mundo enfrenta.",
          "vi": "Than vãn rằng sống một cuộc đời trọn vẹn là điều bất khả thi trong khi vẫn cố gắng làm điều đúng đắn chính là cuộc đấu tranh tâm lý muôn thuở của con người.",
          "tl": "Ang magreklamo na mahirap magpakabanal habang pilit pa ring sumusubok ay ang pinaka-relatable na pakikibaka ng ating pagkatao.",
          "th": "การบ่นว่าการใช้ชีวิตให้บริสุทธิ์นั้นยากเหลือเกิน ทั้งที่ในใจก็ยังพยายามทำตาม เป็นความย้อนแย้งที่มนุษย์ทุกคนต้องเผชิญในเส้นทางชีวิต",
          "sw": "Kulalamika kuwa kuishi maisha matakatifu haiwezekani huku ukijaribu kufanya hivyo ni mapambano ya kawaida ya kibinadamu tunapokabiliana na ugumu wa maisha.",
          "it": "Lamentarsi che vivere senza macchia sia impossibile mentre cerchi comunque di fare del tuo meglio è la lotta quotidiana di tutti noi."
        }
    },
    "2026-09-03": {
        date: "2026-09-03",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 119:73-136",
          "en": "Psalms 119:73-136",
          "ko": "Psalms 119:73-136",
          "zho": "詩篇 119:73-136",
          "es": "Psalms 119:73-136",
          "pt": "Psalms 119:73-136",
          "vi": "Psalms 119:73-136",
          "tl": "Psalms 119:73-136",
          "th": "Psalms 119:73-136",
          "sw": "Psalms 119:73-136",
          "it": "Salmi 119:73-136"
        },
        comment: {
          "ja": "これでもかと律法への愛を語り続ける詩篇119篇の作者、もはや推し活をしている時のオタクの熱量と同じで親近感しかない。",
          "en": "The author of Psalm 119 obsessing over the law with such intensity is honestly just giving major fanboy energy.",
          "ko": "시편 119편 저자가 율법을 향해 쏟아내는 열정적인 고백을 보니 좋아하는 연예인에게 푹 빠진 덕후의 마음이 이런 게 아닐까 싶다.",
          "zho": "詩篇119篇的作者對律法的熱愛，簡直就像是在熱戀期瘋狂安利偶像的粉絲，這份執著既真誠又讓人會心一笑。",
          "es": "La intensidad con la que el autor del Salmo 119 habla de la ley es básicamente el equivalente bíblico de un fan obsesionado con su artista favorito.",
          "pt": "A intensidade com que o autor do Salmo 119 fala sobre a lei é basicamente o equivalente bíblico de um fã obcecado pelo seu ídolo.",
          "vi": "Tác giả Thi Thiên 119 bày tỏ tình yêu với luật pháp mãnh liệt đến mức trông giống như một người hâm mộ đang cuồng nhiệt ủng hộ thần tượng của mình.",
          "tl": "Ang tindi ng pagmamahal ng may-akda ng Awit 119 sa kautusan ay parang isang fan na todo kung mag-promote ng kanyang iniidolo.",
          "th": "ผู้เขียนสดุดี 119 พูดถึงกฎเกณฑ์ด้วยความหลงใหลขนาดนี้ ไม่ต่างอะไรกับแฟนคลับที่กำลังคลั่งไคล้ศิลปินคนโปรดจนถอนตัวไม่ขึ้น",
          "sw": "Mwandishi wa Zaburi 119 anavyozungumzia sheria kwa shauku kubwa inafanana sana na jinsi shabiki anavyomtetea msanii wake anayempenda sana.",
          "it": "L'intensità con cui l'autore del Salmo 119 loda la legge è praticamente il livello di devozione di un fan sfegatato."
        }
    },
    "2026-09-04": {
        date: "2026-09-04",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 119:137-176",
          "en": "Psalms 119:137-176",
          "ko": "Psalms 119:137-176",
          "zho": "詩篇 119:137-176",
          "es": "Psalms 119:137-176",
          "pt": "Psalms 119:137-176",
          "vi": "Psalms 119:137-176",
          "tl": "Psalms 119:137-176",
          "th": "Psalms 119:137-176",
          "sw": "Psalms 119:137-176",
          "it": "Salmi 119:137-176"
        },
        comment: {
          "ja": "迷える羊のように彷徨っていると自認しつつも神の言葉を求め続ける作者、結局のところ人生は迷いながらの正解探しそのもの。",
          "en": "Admitting to wandering like a lost sheep while still chasing after the truth is basically the universal mood of adulthood.",
          "ko": "길 잃은 양처럼 헤매면서도 끝까지 말씀을 붙들겠다는 저자의 고백은, 방황하며 정답을 찾아가는 우리 모두의 인생 그 자체다.",
          "zho": "一邊承認自己像迷途羔羊，一邊又死守著真理不放，這不就是我們在人生路上邊迷茫邊找答案的真實寫照嗎。",
          "es": "Admitir que te sientes como una oveja perdida mientras sigues buscando la verdad es básicamente el sentimiento universal de la vida adulta.",
          "pt": "Admitir que se sente como uma ovelha perdida enquanto continua buscando a verdade é basicamente o sentimento universal da vida adulta.",
          "vi": "Thừa nhận mình như một con chiên lạc lối nhưng vẫn cố gắng bám lấy lẽ thật, đó chính là trạng thái chung của chúng ta khi đối mặt với cuộc đời.",
          "tl": "Ang pag-amin na parang nawawalang tupa habang patuloy na naghahanap ng katotohanan ay ang mismong kwento ng ating buhay na puno ng pagkalito.",
          "th": "การยอมรับว่าตัวเองเหมือนแกะที่หลงทางแต่ก็ยังพยายามไขว่คว้าหาความจริง คือนิยามของชีวิตผู้ใหญ่ที่ต้องเดินไปงมเข็มไปอย่างแท้จริง",
          "sw": "Kukiri kuwa wewe ni kama kondoo aliyepotea huku ukiendelea kutafuta kweli ni hali halisi ya maisha yetu tunapozidi kutafuta majibu katikati ya changamoto.",
          "it": "Ammettere di sentirsi una pecora smarrita mentre si cerca la rotta giusta è la sintesi perfetta di cosa significhi essere adulti."
        }
    },
    "2026-09-05": {
        date: "2026-09-05",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 136:1-26",
          "en": "Psalms 136:1-26",
          "ko": "Psalms 136:1-26",
          "zho": "詩篇 136:1-26",
          "es": "Psalms 136:1-26",
          "pt": "Psalms 136:1-26",
          "vi": "Psalms 136:1-26",
          "tl": "Psalms 136:1-26",
          "th": "Psalms 136:1-26",
          "sw": "Psalms 136:1-26",
          "it": "Salmi 136:1-26"
        },
        comment: {
          "ja": "26回も繰り返し「恵みはとこしえまで」と歌い続ける詩篇のしつこさは、忘れたくないことを自分に言い聞かせるための究極のメモ帳だ。",
          "en": "Repeating the phrase 'His mercy endureth forever' twenty-six times is basically the ancient version of setting a sticky note on your forehead so you don't forget the obvious.",
          "ko": "스물여섯 번이나 반복해서 자비하심을 노래하는 시편은 잊지 않으려고 굳이 포스트잇을 덕지덕지 붙여놓은 내 일기장 같다.",
          "zho": "把同一句話重複唱了二十六遍，這簡直就是古代版的置頂公告，深怕自己哪天腦袋當機給忘了。",
          "es": "Repetir veintiséis veces que su misericordia es eterna es básicamente la forma antigua de ponerse una nota adhesiva en la frente para no olvidar lo importante.",
          "pt": "Repetir vinte e seis vezes que a misericórdia é eterna é o equivalente antigo a colar um lembrete na testa para não esquecer o que realmente importa.",
          "vi": "Việc lặp lại cụm từ lòng thương xót tồn tại đời đời đến hai mươi sáu lần giống như cách chúng ta tự nhắc nhở bản thân bằng hàng tá ghi chú dán khắp nhà.",
          "tl": "Ang pag-uulit ng dalawampu't anim na beses na ang awa ay magpakailanman ay parang paglalagay ng napakaraming sticky note sa salamin para hindi makalimutan ang mahalagang bagay.",
          "th": "การร้องย้ำถึงความเมตตาถึงยี่สิบหกครั้งก็เหมือนกับการแปะโพสต์อิทเตือนตัวเองไว้ทั่วบ้านเพราะกลัวจะลืมเรื่องสำคัญ",
          "sw": "Kurudia mara ishirini na sita kwamba rehema ni za milele ni kama kujibandika noti nyingi za ukumbusho ili usisahau jambo muhimu.",
          "it": "Ripetere ventisei volte che la sua misericordia dura per sempre è praticamente la versione antica di mettersi un post-it sulla fronte per non dimenticare l'ovvio."
        }
    },
    "2026-09-06": {
        date: "2026-09-06",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "詩篇 146:1-10",
          "en": "Psalms 146:1-10",
          "ko": "Psalms 146:1-10",
          "zho": "詩篇 146:1-10",
          "es": "Psalms 146:1-10",
          "pt": "Psalms 146:1-10",
          "vi": "Psalms 146:1-10",
          "tl": "Psalms 146:1-10",
          "th": "Psalms 146:1-10",
          "sw": "Psalms 146:1-10",
          "it": "Salmi 146:1-10"
        },
        comment: {
          "ja": "権力者に頼るなと言い切る詩篇のこの潔さは、結局最後は自分の足で立つしかないという人生の厳しい現実を突きつけてくる。",
          "en": "Telling people not to put their trust in princes is a blunt reminder that at the end of the day, you are the only one who has to live with your own choices.",
          "ko": "권력자에게 의지하지 말라는 단호한 조언은 결국 인생은 누구의 도움도 아닌 내 발로 걸어가는 것이라는 현실을 뼈 때리게 알려준다.",
          "zho": "勸人別把希望寄託在權貴身上，這話聽起來很刺耳，卻無情地戳破了我們總想找個靠山躺平的幻想。",
          "es": "Decir que no pongamos nuestra confianza en los poderosos es un recordatorio directo de que, al final del día, solo nos tenemos a nosotros mismos.",
          "pt": "Dizer para não confiarmos nos poderosos é um lembrete direto de que, no fim das contas, a responsabilidade pela nossa vida é inteiramente nossa.",
          "vi": "Lời khuyên đừng đặt niềm tin vào những người có quyền lực là một lời nhắc nhở thẳng thắn rằng cuối cùng thì bạn vẫn phải tự chịu trách nhiệm cho cuộc đời mình.",
          "tl": "Ang payo na huwag umasa sa mga nasa kapangyarihan ay isang direktang paalala na sa huli, sarili mo lang ang maaasahan mo.",
          "th": "คำเตือนไม่ให้ฝากชีวิตไว้กับผู้มีอำนาจเป็นเหมือนการตบหน้าให้ตื่นว่าสุดท้ายแล้วเราต้องรับผิดชอบชีวิตตัวเอง",
          "sw": "Kushauri watu wasiweke tumaini lao kwa watawala ni ukumbusho wa moja kwa moja kwamba mwisho wa siku, wewe ndiye unayepaswa kubeba mzigo wa maamuzi yako.",
          "it": "Dire di non fidarsi dei potenti è il promemoria definitivo che, alla fine della fiera, siamo noi a dover convivere con le nostre scelte."
        }
    },
    "2026-09-07": {
        date: "2026-09-07",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "箴言 1:1-33",
          "en": "Proverbs 1:1-33",
          "ko": "Proverbs 1:1-33",
          "zho": "箴言 1:1-33",
          "es": "Proverbs 1:1-33",
          "pt": "Proverbs 1:1-33",
          "vi": "Proverbs 1:1-33",
          "tl": "Proverbs 1:1-33",
          "th": "Proverbs 1:1-33",
          "sw": "Proverbs 1:1-33",
          "it": "Proverbi 1:1-33"
        },
        comment: {
          "ja": "知恵が通りで必死に叫んでいるのに誰も聞いていない様子、現代のSNSで有益な情報が埋もれていく光景と重なって切ない。",
          "en": "Wisdom shouting in the streets while everyone ignores her is basically the original version of being left on read.",
          "ko": "지혜가 거리에서 소리치는데 아무도 듣지 않는 모습은 마치 중요한 조언을 해도 아무도 보지 않는 단톡방의 현실 같다.",
          "zho": "智慧在街上大聲疾呼卻沒人理會，這簡直就是現代人發出重要提醒卻被已讀不回的寫照。",
          "es": "La sabiduría gritando en las calles mientras todos la ignoran es básicamente el equivalente antiguo de ser dejado en visto.",
          "pt": "A sabedoria gritando nas ruas enquanto todos a ignoram é basicamente o equivalente antigo de ser deixado no vácuo.",
          "vi": "Sự khôn ngoan kêu gọi giữa phố phường mà chẳng ai thèm nghe, nghe thật giống với cảm giác khi bạn nói điều tử tế mà bị lờ đi.",
          "tl": "Ang karunungan na sumisigaw sa lansangan pero walang nakikinig ay parang pag-post ng seryosong payo na walang nag-like.",
          "th": "ความฉลาดตะโกนปาวๆ อยู่ข้างถนนแต่ไม่มีใครฟัง เหมือนการพิมพ์เตือนเรื่องสำคัญในกลุ่มไลน์แล้วไม่มีใครอ่านเลยสักคน",
          "sw": "Hekima kupiga kelele barabarani bila mtu kusikiliza ni kama vile kutoa ushauri muhimu kwenye kikundi cha WhatsApp na watu wote kukupuuza.",
          "it": "La saggezza che urla per strada mentre tutti la ignorano è praticamente la versione originale dell'essere visualizzati e non ricevuti."
        }
    },
    "2026-09-08": {
        date: "2026-09-08",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "箴言 3:1-35",
          "en": "Proverbs 3:1-35",
          "ko": "Proverbs 3:1-35",
          "zho": "箴言 3:1-35",
          "es": "Proverbs 3:1-35",
          "pt": "Proverbs 3:1-35",
          "vi": "Proverbs 3:1-35",
          "tl": "Proverbs 3:1-35",
          "th": "Proverbs 3:1-35",
          "sw": "Proverbs 3:1-35",
          "it": "Proverbi 3:1-35"
        },
        comment: {
          "ja": "自分の賢さを頼りにするなと言われるたびに、今の自分の計画に自信満々な自分を突きつけられているようで耳が痛い。",
          "en": "Being told not to lean on my own understanding is a direct attack on my ego and my questionable life choices.",
          "ko": "내 명철을 의지하지 말라는 말씀은 내 고집대로 살려는 마음을 정면으로 저격하는 것 같아 뜨끔하다.",
          "zho": "每次讀到不要倚靠自己的聰明，都覺得像是在拆穿我那些自以為是的完美計畫。",
          "es": "Que me digan que no me apoye en mi propia prudencia es un golpe directo a mi ego y a mis planes mal calculados.",
          "pt": "Ser lembrado de não confiar no meu próprio entendimento é um golpe direto no meu ego e nas minhas decisões duvidosas.",
          "vi": "Lời khuyên đừng dựa vào sự khôn ngoan của bản thân giống như một gáo nước lạnh tạt vào sự tự phụ của chính mình.",
          "tl": "Ang payo na huwag umasa sa sariling unawa ay parang pagsampal sa sarili kong diskarte na madalas naman palang mali.",
          "th": "การถูกเตือนไม่ให้พึ่งพาความฉลาดของตัวเองนี่เหมือนโดนตบหน้าเรียกสติเวลาเรามั่นใจในแผนการที่ผิดพลาดของตัวเองเกินไป",
          "sw": "Kuambiwa usitegemee akili zako mwenyewe ni kama kukumbushwa kuwa mipango yangu ya kujivunia mara nyingi haina mwelekeo.",
          "it": "Sentirsi dire di non confidare nella propria intelligenza è un attacco diretto al mio ego e alle mie decisioni di vita quantomeno discutibili."
        }
    },
    "2026-09-09": {
        date: "2026-09-09",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "箴言 4:10-27",
          "en": "Proverbs 4:10-27",
          "ko": "Proverbs 4:10-27",
          "zho": "箴言 4:10-27",
          "es": "Proverbs 4:10-27",
          "pt": "Proverbs 4:10-27",
          "vi": "Proverbs 4:10-27",
          "tl": "Proverbs 4:10-27",
          "th": "Proverbs 4:10-27",
          "sw": "Proverbs 4:10-27",
          "it": "Proverbi 4:10-27"
        },
        comment: {
          "ja": "右にも左にも曲がるなと忠告されるほど、私たちの足元は誘惑という名の落とし穴だらけなのだと痛感する。",
          "en": "Being told not to turn to the right or left just proves how many distractions are waiting to trip us up every single day.",
          "ko": "좌우로 치우치지 말라는 조언을 들을 때마다, 우리 주변이 얼마나 유혹이라는 함정으로 가득한지 새삼 깨닫게 된다.",
          "zho": "被叮囑不可偏左也不可偏右，才發現生活中的誘惑簡直像地雷一樣隨處可見。",
          "es": "Que te digan que no te desvíes ni a la derecha ni a la izquierda solo confirma que el camino está lleno de distracciones esperando a que tropieces.",
          "pt": "Ser avisado para não desviar nem para a direita nem para a esquerda mostra o quanto a vida está cheia de distrações tentando nos tirar do rumo.",
          "vi": "Lời khuyên không được lệch trái hay phải nhắc nhở rằng cuộc sống này đầy rẫy những cám dỗ chực chờ làm ta vấp ngã.",
          "tl": "Ang paalalang huwag lumiko sa kanan o kaliwa ay patunay lang na punong-puno ng tukso ang bawat hakbang natin.",
          "th": "การถูกเตือนไม่ให้หันซ้ายหรือขวาทำให้รู้เลยว่าในแต่ละวันเราต้องเจอสิ่งเร้าที่คอยดึงให้เราออกนอกลู่นอกทางมากแค่ไหน",
          "sw": "Kuambiwa usigeuke kulia wala kushoto ni ukumbusho tosha kuwa maisha yamejaa vishawishi vinavyojaribu kutupoteza njia kila siku.",
          "it": "Ricevere il consiglio di non voltarsi né a destra né a sinistra dimostra solo quante distrazioni ci tendono un agguato ogni singolo giorno."
        }
    },
    "2026-09-10": {
        date: "2026-09-10",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "箴言 16:1-33",
          "en": "Proverbs 16:1-33",
          "ko": "Proverbs 16:1-33",
          "zho": "箴言 16:1-33",
          "es": "Proverbs 16:1-33",
          "pt": "Proverbs 16:1-33",
          "vi": "Proverbs 16:1-33",
          "tl": "Proverbs 16:1-33",
          "th": "Proverbs 16:1-33",
          "sw": "Proverbs 16:1-33",
          "it": "Proverbi 16:1-33"
        },
        comment: {
          "ja": "計画を立てるのは人間ですが、結局その通りにいかないのが人生というもので、神様の采配に身を任せるほうがよっぽど気が楽です。",
          "en": "We make our plans, but life usually has other ideas, and honestly, letting go and trusting the bigger picture is a huge relief.",
          "ko": "계획은 사람이 세우지만 결국 뜻대로 되지 않는 게 인생이라, 모든 것을 신께 맡기는 편이 마음 편할 때가 많다.",
          "zho": "計畫永遠趕不上變化，與其執著於自己的藍圖，不如學會放手交給老天安排。",
          "es": "Nosotros hacemos planes, pero la vida tiene sus propios planes, y al final, soltar el control es lo mejor que podemos hacer por nuestra salud mental.",
          "pt": "A gente faz planos, mas a vida tem outros bem diferentes, e aprender a confiar no destino é o melhor alívio para a ansiedade.",
          "vi": "Con người lập kế hoạch nhưng đời không như mơ, nên đôi khi buông tay để mọi thứ thuận theo ý Trời lại là cách sống nhẹ nhàng nhất.",
          "tl": "Tayong mga tao ang nagpaplano, pero ang buhay ay may sariling takbo, kaya mas mainam na lang na ipaubaya ang lahat sa Kanya.",
          "th": "เราวางแผนไว้ดิบดีแต่ชีวิตมักจะมีเซอร์ไพรส์เสมอ การปล่อยวางและเชื่อมั่นในสิ่งที่เหนือกว่าจึงเป็นวิธีจัดการความเครียดที่ดีที่สุด",
          "sw": "Sisi hupanga mipango, lakini maisha yana njia zake, hivyo kuacha kujaribu kudhibiti kila kitu na kumwamini Mungu ndilo suluhisho la amani.",
          "it": "Noi facciamo piani ma la vita ha tutt'altri programmi, e onor del vero mollare il controllo è una liberazione incredibile."
        }
    },
    "2026-09-11": {
        date: "2026-09-11",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "箴言 22:1-29",
          "en": "Proverbs 22:1-29",
          "ko": "Proverbs 22:1-29",
          "zho": "箴言 22:1-29",
          "es": "Proverbs 22:1-29",
          "pt": "Proverbs 22:1-29",
          "vi": "Proverbs 22:1-29",
          "tl": "Proverbs 22:1-29",
          "th": "Proverbs 22:1-29",
          "sw": "Proverbs 22:1-29",
          "it": "Proverbi 22:1-29"
        },
        comment: {
          "ja": "名声や富よりも人間としての評判が大事だと言われても、結局は目先の損得勘定に振り回されてしまうのが私たちという生き物だ。",
          "en": "Being told that a good reputation is better than riches, yet still obsessing over the next paycheck, is the eternal human irony.",
          "ko": "좋은 평판이 재물보다 낫다는 것을 알면서도 당장의 이익에 눈이 멀어버리는 것이 바로 우리 인간의 본모습이다.",
          "zho": "明明知道名聲比財富更重要，卻還是忍不住被眼前的利益牽著鼻子走，這就是我們人類最真實的寫照。",
          "es": "Que nos digan que la reputación vale más que el dinero, pero seguir obsesionados con la cuenta bancaria, es la ironía humana de siempre.",
          "pt": "Saber que a reputação vale mais que a riqueza, mas ainda assim viver obcecado com o saldo bancário, é a eterna ironia humana.",
          "vi": "Biết rằng danh tiếng quan trọng hơn tiền bạc nhưng vẫn mãi loay hoay với những lợi ích trước mắt, đó chính là bản chất thật của chúng ta.",
          "tl": "Ang sabihing mas mahalaga ang mabuting pangalan kaysa sa kayamanan, ngunit patuloy pa ring naghahabol sa pera, ay ang walang katapusang ironiya ng buhay.",
          "th": "การถูกบอกว่าชื่อเสียงสำคัญกว่าเงินทอง แต่ก็ยังคงหมกมุ่นอยู่กับผลประโยชน์ตรงหน้า คือความย้อนแย้งที่เป็นธรรมชาติที่สุดของมนุษย์เรา",
          "sw": "Kuambiwa kuwa jina zuri ni bora kuliko utajiri, lakini bado tukihangaika na faida za muda mfupi, ndiyo hali halisi ya kibinadamu.",
          "it": "Sentirsi dire che una buona reputazione vale più delle ricchezze e poi continuare a ossessionarsi per lo stipendio è la classica ironia umana."
        }
    },
    "2026-09-12": {
        date: "2026-09-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "箴言 31:1-31",
          "en": "Proverbs 31:1-31",
          "ko": "Proverbs 31:1-31",
          "zho": "箴言 31:1-31",
          "es": "Proverbs 31:1-31",
          "pt": "Proverbs 31:1-31",
          "vi": "Proverbs 31:1-31",
          "tl": "Proverbs 31:1-31",
          "th": "Proverbs 31:1-31",
          "sw": "Proverbs 31:1-31",
          "it": "Proverbi 31:1-31"
        },
        comment: {
          "ja": "スーパーウーマンすぎる箴言31章の女性を読んで、自分の生活の散らかり具合に思わず遠い目をしてしまうのは私だけではないはずだ。",
          "en": "Reading about the Proverbs 31 woman and then looking at the pile of laundry on my own chair is the ultimate reality check.",
          "ko": "잠언 31장의 완벽한 여인을 읽고 나서 내 방구석의 빨래 더미를 보면, 자괴감과 함께 현실의 벽이 느껴지는 것은 어쩔 수 없다.",
          "zho": "讀完箴言31章那位完美的女性，再看看自己房間裡堆積如山的髒衣服，這種現實落差感真是讓人無奈又想笑。",
          "es": "Leer sobre la mujer de Proverbios 31 y luego mirar el desorden de mi propia habitación es el golpe de realidad que no pedí pero que necesitaba.",
          "pt": "Ler sobre a mulher de Provérbios 31 e depois olhar para a pilha de roupas no meu quarto é o choque de realidade que ninguém precisava hoje.",
          "vi": "Đọc về người phụ nữ hoàn hảo trong Châm ngôn 31 rồi nhìn lại đống quần áo bừa bộn của chính mình, đó đúng là một cú tát thực tế.",
          "tl": "Ang basahin ang tungkol sa babae sa Kawikaan 31 at pagkatapos ay tumingin sa tambak ng labada sa kwarto ko ay ang tunay na sampal ng realidad.",
          "th": "การอ่านเรื่องหญิงสาวในสุภาษิตบทที่ 31 แล้วหันมามองกองผ้าที่ยังไม่ได้ซักในห้องตัวเอง คือความจริงอันแสนเจ็บปวดที่พบเจอได้ทุกวัน",
          "sw": "Kusoma kuhusu mwanamke wa Mithali 31 kisha kutazama rundo la nguo chafu chumbani kwangu ni ukweli mchungu ambao sote tunaujua.",
          "it": "Leggere della donna di Proverbi trentuno e poi guardare la pila di vestiti sulla sedia è il controllo della realtà definitivo."
        }
    },
    "2026-09-13": {
        date: "2026-09-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "伝道の書 2:1-26",
          "en": "Ecclesiastes 2:1-26",
          "ko": "Ecclesiastes 2:1-26",
          "zho": "伝道の書 2:1-26",
          "es": "Ecclesiastes 2:1-26",
          "pt": "Ecclesiastes 2:1-26",
          "vi": "Ecclesiastes 2:1-26",
          "tl": "Ecclesiastes 2:1-26",
          "th": "Ecclesiastes 2:1-26",
          "sw": "Ecclesiastes 2:1-26",
          "it": "Ecclesiaste 2:1-26"
        },
        comment: {
          "ja": "あれもこれもと手を出して結局むなしさにたどり着くソロモンの姿、現代のSNSで他人のキラキラした生活を覗き見して疲れ果てている私たちそのものだ。",
          "en": "Solomon chasing every possible pleasure only to realize it is all empty is basically us doomscrolling through everyone else's highlight reels and feeling exhausted.",
          "ko": "온갖 것을 다 누려보고 결국 허무함을 깨닫는 솔로몬의 모습은 마치 남의 화려한 일상을 구경하며 상대적 박탈감에 빠진 현대인의 자화상 같다.",
          "zho": "所羅門嘗試了世間一切享樂卻最終感到空虛，這簡直就像是我們在社群媒體上刷著別人的精采生活，自己卻感到疲憊不堪的樣子。",
          "es": "Salomón probándolo todo y terminando vacío es el retrato perfecto de nosotros tratando de llenar el alma con compras online y likes.",
          "pt": "Salomão tentando de tudo e terminando vazio é o retrato perfeito de nós tentando preencher a alma com compras online e curtidas nas redes sociais.",
          "vi": "Vua Solomon trải nghiệm đủ mọi thú vui rồi nhận ra tất cả chỉ là hư không, nghe sao mà giống tâm trạng chúng ta sau khi lướt mạng xã hội cả ngày rồi thấy trống rỗng.",
          "tl": "Ang pagsubok ni Solomon sa lahat ng bagay at pagkadismaya sa huli ay katulad ng ating paghahanap ng ligaya sa social media na nauuwi lang din sa pagod.",
          "th": "โซโลมอนผู้ลองทำทุกอย่างแต่สุดท้ายก็ว่างเปล่า ไม่ต่างอะไรกับเราที่พยายามเติมเต็มชีวิตด้วยสิ่งของและยอดไลก์จนเหนื่อยล้าไปหมด",
          "sw": "Sulemani kujaribu kila kitu na kuishia na ubatili ni kama sisi tunavyohangaika kutafuta kuridhika kwenye mitandao ya kijamii na kuishia kuchoka.",
          "it": "Salomone che insegue ogni piacere per poi scoprire che è tutto un vuoto pneumatico siamo letteralmente noi che scrolliamo i social fino a esaurimento."
        }
    },
    "2026-09-14": {
        date: "2026-09-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 1:1-31",
          "en": "Isaiah 1:1-31",
          "ko": "Isaiah 1:1-31",
          "zho": "イザヤ書 1:1-31",
          "es": "Isaiah 1:1-31",
          "pt": "Isaiah 1:1-31",
          "vi": "Isaiah 1:1-31",
          "tl": "Isaiah 1:1-31",
          "th": "Isaiah 1:1-31",
          "sw": "Isaiah 1:1-31",
          "it": "Isaia 1:1-31"
        },
        comment: {
          "ja": "どれだけ盛大な儀式や立派な礼拝を捧げても、隣人への優しさを忘れていたら神様に見抜かれているという事実は、現代の宗教的なパフォーマンスへの強烈な皮肉だ。",
          "en": "God calling out empty rituals while ignoring the marginalized is the ultimate wake-up call for anyone putting on a show of faith without actually caring for others.",
          "ko": "화려한 종교 의식에만 치중하고 이웃 사랑은 뒷전인 사람들을 향한 하나님의 일침은 오늘날 보여주기식 신앙 생활을 하는 우리에게 뼈아픈 조언이다.",
          "zho": "上帝指責那些只做表面功夫卻忽略鄰舍的人，這對於現代那些只會表演信仰卻缺乏實際關懷的人來說，簡直是一記當頭棒喝。",
          "es": "Dios criticando los rituales vacíos mientras se ignora al necesitado es un recordatorio directo de que la fe sin acciones es solo un espectáculo.",
          "pt": "Deus criticando rituais vazios enquanto o próximo é ignorado é um lembrete direto de que a fé sem ações é apenas um show.",
          "vi": "Việc Chúa phê phán những nghi lễ phô trương trong khi bỏ quên người khốn khó chính là lời cảnh tỉnh cho những ai chỉ biết thể hiện đức tin qua vẻ bề ngoài.",
          "tl": "Ang pagpuna ng Diyos sa mga walang kabuluhang ritwal habang pinababayaan ang kapwa ay isang matinding paalala na hindi sapat ang pakitang-tao lang sa relihiyon.",
          "th": "พระเจ้าทรงตำหนิพิธีกรรมที่ว่างเปล่าในขณะที่เพิกเฉยต่อเพื่อนมนุษย์ เป็นการเตือนสติเราว่าศรัทธาที่แท้จริงไม่ใช่แค่การแสดงละครทางศาสนา",
          "sw": "Mungu kukemea ibada za kisherehe huku jirani akipuuza ni ukumbusho mkali kwamba imani haipaswi kuwa maonyesho ya nje tu bila upendo wa kweli.",
          "it": "Dio che striglia i riti vuoti è la sveglia definitiva per chi fa sceneggiate di fede senza preoccuparsi davvero del prossimo."
        }
    },
    "2026-09-15": {
        date: "2026-09-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 2:1-22",
          "en": "Isaiah 2:1-22",
          "ko": "Isaiah 2:1-22",
          "zho": "イザヤ書 2:1-22",
          "es": "Isaiah 2:1-22",
          "pt": "Isaiah 2:1-22",
          "vi": "Isaiah 2:1-22",
          "tl": "Isaiah 2:1-22",
          "th": "Isaiah 2:1-22",
          "sw": "Isaiah 2:1-22",
          "it": "Isaia 2:1-22"
        },
        comment: {
          "ja": "剣を鋤に打ち替えるという平和な未来を夢見つつ、結局は銀や金や馬に依存してしまう人間の矛盾、いつの時代も変わりませんね。",
          "en": "Dreaming of turning swords into plowshares while still obsessing over silver, gold, and fancy horses is the ultimate human irony.",
          "ko": "칼을 보습으로 바꾸는 평화를 꿈꾸면서도 결국 금과 은과 말에 집착하는 인간의 모순, 예나 지금이나 변함이 없네요.",
          "zho": "梦想着将刀剑打成犁头的同时，却依然执着于金银与骏马，这种人类的矛盾心理真是古今皆同。",
          "es": "Soñar con convertir espadas en arados mientras seguimos obsesionados con el oro y la plata es la gran contradicción humana.",
          "pt": "Sonhar em transformar espadas em arados enquanto continuamos obcecados por ouro e prata é a grande contradição humana.",
          "vi": "Mơ về việc biến gươm giáo thành lưỡi cày trong khi vẫn ám ảnh bởi vàng bạc và ngựa xe là sự mâu thuẫn điển hình của con người.",
          "tl": "Ang mangarap na gawing araro ang mga espada habang abala pa rin sa ginto at pilak ay ang pinakakaraniwang kahinaan nating mga tao.",
          "th": "ฝันถึงการเปลี่ยนดาบให้เป็นคันไถ แต่ก็ยังหมกมุ่นอยู่กับเงินทองและม้าศึก นี่แหละคือความย้อนแย้งในใจมนุษย์ที่แก้ไม่หาย",
          "sw": "Kuota ndoto ya kubadilisha panga kuwa majembe huku tukiwa bado tunashikilia dhahabu na fedha ni utata wa kibinadamu ambao haujabadilika.",
          "it": "Sognare la pace ma continuare a ossessionarsi per il conto in banca e gli ultimi gadget è la più grande contraddizione umana."
        }
    },
    "2026-09-16": {
        date: "2026-09-16",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 3:13-26",
          "en": "Isaiah 3:13-26",
          "ko": "Isaiah 3:13-26",
          "zho": "イザヤ書 3:13-26",
          "es": "Isaiah 3:13-26",
          "pt": "Isaiah 3:13-26",
          "vi": "Isaiah 3:13-26",
          "tl": "Isaiah 3:13-26",
          "th": "Isaiah 3:13-26",
          "sw": "Isaiah 3:13-26",
          "it": "Isaia 3:13-26"
        },
        comment: {
          "ja": "ファッションや持ち物で武装して自分を大きく見せようとする心理、何千年経ってもSNSのプロフィール欄で戦っている現代人と全く同じですね。",
          "en": "Armoring ourselves with fashion and status symbols to look important is exactly what we do on social media profiles today.",
          "ko": "화려한 옷과 장신구로 자신을 포장하려는 심리는 수천 년 전이나 지금의 SNS 프로필을 꾸미는 우리나 다를 게 없네요.",
          "zho": "试图通过华丽的服饰与饰品来武装自己，这种心态与现代人在社交媒体上精心经营个人主页简直如出一辙。",
          "es": "Tratar de impresionarnos con accesorios y moda es exactamente lo mismo que hacemos hoy al editar nuestros perfiles en redes sociales.",
          "pt": "Tentar impressionar os outros com acessórios e moda é exatamente o que fazemos hoje ao editar nossos perfis nas redes sociais.",
          "vi": "Cố gắng khoe khoang bản thân qua trang phục và phụ kiện chẳng khác nào cách chúng ta đang cố tô vẽ cho trang cá nhân trên mạng xã hội bây giờ.",
          "tl": "Ang paggamit ng mamahaling gamit para magmukhang importante ay katulad lang ng ginagawa natin sa ating mga social media profile ngayon.",
          "th": "การพยายามเสริมภาพลักษณ์ด้วยแฟชั่นและของใช้หรูหรานั้น ไม่ต่างอะไรกับการที่เราพยายามจัดโปรไฟล์บนโซเชียลมีเดียให้ดูดีในทุกวันนี้เลย",
          "sw": "Kujaribu kujionyesha kwa nguo na vitu vya thamani ni sawa kabisa na kile tunachofanya leo kwenye kurasa zetu za mitandao ya kijamii.",
          "it": "Cercare di sembrare importanti attraverso vestiti e apparenze è esattamente quello che facciamo oggi sui profili dei social network."
        }
    },
    "2026-09-17": {
        date: "2026-09-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 5:8-30",
          "en": "Isaiah 5:8-30",
          "ko": "Isaiah 5:8-30",
          "zho": "イザヤ書 5:8-30",
          "es": "Isaiah 5:8-30",
          "pt": "Isaiah 5:8-30",
          "vi": "Isaiah 5:8-30",
          "tl": "Isaiah 5:8-30",
          "th": "Isaiah 5:8-30",
          "sw": "Isaiah 5:8-30",
          "it": "Isaia 5:8-30"
        },
        comment: {
          "ja": "隣の土地まで買い占めて限界まで拡大しようとするイザヤ時代の欲張りさんたち、現代の終わらないラットレースを先取りしすぎている。",
          "en": "People in Isaiah's time trying to hoard every last inch of land are basically the original ancestors of our modern-day workaholic hustle culture.",
          "ko": "남의 땅까지 다 차지하려고 끝도 없이 욕심내는 이사야 시대 사람들을 보니 현대인의 멈추지 않는 무한 경쟁이 떠올라 씁쓸하다.",
          "zho": "以賽亞時代那些拼命擴張領土、想把地都佔為己有的地主，簡直就是現代無止盡內捲生活的鼻祖。",
          "es": "Esos personajes de la época de Isaías que querían acaparar toda la tierra son básicamente los antepasados de nuestra obsesión moderna por tenerlo todo.",
          "pt": "Essas pessoas na época de Isaías querendo comprar todo o terreno vizinho são basicamente os ancestrais da nossa cultura de produtividade frenética.",
          "vi": "Những kẻ thời Ê-sai cố gắng thâu tóm mọi tấc đất xung quanh thực sự là những phiên bản cổ xưa của hội chứng cuồng công việc thời nay.",
          "tl": "Ang mga taong gustong sakupin ang lahat ng lupa noong panahon ni Isaias ay parang mga modernong tao na hindi marunong makuntento sa kanilang mga gamit.",
          "th": "คนที่พยายามกว้านซื้อที่ดินทุกตารางนิ้วในสมัยอิสยาห์ดูไม่ต่างอะไรกับคนยุคนี้ที่หมกมุ่นอยู่กับการสะสมทรัพย์สินจนลืมใช้ชีวิต",
          "sw": "Watu wa wakati wa Isaya waliotaka kumiliki kila kipande cha ardhi ni kama watu wa leo ambao hawatosheki na kile walichonacho.",
          "it": "Quei personaggi dei tempi di Isaia che volevano accumulare tutto sono fondamentalmente i nostri antenati della frenesia lavorativa moderna."
        }
    },
    "2026-09-18": {
        date: "2026-09-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 7:1-25",
          "en": "Isaiah 7:1-25",
          "ko": "Isaiah 7:1-25",
          "zho": "イザヤ書 7:1-25",
          "es": "Isaiah 7:1-25",
          "pt": "Isaiah 7:1-25",
          "vi": "Isaiah 7:1-25",
          "tl": "Isaiah 7:1-25",
          "th": "Isaiah 7:1-25",
          "sw": "Isaiah 7:1-25",
          "it": "Isaia 7:1-25"
        },
        comment: {
          "ja": "パニックでガチガチに震える王様に「とりあえず落ち着いて深呼吸しなよ」と諭すイザヤ、完全にメンタルケアの専門家。",
          "en": "Isaiah telling a terrified king to just take a deep breath and chill out is the exact energy of a therapist trying to handle a full-blown meltdown.",
          "ko": "공포에 질려 벌벌 떠는 왕에게 일단 진정하고 숨부터 쉬라고 다독이는 이사야는 거의 전문 심리 상담사 수준이다.",
          "zho": "看著國王嚇得魂飛魄散，以賽亞還能冷靜地叫他深呼吸放輕鬆，這簡直就是最高級別的心理諮商技巧。",
          "es": "Ver a Isaías diciéndole a un rey aterrorizado que se calme y respire profundo es como ver a un terapeuta tratando de manejar una crisis nerviosa.",
          "pt": "O Isaías dizendo para o rei apavorado apenas respirar fundo e relaxar é exatamente a energia de um terapeuta tentando lidar com um colapso emocional.",
          "vi": "Việc Ê-sai khuyên vị vua đang hoảng loạn hãy bình tĩnh và hít thở sâu giống như một chuyên gia tâm lý đang cố gắng trấn an một người sắp suy sụp.",
          "tl": "Ang payo ni Isaias sa takot na takot na hari na huminga nang malalim at magpakalma ay parang isang therapist na sinusubukang ayusin ang mental breakdown ng pasyente.",
          "th": "การที่อิสยาห์บอกให้กษัตริย์ที่กำลังขวัญเสียใจเย็นลงและหายใจเข้าลึกๆ นั้นช่างเหมือนกับนักจิตวิทยาที่กำลังรับมือกับคนที่กำลังสติแตกสุดๆ",
          "sw": "Isaya kumwambia mfalme aliyejawa na hofu apumue kwa kina na atulie ni kama mshauri anayejaribu kumtuliza mtu aliyepagawa na msongo wa mawazo.",
          "it": "Vedere Isaia che dice a un re terrorizzato di calmarsi e respirare profondo è come avere un terapeuta che cerca di gestire una crisi di nervi."
        }
    },
    "2026-09-19": {
        date: "2026-09-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 9:1-21",
          "en": "Isaiah 9:1-21",
          "ko": "Isaiah 9:1-21",
          "zho": "イザヤ書 9:1-21",
          "es": "Isaiah 9:1-21",
          "pt": "Isaiah 9:1-21",
          "vi": "Isaiah 9:1-21",
          "tl": "Isaiah 9:1-21",
          "th": "Isaiah 9:1-21",
          "sw": "Isaiah 9:1-21",
          "it": "Isaia 9:1-21"
        },
        comment: {
          "ja": "暗闇の中で光を待ちわびるイザヤの言葉は、まるで深夜のSNSで救いを求めてスクロールし続ける私たちの姿そのものだ。",
          "en": "Isaiah talking about waiting for light in the darkness feels exactly like scrolling through social media at 3 AM looking for a glimmer of hope.",
          "ko": "어둠 속에서 빛을 기다리는 이사야의 외침은 마치 새벽에 잠 못 이루며 SNS를 뒤지는 우리의 모습과 너무나 닮았다.",
          "zho": "以賽亞在黑暗中渴求光明的描述，簡直就像我們在深夜裡刷著手機尋求慰藉的真實寫照。",
          "es": "La espera de luz en medio de la oscuridad que describe Isaías se siente igual que cuando buscamos esperanza navegando por el móvil a medianoche.",
          "pt": "A expectativa por uma luz em meio à escuridão descrita por Isaías é igual à nossa busca por esperança rolando o feed do celular na madrugada.",
          "vi": "Lời của Ê-sai về việc chờ đợi ánh sáng trong bóng tối giống hệt cảm giác chúng ta lướt mạng xã hội lúc nửa đêm để tìm chút hy vọng.",
          "tl": "Ang paghihintay ni Isaias ng liwanag sa gitna ng dilim ay parang tayo na nag-i-scroll sa social media sa madaling araw para humanap ng karamay.",
          "th": "คำพูดของอิสยาห์ที่รอคอยแสงสว่างในความมืดดูเหมือนการที่เราไถฟีดโซเชียลในยามดึกเพื่อหาความหวังเล็กๆ น้อยๆ ในชีวิต",
          "sw": "Maneno ya Isaya kuhusu kusubiri nuru gizani ni kama sisi tunavyokaa usiku kucha kwenye simu tukitafuta tumaini.",
          "it": "Attendere la luce nel buio descritto da Isaia sembra esattamente quando scorriamo lo schermo del telefono a notte fonda in cerca di speranza."
        }
    },
    "2026-09-20": {
        date: "2026-09-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 10:20-34",
          "en": "Isaiah 10:20-34",
          "ko": "Isaiah 10:20-34",
          "zho": "イザヤ書 10:20-34",
          "es": "Isaiah 10:20-34",
          "pt": "Isaiah 10:20-34",
          "vi": "Isaiah 10:20-34",
          "tl": "Isaiah 10:20-34",
          "th": "Isaiah 10:20-34",
          "sw": "Isaiah 10:20-34",
          "it": "Isaia 10:20-34"
        },
        comment: {
          "ja": "散々強がっていた人間が最終的に神様を頼るしかないと悟るまでのプロセス、どれだけ遠回りすれば気が済むのかと自分に問いかけたくなる。",
          "en": "Watching people insist on doing things their own way until they finally hit a wall and turn to God is a painful reminder of how much we love the scenic route.",
          "ko": "끝까지 고집을 피우다가 결국 하나님께 돌아오는 사람들의 모습을 보면, 우리 인생이 얼마나 비효율적인 우회로를 즐기는지 실감하게 된다.",
          "zho": "看著人們在撞牆之後才學會轉向神，不得不感嘆我們人類為了繞遠路所付出的努力真是驚人。",
          "es": "Ver a la gente insistir en sus propios planes hasta que no les queda otra que volver a Dios es un recordatorio de lo mucho que nos gusta complicarnos la vida.",
          "pt": "Ver as pessoas insistirem em seus próprios caminhos até finalmente se voltarem para Deus é um lembrete de como amamos complicar o que deveria ser simples.",
          "vi": "Chứng kiến cảnh con người cứ cố chấp làm theo ý mình cho đến khi bế tắc mới chịu quay về với Chúa, thật là một bài học đắt giá về việc đi đường vòng.",
          "tl": "Ang makita ang mga tao na pilit ginagawa ang gusto nila hanggang sa wala na silang mapuntahan kundi ang bumalik sa Diyos ay nakakapagod na paalala kung gaano tayo mahilig magpaligoy-ligoy.",
          "th": "การเห็นผู้คนดื้อรั้นทำตามใจตัวเองจนสุดท้ายต้องหันกลับมาหาพระเจ้า ทำให้รู้เลยว่าเราชอบเลือกเส้นทางที่อ้อมโลกแค่ไหน",
          "sw": "Kuona watu wakisisitiza njia zao wenyewe hadi mwisho wanapomgeukia Mungu inatukumbusha jinsi tunavyopenda njia ndefu na ngumu bila sababu.",
          "it": "Vedere la gente insistere sui propri piani fino a sbattere contro un muro ci ricorda quanto amiamo complicarci la vita."
        }
    },
    "2026-09-21": {
        date: "2026-09-21",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 13:1-22",
          "en": "Isaiah 13:1-22",
          "ko": "Isaiah 13:1-22",
          "zho": "イザヤ書 13:1-22",
          "es": "Isaiah 13:1-22",
          "pt": "Isaiah 13:1-22",
          "vi": "Isaiah 13:1-22",
          "tl": "Isaiah 13:1-22",
          "th": "Isaiah 13:1-22",
          "sw": "Isaiah 13:1-22",
          "it": "Isaia 13:1-22"
        },
        comment: {
          "ja": "バビロンの崩壊を予告するイザヤの言葉は、どれほど強大な組織や権力も時代の流れには勝てないという歴史の無情さを突きつけてくる。",
          "en": "Isaiah predicting Babylon's fall reminds us that no matter how big the empire, history always has a way of hitting the reset button.",
          "ko": "바빌론의 멸망을 예고하는 이사야의 메시지는 아무리 거대한 권력이라도 세월의 흐름 앞에서는 그저 찰나일 뿐이라는 사실을 다시금 깨닫게 한다.",
          "zho": "以賽亞預言巴比倫的崩塌，提醒我們無論勢力多麼龐大，在歷史的洪流面前終究只是過眼雲煙。",
          "es": "La profecía de Isaías sobre la caída de Babilonia nos recuerda que, por más poderoso que sea un imperio, la historia siempre termina pasando factura.",
          "pt": "A profecia de Isaías sobre a queda da Babilônia é um lembrete de que, não importa o tamanho do império, a história sempre cobra seu preço.",
          "vi": "Lời tiên tri của Ê-sai về sự sụp đổ của Ba-by-lôn nhắc nhở chúng ta rằng dù đế chế có hùng mạnh đến đâu, lịch sử vẫn luôn có cách để đưa mọi thứ trở về con số không.",
          "tl": "Ang hula ni Isaias tungkol sa pagbagsak ng Babilonia ay paalala na gaano man kalaki ang isang imperyo, sa huli ay may hangganan din ang lahat.",
          "th": "คำพยากรณ์ของอิสยาห์เรื่องบาบิโลนล่มสลายเตือนใจเราว่า ไม่ว่าอาณาจักรจะยิ่งใหญ่เพียงใด สุดท้ายก็หนีไม่พ้นกฎแห่งความไม่เที่ยงของประวัติศาสตร์",
          "sw": "Utabiri wa Isaya kuhusu kuanguka kwa Babeli unatukumbusha kwamba hata mamlaka iwe kubwa kiasi gani, historia ina njia yake ya kuonyesha kwamba hakuna kinachodumu milele.",
          "it": "La profezia sulla caduta di Babilonia ci ricorda che per quanto grande sia un impero, la storia trova sempre il modo di resettarsi."
        }
    },
    "2026-09-22": {
        date: "2026-09-22",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 14:1-32",
          "en": "Isaiah 14:1-32",
          "ko": "Isaiah 14:1-32",
          "zho": "イザヤ書 14:1-32",
          "es": "Isaiah 14:1-32",
          "pt": "Isaiah 14:1-32",
          "vi": "Isaiah 14:1-32",
          "tl": "Isaiah 14:1-32",
          "th": "Isaiah 14:1-32",
          "sw": "Isaiah 14:1-32",
          "it": "Isaia 14:1-32"
        },
        comment: {
          "ja": "天から落ちた明けの明星のように、高慢のあまり頂点から転がり落ちる姿は、いつの時代も変わらない人間の悲喜劇だ。",
          "en": "Watching someone fall from grace due to their own pride is a timeless tragedy that repeats itself in every generation.",
          "ko": "하늘에서 떨어진 새벽별처럼, 오만함 때문에 정상에서 추락하는 모습은 시대를 막론하고 되풀이되는 인간의 비극이다.",
          "zho": "看著人因驕傲而從巔峰墜落，就像那顆殞落的晨星，這齣人間悲劇在每個時代都如出一轍。",
          "es": "Ver a alguien caer desde la cima por culpa de su propio orgullo es la tragedia humana que se repite en cada generación.",
          "pt": "Ver alguém cair do topo por causa do próprio orgulho é uma tragédia humana que se repete em todas as gerações.",
          "vi": "Chứng kiến một người rơi từ đỉnh cao xuống vì kiêu ngạo, giống như ngôi sao mai rụng xuống, là tấn bi kịch nhân loại vẫn lặp lại qua bao thế hệ.",
          "tl": "Ang makitang mahulog ang isang tao mula sa tuktok dahil sa sobrang pagmamalaki ay isang trahedyang paulit-ulit na nangyayari sa bawat henerasyon.",
          "th": "การได้เห็นใครสักคนตกจากจุดสูงสุดเพียงเพราะความหยิ่งผยองนั้น เป็นโศกนาฏกรรมที่วนเวียนซ้ำซากในทุกยุคสมัยจริงๆ",
          "sw": "Kuona mtu akianguka kutoka kileleni kwa sababu ya kiburi chake ni ile ile huzuni ya kibinadamu inayojirudia katika kila kizazi.",
          "it": "Vedere qualcuno cadere dall'alto a causa della propria superbia è la classica tragedia che si ripete in ogni generazione."
        }
    },
    "2026-09-23": {
        date: "2026-09-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 22:1-25",
          "en": "Isaiah 22:1-25",
          "ko": "Isaiah 22:1-25",
          "zho": "イザヤ書 22:1-25",
          "es": "Isaiah 22:1-25",
          "pt": "Isaiah 22:1-25",
          "vi": "Isaiah 22:1-25",
          "tl": "Isaiah 22:1-25",
          "th": "Isaiah 22:1-25",
          "sw": "Isaiah 22:1-25",
          "it": "Isaia 22:1-25"
        },
        comment: {
          "ja": "神殿の奥底までゴミ溜めのように腐敗させるシェブナという男、地位に執着して自分の墓を立派に作りすぎるあたりが人間臭くて生々しい。",
          "en": "Shebna carving out a fancy tomb for himself while the city is falling apart is the ultimate example of misplaced priorities and ego.",
          "ko": "성전 깊숙한 곳까지 부패하게 만든 셉나라는 인물, 몰락 직전까지 자기 무덤 화려하게 꾸미기에 급급한 모습이 인간의 헛된 욕망을 그대로 보여준다.",
          "zho": "舍伯那在城市崩塌前夕还忙着给自己修筑豪华坟墓，这种将私欲置于大局之上的执着真是让人无奈又讽刺。",
          "es": "Shebna preocupado por construir su lujosa tumba mientras la ciudad se desmorona es el ejemplo perfecto de tener las prioridades totalmente al revés.",
          "pt": "Shebna preocupado em construir seu túmulo luxuoso enquanto a cidade colapsa é o exemplo perfeito de prioridades completamente invertidas.",
          "vi": "Shebna bận rộn xây dựng ngôi mộ xa hoa cho chính mình trong khi thành phố đang sụp đổ là ví dụ điển hình về việc đặt ưu tiên sai chỗ.",
          "tl": "Si Shebna na abala sa pagpapagawa ng magarbong libingan habang guguho na ang lungsod ay malinaw na halimbawa ng maling prayoridad.",
          "th": "เชบนาที่มัวแต่สร้างหลุมศพหรูหราให้ตัวเองในขณะที่เมืองกำลังจะล่มสลาย คือตัวอย่างชัดเจนของการให้ความสำคัญกับสิ่งที่ไม่ควรให้.",
          "sw": "Shebna kushughulika na kujijengea kaburi la kifahari wakati mji unaporomoka ni mfano kamili wa kuweka kipaumbele kwenye mambo yasiyo na maana.",
          "it": "Preoccuparsi di costruire una tomba lussuosa mentre la città cade a pezzi è l'esempio perfetto di avere le priorità totalmente sottosopra."
        }
    },
    "2026-09-24": {
        date: "2026-09-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 24:1-23",
          "en": "Isaiah 24:1-23",
          "ko": "Isaiah 24:1-23",
          "zho": "イザヤ書 24:1-23",
          "es": "Isaiah 24:1-23",
          "pt": "Isaiah 24:1-23",
          "vi": "Isaiah 24:1-23",
          "tl": "Isaiah 24:1-23",
          "th": "Isaiah 24:1-23",
          "sw": "Isaiah 24:1-23",
          "it": "Isaia 24:1-23"
        },
        comment: {
          "ja": "地球全体が揺れ動くような大混乱の中でも、結局は自分の楽しみや損得を優先してしまう人間の懲りなさはある意味で最強。",
          "en": "Even when the whole world is shaking, people still prioritize their own petty comforts and gains, which is honestly a testament to our stubborn nature.",
          "ko": "온 세상이 흔들리는 대혼란 속에서도 끝까지 자기 이익과 즐거움을 포기하지 않는 인간의 끈질긴 고집은 정말 대단할 지경이다.",
          "zho": "即便世界天翻地覆，人类依然能在那一瞬间优先考虑自己的那点小算盘，这种执迷不悟的程度真是让人叹为观止。",
          "es": "Incluso cuando el mundo entero se estremece, los humanos seguimos priorizando nuestras pequeñas comodidades, y esa terquedad es realmente impresionante.",
          "pt": "Mesmo quando o mundo inteiro treme, os humanos continuam priorizando seus pequenos confortos, e essa teimosia é realmente impressionante.",
          "vi": "Ngay cả khi cả thế giới rung chuyển, con người vẫn ưu tiên những lợi ích nhỏ nhen của chính mình, sự ngoan cố đó thật đáng kinh ngạc.",
          "tl": "Kahit yumanig na ang buong mundo, inuuna pa rin ng tao ang sarili nilang interes, na nagpapatunay lang kung gaano tayo katigas ang ulo.",
          "th": "แม้โลกจะสั่นสะเทือนไปทั่ว แต่ผู้คนก็ยังคงให้ความสำคัญกับความสะดวกสบายเล็กๆ น้อยๆ ของตัวเอง ความดื้อรั้นนี้ช่างน่าทึ่งจริงๆ",
          "sw": "Hata wakati ulimwengu unatetemeka, wanadamu bado wanatanguliza starehe zao ndogo, na ukaidi huo ni wa kushangaza kweli.",
          "it": "Anche quando il mondo trema, riusciamo sempre a pensare ai nostri piccoli interessi personali con una caparbietà impressionante."
        }
    },
    "2026-09-25": {
        date: "2026-09-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 26:1-21",
          "en": "Isaiah 26:1-21",
          "ko": "Isaiah 26:1-21",
          "zho": "イザヤ書 26:1-21",
          "es": "Isaiah 26:1-21",
          "pt": "Isaiah 26:1-21",
          "vi": "Isaiah 26:1-21",
          "tl": "Isaiah 26:1-21",
          "th": "Isaiah 26:1-21",
          "sw": "Isaiah 26:1-21",
          "it": "Isaia 26:1-21"
        },
        comment: {
          "ja": "どれだけ隠れて悪巧みをしても神様には全部筒抜けだと知ったとき、自分の検索履歴を見られたときのような冷や汗が出る。",
          "en": "Realizing God sees everything we try to hide is like suddenly having your browser history projected on a giant screen.",
          "ko": "남몰래 꼼수를 부려도 하나님께는 다 들통난다는 사실은 마치 누군가 내 검색 기록을 훔쳐보는 듯한 서늘함을 준다.",
          "zho": "意识到无论怎么躲藏都瞒不过上帝，那种感觉就像是被人当众公开了你的搜索记录一样尴尬。",
          "es": "Saber que Dios ve todo lo que intentamos ocultar da la misma vergüenza que si alguien leyera nuestro historial de búsqueda en voz alta.",
          "pt": "Saber que Deus vê tudo o que tentamos esconder dá o mesmo frio na barriga de quando alguém abre o seu histórico de navegação.",
          "vi": "Nhận ra Chúa thấy hết mọi thứ ta cố giấu cũng giống như cảm giác ai đó bất ngờ đọc to lịch sử duyệt web của mình vậy.",
          "tl": "Ang malaman na nakikita ng Diyos ang lahat ng tinatago natin ay parang biglang binabasa ang iyong browser history sa harap ng maraming tao.",
          "th": "การรู้ว่าพระเจ้าทรงเห็นทุกอย่างที่เราพยายามซ่อนไว้ ให้ความรู้สึกเหมือนมีคนมาเปิดประวัติการเข้าชมเว็บไซต์ของเราให้อ่านกลางที่สาธารณะ",
          "sw": "Kujua kuwa Mungu anaona kila kitu tunachojaribu kuficha ni kama mtu kusoma historia yako ya mtandao mbele za watu wote.",
          "it": "Realizzare che Dio vede tutto quello che cerchiamo di nascondere è come scoprire che qualcuno ha proiettato la cronologia del nostro browser su uno schermo gigante."
        }
    },
    "2026-09-26": {
        date: "2026-09-26",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 28:1-29",
          "en": "Isaiah 28:1-29",
          "ko": "Isaiah 28:1-29",
          "zho": "イザヤ書 28:1-29",
          "es": "Isaiah 28:1-29",
          "pt": "Isaiah 28:1-29",
          "vi": "Isaiah 28:1-29",
          "tl": "Isaiah 28:1-29",
          "th": "Isaiah 28:1-29",
          "sw": "Isaiah 28:1-29",
          "it": "Isaia 28:1-29"
        },
        comment: {
          "ja": "酔っ払ってふらついている祭司たちを見てイザヤが呆れている様子、現代のニュース番組を観ているときの感覚と大差ない。",
          "en": "Isaiah watching the priests stumble around drunk is basically how we all feel watching the evening news these days.",
          "ko": "술에 취해 비틀거리는 제사장들을 보며 한탄하는 이사야의 모습은 요즘 뉴스 프로그램을 볼 때의 내 표정과 다를 게 없다.",
          "zho": "以赛亚看着那些醉得东倒西歪的祭司们感到无奈，这感觉简直和我们看现代新闻节目时一模一样。",
          "es": "Ver a Isaías frustrado por los sacerdotes tambaleándose es exactamente como nos sentimos todos al ver las noticias hoy en día.",
          "pt": "Ver Isaías frustrado com os sacerdotes cambaleando bêbados é exatamente como nos sentimos ao assistir ao noticiário hoje em dia.",
          "vi": "Chứng kiến Ê-sai ngao ngán nhìn các thầy tế lễ say xỉn loạng choạng cũng giống hệt tâm trạng chúng ta khi xem bản tin thời sự thời nay.",
          "tl": "Ang pagkadismaya ni Isaias sa mga paring lasing na naglalakad nang pasuray-suray ay eksaktong nararamdaman natin kapag nanonood ng balita ngayon.",
          "th": "การเห็นอิสยาห์เอือมระอากับบรรดาปุโรหิตที่เมามายจนเดินเซนั้น ไม่ต่างอะไรกับความรู้สึกของเราเวลาดูข่าวในทุกวันนี้เลย",
          "sw": "Kuona Isaya akiwa amekata tamaa na makuhani wanaotembea kwa kulewa ni kama jinsi tunavyohisi tunapotazama habari siku hizi.",
          "it": "Vedere Isaia frustrato dai sacerdoti che barcollano è esattamente la stessa sensazione che proviamo noi oggi guardando il telegiornale la sera."
        }
    },
    "2026-09-27": {
        date: "2026-09-27",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 30:1-33",
          "en": "Isaiah 30:1-33",
          "ko": "Isaiah 30:1-33",
          "zho": "イザヤ書 30:1-33",
          "es": "Isaiah 30:1-33",
          "pt": "Isaiah 30:1-33",
          "vi": "Isaiah 30:1-33",
          "tl": "Isaiah 30:1-33",
          "th": "Isaiah 30:1-33",
          "sw": "Isaiah 30:1-33",
          "it": "Isaia 30:1-33"
        },
        comment: {
          "ja": "助けを求めてエジプトに走るイザヤの時代の民、自分に都合のいい解決策を求めて結局遠回りをする私たちの姿そのものだ。",
          "en": "Running to Egypt for help instead of trusting the source is the ancient equivalent of doomscrolling for answers you know you shouldn't rely on.",
          "ko": "도움을 구하러 이집트로 달려가는 백성들의 모습은 정답을 알면서도 굳이 먼 길을 돌아가는 우리의 고집과 참 닮았다.",
          "zho": "向埃及寻求帮助的百姓，就像是我们明明知道正解却偏偏要走弯路的顽固写照。",
          "es": "Correr hacia Egipto buscando ayuda es el equivalente antiguo de intentar resolver todo a nuestra manera para terminar complicándonos más la vida.",
          "pt": "Correr para o Egito em busca de ajuda é o equivalente antigo de tentar resolver tudo do nosso jeito e acabar complicando ainda mais a vida.",
          "vi": "Chạy sang Ai Cập cầu cứu là hình ảnh phản chiếu sự cố chấp của con người khi cứ muốn tìm lối tắt thay vì tin vào giải pháp đơn giản nhất.",
          "tl": "Ang pagtakbo sa Egipto para humingi ng tulong ay parang tayo lang na pilit naghahanap ng shortcut kahit alam nating mali ang direksyon.",
          "th": "การวิ่งไปหาอียิปต์เพื่อขอความช่วยเหลือก็เหมือนเราที่พยายามหาทางลัดแบบผิดๆ ทั้งที่รู้อยู่เต็มอกว่านั่นไม่ใช่คำตอบ",
          "sw": "Kukimbilia Misri kutafuta msaada ni kama sisi tunavyojaribu kutafuta njia za mkato zenye shaka badala ya kutegemea suluhisho la kweli.",
          "it": "Correre in Egitto a cercare aiuto è l'equivalente antico di cercare soluzioni su internet sapendo benissimo che non dovremmo farlo."
        }
    },
    "2026-09-28": {
        date: "2026-09-28",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 40:1-31",
          "en": "Isaiah 40:1-31",
          "ko": "Isaiah 40:1-31",
          "zho": "イザヤ書 40:1-31",
          "es": "Isaiah 40:1-31",
          "pt": "Isaiah 40:1-31",
          "vi": "Isaiah 40:1-31",
          "tl": "Isaiah 40:1-31",
          "th": "Isaiah 40:1-31",
          "sw": "Isaiah 40:1-31",
          "it": "Isaia 40:1-31"
        },
        comment: {
          "ja": "草のようにすぐ枯れる人間の儚さを嘆きつつ、それでも星の数ほどある約束を信じようとするこの緩急の激しさが人生のリアルだ。",
          "en": "Lamenting how quickly we wither like grass while trying to believe in eternal promises is the ultimate emotional rollercoaster of being human.",
          "ko": "풀처럼 금방 시들어버리는 인간의 나약함을 한탄하면서도 영원한 약속을 붙들려 애쓰는 이 모습이 바로 우리네 인생이다.",
          "zho": "感叹人生如草般脆弱，却又试图抓住永恒的应许，这种反差正是人类最真实的挣扎。",
          "es": "Lamentar que somos como la hierba que se marchita mientras intentamos creer en promesas eternas es la montaña rusa emocional de nuestra existencia.",
          "pt": "Lamentar que somos como a erva que murcha enquanto tentamos acreditar em promessas eternas é a montanha-russa emocional da nossa existência.",
          "vi": "Than thở về thân phận mong manh như cỏ dại mà vẫn cố tin vào những lời hứa vĩnh cửu chính là bản chất đầy mâu thuẫn của con người.",
          "tl": "Ang pagdadalamhati sa pagiging marupok natin gaya ng damo habang pilit na kumakapit sa mga pangakong walang hanggan ang tunay na drama ng buhay.",
          "th": "การบ่นว่าชีวิตเปราะบางเหมือนต้นหญ้าแต่ยังพยายามเชื่อในคำสัญญาที่ยิ่งใหญ่ คือความย้อนแย้งที่น่าเห็นใจของความเป็นมนุษย์",
          "sw": "Kulalamika kuwa sisi ni kama nyasi zinazonyauka huku tukijaribu kuamini ahadi za milele ndiyo hali halisi ya maisha yetu ya kila siku.",
          "it": "Lamentarci di essere fragili come l'erba mentre cerchiamo di credere a promesse eterne è la perfetta montagna rusa emotiva dell'essere umani."
        }
    },
    "2026-09-29": {
        date: "2026-09-29",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 41:1-29",
          "en": "Isaiah 41:1-29",
          "ko": "Isaiah 41:1-29",
          "zho": "イザヤ書 41:1-29",
          "es": "Isaiah 41:1-29",
          "pt": "Isaiah 41:1-29",
          "vi": "Isaiah 41:1-29",
          "tl": "Isaiah 41:1-29",
          "th": "Isaiah 41:1-29",
          "sw": "Isaiah 41:1-29",
          "it": "Isaia 41:1-29"
        },
        comment: {
          "ja": "神様が隣にいるのに、必死で木や石の偶像をこねて安心しようとする人々の姿、現代のスマホ中毒と重なって笑えない。",
          "en": "People frantically crafting idols out of wood and stone while God is right there is basically the ancient version of doomscrolling for validation.",
          "ko": "하나님이 바로 곁에 계시는데도 나무와 돌로 우상을 깎으며 안심하려는 모습은, 현대인의 스마트폰 중독과 묘하게 겹쳐서 씁쓸하다.",
          "zho": "明明神就在身边，人们却还在拼命雕刻木头和石头来寻求安慰，这简直就是现代人焦虑症的写照。",
          "es": "Ver a la gente fabricando ídolos de madera y piedra para sentirse seguros cuando Dios está justo ahí, es como ver a alguien buscando validación en redes sociales en lugar de mirar a quien tiene al lado.",
          "pt": "Ver o povo tentando encontrar segurança em ídolos de madeira e pedra quando Deus está ali, é como ver alguém tentando achar paz no feed das redes sociais enquanto ignora a realidade.",
          "vi": "Việc con người cố gắng tìm sự an tâm nơi những bức tượng gỗ đá khi Chúa đang hiện diện ngay đó, cũng giống như cách chúng ta tìm kiếm sự an ủi trong thế giới ảo thay vì đời thực.",
          "tl": "Ang pagpilit ng mga tao na humanap ng kapanatagan sa mga idolo habang nasa tabi na nila ang Diyos ay parang paghahanap ng validation sa social media sa halip na sa totoong buhay.",
          "th": "การที่ผู้คนพยายามหาความอุ่นใจจากรูปเคารพที่ทำจากไม้และหินทั้งที่พระเจ้าอยู่ตรงนั้น ก็เหมือนกับการที่เราพยายามหาความมั่นใจจากยอดไลก์แทนที่จะมองหาความจริงข้างๆ ตัว",
          "sw": "Watu kujaribu kutafuta usalama kwenye sanamu za mbao na mawe wakati Mungu yupo hapo hapo ni kama mtu anayetafuta faraja kwenye mitandao ya kijamii badala ya maisha halisi.",
          "it": "Vedere la gente costruire idoli per sentirsi sicura quando Dio è proprio lì accanto è come passare ore sui social cercando conferme al posto di guardare la realtà."
        }
    },
    "2026-09-30": {
        date: "2026-09-30",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 42:10-25",
          "en": "Isaiah 42:10-25",
          "ko": "Isaiah 42:10-25",
          "zho": "イザヤ書 42:10-25",
          "es": "Isaiah 42:10-25",
          "pt": "Isaiah 42:10-25",
          "vi": "Isaiah 42:10-25",
          "tl": "Isaiah 42:10-25",
          "th": "Isaiah 42:10-25",
          "sw": "Isaiah 42:10-25",
          "it": "Isaia 42:10-25"
        },
        comment: {
          "ja": "散々手助けを無視して勝手に失敗しては「なんで助けてくれないんだ」と文句を言う、思春期の反抗期みたいな人類の歴史がここに詰まっている。",
          "en": "Ignoring all the help offered, messing up, and then blaming God for not preventing the disaster is the classic rebellious teenager phase of humanity.",
          "ko": "도움을 다 거절하고 사고를 친 뒤에 왜 안 도와주냐고 따지는 모습에서, 인류의 끝없는 사춘기적 반항심이 느껴진다.",
          "zho": "拒绝所有帮助却在闯祸后抱怨神为何不干预，这简直是人类历史上最典型的叛逆期行为。",
          "es": "Ignorar toda la ayuda, meter la pata y luego reclamarle a Dios por qué no intervino es básicamente la fase de rebeldía adolescente de toda la humanidad.",
          "pt": "Ignorar toda a ajuda, fazer besteira e depois reclamar que Deus não impediu o desastre é a fase de rebeldia adolescente mais clássica da história da humanidade.",
          "vi": "Việc từ chối mọi sự giúp đỡ rồi quay sang trách móc Chúa khi gặp rắc rối chẳng khác nào thái độ nổi loạn của một đứa trẻ tuổi teen đối với cha mẹ.",
          "tl": "Ang pagtanggi sa tulong at pagkatapos ay paninisi sa Diyos kapag nagkamali na ay parang tipikal na yugto ng pagrerebelde ng isang tinedyer.",
          "th": "การปฏิเสธความช่วยเหลือทุกอย่างแล้วพอทำพังก็หันมาโทษพระเจ้าว่าทำไมไม่ช่วย คือพฤติกรรมวัยรุ่นใจแตกของมนุษยชาติที่วนลูปไม่จบสิ้น",
          "sw": "Kukataa msaada wote na kisha kumlaumu Mungu baada ya kufanya makosa ni kama hatua ya uasi ya kijana mdogo katika historia ya ubinadamu.",
          "it": "Ignorare ogni aiuto, combinare un disastro e poi prendersela con Dio perché non è intervenuto è la classica fase adolescenziale di tutta l'umanità."
        }
    },
    "2026-10-01": {
        date: "2026-10-01",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 44:1-28",
          "en": "Isaiah 44:1-28",
          "ko": "Isaiah 44:1-28",
          "zho": "イザヤ書 44:1-28",
          "es": "Isaiah 44:1-28",
          "pt": "Isaiah 44:1-28",
          "vi": "Isaiah 44:1-28",
          "tl": "Isaiah 44:1-28",
          "th": "Isaiah 44:1-28",
          "sw": "Isaiah 44:1-28",
          "it": "Isaia 44:1-28"
        },
        comment: {
          "ja": "木を切って半分で暖を取り残り半分で神像を作るイザヤの皮肉、現代の私たちが流行り廃りに右往左往する姿と重なって笑えない。",
          "en": "Isaiah pointing out the irony of using half a log for firewood and the other half for an idol hits a little too close to home regarding our own misplaced priorities.",
          "ko": "나무를 잘라 절반은 불을 피우고 나머지 절반은 신상으로 만드는 이사야의 풍자는, 유행에 따라 가치관을 바꾸는 현대인의 모습을 보는 것 같아 뼈아프다.",
          "zho": "以賽亞嘲諷那些用一半木頭生火、另一半卻拿來當偶像拜的人，這簡直就是現代人對於價值觀隨波逐流的最佳寫照。",
          "es": "La ironía de usar la mitad de un tronco para calentarse y la otra mitad para hacer un ídolo nos recuerda lo absurdo que es a veces nuestro sentido de prioridad.",
          "pt": "A ironia de usar metade de um tronco para se aquecer e a outra metade para esculpir um ídolo mostra o quanto somos bons em complicar o que deveria ser óbvio.",
          "vi": "Sự châm biếm của Ê-sai về việc dùng một nửa khúc gỗ để sưởi ấm và nửa kia để làm thần tượng thật sự là một lời nhắc nhở về những ưu tiên sai lầm của con người.",
          "tl": "Ang paggamit ng kalahati ng kahoy para pampainit at ang kalahati naman ay para sa rebulto ay isang nakakatawang paalala kung gaano tayo kadalas magkamali sa ating mga prayoridad.",
          "th": "การที่อิสยาห์ล้อเลียนคนที่เอาไม้ไปทำฟืนครึ่งหนึ่งแล้วเอาอีกครึ่งไปทำรูปเคารพนั้น ช่างจี้จุดเรื่องความย้อนแย้งในใจคนเราได้ดีเหลือเกิน",
          "sw": "Isaya anapohoji upuuzi wa kutumia nusu ya mti kwa kupasha moto na nusu nyingine kwa kuabudu sanamu, anatukumbusha jinsi tunavyopoteza mwelekeo kirahisi.",
          "it": "L'ironia di usare metà tronco per scaldarsi e l'altra metà per fare un idolo ci ricorda quanto siano assurde le nostre priorità a volte."
        }
    },
    "2026-10-02": {
        date: "2026-10-02",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 45:14-25",
          "en": "Isaiah 45:14-25",
          "ko": "Isaiah 45:14-25",
          "zho": "イザヤ書 45:14-25",
          "es": "Isaiah 45:14-25",
          "pt": "Isaiah 45:14-25",
          "vi": "Isaiah 45:14-25",
          "tl": "Isaiah 45:14-25",
          "th": "Isaiah 45:14-25",
          "sw": "Isaiah 45:14-25",
          "it": "Isaia 45:14-25"
        },
        comment: {
          "ja": "神様が隠れているように見える時ほど、実は一番近くで導いているという逆転の発想、人生の迷子には一番必要な処方箋かもしれない。",
          "en": "The idea that God is hiding just when He is actually guiding us most closely is the kind of plot twist we all need to remember when life gets confusing.",
          "ko": "하나님께서 숨어 계신 것처럼 느껴질 때가 사실은 가장 가까이서 인도하고 계실 때라는 역설, 삶이 막막할 때 꼭 기억해야 할 반전이다.",
          "zho": "神看似隱藏的時候，往往正是祂引領最深的時候，這種反轉思維簡直是人生迷茫時的最佳止痛藥。",
          "es": "Esa idea de que Dios parece estar oculto justo cuando más nos está guiando es el giro de guion que todos necesitamos cuando la vida se pone difícil.",
          "pt": "A ideia de que Deus parece escondido justamente quando está nos guiando de perto é o tipo de reviravolta que precisamos lembrar quando tudo parece confuso.",
          "vi": "Việc Chúa dường như ẩn mình ngay lúc Ngài đang dẫn dắt chúng ta sát sao nhất chính là một cú ngoặt đầy hy vọng cho những lúc ta thấy lạc lối.",
          "tl": "Ang katotohanan na tila nagtatago ang Diyos sa sandaling Siya ang pinakamalapit na gumagabay sa atin ay isang mahalagang paalala sa tuwing tayo ay naliligaw.",
          "th": "ความจริงที่ว่าพระเจ้าดูเหมือนจะซ่อนตัวอยู่ทั้งที่จริงแล้วพระองค์กำลังนำทางเราอยู่นั้น เป็นบทเรียนที่ช่วยปลอบใจได้ดีเหลือเกินในวันที่ชีวิตดูมืดแปดด้าน",
          "sw": "Wazo la kwamba Mungu anaonekana kuwa amejificha wakati ndipo anapotuongoza kwa karibu zaidi ni ukweli tunaohitaji kukumbuka tunapohisi tumepotea.",
          "it": "L'idea che Dio sembri nascosto proprio mentre ci sta guidando più da vicino è quel colpo di scena che dovremmo ricordare quando ci sentiamo persi."
        }
    },
    "2026-10-03": {
        date: "2026-10-03",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 48:1-22",
          "en": "Isaiah 48:1-22",
          "ko": "Isaiah 48:1-22",
          "zho": "イザヤ書 48:1-22",
          "es": "Isaiah 48:1-22",
          "pt": "Isaiah 48:1-22",
          "vi": "Isaiah 48:1-22",
          "tl": "Isaiah 48:1-22",
          "th": "Isaiah 48:1-22",
          "sw": "Isaiah 48:1-22",
          "it": "Isaia 48:1-22"
        },
        comment: {
          "ja": "口では立派なことを言いつつ心はどこか遠くにある人たちへの神様の指摘は、耳が痛すぎて直視できない。",
          "en": "God calling out people who talk a big game but have hearts nowhere near the mark is the kind of reality check that hits a little too close to home.",
          "ko": "입으로는 번지르르하게 말하지만 마음은 딴 곳에 있는 우리를 꿰뚫어 보시는 하나님의 지적은 너무나 아프다.",
          "zho": "嘴上說得頭頭是道，心卻離得很遠，上帝這種直戳痛處的提醒簡直讓人無處遁形。",
          "es": "Que Dios nos señale que nuestra boca dice una cosa pero nuestro corazón está en otra parte es un recordatorio incómodamente honesto.",
          "pt": "Deus apontando que nossa boca diz uma coisa enquanto nosso coração está em outro lugar é o tipo de realidade que dói um pouco.",
          "vi": "Chúa chỉ ra rằng miệng chúng ta nói một đằng nhưng lòng lại nghĩ một nẻo, một sự thật phũ phàng mà ai cũng từng mắc phải.",
          "tl": "Ang pagpuna ng Diyos sa mga taong magaling lang sa salita pero malayo ang puso ay isang realidad na masakit tanggapin.",
          "th": "พระเจ้าชี้ให้เห็นว่าปากพูดอย่างหนึ่งแต่ใจไปอีกทาง เป็นความจริงที่กระแทกใจจนแทบไม่อยากยอมรับ",
          "sw": "Mungu kuonyesha kwamba kinywa chetu kinasema kitu kimoja lakini mioyo yetu iko mahali pengine ni ukweli mchungu tunaopaswa kukubali.",
          "it": "Sentirci dire che la nostra bocca dice una cosa ma il nostro cuore è altrove è quel controllo della realtà che ci fa arrossire non poco."
        }
    },
    "2026-10-04": {
        date: "2026-10-04",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 49:1-26",
          "en": "Isaiah 49:1-26",
          "ko": "Isaiah 49:1-26",
          "zho": "イザヤ書 49:1-26",
          "es": "Isaiah 49:1-26",
          "pt": "Isaiah 49:1-26",
          "vi": "Isaiah 49:1-26",
          "tl": "Isaiah 49:1-26",
          "th": "Isaiah 49:1-26",
          "sw": "Isaiah 49:1-26",
          "it": "Isaia 49:1-26"
        },
        comment: {
          "ja": "自分を忘れたかのように落ち込む私に、刺青のように刻まれているから忘れるわけないだろうと諭す神様の愛は、重いけれど温かい。",
          "en": "God telling us he couldn't possibly forget us because he has us engraved on his palms is the ultimate reassurance for when we feel totally invisible.",
          "ko": "내가 나를 잊은 듯 낙담할 때, 손바닥에 새겨두었으니 잊을 리가 없지 않느냐고 하시는 하나님의 말씀은 참 든든하다.",
          "zho": "當我沮喪到以為被遺忘時，上帝說他把我的名字刻在掌心上，這份安全感簡直是神級的安慰。",
          "es": "Sentirse olvidado y recibir la respuesta de que Dios nos lleva grabados en las palmas de sus manos es la dosis de autoestima que todos necesitamos.",
          "pt": "Sentir-se esquecido e ouvir de Deus que Ele nos tem gravados na palma de Suas mãos é o tipo de conforto que cura qualquer insegurança.",
          "vi": "Khi ta cảm thấy bị bỏ rơi và Chúa nhắc rằng Ngài đã khắc tên ta trong lòng bàn tay mình, đó là sự an ủi tuyệt đối cho những lúc ta thấy mình nhỏ bé.",
          "tl": "Ang pakiramdam na nakalimutan ka na ng mundo hanggang sa ipaalala ng Diyos na nakaukit ka sa kanyang mga palad ay sapat na para gumaan ang loob.",
          "th": "ในวันที่รู้สึกเหมือนถูกลืม การได้ยินว่าพระเจ้าสลักชื่อเราไว้ในอุ้งพระหัตถ์เป็นความอุ่นใจที่หาอะไรเปรียบไม่ได้เลย",
          "sw": "Kuhisi kusahaulika na kisha kukumbushwa na Mungu kwamba amekuchora kwenye viganja vya mikono yake ni faraja kuu kwa wakati wowote wa upweke.",
          "it": "Sentirsi dimenticati e sentirsi rispondere che Dio ci ha incisi sulle palme delle sue mani è la dose di affetto che ci rimette in sesto."
        }
    },
    "2026-10-05": {
        date: "2026-10-05",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 50:1-11",
          "en": "Isaiah 50:1-11",
          "ko": "Isaiah 50:1-11",
          "zho": "イザヤ書 50:1-11",
          "es": "Isaiah 50:1-11",
          "pt": "Isaiah 50:1-11",
          "vi": "Isaiah 50:1-11",
          "tl": "Isaiah 50:1-11",
          "th": "Isaiah 50:1-11",
          "sw": "Isaiah 50:1-11",
          "it": "Isaia 50:1-11"
        },
        comment: {
          "ja": "離婚証明書なんてないと言い放つ神様、どれだけ突き放されても離れてくれない元サヤへの執念が重すぎて逆に愛を感じる。",
          "en": "God insisting there is no divorce paper is the ultimate sign that He is just not letting us go, no matter how much we mess up.",
          "ko": "이혼 증서 따위는 없다고 하시는 하나님, 우리가 아무리 못나게 굴어도 끝까지 포기하지 않으시겠다는 의지가 거의 집착 수준이라 오히려 안심된다.",
          "zho": "上帝說根本沒有離婚證書，這種無論怎麼被推開都不願放手的執著，簡直是極致的深情。",
          "es": "Que Dios insista en que no hay carta de divorcio es la forma más intensa de decir que, pase lo que pase, no piensa soltarnos.",
          "pt": "Deus insistindo que não existe carta de divórcio é a prova de que Ele simplesmente não desiste da gente, não importa o quanto a gente erre.",
          "vi": "Việc Chúa khẳng định không có tờ giấy ly hôn nào là cách Ngài nói rằng dù chúng ta có tệ đến đâu, Ngài cũng nhất quyết không buông tay.",
          "tl": "Ang pagsabi ng Diyos na walang divorce paper ay patunay na kahit gaano pa tayo magkamali, hindi Niya talaga tayo bibitawan.",
          "th": "การที่พระเจ้าบอกว่าไม่มีใบหย่าเป็นวิธีบอกว่าไม่ว่าเราจะทำตัวแย่แค่ไหน พระองค์ก็ไม่มีวันปล่อยมือจากเราแน่นอน",
          "sw": "Mungu kusisitiza kuwa hakuna cheti cha talaka ni njia yake ya kuonyesha kuwa hata tufanye makosa kiasi gani, hatatuacha kamwe.",
          "it": "Dio che insiste sul fatto che non esista alcun atto di divorzio dimostra una caparbietà nell'amarci che ha quasi del paradossale."
        }
    },
    "2026-10-06": {
        date: "2026-10-06",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 51:1-23",
          "en": "Isaiah 51:1-23",
          "ko": "Isaiah 51:1-23",
          "zho": "イザヤ書 51:1-23",
          "es": "Isaiah 51:1-23",
          "pt": "Isaiah 51:1-23",
          "vi": "Isaiah 51:1-23",
          "tl": "Isaiah 51:1-23",
          "th": "Isaiah 51:1-23",
          "sw": "Isaiah 51:1-23",
          "it": "Isaia 51:1-23"
        },
        comment: {
          "ja": "ちっぽけな人間のせいで神様が怒るわけないと分かっていても、つい自分の失敗のせいだと思って凹んでしまうこの自意識過剰さ、人類共通の悩みかもしれない。",
          "en": "The tendency to think our tiny mistakes are enough to ruin God's grand plan is just our ego working overtime.",
          "ko": "우리의 작은 실수가 하나님의 거대한 계획을 망칠 리 없다는 걸 알면서도, 내 탓이라고 자책하는 이 넘치는 자의식은 정말 인간 종특인 듯하다.",
          "zho": "明知渺小的我們不可能搞砸上帝的宏偉計畫，卻還是忍不住把所有挫折都歸咎於自己，這種被害妄想般的自戀真是人類通病。",
          "es": "Saber que nuestros pequeños errores no pueden arruinar el plan de Dios, pero seguir sintiéndonos culpables de todo, es la prueba definitiva de nuestro ego.",
          "pt": "Saber que nossos erros minúsculos não podem estragar o plano de Deus, mas ainda assim nos sentirmos culpados por tudo, é o auge do nosso egocentrismo.",
          "vi": "Dù biết sai lầm nhỏ nhoi của mình chẳng thể làm hỏng kế hoạch của Chúa, nhưng chúng ta vẫn cứ tự trách, đúng là cái tôi của con người thật kỳ lạ.",
          "tl": "Alam nating hindi kayang sirain ng maliliit nating pagkakamali ang plano ng Diyos, pero sige pa rin tayo sa pagsisi sa sarili; talagang mapagbiro ang ating ego.",
          "th": "การที่เรารู้ว่าความผิดพลาดเล็กน้อยของเราไม่สามารถทำลายแผนการอันยิ่งใหญ่ของพระเจ้าได้ แต่เราก็ยังอดโทษตัวเองไม่ได้ นี่มันคืออาการหลงตัวเองชัดๆ",
          "sw": "Kujua kuwa makosa yetu madogo hayawezi kuharibu mpango mkuu wa Mungu, lakini bado tukijilaumu kwa kila kitu, ni uthibitisho wa jinsi tunavyojiona kuwa muhimu sana.",
          "it": "Pensare che i nostri piccoli errori quotidiani possano mandare all'aria i piani divini è solo il nostro ego che lavora straordinari."
        }
    },
    "2026-10-07": {
        date: "2026-10-07",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 52:1-15",
          "en": "Isaiah 52:1-15",
          "ko": "Isaiah 52:1-15",
          "zho": "イザヤ書 52:1-15",
          "es": "Isaiah 52:1-15",
          "pt": "Isaiah 52:1-15",
          "vi": "Isaiah 52:1-15",
          "tl": "Isaiah 52:1-15",
          "th": "Isaiah 52:1-15",
          "sw": "Isaiah 52:1-15",
          "it": "Isaia 52:1-15"
        },
        comment: {
          "ja": "塵を払い落として立ち上がれと言われても、まずは溜まった洗濯物をどうにかしたいと思ってしまうのが人間というもの。",
          "en": "Being told to shake off the dust and rise is all well and good, but most of us are still just trying to figure out how to handle the pile of laundry in the corner.",
          "ko": "먼지를 털고 일어나라는 말은 좋지만, 당장 쌓여있는 빨래부터 처리하고 싶어지는 게 사람 마음입니다.",
          "zho": "被要求抖掉尘土重新站起来时，大多数人其实还在为那一堆没洗的衣服而发愁。",
          "es": "Que te digan que te sacudas el polvo y te levantes suena bien, pero la mayoría de nosotros solo estamos tratando de lidiar con la montaña de ropa sucia.",
          "pt": "Pedirem para sacudir a poeira e levantar é ótimo, mas a maioria de nós ainda está tentando lidar com a pilha de roupa suja no canto.",
          "vi": "Được bảo hãy phủi bụi và đứng dậy nghe thì hay đấy, nhưng hầu hết chúng ta vẫn chỉ đang loay hoay với đống quần áo chưa giặt.",
          "tl": "Ang payong magpagpag ng alikabok at bumangon ay madaling sabihin, pero karamihan sa atin ay abala pa sa pag-iisip kung paano tatapusin ang tambak na labahin.",
          "th": "การถูกบอกให้สะบัดฝุ่นแล้วลุกขึ้นยืนฟังดูดี แต่คนส่วนใหญ่ก็ยังคงวุ่นวายกับการจัดการกองผ้าที่ยังไม่ได้ซักอยู่ดี",
          "sw": "Kuambiwa uondoe mavumbi na uinuke ni jambo zuri, lakini wengi wetu bado tunahangaika tu na rundo la nguo chafu kule kona.",
          "it": "Ci dicono di scrollarci la polvere di dosso e alzarci, ma la verità è che siamo tutti bloccati a fissare la pila di panni da stirare."
        }
    },
    "2026-10-08": {
        date: "2026-10-08",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 53:1-12",
          "en": "Isaiah 53:1-12",
          "ko": "Isaiah 53:1-12",
          "zho": "イザヤ書 53:1-12",
          "es": "Isaiah 53:1-12",
          "pt": "Isaiah 53:1-12",
          "vi": "Isaiah 53:1-12",
          "tl": "Isaiah 53:1-12",
          "th": "Isaiah 53:1-12",
          "sw": "Isaiah 53:1-12",
          "it": "Isaia 53:1-12"
        },
        comment: {
          "ja": "見た目が地味だからと見向きもされない人が実は一番のキーマンだったという展開、現代の職場でもよくある逆転劇すぎて胸に刺さる。",
          "en": "The person everyone ignores because they don't look the part turning out to be the most important one in the room is a career lesson I keep learning the hard way.",
          "ko": "겉모습이 평범하다는 이유로 무시당하던 사람이 사실은 핵심 인물이었다는 전개는, 현대 사회에서도 뼈아프게 와닿는 반전입니다.",
          "zho": "那些因为外表平平而被忽视的人往往才是关键角色，这种职场反转剧现实中也太常见了。",
          "es": "Que la persona a la que todos ignoran por su apariencia resulte ser la más importante es una lección de vida que sigo aprendiendo a las malas.",
          "pt": "A pessoa que todos ignoram por não parecer importante acabar sendo a peça-chave é uma lição de vida que continuo aprendendo da pior forma.",
          "vi": "Người bị phớt lờ chỉ vì vẻ ngoài bình thường lại chính là nhân vật quan trọng nhất, một cú lật kèo thường thấy trong thực tế khiến ta phải suy ngẫm.",
          "tl": "Yung taong binabalewala dahil hindi mukhang bida pero siya pala ang pinaka-importante, parang totoong buhay lang na nakaka-relate.",
          "th": "คนที่ถูกมองข้ามเพราะรูปลักษณ์ภายนอกกลับกลายเป็นคนสำคัญที่สุด เป็นบทเรียนชีวิตที่พบเจอได้บ่อยจนน่าตกใจ",
          "sw": "Mtu anayepuuzwa kwa sababu ya mwonekano wake kumbe ndiye mtu muhimu zaidi ni ukweli mchungu tunaouona kila siku kazini.",
          "it": "La persona che tutti ignorano perché ha un aspetto qualunque che si rivela poi fondamentale è il classico colpo di scena della vita."
        }
    },
    "2026-10-09": {
        date: "2026-10-09",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 54:1-17",
          "en": "Isaiah 54:1-17",
          "ko": "Isaiah 54:1-17",
          "zho": "イザヤ書 54:1-17",
          "es": "Isaiah 54:1-17",
          "pt": "Isaiah 54:1-17",
          "vi": "Isaiah 54:1-17",
          "tl": "Isaiah 54:1-17",
          "th": "Isaiah 54:1-17",
          "sw": "Isaiah 54:1-17",
          "it": "Isaia 54:1-17"
        },
        comment: {
          "ja": "限界までテントを広げろと励まされるイザヤの言葉は、失敗を恐れて縮こまる自分にこそ必要な強引なポジティブさだ。",
          "en": "Being told to expand your tent when you are already feeling small is the kind of aggressive optimism we all need when we are playing it safe.",
          "ko": "텐트를 최대한 넓히라는 이사야의 외침은 실패가 두려워 움츠러든 우리에게 꼭 필요한 무대뽀 긍정의 힘이다.",
          "zho": "被要求扩张帐篷时那种还没准备好却要硬着头皮撑大的感觉，简直就是我们在舒适圈边缘挣扎的真实写照。",
          "es": "Que te digan que ensanches tu tienda cuando apenas puedes con la que tienes es ese empujón optimista que nos hace falta cuando nos da miedo arriesgar.",
          "pt": "Receber a ordem para alargar a tenda quando mal conseguimos lidar com a atual é o tipo de otimismo ousado que precisamos quando temos medo de crescer.",
          "vi": "Được bảo hãy nới rộng lều ngay khi đang cảm thấy nhỏ bé là kiểu lạc quan táo bạo mà tất cả chúng ta đều cần khi cứ mãi thu mình trong vùng an toàn.",
          "tl": "Ang utos na palawakin ang tolda kahit pakiramdam mo ay hindi mo kaya ay ang uri ng matapang na optimismo na kailangan natin kapag takot tayong lumabas sa ating comfort zone.",
          "th": "การถูกบอกให้ขยายเต็นท์ในตอนที่เรารู้สึกตัวเล็กจ้อย คือความมองโลกในแง่ดีแบบหักดิบที่เราทุกคนต้องการเวลาที่เราไม่กล้าเสี่ยง",
          "sw": "Kuambiwa upanue hema lako wakati unahisi mdogo ni aina ya matumaini makali tunayohitaji tunapoogopa kutoka katika eneo letu la usalama.",
          "it": "Chiederci di allargare la tenda proprio quando ci sentiamo minuscoli è quell'ottimismo sfacciato di cui abbiamo disperatamente bisogno."
        }
    },
    "2026-10-10": {
        date: "2026-10-10",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 55:1-13",
          "en": "Isaiah 55:1-13",
          "ko": "Isaiah 55:1-13",
          "zho": "イザヤ書 55:1-13",
          "es": "Isaiah 55:1-13",
          "pt": "Isaiah 55:1-13",
          "vi": "Isaiah 55:1-13",
          "tl": "Isaiah 55:1-13",
          "th": "Isaiah 55:1-13",
          "sw": "Isaiah 55:1-13",
          "it": "Isaia 55:1-13"
        },
        comment: {
          "ja": "金も払わずに飲み食いしに来いという神様の招待状、現代のカフェなら即閉店だがこれこそが究極の恩寵というやつだ。",
          "en": "God inviting everyone to eat and drink for free would bankrupt any modern business, but that is exactly what grace looks like.",
          "ko": "돈도 안 내고 마음껏 먹고 마시라는 하나님의 초대장은 요즘 카페라면 즉시 폐업이지만, 그것이 바로 은혜의 본질이다.",
          "zho": "上帝广发邀请函让人免费吃喝，要是开在现代咖啡馆肯定立刻倒闭，但这才是恩典最硬核的打开方式。",
          "es": "Una invitación divina a comer y beber gratis quebraría cualquier negocio hoy en día, pero así es como funciona la gracia cuando no hay factura de por medio.",
          "pt": "Um convite divino para comer e beber de graça levaria qualquer café moderno à falência, mas é exatamente assim que a graça funciona.",
          "vi": "Lời mời ăn uống miễn phí của Chúa sẽ khiến bất kỳ quán cà phê hiện đại nào phá sản, nhưng đó chính là cách ân điển vận hành.",
          "tl": "Ang paanyaya ng Diyos na kumain at uminom nang libre ay magpapabangkarote sa anumang negosyo ngayon, pero ganyan talaga ang biyaya pag walang bayad.",
          "th": "คำเชิญของพระเจ้าให้มากินดื่มฟรีอาจทำให้ร้านกาแฟสมัยนี้เจ๊งได้ทันที แต่นั่นแหละคือวิถีแห่งพระคุณที่ไม่มีใบเสร็จ",
          "sw": "Mwaliko wa Mungu wa kula na kunywa bila malipo ungefilisi biashara yoyote ya kisasa, lakini ndivyo neema inavyofanya kazi bila bili.",
          "it": "Un invito divino a mangiare e bere gratis farebbe fallire qualsiasi locale oggi, ma è esattamente questa la bellezza della grazia."
        }
    },
    "2026-10-11": {
        date: "2026-10-11",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 57:1-21",
          "en": "Isaiah 57:1-21",
          "ko": "Isaiah 57:1-21",
          "zho": "イザヤ書 57:1-21",
          "es": "Isaiah 57:1-21",
          "pt": "Isaiah 57:1-21",
          "vi": "Isaiah 57:1-21",
          "tl": "Isaiah 57:1-21",
          "th": "Isaiah 57:1-21",
          "sw": "Isaiah 57:1-21",
          "it": "Isaia 57:1-21"
        },
        comment: {
          "ja": "悪人が好き勝手して平穏に暮らす一方で、正しい人が早くいなくなる世の中の理不尽さは、昔も今も変わらないのが一番の悩みどころです。",
          "en": "The irony of the wicked prospering while the righteous vanish early is a frustrating reality that has clearly been around since the dawn of time.",
          "ko": "악인은 잘 사는데 의인은 일찍 세상을 떠나는 불공평한 현실은 예나 지금이나 우리를 참 답답하게 만듭니다.",
          "zho": "坏人过得逍遥自在，好人却早早离世，这种世道的不公简直是千古难题。",
          "es": "Es frustrante ver que el mundo no siempre premia a los buenos, una realidad que nos hace cuestionar todo desde hace milenios.",
          "pt": "A frustrante realidade de ver os maus prosperarem enquanto os bons partem cedo é um dilema que atravessa gerações.",
          "vi": "Việc kẻ ác sống an nhàn còn người tốt lại ra đi sớm là một thực tế trớ trêu tồn tại từ ngàn xưa đến nay.",
          "tl": "Ang nakakainis na realidad na mas nagtatagumpay ang masama habang nauuna pang mawala ang mabubuti ay problema na noon pa man.",
          "th": "ความจริงที่ว่าคนชั่วได้ดีในขณะที่คนดีจากไปเร็วนั้น เป็นเรื่องน่าหงุดหงิดใจที่เกิดขึ้นมาตั้งแต่ยุคโบราณจนถึงปัจจุบัน",
          "sw": "Ukweli mchungu kwamba watu wabaya hufanikiwa huku wema wakitoweka mapema ni tatizo ambalo limekuwepo tangu zamani.",
          "it": "Vedere chi si comporta male passarla liscia mentre i bravi ragazzi faticano è l'eterna ingiustizia che ci fa storcere il naso."
        }
    },
    "2026-10-12": {
        date: "2026-10-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 58:1-14",
          "en": "Isaiah 58:1-14",
          "ko": "Isaiah 58:1-14",
          "zho": "イザヤ書 58:1-14",
          "es": "Isaiah 58:1-14",
          "pt": "Isaiah 58:1-14",
          "vi": "Isaiah 58:1-14",
          "tl": "Isaiah 58:1-14",
          "th": "Isaiah 58:1-14",
          "sw": "Isaiah 58:1-14",
          "it": "Isaia 58:1-14"
        },
        comment: {
          "ja": "断食してアピールしているのに隣の同僚と喧嘩しているなんて、神様も呆れて溜息をついていそうです。",
          "en": "Fasting to look holy while fighting with your neighbor is the kind of hypocrisy that makes heaven roll its eyes.",
          "ko": "금식하며 경건한 척하면서 옆 사람과 싸우는 모습은 하늘에서 봐도 참 기가 막힐 노릇입니다.",
          "zho": "一边禁食一边还要跟邻居吵架，这种伪善简直连老天爷看了都要摇头。",
          "es": "Ayunar para parecer santo mientras te peleas con el vecino es el tipo de hipocresía que hace que el cielo ponga los ojos en blanco.",
          "pt": "Jejuar para parecer espiritual enquanto briga com o vizinho é o tipo de hipocrisia que deixa qualquer um sem palavras.",
          "vi": "Nhịn ăn để tỏ ra đạo mạo nhưng lại quay sang cãi cọ với hàng xóm là kiểu giả tạo khiến người ta chỉ biết thở dài.",
          "tl": "Ang pag-aayuno para magmukhang banal habang nakikipag-away sa kapitbahay ay ang uri ng pagpapaimbabaw na nakakainis kahit sa langit.",
          "th": "การถือศีลอดเพื่อสร้างภาพว่าเคร่งครัดแต่กลับไปทะเลาะกับเพื่อนบ้าน เป็นความย้อนแย้งที่น่าถอนหายใจที่สุด",
          "sw": "Kufunga ili uonekane mtakatifu huku ukigombana na jirani ni aina ya unafiki ambayo inafanya mbingu zishangae.",
          "it": "Digiunare per sembrare santi mentre litighiamo con il vicino di casa è l'ennesima dimostrazione della nostra incoerenza quotidiana."
        }
    },
    "2026-10-13": {
        date: "2026-10-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 59:1-21",
          "en": "Isaiah 59:1-21",
          "ko": "Isaiah 59:1-21",
          "zho": "イザヤ書 59:1-21",
          "es": "Isaiah 59:1-21",
          "pt": "Isaiah 59:1-21",
          "vi": "Isaiah 59:1-21",
          "tl": "Isaiah 59:1-21",
          "th": "Isaiah 59:1-21",
          "sw": "Isaiah 59:1-21",
          "it": "Isaia 59:1-21"
        },
        comment: {
          "ja": "神様が遠くに感じるのは腕が短いからではなく、私たちが勝手に壁を積み上げているせいだという事実に刺さりすぎて息が止まる。",
          "en": "The realization that God isn't distant, but that we keep building walls between us, is a reality check that hits a little too close to home.",
          "ko": "하나님이 멀게 느껴지는 것은 그분의 팔이 짧아서가 아니라 우리가 스스로 벽을 높이 쌓아 올린 탓이라는 사실이 뼈를 때린다.",
          "zho": "覺得神很遙遠並非因為祂的手短，而是我們自己築起了高牆，這真相簡直讓人無法反駁。",
          "es": "Darnos cuenta de que Dios no está lejos, sino que nosotros mismos levantamos muros, es una bofetada de realidad necesaria.",
          "pt": "Perceber que Deus não está longe, mas que nós mesmos construímos muros, é um tapa de realidade que dói um pouco.",
          "vi": "Nhận ra rằng Chúa không hề xa cách mà chính chúng ta tự dựng lên những bức tường ngăn cách thật là một sự thật đau lòng.",
          "tl": "Ang katotohanang hindi malayo ang Diyos kundi tayo ang nagtatayo ng mga pader sa pagitan natin ay isang masakit na realidad.",
          "th": "ความจริงที่ว่าพระเจ้าไม่ได้อยู่ไกล แต่เป็นเราเองที่สร้างกำแพงกั้นขึ้นมานั้น เป็นความจริงที่จุกจนพูดไม่ออก",
          "sw": "Kutambua kuwa Mungu hayuko mbali, bali sisi ndio tunajijengea kuta za kututenganisha, ni ukweli unaoumiza lakini wa kweli.",
          "it": "Realizzare che Dio non è distante, ma siamo noi a costruire barriere con i nostri capricci, è una vera doccia fredda."
        }
    },
    "2026-10-14": {
        date: "2026-10-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 60:1-22",
          "en": "Isaiah 60:1-22",
          "ko": "Isaiah 60:1-22",
          "zho": "イザヤ書 60:1-22",
          "es": "Isaiah 60:1-22",
          "pt": "Isaiah 60:1-22",
          "vi": "Isaiah 60:1-22",
          "tl": "Isaiah 60:1-22",
          "th": "Isaiah 60:1-22",
          "sw": "Isaiah 60:1-22",
          "it": "Isaia 60:1-22"
        },
        comment: {
          "ja": "暗闇の中でも起き上がって光を放てと言われても、まず布団から出るのが一番の難関だという現代人の悲哀。",
          "en": "Being told to rise and shine in the darkness sounds great, but the hardest part is actually getting out of bed in the first place.",
          "ko": "어둠 속에서 일어나 빛을 발하라는 말씀은 멋지지만, 일단 이불 밖으로 나가는 것부터가 우리에겐 가장 큰 도전이다.",
          "zho": "在黑暗中要站起來發光聽起來很勵志，但對現代人來說，最難的一步往往是從被窩裡鑽出來。",
          "es": "Que te digan que te levantes y brilles en la oscuridad suena genial, pero el verdadero desafío es simplemente salir de la cama.",
          "pt": "Dizer para se levantar e brilhar na escuridão soa ótimo, mas o desafio real é conseguir sair da cama de manhã.",
          "vi": "Được bảo hãy đứng dậy và tỏa sáng trong bóng tối nghe thật tuyệt, nhưng thử thách lớn nhất vẫn là làm sao để rời khỏi giường.",
          "tl": "Ang utos na bumangon at magliwanag sa dilim ay maganda, pero ang totoo, ang pinakamahirap ay ang pagbangon sa kama.",
          "th": "การถูกบอกให้ลุกขึ้นและส่องแสงในความมืดฟังดูดี แต่ความท้าทายที่สุดคือการลุกออกจากเตียงในตอนเช้านี่แหละ",
          "sw": "Kuambiwa uinuke na uangaze gizani ni jambo zuri, lakini changamoto kubwa zaidi bado ni ile hatua ya kwanza ya kuamka kitandani.",
          "it": "Ci dicono di alzarci e brillare nel buio, ma il vero miracolo quotidiano resta semplicemente riuscire a scendere dal letto."
        }
    },
    "2026-10-15": {
        date: "2026-10-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 61:1-11",
          "en": "Isaiah 61:1-11",
          "ko": "Isaiah 61:1-11",
          "zho": "イザヤ書 61:1-11",
          "es": "Isaiah 61:1-11",
          "pt": "Isaiah 61:1-11",
          "vi": "Isaiah 61:1-11",
          "tl": "Isaiah 61:1-11",
          "th": "Isaiah 61:1-11",
          "sw": "Isaiah 61:1-11",
          "it": "Isaia 61:1-11"
        },
        comment: {
          "ja": "灰の代わりに髪飾りをというイザヤの言葉は、どん底の時にこそ誰かに一番言われたい最高に気の利いた慰め言葉だ。",
          "en": "Trading ashes for a crown of beauty is the ultimate pick-me-up that we all need when we feel like we are hitting rock bottom.",
          "ko": "재 대신 화관을 씌워주겠다는 이사야의 말은 밑바닥에 있을 때 가장 듣고 싶은 위로의 끝판왕이다.",
          "zho": "以賽亞說要用華冠代替灰塵，這絕對是我們在人生低谷時最想聽到的暖心安慰。",
          "es": "Cambiar las cenizas por una corona de belleza es el tipo de aliento que todos necesitamos cuando sentimos que hemos tocado fondo.",
          "pt": "Trocar cinzas por uma coroa de beleza é o tipo de consolo que todos precisamos desesperadamente quando estamos no fundo do poço.",
          "vi": "Lời hứa thay tro bụi bằng vòng hoa của Ê-sai chính là lời an ủi tuyệt vời nhất mà ai cũng muốn nghe khi đang ở trong giai đoạn tồi tệ nhất.",
          "tl": "Ang ipagpalit ang abo sa korona ng kagandahan ay ang uri ng pampalakas-loob na kailangan nating marinig kapag tayo ay nasa pinakamababa.",
          "th": "การเปลี่ยนเถ้าถ่านให้เป็นมงกุฎแห่งความงดงามคือคำปลอบใจที่ทรงพลังที่สุดที่เราอยากได้ยินในวันที่ชีวิตพังไม่เป็นท่า",
          "sw": "Kubadilishana majivu na taji ya uzuri ni aina ya faraja ambayo sote tunahitaji tunapohisi tumefika mwisho wa safari.",
          "it": "Scambiare la cenere con un diadema di bellezza è la dose di incoraggiamento definitivo che serve a tutti quando si tocca il fondo."
        }
    },
    "2026-10-16": {
        date: "2026-10-16",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 63:1-19",
          "en": "Isaiah 63:1-19",
          "ko": "Isaiah 63:1-19",
          "zho": "イザヤ書 63:1-19",
          "es": "Isaiah 63:1-19",
          "pt": "Isaiah 63:1-19",
          "vi": "Isaiah 63:1-19",
          "tl": "Isaiah 63:1-19",
          "th": "Isaiah 63:1-19",
          "sw": "Isaiah 63:1-19",
          "it": "Isaia 63:1-19"
        },
        comment: {
          "ja": "散々逆らっておいてピンチになると急に「昔の恩を忘れたのか」と神に文句を言うイスラエルの民、現代の私たちと全く同じで苦笑いするしかない。",
          "en": "The Israelites ignoring God until they get in trouble and then asking why He stopped helping them is a level of hypocrisy we all struggle with.",
          "ko": "평소엔 제멋대로 살다가 위기가 닥치면 갑자기 옛날 은혜를 따지는 이스라엘 백성의 모습에서 왠지 모를 찔림이 느껴진다.",
          "zho": "以色列民平常不聽話，一遇困難就質問神為何不幫忙，這種雙標行為簡直像極了偶爾任性的我們。",
          "es": "Ver al pueblo de Israel ignorar a Dios y luego quejarse de que Él no los ayuda en sus problemas es un reflejo de nuestra propia hipocresía cotidiana.",
          "pt": "O povo de Israel ignorar a Deus e depois reclamar que Ele não os ajuda no aperto é uma hipocrisia tão humana que até dói.",
          "vi": "Dân Y-sơ-ra-ên cứ phớt lờ Chúa rồi khi gặp họa lại quay sang trách móc Ngài, kiểu cư xử này đúng là phiên bản lỗi của chính chúng ta.",
          "tl": "Ang mga Israelita na hindi nakikinig sa Diyos pero biglang magrereklamo kapag may problema ay tila salamin ng ating sariling kakulitan.",
          "th": "ชาวอิสราเอลที่ทำตัวห่างเหินจากพระเจ้าแต่พอเดือดร้อนก็รีบหันมาทวงบุญคุณ คือความย้อนแย้งที่น่าขำเพราะมันเหมือนชีวิตเราเป๊ะ",
          "sw": "Waisraeli kupuuza Mungu kisha kulalamika wakati wa shida ni aina ya unafiki wa kibinadamu ambao sote tunajikuta nao mara kwa mara.",
          "it": "Ignorare Dio finché arrivano i guai e poi chiedersi perché non aiuti più è quel livello di ipocrisia con cui facciamo tutti i conti."
        }
    },
    "2026-10-17": {
        date: "2026-10-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 65:1-25",
          "en": "Isaiah 65:1-25",
          "ko": "Isaiah 65:1-25",
          "zho": "イザヤ書 65:1-25",
          "es": "Isaiah 65:1-25",
          "pt": "Isaiah 65:1-25",
          "vi": "Isaiah 65:1-25",
          "tl": "Isaiah 65:1-25",
          "th": "Isaiah 65:1-25",
          "sw": "Isaiah 65:1-25",
          "it": "Isaia 65:1-25"
        },
        comment: {
          "ja": "一日中手を広げて待っているのに無視され続ける神様と、自分のこだわりを突き通して空回りする人間、いつの時代もすれ違いのスケールが大きすぎる。",
          "en": "God spending all day with open arms only to be ignored while humans stubbornly do their own thing is the ultimate example of a bad communication gap.",
          "ko": "하루 종일 두 팔 벌려 기다리는데도 무시당하는 신과 자기 고집대로만 하려는 인간, 시대가 변해도 소통 방식의 엇박자는 여전하네요.",
          "zho": "上帝整天張開雙臂等待卻被無視，而人類卻固執己見地繞圈子，這種溝通上的代溝跨越了整個時代。",
          "es": "Dios esperando con los brazos abiertos todo el día mientras la gente lo ignora por seguir sus propios planes es la definición misma de una mala comunicación.",
          "pt": "Deus esperando de braços abertos o dia todo enquanto as pessoas o ignoram para seguir suas próprias vontades é o exemplo máximo de desencontro.",
          "vi": "Chúa dang tay đợi cả ngày mà vẫn bị phớt lờ, trong khi con người cứ khăng khăng làm theo ý mình, quả là một khoảng cách giao tiếp kinh điển.",
          "tl": "Ang paghihintay ng Diyos nang nakabukas ang mga braso habang binabalewala siya ng tao para sa sariling agenda ay ang sukdulang halimbawa ng hindi pagkakaunawaan.",
          "th": "พระเจ้าทรงรอคอยด้วยอ้อมแขนที่เปิดกว้างทั้งวันแต่ถูกเมิน ในขณะที่มนุษย์ก็เอาแต่ดื้อรั้นทำตามใจตัวเอง นี่คือตัวอย่างคลาสสิกของช่องว่างในการสื่อสารจริงๆ",
          "sw": "Mungu akisubiri kwa mikono wazi kutwa nzima huku akipuuzwa na watu wanaofanya mambo yao wenyewe ni mfano dhahiri wa kutokuelewana kwa kiwango cha juu.",
          "it": "Dio che aspetta tutto il giorno a braccia aperte mentre la gente lo ignora per fare di testa sua è la definizione di un cortocircuito comunicativo."
        }
    },
    "2026-10-18": {
        date: "2026-10-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 66:1-24",
          "en": "Isaiah 66:1-24",
          "ko": "Isaiah 66:1-24",
          "zho": "イザヤ書 66:1-24",
          "es": "Isaiah 66:1-24",
          "pt": "Isaiah 66:1-24",
          "vi": "Isaiah 66:1-24",
          "tl": "Isaiah 66:1-24",
          "th": "Isaiah 66:1-24",
          "sw": "Isaiah 66:1-24",
          "it": "Isaia 66:1-24"
        },
        comment: {
          "ja": "天を玉座、地を足台と言い切る神様の前で、自分の小さな建物を自慢する人間の姿は、どう見ても壮大なコントにしか見えない。",
          "en": "Humans bragging about their tiny buildings before a God who calls the entire universe his footstool is honestly just a hilarious cosmic comedy.",
          "ko": "온 우주를 발등상으로 삼으시는 신 앞에서 자기 건물 좀 지었다고 자랑하는 인간들의 모습은 아무리 봐도 거대한 코미디 같습니다.",
          "zho": "在將整個宇宙視為腳凳的上帝面前，人類為了自己蓋的一點小建築而沾沾自喜，這畫面簡直就是一場大型喜劇。",
          "es": "Ver a los humanos presumiendo de sus pequeños edificios ante un Dios que llama al cielo su trono y a la tierra su estrado es una comedia cósmica de proporciones épicas.",
          "pt": "Ver humanos se gabando de suas pequenas construções diante de um Deus que chama o universo de escabelo de seus pés é uma comédia cósmica e tanto.",
          "vi": "Việc con người khoe khoang về những tòa nhà nhỏ bé trước mặt một Đấng coi cả vũ trụ là bệ chân thật giống như một vở hài kịch quy mô lớn.",
          "tl": "Ang pagyayabang ng tao tungkol sa kanilang maliliit na gusali sa harap ng Diyos na itinuturing ang buong mundo na kanyang tuntungan ay isang nakakatawang komedya.",
          "th": "การที่มนุษย์อวดอาคารสิ่งก่อสร้างเล็กๆ ของตัวเองต่อหน้าพระเจ้าผู้ทรงเรียกสวรรค์ว่าบัลลังก์และแผ่นดินโลกเป็นที่วางพระบาทนั้น ดูเป็นเรื่องตลกขบขันระดับจักรวาลจริงๆ",
          "sw": "Wanadamu kujivunia majengo yao madogo mbele ya Mungu anayeuita ulimwengu wote kiti chake cha miguu ni vichekesho vya kiwango cha juu sana.",
          "it": "Vedere gli umani che si vantano dei loro piccoli edifici davanti a un Dio che chiama la terra il suo sgabello è pura commedia cosmica."
        }
    },
    "2026-10-19": {
        date: "2026-10-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 1:1-19",
          "en": "Jeremiah 1:1-19",
          "ko": "Jeremiah 1:1-19",
          "zho": "エレミヤ書 1:1-19",
          "es": "Jeremiah 1:1-19",
          "pt": "Jeremiah 1:1-19",
          "vi": "Jeremiah 1:1-19",
          "tl": "Jeremiah 1:1-19",
          "th": "Jeremiah 1:1-19",
          "sw": "Jeremiah 1:1-19",
          "it": "Geremia 1:1-19"
        },
        comment: {
          "ja": "まだ子供だからと言い訳して逃げようとするエレミヤと、そんなの知ってるよと言わんばかりに背中を押す神様のやり取り、まるで新入社員と上司の攻防戦を見ている気分だ。",
          "en": "Jeremiah trying to pull the 'I am too young' card and God immediately shutting it down is the ultimate divine version of a boss ignoring your resume gaps.",
          "ko": "아직 어리다는 핑계로 도망치려는 예레미야와 다 알고 있다는 듯 밀어붙이는 하나님의 대화는 마치 신입사원과 직장 상사의 기싸움을 보는 것 같다.",
          "zho": "耶利米试图用年纪小当借口推托，而上帝却直接无视他的退缩，这简直就是职场新人与强势主管的经典攻防战。",
          "es": "Jeremías intentando usar la excusa de que es muy joven y Dios ignorándola por completo es el equivalente divino a cuando tu jefe te dice que tu falta de experiencia no es un problema.",
          "pt": "Jeremias tentando usar a desculpa de ser jovem demais e Deus ignorando isso completamente é o equivalente divino de quando seu chefe diz que sua falta de experiência não é um problema.",
          "vi": "Việc Giê-rê-mi cố gắng viện cớ còn quá trẻ và Chúa thẳng thừng bác bỏ giống hệt cảnh một nhân viên mới đang tìm cách từ chối sếp mình.",
          "tl": "Ang pagdadahilan ni Jeremias na siya ay bata pa at ang hindi pagpapansin ng Diyos dito ay parang eksena lang ng bagong empleyado na sinusubukang tumanggi sa kanyang boss.",
          "th": "การที่เยเรมีย์พยายามอ้างว่าตัวเองยังเด็กเกินไปแต่พระเจ้ากลับเมินเฉยนั้น เหมือนเห็นภาพพนักงานใหม่กำลังพยายามปฏิเสธงานกับหัวหน้าเป๊ะเลย",
          "sw": "Yeremia kujaribu kutumia kisingizio cha kuwa kijana sana na Mungu kukipuuza moja kwa moja ni kama kumuona mfanyakazi mpya akijaribu kukwepa majukumu kutoka kwa bosi wake.",
          "it": "Geremia che cerca di giocare la carta della giovane età e Dio che la scarta all'istante è la versione divina del capo che ignora le tue scuse."
        }
    },
    "2026-10-20": {
        date: "2026-10-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 2:9-37",
          "en": "Jeremiah 2:9-37",
          "ko": "Jeremiah 2:9-37",
          "zho": "エレミヤ書 2:9-37",
          "es": "Jeremiah 2:9-37",
          "pt": "Jeremiah 2:9-37",
          "vi": "Jeremiah 2:9-37",
          "tl": "Jeremiah 2:9-37",
          "th": "Jeremiah 2:9-37",
          "sw": "Jeremiah 2:9-37",
          "it": "Geremia 2:9-37"
        },
        comment: {
          "ja": "壊れた水たまりにすがるより湧き出る泉に行けばいいのに、わざわざ苦労する方を選ぶ人間の習性はいつの時代も変わらないようだ。",
          "en": "People choosing to dig leaky pits instead of just going to the literal fountain of living water is a masterclass in making life harder than it needs to be.",
          "ko": "터진 웅덩이에 매달리느라 고생하는 것보다 샘물로 가면 될 텐데, 굳이 사서 고생하는 인간의 본능은 예나 지금이나 똑같나 보다.",
          "zho": "明明有活水泉源却偏要守着破裂的池子不放，人类这种宁可辛苦也不愿改变的固执，真是跨越千年的经典。",
          "es": "Empeñarse en cavar cisternas rotas en lugar de ir a la fuente de agua viva es el ejemplo perfecto de cómo nos complicamos la vida innecesariamente.",
          "pt": "Insistir em cavar cisternas rachadas em vez de ir à fonte de água viva é o exemplo perfeito de como complicamos a vida desnecessariamente.",
          "vi": "Việc cứ cố đào những bể nước nứt nẻ thay vì đi đến nguồn nước hằng sống là ví dụ điển hình cho việc con người tự làm khổ mình một cách không cần thiết.",
          "tl": "Ang pagpilit na maghukay ng mga sirang balon sa halip na pumunta sa bukal ng buhay na tubig ay ang perpektong halimbawa kung bakit natin pinahihirapan ang sarili natin.",
          "th": "การดันทุรังขุดบ่อที่แตกแทนที่จะไปตักน้ำจากแหล่งน้ำพุที่แท้จริง เป็นตัวอย่างชั้นดีของการที่มนุษย์ชอบทำให้ชีวิตตัวเองยากกว่าที่ควรจะเป็น",
          "sw": "Kushikilia visima vilivyopasuka badala ya kwenda kwenye chemchemi ya maji ya uzima ndio mfano bora wa jinsi tunavyojifanyia maisha kuwa magumu bila sababu.",
          "it": "Aggrapparsi a cisterne rotte invece di andare alla fonte di acqua viva è il manuale perfetto per complicarsi la vita senza motivo."
        }
    },
    "2026-10-21": {
        date: "2026-10-21",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 3:1-25",
          "en": "Jeremiah 3:1-25",
          "ko": "Jeremiah 3:1-25",
          "zho": "エレミヤ書 3:1-25",
          "es": "Jeremiah 3:1-25",
          "pt": "Jeremiah 3:1-25",
          "vi": "Jeremiah 3:1-25",
          "tl": "Jeremiah 3:1-25",
          "th": "Jeremiah 3:1-25",
          "sw": "Jeremiah 3:1-25",
          "it": "Geremia 3:1-25"
        },
        comment: {
          "ja": "散々浮気しておいて神様にしれっと戻ってくるイスラエルの民の図々しさは、もはや一周回って清々しいレベル。",
          "en": "The audacity of Israel to cheat repeatedly and then casually expect a warm welcome back is honestly impressive.",
          "ko": "온갖 배신을 저지르고도 아무렇지 않게 다시 돌아오려는 이스라엘 백성의 뻔뻔함은 거의 경지에 달했다.",
          "zho": "以色列人反覆背棄神卻又厚著臉皮想回頭的樣子，那種理直氣壯的程度實在讓人嘆為觀止。",
          "es": "La audacia de Israel al portarse mal y luego regresar como si nada hubiera pasado es el nivel definitivo de descaro.",
          "pt": "A audácia de Israel em trair e depois voltar casualmente como se nada tivesse acontecido é o auge da cara de pau.",
          "vi": "Sự trơ trẽn của dân Y-sơ-ra-ên khi cứ phản bội rồi lại thản nhiên quay về như chưa có chuyện gì xảy ra thật đáng kinh ngạc.",
          "tl": "Ang kapal ng mukha ng Israel na magtaksil tapos babalik lang na parang walang nangyari ay nakakamangha sa totoo lang.",
          "th": "ความหน้าไม่อายของชาวอิสราเอลที่นอกใจไปทั่วแล้วยังกล้ากลับมาหาพระเจ้าเหมือนไม่มีอะไรเกิดขึ้นนั้นถือว่าสุดยอดจริงๆ",
          "sw": "Ujasiri wa Israeli wa kutenda dhambi kisha kurudi kwa Mungu kana kwamba hakuna kilichotokea ni kiwango cha juu cha ubishi.",
          "it": "La sfacciataggine di Israele che combina guai e poi torna come se nulla fosse è un vero capolavoro di faccia tosta."
        }
    },
    "2026-10-22": {
        date: "2026-10-22",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 7:1-34",
          "en": "Jeremiah 7:1-34",
          "ko": "Jeremiah 7:1-34",
          "zho": "エレミヤ書 7:1-34",
          "es": "Jeremiah 7:1-34",
          "pt": "Jeremiah 7:1-34",
          "vi": "Jeremiah 7:1-34",
          "tl": "Jeremiah 7:1-34",
          "th": "Jeremiah 7:1-34",
          "sw": "Jeremiah 7:1-34",
          "it": "Geremia 7:1-34"
        },
        comment: {
          "ja": "神殿に行けば全てが帳消しになると勘違いしている人たち、現代のSNSで適当な投稿をして免罪符を得ようとする感覚に近い。",
          "en": "People thinking that showing up to the temple acts as a magic eraser for their bad behavior is the ancient version of posting a fake apology online.",
          "ko": "성전에만 가면 모든 죄가 씻길 거라 착각하는 모습은, 온라인에 형식적인 사과문을 올리고 면죄부를 받으려는 현대의 풍경과 다를 게 없다.",
          "zho": "以為去聖殿就能抵銷一切惡行的人，就像現代那些以為發個道歉文就能洗白的人一樣天真。",
          "es": "Creer que asistir al templo borra todas tus malas decisiones es el equivalente antiguo a publicar una disculpa falsa en redes sociales.",
          "pt": "Achar que ir ao templo apaga todas as suas escolhas ruins é o equivalente antigo a postar um pedido de desculpas falso nas redes sociais.",
          "vi": "Việc nghĩ rằng cứ đến đền thờ là mọi tội lỗi sẽ được xóa sạch cũng giống như việc đăng một lời xin lỗi giả tạo lên mạng để tẩy trắng bản thân vậy.",
          "tl": "Ang akala na ang pagpunta sa templo ay pambura ng masasamang gawa ay parang pag-post ng pekeng apology sa social media para malinis ang imahe.",
          "th": "คนที่คิดว่าแค่ไปโบสถ์ก็ล้างความผิดได้ทุกอย่างก็ไม่ต่างอะไรกับคนที่โพสต์ขอโทษแบบขอไปทีบนโซเชียลเพื่อหวังจะจบเรื่อง",
          "sw": "Watu wanaofikiri kuwa kuhudhuria hekaluni kunafuta matendo yao mabaya ni sawa na watu wa kisasa wanaochapisha msamaha wa uongo mtandaoni ili kusafisha sifa zao.",
          "it": "Pensare che frequentare il tempio cancelli ogni cattiva azione è l'equivalente antico di pubblicare una falsa scusa online per ripulirsi la reputazione."
        }
    },
    "2026-10-23": {
        date: "2026-10-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 16:1-21",
          "en": "Jeremiah 16:1-21",
          "ko": "Jeremiah 16:1-21",
          "zho": "エレミヤ書 16:1-21",
          "es": "Jeremiah 16:1-21",
          "pt": "Jeremiah 16:1-21",
          "vi": "Jeremiah 16:1-21",
          "tl": "Jeremiah 16:1-21",
          "th": "Jeremiah 16:1-21",
          "sw": "Jeremiah 16:1-21",
          "it": "Geremia 16:1-21"
        },
        comment: {
          "ja": "結婚も葬式も禁止、おまけに宴会もダメと言われたエレミヤの孤独っぷりは、空気を読みすぎて疲弊する現代人の極致を見ているようだ。",
          "en": "Being told you cannot marry, mourn, or attend parties is the ultimate level of social isolation that even the most introverted among us would find tough.",
          "ko": "결혼도 장례식도 잔치도 금지당한 예레미야의 고독은 눈치 보느라 지친 현대인들이 겪는 외로움의 끝판왕 같다.",
          "zho": "被禁止结婚、参加葬礼和宴会的耶利米，那种无处安放的孤独感，简直就是现代人为了迎合社交而感到疲惫的极致写照。",
          "es": "A Jeremías le prohibieron casarse, ir a funerales y asistir a fiestas, una soledad tan extrema que hace que cualquier plan de fin de semana parezca una gran aventura.",
          "pt": "Jeremias sendo proibido de casar, ir a funerais ou festas é a definição máxima de isolamento social que faria qualquer um surtar hoje em dia.",
          "vi": "Việc bị cấm kết hôn, dự đám tang hay tiệc tùng khiến cuộc đời của Giê-rê-mi trở nên cô độc đến mức khó tin, một kiểu áp lực xã hội mà người hiện đại hẳn sẽ thấy rất quen.",
          "tl": "Ang pagbabawal kay Jeremias na mag-asawa, pumunta sa burol, o dumalo sa mga piging ay isang antas ng pag-iisa na tiyak na hindi kakayanin ng mga taong sanay sa social media.",
          "th": "การที่เยเรมีย์ถูกห้ามไม่ให้แต่งงาน ไปงานศพ หรือไปงานเลี้ยง เป็นความโดดเดี่ยวสุดโต่งที่แม้แต่คนชอบเก็บตัวที่สุดในยุคนี้ก็คงรู้สึกสะทกสะท้าน",
          "sw": "Yeremia kukatazwa kuoa, kuhudhuria mazishi, au kwenda kwenye karamu ni kiwango cha upweke ambacho hata mtu anayependa faragha angeona ni kigumu sana.",
          "it": "Aver il divieto di sposarsi o andare alle feste è un livello di isolamento sociale che metterebbe a dura prova anche il più convinto solitario."
        }
    },
    "2026-10-24": {
        date: "2026-10-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 17:1-27",
          "en": "Jeremiah 17:1-27",
          "ko": "Jeremiah 17:1-27",
          "zho": "エレミヤ書 17:1-27",
          "es": "Jeremiah 17:1-27",
          "pt": "Jeremiah 17:1-27",
          "vi": "Jeremiah 17:1-27",
          "tl": "Jeremiah 17:1-27",
          "th": "Jeremiah 17:1-27",
          "sw": "Jeremiah 17:1-27",
          "it": "Geremia 17:1-27"
        },
        comment: {
          "ja": "心に罪を刻み込むなんて物騒なことを言われているけれど、結局のところ人は誰しも自分の都合の良いことしか覚えていないのがリアルなところだ。",
          "en": "They talk about sins being engraved on the heart, but let’s be honest, we all have a selective memory when it comes to our own mistakes.",
          "ko": "죄가 마음에 새겨진다는 무시무시한 경고를 듣지만, 사실 우리 모두는 자기 잘못은 금방 잊어버리는 선택적 기억력을 가지고 있잖아요.",
          "zho": "虽然说罪孽被刻在心版上听起来很可怕，但说实话，谁还没个选择性记忆，专门记别人的错而忽略自己的呢。",
          "es": "Dicen que el pecado queda grabado en el corazón, pero siendo sinceros, todos tenemos una memoria muy selectiva cuando se trata de reconocer nuestros propios errores.",
          "pt": "Dizem que o pecado fica gravado no coração, mas sejamos honestos, todos nós temos uma memória muito seletiva quando o assunto é reconhecer os próprios erros.",
          "vi": "Dù lời cảnh báo về việc tội lỗi khắc sâu trong lòng nghe thật đáng sợ, nhưng thực tế là ai trong chúng ta cũng có trí nhớ chọn lọc khi nghĩ về lỗi lầm của chính mình.",
          "tl": "Sinasabing ang kasalanan ay nakaukit sa puso, pero sa totoo lang, lahat tayo ay may selective memory pagdating sa sarili nating mga pagkakamali.",
          "th": "คำเตือนเรื่องบาปที่ถูกสลักไว้ในใจฟังดูน่ากลัว แต่เอาเข้าจริงเราทุกคนต่างก็มีความจำแบบเลือกจำเวลาที่ต้องยอมรับความผิดของตัวเอง",
          "sw": "Wanasema dhambi huchorwa moyoni, lakini ukweli ni kwamba sote tuna kumbukumbu za kuchagua pale tunapokosea wenyewe.",
          "it": "Parlare di peccati incisi nel cuore fa impressione, ma si sa che tutti abbiamo una memoria fin troppo selettiva sui nostri sbagli."
        }
    },
    "2026-10-25": {
        date: "2026-10-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 18:11-23",
          "en": "Jeremiah 18:11-23",
          "ko": "Jeremiah 18:11-23",
          "zho": "エレミヤ書 18:11-23",
          "es": "Jeremiah 18:11-23",
          "pt": "Jeremiah 18:11-23",
          "vi": "Jeremiah 18:11-23",
          "tl": "Jeremiah 18:11-23",
          "th": "Jeremiah 18:11-23",
          "sw": "Jeremiah 18:11-23",
          "it": "Geremia 18:11-23"
        },
        comment: {
          "ja": "粘土をこね直す神様の忍耐強さと、何度作り直されてもまた形を変えてしまうエレミヤの不屈のメンタル、どっちも人間には真似できない領域。",
          "en": "God’s patience in reshaping the clay and Jeremiah’s refusal to quit even when life keeps breaking him is a level of emotional endurance most of us can only dream of.",
          "ko": "진흙을 다시 빚으시는 하나님의 인내심과 그 와중에도 꺾이지 않는 예레미야의 멘탈은 인간의 상상을 초월하는 수준이다.",
          "zho": "看著神不斷重塑泥土的耐心，再看看耶利米即使被生活打碎依然堅韌的心理素質，這簡直是人類意志的極限。",
          "es": "La paciencia de Dios al moldear el barro una y otra vez y la tenacidad de Jeremías para seguir adelante, nos dejan claro que la resiliencia es un arte divino.",
          "pt": "A paciência de Deus ao refazer o vaso e a teimosia de Jeremias em não desistir mostram que a resiliência é um exercício diário de fé.",
          "vi": "Kiên nhẫn nhào nặn lại bình gốm và tinh thần không chịu khuất phục của Giê-rê-mi cho thấy sự bền bỉ là một thử thách khó nhằn đối với bất cứ ai.",
          "tl": "Ang pagpapasensya ng Diyos sa paghuhulma ng luwad at ang katatagan ni Jeremias sa gitna ng pagsubok ay nagpapakita na ang pagbangon muli ay hindi madaling gawin.",
          "th": "ความอดทนของพระเจ้าในการปั้นดินใหม่และการที่เยเรมีย์ไม่ยอมแพ้แม้ชีวิตจะแตกสลายซ้ำแล้วซ้ำเล่า เป็นภาพสะท้อนความอดทนที่มนุษย์อย่างเราทำได้ยากจริงๆ",
          "sw": "Uvumilivu wa Mungu katika kurekebisha udongo na ujasiri wa Yeremia wa kutokata tamaa ni mfano wa jinsi tunavyopaswa kusimama imara hata wakati mambo yanaharibika.",
          "it": "La pazienza infinita di Dio nel rimodellare l'argilla e la resistenza di Geremia dimostrano una resilienza che noi possiamo solo sognare."
        }
    },
    "2026-10-26": {
        date: "2026-10-26",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 31:1-40",
          "en": "Jeremiah 31:1-40",
          "ko": "Jeremiah 31:1-40",
          "zho": "エレミヤ書 31:1-40",
          "es": "Jeremiah 31:1-40",
          "pt": "Jeremiah 31:1-40",
          "vi": "Jeremiah 31:1-40",
          "tl": "Jeremiah 31:1-40",
          "th": "Jeremiah 31:1-40",
          "sw": "Jeremiah 31:1-40",
          "it": "Geremia 31:1-40"
        },
        comment: {
          "ja": "過去の失敗を全部忘れて新しい契約を結ぼうとする神様、どんだけ懐が深いのか、もはや人間離れした包容力。",
          "en": "God offering a fresh start and deleting our past mistakes from the server is the ultimate definition of a clean slate.",
          "ko": "과거의 실수를 전부 삭제하고 새로운 계약을 맺어주시는 하나님의 넓은 마음은 인간의 상식을 완전히 뛰어넘는다.",
          "zho": "神選擇遺忘我們過去所有的失敗並給予新約，這種包容力簡直是宇宙級的寬宏大量。",
          "es": "Dios borrando nuestro historial de errores para ofrecernos un nuevo comienzo es la prueba definitiva de que su misericordia no tiene límites.",
          "pt": "Deus deletando nosso histórico de erros para propor uma nova aliança é o exemplo supremo de uma segunda chance.",
          "vi": "Việc Chúa xóa bỏ mọi sai lầm trong quá khứ để thiết lập một giao ước mới chính là minh chứng cho sự bao dung vượt xa tầm hiểu biết của con người.",
          "tl": "Ang paglimot ng Diyos sa ating mga nakaraang pagkakamali para magsimula muli ay patunay ng Kanyang walang hanggang pagpapatawad.",
          "th": "การที่พระเจ้าเลือกจะลืมความผิดพลาดในอดีตของเราทั้งหมดเพื่อทำพันธสัญญาใหม่ เป็นความใจกว้างที่เกินกว่ามนุษย์อย่างเราจะจินตนาการถึง",
          "sw": "Mungu kufuta makosa yetu ya zamani na kutupa mwanzo mpya ni uthibitisho wa jinsi upendo wake unavyozidi uelewa wetu wa kibinadamu.",
          "it": "Dio che cancella tutti i nostri errori passati per offrirci un nuovo inizio è la definizione stessa di una seconda possibilità."
        }
    },
    "2026-10-27": {
        date: "2026-10-27",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 32:1-44",
          "en": "Jeremiah 32:1-44",
          "ko": "Jeremiah 32:1-44",
          "zho": "エレミヤ書 32:1-44",
          "es": "Jeremiah 32:1-44",
          "pt": "Jeremiah 32:1-44",
          "vi": "Jeremiah 32:1-44",
          "tl": "Jeremiah 32:1-44",
          "th": "Jeremiah 32:1-44",
          "sw": "Jeremiah 32:1-44",
          "it": "Geremia 32:1-44"
        },
        comment: {
          "ja": "国が滅びそうな時に不動産投資を命じられたエレミヤ、神様からの指示とはいえ究極の無理ゲーすぎて同情しかない。",
          "en": "God telling Jeremiah to buy real estate while the country is literally collapsing is the ultimate test of faith in a terrible market.",
          "ko": "나라가 망해가는 판국에 부동산을 매입하라는 명령을 받은 예레미야, 아무리 신의 뜻이라지만 이건 너무 가혹한 미션 아닌가요.",
          "zho": "在國家即將滅亡時被要求投資房地產的耶利米，這絕對是史上最讓人摸不著頭緒的投資決策。",
          "es": "Que Dios le pida a Jeremías comprar tierras justo cuando el país se cae a pedazos es el tipo de fe ciega que a cualquiera le daría un ataque de nervios.",
          "pt": "Deus pedir para Jeremias comprar terras enquanto o país desmorona é o tipo de investimento que faria qualquer um questionar a sanidade.",
          "vi": "Được lệnh đi mua đất ngay khi đất nước sắp sụp đổ, Jeremiah chắc hẳn đã phải đấu tranh tư tưởng rất nhiều với quyết định tréo ngoe này.",
          "tl": "Ang utusan si Jeremias na bumili ng lupa habang guguho na ang bansa ay ang pinakamatinding pagsubok ng pananampalataya sa gitna ng krisis.",
          "th": "การที่พระเจ้าสั่งให้เยเรมีย์ซื้อที่ดินในวันที่บ้านเมืองกำลังจะล่มสลายนั้น เป็นการทดสอบความเชื่อที่ชวนให้ปวดหัวที่สุดเลยจริงๆ",
          "sw": "Mungu kumwambia Yeremia anunue ardhi wakati nchi inaporomoka ni kama kuwekeza kwenye soko la hisa wakati wa janga kuu.",
          "it": "Chiedere a Geremia di comprare immobili mentre il paese crolla a pezzi è la prova definitiva di una fede che sfida ogni logica di mercato."
        }
    },
    "2026-10-28": {
        date: "2026-10-28",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 33:1-26",
          "en": "Jeremiah 33:1-26",
          "ko": "Jeremiah 33:1-26",
          "zho": "エレミヤ書 33:1-26",
          "es": "Jeremiah 33:1-26",
          "pt": "Jeremiah 33:1-26",
          "vi": "Jeremiah 33:1-26",
          "tl": "Jeremiah 33:1-26",
          "th": "Jeremiah 33:1-26",
          "sw": "Jeremiah 33:1-26",
          "it": "Geremia 33:1-26"
        },
        comment: {
          "ja": "牢屋の中から未来の希望を語り続けるエレミヤ、もはや環境という概念を超越していて逆に清々しい。",
          "en": "Jeremiah preaching about a hopeful future while sitting in a prison cell is a masterclass in ignoring your surroundings.",
          "ko": "감옥에 갇혀 있으면서도 희망찬 미래를 예언하는 예레미야를 보면, 상황에 굴하지 않는다는 게 뭔지 제대로 보여주는 것 같아요.",
          "zho": "身陷囹圄卻還在暢談未來希望的耶利米，這種無視環境干擾的強大心態簡直是修行典範。",
          "es": "Jeremías hablando de un futuro brillante desde una celda es la prueba de que el optimismo puede ser tan terco como la realidad misma.",
          "pt": "Jeremias falando sobre um futuro esperançoso enquanto está preso mostra que ele vive em um nível de otimismo que a maioria de nós nem sonha em alcançar.",
          "vi": "Jeremiah vẫn say sưa nói về tương lai tươi sáng ngay từ trong ngục tối, một minh chứng cho thấy sự lạc quan không hề biết đến hoàn cảnh là gì.",
          "tl": "Ang patuloy na pangangaral ni Jeremias tungkol sa pag-asa habang nasa loob ng bilangguan ay nagpapakita na ang pananampalataya ay walang pakialam sa rehas.",
          "th": "การที่เยเรมีย์ยังคงพูดถึงความหวังในอนาคตขณะที่ตัวเองยังติดคุกอยู่ เป็นนิยามของคำว่ามองโลกในแง่ดีแบบทะลุกำแพงจริงๆ",
          "sw": "Yeremia kuhubiri kuhusu tumaini la wakati ujao akiwa gerezani ni ushahidi kwamba matumaini hayawezi kufungwa na minyororo.",
          "it": "Geremia che parla di un futuro pieno di speranza mentre è chiuso in cella è una lezione magistrale sull'ignorare le circostanze."
        }
    },
    "2026-10-29": {
        date: "2026-10-29",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 36:1-32",
          "en": "Jeremiah 36:1-32",
          "ko": "Jeremiah 36:1-32",
          "zho": "エレミヤ書 36:1-32",
          "es": "Jeremiah 36:1-32",
          "pt": "Jeremiah 36:1-32",
          "vi": "Jeremiah 36:1-32",
          "tl": "Jeremiah 36:1-32",
          "th": "Jeremiah 36:1-32",
          "sw": "Jeremiah 36:1-32",
          "it": "Geremia 36:1-32"
        },
        comment: {
          "ja": "都合の悪い報告書をナイフで切り刻んで暖炉に放り込む王様の姿は、現代の不都合なメールを未読のままゴミ箱に入れる私たちの姿と重なって笑えない。",
          "en": "The king cutting up the scroll and tossing it into the fire is basically the ancient version of deleting an email you really do not want to deal with.",
          "ko": "불편한 진실이 담긴 편지를 읽기도 전에 불태워버리는 왕의 모습에서, 읽기 싫은 메시지를 무시하고 삭제하는 우리의 일상을 본다.",
          "zho": "國王把令人不悅的報告切碎扔進火爐，這簡直就是現代人把討厭的郵件直接丟進垃圾桶的翻版。",
          "es": "El rey cortando el pergamino y echándolo al fuego es básicamente la versión antigua de ignorar ese correo electrónico que no quieres responder.",
          "pt": "O rei cortando o pergaminho e jogando no fogo é basicamente a versão antiga de deletar aquele e-mail que você não quer responder.",
          "vi": "Vị vua cắt nát cuộn giấy rồi ném vào lửa chẳng khác nào cách chúng ta xóa đi những email khó chịu mà mình không muốn đối mặt.",
          "tl": "Ang hari na pinagpipira-piraso ang sulat at itinapon sa apoy ay parang tayo lang na dinedelete ang email na ayaw nating basahin.",
          "th": "กษัตริย์ที่หั่นม้วนหนังสือแล้วโยนเข้ากองไฟนั้น ไม่ต่างอะไรกับเราที่กดลบอีเมลที่ไม่อยากอ่านทิ้งไปในทันที",
          "sw": "Mfalme kukata hati na kuichoma moto ni kama tu sisi tunavyofuta barua pepe ambazo hatutaki kuzishughulikia.",
          "it": "Il re che taglia il rotolo e lo getta nel fuoco è l'equivalente antico di cestinare quella email che proprio non vogliamo leggere."
        }
    },
    "2026-10-30": {
        date: "2026-10-30",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エレミヤ書 37:1-21",
          "en": "Jeremiah 37:1-21",
          "ko": "Jeremiah 37:1-21",
          "zho": "エレミヤ書 37:1-21",
          "es": "Jeremiah 37:1-21",
          "pt": "Jeremiah 37:1-21",
          "vi": "Jeremiah 37:1-21",
          "tl": "Jeremiah 37:1-21",
          "th": "Jeremiah 37:1-21",
          "sw": "Jeremiah 37:1-21",
          "it": "Geremia 37:1-21"
        },
        comment: {
          "ja": "真実を伝えただけなのに牢屋に入れられるエレミヤ、正論を言うと空気が読めない奴扱いされる現代の職場環境そのもの。",
          "en": "Jeremiah getting thrown in prison just for telling the truth is a harsh reminder of how society treats people who refuse to sugarcoat things.",
          "ko": "진실을 말했다는 이유로 감옥에 갇히는 예레미야를 보니, 쓴소리 한번 했다가 분위기 파악 못 하는 사람 취급받는 요즘 직장 생활이 떠오른다.",
          "zho": "耶利米只因說了實話就被關進牢裡，這簡直就是現代職場中說真話卻被當成白目的真實寫照。",
          "es": "Jeremías siendo encarcelado por decir la verdad es un recordatorio de lo difícil que es ser la única persona honesta en una habitación llena de gente que prefiere mentiras cómodas.",
          "pt": "Jeremias ser preso apenas por dizer a verdade é um lembrete de como a sociedade trata quem se recusa a adoçar as palavras.",
          "vi": "Việc Giê-rê-mi bị tống giam chỉ vì nói ra sự thật cho thấy cái giá phải trả khi bạn là người duy nhất dám thẳng thắn trong một thế giới thích nghe lời đường mật.",
          "tl": "Ang pagkakakulong kay Jeremias dahil lang sa pagsasabi ng totoo ay patunay kung gaano kahirap maging tapat sa mundong ayaw makarinig ng katotohanan.",
          "th": "เยเรมีย์ถูกขังคุกเพียงเพราะพูดความจริง เป็นเครื่องเตือนใจว่าการเป็นคนซื่อสัตย์ในโลกที่ชอบฟังแต่คำหวานนั้นมันช่างยากเย็นเพียงใด",
          "sw": "Yeremia kufungwa gerezani kwa sababu tu ya kusema ukweli ni ukumbusho wa jinsi watu wanavyomchukulia mtu anayekataa kupamba ukweli.",
          "it": "Geremia sbattuto in cella solo per aver detto la verità è lo specchio di quanto sia difficile essere sinceri in un mondo che preferisce le comodità."
        }
    },
    "2026-10-31": {
        date: "2026-10-31",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "哀歌 1:1-22",
          "en": "Lamentations 1:1-22",
          "ko": "Lamentations 1:1-22",
          "zho": "哀歌 1:1-22",
          "es": "Lamentations 1:1-22",
          "pt": "Lamentations 1:1-22",
          "vi": "Lamentations 1:1-22",
          "tl": "Lamentations 1:1-22",
          "th": "Lamentations 1:1-22",
          "sw": "Lamentations 1:1-22",
          "it": "Lamentazioni 1:1-22"
        },
        comment: {
          "ja": "かつての賑わいが嘘のように静まり返った街で独り言をこぼすエルサレム、人生で一度は味わうあの孤独な夜のリアリティが痛い。",
          "en": "Jerusalem sitting alone in a city that used to be full of life is that specific kind of hollow silence we all hit at our lowest points.",
          "ko": "한때 북적였던 도시가 순식간에 텅 비어버린 예루살렘의 모습은 인생에서 누구나 한 번쯤 겪게 되는 공허한 밤의 현실을 그대로 보여줍니다.",
          "zho": "耶路撒冷独自坐在曾经繁华如今却冷清的城中，那种人生低谷时透心凉的孤独感，真的太真实了。",
          "es": "Jerusalén sentada sola en una ciudad que solía estar llena de vida es ese tipo de silencio hueco que todos sentimos en nuestros peores momentos.",
          "pt": "Jerusalém sentada sozinha em uma cidade que costumava ser cheia de vida é aquele tipo de silêncio vazio que todos nós sentimos nos nossos piores momentos.",
          "vi": "Jerusalem ngồi một mình trong thành phố từng náo nhiệt giờ vắng lặng, cảm giác cô đơn trống rỗng đó chính là hiện thực mà ai cũng từng nếm trải.",
          "tl": "Ang pag-upo ni Jerusalem nang mag-isa sa isang lungsod na dati ay puno ng buhay ay ang uri ng katahimikan na nararanasan nating lahat sa ating pinakamababang sandali.",
          "th": "เยรูซาเล็มที่นั่งอยู่ลำพังในเมืองที่เคยคึกคักคือความเงียบเหงาที่บาดลึกแบบที่เราทุกคนเคยเจอในช่วงเวลาที่แย่ที่สุดในชีวิต",
          "sw": "Yerusalemu kukaa peke yake katika mji uliokuwa na watu wengi ni ile hali ya upweke ambayo sote tunapitia tunapokuwa kwenye wakati mgumu.",
          "it": "Gerusalemme seduta da sola in una città silenziosa rappresenta esattamente quel vuoto che tutti proviamo nei momenti più difficili."
        }
    },
    "2026-11-01": {
        date: "2026-11-01",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "哀歌 3:1-66",
          "en": "Lamentations 3:1-66",
          "ko": "Lamentations 3:1-66",
          "zho": "哀歌 3:1-66",
          "es": "Lamentations 3:1-66",
          "pt": "Lamentations 3:1-66",
          "vi": "Lamentations 3:1-66",
          "tl": "Lamentations 3:1-66",
          "th": "Lamentations 3:1-66",
          "sw": "Lamentations 3:1-66",
          "it": "Lamentazioni 3:1-66"
        },
        comment: {
          "ja": "散々な目に遭いながらも「神様は朝ごとに新しい」とつぶやく筆者、どん底でも前向きになろうとする姿勢がいじらしくて逆に泣けてくる。",
          "en": "The author getting absolutely wrecked by life but still insisting that God's mercies are new every morning is the kind of stubborn hope that hits different.",
          "ko": "온갖 고난을 겪으면서도 아침마다 하나님의 자비가 새롭다고 되뇌는 모습은, 밑바닥에서도 희망을 붙들려는 인간의 애처로운 분투를 보여줍니다.",
          "zho": "在经历了一切苦难后依然坚持说上帝的怜悯每天早晨都是新的，这种在谷底强行乐观的态度，反而让人觉得心酸又动容。",
          "es": "El autor, después de sufrir tanto, insistiendo en que las misericordias de Dios son nuevas cada mañana es esa esperanza terca que nos rompe el corazón.",
          "pt": "O autor, depois de sofrer tanto, insistindo que as misericórdias de Deus são novas a cada manhã é aquele tipo de esperança teimosa que nos toca profundamente.",
          "vi": "Tác giả dù chịu đủ mọi đắng cay vẫn khẳng định lòng thương xót của Chúa luôn mới mỗi sáng, một sự kiên cường đầy hy vọng khiến người ta vừa xót xa vừa ngưỡng mộ.",
          "tl": "Ang manunulat na dumanas ng matinding hirap pero pilit pa ring naniniwalang bago ang awa ng Diyos tuwing umaga ay isang uri ng pag-asang nakakapukaw ng damdamin.",
          "th": "ผู้เขียนที่เจอเรื่องร้ายมาสารพัดแต่ยังยืนยันว่าความเมตตาของพระเจ้าใหม่เสมอในทุกเช้า คือความหวังที่ดื้อรั้นจนน่าเห็นใจและซึ้งใจไปพร้อมกัน",
          "sw": "Mwandishi kupitia mateso mengi lakini bado akisisitiza kuwa huruma za Mungu ni mpya kila asubuhi ni ile hali ya kutokukata tamaa inayotugusa ndani kabisa.",
          "it": "L'autore che dopo aver subito di tutto ripete che la misericordia si rinnova ogni mattina ci mostra una speranza testarda che commuove."
        }
    },
    "2026-11-02": {
        date: "2026-11-02",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エゼキエル書 1:1-28",
          "en": "Ezekiel 1:1-28",
          "ko": "Ezekiel 1:1-28",
          "zho": "エゼキエル書 1:1-28",
          "es": "Ezekiel 1:1-28",
          "pt": "Ezekiel 1:1-28",
          "vi": "Ezekiel 1:1-28",
          "tl": "Ezekiel 1:1-28",
          "th": "Ezekiel 1:1-28",
          "sw": "Ezekiel 1:1-28",
          "it": "Ezechiele 1:1-28"
        },
        comment: {
          "ja": "エゼキエルが見たあまりに難解なビジョン、もはや何が何だかわからないまま圧倒されるしかない感覚は、人生の不可解な出来事に直面した時の私たちの姿そのものだ。",
          "en": "Ezekiel trying to process a vision that defies all logic is exactly how we feel when life throws us something we just cannot explain.",
          "ko": "에스겔이 본 이해할 수 없는 환상은 인생의 도저히 풀리지 않는 난제를 마주했을 때 우리가 느끼는 당혹감과 똑 닮았다.",
          "zho": "以西結看見的異象如此深奧難解，那種被震撼到不知所措的感覺，就像我們面對人生難題時的真實寫照。",
          "es": "La visión de Ezequiel es tan compleja que nos deja sin palabras, tal como nos sentimos cuando la vida nos presenta situaciones que no tienen sentido alguno.",
          "pt": "A visão de Ezequiel é tão complexa que nos deixa perplexos, exatamente como nos sentimos quando a vida nos apresenta situações sem explicação.",
          "vi": "Khải tượng của Ê-xê-chi-ên quá khó hiểu khiến ta chỉ biết lặng người, giống hệt cảm giác khi đối mặt với những biến cố kỳ quặc trong đời.",
          "tl": "Ang nakakalitong bisyon ni Ezekiel ay parang buhay natin na minsan ay hindi natin maipaliwanag kung bakit nangyayari ang mga bagay-bagay.",
          "th": "นิมิตที่ซับซ้อนเกินเข้าใจของเอเสเคียลเปรียบเสมือนเหตุการณ์ในชีวิตที่ทำให้เรามึนงงจนทำอะไรไม่ถูกเหมือนกัน",
          "sw": "Maono ya Ezekieli yanayochanganya akili ni kama vile maisha yanavyotuletea mambo tusiyoweza kuelewa hata kidogo.",
          "it": "La visione di Ezechiele è così complessa da lasciarci senza parole, proprio come quando ci troviamo davanti a situazioni inspiegabili."
        }
    },
    "2026-11-03": {
        date: "2026-11-03",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エゼキエル書 2:1-10",
          "en": "Ezekiel 2:1-10",
          "ko": "Ezekiel 2:1-10",
          "zho": "エゼキエル書 2:1-10",
          "es": "Ezekiel 2:1-10",
          "pt": "Ezekiel 2:1-10",
          "vi": "Ezekiel 2:1-10",
          "tl": "Ezekiel 2:1-10",
          "th": "Ezekiel 2:1-10",
          "sw": "Ezekiel 2:1-10",
          "it": "Ezechiele 2:1-10"
        },
        comment: {
          "ja": "神様から渡された巻物を丸ごと食べてこいと言われるエゼキエル、断ることもできず腹をくくって飲み込む姿は、苦手な仕事を任された時の私たちの精一杯の覚悟だ。",
          "en": "Ezekiel being told to eat a scroll and just doing it is the ultimate mood for anyone who has ever had to swallow a bitter pill at work.",
          "ko": "두루마리를 먹으라는 명령에 군말 없이 삼켜버리는 에스겔의 모습은, 하기 싫은 일을 억지로 해내야 하는 우리네 직장인의 애환과 별다를 게 없다.",
          "zho": "以西結被要求吃下書卷就乖乖照做，這簡直就是我們在職場上硬著頭皮吞下苦差事時的寫照。",
          "es": "A Ezequiel le piden que se coma un rollo y él simplemente lo hace, lo cual es el estado de ánimo perfecto cuando nos toca aceptar una tarea que no queremos hacer.",
          "pt": "Ezequiel recebe a ordem de comer um rolo e simplesmente obedece, exatamente como fazemos quando precisamos engolir sapos no trabalho.",
          "vi": "Ê-xê-chi-ên được bảo ăn cuộn sách và ông làm theo, đó chính là tâm trạng của chúng ta mỗi khi phải cắn răng nhận lấy một công việc khó nhằn.",
          "tl": "Ang pagsunod ni Ezekiel na kainin ang scroll ay parang tayo na napipilitang tanggapin ang mahirap na trabahong ayaw naman nating gawin.",
          "th": "เอเสเคียลได้รับคำสั่งให้กินม้วนหนังสือแล้วเขาก็ทำตาม นี่คืออารมณ์เดียวกับเวลาที่เราต้องจำใจรับงานยากๆ ที่ไม่อยากทำเลย",
          "sw": "Ezekieli kuambiwa ale gombo na yeye kutii bila swali ni kama sisi tunavyolazimika kukubali majukumu magumu kazini bila kupenda.",
          "it": "A Ezechiele viene chiesto di ingoiare un rotolo e lui semplicemente lo fa, proprio come quando dobbiamo accettare un compito scomodo senza poter fiatare."
        }
    },
    "2026-11-04": {
        date: "2026-11-04",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エゼキエル書 33:1-33",
          "en": "Ezekiel 33:1-33",
          "ko": "Ezekiel 33:1-33",
          "zho": "エゼキエル書 33:1-33",
          "es": "Ezekiel 33:1-33",
          "pt": "Ezekiel 33:1-33",
          "vi": "Ezekiel 33:1-33",
          "tl": "Ezekiel 33:1-33",
          "th": "Ezekiel 33:1-33",
          "sw": "Ezekiel 33:1-33",
          "it": "Ezechiele 33:1-33"
        },
        comment: {
          "ja": "見張り番の話を聞き流して結局手遅れになるエゼキエルの時代の民、通知を無視して締め切りギリギリに焦る現代人の姿そのものだ。",
          "en": "The people ignoring the watchman until it is too late is just like us ignoring every notification until the deadline hits.",
          "ko": "파수꾼의 경고를 흘려듣다가 뒤늦게 발등에 불이 떨어진 사람들을 보니 마감 직전에야 허둥대는 내 모습이 떠오른다.",
          "zho": "無視守望者的警告直到最後才後悔，這簡直就是現代人無視提醒直到死線臨頭時的寫照。",
          "es": "Ignorar al centinela hasta que es demasiado tarde es el equivalente bíblico a ignorar todas las notificaciones hasta que se vence el plazo.",
          "pt": "Ignorar o vigia até ser tarde demais é exatamente como ignorar todas as notificações até o prazo final estourar.",
          "vi": "Phớt lờ lời cảnh báo của người canh gác rồi mới hối hận, chẳng khác nào chúng ta phớt lờ thông báo cho đến khi hạn chót ập đến.",
          "tl": "Ang pagbalewala sa babala hanggang sa huli ay parang tayo na hindi pinapansin ang mga notification hanggang sa deadline na.",
          "th": "การเมินเฉยต่อคำเตือนจนสายเกินไปนั้นไม่ต่างอะไรกับการที่เรากดปิดการแจ้งเตือนรัวๆ จนถึงนาทีสุดท้ายก่อนงานส่ง",
          "sw": "Kupuuza onyo la mlinzi hadi wakati unapoisha ni kama sisi tunavyopuuza arifa zote hadi dakika ya mwisho.",
          "it": "Ignorare il guardiano fino all'ultimo secondo è proprio come ignorare tutte le notifiche sullo smartphone prima di una scadenza importante."
        }
    },
    "2026-11-05": {
        date: "2026-11-05",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エゼキエル書 34:1-31",
          "en": "Ezekiel 34:1-31",
          "ko": "Ezekiel 34:1-31",
          "zho": "エゼキエル書 34:1-31",
          "es": "Ezekiel 34:1-31",
          "pt": "Ezekiel 34:1-31",
          "vi": "Ezekiel 34:1-31",
          "tl": "Ezekiel 34:1-31",
          "th": "Ezekiel 34:1-31",
          "sw": "Ezekiel 34:1-31",
          "it": "Ezechiele 34:1-31"
        },
        comment: {
          "ja": "自分だけ肥えて群れを顧みない牧者たち、組織のリーダーによくある残念なあるあるを数千年前から指摘されている。",
          "en": "Leaders who get fat while neglecting the flock are a cautionary tale that has been relevant for thousands of years.",
          "ko": "양떼는 돌보지 않고 자기 배만 불리는 목자들을 보니 어느 시대나 리더의 자질 문제는 변함이 없다는 게 씁쓸하다.",
          "zho": "只顧著養肥自己卻不照顧羊群的牧人，這種領導者的通病幾千年來始終如一。",
          "es": "Esos pastores que se engordan a sí mismos mientras descuidan al rebaño son el ejemplo perfecto de un liderazgo que ha fallado durante siglos.",
          "pt": "Pastores que engordam a si mesmos enquanto negligenciam o rebanho são o exemplo clássico de um erro de liderança que nunca sai de moda.",
          "vi": "Những người chăn nuôi chỉ biết làm béo bản thân mà bỏ mặc đàn cừu là một ví dụ kinh điển về sự thất bại trong lãnh đạo suốt bao thế kỷ qua.",
          "tl": "Ang mga pastol na pinapataba ang sarili habang pinapabayaan ang kawan ay klasikong halimbawa ng maling pamumuno na hindi nagbabago sa paglipas ng panahon.",
          "th": "ผู้นำที่มัวแต่กอบโกยผลประโยชน์โดยไม่สนใจลูกน้องคือปัญหาคลาสสิกที่เกิดขึ้นซ้ำรอยมานานหลายพันปีแล้ว",
          "sw": "Wachungaji wanaojinufaisha huku wakipuuza kundi ni mfano kamili wa uongozi mbaya ambao umeendelea kwa maelfu ya miaka.",
          "it": "Quel tipo di leader che pensa solo a se stesso e si dimentica degli altri è purtroppo un classico senza tempo."
        }
    },
    "2026-11-06": {
        date: "2026-11-06",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エゼキエル書 36:1-38",
          "en": "Ezekiel 36:1-38",
          "ko": "Ezekiel 36:1-38",
          "zho": "エゼキエル書 36:1-38",
          "es": "Ezekiel 36:1-38",
          "pt": "Ezekiel 36:1-38",
          "vi": "Ezekiel 36:1-38",
          "tl": "Ezekiel 36:1-38",
          "th": "Ezekiel 36:1-38",
          "sw": "Ezekiel 36:1-38",
          "it": "Ezechiele 36:1-38"
        },
        comment: {
          "ja": "ボロボロの廃墟を再建するなんて無理だと諦めそうになるけれど、神様は意外と私たちの伸びしろを信じて疑わない。",
          "en": "God believing in the restoration of our personal ruins when we have already given up is honestly a bit overwhelming.",
          "ko": "다 무너져가는 폐허를 보며 포기하려 할 때 하나님은 오히려 우리의 가능성을 굳게 믿고 계신다는 점이 참 묘하다.",
          "zho": "當我們看著滿目瘡痍的廢墟想放棄時，上帝卻對我們的潛力有著莫名其妙的信心。",
          "es": "Cuando nos rendimos ante nuestras propias ruinas, Dios sigue apostando por nuestra reconstrucción con una paciencia casi irritante.",
          "pt": "Quando olhamos para nossas ruínas e desistimos, Deus continua acreditando na nossa restauração com uma teimosia divina.",
          "vi": "Khi chúng ta nhìn vào những đống đổ nát của chính mình và muốn bỏ cuộc, Chúa lại luôn tin vào tiềm năng phục hồi của chúng ta một cách lạ kỳ.",
          "tl": "Habang tayo ay sumusuko na sa ating mga pagkakamali, ang Diyos ay patuloy na naniniwala sa ating kakayahang magbago.",
          "th": "ในขณะที่เรามองเห็นแต่ซากปรักหักพังในชีวิตและอยากจะยอมแพ้ พระเจ้ากลับเชื่อมั่นในศักยภาพที่จะฟื้นฟูเราได้อย่างน่าประหลาดใจ",
          "sw": "Wakati tunapoona magofu katika maisha yetu na kutaka kukata tamaa, Mungu anaendelea kuamini uwezo wetu wa kurejeshewa hali ya awali.",
          "it": "Quando pensiamo che tutto sia distrutto e irrecuperabile, c'è sempre qualcuno che continua a credere in noi."
        }
    },
    "2026-11-07": {
        date: "2026-11-07",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エゼキエル書 37:1-28",
          "en": "Ezekiel 37:1-28",
          "ko": "Ezekiel 37:1-28",
          "zho": "エゼキエル書 37:1-28",
          "es": "Ezekiel 37:1-28",
          "pt": "Ezekiel 37:1-28",
          "vi": "Ezekiel 37:1-28",
          "tl": "Ezekiel 37:1-28",
          "th": "Ezekiel 37:1-28",
          "sw": "Ezekiel 37:1-28",
          "it": "Ezechiele 37:1-28"
        },
        comment: {
          "ja": "カサカサの骨の山に息を吹き込んで生き返らせる光景は、もう無理だと腐っていた自分のやる気を起こすには最高の特効薬だ。",
          "en": "Watching dry, brittle bones suddenly get up and dance is the ultimate reality check for anyone who thinks they are too far gone.",
          "ko": "말라비틀어진 뼈들이 다시 살아나는 장면은 더 이상 희망이 없다고 생각하던 내 의지를 자극하는 최고의 자극제다.",
          "zho": "看著枯骨重新長肉復活的畫面，這簡直是給那些覺得自己無可救藥的人最好的強心針。",
          "es": "Ver huesos secos cobrando vida es la dosis de realidad que necesitamos cuando sentimos que nuestra motivación está totalmente agotada.",
          "pt": "Ver ossos secos ganhando vida é o choque de realidade perfeito para quem acha que a própria esperança já virou pó.",
          "vi": "Chứng kiến những bộ xương khô bỗng nhiên hồi sinh là liều thuốc tinh thần mạnh mẽ nhất cho những ai từng nghĩ mình đã hết hy vọng.",
          "tl": "Ang makitang muling mabuhay ang mga tuyong buto ay ang kailangan nating paalala na walang sitwasyon ang masyadong patay para hindi mabigyang-buhay.",
          "th": "การได้เห็นกระดูกแห้งฟื้นคืนชีวิตขึ้นมาใหม่คือยาชูกำลังชั้นดีสำหรับคนที่เคยคิดว่าตัวเองหมดหวังไปแล้ว",
          "sw": "Kuona mifupa mikavu ikipata uhai tena ni dawa bora kabisa kwa yeyote anayehisi kuwa matumaini yake yamekwisha kabisa.",
          "it": "Vedere delle vecchie ossa secche che tornano improvvisamente a vivere è la scossa perfetta quando ci sentiamo completamente svuotati."
        }
    },
    "2026-11-08": {
        date: "2026-11-08",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "エゼキエル書 47:1-23",
          "en": "Ezekiel 47:1-23",
          "ko": "Ezekiel 47:1-23",
          "zho": "エゼキエル書 47:1-23",
          "es": "Ezekiel 47:1-23",
          "pt": "Ezekiel 47:1-23",
          "vi": "Ezekiel 47:1-23",
          "tl": "Ezekiel 47:1-23",
          "th": "Ezekiel 47:1-23",
          "sw": "Ezekiel 47:1-23",
          "it": "Ezechiele 47:1-23"
        },
        comment: {
          "ja": "ちょろちょろ流れる小さな水が気づけば泳ぐしかない大河になる様子は、神様の恵みがいつの間にか自分を包み込んでいる現実そのもの。",
          "en": "Watching a tiny trickle turn into an uncrossable river is a perfect metaphor for how grace sneaks up on you until you are completely submerged.",
          "ko": "졸졸 흐르는 작은 물줄기가 어느새 헤엄쳐야 할 큰 강이 되는 모습은 우리 삶에 스며드는 은혜의 규모를 보여주는 것 같아 새삼 놀랍다.",
          "zho": "看著涓涓細流最後變成無法橫越的大河，就像是神的恩典總在不知不覺中將我們完全包圍。",
          "es": "Ver cómo un pequeño arroyo se convierte en un río inabarcable es la prueba de que la gracia divina siempre termina superando nuestras expectativas.",
          "pt": "Ver um pequeno fio de água se transformar em um rio profundo é a prova de que a graça divina sempre acaba nos cobrindo por completo.",
          "vi": "Nhìn dòng nước nhỏ dần biến thành con sông lớn không thể lội qua, ta thấy được cách ân điển của Chúa âm thầm bao phủ cuộc đời mình.",
          "tl": "Ang makitang ang maliit na agos ay nagiging malalim na ilog ay patunay na ang biyaya ng Diyos ay hindi natin kayang sukatin.",
          "th": "การเห็นสายน้ำเล็กๆ กลายเป็นแม่น้ำที่กว้างใหญ่จนต้องว่ายข้าม เป็นภาพเปรียบเทียบที่ชัดเจนว่าพระคุณของพระเจ้าครอบคลุมชีวิตเรามากแค่ไหน",
          "sw": "Kuona kijito kidogo kikiwa mto mkubwa usioweza kuvukika ni kielelezo kamili cha jinsi neema ya Mungu inavyotuzidi kila siku.",
          "it": "Quel piccolo ruscello che diventa un fiume impossibile da attraversare dimostra come la gentilezza riesca sempre a sorprenderci."
        }
    },
    "2026-11-09": {
        date: "2026-11-09",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ダニエル書 1:1-21",
          "en": "Daniel 1:1-21",
          "ko": "Daniel 1:1-21",
          "zho": "ダニエル書 1:1-21",
          "es": "Daniel 1:1-21",
          "pt": "Daniel 1:1-21",
          "vi": "Daniel 1:1-21",
          "tl": "Daniel 1:1-21",
          "th": "Daniel 1:1-21",
          "sw": "Daniel 1:1-21",
          "it": "Daniele 1:1-21"
        },
        comment: {
          "ja": "王様の豪華な食事を断って豆と水で勝負に出たダニエル、結局は見た目も能力も一番という結果を出して、健康志向が最強であることを証明してしまった。",
          "en": "Daniel turning down the king's buffet to eat beans and water, only to end up looking better than everyone else, is the ultimate win for healthy living.",
          "ko": "왕의 기름진 음식을 거절하고 채식으로 버틴 다니엘이 결국 모두를 압도하는 결과를 낸 건, 역시 자기 관리가 최고의 복수라는 걸 보여준다.",
          "zho": "但以理拒絕了國王的奢華盛宴，堅持吃素喝水，最後卻證明了自律的人狀態最好，這簡直是現代健康生活的終極榜樣。",
          "es": "Daniel rechazando el banquete real para comer legumbres y terminar viéndose mejor que todos es la prueba definitiva de que la disciplina siempre gana.",
          "pt": "Daniel recusando o banquete do rei para comer apenas vegetais e ainda assim superar a todos é a prova definitiva de que a disciplina compensa.",
          "vi": "Daniel từ chối yến tiệc của vua để ăn rau đậu mà vẫn khỏe mạnh hơn người khác, đúng là minh chứng cho việc kỷ luật bản thân luôn mang lại kết quả tốt nhất.",
          "tl": "Ang pagtanggi ni Daniel sa handaan ng hari para kumain ng gulay at tubig, at ang pagiging mas magaling niya kaysa sa iba, ay patunay na ang disiplina ang susi sa tagumpay.",
          "th": "การที่ดาเนียลปฏิเสธอาหารหรูของราชาเพื่อกินเพียงพืชผักแต่กลับดูดีกว่าทุกคน เป็นบทพิสูจน์ว่าวินัยในตัวเองนั้นทรงพลังแค่ไหน",
          "sw": "Daniel kukataa karamu ya mfalme ili ale mboga tu na bado akashinda wote, ni uthibitisho kwamba nidhamu binafsi ndiyo njia bora ya kufanikiwa.",
          "it": "Rifiutare i banchetti sontuosi per un po' d'acqua e verdura e finire in gran forma è la dimostrazione che uno stile di vita semplice vince sempre."
        }
    },
    "2026-11-10": {
        date: "2026-11-10",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ダニエル書 2:1-49",
          "en": "Daniel 2:1-49",
          "ko": "Daniel 2:1-49",
          "zho": "ダニエル書 2:1-49",
          "es": "Daniel 2:1-49",
          "pt": "Daniel 2:1-49",
          "vi": "Daniel 2:1-49",
          "tl": "Daniel 2:1-49",
          "th": "Daniel 2:1-49",
          "sw": "Daniel 2:1-49",
          "it": "Daniele 2:1-49"
        },
        comment: {
          "ja": "夢の内容を言わずに解釈しろと迫る王、部下の心中を察する能力が皆無すぎて今の職場にもいそうなタイプだ。",
          "en": "Demanding someone interpret a dream without telling them what it was is the ultimate boss move of someone who has zero clue how to communicate.",
          "ko": "꿈 내용을 말하지도 않고 해석하라는 왕의 요구를 보니 소통 단절이 심각한 직장 상사가 떠오른다.",
          "zho": "不說夢境內容卻要人解夢，這種強人所難的領導風格，簡直是現代職場上的惡夢。",
          "es": "Pedir que interpreten un sueño sin decir cuál fue es el nivel máximo de exigencia de un jefe que no sabe comunicarse.",
          "pt": "Exigir a interpretação de um sonho sem contar o sonho é o nível máximo de um chefe que não sabe o que quer.",
          "vi": "Yêu cầu giải mã giấc mơ mà không nói giấc mơ đó là gì, đúng là kiểu sếp thích làm khó nhân viên kinh điển.",
          "tl": "Ang utusan ang mga tao na ipaliwanag ang panaginip nang hindi naman sinasabi kung ano ito ay parang boss na walang malinaw na instruction.",
          "th": "การสั่งให้คนอื่นทายฝันโดยไม่บอกว่าฝันว่าอะไร เป็นสไตล์หัวหน้าที่เอาแต่ใจจนน่าปวดหัวจริงๆ",
          "sw": "Kumtaka mtu atafsiri ndoto bila kukuambia ndoto hiyo ni mtindo wa bosi asiye na ujuzi wa mawasiliano.",
          "it": "Pretendere che qualcuno indovini un sogno senza nemmeno raccontarglielo è la mossa tipica di un capo che non sa comunicare."
        }
    },
    "2026-11-11": {
        date: "2026-11-11",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ダニエル書 3:1-30",
          "en": "Daniel 3:1-30",
          "ko": "Daniel 3:1-30",
          "zho": "ダニエル書 3:1-30",
          "es": "Daniel 3:1-30",
          "pt": "Daniel 3:1-30",
          "vi": "Daniel 3:1-30",
          "tl": "Daniel 3:1-30",
          "th": "Daniel 3:1-30",
          "sw": "Daniel 3:1-30",
          "it": "Daniele 3:1-30"
        },
        comment: {
          "ja": "巨大な金の像を建てて全員に拝ませようとする王、承認欲求の暴走がすぎて逆に哀れにすら思えてくる。",
          "en": "Building a giant gold statue just to force everyone to bow down is the most insecure cry for attention in history.",
          "ko": "거대한 금 신상을 세워 모두에게 절하게 만드는 왕을 보니, 인정 욕구가 지나치면 얼마나 우스꽝스러워지는지 알 것 같다.",
          "zho": "為了讓所有人跪拜而建造巨大的金像，這種過度的虛榮心其實暴露了內心的極度空虛。",
          "es": "Construir una estatua de oro gigante solo para obligar a todos a arrodillarse es la forma más patética de pedir atención.",
          "pt": "Construir uma estátua de ouro gigante só para forçar todo mundo a se curvar é a maior carência que já vi na história.",
          "vi": "Xây tượng vàng khổng lồ chỉ để ép mọi người quỳ lạy, đúng là kiểu khao khát được công nhận đến mức lố bịch.",
          "tl": "Ang pagpapatayo ng dambuhalang rebultong ginto para lang pilitin ang lahat na sumamba ay desperadong paraan para mapansin.",
          "th": "การสร้างรูปปั้นทองคำยักษ์เพื่อบังคับให้ทุกคนกราบไหว้ เป็นวิธีเรียกร้องความสนใจที่ดูน่าสมเพชที่สุดในประวัติศาสตร์",
          "sw": "Kujenga sanamu kubwa ya dhahabu ili kulazimisha watu wainame ni njia ya kijinga sana ya kutaka sifa.",
          "it": "Costruire una statua d'oro gigante solo per farsi adorare da tutti è il grido d'attenzione più insicuro della storia."
        }
    },
    "2026-11-12": {
        date: "2026-11-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ダニエル書 4:1-37",
          "en": "Daniel 4:1-37",
          "ko": "Daniel 4:1-37",
          "zho": "ダニエル書 4:1-37",
          "es": "Daniel 4:1-37",
          "pt": "Daniel 4:1-37",
          "vi": "Daniel 4:1-37",
          "tl": "Daniel 4:1-37",
          "th": "Daniel 4:1-37",
          "sw": "Daniel 4:1-37",
          "it": "Daniele 4:1-37"
        },
        comment: {
          "ja": "天下を取って傲慢の極みにいた王が、最終的に草を食べて正気に戻るという展開、人生の絶頂で調子に乗るなという強烈な戒めすぎる。",
          "en": "A king going from the peak of human arrogance to eating grass in the field is a humbling reminder that ego always has a shelf life.",
          "ko": "천하를 호령하던 왕이 오만함의 끝을 달리다 결국 들판에서 풀을 뜯게 되는 모습은, 인생의 정점에서 겸손을 잃지 말라는 뼈 때리는 교훈이다.",
          "zho": "曾經不可一世的國王最後淪落到吃草才找回理智，這簡直是給那些在巔峰時期過度膨脹的人最殘酷的警示。",
          "es": "Ver a un rey pasar de la cima de la soberbia a comer hierba en el campo es el recordatorio definitivo de que el ego siempre termina pasándonos factura.",
          "pt": "Ver um rei ir do auge da arrogância para pastar como um animal é o lembrete definitivo de que o ego sempre tem um prazo de validade.",
          "vi": "Một vị vua từ đỉnh cao của sự kiêu ngạo rơi xuống đến mức phải ăn cỏ ngoài đồng chính là lời nhắc nhở rằng cái tôi quá lớn sẽ luôn khiến ta phải trả giá.",
          "tl": "Ang makitang ang isang hari na nasa tuktok ng kayabangan ay mauwi sa pagkain ng damo ay isang matinding paalala na ang ego ay may hangganan.",
          "th": "กษัตริย์ที่เคยอยู่จุดสูงสุดของความเย่อหยิ่งแต่สุดท้ายต้องไปกินหญ้าคือบทเรียนราคาแพงที่สอนว่าอย่าหลงระเริงในอำนาจจนลืมตัว",
          "sw": "Mfalme anayepanda kutoka kilele cha kiburi na kuishia kula majani ni ukumbusho mkali kwamba kiburi hakidumu milele.",
          "it": "Passare dal culmine della superbia a brucare l'erba nel campo è il promemoria definitivo che l'ego ha sempre una data di scadenza."
        }
    },
    "2026-11-13": {
        date: "2026-11-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ダニエル書 5:1-31",
          "en": "Daniel 5:1-31",
          "ko": "Daniel 5:1-31",
          "zho": "ダニエル書 5:1-31",
          "es": "Daniel 5:1-31",
          "pt": "Daniel 5:1-31",
          "vi": "Daniel 5:1-31",
          "tl": "Daniel 5:1-31",
          "th": "Daniel 5:1-31",
          "sw": "Daniel 5:1-31",
          "it": "Daniele 5:1-31"
        },
        comment: {
          "ja": "神殿の聖なる器で酒盛りをしていたら突然壁に文字が浮かび上がるというホラー展開、調子に乗って線を超えると何が起きるか分からないという教訓。",
          "en": "Partying with holy relics only to have a mysterious hand write your doom on the wall is the ultimate warning that there are lines you really shouldn't cross.",
          "ko": "성전의 기물로 술판을 벌이다 벽에 나타난 글씨를 보고 공포에 질리는 모습은, 선을 넘으면 어떤 대가가 따르는지 보여주는 현실적인 공포물이다.",
          "zho": "拿著聖殿器皿狂歡到一半，牆上突然浮現死亡預告，這簡直是挑戰底線後最令人措手不及的翻車現場。",
          "es": "Hacer una fiesta con objetos sagrados y ver cómo una mano escribe tu destino en la pared es la definición gráfica de lo que pasa cuando cruzas un límite que no debías.",
          "pt": "Fazer uma festa com objetos sagrados e ver uma mão escrever seu destino na parede é a prova definitiva de que algumas linhas não devem ser cruzadas.",
          "vi": "Tiệc tùng bằng đồ thánh rồi đột nhiên thấy chữ viết hiện lên trên tường, đây đúng là bài học nhớ đời về việc biết đâu là giới hạn không nên vượt qua.",
          "tl": "Ang magpakalasing gamit ang mga banal na kagamitan at biglang makakita ng sulat sa dingding ay isang nakakatakot na paalala na may mga hangganang hindi dapat nilalagpasan.",
          "th": "การจัดปาร์ตี้ด้วยของศักดิ์สิทธิ์แล้วจู่ๆ มีมือปริศนามาเขียนคำพิพากษาบนกำแพงคือบทเรียนชั้นดีว่าอย่าหาทำในสิ่งที่ข้ามเส้นเกินไป",
          "sw": "Kufanya sherehe kwa vyombo vitakatifu kisha kuona maandishi ya ajabu ukutani ni onyo kali kwamba kuna mipaka ambayo hupaswi kamwe kuivuka.",
          "it": "Festeggiare con gli oggetti sacri e ritrovarsi una mano misteriosa che scrive sul muro è l'avvertimento perfetto a non oltrepassare certi limiti."
        }
    },
    "2026-11-14": {
        date: "2026-11-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ダニエル書 6:1-28",
          "en": "Daniel 6:1-28",
          "ko": "Daniel 6:1-28",
          "zho": "ダニエル書 6:1-28",
          "es": "Daniel 6:1-28",
          "pt": "Daniel 6:1-28",
          "vi": "Daniel 6:1-28",
          "tl": "Daniel 6:1-28",
          "th": "Daniel 6:1-28",
          "sw": "Daniel 6:1-28",
          "it": "Daniele 6:1-28"
        },
        comment: {
          "ja": "同僚の嫉妬で罠にかけられたダニエルが、それでも窓を開けて祈り続ける姿は、理不尽な職場環境で心を保つ究極のライフハックだ。",
          "en": "Daniel keeping his prayer routine open to the public even when his coworkers are literally plotting his demise is the ultimate masterclass in professional boundaries.",
          "ko": "동료들의 시기 때문에 사자 굴에 들어갈 상황에서도 꿋꿋하게 창문을 열고 기도하는 다니엘은 불합리한 직장 생활의 고수다.",
          "zho": "但以理在同事設下陷阱時依然開窗禱告，這種在惡劣職場環境中保持初心的舉動，簡直是心靈防禦的極致。",
          "es": "Daniel siendo enviado al foso de los leones por ser demasiado honesto es la prueba de que en el trabajo siempre habrá alguien envidioso de tu buena racha.",
          "pt": "Daniel sendo jogado na cova dos leões só por manter seus princípios é o retrato perfeito de como o sucesso incomoda quem não tem nada para oferecer.",
          "vi": "Việc Đa-ni-ên vẫn mở cửa sổ cầu nguyện dù biết đồng nghiệp đang gài bẫy là bài học đỉnh cao về việc giữ vững lập trường nơi công sở độc hại.",
          "tl": "Ang pagdarasal ni Daniel sa harap ng bintana kahit alam niyang may nagbabantay na mga inggitero ay ang ultimate version ng hindi pagpapaapekto sa toxic na katrabaho.",
          "th": "การที่ดาเนียลยังคงเปิดหน้าต่างอธิษฐานทั้งที่รู้ว่าเพื่อนร่วมงานกำลังหาเรื่องกำจัดเขา เป็นวิธีรักษาใจท่ามกลางสภาพแวดล้อมที่แย่ได้ดีที่สุด",
          "sw": "Daniel kuendelea kusali kwa uwazi hata wakati wenzake wakimtega ni mfano bora wa jinsi ya kutoyumba katika mazingira ya kazi yenye fitina.",
          "it": "Daniele che continua a pregare con la finestra aperta nonostante i colleghi tramino contro di lui è la prova che la coerenza sul lavoro richiede una dose notevole di faccia tosta."
        }
    },
    "2026-11-15": {
        date: "2026-11-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ダニエル書 7:1-28",
          "en": "Daniel 7:1-28",
          "ko": "Daniel 7:1-28",
          "zho": "ダニエル書 7:1-28",
          "es": "Daniel 7:1-28",
          "pt": "Daniel 7:1-28",
          "vi": "Daniel 7:1-28",
          "tl": "Daniel 7:1-28",
          "th": "Daniel 7:1-28",
          "sw": "Daniel 7:1-28",
          "it": "Daniele 7:1-28"
        },
        comment: {
          "ja": "ダニエルが見た恐ろしい獣たちの夢、現代の混沌としたニュースフィードを夜中に見続けてしまった時の心理状態と大差ない気がする。",
          "en": "Daniel having a nightmare about terrifying beasts is basically what happens when you scroll through the news right before trying to sleep.",
          "ko": "다니엘이 본 기괴한 짐승들의 환상은 자기 전에 자극적인 뉴스 피드를 너무 오래 본 날 밤의 내 머릿속과 별반 다르지 않다.",
          "zho": "但以理夢見那些怪獸的場景，簡直就像在深夜裡刷了太多負面新聞後，腦海中揮之不去的混亂畫面。",
          "es": "Los sueños de Daniel sobre bestias aterradoras se parecen mucho a cuando te quedas viendo noticias perturbadoras justo antes de irte a dormir.",
          "pt": "Os sonhos de Daniel com feras assustadoras soam exatamente como o que acontece quando a gente lê notícias demais antes de apagar a luz.",
          "vi": "Những giấc mơ về các con thú đáng sợ của Đa-ni-ên chẳng khác nào trạng thái tâm trí sau khi lỡ tay lướt tin tức tiêu cực quá khuya.",
          "tl": "Ang panaginip ni Daniel tungkol sa mga nakakatakot na hayop ay parang yung nararamdaman mo kapag masyado kang babad sa mga negatibong balita bago matulog.",
          "th": "ความฝันเรื่องสัตว์ร้ายของดาเนียลไม่ต่างอะไรกับสภาพจิตใจตอนที่เราไถฟีดข่าวเครียดๆ ก่อนนอนเลยสักนิด",
          "sw": "Ndoto za Daniel kuhusu wanyama wa kutisha zinafanana kabisa na hali ya akili unayopata baada ya kusoma habari za kusikitisha kabla ya kulala.",
          "it": "Gli incubi di Daniele sulle bestie spaventose sono esattamente quello che ti succede quando passi troppo tempo a leggere notizie angoscianti prima di spegnere la luce."
        }
    },
    "2026-11-16": {
        date: "2026-11-16",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ホセア書 1:1-11",
          "en": "Hosea 1:1-11",
          "ko": "Hosea 1:1-11",
          "zho": "ホセア書 1:1-11",
          "es": "Hosea 1:1-11",
          "pt": "Hosea 1:1-11",
          "vi": "Hosea 1:1-11",
          "tl": "Hosea 1:1-11",
          "th": "Hosea 1:1-11",
          "sw": "Hosea 1:1-11",
          "it": "Osea 1:1-11"
        },
        comment: {
          "ja": "神様からとんでもない試練を与えられたホセアの困惑ぶりは、理不尽な上司の指示に頭を抱える現代人の心境そのもの。",
          "en": "Hosea being asked to live out his personal life as a divine object lesson is the ultimate level of 'it's not in my job description'.",
          "ko": "하나님에게 말도 안 되는 지시를 받고 당황하는 호세아의 모습은 무리한 업무를 떠맡은 현대인의 모습과 너무나 닮았다.",
          "zho": "神要何西阿用自己的生活示範真理，這種被迫成為活教材的無奈，簡直就是職場被強迫推銷的寫照。",
          "es": "Que Dios le pida a Oseas vivir su vida como un ejemplo viviente es el nivel definitivo de 'esto no estaba en la descripción de mi puesto'.",
          "pt": "Deus pedir a Oseias para viver sua vida como uma lição objetiva é o nível definitivo de 'isso não estava na descrição do meu cargo'.",
          "vi": "Việc Chúa yêu cầu Ô-sê sống cuộc đời mình như một bài học minh họa là đỉnh cao của sự 'không nằm trong mô tả công việc'.",
          "tl": "Ang utos ng Diyos kay Oseas na gawing aral ang kanyang buhay ay parang boss na pinapagawa ka ng hindi mo naman trabaho.",
          "th": "การที่พระเจ้าให้โฮเชยาใช้ชีวิตเป็นบทเรียนสอนใจนั้น เป็นระดับสูงสุดของคำว่า 'นั่นไม่ใช่หน้าที่ของผมเลยนะ'.",
          "sw": "Mungu kumwambia Hosea aishi maisha yake kama mfano ni kiwango cha juu kabisa cha 'hili halimo kwenye maelezo ya kazi yangu'.",
          "it": "Chiedere a Osea di usare la sua vita privata come lezione divina è il livello supremo del non rientra nelle mie mansioni."
        }
    },
    "2026-11-17": {
        date: "2026-11-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ホセア書 3:1-5",
          "en": "Hosea 3:1-5",
          "ko": "Hosea 3:1-5",
          "zho": "ホセア書 3:1-5",
          "es": "Hosea 3:1-5",
          "pt": "Hosea 3:1-5",
          "vi": "Hosea 3:1-5",
          "tl": "Hosea 3:1-5",
          "th": "Hosea 3:1-5",
          "sw": "Hosea 3:1-5",
          "it": "Osea 3:1-5"
        },
        comment: {
          "ja": "裏切られた相手を自腹を切って買い戻しに行くホセア、もはや執念を超えて愛の究極の空回りを見ているようで切なくなる。",
          "en": "Hosea going out of his way to buy back someone who betrayed him is the kind of unconditional love that makes you wonder if he’s a saint or just glutton for punishment.",
          "ko": "배신한 아내를 제 돈 주고 다시 데려오는 호세아를 보면, 성인군자의 사랑인지 아니면 그냥 고통을 자처하는 건지 헷갈릴 지경이다.",
          "zho": "何西阿花錢贖回背叛自己的人，這種愛法讓人分不清這到底是聖人的慈悲，還是自找苦吃的極致。",
          "es": "Oseas yendo a recomprar a quien lo traicionó es ese tipo de amor incondicional que te hace dudar si es un santo o alguien que disfruta el castigo.",
          "pt": "Oseias indo recomprar quem o traiu é aquele tipo de amor incondicional que te faz questionar se ele é um santo ou alguém que gosta de sofrer.",
          "vi": "Việc Ô-sê bỏ tiền ra mua lại người đã phản bội mình là thứ tình yêu vô điều kiện khiến ta tự hỏi ông là thánh nhân hay là người thích tự làm khổ mình.",
          "tl": "Ang pagbawi ni Oseas sa taong nagtaksil sa kanya ay isang uri ng pagmamahal na nakakalito kung kabanalan ba o pagpapahirap lang sa sarili.",
          "th": "การที่โฮเชยาควักเงินไปไถ่ตัวคนที่หักหลังเขากลับมานั้น เป็นความรักที่ทำให้เราอดสงสัยไม่ได้ว่าเขาเป็นนักบุญหรือคนชอบหาเรื่องใส่ตัวกันแน่",
          "sw": "Hosea kwenda kumkomboa yule aliyemsaliti ni aina ya upendo usio na masharti unaokufanya ujiulize kama yeye ni mtakatifu au anajitaabisha bure.",
          "it": "Osea che spende i suoi risparmi per ricomprare chi lo ha tradito ci fa decisamente dubitare se sia un santo o semplicemente un inguaribile testardo."
        }
    },
    "2026-11-18": {
        date: "2026-11-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ホセア書 6:1-11",
          "en": "Hosea 6:1-11",
          "ko": "Hosea 6:1-11",
          "zho": "ホセア書 6:1-11",
          "es": "Hosea 6:1-11",
          "pt": "Hosea 6:1-11",
          "vi": "Hosea 6:1-11",
          "tl": "Hosea 6:1-11",
          "th": "Hosea 6:1-11",
          "sw": "Hosea 6:1-11",
          "it": "Osea 6:1-11"
        },
        comment: {
          "ja": "朝の霧のようにすぐ消える熱心さで神様にすがりつき、都合が悪くなるとすぐ忘れる我々の信仰、もはや伝統芸の域です。",
          "en": "Our devotion disappearing as quickly as morning dew the moment things get inconvenient is basically our spiritual signature move.",
          "ko": "아침 안개처럼 금방 사라지는 열심으로 매달렸다가 조금만 불편해도 까먹는 우리의 신앙은 거의 전통 예술 수준입니다.",
          "zho": "我们对神的虔诚像晨雾一样转瞬即逝，一旦遇到不顺心的事就抛诸脑后，这种健忘简直是人类的看家本领。",
          "es": "Nuestra fe es como la neblina de la mañana que se esfuma apenas las cosas se ponen difíciles; somos expertos en olvidar promesas.",
          "pt": "Nossa devoção desaparecer como o orvalho da manhã assim que as coisas ficam difíceis é praticamente nossa marca registrada.",
          "vi": "Lòng sốt sắng của chúng ta tan biến nhanh như sương sớm ngay khi gặp chút bất tiện, đúng là tài năng thiên bẩm của con người.",
          "tl": "Ang ating debosyon na parang hamog sa umaga na nawawala agad kapag hindi na maginhawa ay tunay na tatak ng ating pagkatao.",
          "th": "ความศรัทธาที่จางหายไปเหมือนหมอกยามเช้าทันทีที่เริ่มไม่สะดวกสบายนั้นถือเป็นความสามารถพิเศษของมนุษย์เราจริงๆ",
          "sw": "Ibada yetu inayotoweka kama umande wa asubuhi pindi tu mambo yanapokuwa magumu ni kama utaalamu wetu wa asili.",
          "it": "La nostra devozione che svanisce come la nebbia mattutina appena le cose si mettono male è ormai un classico intramontabile."
        }
    },
    "2026-11-19": {
        date: "2026-11-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ホセア書 11:1-12",
          "en": "Hosea 11:1-12",
          "ko": "Hosea 11:1-12",
          "zho": "ホセア書 11:1-12",
          "es": "Hosea 11:1-12",
          "pt": "Hosea 11:1-12",
          "vi": "Hosea 11:1-12",
          "tl": "Hosea 11:1-12",
          "th": "Hosea 11:1-12",
          "sw": "Hosea 11:1-12",
          "it": "Osea 11:1-12"
        },
        comment: {
          "ja": "反抗期まっただ中の子供をそれでも愛し続ける親の苦悩、神様も我々に対して毎日同じような胃の痛みを感じているのかもしれません。",
          "en": "The agony of loving a rebellious child who just won't listen makes you realize God probably deals with a lot of heartburn because of us.",
          "ko": "반항기 자식을 끝까지 사랑하는 부모의 고뇌를 보면, 하나님도 우리 때문에 매일 속을 썩고 계실 것 같다는 생각이 듭니다.",
          "zho": "看着父母对叛逆期的孩子依然不离不弃，才发现上帝为了我们这些不省心的孩子，大概每天都在忍受胃痛。",
          "es": "La angustia de amar a un hijo rebelde que no escucha nos hace pensar que Dios debe sufrir dolores de cabeza constantes por nuestra culpa.",
          "pt": "A agonia de amar um filho rebelde que não ouve nos faz perceber que Deus provavelmente sofre muito por nossa causa todos os dias.",
          "vi": "Nỗi đau đáu khi yêu thương một đứa con nổi loạn không chịu nghe lời khiến ta nhận ra Chúa có lẽ cũng đang rất đau đầu vì chúng ta.",
          "tl": "Ang paghihirap ng magmahal sa isang rebeldeng anak na ayaw makinig ay nagpapaalala sa atin na baka araw-araw ay sumasakit ang ulo ng Diyos dahil sa atin.",
          "th": "ความเจ็บปวดจากการรักลูกที่ดื้อรั้นไม่ยอมฟังทำให้รู้ว่าพระเจ้าคงต้องปวดหัวกับพวกเรามากแค่ไหนในแต่ละวัน",
          "sw": "Maumivu ya kumpenda mtoto mkaidi asiyesikiliza yanatufanya tutambue kuwa Mungu labda anapata maumivu ya moyo kila siku kwa sababu yetu.",
          "it": "L'agonia di amare un figlio ribelle che non ascolta ci fa capire che lassù probabilmente si prendono un bel po' di mal di testa a causa nostra."
        }
    },
    "2026-11-20": {
        date: "2026-11-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ホセア書 13:1-10",
          "en": "Hosea 13:1-10",
          "ko": "Hosea 13:1-10",
          "zho": "ホセア書 13:1-10",
          "es": "Hosea 13:1-10",
          "pt": "Hosea 13:1-10",
          "vi": "Hosea 13:1-10",
          "tl": "Hosea 13:1-10",
          "th": "Hosea 13:1-10",
          "sw": "Hosea 13:1-10",
          "it": "Osea 13:1-10"
        },
        comment: {
          "ja": "神様を忘れてアイドルに熱中し、いざという時に助けてもらおうとするイスラエルの姿、流行りに乗って後で痛い目を見る現代の私たちと何ら変わりません。",
          "en": "Forgetting God to chase after trends only to expect Him to be there when things go south is the ultimate human irony.",
          "ko": "하나님을 잊고 유행을 쫓다가 정작 필요할 때만 찾는 모습은 평소엔 연락 없다가 급할 때만 찾는 지인과 다를 게 없네요.",
          "zho": "忘记上帝去追逐潮流，等到出事了才想起来求救，这种临时抱佛脚的行为简直是人性写照。",
          "es": "Olvidar a Dios por seguir modas pasajeras y luego esperar que Él nos rescate al primer problema es el colmo de la ironía humana.",
          "pt": "Esquecer a Deus para seguir tendências e depois esperar que Ele nos salve quando tudo dá errado é a ironia humana em seu estado bruto.",
          "vi": "Quên mất Chúa để chạy theo những trào lưu rồi lại mong Ngài giải cứu khi gặp hoạn nạn, đó chẳng phải là sự trớ trêu điển hình của con người sao.",
          "tl": "Ang kalimutan ang Diyos para sa mga uso at saka maghanap kapag may problema ay ang pinakamalinaw na halimbawa ng pagiging makasarili ng tao.",
          "th": "ลืมพระเจ้าไปวิ่งตามกระแสพอตกที่นั่งลำบากก็หันกลับมาขอความช่วยเหลือ นี่มันนิสัยมนุษย์ชัดๆ",
          "sw": "Kumsahau Mungu ili kufuata mambo ya kidunia kisha kutaka msaada wakati mambo yanaharibika ni unafiki mkuu wa binadamu.",
          "it": "Dimenticarsi di tutto per seguire le mode del momento e poi pretendere che qualcuno ci tiri fuori dai guai è la quintessenza della nostra incoerenza."
        }
    },
    "2026-11-21": {
        date: "2026-11-21",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨエル書 1:1-20",
          "en": "Joel 1:1-20",
          "ko": "Joel 1:1-20",
          "zho": "ヨエル書 1:1-20",
          "es": "Joel 1:1-20",
          "pt": "Joel 1:1-20",
          "vi": "Joel 1:1-20",
          "tl": "Joel 1:1-20",
          "th": "Joel 1:1-20",
          "sw": "Joel 1:1-20",
          "it": "Gioele 1:1-20"
        },
        comment: {
          "ja": "収穫が全滅して大騒ぎするヨエル記の光景、何気ない日常が崩れた瞬間に初めて当たり前のありがたさに気づく人間の鈍感さを突きつけられます。",
          "en": "Watching the harvest disappear and suddenly realizing how much we took for granted is a harsh reminder of how we only value things once they are gone.",
          "ko": "수확물이 사라지고 나서야 비로소 당연했던 일상의 소중함을 깨닫는 모습은 잃어버리고 나서야 후회하는 우리의 고질병과 똑같네요.",
          "zho": "眼看着收成化为乌有才开始惊慌失措，这种不到失去不知珍惜的毛病，果然是人类的通病。",
          "es": "Ver cómo se pierde todo y apenas entonces valorar lo que dábamos por sentado es el recordatorio más amargo de nuestra falta de gratitud.",
          "pt": "Ver a colheita desaparecer e só então perceber o que tínhamos é aquele lembrete doloroso de que só damos valor quando perdemos.",
          "vi": "Chứng kiến mùa màng mất trắng rồi mới hoảng hốt nhận ra những điều bình thường vốn quý giá, con người quả thật chỉ biết trân trọng khi đã mất đi.",
          "tl": "Ang mapagtanto lang ang halaga ng mga bagay kapag wala na ang mga ito ay isang masakit na paalala kung gaano tayo kawalang-pakialam sa araw-araw.",
          "th": "พอผลผลิตเสียหายถึงได้เริ่มโวยวาย นี่แหละความชะล่าใจของคนเราที่มักจะเห็นค่าของสิ่งที่อยู่ตรงหน้าก็ต่อเมื่อมันหายไปแล้ว",
          "sw": "Kuona mavuno yakipotea na ndipo kuanza kushtuka ni ukumbusho mchungu wa jinsi tunavyothamini vitu pale tu vinapotutoka.",
          "it": "Perdere tutto all'improvviso e accorgersi solo allora di quanto davamo tutto per scontato è la lezione più amara che la vita ci riserva."
        }
    },
    "2026-11-22": {
        date: "2026-11-22",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨエル書 2:19-32",
          "en": "Joel 2:19-32",
          "ko": "Joel 2:19-32",
          "zho": "ヨエル書 2:19-32",
          "es": "Joel 2:19-32",
          "pt": "Joel 2:19-32",
          "vi": "Joel 2:19-32",
          "tl": "Joel 2:19-32",
          "th": "Joel 2:19-32",
          "sw": "Joel 2:19-32",
          "it": "Gioele 2:19-32"
        },
        comment: {
          "ja": "災いの中にいても豊作を約束されるヨエルの言葉は、先行き不透明な現代の私たちにとって究極のライフハックに聞こえる。",
          "en": "Joel promising abundance in the middle of a disaster is the ultimate life hack for anyone living in uncertain times.",
          "ko": "재난 속에서도 풍요를 약속하는 요엘의 메시지는 불안한 시대를 사는 우리에게 최고의 인생 조언처럼 들린다.",
          "zho": "在災難中仍被應許豐收，約珥的這番話簡直是現代人在動盪生活中的終極心靈雞湯。",
          "es": "La promesa de abundancia de Joel en medio del desastre suena como el mejor consejo de vida para estos tiempos inciertos.",
          "pt": "A promessa de Joel sobre a abundância em meio ao desastre soa como o conselho de vida definitivo para tempos incertos.",
          "vi": "Lời hứa về sự sung túc ngay giữa lúc tai ương của Joel nghe như một bí quyết sống tuyệt vời cho thời đại đầy bất ổn này.",
          "tl": "Ang pangako ng kasaganaan ni Joel sa gitna ng sakuna ay parang pinakamagandang payo sa buhay para sa panahon ngayon.",
          "th": "คำสัญญาเรื่องความอุดมสมบูรณ์ท่ามกลางภัยพิบัติของโยเอลฟังดูเหมือนเคล็ดลับการใช้ชีวิตที่ดีที่สุดในยุคที่ไม่แน่นอนนี้",
          "sw": "Ahadi ya Joel ya wingi katikati ya maafa inasikika kama ushauri bora wa maisha kwa nyakati hizi zisizotabirika.",
          "it": "La promessa di riscatto in mezzo al disastro suona proprio come quel barlume di speranza di cui abbiamo bisogno quando tutto sembra andare storto."
        }
    },
    "2026-11-23": {
        date: "2026-11-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "アモス書 1:1-15",
          "en": "Amos 1:1-15",
          "ko": "Amos 1:1-15",
          "zho": "アモス書 1:1-15",
          "es": "Amos 1:1-15",
          "pt": "Amos 1:1-15",
          "vi": "Amos 1:1-15",
          "tl": "Amos 1:1-15",
          "th": "Amos 1:1-15",
          "sw": "Amos 1:1-15",
          "it": "Amos 1:1-15"
        },
        comment: {
          "ja": "近所の国々の悪事をリストアップして神に怒りをぶつけるアモス、まるでSNSで正義感を振りかざす現代人の先駆けのようだ。",
          "en": "Amos listing all the neighbors' sins to fuel divine wrath is basically the ancient version of a heated social media thread.",
          "ko": "이웃 나라들의 죄를 낱낱이 고하며 분노하는 아모스를 보니 SNS에서 정의의 사도처럼 행동하는 현대인의 모습이 겹쳐 보인다.",
          "zho": "阿摩司列舉周遭鄰國的罪行並祈求神降怒，簡直就像是現代人在社群媒體上爭論正義的鼻祖。",
          "es": "Amós enumerando los pecados de todos sus vecinos para incitar la ira divina parece el precursor de un hilo polémico en redes sociales.",
          "pt": "Amós listando os pecados de todos os seus vizinhos para incitar a ira divina parece o precursor de uma discussão acalorada nas redes sociais.",
          "vi": "Việc A-mốt liệt kê tội lỗi của các nước láng giềng để kêu gọi cơn thịnh nộ của Chúa giống hệt như cách người ta tranh cãi nảy lửa trên mạng xã hội ngày nay.",
          "tl": "Ang paglilista ni Amos ng mga kasalanan ng kanyang mga kapitbahay upang galitin ang Diyos ay parang sinaunang bersyon ng isang mainit na thread sa social media.",
          "th": "การที่อาโมสไล่เรียงความผิดของเพื่อนบ้านเพื่อให้พระเจ้าพิโรธนั้นดูเหมือนการจุดชนวนดราม่าในโซเชียลมีเดียในยุคโบราณไม่มีผิด",
          "sw": "Amosi kuorodhesha dhambi za majirani zake ili kuchochea hasira ya Mungu ni kama toleo la zamani la mjadala mkali kwenye mitandao ya kijamii.",
          "it": "Amos che elenca tutti i peccati dei paesi vicini sembra proprio il precursore di chi passa le serate a fare polemica sui social."
        }
    },
    "2026-11-24": {
        date: "2026-11-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "アモス書 3:1-15",
          "en": "Amos 3:1-15",
          "ko": "Amos 3:1-15",
          "zho": "アモス書 3:1-15",
          "es": "Amos 3:1-15",
          "pt": "Amos 3:1-15",
          "vi": "Amos 3:1-15",
          "tl": "Amos 3:1-15",
          "th": "Amos 3:1-15",
          "sw": "Amos 3:1-15",
          "it": "Amos 3:1-15"
        },
        comment: {
          "ja": "ライオンが吠えたら誰だって恐れるのに、神の警告を無視して平然と日常を送れるアモス時代のイスラエルの鈍感力は逆にすごい。",
          "en": "If a lion roars, you run, yet the people in Amos's day managed to ignore clear warnings with impressive, if fatal, levels of nonchalance.",
          "ko": "사자가 울부짖는데도 태연하게 일상을 이어가는 이스라엘 백성들의 둔감함은 거의 기적에 가까운 수준이다.",
          "zho": "狮子吼叫时人人都会害怕，但阿摩司时代的以色列人却能无视警告继续过日子，这种迟钝简直让人叹为观止。",
          "es": "Si un león ruge todos se asustan, pero la capacidad de la gente en tiempos de Amós para ignorar las advertencias y seguir como si nada es pasmosa.",
          "pt": "Se um leão ruge todos tremem, mas a habilidade do povo nos tempos de Amós de ignorar avisos óbvios e seguir a vida normalmente é impressionante.",
          "vi": "Khi sư tử gầm lên ai cũng sợ, nhưng sự thờ ơ của người Israel thời A-mốt trước những lời cảnh báo thật là khó tin.",
          "tl": "Kapag umungal ang leon lahat ay natatakot, pero ang galing ng mga tao sa panahon ni Amos na balewalain ang babala at magpatuloy lang sa buhay.",
          "th": "เมื่อสิงโตคำรามใครๆ ก็ต้องกลัว แต่ความนิ่งเฉยของคนในสมัยอาโมสที่เพิกเฉยต่อคำเตือนนั้นช่างน่าทึ่งจริงๆ",
          "sw": "Simba akinguruma kila mtu anaogopa, lakini uwezo wa watu wa wakati wa Amosi kupuuza onyo na kuendelea na maisha yao ni wa kushangaza.",
          "it": "Se un leone ruggisce tutti scappano, ma la capacità di ignorare i segnali evidenti ai tempi di Amos ha del leggendario."
        }
    },
    "2026-11-25": {
        date: "2026-11-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "アモス書 6:1-14",
          "en": "Amos 6:1-14",
          "ko": "Amos 6:1-14",
          "zho": "アモス書 6:1-14",
          "es": "Amos 6:1-14",
          "pt": "Amos 6:1-14",
          "vi": "Amos 6:1-14",
          "tl": "Amos 6:1-14",
          "th": "Amos 6:1-14",
          "sw": "Amos 6:1-14",
          "it": "Amos 6:1-14"
        },
        comment: {
          "ja": "贅沢三昧で破滅の予兆に気づかない人々の姿は、現代の私たちがついついやってしまう現実逃避の究極系に見える。",
          "en": "Watching people live in total luxury while ignoring the impending collapse is the ultimate, uncomfortable mirror for our own denial.",
          "ko": "호의호식하며 멸망의 징조를 전혀 눈치채지 못하는 모습은 현대인의 현실 부정과 너무나 닮아있다.",
          "zho": "看着人们在极度奢华中对末日迹象视而不见，这简直是现代人逃避现实的终极写照。",
          "es": "Ver a gente viviendo en el lujo total sin notar que todo se está desmoronando es el reflejo perfecto de cómo evadimos la realidad hoy en día.",
          "pt": "Ver pessoas vivendo no luxo sem notar que tudo está desmoronando é o espelho perfeito para a nossa própria negação moderna.",
          "vi": "Chứng kiến cảnh sống xa hoa mà không nhận ra dấu hiệu của sự sụp đổ chính là tấm gương phản chiếu sự chối bỏ thực tại của chính chúng ta.",
          "tl": "Ang makita ang mga taong nagpapakasasa sa luho habang hindi napapansin ang paparating na sakuna ay parang salamin ng ating sariling pagtanggi sa katotohanan.",
          "th": "การเห็นผู้คนใช้ชีวิตหรูหราโดยไม่รู้ตัวถึงหายนะที่กำลังจะมาถึง คือภาพสะท้อนที่ชัดเจนที่สุดของการปฏิเสธความจริงในปัจจุบัน",
          "sw": "Kuona watu wakiishi kwa anasa huku wakipuuza dalili za maangamizi ni kioo kamili cha jinsi tunavyokataa ukweli leo.",
          "it": "Vivere nel lusso totale ignorando che tutto sta crollando è lo specchio perfetto di come tendiamo a evitare la realtà."
        }
    },
    "2026-11-26": {
        date: "2026-11-26",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "アモス書 8:1-14",
          "en": "Amos 8:1-14",
          "ko": "Amos 8:1-14",
          "zho": "アモス書 8:1-14",
          "es": "Amos 8:1-14",
          "pt": "Amos 8:1-14",
          "vi": "Amos 8:1-14",
          "tl": "Amos 8:1-14",
          "th": "Amos 8:1-14",
          "sw": "Amos 8:1-14",
          "it": "Amos 8:1-14"
        },
        comment: {
          "ja": "夏果の籠を見て終焉を悟るアモス、明日から本気出すと言い続けて結局何も変わらない自分たちの姿を突きつけられているようで胃が痛い。",
          "en": "Amos looking at a basket of summer fruit and seeing the end of everything is a bit too close to how I ignore my own looming deadlines.",
          "ko": "여름 과일 바구니를 보고 종말을 예견하는 아모스의 모습은, 미루고 미루다 결국 폭발하는 내 일상을 보는 것 같아 마음이 아프다.",
          "zho": "阿摩司看著夏天的果籃就預見了結局，這簡直就像是看著自己因為拖延而即將崩潰的生活。",
          "es": "Amós viendo el fin en una cesta de fruta de verano es el recordatorio perfecto de que a veces ignoramos las señales obvias hasta que es demasiado tarde.",
          "pt": "Amós ver o fim em uma cesta de frutas de verão é um lembrete desconfortável de que ignoramos os sinais óbvios até que não dê mais.",
          "vi": "Amos nhìn giỏ trái cây mùa hè mà thấy trước sự kết thúc, giống như cách chúng ta cứ phớt lờ những dấu hiệu báo động cho đến khi mọi chuyện quá muộn.",
          "tl": "Ang pagkakita ni Amos sa katapusan sa isang basket ng prutas ay parang tayo na hindi pinapansin ang mga babala hanggang sa huli na ang lahat.",
          "th": "อามอสเห็นจุดจบในตะกร้าผลไม้ฤดูร้อน เหมือนกับเราที่มักจะมองข้ามสัญญาณเตือนชัดๆ จนกระทั่งสายเกินไป",
          "sw": "Amos kuona mwisho kwenye kikapu cha matunda ya kiangazi ni kama sisi tunavyopuuza ishara za wazi hadi inapokuwa imechelewa.",
          "it": "Amos che vede la fine in un cesto di frutta estiva mi ricorda fin troppo bene come ignoro le mie scadenze imminenti."
        }
    },
    "2026-11-27": {
        date: "2026-11-27",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "オバデヤ書 1:1-21",
          "en": "Obadiah 1:1-21",
          "ko": "Obadiah 1:1-21",
          "zho": "オバデヤ書 1:1-21",
          "es": "Obadiah 1:1-21",
          "pt": "Obadiah 1:1-21",
          "vi": "Obadiah 1:1-21",
          "tl": "Obadiah 1:1-21",
          "th": "Obadiah 1:1-21",
          "sw": "Obadiah 1:1-21",
          "it": "Abdia 1:1-21"
        },
        comment: {
          "ja": "高慢で岩の裂け目に隠れていれば安全だと思い込んでいるエドム人、自分の殻に閉じこもるのが一番の防御だと思っている現代人の縮図にしか見えない。",
          "en": "Edom thinking they are untouchable because they live in high rocks is basically every person who thinks their ego is an impenetrable fortress.",
          "ko": "바위 틈에 숨어 안전하다고 믿는 에돔 사람들은, 자신의 자존심이라는 성 안에 갇혀 세상과 담을 쌓은 우리들의 모습과 너무나 닮았다.",
          "zho": "以東人以為躲在岩石縫隙裡就高枕無憂，這簡直就是現代人躲在自負的堡壘裡拒絕改變的寫照。",
          "es": "Edom pensando que está a salvo porque vive en las alturas es la prueba definitiva de que la arrogancia es el escondite más peligroso.",
          "pt": "Edom achando que está seguro só porque mora em rochas altas é a definição perfeita de quem usa o ego como fortaleza.",
          "vi": "Người Ê-đôm tưởng mình an toàn vì sống trong vách đá, chẳng khác nào chúng ta cứ ngỡ cái tôi là pháo đài bất khả xâm phạm.",
          "tl": "Ang pag-aakala ng mga Edomita na ligtas sila sa loob ng kanilang mga bato ay parang tayo na nagtatago sa likod ng ating sariling kayabangan.",
          "th": "ชาวเอโดมคิดว่าตนปลอดภัยเพราะอาศัยอยู่ในซอกหิน ก็เหมือนกับเราที่คิดว่ากำแพงแห่งความหยิ่งผยองจะปกป้องเราได้ตลอดไป",
          "sw": "Edom kufikiri wako salama kwa sababu wanaishi kwenye miamba ni kama sisi tunavyojificha nyuma ya kiburi chetu tukiamini ni ngome.",
          "it": "Edom che si crede intoccabile tra le rocce è la prova che l'arroganza è il rifugio più ingannevole del mondo."
        }
    },
    "2026-11-28": {
        date: "2026-11-28",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨナ書 1:1-17",
          "en": "Jonah 1:1-17",
          "ko": "Jonah 1:1-17",
          "zho": "ヨナ書 1:1-17",
          "es": "Jonah 1:1-17",
          "pt": "Jonah 1:1-17",
          "vi": "Jonah 1:1-17",
          "tl": "Jonah 1:1-17",
          "th": "Jonah 1:1-17",
          "sw": "Jonah 1:1-17",
          "it": "Giona 1:1-17"
        },
        comment: {
          "ja": "神様に言われたことと真逆の方角へ全力で逃げ出すヨナの行動力、見ていて清々しいほどの人間臭さがある。",
          "en": "Jonah booking a ticket in the exact opposite direction of his calling is the most relatable act of spiritual procrastination ever.",
          "ko": "하나님이 가라는 곳과 정반대로 도망치는 요나의 모습에서 미루고 싶은 우리의 본능을 본다.",
          "zho": "約拿聽完任務後立刻買張反方向船票逃跑，這種逃避現實的衝動簡直就是現代人的寫照。",
          "es": "Jonah huyendo en la dirección opuesta a su misión es la representación más honesta de nuestras propias ganas de ignorar lo que nos incomoda.",
          "pt": "Jonas fugindo na direção oposta ao seu chamado é a representação mais honesta da nossa vontade de ignorar o que nos incomoda.",
          "vi": "Việc Jonah tìm cách chạy trốn hoàn toàn ngược hướng với mệnh lệnh là minh chứng rõ nhất cho bản năng trì hoãn của con người.",
          "tl": "Ang pagtakbo ni Jonas sa kabilang direksyon para lang umiwas sa utos ay ang pinaka-relatable na paraan ng pagpapaliban ng gawain.",
          "th": "โยนาห์จองตั๋วเรือหนีไปทิศตรงกันข้ามทันทีที่รู้ว่าต้องไปไหน เป็นการผัดวันประกันพรุ่งที่เข้าถึงใจมนุษย์ที่สุด",
          "sw": "Yona kukimbia kuelekea upande mwingine kabisa wa wito wake ni mfano halisi wa jinsi tunavyopenda kukwepa majukumu.",
          "it": "Giona che prenota un biglietto nella direzione opposta alla sua chiamata è l'atto di procrastinazione spirituale più onesto di sempre."
        }
    },
    "2026-11-29": {
        date: "2026-11-29",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ヨナ書 3:1-10",
          "en": "Jonah 3:1-10",
          "ko": "Jonah 3:1-10",
          "zho": "ヨナ書 3:1-10",
          "es": "Jonah 3:1-10",
          "pt": "Jonah 3:1-10",
          "vi": "Jonah 3:1-10",
          "tl": "Jonah 3:1-10",
          "th": "Jonah 3:1-10",
          "sw": "Jonah 3:1-10",
          "it": "Giona 3:1-10"
        },
        comment: {
          "ja": "渋々ニネベに行って適当に説教したら街全体が悔い改め始めた時のヨナの困惑、成功したのに面白くないという皮肉な現実。",
          "en": "Jonah doing the bare minimum and accidentally sparking a mass revival is the ultimate irony for someone who really didn't want to be there.",
          "ko": "억지로 설교하러 갔는데 도시 전체가 회개해 버리니 오히려 당황하는 요나의 모습이 참 인간적이다.",
          "zho": "約拿只是隨便講了幾句，結果全城竟然都悔改了，這種意外的成功反而讓他更不爽，簡直是職場悲喜劇。",
          "es": "Jonah haciendo el mínimo esfuerzo y logrando que toda una ciudad se arrepienta es la ironía perfecta de alguien que no quería estar ahí.",
          "pt": "Jonas fazendo o esforço mínimo e conseguindo que toda a cidade se arrependesse é a ironia perfeita para quem não queria estar ali.",
          "vi": "Việc Jonah miễn cưỡng đi rao giảng mà cả thành phố lại ăn năn thực sự là một sự mỉa mai cho người không hề muốn ở đó.",
          "tl": "Ang pag-aalinlangan ni Jonas habang nakikita ang buong lungsod na nagbago dahil sa kanyang sermon ay isang klasikong halimbawa ng hindi inaasahang tagumpay.",
          "th": "โยนาห์ไปเทศนาแบบขอไปทีแต่คนทั้งเมืองกลับกลับใจกันหมด ความสำเร็จที่เจ้าตัวไม่ต้องการนี้ช่างย้อนแย้งเสียจริง",
          "sw": "Yona kuhubiri kwa shingo upande na kuona mji mzima ukitubu ni kejeli ya hali ya juu kwa mtu ambaye hakutaka kuwa hapo kabisa.",
          "it": "Giona che fa il minimo sforzo e finisce per convertire un'intera città è la beffa perfetta per chi voleva solo sparire."
        }
    },
    "2026-11-30": {
        date: "2026-11-30",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ミカ書 1:1-16",
          "en": "Micah 1:1-16",
          "ko": "Micah 1:1-16",
          "zho": "ミカ書 1:1-16",
          "es": "Micah 1:1-16",
          "pt": "Micah 1:1-16",
          "vi": "Micah 1:1-16",
          "tl": "Micah 1:1-16",
          "th": "Micah 1:1-16",
          "sw": "Micah 1:1-16",
          "it": "Michea 1:1-16"
        },
        comment: {
          "ja": "神様が山を溶かすほどの勢いで怒っているのに自分はまだ日常の些細なことで消耗しているのを見ると、スケールの差に少し笑えてくる。",
          "en": "Watching God melt mountains in anger while I am still stressed about my daily to-do list really puts my life in perspective.",
          "ko": "산이 녹아내릴 정도의 거대한 분노 속에서도 여전히 일상의 사소한 일로 전전긍긍하는 내 모습을 보면 왠지 헛웃음이 난다.",
          "zho": "看著神發怒到山崩地裂，反觀自己還在為生活瑣事焦慮，這種格局的差距實在讓人哭笑不得。",
          "es": "Ver a Dios derretir montañas con su ira mientras yo me sigo estresando por cosas insignificantes me hace cuestionar mis prioridades.",
          "pt": "Ver Deus derretendo montanhas com Sua ira enquanto eu ainda me estresso com coisas banais do dia a dia é um choque de realidade.",
          "vi": "Chứng kiến cảnh Chúa nổi giận đến mức làm tan chảy cả núi non, trong khi bản thân vẫn cứ loay hoay với mấy chuyện vụn vặt thường ngày, thật là một sự chênh lệch đáng suy ngẫm.",
          "tl": "Nakakatawa isiping ang Diyos ay nagpapaalog ng mga bundok sa galit habang ako ay problemado pa rin sa mga maliliit na bagay sa buhay.",
          "th": "เห็นพระเจ้าพิโรธจนภูเขาละลาย แต่เรายังมานั่งเครียดกับเรื่องจุกจิกในชีวิตประจำวัน มันก็น่าขำดีเหมือนกันนะ",
          "sw": "Kuona Mungu akiyeyusha milima kwa hasira wakati mimi bado ninasumbuka na mambo madogo ya kila siku kunanifanya nitabasamu kwa aibu.",
          "it": "Guardare Dio sciogliere le montagne mentre io mi preoccupo delle solite sciocchezze ridimensiona parecchio la mia giornata."
        }
    },
    "2026-12-01": {
        date: "2026-12-01",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ミカ書 4:1-13",
          "en": "Micah 4:1-13",
          "ko": "Micah 4:1-13",
          "zho": "ミカ書 4:1-13",
          "es": "Micah 4:1-13",
          "pt": "Micah 4:1-13",
          "vi": "Micah 4:1-13",
          "tl": "Micah 4:1-13",
          "th": "Micah 4:1-13",
          "sw": "Micah 4:1-13",
          "it": "Michea 4:1-13"
        },
        comment: {
          "ja": "剣を鋤に打ち替えるなんて理想的すぎて、現代のSNSでレスバしている人たちに見せてやりたい気分。",
          "en": "Turning swords into plowshares is such a beautiful ideal, I wish I could show it to everyone currently arguing in social media comments.",
          "ko": "칼을 쟁기로 바꾸는 평화로운 세상이라니, 지금 SNS에서 키보드 배틀을 벌이는 사람들에게 꼭 보여주고 싶은 장면이다.",
          "zho": "要把刀劍打成犁頭這件事太理想了，真想讓現在那些在社群媒體上爭吵不休的人都看一看。",
          "es": "Convertir las espadas en arados es un ideal tan noble que desearía que todos los que pelean en redes sociales pudieran verlo.",
          "pt": "Transformar espadas em arados é um ideal tão bonito; bem que eu queria mostrar isso para quem vive discutindo nos comentários das redes sociais.",
          "vi": "Biến gươm đao thành lưỡi cày là một viễn cảnh quá đỗi tuyệt vời, ước gì những người đang tranh cãi trên mạng xã hội hôm nay đều thấy được điều này.",
          "tl": "Ang paggawa ng mga espada para maging araro ay isang napakagandang ideyal na sana ay makita ng mga taong laging nagtatalo sa social media.",
          "th": "การเปลี่ยนดาบให้เป็นผาลไถเป็นอุดมคติที่งดงามมาก อยากให้พวกคนที่กำลังทะเลาะกันในโซเชียลมีเดียได้เห็นจริงๆ",
          "sw": "Kugeuza panga kuwa majembe ni wazo zuri sana, natamani ningewaonyesha wale wote wanaogombana kwenye mitandao ya kijamii leo.",
          "it": "Trasformare le spade in vomeri è un'idea così nobile che andrebbe fatta leggere a chi litiga furiosamente sui social media."
        }
    },
    "2026-12-02": {
        date: "2026-12-02",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ミカ書 6:1-16",
          "en": "Micah 6:1-16",
          "ko": "Micah 6:1-16",
          "zho": "ミカ書 6:1-16",
          "es": "Micah 6:1-16",
          "pt": "Micah 6:1-16",
          "vi": "Micah 6:1-16",
          "tl": "Micah 6:1-16",
          "th": "Micah 6:1-16",
          "sw": "Micah 6:1-16",
          "it": "Michea 6:1-16"
        },
        comment: {
          "ja": "神様が求めているのは豪華な儀式ではなく公正と慈しみだと突きつけられるミカ書、結局シンプルが一番難しいという現実を突きつけてくる。",
          "en": "Micah pointing out that God wants justice and kindness instead of fancy rituals is a brutal reminder that the simplest things are usually the hardest.",
          "ko": "화려한 제사보다 공의와 인자를 원하신다는 미가의 지적은 가장 단순한 것이 실천하기엔 가장 어렵다는 뼈아픈 진실을 찌른다.",
          "zho": "彌迦書提醒我們神要的是公義與慈愛而非繁瑣儀式，這再次證明了越簡單的事其實越難做到。",
          "es": "Miqueas nos recuerda que Dios prefiere la justicia y la misericordia a los rituales costosos, demostrando que lo sencillo es a menudo lo más difícil de cumplir.",
          "pt": "Miqueias lembrando que Deus prefere justiça e misericórdia a rituais caros é o lembrete perfeito de que as coisas simples são as mais difíceis de praticar.",
          "vi": "Việc Mi-chê chỉ ra rằng Chúa muốn sự công bằng và lòng nhân từ hơn là các nghi lễ cầu kỳ là lời nhắc nhở phũ phàng rằng những điều đơn giản nhất thường khó thực hiện nhất.",
          "tl": "Ang pagpapaalala ni Mikas na mas gusto ng Diyos ang katarungan at awa kaysa sa mga magarbo at ritwal ay isang sampal na katotohanan na ang pinakasimpleng bagay ang siyang pinakamahirap gawin.",
          "th": "มีคาห์เตือนสติเราว่าพระเจ้าต้องการความยุติธรรมและความเมตตามากกว่าพิธีกรรมที่หรูหรา ซึ่งเป็นความจริงที่เจ็บปวดว่าสิ่งที่ง่ายที่สุดมักทำได้ยากที่สุด",
          "sw": "Mika anapokumbusha kuwa Mungu anataka haki na fadhili badala ya matambiko ya gharama ni ukumbusho mchungu kwamba mambo rahisi ndiyo magumu zaidi kutekeleza.",
          "it": "Michea ci ricorda che l'equità e la compassione valgono più di mille cerimonie, confermando che le cose semplici sono sempre le più difficili."
        }
    },
    "2026-12-03": {
        date: "2026-12-03",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ナホム書 1:1-15",
          "en": "Nahum 1:1-15",
          "ko": "Nahum 1:1-15",
          "zho": "ナホム書 1:1-15",
          "es": "Nahum 1:1-15",
          "pt": "Nahum 1:1-15",
          "vi": "Nahum 1:1-15",
          "tl": "Nahum 1:1-15",
          "th": "Nahum 1:1-15",
          "sw": "Nahum 1:1-15",
          "it": "Naum 1:1-15"
        },
        comment: {
          "ja": "神様が敵に対して激しく怒るナホム書の描写、普段の優しい神様とは違う一面に、人間側も怒っていいんだと妙な安心感を覚える。",
          "en": "The intense anger of God in Nahum feels like a jarring departure from the usual gentle tone, yet it is oddly comforting to know that righteous indignation has its place.",
          "ko": "나훔서에 나타난 하나님의 불같은 진노는 평소의 인자한 모습과 대비되어, 때로는 의로운 분노도 필요하다는 사실에 묘한 안도감을 준다.",
          "zho": "那鴻書中描述神對敵人的強烈憤怒，與平時溫和的形象形成鮮明對比，但也讓人感到某種程度的釋然：原來義憤是有其位置的。",
          "es": "La intensa ira de Dios en Nahúm contrasta con su imagen habitual de bondad, dándonos un extraño alivio al confirmar que a veces la indignación justa es necesaria.",
          "pt": "A ira intensa de Deus em Naum contrasta com a Sua imagem habitual de bondade, trazendo um estranho alívio ao confirmar que a indignação justa tem o seu lugar.",
          "vi": "Sự thịnh nộ dữ dội của Chúa trong sách Na-hum tương phản với hình ảnh hiền lành thường thấy, mang lại cảm giác nhẹ nhõm kỳ lạ khi biết rằng sự phẫn nộ chính đáng cũng có vị trí của nó.",
          "tl": "Ang matinding galit ng Diyos sa Nahum ay malayo sa karaniwan niyang imahe, ngunit nakakapanatag isiping may lugar din pala ang matuwid na galit.",
          "th": "ความโกรธเกรี้ยวของพระเจ้าในหนังสือนาฮูมดูแตกต่างจากภาพลักษณ์ที่อ่อนโยนปกติ แต่มันก็น่าโล่งใจที่ได้รู้ว่าความโกรธที่ถูกต้องนั้นมีที่ทางของมัน",
          "sw": "Hasira kali ya Mungu katika Nahumu inatofautiana na sura yake ya upole, lakini inaleta faraja ya ajabu kujua kwamba hasira ya haki ina nafasi yake.",
          "it": "La furia di Dio in Nahum spiazza un po', ma fa quasi comodo sapere che arrabbiarsi per le ingiustizie è concesso."
        }
    },
    "2026-12-04": {
        date: "2026-12-04",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ハバクク書 1:1-17",
          "en": "Habakkuk 1:1-17",
          "ko": "Habakkuk 1:1-17",
          "zho": "ハバクク書 1:1-17",
          "es": "Habakkuk 1:1-17",
          "pt": "Habakkuk 1:1-17",
          "vi": "Habakkuk 1:1-17",
          "tl": "Habakkuk 1:1-17",
          "th": "Habakkuk 1:1-17",
          "sw": "Habakkuk 1:1-17",
          "it": "Abacuc 1:1-17"
        },
        comment: {
          "ja": "神様に「なぜ悪を放置するのですか」と強気で詰め寄るハバククは、理不尽な世の中に直面した時の私たちの本音そのものです。",
          "en": "Habakkuk demanding to know why God is ignoring all the chaos is exactly how we all feel when life just doesn't make sense.",
          "ko": "세상의 불의를 보고 하나님께 왜 가만히 계시냐고 따져 묻는 하박국은 부조리한 현실 앞에서 울분을 터뜨리는 우리 자신의 모습과 다를 게 없네요.",
          "zho": "哈巴谷質問神為何對惡行視而不見，這種面對混亂現實時的無力感與憤慨，簡直就是現代人的寫照。",
          "es": "Habacuc exigiéndole respuestas a Dios sobre por qué permite el mal es básicamente nosotros cada vez que leemos las noticias.",
          "pt": "Habacuque questionando a Deus por que Ele tolera o mal é o retrato fiel de quando a gente tenta entender as injustiças da vida.",
          "vi": "Habacúc chất vấn Chúa vì sao Ngài làm ngơ trước cái ác chính là tâm trạng của chúng ta mỗi khi đối diện với những nghịch lý của cuộc đời.",
          "tl": "Ang pagtatanong ni Habacuc kay Lord kung bakit hinahayaan ang kasamaan ay eksaktong nararamdaman nating lahat kapag hindi patas ang mundo.",
          "th": "ฮาบากุกที่กล้าถามพระเจ้าว่าทำไมถึงเพิกเฉยต่อความชั่วร้าย คือตัวแทนความรู้สึกอึดอัดใจของเราเวลาเจอเรื่องไม่ยุติธรรมในชีวิตจริง",
          "sw": "Habakuki kumuuliza Mungu kwa nini anavumilia uovu ni taswira halisi ya jinsi tunavyohisi tunapokabiliwa na mambo yasiyo na haki maishani.",
          "it": "Habacuc che chiede conto a Dio del caos nel mondo rappresenta esattamente il nostro stato d'animo quando leggiamo le notizie."
        }
    },
    "2026-12-05": {
        date: "2026-12-05",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ゼパニヤ書 1:1-18",
          "en": "Zephaniah 1:1-18",
          "ko": "Zephaniah 1:1-18",
          "zho": "ゼパニヤ書 1:1-18",
          "es": "Zephaniah 1:1-18",
          "pt": "Zephaniah 1:1-18",
          "vi": "Zephaniah 1:1-18",
          "tl": "Zephaniah 1:1-18",
          "th": "Zephaniah 1:1-18",
          "sw": "Zephaniah 1:1-18",
          "it": "Sofonia 1:1-18"
        },
        comment: {
          "ja": "神様の怒りを前にして「きっと何もしてくれないだろう」と高をくくっていたゼパニヤ時代の住民たち、油断しきった日常に突然の通告が来る怖さは現代の私たちにも刺さります。",
          "en": "The people in Zephaniah’s day thinking God wouldn't do anything about their mess is a sobering reminder that complacency is a dangerous game.",
          "ko": "하나님은 아무 일도 안 하실 거라며 태평하게 살던 당시 사람들은, 설마 하다가 큰코다치는 우리들의 안일함을 정확히 꼬집고 있네요.",
          "zho": "西番雅時代的人以為神不會介入，這種對現狀過度自信而忽視危機的心態，在現代生活中也隨處可見。",
          "es": "La gente en tiempos de Sofonías pensando que Dios no haría nada es el recordatorio perfecto de que la complacencia siempre nos pasa factura.",
          "pt": "Aquelas pessoas na época de Sofonias achando que Deus não faria nada são o exemplo clássico de quem confia demais na própria sorte.",
          "vi": "Người dân thời Xê-pha-ni tưởng rằng Chúa sẽ không can thiệp chính là bài học đắt giá về sự chủ quan trong cuộc sống hằng ngày của chúng ta.",
          "tl": "Ang mga tao noong panahon ni Zefanias na akala ay walang gagawin ang Diyos ay paalala na ang pagiging kampante ay isang mapanganib na bisyo.",
          "th": "ผู้คนที่คิดว่าพระเจ้าจะไม่ทำอะไรในสมัยของเศฟันยาห์ คือตัวอย่างชัดเจนของความประมาทที่คิดว่าตัวเองรอดจากผลของการกระทำได้เสมอ",
          "sw": "Watu wa wakati wa Sefania waliodhani Mungu hatatenda lolote ni ukumbusho kwamba kujiamini kupita kiasi katika uzembe ni hatari sana.",
          "it": "La gente ai tempi di Sofonie convinta che Dio non avrebbe fatto nulla dimostra quanto la pigrizia mentale sia un gioco pericoloso."
        }
    },
    "2026-12-06": {
        date: "2026-12-06",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ゼパニヤ書 3:1-20",
          "en": "Zephaniah 3:1-20",
          "ko": "Zephaniah 3:1-20",
          "zho": "ゼパニヤ書 3:1-20",
          "es": "Zephaniah 3:1-20",
          "pt": "Zephaniah 3:1-20",
          "vi": "Zephaniah 3:1-20",
          "tl": "Zephaniah 3:1-20",
          "th": "Zephaniah 3:1-20",
          "sw": "Zephaniah 3:1-20",
          "it": "Sofonia 3:1-20"
        },
        comment: {
          "ja": "散々背を向けていたのに最後は「歌って喜べ」と言われるゼパニヤ書の展開、神様のツンデレ具合が振り切れていてついていけない。",
          "en": "Zephaniah telling people who spent all their time ignoring God to suddenly sing for joy is the ultimate divine emotional whiplash.",
          "ko": "하나님께 등을 돌리고 살다가 갑자기 노래하며 기뻐하라는 제바냐의 메시지는 거의 감정의 롤러코스터 수준이다.",
          "zho": "西番雅书里那种明明大家一直背道而驰，最后却被要求歌唱欢呼的展开，神这种忽冷忽热的设定真是让人摸不着头脑。",
          "es": "Que Sofonías pida cantar de alegría a un pueblo que se la pasó dándole la espalda a Dios es un giro dramático difícil de procesar.",
          "pt": "Pedir para o povo cantar de alegria depois de terem passado tanto tempo virando as costas para Deus é uma mudança de humor divina e tanto.",
          "vi": "Việc Xê-pha-ni bảo những người vốn quay lưng với Chúa hãy cất tiếng hát vui mừng là một cú bẻ lái cảm xúc đầy bất ngờ.",
          "tl": "Ang utos ni Zefanias na kumanta sa tuwa ang mga taong matagal nang tinalikuran ang Diyos ay parang isang matinding emosyonal na sorpresa.",
          "th": "การที่เศฟันยาห์บอกให้ผู้คนที่หันหลังให้พระเจ้ากลับมาร้องเพลงด้วยความยินดี เป็นอะไรที่ปรับอารมณ์ตามแทบไม่ทัน",
          "sw": "Zefania kuwaambia watu waliompuuza Mungu kwa muda mrefu waanze kuimba kwa furaha ni kama mabadiliko ya ghafla ya hisia ambayo ni vigumu kuyaelewa.",
          "it": "Chiedere a un popolo che ha appena voltato le spalle a Dio di cantare di gioia è il colpo di scena emotivo definitivo."
        }
    },
    "2026-12-07": {
        date: "2026-12-07",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ハガイ書 1:1-15",
          "en": "Haggai 1:1-15",
          "ko": "Haggai 1:1-15",
          "zho": "ハガイ書 1:1-15",
          "es": "Haggai 1:1-15",
          "pt": "Haggai 1:1-15",
          "vi": "Haggai 1:1-15",
          "tl": "Haggai 1:1-15",
          "th": "Haggai 1:1-15",
          "sw": "Haggai 1:1-15",
          "it": "Aggeo 1:1-15"
        },
        comment: {
          "ja": "自分の家を豪華にすることに必死で神殿を放置していたハガイ時代の民、現代のライフスタイルそのもので耳が痛い。",
          "en": "Haggai’s people prioritizing their own home renovations over the temple is just a 2,500-year-old version of our daily procrastination.",
          "ko": "자기 집 꾸미느라 성전 짓는 건 뒷전이었던 학개 시대 사람들을 보니 왠지 모르게 뜨끔하다.",
          "zho": "哈该时代的人忙着装潢自家却把圣殿丢在一旁，这种现代人的既视感真是让人心虚。",
          "es": "La gente de la época de Hageo, obsesionada con renovar sus casas mientras el templo seguía en ruinas, suena demasiado a nuestras propias prioridades modernas.",
          "pt": "O povo da época de Ageu, mais preocupado com a decoração da própria casa do que com o templo, é o espelho perfeito das nossas prioridades atuais.",
          "vi": "Người dân thời A-ghê mải mê trang hoàng nhà cửa mà bỏ bê đền thờ chính là hình ảnh phản chiếu sự trì hoãn của chúng ta ngày nay.",
          "tl": "Ang pag-una ng mga tao kay Ageo sa pag-aayos ng sarili nilang bahay habang napapabayaan ang templo ay parang salamin ng ating sariling mga prayoridad.",
          "th": "ผู้คนในสมัยฮักกัยที่มัวแต่แต่งบ้านจนลืมสร้างพระวิหาร ช่างเป็นภาพสะท้อนชีวิตสมัยใหม่ที่ทำเอาเราสะดุ้งได้เหมือนกัน",
          "sw": "Watu wa wakati wa Hagai waliojishughulisha na ukarabati wa nyumba zao huku wakipuuza hekalu ni mfano kamili wa jinsi tunavyopanga vipaumbele vyetu leo.",
          "it": "La gente dei tempi di Aggeo ossessionata dal ristrutturare casa mentre il tempio cadeva a pezzi suona troppo simile alle nostre priorità quotidiane."
        }
    },
    "2026-12-08": {
        date: "2026-12-08",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ゼカリヤ書 1:1-21",
          "en": "Zechariah 1:1-21",
          "ko": "Zechariah 1:1-21",
          "zho": "ゼカリヤ書 1:1-21",
          "es": "Zechariah 1:1-21",
          "pt": "Zechariah 1:1-21",
          "vi": "Zechariah 1:1-21",
          "tl": "Zechariah 1:1-21",
          "th": "Zechariah 1:1-21",
          "sw": "Zechariah 1:1-21",
          "it": "Zaccaria 1:1-21"
        },
        comment: {
          "ja": "先祖が聞き入れなかった警告をそのまま繰り返して失敗する歴史のループ、人間はいつになっても学習能力をアップデートできない生き物らしい。",
          "en": "Watching generations repeat the same mistakes despite clear warnings feels like a reminder that humans are perpetually stuck in a loop of refusing to learn.",
          "ko": "조상들이 무시했던 경고를 똑같이 반복하며 실패하는 역사의 굴레를 보면 인간의 학습 능력은 참 업데이트가 안 된다는 생각이 든다.",
          "zho": "看著祖先不聽勸告的歷史不斷重演，才發現人類這種生物似乎永遠學不會記取教訓。",
          "es": "Ver a generaciones repetir los mismos errores a pesar de las advertencias es la prueba de que los humanos somos expertos en tropezar con la misma piedra.",
          "pt": "Ver gerações repetirem os mesmos erros apesar dos avisos é a prova de que os humanos são especialistas em tropeçar na mesma pedra.",
          "vi": "Chứng kiến các thế hệ lặp lại sai lầm dù đã được cảnh báo cho thấy con người dường như mãi mãi không chịu rút kinh nghiệm.",
          "tl": "Ang makitang inuulit ng bawat henerasyon ang parehong pagkakamali sa kabila ng babala ay patunay na sadyang matigas ang ulo ng tao.",
          "th": "การได้เห็นคนรุ่นหลังทำพลาดซ้ำรอยบรรพบุรุษทั้งที่ถูกเตือนแล้ว เป็นเครื่องยืนยันว่ามนุษย์เราไม่ค่อยจะจำบทเรียนกันเลย",
          "sw": "Kuona vizazi vikirudia makosa yale yale licha ya maonyo ni ushahidi kwamba binadamu ni wabishi sana katika kujifunza.",
          "it": "Vedere intere generazioni ripetere gli stessi errori nonostante i chiari avvertimenti conferma che gli umani sono bloccati nel loop di non imparare mai."
        }
    },
    "2026-12-09": {
        date: "2026-12-09",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ゼカリヤ書 3:1-10",
          "en": "Zechariah 3:1-10",
          "ko": "Zechariah 3:1-10",
          "zho": "ゼカリヤ書 3:1-10",
          "es": "Zechariah 3:1-10",
          "pt": "Zechariah 3:1-10",
          "vi": "Zechariah 3:1-10",
          "tl": "Zechariah 3:1-10",
          "th": "Zechariah 3:1-10",
          "sw": "Zechariah 3:1-10",
          "it": "Zaccaria 3:1-10"
        },
        comment: {
          "ja": "汚れた服を脱がされて新しい服を着せてもらうヨシュア、過去の失敗をいつまでも引きずって恥じている自分に聞かせたい光景だ。",
          "en": "Joshua getting his filthy clothes swapped for clean ones is the ultimate reminder that we need to stop wearing our past mistakes like a uniform.",
          "ko": "더러운 옷을 벗고 새 옷을 입는 여호수아의 모습은 과거의 실수라는 낡은 옷을 이제는 좀 벗어던지라는 따끔한 조언 같다.",
          "zho": "約書亞脫下髒衣服換上新袍的畫面，簡直是在提醒我們別再把過去的錯誤當成制服穿在身上了。",
          "es": "Ver a Josué cambiándose las ropas sucias por unas limpias es el recordatorio perfecto de que debemos dejar de vestir nuestros errores del pasado.",
          "pt": "Ver Josué trocando roupas sujas por limpas é o lembrete perfeito de que precisamos parar de usar nossos erros do passado como uniforme.",
          "vi": "Hình ảnh Giô-suê cởi bỏ bộ y phục bẩn thỉu để mặc đồ mới là lời nhắc nhở rằng chúng ta nên thôi mặc mãi chiếc áo của những sai lầm cũ.",
          "tl": "Ang pagpapalit ng maruming damit ni Josue ay isang paalala na dapat na nating hubarin ang mga pagkakamali ng nakaraan na parang uniporme.",
          "th": "ภาพที่โยชูวาได้เปลี่ยนจากชุดเปื้อนโคลนมาเป็นชุดใหม่ คือการเตือนใจชั้นดีว่าเราควรเลิกเอาความผิดพลาดในอดีตมาสวมใส่เป็นชุดประจำตัวได้แล้ว",
          "sw": "Kuona Yoshua akivua nguo chafu na kuvaa mpya ni ukumbusho kamili kwamba tunapaswa kuacha kuvaa makosa yetu ya zamani kama sare.",
          "it": "Giosuè che si toglie i vestiti sporchi per metterne di puliti è il promemoria perfetto per smettere di indossare i nostri errori passati come una divisa."
        }
    },
    "2026-12-10": {
        date: "2026-12-10",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ゼカリヤ書 7:1-14",
          "en": "Zechariah 7:1-14",
          "ko": "Zechariah 7:1-14",
          "zho": "ゼカリヤ書 7:1-14",
          "es": "Zechariah 7:1-14",
          "pt": "Zechariah 7:1-14",
          "vi": "Zechariah 7:1-14",
          "tl": "Zechariah 7:1-14",
          "th": "Zechariah 7:1-14",
          "sw": "Zechariah 7:1-14",
          "it": "Zaccaria 7:1-14"
        },
        comment: {
          "ja": "断食の意味を形だけで済ませようとする民に冷や水を浴びせるゼカリヤ、形式主義に走りがちな現代人の耳に痛すぎる。",
          "en": "Zechariah calling out people who treat fasting like a hollow checklist is a reality check for anyone who prioritizes ritual over empathy.",
          "ko": "형식적인 금식에만 매달리는 사람들에게 던지는 스가랴의 일침은 본질을 잊기 쉬운 우리 모두의 뒤통수를 때린다.",
          "zho": "撒迦利亚戳破那些只重仪式不重内心的禁食，简直是给现代人的一记当头棒喝。",
          "es": "Zecarías recordándonos que el ayuno no es solo una lista de tareas pendientes es el correctivo que todos necesitamos de vez en cuando.",
          "pt": "Zacarias lembrando que o jejum não é apenas uma lista de tarefas é o choque de realidade que todos nós precisamos.",
          "vi": "Zechariah thẳng thắn vạch trần việc chỉ giữ hình thức trong ăn chay, một bài học đắt giá cho những ai hay chạy theo thủ tục.",
          "tl": "Ang pagpuna ni Zacarias sa mga taong pormalidad lang ang habol sa pag-aayuno ay tumatama sa ating ugaling mahilig sa checklist.",
          "th": "เศคาริยาห์ตอกหน้าคนที่ถือศีลอดแค่ให้ครบพิธี เป็นการเตือนสติที่เจ็บจี๊ดสำหรับใครที่ชอบทำอะไรแค่พอเป็นพิธี",
          "sw": "Zekaria kukemea watu wanaofunga kwa mazoea bila moyo ni ukumbusho mgumu kwa mtu yeyote anayepoteza maana katika ibada.",
          "it": "Zaccaria che critica chi tratta il digiuno come una semplice lista di cose da fare è una sveglia per chiunque metta i rituali davanti all'empatia."
        }
    },
    "2026-12-11": {
        date: "2026-12-11",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ゼカリヤ書 9:1-17",
          "en": "Zechariah 9:1-17",
          "ko": "Zechariah 9:1-17",
          "zho": "ゼカリヤ書 9:1-17",
          "es": "Zechariah 9:1-17",
          "pt": "Zechariah 9:1-17",
          "vi": "Zechariah 9:1-17",
          "tl": "Zechariah 9:1-17",
          "th": "Zechariah 9:1-17",
          "sw": "Zechariah 9:1-17",
          "it": "Zaccaria 9:1-17"
        },
        comment: {
          "ja": "王がロバに乗ってやってくるという平和の演出、派手な成功を求める世の中の価値観を完全にひっくり返していて痛快だ。",
          "en": "A king riding into town on a donkey is the ultimate anti-climax, perfectly flipping our obsession with flashy success on its head.",
          "ko": "화려한 군마 대신 나귀를 타고 나타난 왕의 모습은 성공에 목매는 우리 시대의 가치관을 제대로 비웃어준다.",
          "zho": "国王骑着驴入城这种反高潮的登场，彻底颠覆了现代人对成功一定要排场的执念。",
          "es": "Un rey entrando en un burro es la mayor bofetada a nuestra obsesión moderna por la grandeza y el espectáculo.",
          "pt": "Um rei entrando montado em um jumentinho é o maior tapa na nossa obsessão moderna por grandiosidade e sucesso chamativo.",
          "vi": "Vị vua cưỡi lừa vào thành là sự đối lập hoàn hảo với những hào nhoáng của thế giới, khiến chúng ta phải nhìn lại định nghĩa về sự thành công.",
          "tl": "Ang pagpasok ng hari na nakasakay sa asno ay isang sampal sa ating obsession sa magarbo at maingay na tagumpay.",
          "th": "การที่กษัตริย์เสด็จมาบนหลังลาเป็นการหักมุมที่ตบหน้าความบ้าคลั่งเรื่องความสำเร็จที่ดูหรูหราอลังการในยุคนี้ได้เจ็บแสบมาก",
          "sw": "Mfalme kuingia mjini akiwa amepanda punda ni kinyume kabisa na jinsi tunavyotafuta mafanikio ya kishindo leo hii.",
          "it": "Un re che arriva in città su un asinello ribalta completamente la nostra moderna ossessione per il successo appariscente."
        }
    },
    "2026-12-12": {
        date: "2026-12-12",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ゼカリヤ書 11:1-17",
          "en": "Zechariah 11:1-17",
          "ko": "Zechariah 11:1-17",
          "zho": "ゼカリヤ書 11:1-17",
          "es": "Zechariah 11:1-17",
          "pt": "Zechariah 11:1-17",
          "vi": "Zechariah 11:1-17",
          "tl": "Zechariah 11:1-17",
          "th": "Zechariah 11:1-17",
          "sw": "Zechariah 11:1-17",
          "it": "Zaccaria 11:1-17"
        },
        comment: {
          "ja": "羊飼いが杖をへし折って辞めていく姿、もはやブラック企業をバックレる現代人の断末魔にしか見えない。",
          "en": "The shepherd snapping his staff and quitting is the ancient equivalent of a middle manager finally rage-quitting their job.",
          "ko": "목자가 지팡이를 꺾고 떠나는 모습은 마치 한계에 다다른 직장인이 사표를 던지는 뒷모습과 다를 바 없다.",
          "zho": "牧人折斷杖而離去，這簡直就是現代人受不了職場壓力直接辭職不幹的寫照。",
          "es": "Ver al pastor romper su vara y renunciar es como ver a alguien finalmente renunciar a un trabajo tóxico después de aguantar demasiado.",
          "pt": "O pastor quebrando o cajado e indo embora é o equivalente bíblico de alguém finalmente explodindo e pedindo demissão de um emprego insuportável.",
          "vi": "Hình ảnh người chăn cừu bẻ gãy gậy và bỏ đi giống hệt tâm trạng của một nhân viên văn phòng cuối cùng cũng chịu hết nổi và nộp đơn nghỉ việc.",
          "tl": "Ang pastol na binali ang kanyang baston at umalis ay parang isang empleyadong bigla na lang nag-resign dahil hindi na kinaya ang stress.",
          "th": "ภาพผู้เลี้ยงแกะหักไม้เท้าแล้วเดินหนีไปดูเหมือนพนักงานที่ทนไม่ไหวแล้วตัดสินใจลาออกกะทันหันชัดๆ",
          "sw": "Mchungaji kuvunja fimbo yake na kuondoka ni kama mfanyakazi anayechoka na kazi yake kisha kuamua kuacha ghafla.",
          "it": "Il pastore che spezza il bastone e se ne va è l'equivalente antico di qualcuno che molla un lavoro tossico dopo aver sopportato troppo."
        }
    },
    "2026-12-13": {
        date: "2026-12-13",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "ゼカリヤ書 13:1-9",
          "en": "Zechariah 13:1-9",
          "ko": "Zechariah 13:1-9",
          "zho": "ゼカリヤ書 13:1-9",
          "es": "Zechariah 13:1-9",
          "pt": "Zechariah 13:1-9",
          "vi": "Zechariah 13:1-9",
          "tl": "Zechariah 13:1-9",
          "th": "Zechariah 13:1-9",
          "sw": "Zechariah 13:1-9",
          "it": "Zaccaria 13:1-9"
        },
        comment: {
          "ja": "預言者が傷跡の理由を誤魔化して友達の家でついた傷だと言い張るの、気まずい空気を読んだ嘘としてはあまりに切ない。",
          "en": "The prophet trying to pass off his scars as a casual injury from hanging out with friends is the most awkward lie ever.",
          "ko": "상처의 이유를 친구 집에서 생긴 것이라고 둘러대는 예언자의 변명은 세상에서 가장 어설픈 거짓말이다.",
          "zho": "先知為了遮掩身上的傷痕而謊稱是在朋友家弄傷的，這藉口拙劣得讓人心酸。",
          "es": "El profeta tratando de ocultar sus heridas diciendo que fue un accidente en casa de un amigo es la mentira más incómoda de la historia.",
          "pt": "O profeta tentando disfarçar as cicatrizes dizendo que se machucou na casa de amigos é aquela desculpa esfarrapada que todo mundo já deu.",
          "vi": "Việc vị tiên tri cố tình che giấu vết sẹo bằng cách đổ lỗi cho một tai nạn ở nhà bạn bè là một lời nói dối vụng về đến đáng thương.",
          "tl": "Ang pagdadahilan ng propeta na ang kanyang mga sugat ay nakuha lang niya sa bahay ng kanyang kaibigan ay ang pinaka-awkward na kasinungalingan.",
          "th": "การที่ผู้เผยพระวจนะพยายามอ้างว่ารอยแผลเกิดจากการไปบ้านเพื่อนเป็นคำโกหกที่ดูน่าอึดอัดที่สุดเลย",
          "sw": "Nabii kujaribu kuficha makovu yake kwa kusema alijeruhiwa nyumbani kwa rafiki yake ni uongo wa aibu sana.",
          "it": "Il profeta che cerca di giustificare le sue ferite dicendo che sono un incidente domestico è la scusa più imbarazzante di sempre."
        }
    },
    "2026-12-14": {
        date: "2026-12-14",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "マラキ書 1:1-5",
          "en": "Malachi 1:1-5",
          "ko": "Malachi 1:1-5",
          "zho": "マラキ書 1:1-5",
          "es": "Malachi 1:1-5",
          "pt": "Malachi 1:1-5",
          "vi": "Malachi 1:1-5",
          "tl": "Malachi 1:1-5",
          "th": "Malachi 1:1-5",
          "sw": "Malachi 1:1-5",
          "it": "Malachia 1:1-5"
        },
        comment: {
          "ja": "愛していると言われても「いつ愛してくれたの？」と即座に疑ってかかるマラキの民、まるで思春期の反抗期そのもの。",
          "en": "The people asking God 'How have you loved us?' is the ultimate teenage rebellion phase played out on a national scale.",
          "ko": "사랑한다는 말에 언제 그랬냐며 즉각 반문하는 이스라엘 백성은 영락없는 사춘기 자녀의 모습이다.",
          "zho": "面对上帝的爱却反问“你何时爱过我们”，这种自我怀疑的叛逆感简直就像处于青春期的孩子。",
          "es": "Preguntarle a Dios cómo nos ha amado cuando las cosas se ponen difíciles es la reacción más humana y adolescente de la historia.",
          "pt": "Perguntar a Deus como Ele nos amou, logo quando as coisas apertam, é a reação mais humana e adolescente da história.",
          "vi": "Hỏi Chúa rằng Ngài đã yêu mình thế nào ngay khi gặp khó khăn chính là phản ứng mang tính 'nổi loạn tuổi mới lớn' nhất của con người.",
          "tl": "Ang pagtatanong sa Diyos kung paano Niya tayo minahal sa gitna ng hirap ay ang pinaka-human at parang teenager na reaksyon kailanman.",
          "th": "การถามพระเจ้าว่ารักเราตอนไหนในวันที่ชีวิตดูแย่ คือปฏิกิริยาของมนุษย์วัยรุ่นที่ชัดเจนที่สุดเท่าที่เคยมีมา",
          "sw": "Kuuliza Mungu umetupenda lini wakati mambo yanapokuwa magumu ni jibu la kibinadamu na la kitoto zaidi kuwahi kutokea.",
          "it": "Chiedere a Dio come ci ha amati quando le cose vanno storte è la classica ribellione adolescenziale su scala nazionale."
        }
    },
    "2026-12-15": {
        date: "2026-12-15",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "マラキ書 1:6-14",
          "en": "Malachi 1:6-14",
          "ko": "Malachi 1:6-14",
          "zho": "マラキ書 1:6-14",
          "es": "Malachi 1:6-14",
          "pt": "Malachi 1:6-14",
          "vi": "Malachi 1:6-14",
          "tl": "Malachi 1:6-14",
          "th": "Malachi 1:6-14",
          "sw": "Malachi 1:6-14",
          "it": "Malachia 1:6-14"
        },
        comment: {
          "ja": "神様への捧げ物としてわざわざ傷物や病気の動物を選んで持ってくるあたり、人間はどこまでも自分に都合よくズルをする生き物だ。",
          "en": "Bringing God the leftovers and the sick animals as an offering is the ultimate proof that humans will cut corners even when worshipping.",
          "ko": "하나님께 드릴 제물로 병든 것을 골라 가져오는 모습은, 신앙생활에서도 어떻게든 요령을 피우려는 인간의 본성을 적나라하게 보여준다.",
          "zho": "把病残的牲畜拿来献祭，这证明了即便在敬拜时，人类也总想找捷径偷懒。",
          "es": "Llevarle a Dios lo que sobra o lo que está enfermo es la prueba de que, incluso al adorar, los humanos buscamos el camino más fácil.",
          "pt": "Levar a Deus o que sobra ou o que está doente é a prova de que, até na adoração, os humanos buscam o caminho mais fácil.",
          "vi": "Việc dâng lên Chúa những con vật ốm yếu chỉ chứng minh rằng con người luôn tìm cách làm tắt ngay cả trong việc thờ phượng.",
          "tl": "Ang paghahandog sa Diyos ng mga sirang hayop ay patunay na kahit sa pagsamba, hilig pa rin ng tao na maghanap ng shortcut.",
          "th": "การนำสัตว์ป่วยมาถวายพระเจ้าเป็นหลักฐานชั้นดีว่า แม้แต่ในการนมัสการ มนุษย์เราก็ยังพยายามหาทางลัดเสมอ",
          "sw": "Kumtolea Mungu wanyama wagonjwa ni ushahidi kuwa hata katika ibada, wanadamu hupenda kutafuta njia za mkato.",
          "it": "Portare a Dio gli animali malati e gli scarti dimostra che cerchiamo scorciatoie persino quando facciamo le cose per Lui."
        }
    },
    "2026-12-16": {
        date: "2026-12-16",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "マラキ書 2:1-9",
          "en": "Malachi 2:1-9",
          "ko": "Malachi 2:1-9",
          "zho": "マラキ書 2:1-9",
          "es": "Malachi 2:1-9",
          "pt": "Malachi 2:1-9",
          "vi": "Malachi 2:1-9",
          "tl": "Malachi 2:1-9",
          "th": "Malachi 2:1-9",
          "sw": "Malachi 2:1-9",
          "it": "Malachia 2:1-9"
        },
        comment: {
          "ja": "教える立場にありながら自分は守らないという祭司たちの矛盾、どこまでいっても人間は肩書きだけで偉くなれるわけではないという現実。",
          "en": "Priests failing to practice what they preach reminds us that a fancy title doesn't automatically fix a broken character.",
          "ko": "가르치는 위치에 있으면서 정작 본인은 지키지 않는 사제들의 모습은 직함이 인품을 보장해주지는 않는다는 뼈아픈 현실을 보여준다.",
          "zho": "身為導師卻知法犯法，祭司們的雙標提醒我們，頭銜永遠無法掩蓋品格的漏洞。",
          "es": "Ver a los sacerdotes predicar lo que no practican es un recordatorio de que el título no hace a la persona, sino sus acciones.",
          "pt": "Sacerdotes que pregam o que não vivem provam que um cargo importante não torna ninguém automaticamente melhor que os outros.",
          "vi": "Các tư tế rao giảng nhưng không thực hành là minh chứng rõ nhất cho việc chức vụ không thể che đậy những khiếm khuyết trong nhân cách.",
          "tl": "Ang mga paring nagtuturo pero hindi ginagawa ang kanilang sinasabi ay paalala na ang katungkulan ay hindi garantiya ng integridad.",
          "th": "ปุโรหิตที่สอนคนอื่นแต่ตัวเองกลับไม่ทำตาม เป็นเครื่องเตือนใจว่าตำแหน่งไม่ได้การันตีคุณภาพของคนเสมอไป",
          "sw": "Makuhani wanaohubiri wasichokifanya ni ukumbusho kwamba vyeo havitufanyi kuwa bora kuliko matendo yetu halisi.",
          "it": "Vedere i sacerdoti predicare bene e razzolare male ci ricorda che un bel titolo non sistema un carattere debole."
        }
    },
    "2026-12-17": {
        date: "2026-12-17",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "マラキ書 2:10-17",
          "en": "Malachi 2:10-17",
          "ko": "Malachi 2:10-17",
          "zho": "マラキ書 2:10-17",
          "es": "Malachi 2:10-17",
          "pt": "Malachi 2:10-17",
          "vi": "Malachi 2:10-17",
          "tl": "Malachi 2:10-17",
          "th": "Malachi 2:10-17",
          "sw": "Malachi 2:10-17",
          "it": "Malachia 2:10-17"
        },
        comment: {
          "ja": "家庭の誓いを軽んじておきながら祭壇で涙を流す人々、神様はそんなパフォーマンスよりも日々の誠実さを見ておられるはず。",
          "en": "People crying at the altar while breaking promises to their families is a classic case of trying to outsource integrity to a religious ritual.",
          "ko": "가정에서의 약속을 가볍게 여기면서 제단에서 눈물을 흘리는 모습은 종교적 의식으로 자신의 잘못을 덮으려는 인간의 얄팍한 꼼수를 보여준다.",
          "zho": "一邊在祭壇前流淚一邊背棄家庭誓約，這種試圖用儀式來掩蓋私德缺陷的做法，神一眼就看穿了。",
          "es": "Llorar ante el altar mientras se traicionan los compromisos familiares es el intento humano de compensar con rituales lo que nos falta en integridad.",
          "pt": "Chorar no altar enquanto se quebram promessas familiares é a tentativa clássica de usar a religião para encobrir falhas de caráter.",
          "vi": "Khóc lóc trước bàn thờ trong khi thất hứa với gia đình là minh chứng cho việc con người thường dùng nghi lễ để che đậy sự thiếu trung thực.",
          "tl": "Ang pagluha sa altar habang binabalewala ang pangako sa pamilya ay isang klasikong pagtatangka na gamitin ang relihiyon para pagtakpan ang maling ugali.",
          "th": "การร้องไห้ที่แท่นบูชาในขณะที่ผิดคำสัตย์กับครอบครัว คือการพยายามใช้พิธีกรรมมากลบเกลื่อนความบกพร่องในชีวิตจริง",
          "sw": "Kulia mbele ya madhabahu huku ukivunja ahadi za kifamilia ni jaribio la kibinadamu la kutumia dini kuficha dosari za tabia.",
          "it": "Piangere sull'altare mentre si tradiscono le promesse fatte in famiglia è il classico tentativo di delegare l'onestà a un rituale religioso."
        }
    },
    "2026-12-18": {
        date: "2026-12-18",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "マラキ書 3:1-5",
          "en": "Malachi 3:1-5",
          "ko": "Malachi 3:1-5",
          "zho": "マラキ書 3:1-5",
          "es": "Malachi 3:1-5",
          "pt": "Malachi 3:1-5",
          "vi": "Malachi 3:1-5",
          "tl": "Malachi 3:1-5",
          "th": "Malachi 3:1-5",
          "sw": "Malachi 3:1-5",
          "it": "Malachia 3:1-5"
        },
        comment: {
          "ja": "きれいに掃除された家で待機していても、肝心の主人が突然予告なしに帰宅して慌てふためく、あの抜き打ち検査のような緊張感。",
          "en": "The sudden arrival of the Lord feels less like a grand event and more like that heart-stopping moment when you realize you forgot to clean the house before guests arrive.",
          "ko": "손님 온다는 소식에 대청소하며 안절부절못하는 마음이 마치 주님을 기다리는 우리의 모습과 너무 닮았네요.",
          "zho": "就像是突然接到通知說貴賓要來訪，那種為了準備迎接而手忙腳亂的心情，其實就是我們等待主的心境。",
          "es": "La llegada del Señor se siente menos como un evento grandioso y más como ese momento de pánico cuando te das cuenta de que no limpiaste la casa antes de que lleguen las visitas.",
          "pt": "A chegada do Senhor parece menos um evento grandioso e mais aquele momento de pânico ao perceber que não arrumou a casa antes das visitas chegarem.",
          "vi": "Sự xuất hiện bất ngờ của Chúa giống như khoảnh khắc hoảng hốt khi bạn nhận ra mình chưa dọn dẹp nhà cửa trước khi khách đến chơi.",
          "tl": "Ang pagdating ng Panginoon ay parang yung biglang pagdating ng bisita kung kailan hindi pa malinis ang bahay, nakaka-tense na nakakahiya.",
          "th": "การเสด็จมาของพระเจ้าให้ความรู้สึกเหมือนตอนที่มีแขกมาบ้านโดยไม่บอกล่วงหน้า แล้วเราก็ต้องรีบเก็บกวาดบ้านให้ทันเวลา",
          "sw": "Kuja kwa Bwana kwa ghafla ni kama ule wakati unapoona wageni wanakuja nyumbani na hujasafisha, ile hali ya wasiwasi na haraka.",
          "it": "L'arrivo improvviso del Signore sembra meno un grande evento e più il panico totale quando ti accorgi di non aver pulito casa prima che arrivino gli ospiti."
        }
    },
    "2026-12-19": {
        date: "2026-12-19",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "マラキ書 3:6-18",
          "en": "Malachi 3:6-18",
          "ko": "Malachi 3:6-18",
          "zho": "マラキ書 3:6-18",
          "es": "Malachi 3:6-18",
          "pt": "Malachi 3:6-18",
          "vi": "Malachi 3:6-18",
          "tl": "Malachi 3:6-18",
          "th": "Malachi 3:6-18",
          "sw": "Malachi 3:6-18",
          "it": "Malachia 3:6-18"
        },
        comment: {
          "ja": "神様がどれだけ愛を伝えても「具体的に何をしたっていうの？」と返してしまうあたり、思春期の子供と親の会話を見ている気分になる。",
          "en": "God saying He loves them and the people replying with 'What have you done for me lately?' is the ultimate portrait of a thankless teenager talking to a parent.",
          "ko": "하나님은 사랑을 고백하시는데 우리는 늘 '그래서 우리한테 해준 게 뭔데요?'라고 되묻는, 사춘기 자녀와 부모의 대화 같은 답답함.",
          "zho": "神說祂愛我們，我們卻回嘴說「你到底為我們做了什麼？」，這對話簡直就是青春期孩子與父母的經典翻版。",
          "es": "Dios diciendo que nos ama y nosotros respondiendo '¿y qué has hecho por mí últimamente?' es el retrato perfecto de un adolescente malagradecido hablando con sus padres.",
          "pt": "Deus dizendo que nos ama e nós respondendo com 'o que você fez por mim ultimamente?' é o retrato perfeito de um adolescente ingrato falando com os pais.",
          "vi": "Chúa nói Ngài yêu thương chúng ta còn chúng ta lại hỏi 'Ngài đã làm được gì cho con?', đúng là kiểu đối thoại giữa cha mẹ và đứa con tuổi dậy thì.",
          "tl": "Ang pagsasabi ng Diyos na mahal Niya tayo at ang pagsagot natin ng 'ano naman ang ginawa Mo?' ay parang pagtatalo ng magulang at anak na teenager.",
          "th": "พระเจ้าบอกว่ารักเรา แต่เรากลับถามว่า 'แล้วพระองค์ทำอะไรให้บ้าง?' นี่มันบทสนทนาคลาสสิกของลูกวัยรุ่นกับพ่อแม่ชัดๆ",
          "sw": "Mungu kusema anatupenda nasi tunajibu 'umetufanyia nini?' ni kama mazungumzo ya kawaida kati ya mzazi na kijana aliyekosa shukrani.",
          "it": "Dio che dice di amarci e noi che rispondiamo con cosa ha fatto per noi di recente è il ritratto perfetto di un adolescente ingrato."
        }
    },
    "2026-12-20": {
        date: "2026-12-20",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "マラキ書 4:1-6",
          "en": "Malachi 4:1-6",
          "ko": "Malachi 4:1-6",
          "zho": "マラキ書 4:1-6",
          "es": "Malachi 4:1-6",
          "pt": "Malachi 4:1-6",
          "vi": "Malachi 4:1-6",
          "tl": "Malachi 4:1-6",
          "th": "Malachi 4:1-6",
          "sw": "Malachi 4:1-6",
          "it": "Malachia 4:1-6"
        },
        comment: {
          "ja": "最後の一行で親子の絆について触れるあたり、神様もやっぱり家庭内のゴタゴタを一番心配しているんだなと感じて少し安心する。",
          "en": "God finishing His grand final message by talking about family dynamics shows He really knows where the real stress happens.",
          "ko": "마지막에 부모와 자녀의 관계를 언급하시는 것을 보니 하나님도 결국 가정 내의 평화가 가장 중요하다는 걸 아시는 모양이다.",
          "zho": "上帝在最后还不忘叮嘱亲子关系，看来祂也深知家庭内部的鸡毛蒜皮才是人生最大的挑战。",
          "es": "Que Dios termine su mensaje final hablando de la familia demuestra que sabe perfectamente dónde están nuestros mayores problemas.",
          "pt": "Deus terminar Sua última mensagem falando sobre laços familiares mostra que Ele sabe bem onde mora a nossa maior dor de cabeça.",
          "vi": "Việc Chúa kết thúc thông điệp cuối cùng bằng cách nhắc đến tình cảm gia đình cho thấy Ngài hiểu rõ những rắc rối đời thường nhất.",
          "tl": "Ang pagtatapos ng mensahe ng Diyos sa pagbanggit ng pamilya ay patunay na alam Niya kung saan tayo nahihirapan araw-araw.",
          "th": "การที่พระเจ้าปิดท้ายคำพยากรณ์ด้วยเรื่องความสัมพันธ์ในครอบครัว แสดงว่าพระองค์รู้ดีว่าเรื่องในบ้านคือเรื่องที่น่าปวดหัวที่สุด",
          "sw": "Mungu kuhitimisha ujumbe wake kwa kuzungumzia familia inaonyesha Anajua vyema mahali ambapo changamoto zetu kubwa zipo.",
          "it": "Che Dio finisca il suo discorso parlando di dinamiche familiari dimostra che sa benissimo dove si concentrano i nostri stress quotidiani."
        }
    },
    "2026-12-21": {
        date: "2026-12-21",
        scripture: {
          "ja": "新約聖書",
          "en": "New Testament",
          "ko": "New Testament",
          "zho": "新約聖書",
          "es": "New Testament",
          "pt": "New Testament",
          "vi": "New Testament",
          "tl": "New Testament",
          "th": "New Testament",
          "sw": "New Testament",
          "it": "Nuovo Testamento"
        },
        chapter: {
          "ja": "ルカによる福音書 2:7",
          "en": "Luke 2:7",
          "ko": "Luke 2:7",
          "zho": "ルカによる福音書 2:7",
          "es": "Luke 2:7",
          "pt": "Luke 2:7",
          "vi": "Luke 2:7",
          "tl": "Luke 2:7",
          "th": "Luke 2:7",
          "sw": "Luke 2:7",
          "it": "Luca 2:7"
        },
        comment: {
          "ja": "救い主の誕生という歴史的イベントなのに宿屋に空きがないという現実、人生のハイライトは案外こんなドタバタの中で起きるものらしい。",
          "en": "The Savior being born in a manger because there was no room at the inn is a reminder that life's biggest moments usually happen during a total mess.",
          "ko": "구원자가 태어나는 역사적인 날에 방이 없어서 마구간으로 갔다는 건, 인생의 위대한 순간은 꼭 엉망진창인 상황에서 시작된다는 증거다.",
          "zho": "救主降生却连个房间都没有，这告诉我们人生的高光时刻往往发生在最手忙脚乱的时候。",
          "es": "Que el Salvador naciera en un pesebre por falta de espacio nos recuerda que los momentos más importantes de la vida suelen ocurrir en medio del caos.",
          "pt": "O Salvador nascer em uma manjedoura por falta de vaga na hospedaria prova que os momentos mais épicos da vida acontecem no meio da bagunça.",
          "vi": "Việc Đấng Cứu Thế phải sinh ra trong máng cỏ vì hết chỗ trọ cho thấy những khoảnh khắc vĩ đại nhất đời người thường đến vào lúc hỗn loạn nhất.",
          "tl": "Ang pagsilang ng Tagapagligtas sa isang sabsaban dahil walang matuluyan ay paalala na ang mahahalagang pangyayari sa buhay ay madalas nagaganap sa gitna ng kaguluhan.",
          "th": "การที่พระผู้ช่วยให้รอดต้องประสูติในรางหญ้าเพราะไม่มีห้องว่าง เตือนให้รู้ว่าช่วงเวลาที่สำคัญที่สุดในชีวิตมักเกิดขึ้นท่ามกลางความวุ่นวายเสมอ",
          "sw": "Mwokozi kuzaliwa kwenye hori kwa sababu ya kukosa nafasi ni ukumbusho kwamba matukio makubwa ya maisha hutokea katikati ya machafuko.",
          "it": "Il Salvatore nato in una stalla per mancanza di posti ci ricorda che i momenti più importanti della vita arrivano sempre nel bel mezzo del caos."
        }
    },
    "2026-12-22": {
        date: "2026-12-22",
        scripture: {
          "ja": "モルモン書",
          "en": "Book of Mormon",
          "ko": "Book of Mormon",
          "zho": "モルモン書",
          "es": "Book of Mormon",
          "pt": "Book of Mormon",
          "vi": "Book of Mormon",
          "tl": "Book of Mormon",
          "th": "Book of Mormon",
          "sw": "Book of Mormon",
          "it": "Libro di Mormon"
        },
        chapter: {
          "ja": "2 ニーファイ 11:4",
          "en": "2 Nephi 11:4",
          "ko": "2 Nephi 11:4",
          "zho": "2 ニーファイ 11:4",
          "es": "2 Nephi 11:4",
          "pt": "2 Nephi 11:4",
          "vi": "2 Nephi 11:4",
          "tl": "2 Nephi 11:4",
          "th": "2 Nephi 11:4",
          "sw": "2 Nephi 11:4",
          "it": "2 Nefi 11:4"
        },
        comment: {
          "ja": "すべてが誰かの予言の伏線だと言われると、人生のどんな些細な出来事も壮大な物語の一部に見えてくるから不思議だ。",
          "en": "When everything is framed as a foreshadowing, even your most mundane Tuesday starts feeling like a plot point in an epic.",
          "ko": "세상의 모든 일이 누군가의 예언을 위한 복선이라고 생각하면, 내 인생의 사소한 일들도 거대한 서사의 일부처럼 느껴진다.",
          "zho": "如果生活中的一切都是伏筆，那麼我們日常經歷的瑣碎小事，其實都是這場史詩巨作的一部分。",
          "es": "Cuando ves que todo es una señal de algo más grande, incluso tus lunes más aburridos parecen parte de una gran épica.",
          "pt": "Quando tudo é visto como um prenúncio, até os dias mais comuns começam a parecer parte de um grande roteiro épico.",
          "vi": "Khi mọi thứ đều được coi là điềm báo, ngay cả những chuyện vụn vặt nhất trong ngày cũng bỗng trở thành một phần của kịch bản sử thi.",
          "tl": "Kapag ang lahat ng bagay ay itinuturing na pahiwatig, maging ang mga simpleng pangyayari sa buhay ay nagmumukhang bahagi ng isang malaking kuwento.",
          "th": "เมื่อทุกอย่างถูกมองว่าเป็นคำบอกใบ้ แม้แต่เรื่องธรรมดาที่สุดในชีวิตก็ดูเหมือนจะเป็นส่วนหนึ่งของเรื่องราวที่ยิ่งใหญ่",
          "sw": "Unapochukulia kila kitu kama ishara ya mambo yajayo, hata mambo madogo ya kila siku yanaanza kuhisi kama sehemu ya hadithi kuu.",
          "it": "Quando tutto viene visto come una profezia, persino il martedì più noioso sembra improvvisamente parte di una grande storia epica."
        }
    },
    "2026-12-23": {
        date: "2026-12-23",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 16:4",
          "en": "Exodus 16:4",
          "ko": "Exodus 16:4",
          "zho": "出エジプト記 16:4",
          "es": "Exodus 16:4",
          "pt": "Exodus 16:4",
          "vi": "Exodus 16:4",
          "tl": "Exodus 16:4",
          "th": "Exodus 16:4",
          "sw": "Exodus 16:4",
          "it": "Esodo 16:4"
        },
        comment: {
          "ja": "毎日空から食料が降ってくるのに文句を言い出すイスラエル人、贅沢な悩みの極致すぎて逆に親近感が湧く。",
          "en": "The Israelites complaining about free food falling from the sky is the ultimate example of being spoiled and I honestly kind of get it.",
          "ko": "매일 하늘에서 음식이 공짜로 떨어지는데도 불평을 늘어놓는 이스라엘 백성들을 보니, 사람의 욕심은 끝이 없다는 게 새삼 실감 난다.",
          "zho": "以色列人對於從天而降的免費食物還能挑三揀四，這種極致的奢華煩惱讓人覺得又好氣又好笑。",
          "es": "Que los israelitas se quejaran de la comida que caía del cielo es el ejemplo perfecto de que nunca estamos satisfechos con nada.",
          "pt": "Os israelitas reclamando da comida que caía do céu é a prova definitiva de que o ser humano sempre encontra algo para se queixar.",
          "vi": "Việc dân Y-sơ-ra-ên than phiền về thức ăn từ trên trời rơi xuống chính là minh chứng rõ nhất cho việc con người chẳng bao giờ thấy đủ.",
          "tl": "Ang mga Israelita na nagrereklamo sa pagkaing nahuhulog mula sa langit ay patunay na kahit gaano kasagana, laging may hahanapin ang tao.",
          "th": "การที่ชาวอิสราเอลบ่นเรื่องอาหารที่ตกลงมาจากฟ้าเป็นเครื่องพิสูจน์ว่ามนุษย์เราไม่เคยพอใจกับอะไรเลยจริงๆ",
          "sw": "Waisraeli kulalamika kuhusu chakula kinachoshuka kutoka mbinguni ni uthibitisho kwamba binadamu hatutosheki na chochote.",
          "it": "Gli israeliti che si lamentano del cibo gratis che cade dal cielo sono la prova che siamo capaci di trovare da ridire su qualsiasi cosa."
        }
    },
    "2026-12-24": {
        date: "2026-12-24",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "出エジプト記 17:1-6",
          "en": "Exodus 17:1-6",
          "ko": "Exodus 17:1-6",
          "zho": "出エジプト記 17:1-6",
          "es": "Exodus 17:1-6",
          "pt": "Exodus 17:1-6",
          "vi": "Exodus 17:1-6",
          "tl": "Exodus 17:1-6",
          "th": "Exodus 17:1-6",
          "sw": "Exodus 17:1-6",
          "it": "Esodo 17:1-6"
        },
        comment: {
          "ja": "喉が渇いただけで即座に指導者を責め立てるイスラエルの民、極限状態での他責思考の速さがリアルすぎる。",
          "en": "The Israelites going from zero to blaming their leader the moment they felt thirsty is peak human frustration.",
          "ko": "목이 마르다는 이유로 즉시 리더를 비난하기 시작하는 이스라엘 백성들의 모습에서 극한 상황 속 인간의 본성을 본다.",
          "zho": "以色列民一感到口渴就立刻指責領袖，這種在極限狀態下推卸責任的速度實在太真實了。",
          "es": "Los israelitas culpando a su líder a la primera señal de sed es el ejemplo perfecto de cómo nos comportamos bajo presión.",
          "pt": "Os israelitas culpando o líder ao primeiro sinal de sede é o retrato perfeito de como agimos sob pressão.",
          "vi": "Người Israel đổ lỗi cho lãnh đạo ngay khi thấy khát là minh chứng rõ nhất cho bản năng đổ lỗi khi gặp khó khăn.",
          "tl": "Ang mga Israelita na agad sinisi ang kanilang lider sa sandaling mauhaw ay isang totoong repleksyon ng ugali nating magsisihan kapag nahihirapan.",
          "th": "ชาวอิสราเอลที่โทษผู้นำทันทีที่รู้สึกหิวน้ำ สะท้อนให้เห็นว่ามนุษย์เรามักจะโทษคนอื่นก่อนเสมอเมื่อเจอความลำบาก",
          "sw": "Waisraeli kumlaumu kiongozi wao mara tu walipohisi kiu ni dhihirisho la jinsi tunavyotafuta wa kulaumiwa tunapokuwa na shida.",
          "it": "Gli israeliti che danno la colpa al leader alla prima goccia di sete sono lo specchio perfetto di quanto siamo impazienti sotto pressione."
        }
    },
    "2026-12-25": {
        date: "2026-12-25",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "民数記 21:4-9",
          "en": "Numbers 21:4-9",
          "ko": "Numbers 21:4-9",
          "zho": "民数記 21:4-9",
          "es": "Numbers 21:4-9",
          "pt": "Numbers 21:4-9",
          "vi": "Numbers 21:4-9",
          "tl": "Numbers 21:4-9",
          "th": "Numbers 21:4-9",
          "sw": "Numbers 21:4-9",
          "it": "Numeri 21:4-9"
        },
        comment: {
          "ja": "不平不満を言った直後に災難に遭い慌てて助けを求める民、まさに喉元過ぎれば熱さを忘れる我々の縮図だ。",
          "en": "People complaining and then panicking for help the moment things go south is basically the classic human cycle of irony.",
          "ko": "불평하다가 막상 재앙이 닥치자 다급하게 살려달라고 외치는 모습은 우리가 평소에 겪는 전형적인 시행착오다.",
          "zho": "抱怨完之後一遇到災難就慌忙求救，這簡直是人類記吃不記打的經典寫照。",
          "es": "Quejarse y luego entrar en pánico pidiendo ayuda al primer problema es el ciclo irónico de nuestra propia naturaleza humana.",
          "pt": "Reclamar e depois entrar em pânico pedindo ajuda ao primeiro problema é o ciclo irônico da nossa própria natureza humana.",
          "vi": "Than vãn rồi cuống cuồng cầu cứu khi gặp họa là vòng lặp trớ trêu điển hình của con người.",
          "tl": "Ang magreklamo at pagkatapos ay magpanic at humingi ng tulong kapag nagkaproblema ay ang klasikong siklo ng ating pagkatao.",
          "th": "การบ่นพึมพำแล้วกลับมาตื่นตระหนกขอความช่วยเหลือทันทีที่เจอเรื่องร้าย คือวัฏจักรความย้อนแย้งสุดคลาสสิกของมนุษย์เรา",
          "sw": "Kulalamika kisha kutapatapa kuomba msaada mara tu matatizo yanapojitokeza ni mzunguko wa kawaida wa tabia za kibinadamu.",
          "it": "Lamentarsi continuamente e poi correre ai ripari quando le cose vanno male è il grande classico della nostra incoerenza quotidiana."
        }
    },
    "2026-12-26": {
        date: "2026-12-26",
        scripture: {
          "ja": "旧約聖書",
          "en": "Old Testament",
          "ko": "Old Testament",
          "zho": "旧約聖書",
          "es": "Old Testament",
          "pt": "Old Testament",
          "vi": "Old Testament",
          "tl": "Old Testament",
          "th": "Old Testament",
          "sw": "Old Testament",
          "it": "Vecchio Testamento"
        },
        chapter: {
          "ja": "イザヤ書 11:1-5",
          "en": "Isaiah 11:1-5",
          "ko": "Isaiah 11:1-5",
          "zho": "イザヤ書 11:1-5",
          "es": "Isaiah 11:1-5",
          "pt": "Isaiah 11:1-5",
          "vi": "Isaiah 11:1-5",
          "tl": "Isaiah 11:1-5",
          "th": "Isaiah 11:1-5",
          "sw": "Isaiah 11:1-5",
          "it": "Isaia 11:1-5"
        },
        comment: {
          "ja": "切り株から芽が出るという地味な再出発、派手な成功ばかり追い求める現代人にはかなり耳が痛い話だ。",
          "en": "The idea of a sprout coming from a dead stump is a humbling reminder for those of us obsessed with overnight success.",
          "ko": "그루터기에서 싹이 난다는 소박한 시작은 화려한 성공만 쫓는 현대인들에게 뼈를 때리는 메시지다.",
          "zho": "从枯木中萌发出新芽的低调重生，对于那些只追求速成成功的现代人来说真是莫大的讽刺。",
          "es": "La idea de que un brote nazca de un tronco seco es una lección de humildad para quienes solo buscamos el éxito inmediato.",
          "pt": "A ideia de um broto nascendo de um toco seco é um lembrete humilde para quem vive obcecado por sucesso instantâneo.",
          "vi": "Hình ảnh chồi non mọc lên từ gốc cây khô là lời nhắc nhở nhẹ nhàng cho những ai chỉ mải mê đuổi theo thành công hào nhoáng.",
          "tl": "Ang ideya ng isang usbong na lumalabas mula sa tuyot na puno ay isang mapagpakumbabang paalala para sa atin na laging nagmamadali sa tagumpay.",
          "th": "การที่หน่อไม้งอกออกมาจากตอไม้ที่ดูไร้ชีวิต เป็นเครื่องเตือนใจชั้นดีสำหรับคนที่มัวแต่ไขว่คว้าหาความสำเร็จแบบทางลัด",
          "sw": "Wazo la chipukizi kuchipuka kutoka kwenye shina lililokauka ni ukumbusho wa unyenyekevu kwa wale wanaotafuta mafanikio ya haraka.",
          "it": "Un germoglio che spunta da un tronco tagliato è una bella boccata d'aria per chi è ossessionato dal successo immediato."
        }
    },
    "2026-12-27": {
        date: "2026-12-27",
        scripture: {
          "ja": "新約聖書",
          "en": "New Testament",
          "ko": "New Testament",
          "zho": "新約聖書",
          "es": "New Testament",
          "pt": "New Testament",
          "vi": "New Testament",
          "tl": "New Testament",
          "th": "New Testament",
          "sw": "New Testament",
          "it": "Nuovo Testamento"
        },
        chapter: {
          "ja": "ルカによる福音書 2:13-14",
          "en": "Luke 2:13-14",
          "ko": "Luke 2:13-14",
          "zho": "ルカによる福音書 2:13-14",
          "es": "Luke 2:13-14",
          "pt": "Luke 2:13-14",
          "vi": "Luke 2:13-14",
          "tl": "Luke 2:13-14",
          "th": "Luke 2:13-14",
          "sw": "Luke 2:13-14",
          "it": "Luca 2:13-14"
        },
        comment: {
          "ja": "救い主の誕生を羊飼いに告げる軍勢、地上の騒がしいニュースよりも先に聞くべき本当の速報はこれのことだ。",
          "en": "A heavenly host appearing to shepherds instead of the elite is the ultimate reminder that the best news often skips the headlines.",
          "ko": "엘리트가 아닌 양치기들에게 가장 먼저 나타난 천군들, 진짜 중요한 소식은 항상 뉴스 헤드라인을 비껴가는 법이다.",
          "zho": "天军没有向权贵报喜，反而第一时间告诉了牧羊人，看来真正重要的头条新闻往往不在报纸上。",
          "es": "Que el anuncio del nacimiento fuera para unos pastores y no para la élite demuestra que las mejores noticias no salen en los titulares.",
          "pt": "O fato de a notícia ter sido dada aos pastores em vez da elite prova que as melhores novidades raramente estão nas manchetes.",
          "vi": "Việc các thiên sứ hiện ra với những người chăn cừu thay vì giới thượng lưu cho thấy tin tức quan trọng nhất thường không nằm trên mặt báo.",
          "tl": "Ang pagpapakita ng mga anghel sa mga pastol sa halip na sa mga makapangyarihan ay patunay na ang pinakamahalagang balita ay hindi laging nasa headline.",
          "th": "การที่เหล่าทูตสวรรค์เลือกบอกข่าวดีกับคนเลี้ยงแกะแทนที่จะเป็นคนใหญ่คนโต ยืนยันได้ว่าข่าวที่สำคัญที่สุดมักไม่อยู่ในพาดหัวข่าว",
          "sw": "Jeshi la mbinguni kuwatokea wachungaji badala ya watu mashuhuri ni ushahidi kwamba habari muhimu zaidi mara nyingi hazipo kwenye vichwa vya habari.",
          "it": "Dare la notizia della nascita ai pastori e non ai potenti dimostra che le cose che contano davvero saltano sempre i titoli dei giornali."
        }
    }
};

/**
 * Helper function to retrieve daily comment or fallback
 */
export function getAiDailyComment(dateStr: string, lang: string = 'ja') {
    const item = AI_DAILY_COMMENTS[dateStr];
    if (item) {
        return {
            scripture: item.scripture[lang] || item.scripture['en'] || item.scripture['ja'],
            chapter: item.chapter[lang] || item.chapter['en'] || item.chapter['ja'],
            comment: item.comment[lang] || item.comment['en'] || item.comment['ja']
        };
    }

    const fallbacks: Record<string, { scripture: string; chapter: string; comment: string }> = {
        ja: {
            scripture: "聖典学習",
            chapter: "今日の聖句",
            comment: "毎日少しずつ聖典を読み進めるその一歩が、何より尊い習慣です。"
        },
        en: {
            scripture: "Scripture Study",
            chapter: "Today's Scripture",
            comment: "Taking one small step each day to open the scriptures is a habit worth keeping."
        },
        it: {
            scripture: "Studio delle Scritture",
            chapter: "Scrittura di oggi",
            comment: "Fare anche solo un piccolo passo ogni giorno per aprire le Scritture è un'abitudine preziosa da custodire."
        }
    };

    return fallbacks[lang] || fallbacks['en'];
}
